...
\begin{itemize}
    ...
    \item {
        Introducing a feature mining method to help the model to pay more attention to specific features in the data, which has much information about the emotion of the sample, such as hashtags and emojis.
    }
\end{itemize}
...
In this section, we first discuss emotion detection studies that have been previously investigated, especially in Persian texts. We then provide a brief review of approaches for handling imbalanced datasets. Finally, we will look at the feature mining approaches in English and Persian texts.
...
\subsubsection{Feature Mining}
\label{subsubsec:featuremining}

We mined many statistical and lexical-based characteristics previously used in addition to utilizing a different loss function\cite{hernandez2015applying, yaghoobian2021sarcasm, nandwani2021review}. Using the splitting token "\textless/s\textgreater\textless/s\textgreater", all text features are simply added to the pre-processed tweets. Figure \ref{fig:featureextraction} shows how this process works.
...
\begin{itemize}
    ...
    \item {Parts of Speech(POS): Additionally, POS data is utilized as a crucial component.}
...
We decreased our dataset imbalance using data augmentation, undersampling, and class weights. Table \ref{table:finaldataset} shows information about the final dataset to the model. In addition to these methods, feature mining introduced in Section \ref{subsubsec:featuremining} and the F1-Cross-Entropy loss function mentioned in Section \ref{subsubsec:lossfunction} were also helpful.
...
Since we were facing with imbalanced data, we used some approaches to solve this issue which is introduced in Sections \ref{subsec:dataaugmentation}, \ref{subsubsec:datapreprocessing}, \ref{subsubsec:lossfunction}, \ref{subsubsec:featuremining}, and \ref{subsubsec:undersampling}. The combination of all of these methods resulted in significant improvement in the result of our method.
...
\begin{table}[]
    ...
    \end{tabular}
    \caption{\label{table:oursresults}The results of ParsBERT on the EmoPars using F1-Cross-Entropy, Data Augmentation, Undersampling, Class Weights, and Feature Mining.} 
\end{table}

Table \ref{table:oursresults} shows our approach has approximately double the F1-score in each emotion. We believe this result is due to using a small amount of each method for handling the imbalanced data. Using just data augmentation can make redundant samples, and under-sampling can decrease the number of samples. Nevertheless, using both of them and adding more attention by feature mining can help us more. Moreover, the F1-Cross-Entropy tries to increase the F1-score instead of Accuracy, which will help us to reach higher precision and recall.
...
Figure \ref{fig:ourvsnon} shows the influence of our approach on training. Feature mining has a significant impact on the initial loss of the model. Then, the loss function helps the model. In addition, since the model sees more samples in the positive class, it can reach better results.
