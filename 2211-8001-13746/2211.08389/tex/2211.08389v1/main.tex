...
% \address{NuHAG, Faculty of Mathematics, University of Vienna, Oskar-Morgenstern-Platz 1, 1090 Vienna, Austria}
\author[H.\ F\"uhr]{Hartmut Führ %\orcidlink{0000-0001-5718-6449}
\thankssymb{1}}
\email{fuehr@mathga.rwth-aachen.de}

\author[I.\ Shafkulovska]{Irina Shafkulovska %\orcidlink{0000-0003-1675-3122}
\thankssymb{2}\textsuperscript{,}\thankssymb{3}}
...
Recall that the metaplectic group is the universal cover of the symplectic group, i.e., there exists a surjective Lie group homomorphism
...
\begin{theorem*}%\marginnote{\is{Change here}}
...
\end{equation}
pointwise defined on $L^1(\R^d)$ and continuously extended by duality on $\mathcal{S}'(\R^{d})$. % (or any dual space of a subspace $\mathcal{S_C}\subseteq L^1(\R^d)$). 
...
%The central quadratic time-frequency representation in this paper is the cross-ambiguity function.
...
\end{split}
\end{equation}
% of functions $f,g\in L^2(\R^d)$ 
% \begin{align*}
%     \AF(f,g)(x,\omega) &=\int_{\R^d} f\left(t+\frac{x}{2}\right) \overline{g\left(t-\frac{x}{2}\right)} e^{-2\pi i \omega\cdot t}\, dt = \left\langle f, T_{x/2}M_\omega T_{x/2} g\right\rangle.
% \end{align*}
The latter equality allows to extend the definition of the cross-ambiguity function for any $f\in \mathcal{S}'(\R^d)$, $g\in  \mathcal{S}(\R^d)$. It corresponds to the symmetric time-frequency shift 
\begin{equation}
...
%We will always assume that the weight $v$ controlling the moderateness of the weight $m$ to be \textit{submultiplicative}, i.e. it fulfills 
%\[
%   v(x + y)\leq C\, v(x)\cdot v(y),\qquad x,y,\in\R^{2d}
%\] These conditions ensure the invariance of the space under shifts, with explicit control over the norms of the shift operators. We further assume all weights in this paper to be locally bounded. Submultiplicativity is truly necessary, as a reversed version holds: $L^{p,p}_m(\R^{2d})$ is translation invariant with respect to all translation operators $T_z,z\in\R^{2d}$ if and only if $m$ is $v$-moderate with respect a submultiplicative $v$ \cite[Prop. 11.1.2.]{Groechenig2001}).
...
\begin{proposition}%[Feichtinger, "83 \cite%[Thm. 6.1 (D)]
%{Feichtinger1983}]
...
\end{enumerate}
\end{proposition}
%The polynomial boundedness condition mentioned in the proposition puts the weight $m$ into focus. A submultiplicative weight grows at most exponentially \cite{Groechenig2007}, so the Schwartz functions might not be suitable for weight functions which grow faster than any polynomial. In particular, (dense) inclusions as \eqref{eq:inclusions} do not hold, which many proofs with modulation spaces are based on.  In this case, we replace the pair $\mathcal{S}(\R^d), \mathcal{S}'(\R^d)$ with $\mathcal{S_C}(\R^d), \mathcal{S_C}'(\R^d)$,
%\begin{equation}
%    \mathcal{S_C}(\R^d)\coloneqq \left \lbrace f\in L^2(\R^d) : f= \AF(\dotp, g_0)^* F,\, F\in L^{\infty}(\R^{2d}),\ \supp{(F)} \text{ compact} \right\rbrace
%\end{equation}
%where $g_0$ denotes the Gaussian. In that case, \thref{prop:inclusions}, in particular equation \eqref{eq:inclusions}, holds with $\mathcal{S_C}(\R^d)$ instead of $\mathcal{S}(\R^d)$. More about the generalisation can be found in \cite[Sec. 11.4]{Groechenig2001} and \cite{Groechenig2007}. 
...
\begin{equation*}
    ...
    \end{pmatrix},
\end{equation*}
with $I_d$ denoting the $d$-dimensional identity matrix, %in $\R^{d\times d}$, 
allows to express this bilinear form as $[\lambda,\lambda'] = \lambda \cdot \J \lambda'$. Now the symplectic group $\Sp\leq \mathrm{SL}(2d,\R)$ consists of the matrices $S\in \mathrm{GL}(2d,\R)$ preserving the commutator relation, i.e., they satisfy  %\marginnote{\footnotesize \is{$[,]_{-}$ is apparently used in quantum field theory to differentiate between commutator and anticommutator.}}
...
%Due to the minus sign, we cannot call them permutation matrices, but they  almost act as such, in a sense that (up to the sign) they permute the $i$-th with the $i+d$-th entry of a vector and leave the rest unchanged.
%    To visualize this, \marginnote{\is{does this help or is it further confusing?}}
%\begin{equation}
%    \Pi_{i}=\begin{pmatrix}
%    I_d-e_i\cdot e_i^T & e_i\cdot e_i^T\\
%    -e_i\cdot e_i^T
%    & I_d-e_i\cdot e_i^T
%    \end{pmatrix}.
%\end{equation}
...
\begin{proposition}\thlabel{prop:closed_operator} %\marginnote{\is{The old version of the proposition is in the other file}}
...
\end{equation}
With this notation, the interplay between the symplectic action on $L^{p,q}(\R^{2d})$ and the metaplectic action on $\MSpace{p,q}{}$ can be best illustrated by the commutative diagram shown in Figure \ref{fig:comm_diag}. %\marginnote{\is{the label should be on the caption, not on the figure, that resolved the problem}}
...
We now state the main result of this section. %\marginnote{\is{It would be nice if the result is on one page.}}
...
\end{theorem}
%\begin{proof}
%The determinant of a symplectic matrix is 1, which can be read from the decomposition in  \thref{prop:decomposition}. It holds
%\begin{equation}
%    \norm{\ST f}_{p} = \norm{f}_{p}
%\end{equation} 
%\begin{enumerate}[(i)]
%\item due to the transformation formula (TF), if $p<\infty$,
%\item due to $S(\R^{2d}) = \R^{2d}$, if $p=\infty$.
%\end{enumerate}
%\end{proof}
As a consequence of this observation and the already established implication (a) $\Rightarrow$ (c), we get the boundedness of $\widehat{S}: \MSpace{p}{} \to \MSpace{p}{}$, for any symplectic matrix $S$. This was noted in \cite{CauliEtAl2019} but was probably known prior to that.%\marginnote{\is{ I'd also point out that it is a standard result}}
...
The following, somewhat extreme example, based on interchanging the variables $x,\omega$, gives a hint why the intuition is correct. Pick radially symmetric functions $f\in L^p(\R^{d})\setminus L^q(\R^{d})$ and $g\in L^q(\R^{d})\setminus L^p(\R^{d})$. The tensor product $(x,\omega)\mapsto f\otimes g(x,\omega)=f(x)g(\omega)$ is in $L^{p,q}(\R^{2d})$, but $g\otimes f = \w{\J}f\otimes g \notin L^{p,q}(\R^{2d})$, so intuitively, mixing of the variables should be held under control as much as possible. The example shows that $\w{\J}:L^{p,q}(\R^{2d})\to L^{p,q}(\R^{2d})$ is not well defined. %Since 
%$\AF(\MSpace{p,q}{},g )\subsetneq L^{p,q}(\R^{2d})$ is only a closed subspace of $L^{p,q}(\R^{2d})$, the equivalence between (a) and (c) in \thref{thm:classification} is not trivial.
...
%For $S=D_L$ holds
%\[C_S=\abs{\det \ L}^{\frac{1}{p}}\ \abs{\det \ L^{-T}}^{\frac{1}{q}} = \abs{\det \ L}^{\frac{1}{p}}\ \abs{\det \ L}^{-\frac{1}{q}} = \abs{\det \ L}^{\frac{1}{p}-\frac{1}{q}}.\]

%For $S=U_P$ holds $C_S=1$.

%The symplectic matrices form a group, hence a matrix is symplectic if and only if its inverse is symplectic. Therefore, the decomposition in \thref{prop:decomposition}
%\begin{equation}
%    S=\prod\limits_{i\in K}\Pi_i\ V_Q D_L U_P
%\end{equation}
%A matrix $S$ is not an upper block triangular matrix if and only if $\SI$ is not an upper block triangular matrix. This means that both have a non-trivial factor $\prod\limits_{i\in K}\Pi_i\ V_Q$ in the decomposition.
...
%\marginnote{\is{I'm not sure if we should put the specific clean-up in an appendix.}}
...
%The theorems \ref{thm:p=q_unweighted} and \ref{thm:pq_bounded_unweighted} \marginnote{\is{\footnotesize I don't understand why I was looking at different window functions. The Gaussian suffices.}}
%handle the cases when $\ST$ is bounded. It remains to be shown that $\ST$ is unbounded on $\AF(\MSpace{p,q}{},\,g)$ if $p\neq q$ and $S$ is not an upper block triangular matrix. The window function can be any Schwartz function, but we begin with the special case of the Gaussian $g(x)=e^{-\pi x\cdot  x}$.

%\underline{ We can make the adjustment for an arbitrary Schwartz function}.
...
A & B\\C & D\end{pmatrix}
\end{equation}
and compute $f \circ \SI$ as %\marginnote{\is{Should this be a lemma?}}
\begin{equation}
...
%\marginnote{\is{I think the proof below is unnecessary. Also, I don't really like $R_m$, but we are running out of letters. Suggestions for $R_m$ are encouraged.}}
%\textcolor{gray}{A standard estimate results in
%\begin{equation}
%\norm{\frac{m}{m\circ \SI} f}_{p,q}\leq R_m\norm{f}_{p,q}.
%\end{equation}
%Assume now $C_m=\infty$. Then there are disjoint measurable sets %$(A_n)_{n\in\N}$ with a positive measure and
%\[ \lambda(A_n)\leq 1,\quad\quad \frac{m}{m\circ \SI}>n\quad
%\text{ on }A_n\; \text{for all }n\in\N.\]
%According to Cavallieri's principle, the sets
%\[
%A_n(\omega)\coloneqq\{x\in\R^{d}\mid (x,\omega)\in A_n\},\qquad \omega\in\R^{d}
%\]
%are measurable a. e. in $\omega\in\R^{d}$, the function \[\omega\mapsto\lambda(A_n(\omega))\] is measurable and it holds
%\[\int_{\R^{d}}\lambda(A_n(\omega))\ d\omega=\lambda(A_n).\]
%We set
%\[a_n(x,\omega)\coloneqq a_n(0,\omega)=\lambda(A_n(\omega))^{1-\frac{1}{p}}.
%\]
%For all $n\in\N$ is $a_n$ a. e. well-defined and measurable. It holds
%\begin{equation}\begin{split}
%\norm{a_n\chi_{A_n}}_{p,q} &= \norm{\omega \mapsto a_n(\omega)\dotp\norm{\chi_{a_n(\omega)}}_p}_{q}\\
% &= \norm{\omega \mapsto a_n(\omega)\dotp \lambda(\omega)^\frac{1}{p}}_{q} \\
%&= \norm{\omega\mapsto\lambda(A_n(\omega))}_q = \lambda(A_n)^\frac{1}{q}
% \end{split}\end{equation}
%On the other hand,
%\begin{equation}\begin{split}
%\norm{M_m\ a_n\chi_{A_n}}_{p,q} &= \norm{\omega \mapsto a_n(\omega)\dotp\norm{\frac{m}{m\circ\SI}\chi_{a_n(\omega)}}_p}_{q}\\
%&\geq \norm{\omega \mapsto a_n(\omega)\dotp\norm{n\chi_{a_n(\omega)}}_p}_{q}\\
% &= n\norm{\omega \mapsto a_n(\omega)\dotp \lambda(\omega)^\frac{1}{p}}_{q} \\
%&=n \norm{\omega\mapsto\lambda(A_n(\omega))}_q = n\lambda(A_n)^\frac{1}{q}.
% \end{split}\end{equation}
%This implies that $M_m$ is unbounded.}
...
%\is{I found a paper by M. D\"oerfler and B. Torresani about STFT Multipliers \cite{DoerflerTorresani2010}. Do we mention it in the prose or just add it to the reference list?}
...
%We observe 
%\begin{equation}
%    \essinf\ \frac{m}{m\circ\SI} = \essinf\ \frac{m\circ S}{m} = \essinf\ \frac{1}{\frac{m}{m \circ S}} = \frac{1}{\esssup \frac{m}{m \circ S}}.
%\end{equation} 
%This means, as long as we are looking at a subclass of the symplectic matrices which is closed under inversions, it suffices to globally check for 
%\begin{equation}
%    \esssup \frac{m}{m \circ S}<\infty\quad\text{ or }\quad\esssup \frac{m\circ S}{m}<\infty.
%\end{equation}
\begin{example}
We begin with
\begin{equation}
...
The second unresolved question concerns the mapping properties of metaplectic operators $\widehat{S}:\MSpace{p,q}{m} \to \MSpace{p,q}{m}$ in the case where $\widehat{S}$ is unbounded on the unweighted modulation space $\MSpace{p,q}{}$, and the weights $m$ and $m \circ S^{-1}$ are not equivalent. We presented examples that this case can occur for fairly natural choices of weights. %In this case, it is conceivable that some compensation effect could ultimately render $\widehat{S}:\MSpace{p,q}{m} \to \MSpace{p,q}{m}$ a bounded operator, similar to the observation (made above) that composition of two unbounded operators may well be bounded. 
...
%Note also that in this setting the dichotomy that we observed elsewhere, namely that the operator in question is either unbounded and not everywhere defined, or it is an automorphism of the space in question, may also not hold. 

\begin{appendix}
\section{Calculation for Equation \texorpdfstring{\eqref{eq:calculation_A_Gauss}}{(3.1)}}\label{app:calculation_A_Gauss}
To prove the unboundedness of $\mathcal{D}_S$ when $S$ is not an upper block triangular matrix, we need the following technical lemma. % \marginnote{\is{should it be deleted?}}
