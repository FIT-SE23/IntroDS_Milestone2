...
$\varphi\colon V \to T_{\delta}^*S$ a symplectomorphism
for some $\delta>0$.
Let $0 < \epsilon < \delta$ small enough, such that $c(U) \subset U$ 
for $U:= \varphi ^{-1}(T_{\epsilon}^*S)$.
Consider 
$\Phi$
as a map from
$\colon V_0 \times V_0 \times \mathbb{C}$
to
$V \times V \times \mathbb{C}$.
...
\end{align*}
Let $\phi_t \colon T^*S \to T^*S$ be the Hamiltonian isotopy from Proposition \ref{INVlinear:prop:main}. Consider the Hamiltonian isotopy
\begin{align*}
\Psi_t\colon T_\delta^*S \times T_\delta^*S \times T^*\mathbb{R} &\to T_\delta^*S \times T_\delta^*S \times T^*\mathbb{R} \\
(\xi_1,\xi_2, p) &\mapsto (\phi_t(\xi_1), -\phi_t(-\xi_2)p).
\end{align*}
We consider surgery in $T_{\epsilon}^*S$, so that the handle
$\hat{H}_{\nu}$ is contained in $T_{\epsilon}^*S$.
We claim that
\begin{itemize}
\item $\Psi_1(\hat{H}_{\nu}) = \Phi(\hat{H}_{\nu})$,
...
\begin{itemize}
\item 
Let $\xi \in T^*S$ and $(q,p) \in T^*\R$ such that $$\sqrt{\vert \vert \xi \vert \vert ^2 + p^2}< \epsilon.$$
We introduce the following abbreviations:
\[
    s(\vert \vert \xi \vert \vert, \vert \vert p \vert \vert ) = \nu\left(\sqrt{\vert \vert \xi \vert \vert ^2 + p^2}\right) \frac{\vert \vert \xi \vert \vert}{\sqrt{\vert \vert \xi \vert \vert ^2 + p^2}}
\]
and
\[
    r(\vert \vert \xi \vert \vert, \vert \vert p \vert \vert ) = \nu\left(\sqrt{\vert \vert \xi \vert \vert ^2 + p^2}\right) \frac{\vert \vert p \vert \vert}{\sqrt{\vert \vert \xi \vert \vert ^2 + p^2}}.
\]
Elements of $\hat{H}_{\nu}$ are of the form 
$$\alpha :=(\xi, \psi_{s(\vert \vert \xi \vert \vert, \vert \vert p \vert \vert)}^\sigma (-\xi), (r(\vert \vert \xi \vert \vert, \vert \vert p \vert \vert) + q,p)).$$
Therefore, elements of $\Psi_1(\hat{H}_{\nu})$ are of the form
$$\Psi_1(\alpha) = (cc_0^* (\xi), -cc_0^*(-\psi_{s(\vert\vert \xi\vert \vert)}^\sigma (- \xi)), (r(\vert \vert \xi \vert \vert, \vert \vert p \vert \vert) + q,p)).$$
Put
...
Thus
\[
    \Psi_1(\alpha) = (c(-\psi_{s(\vert \vert \zeta \vert \vert)}^\sigma (-\zeta)), -c(\zeta), (r(\vert \vert \zeta\vert \vert, \vert \vert p \vert \vert ) + q, p))
\]
which are precisely the elements of $\Phi(\hat{H}_{\nu})$. 
\item ${\Psi_t}\vert_{S\times S \times \mathbb{R}} = \mathrm{id}$: 
\begin{align*}
\Psi_t(x,y,p) &= (c\sigma_t(x), -c\sigma_t(-y),p) \\
&=(\phi_t(x), -\Phi_t(-y),p) \\
&= (x,y,p)
\end{align*}
because $\phi_t \vert _ S = \mathrm{id}$.
\item $\Psi_t(\xi, -\xi,p) = (c\sigma_t(\xi),-c\sigma_t(\xi),p)$ and thus 
