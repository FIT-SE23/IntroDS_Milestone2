...
\paragraph{Simplices and meshes} An {\em $n$-simplex} ($n = 2$ for a triangle or $3$ for a tetrahedron) is the closed convex hull of $n+1$ affinely independent points in $\R^3$ called its vertices. A {\em face} of a simplex $S$ is a $(n-1)$-simplex made from $n$ vertices of $S$. 

A $n$-dimensional {\em mesh} $\mathcal{M}$ is a finite set of $n$-simplices such that if $K,K' \in \mathcal{M}$, then the intersection $K \cap K'$ is either empty, or equal to a common subsimplex (i.e. a vertex, edge, or face) of both $K$ and $K'$. The set of faces of a mesh $\mathcal{M}$, denoted by $\mathcal{F}(\mathcal{M})$, is the set of faces of the simplices of $\mathcal{M}$. The boundary of an $n$-dimensional mesh $\mathcal{M}$ is defined as the subset of $\mathcal{F}(\mathcal{M})$ whose elements are the face of exactly one $n$-simplex in  $\mathcal{M}$. The {\em geometry} of $\mathcal{M}$, denoted by $\abs{\mathcal{M}}$, is the union of all of its elements, i.e. 
...
We do not require $\mathcal{M}_{\Gamma}$ to be regular, however, we impose that $\Gamma$ be a {\em multi-screen}, in the sense of Claeys and Hiptmair \cite{claeys2013integral}. For instance, $\mathcal{M}_\Gamma$ may be be as in \Cref{junction2}.
\begin{figure}[H]
	\centering
...
\paragraph{Finite-dimensional subspaces} If $\mathcal{M}_{\Omega,h}$ and $\mathcal{M}_{\Gamma,h}$ are meshes of $\Omega$ and $\Gamma$ (possibly different from $\mathcal{M}_\Omega$ and $\mathcal{M}_\Gamma$), we say that the pair $(\mathcal{M}_{\Omega,h},\mathcal{M}_{\Gamma,h})$ is {\em admissible} if $\mathcal{M}_{\Gamma,h} \subset \mathcal{F}(\mathcal{M}_{\Omega,h}) \setminus \partial \mathcal{M}_{\Omega,h}$. Given an admissible pair $(\mathcal{M}_{\Omega,h},\mathcal{M}_{\Gamma,h})$, let $V_h(\Omega \setminus \Gamma)$ be the finite-dimensional subspace of $H^1(\Omega \setminus \Gamma)$ consisting of piecewise linear functions on $\mathcal{M}_\Omega$, that is,
\begin{equation*}
...
\begin{theorem}
	\label{thm}
 	For each $\gamma_0 > 0$, there exists a constant $C(\gamma_0) > 0$ such that the following holds. Suppose that $(\mathcal{M}_{\Omega,h},\mathcal{M}_{\Gamma,h})$ is an admissible pair of meshes of $\Omega$ and $\Gamma$, and assume that $\mathcal{M}_{\Omega,h}$ and $\mathcal{M}_{\Gamma,h}$ are $\gamma_0$-shape-regular. Then there exists a linear operator $\Pi_h: \mathbb{H}^{1/2}(\Gamma) \to \mathbb{V}_h(\Gamma)$ such that
	\begin{itemize}
		\item[(i)] $\norm{\Pi_h u}_{\mathbb{H}^{1/2}} \leq C(\gamma_0) \norm{u}_{\mathbb{H}^{1/2}}$ for all $u \in \mathbb{H}^{1/2}(\Gamma)$,
		\item[(ii)] $\Pi_h u_h = u_h$ for all $u_h \in \mathbb{V}_h(\Gamma)$, 
...
Theorem \ref{thm} has the following important consequences. First, it gives a uniform bound for the stability of the {\em discrete harmonic lifting}
...
\begin{corollary}[Uniform bound for the discrete harmonic lifting]
	\label{discHarmLift}
...
\begin{corollary}[Discrete quotient norm]
	\label{discJumpNorm}
...
The proofs of \Cref{discHarmLift,discJumpNorm} are deferred to the appendix. The rest of this work is dedicated to prove \Cref{thm}. From now on, we fix a constant $\gamma_0 > 0$ and a $\gamma_0$-regular pair of admissible meshes $\mathcal{M}_{\Gamma,h}$ and $\mathcal{M}_{\Omega,h}$. The dependence in $h$ will be omitted from some of the notation. We shall also use the letter $C$ to denote a generic positive constants which only depends on $\Gamma$, $\Omega$ and $\gamma_0$, but not on the size of the elements of $\mathcal{M}_{\Gamma,h}$ and $\mathcal{M}_{\Omega,h}$. 


\section{Basis of the discrete multi-trace space}
\label{sec:BasisMulti}
In order to construct the operator $\Pi_h$ of \Cref{thm}, we first construct a basis of $\mathbb{V}_h(\Gamma)$. The main ideas are from \cite{averseng2022fractured}. Let us denote by $\{\mathbf{x}_1\,,\ldots\,,\mathbf{x}_N\}$ be the vertices of $\mathcal{M}_{\Omega,h}$ with the common vertices between $\mathcal{M}_{\Omega,h}$ and $\mathcal{M}_{\Gamma,h}$ given by $\mathbf{x}_1,\ldots,\mathbf{x}_M$. Let $V_h(\Omega)$ be the space of functions in $V_h(\Omega \setminus \Gamma)$ which are continuous. Notice that
...
\[\phi_i(\mathbf{x}_{i'}) = \delta_{i,i'}\,, \quad 1 \leq i,i' \leq N\,.\] 
For each $i \in \{1,\ldots,N\}$, the {\em star} of $\mathbf{x}_i$, denoted by $\textup{st}(\mathbf{x}_i,\mathcal{M}_{\Omega,h})$, is the set of tetrahedrons $K \in \mathcal{M}_{\Omega,h}$ containing $\mathbf{x}_i$ as a vertex. We define a graph $\mathcal{G}(\mathbf{x}_i)$ with 
\begin{itemize}
	\item {\bf Nodes:} The elements of $\textup{st}(\mathbf{x}_i,\mathcal{M}_{\Omega,h})$
	\item {\bf Edges:} The pairs $\{K,K'\}$ such that $K$ and $K'$ share a face not contained in $\mathcal{M}_\Gamma$. 
\end{itemize}
Let $\gamma_{i,1}, \ldots, \gamma_{i,q_i}$ be the connected components of $\mathcal{G}(\mathbf{x}_i)$, and let 
...
\[\phi_{i,j}(\mathbf{x}) \isdef \begin{cases}\phi_i( \vec x) & \textup{for } \vec x \in \textup{int}(\abs{\gamma_{i,j}}) \\
	0 & \textup{otherwise}.	
\end{cases}\]
...
\end{lemma}
The proof is given in the appendix. For $u_h \in V_h(\Omega \setminus \Gamma)$, we will denote by $u_h(\gamma_{i,j})$ the coefficient of $u_h$ on the split basis function $\phi_{i,j}$. 
...
and \Cref{basisVh}, a simple algebraic reasoning shows that $\{\widetilde{\phi}_{i,j}\}_{(i,j) \in \widetilde{H}}$ is a basis of $Y_h(\Omega)$. 
Define
...
\begin{lemma}
	...
	Moreover, $\{\eta_i\}_{1 \leq i \leq M}$ is a basis of $V_h([\Gamma])$ and $\{\widetilde{\eta}_{i,j}\}_{(i,j) \in \widetilde{H}}$ is a basis of $Y_h(\Gamma)$.  
\end{lemma}
%
%
%where, for any $u \in V_h(\Omega \setminus \Gamma)$, the coefficients $u(\mathbf{x}_{i,j})$ are uniquely defined by
%\begin{equation*}
%	u = \sum_{i = 1}^{N} \sum_{j=1}^{q_j} u(\mathbf{x}_{i,j})\phi_{i,j}\,.
%\end{equation*}
...
\begin{definition}
	...
	\[\forall u \in \mathbb{H}^{1/2}(\Gamma)\,, \quad \Pi_h u \isdef \sum_{k = 1}^M \dduality{u}{\vec \psi_k}\eta_k \,\,\, + \sum_{(i,j) \in \mathcal{B}} \dduality{u}{\vec \psi_{i,j}} \widetilde{\eta}_{i,j}\,.\]
	where $\vec \psi_k = \pi_N(\vec w_k)$, $\vec \psi_{i,j} = \pi_N(\vec w_{i,j})$, and the vector fields $\vec w_k$ and $\vec w_{i,j}$ are given by \Cref{prop1}.
\end{definition}
...
\begin{equation}
	...
	\end{cases}
\end{equation}
for all $(k,k') \in \{1,\ldots,N\}^2$ and $((i,j),(i',j')) \in \widetilde{H}^2$, where $h_i$ is the diameter of $\abs{\textup{st}(\mathbf{x}_i,\mathcal{M}_{\Omega,h})}$.
\end{proposition}
The proof of this proposition is the object of the next section. From the relations \eqref{dualityWiWij}, it follows that 
\[\begin{cases}
	\dduality{\phi_{k}}{\vec \psi_{k'}}  = \delta_{k,k'}\,,& \dduality{\phi_k}{\vec \psi_{i',j'}} = 0\,, \\
	\dduality{\phi_{i,j}}{\vec \psi_{k'}} = 0\,,& \dduality{\phi_{i,j}}{\vec \psi_{i',j'}} = \delta_{i,i'}\delta_{j,j'} \,,
\end{cases}\]
...
\end{equation*}
is satisfied, due to the polarity of $H^{1/2}([\Gamma])$ and $H^{-1/2}([\Gamma])$. This proves properties (ii) and (iii) of Theorem \ref{thm}, so it remains to prove the uniform $\mathbb{H}^{1/2}$ stability of $\Pi_h$. For this, we mostly follow the method of \cite{scott1990finite}. We consider another projection $\mathcal{Z}_h: H^1(\Omega \setminus \Gamma) \to V_h(\Omega \setminus \Gamma)$ with the property that 
\begin{equation}
	\label{commutZh}
...
\[\mathcal{Z}_hf \isdef \sum_{i = M+1}^N  \left(\int_{K_i}\psi_i(x) f(x)dx\right)\phi_i + \sum_{i = 1}^{M} \dduality{\pi_D(f)}{\vec \psi_i} \phi_{i} + \sum_{(i,j) \in \widetilde{H}} \dduality{\pi_D(f)}{\vec \psi_{i,j}} \widetilde{\phi}_{i,j}\,.\]
It is immediate that $\mathcal{Z}_h$ is a projection and that eq.~\eqref{commutZh} is satisfied.
For a tetrahedron $K \in \mathcal{M}_{\Omega,h}$, we denote by  $\textup{st}(K,\mathcal{M}_{\Omega,h})$ the set of tetrahedrons of $\mathcal{M}_{\Omega,h}$ sharing at least a vertex with $K$. The estimate of \cite[Thm 3.1]{scott1990finite} carries over for $\mathcal{Z}_h$ with minor modifications as we show now.
\begin{lemma}
	\label{lemUK}
...
\begin{proof}
	...
	\[\norm{\phi_i}_{H^1(K)} \leq C \sqrt{h_K}\,,\]
	...
	\end{equation*}
	since $\vec{w}_i$ is zero outside $\textup{st}(\mathbf{x}_i,\mathcal{M}_{\Omega,h}) \subset S_K$.  By the previous estimates on $\vec{w}_i$, using the shape-regularity of $\mathcal{M}_{\Omega,h}$ and some elementary inequalities, this leads to
...
\end{proof}
Using now the exact same arguments as in \cite[Thm 4.1]{scott1990finite} and \cite[Cor 4.1]{scott1990finite}, we deduce 
\begin{equation}
	\label{brambleIneq}
	\norm{\mathcal{Z}_h f}_{{H}^1(\Omega \setminus \Gamma)} \leq C\norm{f}_{{H}^1(\Omega \setminus \Gamma)}\,.
\end{equation}
\begin{remark}
	The main tool, to derive \eqref{brambleIneq} from \Cref{lemUK}, is a Bramble-Hilbert inequality in domains $U$ consisting of face-connected and shape-regular unions of tetrahedrons. To apply this result, a crucial requirement is that the restrictions of elements $V_h(\Omega \setminus \Gamma)$ to each tetrahedron $K \in \mathcal{M}_\Omega$ span all polynomial functions of degree $1$ in $K$.  
\end{remark}
\begin{corollary}
	There holds
...
The main tool for constructing the required vector fields is the set of bridge functions that we define now. Let $i \in \{1,\ldots,N\}$ and $1 \leq j,k \leq q_i$. We say that $\{j,k\}$ is a {\em bridge} around $\mathbf{x}_i$ if there exist two tetrahedrons $K \in \gamma_{i,j}$, $K' \in \gamma_{i,k}$ such that $K$ and $K'$ have a common face $F \in \mathcal{M}_{\Gamma,h}$. The set of bridges around $\mathbf{x}_i$ is denoted by $\mathcal{B}(i)$. 
...
\begin{proof}
	...
	\[\vec b_{K,F,\psi}(\vec y) \isdef \frac{1}{h(K,F)} \sum_{m = 0}^2\psi(\vec x_m) \lambda_m(\vec y)(\vec x_m - \vec y)\,, \quad \vec y \in K\,,\]
	...
	Fix a bridge $\{j,k\} \in \mathcal{B}(i)$, and let $K_j \in \gamma_{i,j}$ and $K_k \in \gamma_{i,k}$ be two tetrahedrons sharing a face $F_{jk} \in \mathcal{M}_{\Gamma,h}$. The face $F_{jk}$ contains the vertex $\vec x_i$. Following \cite{scott1990finite}, we define an affine function $\psi$ such that, for any affine function $\rho$, 
...
\end{lemma}
\begin{proof}
	We start by introducing the graph $\mathcal{G}^*(\mathbf{x}_i)$ defined by
	\begin{itemize}
...
The same argument allows to check that for all $i' \in \{1,\ldots,N\}$ and $(i'',j'') \in \widetilde{H}$ with $i'' \neq i$, there holds
...
\appendix
\section{Proofs of \Cref{discHarmLift,discJumpNorm,basisVh}}

\begin{proof}[Proof of \Cref{discHarmLift}]
	...
	\[\forall \widetilde{\varphi} \in \widetilde{H}^{1/2}(\Gamma)\,, \quad \norm{\Phi \widetilde{\varphi}}_{\mathbb{H}^{1/2}} = \norm{\widetilde{\varphi}}_{\widetilde{H}^{1/2}}\,.\]
	...
	\[\begin{aligned}
		...
		& \leq C(\gamma_0) \norm{\Phi \widetilde{\varphi}_h}_{\mathbb{H}^{1/2}}\\
		& = C(\gamma_0) \norm{\widetilde{\varphi}_h}_{\widetilde{H}^{1/2}}\,.
	\end{aligned}\]
...
\begin{proof}[Proof of  \Cref{discJumpNorm}]
...
\end{proof}
\begin{proof}[Proof of \Cref{basisVh}]
	Suppose that there exist real coefficients $\lambda_{i,j}$ such that 
	\begin{equation}
		\label{linDepPhiIJ}
		\forall \vec x \in \Omega\,, \quad \sum_{(i,j) \in \mathcal{H}(\Omega)} \lambda_{i,j} \phi_{i,j}(\mathbf{x}) = 0\,.
	\end{equation}
	...
	\end{equation}
	As before, the first limit can be shown easily. The second one is obvious if $K = K_{i_p,j_p}$. If $K$ shares a face $F \notin \mathcal{M}_\Gamma$ with $K_{i_p,j_p}$, then it is a consequence of the well-known property that for $u \in H^1(\Omega \setminus \Gamma)$, the traces of $u$ of $F$ from $K$ and $K_{i_p,j_p}$ must agree. Otherwise, by definition of $\mathcal{G}(\mathbf{x}_i)$, one can consider a face-connected path of tetrahedrons from $K$ to $K_{i_p,j_p}$, and the desired limit is established by repeating the previous argument for each pair of consecutive tetrahedrons in this path.
