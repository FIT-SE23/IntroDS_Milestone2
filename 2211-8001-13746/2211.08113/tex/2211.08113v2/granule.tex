...
A large number of observations show that \cite {1,2,3,4,5,6,7}, granule has a critical diameter, which can be divided into lage granule and mini granule according to the diameter. The number of the lage granule decreases rapidly with the increase of diameter, while the number of mini granule monotonously increases or remains with the decrease of diameter. The  average brightness, maximum brightness and fractal dimension of the two kinds of granule is different. The critical diameter obtained by different physical quantities is different, but the range is close  $(1 ''. 3  \sim 1 ''. 5)$ .



In the study of rotating turbulent thermal convection and solar differential rotation \cite{8}, the author found that the rotating of fluid cell influence the criterion of convection. Which makes  small fluid cell in convection state and lage fluid cell in  forced convection state. This theory can be further extended to granulation.


\section{ Thermal Properties of Rotating Fluid cell}

\subsection{ Additional pressure provided by rotation in a rotating fluid cell}



The radial pressure gradient in a uniformly rotating fluid is
\begin{equation}
	\frac{\mathrm{d} p}{\mathrm{d} R} = \rho \Omega^2 R
	\label{Eq.1}
\end{equation}

The pressure distribution along the radial direction is
\begin{equation}
		p = p_O + \frac{1}{2} \rho \Omega^2 R^2
	\label{Eq.2}
\end{equation}

Where $p_O$ is the pressure of the rotation axis $O$ . Suppose that a point $\mathrm {A}$ in the rotating fluid is $R$ away from the rotation axis, a point $\mathrm{B}$ in its neighborhood is $l$ away from the point $\mathrm{A}$ , and the pressure of the point $\mathrm {B}$ can be expressed as
\begin{equation}
	p_{\mathrm{B}} = p_O + \frac{1}{2} \rho \Omega^2 R^2 + \rho \Omega^2 R l \mathrm{cos} \left\langle \boldsymbol{R , l} \right\rangle  + \frac{1}{2} \rho \Omega_2 l_2
	\label{Eq.3}
\end{equation}
...
Where, the $\rho \Omega^2 R l \mathrm{cos} \left\langle \boldsymbol{R, l} \right\rangle$ term is the pressure difference caused by the radial pressure gradient, and the $\frac{1}{2} \rho \Omega^2 R^2$ term is the second-order pressure difference caused by the pressure gradient unique to the rotating fluid. This term is independent of the pressure gradient direction, but only related to the distance between two points. For fluid cells with the concept of size, the average pressure on the surface is generally greater than the average pressure inside, In some convection, it is necessary to consider the second order pressure difference, which will lead to new characteristics of the rotating fluid.


Take an independent cylinder fluid cell with radius of $a$ , height of $2 a$ , and rotational speed of $\Omega$ from the rotating fluid as the research object. Since the fluid mass rotates, there is a pressure gradient along the radial direction in the cylinder
\begin{equation}
	\frac{\mathrm{d} p}{\mathrm{d} r} = \rho \Omega^2 r
	\label{Eq.5}
\end{equation}

The pressure distribution along the radial direction in the rotating cylinder is
\begin{equation}
	p = p_{o} + \frac{1}{2} \rho \Omega^2 r^2
	\label{Eq.6}
\end{equation}

Where $p_{o}$ is the pressure on the rotation axis of the cylinder.  $p_{o}$ is not the same with the Same temperature $T$ and different speed $\Omega$ ; and  $p_{o}$ is also not the same with  different temperature $T$ and the Same speed $\Omega$ . $p_{o}$ includes both temperature and rotating speed items. It is necessary to decouple the pressure generated by temperature $T$ and speed $ \Omega$ . The average pressure on the cylinder surface can be expressed as
\begin{equation}
	\bar{p}_{S}  = \frac{\iint_{S} p \mathrm{d} S} {\iint_S \mathrm{d} S} = \bar{p}_{T} + \bar{p}_{\Omega} 
	\label{Eq.7}
\end{equation}

Where, $\bar{p}_T$ only includes temperature item, $\bar{p}_\Omega$ only includes the rotational speed term to complete the decoupling of the average pressure on the cylinder surface.
...
The \eneqref{Eq.8} is substituted into the \eneqref{Eq.9}, Tidied up
\begin{equation}
	\int_0 ^a \left( \frac{p}{\bar{p}_{T}} \right) ^ \frac{1}{\gamma} r \mathrm{d} r = \int_0 ^a \frac{1}{2} a^2
	\label{Eq.10}
\end{equation}
...
It can be seen from the formula, $\bar{p}_T$ is the pressure of the cylinder at the radius of $\frac{\sqrt{2}}{2} a$ . Within the linear range, the temperature and pressure at this point are independent of the rotational speed, and the temperature at this point $T$ can be used as the characteristic temperature of the cylinder. 
...
Different shapes, $k_\Omega$ has different values.
...
\subsection{Transient Equilibrium of Pressure}



When the rotating equivalent temperature of the fluid cell is different from that of the environmental fluid, the rotational speed of the fluid cell is different from that of the environmental fluid, which will lead to the imbalance between the fluid cell and the environmental fluid, resulting in inertial waves, which will make the rotational speed of the fluid cell and the environmental fluid in the oscillate finally tend to be consistent. The characteristic time of the evolution of the inertial waves is the rotational period of the fluid cell or the environmental fluid $T_0$ . When the evolution characteristic time of expansion process $t$ is far less than $T_0$ , the influence of inertial wave can be ignored when studying this process, and the local pressure can be unbalanced.



In the convection process, for small-scale spherical fluid clusters, the acoustic wave propagation in the expansion process is much faster than the inertial wave, and the surface pressure balance of the fluid cell is much faster than the speed balance. Therefore, the fluid cell balance is marked by the surface pressure balance, and the evolution time $t $ to reach the balance is significantly greater than $\frac{a}{v_c}$ , where $a$ is the characteristic size of the fluid cell, $v_c$ is the sound speed, at this time, the pressure balance meets
\begin{equation}
...
The force acting on the disturbed air cell will keep it moving away from its initial position, creating convection.



There are two temperatures in the turbulent thermal convection of the sun. The isotropic expansion of the rotating equivalent temperature can be regarded as a thermal process with a specific heat ratio of $\frac{5}{3}$ while the gas in the solar covection zone exists as atoms or ions and can be regarded as a monoatomic gas with a specific heat ratio of $\frac{5}{3}$ at which time the adiabatic temperature gradient can be expressed as
\begin{equation}
...
Obviously, because of the additional terms of pressure distribution in rotating fluids, the pressure changes resulting from changes in the position of the fluid cell
 will be more complex, so the adiabatic gradient can be expressed more simply as a density gradient. The new convection criterion is
\begin{equation}
...
\subsection{Size form of convection criterion}


According to \eneqref{Eq.16}, with
\begin{equation}
...
The substitution of \eneqref{Eq.30} and \eneqref{Eq.31} into \eneqref{Eq.28} has
\begin{equation}
...
By substituting the \eneqref{Eq.16} into the \eneqref{Eq.33} , we have
\begin{equation}
	a^2 = \frac
...
(4)when $\left| \frac{\mathrm{d} T}{T \mathrm{d} l} \right| > \left| \frac{2}{3} \frac{\mathrm{d} \rho}{\rho \mathrm{d} l} \right|$ and $\left| \frac{\mathrm{d} \Omega}{\Omega \mathrm{d} l} \right| < \left| \frac{2}{3} \frac{\mathrm{d} \rho}{\rho \mathrm{d} l} \right|$ , Convection is driven by both speed gradient and temperature gradient at the same time, and the fluid cell size is unrestricted.

\subsection{ Size Limitation of granulation}



In the solar convection zone, the granule exists in the range of 5 to 16 minutes, which is much smaller than the solar rotation cycle, so the fluid cell expansion is statistically close to isotropy. The thermal properties of  rotation can be fully represented.


Convection in the solar convection zone is driven by the temperature gradient, so there is $\left| \frac{\mathrm{d} T}{T \mathrm{d} l} \right| > \left| \frac{2}{3} \frac{\mathrm{d} \rho}{\rho \mathrm{d} l} \right|$ . Under the vorticity transport  of rotating turbulent thermal convection, the relative speed gradient $\frac{\mathrm{d} \Omega}{\Omega \mathrm{d} l}$ approximates $\frac{2}{3} \frac{\mathrm{d} \rho}{\rho \mathrm{d} l}$ , while the relative speed gradient $\frac{\mathrm{d} \Omega}{\Omega \mathrm{d} l}$ affected by viscosity approximates zero. Therefore, the final relative speed gradient is between both,$\left| \frac{\mathrm{d} \Omega}{\Omega \mathrm{d} l} \right| < \left| \frac{2}{3} \frac{\mathrm{d} \rho}{\rho \mathrm{d} l} \right|$, which satisfies the second case above. The convection criterion has the form of size  $a < a_\mathrm{ad}$ .



When $a < a_\mathrm{ad}$ , thermal convection occurs spontaneously; When $a > a_\mathrm{ad}$ , the fluid cell will have an equilibrium position, the disturbed fluid cell will oscillate near the equilibrium position and decelerate due to dissipation of viscous. natural convection can stimulate forced convection in rotating turbulent thermal convection, so the critical size $a_\mathrm{ad}$ is not the upper limit of the size of the fluid cell. But while $a > a_\mathrm{ad}$ ,the number of fluids cell decreases sharply with the increase of $a$ . This is reflected in the size distribution characteristics of granule.A large number of observations have shown that there is a critical diameter with the granule, which can be divided into mini granule and lage granule according to the diameter. The number of lage granule decreases rapidly with the increase of diameter. The number of mini grains increases monotonously or remains flat with the decrease of diameter. There are differences on average brightness, maximum brightness and fractal dimension between the two types of granule. Different physical quantities give different critical diameter, but the range is close $(1'.3 \sim 1''.5)$ , which is in agreement with the theory.
