...
On a spacetime manifold $\M$, there exists a time function $t:\M\rightarrow\R_+$ such that we can foliate it as $\M=\left(\Sigma_s\right)_{s\in(0,t_0]}$. Each of these three-dimensional manifolds $\Sigma_s=t^{-1}(\{s\})$ is a spacelike Cauchy hypersurface that is diffeomorphic to $M$, and will be in CMC gauge (see \eqref{eq:CMC}). We will often simply think of $\Sigma_s$ as $\{s\}\times M$.
...
Within this paper, the second fundamental form with regards to $\Sigma_t$ is defined as
\[k=-\g(\nabbar_X\del_0,Y)\]
for $\Sigma_t$-tangent vector fields $X$ and $Y$. The Riemann curvature tensor of $\g$ is taken to be
\[\nabbar_\alpha Z_\beta-\nabbar_\beta Z_\alpha={\Riem[\g]_{\alpha\beta\gamma}}^\delta Z_\delta\]
...
For two nonnegative scalar functions $\zeta_1,\zeta_2$, we write $\zeta_1\lesssim\zeta_2$ iff there exist some abstract constant $K>0$ such that $\zeta_1\leq K\zeta_2$. This implicit constant may depend on information from the FLRW reference solution at the starting point of the evolution (in particular on $\gamma$ and $a(t_0)$, see Def. \ref{def:spatial-mf}) and combinatorial quantities. We extend this notation to a real function $\zeta_1^\prime$ by
\[\zeta_1^\prime\lesssim\zeta_2 :\Leftrightarrow \max\left(\zeta_1^\prime,0\right)\lesssim\zeta_2\,.\]
Additionally, we write $\zeta_1\simeq\zeta_2$ iff $\zeta_1\lesssim\zeta_2\lesssim\zeta_1$ is satisfied.\\
...
We denote by $\epsilonLC_{\alpha\beta\gamma\delta}$ the Levi Civita tensor with regards to $\g$ and define Levi Civita tensor on spatial hypersurfaces $\Sigma_t$ by $\epsilonLC[g]_{ijk}=\epsilonLC_{0ijk}$. Notice that this corresponds to the Levi Civita tensor associated to $g$. Further, $\epsilonLC[G]_{ijk}=a^{-3}\epsilonLC[g]_{ijk}$ is the Levi Civita tensor with respect to the rescaled metric $G$ (see \eqref{eq:rescalingGK}).

For $\Sigma_t$-tangent $(0,2)$-tensors $A,\tilde{A}$, we define the following objects:

\begin{align*}
A\cdot\tilde{A}&=A^a_b\tilde{A}^b_a=\langle A,\tilde{A}\rangle_g\\
...
We will denote as $\mathfrak{T}_1\ast\dots\ast\mathfrak{T}_l$ and $\Sigma_t$-tangent tensors $\mathfrak{T}_i$, any multiple of $(\mathfrak{T}_i)$, with regards to the rescaled adapted spatial metric $G$ or as standard multiplication if no summation over indices occurs between factors. Constant prefactors and contractions with regards to $G$ are also surpressed in this notation. 

When working with terms where such notation is used, we will estimate these inner products by $\lesssim \prod_{i=1}^l\lvert\mathfrak{T}_i\rvert_G$, making any constant in front irrelevant, and further we can view any contraction with regards to $G$ as a product of the non-contracted tensor $\mathfrak{T}$ with $G$ or $G^{-1}$, and estimate that up to constant by $\lvert G\rvert_G\lvert\mathfrak{T}\rvert_G$, where the first factor is simply $\sqrt{3}$.\\
...
For a scalar function $\zeta$, an $(r,s)$-tensor field $\mathfrak{T}$ and \textit{capitalized} integers $I,J,\dots\in\N_0$, we denote by $\nabla^I\zeta$ and $\nabla^I\mathfrak{T}$ the tensors $\nabla_{l_1}\dots\nabla_{l_I}\zeta$ and\linebreak
$\nabla_{l_1}\dots\nabla_{l_I}{\mathfrak{T}^{i_1\dots i_r}}_{j_1\dots j_s}$. We extend this notation to other covariant derivatives analogously. To avoid potential ambiguity with an index raised by $g$ or $\g$, we will apply the following convention:
\begin{itemize}
...
\subsection{FLRW spacetimes and Friedman equations}\label{subsec:FLRW}
...
$(M,\gamma)$ is a three-dimensional, connected, closed, orientable Riemannian manifold with constant sectional curvature $-\frac19$, hence Ricci tensor $\Ric[\gamma]=-\frac29\gamma_{ij}$ and scalar curvature $R[\gamma]=-\frac23$. 
\end{definition}
\begin{remark}[On orientability]
It is likely that orientability could be dropped as an assumption on the spatial geometry (which, given the sectional curvature, is a restriction on topology by Cartan's theorem). We use it to be able to integrate over volume forms and to integrate by parts, but using a more general notion of integration over Riemannian manifolds and of Stokes' theorem may allow our results to be extended. 
\end{remark}

With this in hand, we can express our classical family solutions to the Einstein scalar-field system as follows:

\begin{lemma}[FLRW solutions and Friedman equations]\label{lem:FLRW}
We consider FLRW spacetimes $(\M,{\g}_{FLRW})$ with $\M=(0,\infty)\times M$, where $(M,\gamma)$ is as in Definition \ref{def:spatial-mf} and where
\begin{equation}\label{eq:FLRW-metric}
{\g}_{FLRW}=-dt^2+a(t)^2\gamma
\end{equation}
holds for some $a\in C^\infty((0,\infty))$, with the conventions $a(0)=0$ and $\dot{a}(T)>0$ for some arbitrary $T>0$. Further, we choose a (smooth) scalar function $\phibar$ such that the following holds:
\begin{equation}\label{eq:FLRW-wave}
\del_t\phibar=C\cdot a(t)^{-3},\quad \nabbar\:\phibar=0, \quad \square_{\g_{FLRW}}\phibar=0
\end{equation}
Such a pair $(\g_{FLRW},\phibar)$ solves the Einstein scalar-field system \eqref{eq:ESF1}-\eqref{eq:EMT} on $\M$ if and only if $a$ satisfies the Friedman equations:
...
\end{lemma}
\begin{proof}
From \cite[p.345]{ONeill83}, one collects the following expressions for the Ricci tensor:
\begin{gather*}
\numberthis\label{eq:FLRW-Ric00}\Ric[\g_{FLRW}](\del_t,\del_t)=-3\frac{\ddot{a}}{a},\quad \Ric[\g_{FLRW}](\del_t,\del_j)=0,\\
\Ric[\g_{FLRW}](\del_i,\del_j)=\left(2\left(\frac{\dot{a}}a\right)^2+\frac{2\kappa}{a^2}+\frac{\ddot{a}}{a}\right)(\g_{FLRW})_{ij}\\
R[\g_{FLRW}]=6\left(\left(\frac{\dot{a}}{a}\right)^2+\frac\kappa{a^2}+\frac{\ddot{a}}{a}\right)
\end{gather*}
Thus, denoting the Einstein tensor by \text{Einst} and inserting $\kappa=-\frac19$, one computes
\begin{gather*}
{\text{Einst}}(\del_t,\del_t)=3\left[\left(\frac{\dot{a}}{a}\right)^2-\frac1{9a^2}\right],\quad {\text{Einst}}(\del_t,\del_j)=0\\
{\text{Einst}}(\del_i,\del_j)=(\g_{FLRW})_{ij}\left[-\left(\frac{\dot{a}}{a}\right)^2+\frac1{9a^2}-2\frac{\ddot{a}}{a}\right]
\end{gather*}
Hence, using \eqref{eq:FLRW-wave}, we have
\begin{equation}\label{eq:preFriedman1}
\left(\frac{\dot{a}}{a}\right)^2-\frac1{9a^2}=\frac{8\pi}3T(\del_t,\del_t)=\frac{4\pi}3C^2a^{-6}
\end{equation}
and \eqref{eq:Friedman} now follows by rearranging, since $\dot{a}(T)>0$ is true for some $T>0$ and $\lvert\dot{a}\rvert>1$ prevents sign change, thus $\dot{a}>0$ holds on $(0,\infty)$. Furthermore, \eqref{eq:ESF1} implies
\begin{equation}\label{eq:preFriedman2}
\left[-\left(\frac{\dot{a}}{a}\right)^2+\frac1{9a^2}-2\frac{\ddot{a}}{a}\right](\g_{FLRW})_{ij}=8\pi T_{ij}=4\pi C^2a^{-6}(\g_{FLRW})_{ij}\,.
\end{equation}
Dropping $(\g_{FLRW})_{ij}$ and using \eqref{eq:Friedman}, one obtains \eqref{eq:Friedman2} after rearranging. Conversely and since $\text{Einst}(\del_t,\del_j)=0=T(\del_t,\del_j)$ is immediate, \eqref{eq:preFriedman1} and \eqref{eq:preFriedman2} shows that the Friedman equations implies that $(\g_{FLRW},\phibar)$ solves \eqref{eq:ESF1}-\eqref{eq:EMT}. That \eqref{eq:Friedman} implies \eqref{eq:Friedman2} follows simply by derivation of $\dot{a}^2$.
\end{proof}
...
\begin{lemma}[Scale factor analysis]\label{lem:scale-factor}Let $a$ solve \eqref{eq:Friedman} with $a(0)=0$. Then $a$ is analytic on $(0,\infty)$ and extends to a continuous function on $[0,\infty)$ with $a(t)\simeq t^{\frac13}$ being satisfied near $t=0$. Further, for any $p>0$, there exist constants $c>0$ and $K_p>0$, where $c$ is independent of $p$ and $K_p$ depends analytically on $p$, such that 
\begin{equation}\label{eq:a-exp-est}
\exp\left(p\int_t^{t_0}a(s)^{-3}\,ds\right)\leq K_pa(t)^{-cp}
\end{equation}
holds for any $t\in(0,t_0]$ as well as
\begin{equation}\label{eq:a-integrals}
\int_t^{t_0}a(s)^{-3-p}\,ds\leq\frac1p a(t)^{-p},\quad \int_t^{t_0}a(s)^{-3+p}\,ds\lesssim\frac{1}p\,.
\end{equation}
Moreover, for any $t\in(0,t_0]$ and any $q>0$, there exists a constant $c>0$ such that one has
\begin{equation}\label{eq:log-est}
\int_t^{t_0}a(s)^{-3}\,ds\lesssim \frac{1}qa(t)^{-cq}\,.
\end{equation}
...
\end{remark}
\begin{proof}
For the first statement, we refer to \cite[Lemma 2.1]{Urban22} with $\gamma=2$.

We also collect from there
%%%%%%%%%
\footnote{\label{fn:t0-small}In \cite[Lemma 2.1]{Urban22}, one at first only has $\int_t^{t_0}a(s)^{-3}\lesssim \log(t_0)-\log(t)$ for $t_0$ small enough that we can estimate $a(t)$ by $t^\frac13$ up to constant. However, assuming this inequality were to hold up to $\overline{t}>0$ and one had $t_0>\overline{t}$, the contribution $\int_{\overline{t}}^{t_0}a(s)^{-3}\,ds$ only adds a constant that we can absorb into our notation. Similarly, $a(t)\simeq t^\frac13$ can be assumed to hold on $(0,t_0]$ for any fixed $t_0>0$, and we can ignore this technicality in proving the integral formulas in Lemma \ref{lem:scale-factor}.} that, for $t<t_0$, 
%%%%%%%%%%
\[\int_t^{t_0}a(s)^{-3}\,ds\lesssim 1+\lvert\log(t)\rvert\]
is satisfied. Hence, there exists some $c^\prime>0$ such that the following holds (with $t<1$ without loss of generality, see Footnote \ref{fn:t0-small}):
\[\exp\left(p\int_t^{t_0}a(s)^{-3}\,ds\right)\leq \exp(c^\prime\cdot p\log(t_0))\cdot\exp(-c^\prime\cdot p)\leq K_p t^{-c^\prime p}\]
\eqref{eq:a-exp-est} follows by applying $a(t)\simeq t^\frac13$ (see Footnote \ref{fn:t0-small}).\\
Noting that $a^{-3}\simeq \nicefrac{\dot{a}}a$ holds, one further has
\begin{equation}
\int_t^{t_0}a(s)^{-3-p}\,ds\lesssim\int_{a(t)}^{a(t_0)}y^{-1-p}dy=\frac1p\left(a(t)^{-p}-a(t_0)^{-p}\right)\leq\frac1p a(t)^{-p}\,,
\end{equation}
the other inequality in \eqref{eq:a-integrals} follows analogously. \\
...
From here on out, we impose the CMC condition
...
\begin{aligned}
\del_t\tau&=(-3)\frac{a\cdot\ddot{a}-\dot{a}^2}{a^2}\\
&=3\left(\frac{8\pi}3C^2a^{-6}+\frac19 a^{-2}+\frac{4\pi}3C^2a^{-6}\right)\\
&=12\pi C^2a^{-6}+\frac13a^{-2}
\end{aligned}
\end{equation}
and
\begin{equation}\label{eq:Hubble2}
\tau^2=9\frac{\dot{a}^2}{a^2}=12\pi C^2a^{-6}+a^{-2}\,.
\end{equation}
...
In this subsection, we (re-)establish Bel-Robinson variables and how they behave within the Einstein scalar-field system.\\

The Riemann curvature tensor of $\g$ into pure-trace and tracefree components can be decomposed as follows:
\begin{align*}
\Riem[\g]_{\alpha\beta\gamma\delta}=&\,P[\g]_{\alpha\beta\gamma\delta}+W[\g]_{\alpha\beta\gamma\delta}\\
P[\g]_{\alpha\beta\gamma\delta}=&\,\frac12\left(\g_{\alpha\gamma}\Ric[\g]_{\beta\delta}-\g_{\alpha\delta}\Ric[\g]_{\beta\gamma}+\g_{\beta\delta}\Ric[\g]_{\alpha\gamma}-\g_{\beta\gamma}\Ric[\g]_{\alpha\delta}\right)\\
&-\frac16R[\g](\g_{\alpha\gamma}\g_{\beta\delta}-\g_{\alpha\delta}\g_{\beta\gamma})
\end{align*}
Further, we define the dual $W^\ast$ of the Weyl tensor $W\equiv W[\g]$ as
\[W^\ast_{\alpha\beta\gamma\delta}=\frac12\epsilonLC_{\alpha\beta\mu\nu}{W^{\mu\nu}}_{\gamma\delta}\,.\]
The electric and magnetic components of the Weyl tensor are now defined as
...
\Ric[\g]_{\mu\nu}=8\pi(T_{\mu\nu}-\frac12 T g_{\mu\nu})=8\pi\nabbar_\mu\phi\nabbar_\nu\phi,\quad R=8\pi\nabbar^\alpha\phi\nabbar_\alpha\phi\,.
\end{equation}
So, we can now collect:
\begin{align*}
\numberthis\label{eq:J}J_{i0j}=&\,4\pi\Bigr[\nabbar_0(\nabbar_i\phi\nabbar_j\phi)-\nabbar_j(\nabbar_i\phi\nabbar_0\phi)\Bigr]
-\frac{2\pi}3\Bigr[\nabbar_0(\nabbar^\alpha\phi\nabbar_\alpha\phi)\Bigr]g_{ij}\\
=&\,4\pi\Bigr[\nabla_i(\del_0\phi)\nabla_j\phi+{k^l}_i\nabla_l\phi\nabla_j\phi+\nabla_j(\del_0\phi)\nabla_i\phi-\del_0\phi\nabla_i\nabla_j\phi\\
&\,\phantom{4\pi}-k_{ij}(\del_0\phi)^2+n^{-1}\nabla_jn\cdot\nabla_i\phi\cdot\del_0\phi\Bigr]-\frac{2\pi}3\Bigr[\del_0(\nabbar^\alpha\phi\nabbar_\alpha\phi)\Bigr]g_{ij}\\
...
The Bel-Robinson variables then behave as follows under the Einstein scalar-field system:
...
\begin{subequations}
\begin{align}
\del_tE_{ij}&=n\curl{B}_{ij}-(\nabla n\wedge B)_{ij}-\frac52n\left(E\times k\right)_{ij}-\frac23n(E\cdot k)g_{ij}-\frac{\tau}2n\cdot E_{ij}-nJ_{i0j} \label{eq:EEqE}\\
\del_tB_{ij}&=-n\curl{E}_{ij}+(\nabla n\wedge E)_{ij}-\frac52n\left(B\times k\right)_{ij}-\frac23n(B\cdot k)g_{ij}-\frac{\tau}2n\cdot B_{ij}-nJ_{i0j}^\ast \label{eq:EEqB}
\end{align}
\end{subequations}
...
\end{lemma}
\begin{proof}
For \eqref{eq:EEqE}-\eqref{eq:EEqB}, we refer to \cite[(3.11a)-(3.11b)]{AM03}. For \eqref{eq:constr-E}, we start out with the Gauss equation (see \cite[Chapter 2.3, (2.23)]{Rendall08}):
...
\end{remark}
\begin{remark}[Inital data for Bel-Robinson variables]\label{rem:init-BR}
Furthermore, note that given initial data $(M,\mathring{g},\mathring{k},\mathring{\pi},\mathring{\psi})$ on $\Sigma_{t_0}$ in the sense of Remark \ref{rem:initial-data}, and defining $\hat{\mathring{k}}=\mathring{k}-\frac{\tau}3\mathring{g}$, we can use \eqref{eq:constr-E} and \eqref{eq:constr-B} to define the following $(0,2)$-tensors:
\begin{subequations}
\begin{align}
...
\del_t(\Sigma^\sharp)^a_b=&\,\tau N(\Sigma^\sharp)^a_b-a\nabla^{\sharp a}\nabla_bN+(N+1)a\left[\left(\Ric[G]^\sharp\right)^a_b+\frac 29\I^a_b\right]-8\pi (N+1)a\nabla^{\sharp a}\phi\nabla_b\phi\,\numberthis\label{eq:REEqSigmaSharp}\\
&+N\left(4\pi C^2a^{-3}+\frac19 a\right)\I^a_b\,,
\end{align*}
\end{subequations}
...
\RB_{ij}=&\,-a^3\curl_g\Sigma_{ij}\numberthis\label{eq:REEqConstrB}\,,
\end{align*}
\end{subequations}
the \textbf{rescaled lapse equation}
\begin{subequations}
\begin{equation}\label{eq:REEqLapse1}
\Lap N=(12\pi C^2a^{-4}+3)N+(N+1)a^{-4}\left[\langle\Sigma,\Sigma\rangle_G+8\pi\Psi^2+16\pi C\Psi\right]
\end{equation}
or equivalently
\begin{equation}\label{eq:REEqLapse2}
\Lap N=(12\pi C^2a^{-4}+3)N+(N+1)\left[R[G]+\frac23-8\pi\lvert\nabla\phi\rvert_G^2\right]\,,
\end{equation}
\end{subequations}
the \textbf{rescaled evolution equations for the Bel-Robinson variables}\footnote{The dot, wedge and cross products as well as $\curl$ as an operator still include raising and lowering with regards to the unrescaled spatial metric, this will be dealt with in estimation to avoid too much notational baggage.}
\begin{subequations}
\begin{align*}
\numberthis\label{eq:REEqE}\del_t\RE_{ij}%=&\,(N+1)\curl\RB_{ij}-(\nabla N\wedge\RB)_{ij} \\
%&-(N+1)\left[\frac52(\RE\times k)_{ij}+\frac23(\RE\cdot k)a^2G_{ij}+\frac\tau2\RE_{ij}+a^4J_{i0j}\right]+4\frac{\dot{a}}a\RE_{ij}\\
=&\,\left(3-N\right)\frac{\dot{a}}a\RE_{ij}-(\nabla N\wedge\RB)_{ij}+(N+1)\curl\RB_{ij}\\
&\,-(N+1)\left[\frac52a^{-1}(\RE\times\Sigma)_{ij}+\frac23a^{-3}\langle\RE,\Sigma\rangle_GG_{ij}\right]\\
&\,+4\pi(N+1) a^{-3}(\Psi+C)^2\Sigma_{ij}-4\pi(N+1) \dot{a}a^3\nabla_i\phi\nabla_j\phi-4\pi a\nabla_jN\nabla_i\phi(\Psi+C)\\
&\,-4\pi a(N+1)\left[\nabla_i\Psi\nabla_j\phi+\nabla_j\Psi\nabla_i\phi+(\Sigma^\sharp)^l_i\nabla_l\phi\nabla_j\phi-(\Psi+C)\nabla_i\nabla_j\phi\right]\\
...
\numberthis\label{eq:REEqB}\del_t\RB_{ij}=&\,\frac{\dot{a}}a\left(3-N\right)\RB_{ij}+(\nabla N\wedge\RE)_{ij}-(N+1)\curl\RB_{ij}\\
&\,-(N+1)\left[\frac52a^{-1}(\RB\times\Sigma)_{ij}+\frac23(\RB\cdot\Sigma)aG_{ij}+\curl\RE_{ij}\right]\,\\
&\,-4\pi(N+1)\epsilonLC[G]_{lmj}\left(a^3\nabla^{\sharp l}\nabla_j\phi\nabla^{\sharp m}\phi+a^{-1}(\Sigma^\sharp)^l_i\nabla^{\sharp m}\phi(\Psi+C)\right)\\
&\,-(N+1)\epsilonLC[G]_{imj}\left(4\pi a^2\nabla^{\sharp m}\phi(\Psi+C)+\frac{2\pi}3a^5\nabla^{\sharp m}\left(a^{-6}(\Psi+C)^2+a^{-2}\lvert\nabla\phi\rvert_G^2\right)\right)\,,
\end{align*}
\end{subequations}
...
\begin{proof}
For the first identity in \eqref{eq:REEqChr}, we refer to \cite[Lemma 2.27]{Chow06} and insert the evolution equation \eqref{eq:REEqG}. Otherwise, all equations simply follow by computing the effects of rescaling on the equations from Proposition \ref{prop:eq} as well as the Bel-Robinson evolution equations \eqref{eq:EEqE}-\eqref{eq:EEqB} and constraint equations \eqref{eq:constr-E}-\eqref{eq:constr-B}. Notice that one already finds a solution to the system in Proposition \eqref{prop:eq} with the rescaled variables excluding \eqref{eq:REEqE}, \eqref{eq:REEqB}, \eqref{eq:REEqConstrE} and \eqref{eq:REEqConstrB}. Conversely, all of the rescaled equations are satisfied by a solution to Proposition \ref{prop:eq} at sufficiently high regularity. Hence, solving the full rescaled system is always sufficient to solve the Einstein system in Proposition \ref{prop:eq} and they are equivalent if the initial data is regular enough to ensure sufficiently high regularity of solutions.\\
\end{proof}

\subsection{Commuted equations}\label{subsec:comm-eq}

We collect commuted versions of the equations for the rescaled variables in Proposition \ref{prop:REEq} in this subsection. For the sake of brevity, we won't state all possible commutations for every equation, but restrict ourselves to the ones we actually need within the bootstrap argument. We also refer to Appendix \ref{subsec:commutators} for expressions for commutators of spatial differential operators with each other and with $\del_t$.

\subsubsection{Laplace commuted equations}\label{subsubsec:comm-eq-Lap}

The commuted equations in this section will be the essential ones within the core energy argument, namely the equations from Proposition \ref{prop:REEq} commuted with Laplacians with respect to the rescaled spatial metric $G$.  The terms written down explicity in Lemma \ref{lem:laplace-commuted-eq} are ones that dominate the evolution behaviour or that are the least small high order terms, which both require careful treatment within the bootstrap argument. The error terms are broadly categorized into three groups:
\begin{itemize}
\item \enquote{Borderline} terms are terms that critically contribute to the divergence of our energies toward the Big Bang singularity. This almost always takes the form of adding energy terms at the same order as the evolved variable scaled by factors of the type $\epsilon a^{-3}$ or $\epsilon a^{-3-c\sqrt{\epsilon}}$, which ultimately causes the energies to slightly diverge since $a^{-3}$ is barely not integrable (see \eqref{eq:a-integrals}).\\
\item \enquote{Junk} terms are terms that are sub-critical in the sense that they lead to integrable error terms, or terms that only contain lower order derivatives of the solution variables, neither of which substantially influence the behaviour of our energies.\\
\item \enquote{Top} order terms (which only appear in \eqref{eq:comeq-RE} and \eqref{eq:comeq-RB}) are terms that are terms that are junk terms for low order energies, but become borderline terms at top order.
\end{itemize}
...
\del_t\Lap^\frac{L}2\Sigma=-a\nabla^2\Lap^\frac{L}2N+a(N+1)\Lap^\frac{L}2\Ric[G]^\sharp+\mathfrak{S}_{L,Border}+\mathfrak{S}_{L,Junk}\,,
\end{equation}
...
and
\begin{equation}
\curl\Lap^{\frac{L}2}\Sigma=-a^{-3}\Lap^{\frac{L}2}\RB+a^{-1}\epsilonLC[G]\ast\nabla\Lap^{\frac{L}2-1}\Ric[G]\ast\Sigma+\numberthis\tilde{\mathfrak{M}}_{L,Junk}\,,\label{eq:comeq-mom-curl}
\end{equation}
...
\begin{subequations}
\begin{align*}
\del_t\Lap^{\frac{L}2}\RE=&\,\frac{\dot{a}}a\left(3-N\right)\Lap^{\frac{L}2}\RE+(N+1)\curl\Lap^{\frac{L}2}\RB-\nabla\Lap^\frac{L}2 N\wedge \RB\numberthis\label{eq:comeq-RE}\\
&\,+4\pi C^2a^{-3}(N+1)\Lap^{\frac{L}2}\Sigma-4\pi a(\Psi+C)\nabla\Lap^{\frac{L}2}N\nabla\phi\\
...
\del_t\Lap^{\frac{L}2}\RB=&\,\frac{\dot{a}}a(3-N)\Lap^{\frac{L}2}\RB-(N+1)\curl\Lap^{\frac{L}2}\RE+\nabla\Lap^{\frac{L}2}N\wedge\RE \numberthis\label{eq:comeq-RB}\\
...
\begin{subequations}
\begin{align*}
\del_t\Lap^\frac{L}2\Ric[G]_{ij}=&\,a^{-3}\left(\Lap^{\frac{L}2+1}\Sigma_{ij}-2\nabla^{\sharp m}\nabla_{(i}\Lap^{\frac{L}2}\Sigma_{j)m}\right)-\frac{\dot{a}}a\left(\nabla_{i}\nabla_j\Lap^\frac{L}2N+\Lap^{\frac{L}2+1}N\cdot G_{ij}\right)\numberthis\label{eq:comeq-Ric-even}\\
&\,+\left(\mathfrak{R}_{L,Border}\right)_{ij}+\left(\mathfrak{R}_{L,Junk}\right)_{ij}\\
%%%%%%%%%%%%%%%%%%%%
...
&\,-\frac{\dot{a}}a\left(\nabla_k\nabla_i\nabla_j\Lap^\frac{L}2N+\nabla_k\Lap^{\frac{L}2+1}N\cdot G_{ij}\right)+\left(\mathfrak{R}_{L+1,Border}\right)_{ijk}+\left(\mathfrak{R}_{L+1,Junk}\right)_{ijk}\,.
\end{align*}
\end{subequations}
\end{lemma}
Note that, despite being phrased with regard to some arbitrary coordinate system for notational convenience, \eqref{eq:comeq-Psi-odd} and \eqref{eq:comeq-Ric-even}-\eqref{eq:comeq-Ric-odd} establish global evolution equations for the globally defined tensor fields $\nabla\Lap^{\frac{L}2}\Psi,\,\Lap^\frac{L}2\Ric[G]$ and $\nabla\Lap^\frac{L}2\Ric[G]$.
\begin{proof}
...
Herein, we state the norms and energies we use to control the solution variables. These will allow us to state our initial data and bootstrap assumptions, and we then provide which improvement we aim to achieve for the latter. Note that we don't (and can't) provide the coerciveness of our energies immediately (or at least not in a manner useful to our analysis), but will establish Sobolev norm control in the proof of Corollary \ref{cor:H-imp}, the key ingredient being Lemma \ref{lem:Sobolev-norm-equivalence-improved}. Furthermore, we collect a local well-posedness statement from previous work in Section \ref{subsec:lwp}.

\subsection{Norms}\label{subsec:norm}

Recall that $\gamma$ is the hyperbolic spatial reference metric on $M$ introduced in Definition \ref{def:spatial-mf}, which we view as a metric on any foliation hypersurface $\Sigma_t$ (see Remark \ref{rem:initial-data}), and $G$ is the rescaled spatial metric arising from the evolution (see Definition \ref{def:rescaled}).

\begin{definition}[Pointwise norms and volume forms] \label{def:pw-stuff}
We denote by $\lvert\cdot\rvert_\gamma$ (resp. $\lvert\cdot\rvert_{G(t,\cdot)}$) the pointwise norm with regards to $\gamma$ (resp. $G(t,\cdot)$). For the sake of simplicity, we define $\lvert\zeta\rvert_\gamma=\lvert\zeta\rvert_{G(t,\cdot)}=\lvert\zeta(t,\cdot)\rvert$ for any scalar function $\zeta$ on $\Sigma_t$.\\
The volume forms on $\Sigma_t$ with respect to $\gamma$ and $G(t,\cdot)$ are written as $\vol{\gamma}$ and $\vol{G(t,\cdot)}$ (or just $\vol{G}$). \end{definition}
...
\begin{remark}[Time dependence is supressed in notation]
When the choice of $t$ and $\Sigma_t$ is clear from context, we will often drop time dependences of $G$, $\lvert\cdot\rvert_G$, $\vol{G}$ and $\mathfrak{T}$, surpress the hypersurface $\Sigma_t$ in the Sobolev and supremum norms, and simply write $\int_M$ instead of $\int_{\Sigma_t}$. For example, we write
...
\begin{subequations}
\begin{align*}
\mathcal{H}=&\,\|\Psi\|_{H^{19}_G}+\|\nabla\phi\|_{H^{18}_G}+a^2\|\nabla\phi\|_{\dot{H}^{19}_G}+\|\Sigma\|_{H^{18}_G}+\|\RE\|_{H^{18}_G}+\|\RB\|_{H^{18}_G} \numberthis\label{eq:def-H}\\
&\,+\|G-\gamma\|_{H^{18}_G}+\|\Ric[G]+\frac29G\|_{H^{16}_G}+a^{-4}\|N\|_{H^{16}_G}+a^{-2}\|N\|_{\dot{H}^{17}_G}+\|N\|_{\dot{H}^{18}_G}\\
\mathcal{C}=&\,\|\Psi\|_{C^{17}_G}+\|\nabla\phi\|_{C^{16}_G}+\|\Sigma\|_{C^{16}_G}+\|\RE\|_{C^{16}_G}+\|\RB\|_{C^{16}_G} \numberthis\label{eq:def-C}\\
&\,+\|G-\gamma\|_{C^{16}_G}%+\|\Gamma-\Gamhat\|_{C^{M-5}_G}
+\|\Ric[G]+\frac29G\|_{C^{14}_G}+a^{-4}\|N\|_{C^{14}_G}+a^{-2}\|N\|_{\dot{C}^{15}_G}+\|N\|_{\dot{C}^{16}_G}\\
\mathcal{C}_\gamma=&\,\|\Psi\|_{C^{17}_\gamma}+\|\nabla\phi\|_{C^{16}_\gamma}+\|\Sigma\|_{C^{16}_\gamma}+\|\RE\|_{C^{16}_\gamma}+\|\RB\|_{C^{16}_\gamma} \numberthis\label{eq:def-C-gamma}\\
...
\begin{subequations}
\begin{align}
\E^{(l)}(\phi,t)=&\,(-1)^l\int_M\Psi\Lap^l\Psi-a^4\phi\Lap^{l+1}\phi\,\vol{G}\\
...
\end{cases}\\
\E^{(l)}(W,t)=&\,(-1)^l\int_M \langle\RE,\Lap^l \RE\rangle_G +\langle\RB,\Lap^l\RB\rangle_G\,\vol{G}\\\E^{(l)}(\Sigma,t)=&\,(-1)^l\int_M\langle\Sigma,\Lap^l\Sigma\rangle_G\,\vol{G}\\
%\E^{(l)}(G,\cdot)&=(-1)^l\int_M\langle G-\gamma,\Lap^l(G-\gamma)\rangle_G\,\vol{G}\\
\E^{(l)}(\Ric,\cdot)=&\,(-1)^l\int_M\left\langle \Ric[G]+\frac29G,\Lap^l\left(\Ric[G]+\frac29G\right)\right\rangle_G\,\vol{G}\\
\E^{(l)}(N,\cdot)=&\,(-1)^l\int_M\langle N,\Lap^lN\rangle_G\,\vol{G}
\end{align}
\end{subequations}
...
\end{definition}
For any $l\in\N_0$ and any smooth functions $f_1,f_2\in C^\infty(\R_+)$, we have
...
\begin{align*}
\E^{(2l+1)}(\Sigma,\cdot)&\,=-\int_M \langle\Lap^l\Sigma,\Lap^{l+1}\Sigma\rangle_G\,\vol{G}\leq \int_M\lvert\Lap^l\Sigma\rvert_G\cdot\lvert\Lap^{l+1}\Sigma\rvert_G\,\vol{G}\\
&\,\leq \sqrt{\E^{(2l)}(\Sigma,\cdot)}\sqrt{\E^{(2l+2)}(\Sigma,\cdot)}\,.
\end{align*}
...
\subsection{Assumptions for initial data}\label{subsec:init}
...
For some small enough $\epsilon\in(0,1)$ and the solution norms $\mathcal{H}$ and $\mathcal{C}$ as in Definition \ref{def:sol-norm}, we assume the rescaled inital data to be close to that of the FLRW solution in Lemma \ref{lem:FLRW} in the following sense:
\begin{subequations}
\begin{equation}\label{eq:init-ass}
\mathcal{H}(t_0)+\mathcal{C}(t_0)\lesssim\epsilon^2
\end{equation}
Additionally, with energies as in Definition \ref{def:energies}, we make the following assumption for top order data:
\begin{align*}
\numberthis\label{eq:init-ass-top}\E^{(20)}(\phi,t_0)+\E^{(20)}(\Sigma,t_0)+\E^{(20)}(W,t_0)+\|\Lap^{10}\phi\|_{L^2_G}^2+\E^{(18)}(\Ric,t_0)&\\
+\E^{(20)}(W,t_0)+a(t_0)^4\E^{(21)}(\phi,t_0)+a(t_0)^4\E^{(21)}(\Sigma,t_0)+a(t_0)^4\E^{(19)}(\Ric,t_0)&\,
...
\end{subequations}
\end{assumption}
\eqref{eq:ass-lwp} is only needed to ensure local well-posedness as sufficiently high regularity (see Lemma \ref{lem:lwp}), hence we break from convention and phrase it in terms of $H_\gamma$-norms for convenience even if we could equivalently replace $H_\gamma$ with $H_G$. The assumptions on $\mathcal{H}$ imply more explicitly:
\begin{align*}
\numberthis\label{eq:init-ass-en}
\E^{(\leq 19)}(\phi,t_0)+\E^{(\leq 18)}(\Sigma,t_0)+\E^{(\leq 18)}(W,t_0)+\E^{(\leq 16)}(\Ric,t_0)&\\
+\|\nabla\phi\|_{H^{18}_G}^2+\E^{(16)}(N,t_0)+a(t_0)^{-2}\E^{(15)}(N,t_0)+a(t_0)^{-4}\E^{(\leq 14)}(N,t_0)
...
\begin{remark}[Initial data size in $C_\gamma(t_0)$]\label{rem:init-Cgamma}
...
Notice that by \eqref{eq:Christoffel-norm-handwaving}, \eqref{eq:init-ass} implies that, on any coordinate neighbourhood $U_{t_0}\subseteq \Sigma_{t_0}$,
\begin{subequations}
\begin{equation}\label{eq:init-ass-Chr-C}
\|\Gamma-\Gamhat\|_{C^{15}_G(U_{t_0})}\lesssim\epsilon^4\,,
%\|\Gamma-\Gamhat\|_{C^{M-5}_G(U_{t_0})}\lesssim\epsilon^4\,,
\end{equation}
and arguing along similar lines and using $L^2-L^\infty$-estimates, also
\begin{equation}
\|\Gamma-\Gamhat\|_{H^{17}_G(U_{t_0})}\lesssim\epsilon^4\,.
...
C_\gamma(t_0)\lesssim\epsilon^2
\end{equation}
We refer to the proof of Lemma \ref{lem:G-gamma-norm-switch} for a more detailed term analysis.
\end{remark}

\begin{remark}[Redundancies in the initial data assumptions]
Similar to Remark \ref{rem:redundancy}, one could also reduce the top order assumptions in \eqref{eq:init-ass-top}. In particular, we highlight that the Bel-Robinson energy can be entirely controlled by the other terms that occur due to the additional scaled $\Sigma$-energy at order $21$, or vice versa we could drop the latter in favour of the former. This will be reflected in Lemma \ref{lem:en-est-Sigma-top}.
\end{remark}
...
Let $N\geq 4$ be an integer and $(M,\mathring{g},\mathring{k},\mathring{\pi},\mathring{\psi})$ be geometric initial data to the Einstein scalar-field system (see Remark \ref{rem:initial-data}) and assume that one has
\[\|\mathring{g}-\gamma\|_{H^{N+1}_\gamma(M)}+\|\mathring{k}+\frac{\tau(t_0)}3\gamma\|_{H^N_\gamma(M)}+\|\mathring{\pi}\|_{H^{N}_{\gamma}(M)}+\|\mathring{\psi}-C\|_{H^N_\gamma(M)}<\infty\,\]
as well as, for some sufficiently small $\eta^\prime>0$,
\[\|\mathring{\psi}-C\|_{C^0_\gamma(M)}\leq\eta^\prime\,.\]
Then, the CMC-transported Einstein scalar-field system (respectively the rescaled system) is locally well-posed in the following sense: The initial data $(\mathring{g},\mathring{k},\mathring{\pi},\mathring{\psi})$ launches a unique classical solution $(g,k,n,\nabla\phi,\del_t\phi)$ to \eqref{eq:Hamilton}-\eqref{eq:Momentum}, \eqref{eq:EEqg}-\eqref{eq:EEqk}, \eqref{eq:wave} and \eqref{eq:EEqLapse} on $[t_1,t_0]\times M$ for some $t_1\in(0,t_0)$ that satisfies ${k^l}_l=-3\frac{\dot{a}}a$ and $n>0$, launches a solution to the Einstein scalar-field system and such that the variables enjoy the following regularity:
\begin{align*}
...
\end{align*}
The rescaled variables $(G,\Sigma,N,\nabla\phi,\Psi)$ enjoy the analogous regularity. If $(\mathfrak{t},t_0]$ is the maximal interval on which the above holds, then either $\mathfrak{t}=0$ holds or one of the following blow-up criteria are satisfied:
\begin{enumerate}
...
Note that the additional initial data requirement in the stiff-fluid setting that the pressure is strictly positive is covered by the smallness assumption on $\mathring{\psi}-C$, since the pressure corresponds to $\lvert\mathring{\psi}\rvert^2+\lvert\mathring{\pi}\rvert_{\mathring{g}}^2$ and the assumptions on $\Psi$ and $\nabla\phi$ ensure that (after embedding) this quantity behaves like $C^2+\O{\epsilon}$ at $\Sigma_{t_0}$ up to a $t_0$-dependent constant.
\end{proof}
...
\end{proof}
Assumption \ref{ass:init} (and in particular \eqref{eq:ass-lwp}) ensures the assumptions of Lemma \ref{lem:lwp} for $N=22$ as long as one chooses $\epsilon$ to be sufficiently small compared to $C$. Hence, Lemma \ref{lem:lwp} and Corollary \ref{cor:lwp-BR} are satisfied at this order, which implies
...
\noindent From here on out, we will tacitly assume all energies to be continuously differentiable on the domain of existence, and similarly all variables to be continuously differentiable for the lower order $C_G$-norm improvements in Section \ref{subsec:AP}.
%one has:
...
\end{equation}
as well as the energy inequalities
\begin{subequations}
\begin{align}
...
\begin{subequations}
\begin{align}
\|\Psi\|_{C^{17}_G}\leq&\,K_0\epsilon a^{-c_0\sigma} \label{eq:BsPsi}\\
\|\nabla\phi\|_{C^{16}_G}\leq&\,K_0\epsilon a^{-c_0\sigma} \label{eq:Bsnablaphi}\\
...
Potentially updating $c_0$  to $c\geq c_0$ in a way that does not depend on $\epsilon$ or $\sigma$, this also implies on any coordinate neighbourhood $U$ that
\begin{equation}\label{eq:BsChr}
\|\Gamma-\Gamhat\|_{C^{15}_G(U)}\leq K_0\epsilon a^{-c_0\sigma}
...
\end{align*}
for any smooth function $\zeta\in C^\infty(\Sigma_t)$, any $\Sigma_t$-tangent tensor $\mathfrak{T}$ and a constant $c>0$. Again, this is essentially a direct consequence of \eqref{eq:Christoffel-norm-handwaving}, and this can be shown just like the improved version in Lemma \ref{lem:G-gamma-norm-switch}. Applying this to each norm in $\mathcal{C}$, we get
...
Our goal is to improve the energy inequalities to
\begin{align*}
\E^{(\leq 19)}(\phi,t)\lesssim&\,\epsilon^{\frac{11}4}a(t)^{-c_1\epsilon^\frac18}\\
\E^{(\leq 18)}(\Sigma,t)\lesssim&\,\epsilon^\frac52a(t)^{-c_1\epsilon^\frac18}\\
\E^{(\leq 18)}(W,t)\lesssim&\,\epsilon^\frac{5}2a(t)^{-c_1\epsilon^\frac18}\\
\E^{(\leq 16)}(\Ric,t)\lesssim&\,\epsilon^\frac94a(t)^{-c_1\epsilon^\frac18}\\
\|\nabla\phi\|_{H^{18}_G}^2\lesssim&\,\epsilon^\frac52a^{-c_1\epsilon^\frac18}\\
\E^{(\leq 16)}(N,t)+a^4\E^{(17)}(N,t)+a^8\E^{(18)}(N,t)\lesssim&\,\epsilon^2a^{8-c_1\epsilon^\frac18}
%\E^{(\leq M-1)}(\phi,t)\lesssim&\,\epsilon^{\frac{11}4}a(t)^{-c_1\epsilon^\frac18}\\
%\E^{(\leq M-2)}(\Sigma,t)\lesssim&\,\epsilon^\frac52a(t)^{-c_1\epsilon^\frac18}\\
%\E^{(\leq M-2)}(W,t)\lesssim&\,\epsilon^\frac{21}8a(t)^{-c_1\epsilon^\frac18}\\
%\E^{(\leq M-4)}(\Ric,t)\lesssim&\,\epsilon^\frac94a(t)^{-c_1\epsilon^\frac18}\\
%\|\nabla\phi\|_{H^{M-3}_G}^2\lesssim&\,\epsilon^\frac94a^{-c_1\epsilon^\frac18}\\
%\E^{(\leq M-4)}(N,t)+a^4\E^{(M-3)}(N,t)+a^8\E^{(M-2)}(N,t)\lesssim&\,\epsilon^2a^{8-c_1\epsilon^\frac18}
\end{align*}
and additionally
\begin{equation*}
\mathcal{C}\lesssim\epsilon^\frac98a^{-c_1\epsilon^\frac18}\,,
\end{equation*}
where $c_1,c_2>0$ are positive constants independent of $\sigma$ and $\epsilon$. Notice how this is actually an improvement if we choose $\sigma$ suitably and then choose $\epsilon$ sufficiently small: Any update between $K_0$ and $K_1$ can be balanced out since we gain at least the additional prefactor $\epsilon^\frac18$ in each estimate, which we can then choose to have been suitably small. Similarly, we improve the power of $a$ if we have $\epsilon^{\frac18}\cdot\sigma^{-1}<\frac{c_0}{c_1}$. If we then retroactively choose $\sigma$ large enough compared to $\epsilon$ but small overall -- for example $\sigma=\epsilon^\frac{1}{16}$ -- and then ensure that $\max\{c_0,c_1\}\epsilon^\frac1{16}<1$ as well as $c_1\epsilon^\frac1{16}<c_0$ are satisfied by choosing $\epsilon$ to have been small enough, we have strictly improved the bootstrap assumptions.
...
Next, we show that the kinetic variables $\Psi$ and $\Sigma$ as well as the rescaled Bel-Robinson variable $\RE$ remain small on the bootstrap interval, which will be essential in dealing with borderline terms for the low level supremum norm estimates in Lemma \ref{lem:AP} as well as throughout the energy argument:
...
\|\RB\|_{C^{11}_G}\lesssim&\,\epsilon a^{1-c\sqrt{\epsilon}}\label{eq:APmidB}\\
...
\begin{align}
\|[\del_t,\nabla^l]\zeta\|_{C^0_G}\lesssim&\,a^{-3}\|N+1\|_{C^{l-1}_G}\|\Sigma\|_{C^{l-1}_G}\|\zeta\|_{C^{l-1}_G}+\frac{\dot{a}}a\|N\|_{C^{l-1}_G}\|\zeta\|_{C^{l-1}_G} \label{eq:aux-comm-est-zeta}\\
\|[\del_t,\nabla^l]\mathfrak{T}\|_{C^0_G}\lesssim&\,a^{-3}\|N+1\|_{C^{l}_G}\left(\|\nabla^l\Sigma\|_{C^0_G}\|\mathfrak{T}\|_{C^0_G}+\|\Sigma\|_{C^{l-1}_G}\|\mathfrak{T}\|_{C^{l-1}_G}\right)+\frac{\dot{a}}a\|N\|_{C^l_G}\|\mathfrak{T}\|_{C^{l-1}_G} \label{eq:aux-comm-est-T}
\end{align}
...
\end{equation}
to be satisfied for some $J\in\{1,\dots,12\}$ (For $J=1$, this is true by \eqref{eq:APSigma}). Observe 
\[\del_t\lvert\nabla^J\Sigma\rvert_G^2=2\langle\del_t\nabla^J\Sigma,\nabla^J\Sigma\rangle_G+\del_tG^{-1}\ast\nabla^J\Sigma\ast\nabla^J\Sigma\]
 and that, up to the commutator, all terms in \eqref{eq:EEq-aux-Sigma} are bounded by $\epsilon a^{-c\sigma}$ for any admissible $J$. Hence and using \eqref{eq:AP-deltG},
...
\underline{\eqref{eq:APmidG}-\eqref{eq:APmidG-1}:} Note that \eqref{eq:AP-deltG} implies the estimate at order $0$ since one has, for example,
...
which we can apply the Gronwall lemma to after integrating as before, along with \eqref{eq:log-est} for the error term.\\
For higher orders, inserting \eqref{eq:APmidSigma} and \eqref{eq:BsN} into \eqref{eq:EEq-aux-G} implies
...
This allows to perform the iteration until we achieve \eqref{eq:APmidG} with $J=12$. The argument for $G^{-1}-\gamma^{-1}$ is completely analogous.\\

\underline{\eqref{eq:APmidphi}:} We only prove the statement for $C^0_G$, the full estimate extends from there by the same iterative arguments as above. Considering \eqref{eq:REEqNablaPhi} and applying Lemma \ref{lem:lapse-maxmin} and \eqref{eq:APPsi}, we have with \eqref{eq:Friedman}
\begin{equation*}
...
\underline{\eqref{eq:APmidB}:} This is obtained immediately from \eqref{eq:EEq-aux-B} using \eqref{eq:APmidSigma}. Notice that the Levi-Civita tensor can be absorbed into the implicit constants since $\lvert\epsilon[G]\rvert=\sqrt{6}$ holds (see \eqref{eq:LCS-contr3}).\\

\underline{\eqref{eq:APmidE}:} This follows like in the proof of \eqref{eq:APE} from applying \eqref{eq:BsRic}, \eqref{eq:Bsnablaphi} and \eqref{eq:APmidSigma} to the constraint equation \eqref{eq:EEq-aux-E}.
...
Let $\mu_G=\sqrt{\det G}$ denote the volume element with regards to $G$. It satisfies
...
\[\del_t\mu_G=\frac1{2\sqrt{\det G}}\del_t\det G=\frac12\sqrt{\det G}(G^{-1})^{ij}\del_tG_{ij}=-N\tau\mu_G\,.\]
...
Finally, we obtain with $\vol{G}=\frac{\mu_G}{\mu_\gamma}\vol{\gamma}$:
\begin{align*}
...
\begin{lemma}[Preliminary Sobolev norm estimates]\label{lem:Sobolev-norm-equivalence-improved} Let $\zeta$ be a scalar function and $\mathfrak{T}$ be a symmetric $\Sigma_t$-tangent $(0,2)$-tensor, and let $l\in\{1,\dots,10\}$. Then, on $(t_{Boot},t_0]$, the following estimates are satisfied:
\begin{subequations}
\begin{align}
\|\nabla^2\zeta\|_{L^2_G}^2\lesssim&\,\|\Lap\zeta\|_{L^2_G}^2+a^{-c\sqrt{\epsilon}}\|\nabla\zeta\|_{L^2_G}^2\label{eq:Sobolev-norm-equiv-zetalow}\\
\|\zeta\|_{H^{2l}_G}^2\lesssim&\,\|\Lap^{l} \zeta\|_{L^2_G}^2+a^{-c\sqrt{\epsilon}}\left(\sum_{m=0}^{l-1}\|\Lap^m\zeta\|_{L^2_G}^2+\underbrace{\|\zeta\|_{C_G^{l-1}}^2\E^{(\leq 2l-3)}(\Ric,\cdot)}_{\text{not present for l=1}}\right) \label{eq:Sobolev-norm-equiv-zeta2l}\\
\sum_{m=1}^{2l+1}\|\zeta\|_{\dot{H}^{m}_G}^2\lesssim&\,\|\nabla\Lap^{l}\zeta\|_{L^2_G}^2+a^{-c\sqrt{\epsilon}}\left(\sum_{m=0}^{l-1}\|\nabla\Lap^m\zeta\|_{L^2_G}^2+\|\nabla\zeta\|_{C_G^{l-1}}^2\E^{(\leq2l-2)}(\Ric,\cdot)\right)\label{eq:Sobolev-norm-equiv-zeta2l+1}\\
\sum_{m=1}^{2l+1}\|\nabla\zeta\|_{\dot{H}^{m}_G}^2\lesssim&\,\|\nabla^2\Lap^l\zeta\|_{L^2_G}^2+a^{-c\sqrt{\epsilon}}\left(\sum_{m=0}^{l-1}\|\nabla^2\Lap^m\zeta\|_{L^2_G}^2+\|\nabla\zeta\|_{C^l_G}^2\E^{(\leq 2l-1)}(\Ric,\cdot)\right)\label{eq:Sobolev-norm-equiv-nablazeta2l+1}
\end{align}
\end{subequations}
and
\begin{subequations}
\begin{align}
\|\mathfrak{T}\|_{H^{2l}_G}^2\lesssim&\,\|\Lap^{l} \mathfrak{T}\|_{L^2_G}^2+a^{-c\sqrt{\epsilon}}\left(\sum_{m=0}^{l-1}\|\Lap^m\mathfrak{T}\|_{L^2_G}^2+\|\mathfrak{T}\|_{C_G^{l-1}}^2\E^{(\leq 2l-2)}(\Ric,\cdot)\right)\label{eq:Sobolev-norm-equiv-T2l}\\
\sum_{m=1}^{2l+1}\|\mathfrak{T}\|_{\dot{H}^{m}_G}^2\lesssim&\,\|\nabla\Lap^{l}\mathfrak{T}\|_{L^2_G}^2+a^{-c\sqrt{\epsilon}}\left(\sum_{m=0}^{l-1}\|\nabla\Lap^m\mathfrak{T}\|_{L^2_G}^2+\|\mathfrak{T}\|_{C_G^{l}}^2\E^{(\leq2l-1)}(\Ric,\cdot)\right)\label{eq:Sobolev-norm-equiv-T2l+1}
\end{align}
\end{subequations}
More precisely, the Ricci energy terms can be dropped in all of the above estimates for $l\leq 5$.
\end{lemma}
\begin{proof} Since the arguments for all of the inequalities above are very similar, we only prove \eqref{eq:Sobolev-norm-equiv-T2l} in full and then adress potential differences for the other estimates.\\
Denoting
\[\tilde{\mathfrak{T}}_{i_1\dots i_{2l}k_1k_2}=\nabla_{i_1}\dots\nabla_{i_{2l}}\mathfrak{T}_{k_1k_2}\,,\]
we compute with the commutator formula \eqref{eq:[Lap,nabla]T} and strong $C_G$-norm estimate \eqref{eq:APmidRic}
\begin{align*}
\int_M|\nabla^2\tilde{\mathfrak{T}}|_G^2=&\,-\int_M\langle\nabla\tilde{\mathfrak{T}},\Lap\nabla\tilde{\mathfrak{T}}\rangle_G\,\vol{G}\\
...
\lesssim& \int_M\lvert\Lap\tilde{\mathfrak{T}}\rvert_G^2\,\vol{G}+(1+\|\Ric[G]+2G\|_{{C}^1_G})\cdot\left[\int_M\lvert\tilde{\mathfrak{T}}\rvert_G^2\,\vol{G}+\int_M\lvert\nabla\tilde{\mathfrak{T}}\rvert_G^2\,\vol{G}\right]\\
\lesssim&\int_M \lvert\Lap\tilde{\mathfrak{T}}\rvert_G^2+a^{-c\sqrt{\epsilon}}\int_M\lvert\tilde{\mathfrak{T}}\rvert_G^2\,\vol{G}
\end{align*}
In the final step, we used integration by parts
...
and updated $c$. This already shows \eqref{eq:Sobolev-norm-equiv-T2l} for $l=1$. Assume now that \eqref{eq:Sobolev-norm-equiv-T2l} holds up to some $l\in\N,\, l\leq 9$ and \textit{any} symmetric $\Sigma_t$-tangent $(0,2)$-tensor field. By applying \eqref{eq:[Lap-l,nabla2]T}, we have
\begin{align*}
\Lap\tilde{\mathfrak{T}}=&\,\Lap\nabla^{2l}\mathfrak{T}\\
=&\,\nabla^{2l}\Lap\mathfrak{T}+[\Lap,\nabla^2]\nabla^{2l-2}\mathfrak{T}+\dots+\nabla^{2l-2}[\Lap,\nabla^2]\mathfrak{T}\\
=&\,\nabla^{2l}\Lap\mathfrak{T}+\sum_{I_\Ric+I_\mathfrak{T}=2l}\nabla^{I_\Ric}\Ric[G]\ast\nabla^{I_\mathfrak{T}}\mathfrak{T}\numberthis\label{eq:lap-sob-equiv-crucial}\\
...
\end{align*}
Here, $(NL)$ summarizes the remaining nonlinearities after successively replacing $\Ric[G]$ by $\Ric[G]+\frac29G$, in which each summand contains at least one term of the form $\nabla^{I_{\Ric}}\left(\Ric[G]+\frac29G\right)$ with $I_{\Ric}>0$. Subsequently, we have using the strong $C_G$-norm estimate \eqref{eq:APmidRic} that
\begin{align*}
\|\mathfrak{T}\|_{\dot{H}^{2(l+1)}_G}^2=\int_M \lvert\nabla^2\tilde{\mathfrak{T}}\rvert_G^2\,\vol{G}\lesssim \|\Lap\mathfrak{T}\|_{\dot{H}^{2l}_G}^2+\left(1+\sqrt{\epsilon}a^{-c\sqrt{\epsilon}}\right)\|\mathfrak{T}\|_{H^{2l}_G}^2+\|\mathfrak{T}\|_{C^l_G}^2\|\Ric[G]+\frac29G\|_{H^{2l}_G}^2\,.
\end{align*}
By assumption, we can estimate $\|\Lap\mathfrak{T}\|_{H^{2l}_G}^2$, $\|\mathfrak{T}\|_{H^{2l}_G}^2$ and $\|\Ric[G]+2G\|_{H^{2l}_G}^2$ as in \eqref{eq:Sobolev-norm-equiv-T2l}, and get the following:
\begin{align*}
\int_M\lvert\nabla^{2l+2}\mathfrak{T}\rvert_G^2\,\vol{G}\lesssim&\,\left[\|\Lap^{l}\Lap\mathfrak{T}\|^2_{L^2_G}+a^{-c\sqrt{\epsilon}}\sum_{m=0}^{l-1}\|\Lap^{m+1}\mathfrak{T}\|_{L^2_G}^2+\|\Lap\mathfrak{T}\|_{C^{l-1}_G}^2\E^{(\leq 2l-2)}(\Ric,\cdot)\right]\\
&\,+\left[a^{-c\sqrt{\epsilon}}\|\Lap^l\mathfrak{T}\|_{L^2_G}^2+a^{-c\sqrt{\epsilon}}\sum_{m=0}^{l-1}\|\Lap^{m}\mathfrak{T}\|^2_{L^2_G}+a^{-c\sqrt{\epsilon}}\|\mathfrak{T}\|_{C^{l-1}_G}^2\E^{(\leq 2l-2)}(\Ric,\cdot)\right]\\
&\,+a^{-c\sqrt{\epsilon}}\|\mathfrak{T}\|_{C^{l}_G}^2\left[\E^{(2l)}(\Ric,\cdot)+\left(\E^{(\leq 2l-2)}(\Ric,\cdot)+\epsilon a^{-c\sqrt{\epsilon}}\E^{(\leq 2l-2)}(\Ric,\cdot)\right)\right]\\
\lesssim&\,\|\Lap^{l+1}\mathfrak{T}\|_{L^2_G}^2+a^{-c\sqrt{\epsilon}}\left(\sum_{m=0}^l\|\Lap^m\mathfrak{T}\|_{L^2_G}^2+\|\mathfrak{T}\|_{C^{l}_G}^2\E^{(\leq 2l)}(\Ric,\cdot)\right)
\end{align*}
...
where the latter tensor is precisely $\Lap\tilde{T}$ which we just treated, and the former is covered by the assumption at order $2l$. So, \eqref{eq:Sobolev-norm-equiv-T2l} now follows for $l+1$, and thus by iteration up to $l=10$.\\
The proof of \eqref{eq:Sobolev-norm-equiv-T2l+1} is analogous -- we note that since we actually only needed a strong estimate on $\|\Ric[G]+2G\|_{C^9_G}$ for the previous inequality, but \eqref{eq:APmidRic} holds at $C^{10}_G$, this gives enough room to extend the argument in full despite the extra derivative order.\\

For both, note that we only need to estimate the Ricci terms in the $L^2_G$-norm if one can't apply the a priori estimate \eqref{eq:APmidRic} to all $\nabla^{I_\Ric}\Ric[G]$ that occur in \eqref{eq:lap-sob-equiv-crucial}, and thus we could easily adjust the proof such that the Ricci energy does not occur in any of the proofs as long as $2l-1\leq 10$ is satisfied, so for $l\leq 5$.\\

The estimates \eqref{eq:Sobolev-norm-equiv-zeta2l}-\eqref{eq:Sobolev-norm-equiv-nablazeta2l+1} are proved identically, the only difference being that one order of curvature less enters in the commutator terms in \eqref{eq:lap-sob-equiv-crucial}, leading to one order less in curvature in total.\\

For \eqref{eq:Sobolev-norm-equiv-zetalow}, we note that we can avoid incurring any $L^2$-norm by carefully repeating the argument we made for $\tilde{\mathfrak{T}}$ using \eqref{eq:[Lap,nabla]SF}:
\begin{align*}
\int_M\lvert\nabla^2\zeta\rvert_G^2\,\vol{G}=&\,-\int_M\langle\nabla\zeta,\Lap\nabla\zeta\rangle_G\,\vol{G}\\
=&\,\int_M-\langle\nabla\zeta,\nabla\Lap\zeta\rangle_G+\int_M\Ric[G]\ast\nabla\zeta\ast\nabla\zeta\,\vol{G}\\
\lesssim&\,\int_M\lvert\Lap\zeta\rvert_G^2+a^{-c\sqrt{\epsilon}}\int_M\lvert\nabla\zeta\rvert_G^2\,\vol{G}
\end{align*}
\end{proof}

\begin{remark}
We quickly stress that Lemma \ref{lem:Sobolev-norm-equivalence-improved} is crucial for everything after this point in multiple ways:\\

Firstly, the $L^2_G$-norms containing $\zeta$ and $\mathfrak{T}$ on the right hand sides of all inequalities above except \ref{eq:Sobolev-norm-equiv-nablazeta2l+1} are in precisely the form the energies in Definition \ref{def:energies} take. Hence, this is what will actually yield coercivity of these energies up to order $21$ since we can apply the a priori estimates from Lemma \ref{lem:AP} (or, in the case of the lapse, just \eqref{eq:BsN}) up to $l=10$ for any of the solution variables, and thus is sufficient in controlling the norms in $\mathcal{H}$ by energies (see Definition \ref{def:sol-norm}). This will be shown more explicity as an intermediary step in improving the bootstrap assumptions for $\mathcal{C}$ (see proof of Corollary \ref{cor:H-imp}).\\
...
Finally, some top order terms also do not appear in a way that their $L^2$-norm is directly the square root of an energy (see, for example, the term $a\nabla^2\Lap^{\frac{L}2}N$ in \eqref{eq:comeq-Sigma}), and some borderline terms would lead to nonintegrable divergences at that level of derivatives if we were to incur additional divergences in estimation (see, for example, the first term in \eqref{eq:comeq-Sigma-border}). Lemma \ref{lem:Sobolev-norm-equivalence-improved} precisely provides a way to relate these terms to energies without losing precision at top order in terms of powers of $a$. Additionally, by applying the estimate for terms of the form $\Lap^\frac{L}2\zeta$ and $\Lap^\frac{L}2\mathfrak{T}$, one can avoid high order curvature energies when dealing with such terms, gaining sufficient precision to control commutator terms.
\end{remark}

\section{Big Bang stability: Elliptic lapse estimates}\label{sec:lapse}

In this section, we study the elliptic structure of the equations \eqref{eq:REEqLapse1}-\eqref{eq:REEqLapse2}, which admit estimates controlling (time-scaled) lapse energies by other energy quantities. To this end, we abstract these equations as follows:


\begin{definition}[Elliptic operators] For any (sufficiently regular) scalar function $\zeta$ on $\Sigma_t$, we define the following differential operators:
\begin{subequations}
\begin{align}
...
\begin{subequations}
\begin{align}
\label{eq:[L,Lap]}[\L,\Lap]\zeta=&\,\Lap f\cdot\zeta+2\langle\nabla f,\nabla\zeta\rangle_G=\nabla F\cdot\zeta+2\langle\nabla F,\nabla\zeta\rangle_G\\
\label{eq:[Ltilde,Lap]}[\Ltilde,\Lap]\zeta=&\,\nabla \tilde{F}\cdot\zeta+2\langle\nabla \tilde{F},\nabla\zeta\rangle_G
\end{align}
\end{subequations}
%and
...
\begin{proof} We first test \eqref{eq:ell-L-eq} with $-\zeta$ and integrate:
...
Hence, after rearranging and using that $f-6\pi C^2$ is bounded from below by a positive constant (see the first point in Remark \ref{ass:lapse}), we compute
\[\int_M (a^4\lvert\nabla \zeta\rvert_G^2+\lvert\zeta\rvert^2)\vol{G}\lesssim \|Z\|^2_{L^2_G}\,.\]
Next, we test \eqref{eq:ell-L-eq} with $a^4\Lap \zeta$, integrate and use integration by parts:
\begin{align*}
...
The following estimates hold for any $l\in\{0,\dots,11\}$:
\begin{align*}
...
\end{corollary}
\begin{proof}
Once again, we prove the statement by induction over $l\in\N$: For $l=0$, the estimates immediately follow from \eqref{eq:lapse-with-op} and Lemma \ref{lem:ell-lapse-1}. %, respectively \eqref{eq:lapse-with-op-tilde} and Lemma \ref{lem:ell-lapse-2}.
Assume the statement to be satisfied up to $l-1$ for some $l\in\N_0,\,l\leq 11$. We have applying \eqref{eq:[L,Lap]} iteratively:
\begin{align*}
...
Hence, using \eqref{eq:Sobolev-norm-equiv-zeta2l+1} (replacing $l$ with $l-1$) and again \eqref{eq:BsN}, we get
\begin{align*}
...
\end{equation*}
The lower order lapse terms as well as $\|\nabla\Lap^{l-1}F\|_{L^2_G}$ can be estimated similarly, just without having to redistribute the prefactor. Updating $c>0$, we get for some $K>0$:
\begin{align*}
...
For any $l\in\{0,\dots,11\}$, one has
\begin{align*}
...
Note that, by Corollary \ref{cor:ell-lapse-L}, all that needs to be done is to relate all $L^2$- and Sobolev norms of $F$ that occur to the respective energies. \\
Schematically, we have
\[\Lap^{l}F=16\pi(\Lap^l\Psi)(\Psi+C)+2\langle\Lap^l\Sigma,\Sigma\rangle_G+\sum_{I=1}^{2l-1}\left(\nabla^I\Psi\ast\nabla^{2l-I}\Psi+\nabla^I\Sigma\ast\nabla^{2l-I}\Sigma\right)\,.\]
For the first two terms, we can use \eqref{eq:APPsi} and \eqref{eq:APSigma} respectively to bound $\lvert\Sigma\rvert_G$ and $\lvert\Psi+C\rvert$ by $\epsilon$ and $1$ up to constant, respectively. For the remaining terms, we similarly always bound the lower order in $L^\infty_G$ with \eqref{eq:APmidPsi}-\eqref{eq:APmidSigma}. Note that we can estimate the term caused by $\|\nabla\Sigma\ast\nabla^{2l-1}\Sigma\|_{L^2_G}^2$ with \eqref{eq:APmidSigma} and \eqref{eq:Sobolev-norm-equiv-T2l+1} as
\begin{align*}
...
\end{align*}
and can deal the equivalent term arising from $\Psi$ similarly. Regarding the Sobolev norms of $F$ up to order $2l-2$ in Corollary \ref{cor:ell-lapse-L}, we get weaker error terms by similar arguments.
...
While the estimates in the previous subsection are useful at high orders, they aren't enough to close the bootstrap assumptions for $N$. This can be achieved by deriving estimates in terms of $\Ltilde$ -- however, due to the explicit presence of Ricci terms in this version of the lapse equation, we use this to bound $N$ at high orders. Since the arguments are largely identical to the ones above, we only sketch the proofs.

\begin{remark}\label{rem:L-Ltilde-bridge}
Note that, when replacing $f$ by $\tilde{f}$ and $F$ by $\tilde{F}$ in Remark \ref{ass:lapse}, the same statements hold for a suitable constant $K$. In fact, the bootstrap assumptions on $\Ric[G]$ and $\nabla\phi$ even imply $\|\tilde{F}\|_{C^1_G}\lesssim \epsilon a^{4-c\sigma}$, noting $\lvert R[G]+\nicefrac23\rvert_G\leq\lvert G^{-1}\rvert_G\lvert\Ric[G]+\nicefrac29G\rvert_G\lesssim \lvert\Ric[G]+\nicefrac29G\rvert_G$ and $\lvert\nabla R[G]\rvert_G=\lvert\nabla(R[G]+\nicefrac23)\rvert_G\lesssim \lvert\Ric[G]+\nicefrac29G\rvert_G$.%, which more than ensures the first two points, while we just copy the third point, and the fourth can once again be ensured by choosing $\epsilon$ sufficiently small. 
\end{remark}
...
\begin{corollary}For $l\in\{0,\dots,12\}$, the following holds:
...
\end{align*}
\end{corollary}
\begin{proof} We again go through the same iterative procedure, where we assume the estimate to hold up to $l-1$, and noting that $l=0$ follows immediately from Lemma \ref{lem:ell-lapse-Ltilde}, \eqref{eq:APmidphi} and $\lvert R[G]+\nicefrac23\rvert\leq \sqrt{3}\lvert\Ric[G]+\nicefrac29G\rvert_G$.\\
...
%We compute
%\begin{align*}
%\Lap^l\tilde{F}=&\,a^4\left[\Lap^l(R[G]+6)+\sum_{I_1+I_2=2l}\nabla^{I_1+1}\phi\ast\nabla^{I_2+1}\phi\right]\\
%=&\,a^4\left[(G^{-1})^{mn}(\Lap^l(\Ric[G]+2G)_{mn})+\sum_{I_1+I_2=2l}\nabla^{I_1+1}\phi\ast\nabla^{I_2+1}\phi\right]\,.
%\end{align*}
%Hence, we get the statement using \eqref{eq:APmidphi} and by utilizing Lemma \ref{lem:ell-lapse-Ltilde} repeatedly.
\end{proof}
%\begin{remark}
...
In this section, we derive energy estimates for matter variables and the geometric quantities as well as Sobolev norm estimates for $\Lap^\frac{L}2\phi$ and metric quantities. 

In deriving all of the inequalities in this section beside the elliptic inequality in Lemma \ref{lem:en-est-Sigma-top}, we will use the same basic strategy. Hence, we give a brief overview on the form our integral inequalities are going to take and how we intend to obtain bootstrap improvements (see Remark \ref{rem:bs-strategy}) from there:

\begin{remark}[Integral inequalities and the bootstrap argument]\label{rem:en-est-strat}
...
&\,+\underbrace{\int_t^{t_0}a(s)^{-1}\langle\text{energies at derivative level }L+1\rangle\,ds}_{\text{not present at top order}}\\
...
For some inequalities, we won't be able to derive any additional $\epsilon$-prefactors in the penultimate line. However, combining the respective inequalities for the core energy mechanism at each derivative level with appropriate scaling, this will then combine to an inequality of the following form for a total energy, which we informally denote by $\mathcal{F}_{\text{total},L}$:
...
&\,+\underbrace{\int_t^{t_0}\epsilon^\frac14a(s)^{-1}\langle\text{energies at derivative level }L+1\rangle\,ds}_{\text{not present at top order}}\\
...
Applying the Gronwall lemma (see Lemma \ref{lem:gronwall}) and the initial data assumption (which implies $\mathcal{F}_{\text{total},L}\lesssim\epsilon^4$) then yields

\begin{align*}
\mathcal{F}_{\text{total},L}\lesssim&\,\biggr(\epsilon^4+\underbrace{\int_t^{t_0}\epsilon^\frac18 a(s)^{-3-c\epsilon^\frac18}\langle\text{already improved terms}\rangle\,ds}_{\text{not present for }L=0}\\
&\,+\underbrace{\int_t^{t_0}\epsilon^\frac14a(s)^{-1}\langle\text{energies at derivative level }L+1}_{\text{not present at top order}}\rangle\,ds\biggr)\cdot\\
&\,\cdot\exp\left(K\cdot \int_t^{t_0}\epsilon^\frac18a(s)^{-3}+a(s)^{-1-c\sqrt{\epsilon}}\,ds\right)
\end{align*}
for some constant $K>0$. By \eqref{eq:log-est} and \eqref{eq:a-integrals}, the exponential factor in the final line can be bounded by $a^{-c\epsilon^\frac18}$, up to constant and updating $c>0$.
Regarding the terms at level $L+1$, we can insert the bootstrap assumptions for energies and Sobolev norms (see \eqref{eq:BsEnSF}-\eqref{eq:BsEnN}), and since $a^{-1-c\sigma}$ is integrable, this results in an error term that is at worst bounded by $\epsilon^{2+\frac14}=\epsilon^\frac94$. In fact, it will turn out to be bounded by $\epsilon^\frac{11}4$ due to the precise nature of these terms and scaling hierarchy in \eqref{eq:BsEnSF}-\eqref{eq:BsEnRic}. Hence, for $L=0$, this implies $\mathcal{F}_{\text{total},0}\lesssim\epsilon^\frac{11}4a^{-c\epsilon^\frac18}$, and thus leads to improved bounds for base level energy quantities (see Remark \ref{rem:bs-strategy} for the precise scaling hierarchy that will achieve). 
By iterating this argument for $L>0$, the already improved terms will then be bounded (at worst) by $\epsilon^\frac{11}4a^{-c\epsilon^\frac18}$ , and \eqref{eq:a-integrals} shows that the first line can be bounded by $\epsilon^{\frac{11}4}a^{-c\epsilon^\frac18}$ after updating $c$. The second line, if present, is then handled as before, allowing to bound $\mathcal{F}_{\text{total},L}$ by $\epsilon^\frac{11}4a^{-c\epsilon^\frac18}$ for any $L$ up to and including top order.\\

In summary, before top order, the scaling hierarchy in the bootstrap assumptions will keep error terms from the next derivative level small enough that the total energy can be improved, and this can be repeated iteratively over even derivative levels until top order.\\
At top order, we will be able to use the additional estimate in Lemma \ref{lem:en-est-Sigma-top} to bound these next level terms by the total energy quantity itself, thus eliminating these error terms and closing the argument. Instead, using this elliptic estimate incurs an additional term of the form
\[\epsilon^\frac14\mathcal{F}_{top}(t)+\epsilon^3 a(t)^{2-c\epsilon^\frac18}\]
on the right hand side of \eqref{eq:toy-energy}, given appropriate $\epsilon$-weights on the highest order terms within $\mathcal{F}_{top}(t)$ and using the already obtained improvements before top order. The former can be absorbed in the left hand side by adjusting the implicit constant in \enquote{$\lesssim$} for $\epsilon>0$ small enough, and the latter is dominated by $\epsilon^\frac{11}4a(t)^{-c\epsilon^\frac18}$. Otherwise, the argument at top order goes through just as it does before top order.
\end{remark}
\vspace{1em}
Recall that $L^2_G$-norm estimates for error terms arising in the Laplace-commuted equations in Lemma \ref{lem:laplace-commuted-eq} are collected in Section \ref{subsec:L2-error-est}. Low order estimates (in particular estimates for $L=2$) could often be improved if needed by more carefully avoiding curvature error terms, but we refrain from doing so where it is not necessary to keep estimates as unified as possible. Moreover, from here on out, we use $M$ to denote energy estimates at the top derivative level to easily distinguish them from lower levels while keeping the estimates easy to compare. We will later explicitly choose $M=20$, but will only use that it is even and between $4$ and $20$ until then.
...
[Scalar field energy estimates before top order] Let $t\in(t_{Boot},t_0]$. Then, one has
\begin{align*}
\numberthis\label{eq:en-est-SF0}&\,\E^{(0)}(\phi,t)+\int_t^{t_0}\dot{a}(s)a(s)^{3}\E^{(1)}(N,s)+\frac{\dot{a}(s)}{a(s)}\E^{(0)}(N,s)\,ds\\
\lesssim&\,\E^{(0)}(\phi,t_0)+\int_t^{t_0}\epsilon a(s)^{-3}\E^{(0)}(\phi,s)+\epsilon a^{-3}\E^{(L)}(\Sigma,s)\,ds\,.
\end{align*}
Further, for any $L\in 2\N,\,2\leq L\leq 20$, the following estimate is satisfied: 
\begin{align*}
\numberthis\label{eq:en-est-SF}&\,\E^{(L)}(\phi,t)+\int_t^{t_0}\dot{a}(s)a(s)^{3}\E^{(L+1)}(N,s)+\frac{\dot{a}(s)}{a(s)}\E^{(L)}(N,s)\,ds\\
\lesssim&\,\E^{(L)}(\phi,t_0)+\int_t^{t_0}\left(\sqrt{\epsilon}a(s)^{-3}+a(s)^{-1-c\sqrt{\epsilon}}\right)\E^{(L)}(\phi,s)\,ds\\
&\,+\int_t^{t_0}\epsilon a(s)^{-3}\E^{(L)}(\Sigma,s)+\epsilon^\frac32 a(s)^{-3}\E^{(L-2)}(\Ric,\cdot)\,ds\\
&\,+\int_t^{t_0}\sqrt{\epsilon}a(s)^{-3-c\sqrt{\epsilon}}\E^{(\leq L-2)}(\phi,s)+\epsilon a(s)^{-3-c\sqrt{\epsilon}}\E^{(\leq L-2)}(\Sigma,s)\,ds\\
&\,+\int_t^{t_0}\langle \text{SF-Err}\rangle_L(s)\,ds
\end{align*}
with
\begin{equation*}
...
\end{align*}
Note that the first line has a negative sign, so (after absorbing a few terms into it without changing the sign, see namely lapse quantities in \eqref{eq:en-est-SF-Friedman-ineq} and \eqref{eq:diff-ineq-SF1}), we pull it to the left hand side of the differential equality. Further, the first term in the second line of \eqref{eq:SF-en-lapse-trick} precisely cancels the third term in \eqref{eq:diff-eq-SF1}. That leaves the borderline and junk terms in \eqref{eq:SF-en-lapse-trick}, for which we use \eqref{eq:L2-Border-N} and \eqref{eq:L2-junk-N} (along with $\dot{a}\simeq a^{-2}$ due to \eqref{eq:Friedman}) to get for $L\geq 4$:
\begin{align*}
...
\end{align*}
Again, the estimate holds for $L=0$ if the last two lines are dropped.
...
To bound the error terms in \eqref{eq:diff-eq-SF3}, we insert the the borderline term estimates \eqref{eq:L2-Border-P-even} and \eqref{eq:L2-Border-Q-even} as well as the junk term estimates \eqref{eq:L2-junk-P-even} and \eqref{eq:L2-junk-Q-even}, where \eqref{eq:ibp-trick} is used to estimate odd order by even order energies where needed. Furthermore, observe that we can estimate the $\mathfrak{Q}_L$-terms as
\[\left(a^2\|\mathfrak{Q}_L\|_{L^2_G}\right)\cdot \sqrt{\E^{(L)}(\phi,\cdot)},\]
so all borderline and junk terms arising from it, beside the scalar field norms, are dominated by terms occurring elsewhere.\\
Finally, all terms that remain, namely \eqref{eq:diff-eq-SF4} and the first term in \eqref{eq:diff-eq-SF5}, can be bounded by $\epsilon a^{-3}\E^{(L)}(\phi,\cdot)$ thanks to the strong base level estimate \eqref{eq:APSigma} and \eqref{eq:BsN}. In summary, and always only keeping the worst terms for each energy and squared norm, this yields for $L\geq 4$:
\begin{subequations}
\begin{align*}
&-\del_t\E^{(L)}(\phi,\cdot)+\dot{a}a^{3}\E^{(L+1)}(N,\cdot)+\frac{\dot{a}}a\E^{(L)}(N,\cdot)\\
\numberthis\label{eq:diff-ineq-SF1}\lesssim&\,\left(\sqrt{\epsilon}a^{-3}+a^{-1-c\sqrt{\epsilon}}\right)\E^{(L)}(\phi,\cdot)+\left(\epsilon a^{-3}+\sqrt{\epsilon}a^{-1-c\sqrt{\epsilon}}\right)\left(a^4\E^{(L+1)}(N,\cdot)+\E^{(L)}(N,\cdot)\right)\\
\numberthis\label{eq:diff-ineq-SF2}&\,+\epsilon a^{-3}\E^{(L)}(\Sigma,\cdot)+\epsilon^\frac32 a^{-3}\E^{(L-2)}(\Ric,\cdot)\\
\numberthis\label{eq:diff-ineq-SF3}&\,+\sqrt{\epsilon}a^{-3-c\sqrt{\epsilon}}\E^{(\leq L-2)}(\phi,\cdot)+\epsilon^\frac32a^{-3-c\sqrt{\epsilon}}\|\nabla\Lap^{\frac{L}2-1}\phi\|^2_{L^2_G}+\epsilon a^{-1-c\sqrt{\epsilon}}\|\nabla\phi\|_{H^{L-2}_G}^2\\
\numberthis\label{eq:diff-ineq-SF4}&\,+\epsilon a^{-3-c\sqrt{\epsilon}}\E^{(\leq L-2)}(\Sigma,\cdot)+\left[\epsilon a^{-3-c\sqrt{\epsilon}}+\sqrt{\epsilon}a^{-1-c\sqrt{\epsilon}}\right]\E^{(\leq L-2)}(N,\cdot)\\
\numberthis\label{eq:diff-ineq-SF5}&\,+\underbrace{\epsilon^\frac32a^{-3-c\sqrt{\epsilon}}\|\nabla\phi\|^2_{H^{L-3}_G}+\epsilon^\frac32a^{-3-c\sqrt{\epsilon}}\E^{(\leq L-4)}(\Ric,\cdot)}_{\text{not present for }L=2}
\end{align*}
\end{subequations}
The lapse energies in \eqref{eq:diff-ineq-SF1} can now also be absorbed into those on the left hand side of the inequality by updating the implicit constant in \enquote{$\lesssim$}. We can treat the lower order lapse energies in \eqref{eq:diff-ineq-SF4} with Corollary \ref{cor:en-est-lapse} and see that the resulting terms are all dominated by terms we already have on the right hand side of the inequality above.\\

For $L\geq 4$, we can estimate the second term in \eqref{eq:diff-ineq-SF3} (updating $c$) by
...
up to curvature terms dominated by those in \eqref{eq:diff-ineq-SF5}, and the first term can be dealt with as before. Consequently, for $L\geq 4$, the scalar field norms in \eqref{eq:diff-ineq-SF3} and \eqref{eq:diff-ineq-SF5} can be replaced by
...
As mentioned earlier, \eqref{eq:diff-eq-SF3} is replaced by the following term for $L=0$:
...
To close the argument at top order, we will have to use an elliptic estimate on $\Sigma$-energies (see Lemma \ref{lem:en-est-Sigma-top}). However, this results in only being able to bound $a^4\E^{(M+1)}(\Sigma,\cdot)$ by a small and slightly divergent factor and not the energy itself, so we can also only control $a^4\E^{(M+1)}(\phi,\cdot)$ in that fashion, but will also need to gain some control on this scalar field energy. Since odd orders are not covered by the previous lemma, we need to derive this estimate separately; however, the argument is structurally mostly unchanged:

\begin{lemma}[Top order scalar field energy estimate]\label{lem:en-est-SF-top}
For $M\in2\N$, $4\leq M\leq 20$, we have:
\begin{align*}
\numberthis\label{eq:en-est-SF-top}&\,a(t)^4\E^{(M+1)}(\phi,t)+\int_t^{t_0}\left\{\dot{a}(s)a(s)^7\E^{(M+2)}(N,s)+\dot{a}(s)a(s)^3\E^{(M+1)}(N,s)\right\}\,ds\\
\lesssim&\,a(t_0)^4\E^{(M+1)}(\phi,t_0)+\int_t^{t_0}\left(\epsilon a(s)^{-3}+a(s)^{-1-c\sqrt{\epsilon}}\right)\cdot a(s)^4\E^{(M+1)}(\phi,s)\,ds\\
&\,+\int_t^{t_0}\left\{\epsilon a(s)^{-3}\cdot a^4\E^{(M+1)}(\Sigma,s)+\left(\epsilon a(s)^{-3}+a(s)^{-1-c\sqrt{\epsilon}}\right)\E^{(M)}(\phi,s)+\epsilon a(s)^{-3}\E^{(M)}(\Sigma,s)\right.\\
&\,\phantom{+\int_t^{t_0}}+\epsilon a(s)^{-1-c\sqrt{\epsilon}}\cdot a(s)^4\E^{(M-1)}(\Ric,s)+\left(\epsilon^3a^{-3}+\epsilon a^{-1-c\sqrt{\epsilon}}\right)\E^{(M-2)}(\Ric,s)\\
&\,\phantom{+\int_t^{t_0}}+{\epsilon}a(s)^{-3-c\sqrt{\epsilon}}\left(\E^{(\leq M-2)}(\phi,s)+\E^{(\leq M-2)}(\Sigma,s)\right)\\
&\,\phantom{+\int_t^{t_0}}+\epsilon^2 a(s)^{-1-c\sqrt{\epsilon}}\left(\|\nabla^2\Lap^{\frac{M}2-1}\phi\|_{L^2_G(\Sigma_s)}^2+\|\nabla\phi\|_{H^{M-2}_G(\Sigma_s)}^2\right)+\epsilon a(s)^{3-c\sqrt{\epsilon}}\cdot \|\nabla\phi\|_{H^M_G(\Sigma_s)}^2\\
&\,\phantom{+\int_t^{t_0}}\left.+\left(\epsilon^3a(s)^{-3-c\sqrt{\epsilon}}+\epsilon^2a(s)^{-1-c\sqrt{\epsilon}}\right)\E^{(\leq M-4)}(\Ric,s)\right\}\,ds
\end{align*}
\end{lemma}
\begin{remark}
...
\begin{lemma}[Bel-Robinson energy estimates before top order]\label{lem:en-est-BR} Let $t\in(t_{Boot},t_0]$. Then one has
\begin{align*}
\numberthis\label{eq:en-est-BR0}&\,\E^{(0)}(W,t)+\int_t^{t_0}\int_M\left[8\pi C^2a(s)^{-3}(N+1)\langle\Sigma,\RE\rangle_G+6\frac{\dot{a}(s)}{a(s)}(N+1)\lvert\RE\rvert_G^2\right]\,\vol{G}\,ds\\
\lesssim&\,\E^{(0)}(W,t_0)+\int_t^{t_0}\left({\epsilon}a(s)^{-3}+a(s)^{-1-c\sqrt{\epsilon}}\right)\E^{(0)}(W,s)+a(s)^{-1}\E^{(1)}(\phi,s)\,ds\\
&\,\phantom{\E^{(0)}(W,t_0)}+\int_t^{t_0}\epsilon a(s)^{-1-c\sqrt{\epsilon}}\E^{(0)}(\phi,s)+\epsilon a(s)^{-3}\E^{(0)}(\Sigma,s)\,ds
\end{align*}
as well as, for $L\in 2\N,\,2\leq L\leq 20$,
\begin{align*}
\numberthis\label{eq:en-est-BR}&\,\E^{(L)}(W,t)+\int_t^{t_0}\int_M\left[8\pi C^2a(s)^{-3}\langle\Lap^\frac{L}2\Sigma,\Lap^\frac{L}2\RE\rangle_G+6\frac{\dot{a}(s)}{a(s)}\int_M(N+1)\lvert\Lap^\frac{L}2\RE\rvert_G^2\right]\,\vol{G}\,ds\\
\lesssim&\,\E^{(L)}(W,t_0)+\int_t^{t_0}\left(\epsilon a(s)^{-3}+a(s)^{-1-c\sqrt{\epsilon}}\right)\E^{(L)}(W,s)\,ds\\
&\,+\int_t^{t_0}\Bigr\{a(s)^{-1}\E^{(L+1)}(\phi,s)+\left(\epsilon a(s)^{-3}+a(s)^{-1}\right)\E^{(L)}(\phi,s)+\epsilon a(s)^{-3}\E^{(L)}(\Sigma,s)\\
&\,\phantom{\int_t^{t_0}}+\epsilon a(s)^{-1}\E^{(L-1)}(\Ric,\cdot)+\epsilon^3 a(s)^{-3}\E^{(L-2)}(\Ric,s)\\
&\,\phantom{\int_t^{t_0}}+\left(\epsilon a(s)^{-3-c\sqrt{\epsilon}}+a(s)^{-1-c\sqrt{\epsilon}}\right)\E^{(\leq L-2)}(\phi,s)+\epsilon a(s)^{-3-c\sqrt{\epsilon}}\left(\E^{(\leq L-2)}(\Sigma,s)+\E^{(\leq L-2)}(W,s)\right)\\
&\,\phantom{\int_t^{t_0}}+\underbrace{\epsilon a(s)^{-3-c\sqrt{\epsilon}}\E^{(\leq L-4)}(\Ric,s)}_{\text{not present for }L=2}\Bigr\}\,ds\,.
\end{align*}
\end{lemma}
\begin{remark}
We preemptively note that the error terms on the left hand side, once combined with the similar terms on the left hand side in Lemma \ref{lem:en-est-Sigma} and given suitable weights, will turn out to have positive sign and can then be dropped, even if they do not have definite sign in isolation.\\

The main idea in deriving this inequality is that we can use the algebraic identity \eqref{eq:div-to-curl} and integration by parts to exploit the Maxwell system that lies at the core of the Bel-Robinson evolution equations. As a result, we avoid having higher order energies of the Bel-Robinson variables on the right hand side of the integral energy inequalities (which would break the bootstrap argument), then only having to deal with scalar field and Ricci energies at the next derivative level. Below top order, the bootstrap assumptions will then suffice to bound these terms.
\end{remark}
\begin{proof}
...
&\,\phantom{\int_M}+2(N+1)\left(\langle\curl\Lap^{\frac{L}2}\RE,\Lap^{\frac{L}2}\RB\rangle_G-\langle\curl\Lap^{\frac{L}2}\RB,\Lap^{\frac{L}2}\RE\rangle_G\right)\label{eq:en-eq-BR2}\\
&\,\phantom{\int_M}+2\left(\langle\nabla\Lap^\frac{L}2N\wedge\RB,\Lap^\frac{L}2\RE\rangle_G-\langle\nabla\Lap^{\frac{L}2}N\wedge\RE,\Lap^{\frac{L}2}\RB\rangle_G\right)\label{eq:en-eq-BR3}\\
...
\end{align}
\end{subequations}
For \eqref{eq:en-eq-BR1}, we pull $6(N+1)\frac{\dot{a}}a\lvert\Lap^{\frac{L}2}\RE\rvert_G^2$ to the left. This leaves
...
Regarding \eqref{eq:en-eq-BR2}, note that by \eqref{eq:div-to-curl}, we have
\begin{align*}
\langle\curl\Lap^{\frac{L}2}\RE,\Lap^{\frac{L}2}\RB\rangle_G-&\,\langle\curl\Lap^{\frac{L}2}\RB,\Lap^{\frac{L}2}\RE\rangle_G=a^4\left(\curl\Lap^{\frac{L}2}\RE\cdot\Lap^{\frac{L}2}\RB-\curl\Lap^{\frac{L}2}\RB\cdot\Lap^{\frac{L}2}\RE\right)\\
=&\,-a^4\div_g\left(\Lap^{\frac{L}2}\RE\wedge\Lap^{\frac{L}2}\RB\right)=-a^2\div_G\left(\Lap^{\frac{L}2}\RE\wedge\Lap^{\frac{L}2}\RB\right).
\end{align*}
...
\begin{align*}
&\left\lvert\int_M 2a^2(N+1)\div_G\left(\Lap^{\frac{L}2}\RE\wedge\Lap^{\frac{L}2}\RB\right)\,\vol{G}\right\rvert\\
=&\left\lvert\int_M2a^2\langle\nabla N,\Lap^{\frac{L}2}\RE\wedge\Lap^{\frac{L}2}\RB\rangle_G\,\vol{G}\right\rvert\\
...
Moving on to \eqref{eq:en-eq-BR5}-\eqref{eq:en-eq-BR7}, we see [using \eqref{eq:APPsi} and \eqref{eq:APmidphi}, \eqref{eq:Sobolev-norm-equiv-zetalow} with $\zeta=\Lap^\frac{L}2\phi$ as well as \eqref{eq:ibp-trick}]:
\begin{align*}
\left\lvert\eqref{eq:en-eq-BR5}\right\rvert\lesssim&\,a^{-1}\cdot\left(a^2\|\nabla^2\Lap^{\frac{L}2}\phi\|_{L^2_G}\right)\cdot\sqrt{\E^{(L)}(W,\cdot)}\\
\lesssim&\,a^{-1}\left(\sqrt{\E^{(L+1)}(\phi,\cdot)}+a^{-c\sqrt{\epsilon}}\sqrt{\E^{(L-1)}(\phi,\cdot)}\right)\cdot\sqrt{\E^{(L)}(W,\cdot)}\,,\\
\lesssim&\,a^{-1-c\sqrt{\epsilon}}\E^{(L)}(W,\cdot)+a^{-1}\left(\E^{(L+1)}(\phi,\cdot)+\E^{(L)}(\phi,\cdot)+\E^{(\leq L-2)}(\phi,\cdot)\right)\,,\\[0.5em]
...
\begin{align*}
&\,-\del_t\E^{(L)}(W,\cdot)+8\pi C^2a^{-3}\int_M(N+1)\langle\Sigma,\RE\rangle_G\,\vol{G}+6\frac{\dot{a}}a\int_M(N+1)\lvert\Lap^\frac{L}2\RE\rvert_G^2\,\vol{G}\\
...
\end{align*}
Applying \eqref{eq:Sobolev-norm-equiv-zetalow} to the last term in the third line, we can bound it by
...
Since $\E^{(L+1)}(\phi,\cdot)$ and $\E^{(L-1)}(\Ric,\cdot)$ appear in these estimates, for both of which we only have weak control at our disposal for $L=20$, we need a different top order estimate:

\begin{lemma}[Bel-Robinson energy estimate at top order]\label{lem:en-est-BR-top} For $M\in2\N$, $4\leq M\leq 20$, and $t\in(t_{Boot},t_0]$, we have:
\begin{align*}
\numberthis\label{eq:en-est-BR-top}&\,\E^{(M)}(W,t)+\int_t^{t_0}\int_M\left[8\pi C^2a(s)^{-3}(N+1)\langle\Lap^\frac{M}2\Sigma,\Lap^\frac{M}2\RE\rangle_G+6\frac{\dot{a}(s)}{a(s)}\lvert\Lap^\frac{M}2\RE\rvert_G^2\right]\,\vol{G}\,ds\\
&\,\lesssim\E^{(M)}(W,t_0)+\int_t^{t_0}\left(\epsilon^\frac18 a(s)^{-3}+a(s)^{-1-c\sqrt{\epsilon}}\right)\E^{(M)}(W,s)\,ds\\
&\,+\int_t^{t_0}\left\{\epsilon^{-\frac18}a(s)^{-3}\cdot a(s)^4\E^{(M+1)}(\phi,s)+\left(\epsilon^\frac18 a(s)^{-3}+a(s)^{-1}\right)\E^{(M)}(\phi,s)\right.\\
&\,\phantom{\int_t^{t_0}}+\epsilon a(s)^{-3}\E^{(M)}(\Sigma,s)+\epsilon^\frac{7}8a(s)^{-3}\cdot a(s)^4\E^{(M-1)}(\Ric,s)+\epsilon^\frac{31}8 a(s)^{-3}\E^{(\leq M-2)}(\Ric,s)\\
&\,\phantom{\int_t^{t_0}}+\left(\epsilon^\frac{15}8 a(s)^{-3-c\sqrt{\epsilon}}+a(s)^{-1-c\sqrt{\epsilon}}\right)\E^{(\leq M-2)}(\phi,s)\\
&\,\left.\phantom{\int_t^{t_0}}+\epsilon^\frac{15}8 a(s)^{-3-c\sqrt{\epsilon}}\left(\E^{(\leq M-2)}(\Sigma,s)+\E^{(\leq M-2)}(W,s)\right)+\epsilon^\frac{15}8 a(s)^{-3-c\sqrt{\epsilon}}\E^{(\leq M-4)}(\Ric,s)\right\}\,ds
\end{align*}
\end{lemma}
\begin{proof}
The proof is analogous to that of \eqref{eq:en-est-BR} up to some specific terms and distributing of powers of $\epsilon$ in Young inequalities differently.\\
From \eqref{eq:en-eq-BR1}-\eqref{eq:en-eq-BR10} (replacing $L$ with $M$), note that the worst term with $\E^{(M+1)}(\phi,\cdot)$ arises from \eqref{eq:en-eq-BR5}, which is now estimated as follows:
\begin{align*}
\lvert\eqref{eq:en-eq-BR5}\rvert\lesssim&\,\left(a^{-3}\sqrt{a^4\E^{(M+1)}(\phi,\cdot)}+a^{-1}\sqrt{\E^{(M)}(\phi,\cdot)}+a^{-1-c\sqrt{\epsilon}}\sqrt{\E^{(M-2)}(\phi,\cdot)}\right)\sqrt{\E^{(M)}(W,\cdot)}\\
\lesssim&\,\left(\epsilon^\frac18a^{-3}+a^{-1-c\sqrt{\epsilon}}\right)\E^{(M)}(W,\cdot)+\epsilon^{-\frac18}a^{-3}\cdot a^4\E^{(M+1)}(\phi,\cdot)+a^{-1}\E^{(\leq L)}(\phi,\cdot)
\end{align*}
Furthermore, we treat terms $\mathfrak{E}_{M,top}$ and $\mathfrak{B}_{M,top}$ (see \eqref{eq:L2-top-E} and \eqref{eq:L2-top-B}) that give rise to the highest order curvature terms differently:
\begin{align*}
\|\mathfrak{E}_{M,top}\|_{L^2_G}\sqrt{\E^{(M)}(W,\cdot)}\lesssim&\,\sqrt{\epsilon} a^{-1-c\sqrt{\epsilon}}\sqrt{a^4\E^{(M-1)}(\Ric,\cdot)}\sqrt{\E^{(M)}(W,\cdot)}\\
\lesssim&\,\epsilon^\frac18a^{-1-c\sqrt{\epsilon}}\E^{(M)}(W,\cdot)+\epsilon^\frac{7}8a^{-1}\cdot a^4\E^{(M-1)}(\Ric,\cdot)\\
\|\mathfrak{B}_{M,top}\|\sqrt{\E^{(M)}(W,\cdot)}\lesssim&\,\epsilon a^{-3}\sqrt{a^4\E^{(M-1)}(\Ric,\cdot)}\sqrt{\E^{(M)}(W,\cdot)}\\
\lesssim&\,\epsilon^\frac18 a^{-3}\E^{(M)}(W,\cdot)+\epsilon^\frac{15}8 a^{-3}\cdot a^4\E^{(M-1)}(\Ric,\cdot)
\end{align*}
Hence, both top order curvature terms can be bounded by $\epsilon^\frac78a^{-3}\cdot a^4\E^{(M-1)}(\Ric,\cdot)$.
\end{proof}


\subsection{Energy estimates for the second fundamental form}\label{subsec:en-Sigma}

For the energy estimates for $\Sigma$, we again first derive integral estimates before top order:

\begin{lemma}[Energy estimates for the second fundamental form before top order]\label{lem:en-est-Sigma} Let $t\in(t_{Boot},t_0]$. Then, one has:
\begin{align*}
...
\end{align*}
For $L\in 2\N, L\leq 20$, the following holds:
\begin{align*}
\numberthis\label{eq:en-est-Sigma}&\,\E^{(L)}(\Sigma,t)+2\int_t^{t_0}\int_M\left[a(s)^{-3}(N+1)\langle\Lap^\frac{L}2\RE,\Lap^{\frac{L}2}\Sigma\rangle_G\,\vol{G}\,ds+\frac{\dot{a}(s)}{a(s)}(N+1)\lvert\Lap^{\frac{L}2}\Sigma\rvert_G^2\right]\,\vol{G}\,ds\\
...
&\,\phantom{\int_t^{t_0}}+\epsilon^2a(s)^{-3-c\sqrt{\epsilon}}\E^{(\leq L-2)}(\Sigma,s)+\left(\epsilon^\frac{15}8a(s)^{-3-c\sqrt{\epsilon}}+\epsilon a(s)^{-1-c\sqrt{\epsilon}}\right)\E^{(\leq L-2)}(\phi,s)\\
&\,\phantom{\int_t^{t_0}}+\underbrace{\epsilon^2a(s)^{-3-c\sqrt{\epsilon}}\E^{(\leq L-4)}(\Ric,s)}_{\text{not present for }L=2}\Bigr\}\,ds
\end{align*}
\end{lemma}
\begin{remark}
...
Additionally, we can exploit the structure of the momentum constraint equations to gain an elliptic estimate for $\E^{(M+1)}(\Sigma,\cdot)$. Crucially, the upper bound only depends on $\Sigma$-, scalar field and Bel-Robinson energies up to order $M$, and appropriately small and time-scaled curvature contributions up to order $M-1$. This will allow us to close the argument since we do not need to additionally consider the Bel-Robinson energy at order $M+1$ to balance out error terms as on the left hand side of \eqref{eq:en-est-Sigma}, which would require higher order scalar field and curvature energies, and thus makes it possible to close the bootstrap argument.

\begin{lemma}[Top order energy estimate for the second fundamental form]\label{lem:en-est-Sigma-top} For any $M\in 2\Z_+$, $2\leq M\leq 20$, we have
\begin{align*}
\numberthis\label{eq:en-est-Sigma-top}a^4\E^{(M+1)}(\Sigma,\cdot)\lesssim&\,\left(a^{4-c\sqrt{\epsilon}}+\epsilon a^{2-c\sqrt{\epsilon}}\right)\E^{(M)}(\Sigma,\cdot)+\E^{(M)}(\phi,\cdot)+\E^{(M)}(W,\cdot)+\epsilon^2a^4\E^{(M-1)}(\Ric,\cdot)\\
&\,+\epsilon a^{2-c\sqrt{\epsilon}}\E^{(\leq M-2)}(\phi,\cdot)+a^{2-c\sqrt{\epsilon}}\E^{(\leq M-2)}(\Sigma,\cdot)+\epsilon a^{2-c\sqrt{\epsilon}}\E^{(\leq M-2)}(\Ric,\cdot)
\end{align*}
\end{lemma}
\begin{proof}
...
\end{equation}
In particular, for $U=\Lap^{\frac{M}2}\Sigma$ and after rescaling, this reads:
\begin{align*}
&\,\int_M \left\lvert\nabla\Lap^{\frac{M}2}\Sigma\right\rvert_G^2+3{\left(\Ric[G]^\sharp\right)^i}_j{\left(\Lap^{\frac{M}2}\Sigma^\sharp\right)^j}_l{\left(\Lap^{\frac{M}2}\Sigma^\sharp\right)^l}_i-\frac{R[G]}2\lvert\Lap^\frac{M}2\Sigma\rvert_G^2\,\vol{G}\\
=&\,\int_M\frac32\cdot\lvert\div_G\Lap^\frac{M}2\Sigma\rvert_G^2+a^2\lvert\curl\Lap^\frac{M}2\Sigma\rvert_G^2\,\vol{G}
\end{align*}
The last two terms on the left hand side can be estimated by $(1+\sqrt{\epsilon}a^{-c\sqrt{\epsilon}})\E^{(M)}(\Sigma,\cdot)$ in absolute value using the strong $C_G$-norm estimate \eqref{eq:APmidRic}. Thus, inserting the Laplace-commuted rescaled momentum constraint equations \eqref{eq:comeq-mom-div} and \eqref{eq:comeq-mom-curl}, we obtain:
\begin{align*}
&\,\E^{(M+1)}(\Sigma,\cdot)\\
\lesssim&\,\left(1+\sqrt{\epsilon}a^{-c\sqrt{\epsilon}}\right)\E^{(M)}(\Sigma,\cdot)\\
+&\,\int_M\left\{\lvert\Psi+C\rvert^2\left\lvert\nabla\Lap^{\frac{M}2}\phi\right\rvert_G^2+\lvert\nabla\phi\rvert_G^2\left\lvert\Lap^{\frac{M}2-1}\Ric[G]\right\rvert_G^2+\lvert\Sigma\rvert_G^2\left\lvert\nabla\Lap^{\frac{M}2-1}\Ric[G]\right\rvert_G^2+\lvert\mathfrak{M}_{L,Junk}\rvert_G^2\right.\\
&\,\phantom{\int_M}\left.+a^{-4}\lvert\Lap^\frac{M}2\RB\rvert_G^2+\lvert\Sigma\rvert_G^2\left\lvert\nabla\Lap^{\frac{M}2-1}\Ric[G]\right\rvert_G^2+\lvert\nabla\Sigma\rvert_G^2\left\lvert\nabla^2\Lap^{\frac{M}2-2}\Ric[G]\right\rvert_G^2+\left\lvert\tilde{\mathfrak{M}}_{L,Junk}\right\rvert_G^2\,\right\}\vol{G}
\end{align*}

After re-arranging, using the strong $C_G$-norm estimates \eqref{eq:APPsi}, \eqref{eq:APmidphi}, \eqref{eq:APSigma} and \eqref{eq:APmidSigma} and multiplying by $a^4$ on both sides, we get

\begin{align*}
a^4\E^{(M+1)}(\Sigma,\cdot)\lesssim&\,\left(1+\sqrt{\epsilon}a^{-c\sqrt{\epsilon}}\right)a^4\E^{(M)}(\Sigma,\cdot)+\E^{(M)}(\phi,\cdot)+\E^{(M)}(W,\cdot)+\epsilon^2a^4\E^{(M-1)}(\Ric,\cdot)\\
&\,+\epsilon^2a^{4-c\sqrt{\epsilon}}\E^{(M-2)}(\Ric,\cdot)+a^4\|\mathfrak{M}_{M,Junk}\|_{L^2_G}^2+a^4\|\tilde{\mathfrak{M}}_{M,Junk}\|_{L^2_G}^2
\end{align*}
...
To control commutator errors, we will also need some additional estimates on curvature energies. Unlike the other energies, these inequalities don't rely on any particular structure within the equations and instead just rely on pointwise estimates, the Young inequality and near-coercivity of energies in the sense of Lemma \ref{lem:Sobolev-norm-equivalence-improved}. For the sake of convenience, we phrase these estimates for $\E^{(L-2)}(\Ric,\cdot)$ since this is the order needed when improving behaviour of the total energy at order $L$.

\begin{lemma}[Curvature energy estimates before top order]\label{lem:en-est-Ric}
Let $L\in 2\Z$, $4\leq L\leq 18$ and $t\in(t_{Boot},t_0]$. Then, one has
\begin{align*}
...
&\,+\left(\|\mathfrak{R}_{L-2,Border}\|_{L^2_G}+\|\mathfrak{R}_{L-2,Junk}\|_{L^2_G}\right)\sqrt{\E^{(L-2)}(\Ric,\cdot)}\\
\lesssim&\,\left[\epsilon^{\frac18} a^{-3}+a^{8-c\sigma}\right]\E^{(L-2)}(\Ric,\cdot)+\epsilon^{-\frac18}a^{-3}\left[\E^{(L)}(\Sigma,\cdot)+\E^{(L)}(\phi,\cdot)\right]\\
...
&\,+\left(\|\mathfrak{R}_{L-2,Border}\|_{L^2_G}+\|\mathfrak{R}_{L-2,Junk}\|_{L^2_G}\right)\sqrt{\E^{(L-2)}(\Ric,\cdot)}
\end{align*}
The statement for $L\geq 4$ now follows inserting the borderline and junk term estimates \eqref{eq:L2-Border-R-even} and \eqref{eq:L2-junk-R-even} and applying the lapse energy estimates from Corollary \ref{cor:en-est-lapse}. \\
The inequality for $\E^{(0)}(\Ric,\cdot)$ follows almost identically by inserting \eqref{eq:REEqRic} instead of \eqref{eq:comeq-Ric-even} as well as \eqref{eq:REEqG} for the additional $\del_tG\ast(\Ric[G]+\nicefrac29G)$-terms. These can be estimated as
\begin{align*}
\lesssim&\,\int_Ma^{-3}\Sigma\ast(\Ric[G]+\frac29G)+\frac{\dot{a}}a N\cdot G\ast(\Ric[G]+\frac29G)\,\vol{G}\\
\lesssim&\,a^{-3}\left(\sqrt{\E^{(0)}(\Sigma,\cdot)}+\sqrt{\E^{(0)}(N,\cdot)}\right)\sqrt{\E^{(0)}(\Ric,\cdot)}\,,
\end{align*}
and this can be treated as with similar terms at higher orders.
\end{proof}
...
%\end{proof}
We also collect the following estimate:

\begin{lemma}[Top order curvature energy estimate]\label{lem:en-est-Ric-top} For $M\in2\N$, $4\leq M\leq 20$ and $t\in(t_{Boot},t_0]$, the following holds:
\begin{align*}
\numberthis\label{eq:en-est-Ric-top}a(t)^4\E^{(M-1)}(\Ric,t)\lesssim&\,a(t_0)^4\E^{(M-1)}(\Ric,t_0)\\
&\,+\int_t^{t_0}\left(\epsilon^\frac18a(s)^{-3}+a(s)^{-1-c\sqrt{\epsilon}}\right)\left(a(s)^4\E^{(M-1)}(\Ric,s)\right)\,ds\\
&\,+\int_t^{t_0}\epsilon^{-\frac18}a(s)^{-3}\cdot a(s)^4\E^{(M+1)}(\Sigma,s)\,ds\\
&\,+\int_t^{t_0}\Bigr\{\epsilon^{-\frac18}a(s)^{-3}\E^{(M)}(\phi,s)+\left(\epsilon^\frac{15}8a(s)^{-3}+a(s)^{-1-c\sqrt{\epsilon}}\right)\E^{(M)}(\Sigma,s)\\
&\,\phantom{+\int_t^{t_0}}+\left(\epsilon^\frac{15}8a(s)^{-3-c\sqrt{\epsilon}}+a(s)^{-1-c\sqrt{\epsilon}}\right)\left(\E^{(\leq M-2)}(\phi,s)+\E^{(\leq M-2)}(\Sigma,s)\right)\\
&\,\phantom{+\int_t^{t_0}}+\epsilon^\frac{15}8a(s)^{-3}\E^{(M-2)}(\Ric,\cdot)+\epsilon^\frac{15}8a^{-3-c\sqrt{\epsilon}}\E^{(\leq M-4)}(\Ric,s)\Bigr\}ds
\end{align*}
\end{lemma}
\begin{proof}
The proof is very similar to that of Lemma \ref{lem:en-est-Ric} since we didn't exploit any structure within \eqref{eq:comeq-Ric-even} that doesn't equally occur in \eqref{eq:comeq-Ric-odd}. We get
\begin{align*}
...
\begin{lemma}[Sobolev norm estimates for Christoffel symbols]\label{lem:int-est-Chr} Let $U$ be a coordinate neighbourhood on $M$, viewed as a coordinate neighbourhood on $\Sigma_t$ for $t\in(t_{Boot},t_0]$. For any $l\in\N,\,l\leq 17$, the following Sobolev estimate then holds:
...
\end{lemma}
\begin{proof}
Inserting the auxiliary evolution equation \eqref{eq:auxeq-Chr} and Lemma \ref{lem:aux-comm} to write out the commutator terms, we get for $J\in\N$, $J\leq 17$:
\begin{align*}
...
&\,\phantom{\int_M}+2\langle[\del_t,\nabla^J](\Gamma-\Gamhat),\nabla^l(\Gamma-\Gamhat)\rangle_G-3N\frac{\dot{a}}a\left\lvert\nabla^J(\Gamma-\Gamhat)\right\rvert_G^2\Bigr]\,\vol{G}
\end{align*}
For the commutator terms, we recall that \eqref{eq:APmidG} implies
...
\end{align*}
is satisfied. Consequently and using the same strong $C_G$-norm estimates along with Lemma \ref{lem:lapse-maxmin}, the differential inequality becomes
\begin{align*}
-\del_t\|\Gamma-\Gamhat\|_{\dot{H}^J_G}^2\lesssim&\,\epsilon a^{-3}\|\nabla^J(\Gamma-\Gamhat)\|_{L^2_G}^2\\
&\,+a^{-3}\left(\|N\|_{H^{J+1}_G}+\|\Sigma\|_{H^{J+1}_G}\right)\|\nabla^l(\Gamma-\Gamhat)\|_{L^2_G}\\
...
\begin{align*}
\|\Gamma-\Gamhat\|_{\dot{H}^J_G(\Sigma_t)}^2\lesssim&\,\epsilon^4a(t)^{-c\epsilon^\frac18}+\epsilon^{-\frac14}a(t)^{-c\epsilon^\frac18}\sup_{s\in(t,t_0]}\left(\|N\|_{H^J_G(\Sigma_s)}^2+\|\Sigma\|_{H^J_G(\Sigma_s)}^2\right)\\
&\,+\epsilon^\frac{46}8a(t)^{-c\epsilon^\frac18}+\epsilon^\frac{6}8a(t)^{-c\epsilon^\frac18}\sup_{s\in(t,t_0)}\left(\|N\|_{H^{J-1}(\Sigma_s)}^2+\|\Sigma\|_{H^{J-1}(\Sigma_s)}^2\right)\,,
\end{align*}
...
\end{align*}
For the first two terms, we can apply \eqref{eq:APSigma} and \eqref{eq:BsN} respectively and get
\begin{align*}
...
\end{align*}
The statement for $l=0$ now follows taking the supremum over the norms under the integral and applying \eqref{eq:log-est}.\\
...
In this section, we combine the energy estimates obtained in the last two sections to improve the boostrap assumptions for the energies themselves, and then show how this improves the behaviour of the solution norms. For an outline of the energy improvement arguments that we perform in Sections \ref{subsec:bs-imp-core} and \ref{subsec:bs-imp-top}, we refer back to Remark \ref{rem:en-est-strat}.

Before carrying out the improvements themselves, we quickly collect an estimate that shows that combining Lemma \ref{lem:en-est-BR} and Lemma \ref{lem:en-est-Sigma} yields sufficient control on the energies themselves:
...
\end{lemma}
\begin{proof}
First, we recall that $N+1>0$ holds by Lemma \ref{lem:lapse-maxmin}. Additionally, we can apply \eqref{eq:diff-ineq-Friedman} and the Young inequality and get the following:
\begin{align*}
...
This shows that $\E^{(L)}(W,\cdot)+4\pi C^2\E^{(L)}(\Sigma,\cdot)$ is controlled by the sum of the left hand sides of the inequalities in Lemma \ref{lem:en-est-BR} and \ref{lem:en-est-Sigma} for $L\in 2\N, 0\leq L\leq 20$. %While this specific form relies on the inequality \eqref{eq:diff-ineq-Friedman}, using \eqref{eq:diff-ineq-Friedman-weak} instead would only add energy terms of the form $a^{-1}\left(\E^{(L)}(W,\cdot)+\E^{(L)}(\Sigma,\cdot)\right)$ on the right hand side, which can easily be dealt with in the Gronwall argument. Thus, while the Lemma itself relies on the sectional curvature being nonpositive, the Gronwall argument can easily be adapted to $\kappa=1$ as well.
%\end{remark}


\subsection{Improving energies before top order}\label{subsec:bs-imp-core}


\begin{prop}[Improved energy behaviour, part 1]\label{prop:en-bs-imp} Under the bootstrap assumptions (see Assumption \ref{ass:bootstrap}) and the initial data assumptions \eqref{ass:init}, the following improved estimates hold on $(t_{Boot},t_0]$:
\begin{subequations}
\begin{align}
\E^{(\leq 18)}(\phi,\cdot)\lesssim&\,\epsilon^\frac{11}4a^{-c\epsilon^\frac18}\label{eq:en-bs-imp-SF}\\
\E^{(\leq 18)}(\Sigma,\cdot)\lesssim&\,\epsilon^\frac52a^{-c\epsilon^\frac18}\label{eq:en-bs-imp-Sigma}\\
\E^{(\leq 18)}(W,\cdot)\lesssim&\,\epsilon^\frac{21}8a^{-c\epsilon^\frac18}\label{eq:en-bs-imp-BR}\\
\E^{(\leq 16)}(\Ric,\cdot)\lesssim&\,\epsilon^\frac94a^{-c\epsilon^\frac18}\label{eq:en-bs-imp-Ric}\\
\sum_{m=1}^{9}\|\Lap^m\phi\|^2_{L^2_G}+\|\nabla\phi\|_{H^{17}_G}^2\lesssim&\,\epsilon^\frac52 a^{-c\epsilon^\frac18}\label{eq:en-bs-imp-phi}\\
\E^{(\leq 16)}(N,\cdot)+a^4\E^{(17)}(N,\cdot)+a^8\E^{(18)}(N,\cdot)\lesssim&\,\epsilon^\frac94a^{8-c\epsilon^\frac18}\label{eq:en-bs-imp-N}
\end{align}
\end{subequations}
\end{prop}
\begin{proof}
...
\[\E^{(0)}_{total}:=\E^{(0)}(\phi,\cdot)+\epsilon^\frac14\left(\E^{(0)}(W,\cdot)+4\pi C^2\E^{(0)}(\Sigma,\cdot)\right)\]
by the sum of the left hand side of \eqref{eq:en-est-SF0}, the left hand side of \eqref{eq:en-est-BR0} weighted by $\epsilon^\frac14$ and the left hand side of \eqref{eq:en-est-Sigma0} weighted by $4\pi C^2\cdot\epsilon^\frac14$.\footnote{We need to weight $\E^{(0)}(\Sigma,\cdot)$ in the total energy by $K\cdot\epsilon^\frac14$ for some $K>0$ to balance out the $\epsilon^{-\frac18}$-weight from the scalar field energy on the right hand side of \eqref{eq:en-est-Sigma0}. The weight on the Bel-Robinson energy is then needed to obtain the cancellation in Lemma \ref{lem:en-error-cancellation}.} Combining said estimates and inserting the initial data assumption from \eqref{eq:init-ass-en}, the following holds in total:
\begin{equation}\label{eq:en-eq-total0}
\E^{(0)}_{total}(t)\lesssim\epsilon^4+\int_t^{t_0}\left(\epsilon^\frac18a(s)^{-3}+a(s)^{-1-c\sqrt{\epsilon}}\right)\E^{(0)}_{total}(s)+\epsilon^\frac14a(s)^{-1}\E^{(1)}(\phi,s)\,ds
\end{equation}
Applying the Gronwall lemma to \eqref{eq:en-eq-total0}, we get for some suitable constant $c^\prime>0$:
\begin{equation*}
\E^{(0)}_{total}(t)\lesssim \exp\left(c^\prime\int_t^{t_0}\epsilon^\frac18 a(s)^{-3}+a(s)^{-1-c\sqrt{\epsilon}}\,ds\right)\cdot\left(\epsilon^4+\epsilon^\frac14\int_t^{t_0}a(s)^{-1}\E^{(1)}(\phi,s)\,ds\right)
\end{equation*}
Inserting the bootstrap assumption \eqref{eq:BsEnSF} and applying \eqref{eq:a-exp-est} and \eqref{eq:a-integrals} with $c\sqrt{\epsilon}<c\sigma<1$, we get the following for some $c_1>0$:
\begin{equation}
\E^{(0)}_{total}\lesssim \left(\epsilon^4+\epsilon^{\frac14+\frac{5}2}\right)a^{-c^\prime\epsilon^\frac18}\lesssim \epsilon^\frac{11}4a^{-c^\prime\epsilon^\frac18}
\end{equation}
...
\begin{subequations}
\begin{align}
\E^{(\leq L-2)}(\phi,\cdot)\lesssim&\,\epsilon^\frac{11}4a^{-c\epsilon^\frac18}\label{eq:ind-ass-ESF}\\
\E^{(\leq L-2)}(\Sigma,\cdot)\lesssim&\,\epsilon^\frac52a^{-c\epsilon^\frac18}\label{eq:ind-ass-Sigma}\\
\E^{(\leq L-2)}(W,\cdot)\lesssim&\,\epsilon^\frac{5}2a^{-c\epsilon^\frac18}\label{eq:ind-ass-BR}
\end{align}
on $(t_{Boot},t_0]$, as well as, if $L\geq 4$,
\begin{align}
...
\E^{(\leq L-4)}(\Ric,\cdot)\lesssim&\,\epsilon^\frac94a^{-c\epsilon^\frac18}\label{eq:ind-ass-Ric}\\
\E^{(\leq L-4)}(N,\cdot)+a^4\E^{(L-3)}(N,\cdot)+a^8\E^{(L-2)}(N,\cdot)\lesssim&\,\epsilon^\frac94a^{8-c\epsilon^\frac18}\label{eq:ind-ass-N}
\end{align}
\end{subequations}
Note that \eqref{eq:en-bs-imp-base-SF}-\eqref{eq:en-bs-imp-base-BR} means this holds true for $L=2$. We define 
\begin{equation*}
\E^{(L)}_{total}:=\E^{(L)}(\phi,\cdot)+\epsilon^\frac14\left(\E^{(L)}(W,\cdot)+4\pi C^2\E^{(L)}(\Sigma,\cdot)+\|\Lap^\frac{L}2\phi\|_{L^2_G}^2\right)+\epsilon^\frac12\E^{(L-2)}(\Ric,\cdot)
\end{equation*}
for $L\geq 2$.\footnote{The weights on $\E^{(L)}(\Sigma,\cdot)$ and $\E^{(L)}(W,\cdot)$ in $\E^{(L)}_{total}$ are necessary for the same reasons as at order $0$. We need to scale the curvature energy by $\epsilon^\frac12$ to account for $\epsilon^{-\frac18}a^{-3}\E^{(L)}(\Sigma,\cdot)$ in \eqref{eq:en-est-Ric}, and the scalar field norm by $\epsilon^\frac14$ to account for $\epsilon^{-\frac18}a^{-3}\E^{(L)}(\phi,\cdot)$. } Again due to Lemma \ref{lem:en-error-cancellation}, this is controlled by the respective weighted left hand sides of \eqref{eq:en-est-SF}, \eqref{eq:en-est-BR}, \eqref{eq:en-est-Sigma}, \eqref{eq:en-est-Ric} and \eqref{eq:int-est-Lapphi}. As a result and again applying the initial data assumption \eqref{eq:init-ass-en}, we have the following:
\begin{align*}
...
\end{equation*}
The error term for $L=2$ arises from \eqref{eq:int-est-Lapphi} and \eqref{eq:en-est-SF}, and for $L\geq 4$ from \eqref{eq:en-est-SF} exclusively. Regarding the curvature energies, any summand with $\E^{(L-3)}(\Ric,\cdot)$ can be split using \eqref{eq:ibp-trick}, the resulting summands containing $\E^{(L-2)}(\Ric,\cdot)$ can always be absorbed in to the total energy term in the first line, and anything with $\E^{(\leq L-4)}(\Ric,\cdot)$ is tracked in $\langle\text{Err}\rangle_L$ for $L\geq 4$.\\

By inserting the bootstrap assumptions \eqref{eq:BsEnSF} and \eqref{eq:BsEnRic} in the second line of the total energy inequality and the induction assumptions \eqref{eq:ind-ass-ESF}-\eqref{eq:ind-ass-BR} in the following two lines, we obtain while updating $c$:
\begin{align*}
...
\end{align*}
For $L=2$, we get with Corollary \ref{cor:int-est-nablaphi0-botch} and updating $c>0$ that
\begin{align*}
...
\end{align*}
The first line is negligible compared to terms already present on the right of \eqref{eq:total-diff-ineq-induction}. The first term in the second line can be estimated using integration by parts and applying \eqref{eq:en-bs-imp-base-SF} as follows:
\begin{align*}
...
\end{align*}
wherein again the second term can be ignored compared to \eqref{eq:total-diff-ineq-induction}. Thus, \eqref{eq:total-diff-ineq-induction} can be rewritten as
\begin{align*}
...
follows from there by the same argument as for $L=2$.

This shows that \eqref{eq:ind-ass-ESF}-\eqref{eq:ind-ass-BR} and \eqref{eq:ind-ass-Ric} can be extended to level $L$ (where $L-1$ is immediately covered by \eqref{eq:drop-odd-en}), and the bound on $\sum_{m=0}^{\frac{L}2-1}\|\Lap^m\phi\|_{L^2_G}^2$ in \eqref{eq:ind-ass-phi} can be extended to  $\|\Lap^\frac{L}2\phi\|_{L^2_G}$. Hence, it remains to show
...
to close the induction. To show the former, we can apply estimates from Lemma \ref{lem:Sobolev-norm-equivalence-improved}, namely \eqref{eq:Sobolev-norm-equiv-nablazeta2l+1} and \eqref{eq:Sobolev-norm-equiv-zetalow} with $\zeta=\phi$ as well as \eqref{eq:Sobolev-norm-equiv-zeta2l} for $\zeta=\Lap^m\phi,\,m\geq 1$:
\begin{align*}
...
and then \eqref{eq:en-bs-imp-phi} follows.\\
Finally, applying the obtained improved estimated for $\nabla\phi$ and $\Ric[G]$ to to Corollary \ref{cor:en-est-lapse-tilde} for $0\leq 2l\leq L-4$, we also get the desired improvement for the lapse energies. This closes the induction for $L\leq 18$.
...
To be able to ultimately control $\mathcal{H}$, we don't yet have sufficient energy control on the scalar field (beside not having derived control of the metric yet). Doing this and closing the bootstrap argument requires adding scaled energy quantities in the total energy at top order:
...
The terms in \eqref{eq:en-est-top-total3} can be simplified as follows: First, by \eqref{eq:Sobolev-norm-equiv-zetalow}, one can replace $\|\nabla^2\Lap^9\phi\|_{L^2_G}^2$ by $\|\Lap^{10}\phi\|_{L^2_G}^2+\|\Lap^9\phi\|_{L^2_G}^2$ up to updating constants. The former can now be estimated by\linebreak $a(s)^{-1-c\sqrt{\epsilon}}\E_{top}(s)$ and thus absorbed into \eqref{eq:en-est-top-total1}. For the remaining two terms, we can use \eqref{eq:Sobolev-norm-equiv-zeta2l+1} with $l=9$ and $l=10$ and integration by parts to estimate them as follows:
\begin{align*}
&\,\epsilon^2a^{-1-c\sqrt{\epsilon}}\|\nabla\phi\|_{H^{18}_G(\Sigma_s)}^2+\epsilon a^{3-c\sqrt{\epsilon}}\|\nabla\phi\|_{H^{20}_G(\Sigma_s)}^2\\
\lesssim&\,a^{-1-c\sqrt{\epsilon}}\left(\epsilon^2\|\nabla\Lap^9\phi\|_{L^2_G}^2+\epsilon a^{4}\|\nabla\Lap^{10}\phi\|_{L^2_G}^2+\epsilon a^{-1-c\sqrt{\epsilon}}\|\nabla\phi\|_{H^{16}_G}^2\right)+\epsilon^2 a^{-1-c\sqrt{\epsilon}}\E^{\leq 18}(\Ric,\cdot)\\
\lesssim&\,\left[\epsilon^2a^{-1-c\sqrt{\epsilon}}\|\Lap^{10}\phi\|_{L^2_G}^2+\epsilon a^{-1-c\sqrt{\epsilon}}\E^{(20)}(\phi,\cdot)+\epsilon^2a^{-1-c\sqrt{\epsilon}}\E^{(18)}(\Ric,\cdot)\right]\\
&\,+\epsilon a^{-1-c\sqrt{\epsilon}}\|\nabla\phi\|_{H^{16}_G}^2+\epsilon^2a^{-1-c\sqrt{\epsilon}}\E^{(\leq 16)}(\Ric,\cdot)\\
\lesssim&\,a^{-1-c\sqrt{\epsilon}}\E_{top}+\epsilon a^{-1-c\sqrt{\epsilon}}\|\nabla\phi\|_{H^{16}_G}^2+\epsilon^2a^{-1-c\sqrt{\epsilon}}\E^{(\leq 16)}(\Ric,\cdot)
\end{align*}
Hence, after performing this rearrangement and inserting the energy improvements from Proposition \ref{prop:en-bs-imp}, we get
\[\eqref{eq:en-est-top-total2}-\eqref{eq:en-est-top-total4}\lesssim\int_t^{t_0}a(s)^{-1-c\sqrt{\epsilon}}\E_{top}(s)+\epsilon^{\frac{23}8}a(s)^{-3-c\epsilon^\frac18}\,ds\,.\]
Here, the second summand bounding all error terms arises from 
\[\epsilon^\frac38a(s)^{-3-c\sqrt{\epsilon}}\E^{(\leq 18)}(\Sigma,s)\,.\]
Regarding \eqref{eq:en-est-top-total5}-\eqref{eq:en-est-top-total7}, notice that the first four summands can be bounded from above by $\epsilon^\frac14\E_{top}(t)$ up to constant. For the remaining three terms, we can again insert the improved estimates \eqref{eq:en-bs-imp-SF}, \eqref{eq:en-bs-imp-Sigma} and \eqref{eq:en-bs-imp-Ric}, bounding them by $\epsilon^3a(t)^{2-c\epsilon^\frac18}$. \\
...
\begin{align*}
(1-K\epsilon^\frac14)\E_{top}(t)\lesssim&\,\epsilon^4+\int_t^{t_0}\left(\epsilon^\frac18a(s)^{-3}+a(s)^{-1-c\sqrt{\epsilon}}\right)\E_{top}(s)\,ds+\int_t^{t_0}\epsilon^\frac{23}8a(s)^{-3-c\epsilon^\frac18}\,ds\\
&\,+\epsilon^3a(t)^{2-c\epsilon^\frac18}\\
\lesssim&\,\epsilon^3+\epsilon^{\frac{11}4}a(t)^{-c\epsilon^\frac18}+\int_t^{t_0}\left(\epsilon^\frac18a(s)^{-3}+a(s)^{-1-c\sqrt{\epsilon}}\right)\E_{top}(s)\,ds\,
\end{align*} 
The prefactor on the left hand side is positive for small enough $\epsilon>0$, and then applying the Gronwall lemma shows \eqref{eq:en-bs-imp-top}.\\

Since this in particular means $\E^{(20)}(\phi,\cdot)\lesssim\epsilon^\frac{11}4a^{-c\epsilon^\frac18}$, combining this with \eqref{eq:en-bs-imp-SF} and applying \eqref{eq:drop-odd-en} yields \eqref{eq:en-bs-imp-top-SF}. Finally, with \eqref{eq:en-bs-imp-phi}, \eqref{eq:en-bs-imp-top} also implies
...
Hence, Lemma \ref{lem:Sobolev-norm-equivalence-improved} implies \eqref{eq:en-bs-imp-top-phi} as in the previous proof.
\end{proof}
...
\begin{subequations}
\begin{align}
\mathcal{H}\lesssim&\,\epsilon^\frac98 a^{-c\epsilon^\frac18}\label{eq:H-norm-imp}\\
\|\Sigma\|_{H^{18}_G}^2\lesssim&\,\epsilon^\frac52a^{-c\epsilon^\frac18}\label{eq:H-Sigma-imp}\\
\|N\|_{H^{18}_G}^2\lesssim&\,\epsilon^\frac{11}4a^{-c\epsilon^\frac18}\label{eq:H-lapse-imp}
\end{align}
\end{subequations}
\end{corollary}
\begin{proof}
First, we apply the improved energy estimates from Propositions \ref{prop:en-bs-imp} and \ref{prop:en-bs-imp-top} as well as the strong $C_G$-norm bounds from Lemma \ref{lem:AP} to the near-coercivity estimates in Lemma \ref{lem:Sobolev-norm-equivalence-improved}. With this, we directly obtain the following Sobolev norm estimates (updating $c$):
\begin{align*}
\|\Psi\|_{H^{19}_G}^2\lesssim&\,\epsilon^\frac{11}4a^{-c\epsilon^\frac18}+\epsilon a^{-c\epsilon^\frac18}\cdot\epsilon^\frac94a^{-c\epsilon^\frac18}\lesssim\epsilon^\frac{11}4a^{-c\epsilon^\frac18}\\
\|\Sigma\|_{H^{18}_G}^2\lesssim&\,\epsilon^\frac{5}2a^{-c\epsilon^\frac18}\\
\|\Ric[G]+\frac29G\|_{H^{16}_G}^2\lesssim&\,\epsilon^\frac94 a^{-c\epsilon^\frac18}\\
\|\RE\|_{H^{18}_G}^2+\|\RB\|_{H^{18}_G}^2\lesssim&\,\epsilon^\frac{5}2a^{-c\epsilon^\frac18}
%a^8\|N\|_{\dot{H}^{18}_G}^2+a^4\|N\|^2_{\dot{H}^{17}_G}+\|N\|^2_{\dot{H}^{16}_G}\lesssim&\,\epsilon^\frac94a^{4-c\epsilon^\frac18}\,,
\end{align*}
Note that, from Proposition \ref{prop:en-bs-imp-top}, we utilize \eqref{eq:en-bs-imp-top-SF} to obtain the bootstrap improvement for $\Psi$, and the bound for $\|\nabla\phi\|_{H^{18}_G}$ has already been shown in \eqref{eq:en-bs-imp-phi} -- the rest follow already from Proposition \ref{prop:en-bs-imp}.\\
We take particular care in showing that the improved bound holds for $a^2\|\nabla\phi\|_{\dot{H}^{19}_G}$: Considering \eqref{eq:Sobolev-norm-equiv-nablazeta2l+1}, we need to obtain sufficient control on $\E^{(17)}(\Ric,\cdot)$. By Proposition \ref{prop:en-bs-imp-top}, we have
\[\E^{(\leq 17)}(\Ric,\cdot)\leq \E^{(\leq 16)}(\Ric,\cdot)+\E^{(18)}(\Ric,\cdot)\lesssim\epsilon^\frac{9}4a^{-c\epsilon^\frac18}\,.\]
Thus, with \eqref{eq:APmidphi} in the first line, \eqref{eq:Sobolev-norm-equiv-zeta2l} at low orders and the above estimate in the second, as well as \eqref{eq:en-bs-imp-top-SF} and \eqref{eq:en-bs-imp-top-phi} in the final line, we get the following (updating $c$ in every line):
\begin{align*}
a^4\|\nabla\phi\|_{H^{19}_G}^2\lesssim&\, a^4\|\nabla\phi\|_{L^2_G}^2+a^{4-c\sqrt{\epsilon}}\sum_{m=0}^9\|\nabla^2\Lap^m\phi\|_{L^2_G}^2+\epsilon a^{4-c\sqrt{\epsilon}}\cdot \E^{(\leq 17)}(\Ric,\cdot)\\
\lesssim&\,a^{4-c\sqrt{\epsilon}}\left(\|\Lap^{9}\phi\|_{L^2_G}^2+\|\Lap^{10}\phi\|_{L^2_G}^2+\|\nabla\phi\|_{H^{17}_G}^2\right)+\epsilon^\frac{13}4a^{-c\epsilon^\frac18}\\
\lesssim&\,a^{-c\sqrt{\epsilon}}\cdot \E^{(20)}(\phi,\cdot)+a^{4-c\sqrt{\epsilon}}\|\nabla\phi\|_{H^{17}_G}^2+\epsilon^\frac{13}4a^{-c\epsilon^\frac18}\\
\lesssim&\,\epsilon^\frac{5}2 a^{-c\epsilon^\frac18}
\end{align*}
...
\end{align*}
and subsequently, again applying Lemma \ref{lem:Sobolev-norm-equivalence-improved},
\begin{equation*}
a^8\|N\|^2_{\dot{H}^{20}_G}+a^4\|N\|^2_{\dot{H}^{19}_G}+\|N\|^2_{H^{18}_G}\lesssim\epsilon^\frac{11}4a^{-c\epsilon^\frac18}\,.
\end{equation*}
Finally, having now shown \eqref{eq:H-Sigma-imp} and \eqref{eq:H-lapse-imp}, we can apply these to \eqref{eq:norm-est-G} to get
\begin{equation*}
\|G-\gamma\|^2_{H^{18}_G}\lesssim a^{-c\epsilon^\frac18}\left(\epsilon^4+\epsilon^{-\frac14+\frac52}+\epsilon^{-\frac14+\frac{11}4}\right)\lesssim\epsilon^\frac{9}4a^{-c\epsilon^\frac18}\,,
\end{equation*}
...
Intuitively, the bounds on $\mathcal{C}$ should now follow from $\mathcal{H}$ by the standard Sobolev embedding. However, since both of these norms are with respect to $G$, the embedding constant may be time dependent. To circumvent this issue, we need to switch between norms with repsect to $G$ and $\gamma$ and then apply the embedding with respect to $C_\gamma$ and $H_\gamma$. The following lemma ensures that we still obtain bootstrap improvements after performing these norm switches:

\begin{lemma}[Moving between norms]\label{lem:G-gamma-norm-switch} Let $l\in\N,\,l\leq 20$, $\zeta$ be a scalar field, $\mathfrak{T}$ be an arbitrary $\Sigma_t$-tangent tensor and let $U$ be either a coordinate neighbourhood on $\Sigma_t$ or $\Sigma_t$ itself. Then, for some multivariate polynomial $P_l$ with $P_l(0,0)=0$ and on any open subset $U$ of $\Sigma_t$, we have
\begin{subequations}
\begin{align}
\|\zeta\|_{C^l_G(U)}\lesssim&\,a^{-c\sqrt{\epsilon}}\|\zeta\|_{C^l_\gamma(U)}+a^{-c\sqrt{\epsilon}}\|\zeta\|_{C_\gamma^{\max\left\{0,\lfloor\frac{l-1}2\rfloor\right\}}(U)}\cdot P_l\left(\|G-\gamma\|_{C^{l-1}_\gamma(U)},\|G^{-1}-\gamma^{-1}\|_{C^{l-1}_\gamma(U)}\right) \label{eq:C-norm-exch-zeta}\\
\|\mathfrak{T}\|_{C^l_G(U)}\lesssim&\,a^{-c\sqrt{\epsilon}}\|\mathfrak{T}\|_{C^l_\gamma(U)}+a^{-c\sqrt{\epsilon}}\|\mathfrak{T}\|_{C_\gamma^{\max\left\{0,\lfloor\frac{l-1}2\rfloor\right\}}(U)}\cdot P_l\left(\|G-\gamma\|_{C^l_\gamma(U)},\|G^{-1}-\gamma^{-1}\|_{C^{l-1}_\gamma(U)}\right)\label{eq:C-norm-exch-T}
\end{align}
\end{subequations}
as well as the same inequalities with the roles of $G$ and $\gamma$ reversed. For $l\leq 12$, this reduces to:
\begin{subequations}
\begin{align}
a^{c\sqrt{\epsilon}}\|\zeta\|_{C^l_\gamma(U)}\lesssim\|\zeta\|_{C^l_G(U)}\lesssim\,a^{-c\sqrt{\epsilon}}\|\zeta\|_{C^l_\gamma(U)} \label{eq:AP-exch-zeta}\\
a^{c\sqrt{\epsilon}}\|\mathfrak{T}\|_{C^l_\gamma(U)}\lesssim\|\mathfrak{T}\|_{C^l_G(U)}\lesssim\,a^{-c\sqrt{\epsilon}}\|\mathfrak{T}\|_{C^l_\gamma(U)}\,\label{eq:AP-exch-T}
\end{align}
\end{subequations}
Further, one has
\begin{subequations}
\begin{align}
\|\zeta\|_{H^l_\gamma(U)}^2\lesssim&\,a^{-c\sqrt{\epsilon}}\|\zeta\|^2_{H^l_G(U)}+a^{-c\epsilon^\frac18}\|\zeta\|_{C_G^{\lceil \frac{l-1}2\rceil}(\Sigma_t)}^2\left(\epsilon^4+\epsilon^{-\frac14}\sup_{s\in(t,t_0)}\left(\|N\|_{H^{l-1}_G(\Sigma_s)}^2+\|\Sigma\|_{H^{l-1}_G(\Sigma_s)}^2\right)\right)\label{eq:H-norm-exch-zeta}\,,\\
\|\mathfrak{T}\|^2_{H^l_\gamma(U)}\lesssim&\,a^{-c\sqrt{\epsilon}}\|\mathfrak{T}\|^2_{H^l_G(U)}+a^{-c\epsilon^\frac18}\|\mathfrak{T}\|_{C_G^{\lceil \frac{l-1}2\rceil}(\Sigma_t)}^2\left(\epsilon^4+\epsilon^{-\frac14}\sup_{s\in(t,t_0)}\left(\|N\|_{H^{l}_G(\Sigma_s)}^2+\|\Sigma\|_{H^{l}_G(\Sigma_s)}^2\right)\right) \label{eq:H-norm-exch-T}
\end{align}
\end{subequations}
%with
%\begin{equation}
...
\end{lemma}
\begin{remark}
While we only need the tensorial inequalities for gradient vector fields and $(0,2)$-tensors when applied to norms in $\mathcal{H}$ and $\mathcal{C}$, the proof is more simple when considering tensors of arbitrary rank.
\end{remark}
\begin{proof}
...
\end{align*}
We can successively replace $G^{\pm 1}$ by $(G^{\pm 1}-\gamma^{\pm 1})+\gamma^{\pm 1}$ to transform the first factor into a sum of products of $(G^{\pm 1}-\gamma^{\pm 1})$ and $\gamma^{\pm 1}$, where at least one of the former and one of the latter always occurs. Taking the $\lvert\cdot\rvert_\gamma$-norm of each factor and using \eqref{eq:APmidG}-\eqref{eq:APmidG-1} as well as $\lvert\gamma^{\pm 1}\rvert_\gamma=\sqrt{3}$, this yields
\begin{equation*}
...
\end{equation*}
and thus \eqref{eq:C-norm-exch-T} (and \eqref{eq:AP-exch-T}) for $l=0$ after taking the supremum.\\
To show \eqref{eq:H-norm-exch-T} at base level, consider
\begin{align*}
&\,\int_U \lvert\mathfrak{T}\rvert_G^2\,\vol{G}-\int_U\lvert\mathfrak{T}\rvert_{\gamma}^2\,\vol{\gamma}\\
=&\,\int_U \left(\lvert\mathfrak{T}\rvert_G^2-\lvert\mathfrak{T}\rvert_\gamma^2\right)\,\vol{G}+\int_U\lvert\mathfrak{T}\rvert_\gamma^2\frac{\mu_G-\mu_\gamma}{\mu_\gamma}\,\vol{\gamma}\,.
\end{align*}
...
\[(1-\epsilon)\|\mathfrak{T}\|_{L^2_\gamma}^2\lesssim (1+\sqrt{\epsilon}a^{-c\sqrt{\epsilon}})\|\mathfrak{T}\|_{L^2_G}^2\]
follows, implying the statement .\\

We first perform the iteration for \eqref{eq:C-norm-exch-T}, assuming the statement and the analogue with $\gamma$ and $G$ reversed to hold up to order $l-1$ on any coordinate neighbourhood and on $\Sigma_t$. As above, note that
\begin{equation*}
\left\lvert\,\left\lvert\nabla^J\mathfrak{T}\right\rvert_G^2-\left\lvert\nabhat^J\mathfrak{T}\right\rvert_\gamma^2\right\rvert\lesssim\sqrt{\epsilon}a^{-c\sqrt{\epsilon}}\left\lvert\nabhat^J\mathfrak{T}\right\rvert_\gamma^2+\left\lvert\left\lvert\nabla^J\mathfrak{T}\right\rvert_\gamma^2-\left\lvert\nabhat^J\mathfrak{T}\right\rvert_\gamma^2\right\rvert\,
\end{equation*}
...
\end{align*}
Regarding $\nabla^J\mathfrak{T}-\nabhat^J\mathfrak{T}$, we have the following schematic decomposition locally on some coordinate neighbourhood $V\subseteq\Sigma_t$:
\begin{align*}
...
&\,+\langle\text{nonlinear terms and their derivatives}\rangle\,,
\end{align*} 
Here, $\ast_\gamma$ encodes the analogous schematic product notation with regards to $\gamma$ (see subsection \ref{subsubsec:schematic-notation}). Regarding the Christoffel symbols, notice \eqref{eq:C-norm-exch-T} with roles of $\gamma$ and $G$ reversed holding up to $l-1$ in particular implies that, for any $m\in\{0,\dots,l-1\}$ and some multivariate polynomial $\tilde{P}_m$, we have
\[\|\Gamma-\Gamhat\|_{C_\gamma^{m-1}(V)}\lesssim a^{-c\sqrt{\epsilon}}\tilde{P}_m(\|\Gamma-\Gamhat\|_{C_G^{m-1}(V)},\|G-\gamma\|_{C^{m}_G(V)},\|G^{-1}-\gamma^{-1}\|_{C^{m}_G(V)})\,.\]
As explained in Remark \ref{rem:relation-metric-Chr}, we can bound $\|\Gamma-\Gamhat\|_{C_G^{m-1}(V)}$ by a polynomial in $\|G-\gamma\|_{C_G^m(V)}$. Hence, the right hand side collapses into a polynomial in $\|G-\gamma\|_{C_G^m(V)}$ and we can apply \eqref{eq:APmidG} to obtain 
\begin{equation}\label{eq:APmidChr}
\|\Gamma-\Gamhat\|_{C_\gamma^{11}(V)}\lesssim \sqrt{\epsilon}a^{-c\sqrt{\epsilon}}\,.
\end{equation}
Moving back to \eqref{eq:nabla-nabhat} and just considering the first line for now, we obtain using \eqref{eq:APmidChr} and
\begin{align*}
\numberthis\label{eq:C-norm-exch-T-last-step}\left\lvert\|\mathfrak{T}\|_{\dot{C}^l_G(V)}^2-\|\mathfrak{T}\|^2_{\dot{C}_\gamma^l(V)}\right\rvert\lesssim&\,a^{-c\sqrt{\epsilon}}\left(\|\mathfrak{T}\|_{C^l_\gamma(V)}^2+\sum_{m=0}^{l-1}\|\nabla^{m}\mathfrak{T}\|_{C^0_\gamma(V)}^2\right)\\
&\,+\left(\|\mathfrak{T}\|_{C^{\lfloor\frac{l-1}2\rfloor}_\gamma(V)}^2+\sum_{m=0}^{\lfloor\frac{l-1}2\rfloor}\|\nabla^{m}\mathfrak{T}\|_{C^0_\gamma(V)}^2\right)\|\Gamma-\Gamhat\|_{C^{l-1}_\gamma(V)}^2\,\\
&\,+\langle\text{nonlinear terms}\rangle
\end{align*}
We can rewrite $\nabla^m\mathfrak{T}$-norms in $C_\gamma$ as ones in $C_G$ up to $a^{-c\sqrt{\epsilon}}$ as before. Then, we can apply the already obtained estimates up to order $l-1$ show that the first two lines of the right hands side can be estimated by the right hand side of \eqref{eq:C-norm-exch-T}. The terms with higher order nonlinearities are essentially similar since no terms with higher order derivatives occur, neither do we get additional factors in $\mathfrak{T}$, we only incur additional powers of $(\Gamma-\Gamhat)$-norms that can be controlled by terms that are already present. Further, we can again control any Christoffel terms in $C^{l-1}_\gamma$ by a polynomial in $\|G-\gamma\|_{C^l_\gamma(M)}$. Hence, the nonlinear terms don't qualitatively change the estimates, proving \eqref{eq:C-norm-exch-T} at order $l$ on any coordinate neighbourhood after rearranging as well as the analogue with $\gamma$ and $G$ switched. Since any terms on the right hand side can be estimated by their norms on $\Sigma_t$ and $\Sigma_t$ is compact, this then also implies the estimate on $\Sigma_t$ at order $l$ and closes the proof of \eqref{eq:C-norm-exch-T}. \\
...
Now, assume \eqref{eq:H-norm-exch-T} to be proven up to order $l-1$. By analogous arguments as at order zero, we get on any coordinate neighbourhood $V$:
\[\int_V\lvert\nabhat^l\mathfrak{T}\rvert_\gamma^2\,\vol{\gamma}\lesssim \left\lvert\int_V\left(\lvert\nabla^l\mathfrak{T}\rvert_G^2-\lvert\nabhat^l\mathfrak{T}\rvert_{\gamma}^2\right)\,\vol{G}\right\rvert+(1+\sqrt{\epsilon}a^{-c\sqrt{\epsilon}})\int_V\lvert\nabla^l\mathfrak{T}\rvert_G^2\,\vol{G}\,,\]
...
\begin{equation*}
\left\lvert\,\left\lvert\nabla^l\mathfrak{T}\right\rvert_G^2-\left\lvert\nabhat^l\mathfrak{T}\right\rvert_\gamma^2\right\rvert\lesssim\sqrt{\epsilon}a^{-c\sqrt{\epsilon}}\left\lvert\nabla^l\mathfrak{T}\right\rvert_G^2+\left\lvert\left\lvert\nabla^l\mathfrak{T}\right\rvert_G^2-\left\lvert\nabhat^l\mathfrak{T}\right\rvert_G^2\right\rvert\,,
\end{equation*}
rewriting the second term as
\begin{equation*}
\left\lvert 2\langle\nabla^l\mathfrak{T}-\nabhat^l\mathfrak{T},\nabla\mathfrak{T}\rangle_G-\lvert\nabla^l\mathfrak{T}-\nabhat^l\mathfrak{T}\rvert_G^2\right\rvert
\end{equation*}
and have the following, applying Lemma \ref{lem:int-est-Chr}:
\begin{align*}
&\,\left\rvert\int_V\left\{2\langle\nabla^l\mathfrak{T}-\nabhat^l\mathfrak{T},\nabla\mathfrak{T}\rangle_G-\lvert\nabla^l\mathfrak{T}-\nabhat^l\mathfrak{T}\rvert_G^2\right\}\vol{G}\right\rvert\\
\lesssim&\,\left[\|\Gamma-\Gamhat\|_{H^{l-1}_G(V)}\left(\|\mathfrak{T}\|_{C^{\lceil\frac{l-1}2\rceil}_G(V)}+\|\nabhat^{\leq l-1}\mathfrak{T}\|_{C^{0}_G(V)}\right)\right.\\
&\,\left.+\sqrt{\epsilon}a^{-c\sqrt{\epsilon}}\left(\|\mathfrak{T}\|_{H^{l-1}_G(V)}+\|\nabhat^{\leq l-1}\mathfrak{T}\|_{H^{l-1}_G(V)}\right)\right]\cdot\|\mathfrak{T}\|_{\dot{H}_G^l(V)}\\
\lesssim&\,a^{-c\sqrt{\epsilon}}\left(\|\mathfrak{T}\|_{H^l_G(V)}^2+\|\nabhat^{(l-1)}\mathfrak{T}\|_{H^{l-1}_G(V)}^2\right)\\
&\,+\left(\|\mathfrak{T}\|^2_{C^{\lceil\frac{l-1}2\rceil}_G(\Sigma_t)}+\|\nabhat^{\leq l-1}\mathfrak{T}\|^2_{C^{\lceil\frac{l-1}2\rceil}_G(\Sigma_t)}\right)\cdot \\
&\,\quad \cdot a^{-c\epsilon^\frac18}\left(\epsilon^4+\epsilon^{-\frac14}\sup_{s\in(t,t_0)}\left(\|N\|_{H^{l}_G(\Sigma_s)}^2+\|\Sigma\|_{H^{l}_G(\Sigma_s)}^2\right)\right)
\end{align*}
By the same arguments as earlier, we have $\|\nabhat^{\leq l-1}\mathfrak{T}\|_{H^{l-1}_G(V)}\lesssim a^{-c\sqrt{\epsilon}}\|\mathfrak{T}\|_{H^{l-1}_\gamma(V)}$ and can then apply the induction hypothesis. This proves \eqref{eq:H-norm-exch-T} on any coordinate neighbourhood $V$, and the estimate for $\Sigma_t$ follows as before using compactness. 
\end{proof}
...
\mathcal{C}+\mathcal{C}_\gamma\lesssim\epsilon^\frac98a^{-c\epsilon^\frac18}
\end{equation}
\end{corollary}
\begin{proof}
We want to first apply the Sobolev norm estimates in Lemma \ref{lem:G-gamma-norm-switch} to \eqref{eq:H-norm-exch-zeta}-\eqref{eq:H-norm-exch-T}, to then control $\mathcal{C}_\gamma$ via the standard Sobolev embedding $H^{l+2}_\gamma(M)\hookrightarrow C^l(M)$, and finally control $\mathcal{C}$ with \eqref{eq:C-norm-exch-zeta}-\eqref{eq:C-norm-exch-T}.\\
...
\lesssim&\,\epsilon^\frac94a^{-c\epsilon^\frac18}+\epsilon a^{-c\epsilon^\frac18}\left(\epsilon^4+\epsilon^\frac94\right)\\
\lesssim&\,\epsilon^\frac 94\cdot a^{-c\epsilon^\frac18}
\end{align*}
In particular, we can update $c$ such that
\[\lvert P(\|G-\gamma\|_{C^{16}_\gamma(\Sigma_t)},\|G-\gamma\|_{C^{16}_\gamma(\Sigma_t)})\rvert\lesssim\epsilon^\frac98 a^{-c\epsilon^\frac18}\]
holds for any multivariate polynomial $P$ that appears when applying \eqref{eq:C-norm-exch-zeta}-\eqref{eq:C-norm-exch-T}. Again using the strong $C_G$-norm estimates from Lemma \ref{lem:AP}, this implies
...
As mentioned in Remark \ref{rem:initial-data}, it may seem that the generality of the results in Theorem \ref{thm:main} is restricted by taking the initial data on $\Sigma_{t_0}$ to be CMC. However, as long as one remains close enough to a constant time hypersurface of the FLRW reference metric (which is CMC), one can embed any geometry on $\Sigma_{t_0}$ into $\M$ in such a way that the hypersurface is CMC and the second fundamental form remains close to that of the FLRW solution. To make this a bit more precise, and also since this it is a little less involved than the arguments in \cite{Rodnianski2014}, we will briefly sketch how the arguments from \cite[Section 2.5]{FajKr20} analogize to our setting. In particular, we remark that one doesn't have to make any assumptions on the curvature tensor $(M,\gamma)$ for this to hold.\\

First, note that we can locally evolve our data within harmonic gauge (for well-posedness, consider the analogue of \cite[Proposition 14.1]{Rodnianski2014} in the present setting along with \eqref{eq:ass-lwp}) to get a $C^{17}$-regular family of metrics with near-FLRW initial data. Consider the Banach manifold $\mathcal{M}^{17}$ as the set of $C^{17}$ Lorentz metrics on $I\times\M$ for an open interval $I$ around $t_0$ such that the surfaces of constant time are Riemannian, endowed with the norm
...
(\tilde{g},f)\mapsto \text{mean curvature of }\tilde{g}_t\text{ embedded along }\iota_f\,.
\end{gather*}
Finally, we take $H=H_0-\tau(t_0)$. Then, we obviously have $H(\g_{FLRW},t_0)=0$, and can compute the differential of $H$ in the second argument using \eqref{eq:FLRW-Ric00}, \eqref{eq:Friedman} and \eqref{eq:Friedman2}:
\begin{align*}
...
\end{align*}
Since $\Lap_\gamma$ has no positive eigenvalues for any Riemannian metric, it follows that $(\g_{FLRW},t_0)$ is a regular value of $H$ and, by the implicit function theorem for Banach manifolds, this means there is a (unique) smooth function $F$ that maps an open neighbourhood of $\g_{FLRW}$ in $\mathcal{M}^{17}$ to an open neighbourbood of the constant function $x\mapsto t_0$ in $C^{17}(M,I)$ such that $H(\cdot,F(\cdot))=0$ holds on that neighbourhood. Hence, and using a similar argument to keep Sobolev norms in check, we can a surface with mean curvature $\tau(t_0)$ along the harmonic gauge evolution of the original initial data such that the data on that surface is close to the FLRW initial data in the sense of Assumption \ref{ass:init} for small enough $\epsilon$.
\end{remark}

\begin{theorem}[Stability of Big Bang formation]\label{thm:main}
Let $(M,\mathring{g},\mathring{k},\mathring{\pi},\mathring{\psi})$ be geometric initial data to the Einstein scalar-field system as discussed in Remark \ref{rem:initial-data}. Further, let the data be embedded into a time-oriented 4-manifold such that it induces initial data for the rescaled solution variables (see Definition \ref{def:rescaled}) at the initial hypersurface $\Sigma_{t_0}$. We also assume this rescaled initial data is close to that of the FLRW reference solution (see \eqref{eq:FLRW-metric} and \eqref{eq:FLRW-wave}) in the sense that
\begin{equation}
\mathcal{H}(t_0)+\mathcal{C}(t_0)\lesssim \epsilon^2
\end{equation}
is satisfied (with $\mathcal{H}$ and $\mathcal{C}$ as in Definition \ref{def:sol-norm}) as well as the top order energy assumption \eqref{eq:init-ass-top} for some small enough $\epsilon>0$. Finally, we make the additional regularity assumption \eqref{eq:ass-lwp}.\footnote{Essentially, this translates to smallness in $H_\gamma^{21}$ and $C_\gamma^{17}$ and $H_\gamma^{23}$-regularity for our solution variables, and that for $\epsilon=0$, the solution is the FLRW reference solution.}\\
...
\mathcal{H}(t)+\mathcal{C}(t)+\mathcal{C}_{\gamma}(t)\lesssim \epsilon^\frac94 a(t)^{-c\epsilon^\frac18}
\end{equation}
for some $c>0$ and any $t\in(0,t_0]$. In particular, this implies the following statements:\\[1em]

\textbf{Asymptotic behaviour of solution variables:} We denote the solution metric as $\g=-n^2dt^2+g$, the second fundamental form (viewed as a $(1,1)$-tensor) with respect to $\Sigma_t$ as $k$ and the volume form with regards to $g$ on $\Sigma_t$ by $\vol{g}$. There exist \footnote{We view these interchangeably as objects on $M$ or as time independent objects on $(0,t_0]\times M$.} a smooth function $\Psi_{Bang}\in C_\gamma^{15}(M)$, a $(1,1)$-tensor field $K_{Bang}\in C^{15}_\gamma(M)$ and a volume form $\vol{Bang}\in C^{15}_\gamma(M)$ such that the following estimates hold for any $t\in(0,t_0]$:
\begin{subequations}
\begin{align}
...
\end{cases}\\
%%%%%%%%%%%%%%%%%%%%
\label{eq:asymp-phi}\left\|\phi-\int_t^{t_0}a(s)^{-3}\,ds\cdot(\Psi_{Bang}+C)\right\|_{\dot{C}^l_\gamma(\Sigma_t)}\lesssim&\,\begin{cases}
%\epsilon a^{4-c\sqrt{\epsilon}} & l\leq 13\\
...
\noindent Further, these footprint states satisfy the equations
\begin{subequations}
\begin{align}
...
\begin{equation}
\label{eq:asymp-G}\left\|g\odot\exp\left[\left(2\int_t^{t_0}a(s)^{-3}\,ds\right)\cdot K_{Bang}\right]-M_{Bang}\right\|_{C^l_\gamma(\Sigma_t)}\lesssim\begin{cases}
%\epsilon a(t)^{4-c\sqrt{\epsilon}} & l\leq 12\\
 \epsilon a(t)^{4-c\epsilon^\frac18} & l\leq 14\\
\epsilon a(t)^{2-c\epsilon^\frac18} & l=15\,.
\end{cases}
\end{equation}
Moreover, the Bel-Robinson variables $E$ and $B$ satisfy the following estimates:
\begin{subequations}
\begin{align}
...
\textbf{Causal disconnectedness:} Let $\curve$ be a past directed causal curve on $((0,t_0]\times M,\g)$ with domain $[s_1,s_{max})$ such that $\curve(s_1)\in \Sigma_{t_0}$ and $s_{max}$ is maximal. Then, there exists a constant $\mathcal{K}>0$ that does not depend on $\curve$ such that one has
\begin{equation}\label{eq:causal-disconn}
L[\curve]=\int_{s_1}^{s_{max}}\sqrt{\gamma_{ab}(\curve(s))\dot{\curve}^a(s)\dot{\curve}^b(s)}\,ds\leq \mathcal{K} a(t)^{2-c\epsilon^\frac18}\,,
\end{equation}
where $\gamma$ is the spatial reference metric on $M$.\\

\textbf{Geodesic incompleteness:} Let $\curve$ be a past directed, affinely parametrized causal geodesic emanating from $\Sigma_{t_0}$ with parameter time $\mathcal{A}$ normalized to $\mathcal{A}(t_0)=0$. Then,
...
\end{equation}
holds for suitable constants $\mathcal{K}_1,\mathcal{K}_2>0$ that are independent of $\curve$, and thus any such geodesic crashes into the Big Bang hypersurface in finite time.\\
...
\left\|a^6\lvert k\rvert_g^2-(K_{Bang})^i_j(K_{Bang})^j_i\right\|_{C^0_\gamma(\Sigma_t)}\lesssim \epsilon a^{4-c{\epsilon}^\frac18}
\end{equation}
Further, under the decomposition $\Riem[\g]=P[\g]+W[\g]$,
...
\end{equation}
Finally, the Kretschmann scalar $\mathcal{K}=\Riem[\g]_{\alpha\beta\gamma\delta}\Riem[\g]^{\alpha\beta\gamma\delta}$ exhibits the following blow-up behaviour:
\begin{equation}\label{eq:blowup-Kretschmann}
\left\|a^{12}\mathcal{K}-\frac{5}3\cdot(8\pi)^2(\Psi_{Bang}+C)^4\right\|_{C^0(M)}\lesssim \epsilon a^{4-c\epsilon^\frac18}
\end{equation}
\end{subequations}
\end{theorem}
...
\end{remark}
\begin{proof}
The initial data assumptions (see Assumption \ref{ass:init}) imply the local well-posedness results in Section \ref{subsec:lwp} for $N=21$. In particular, these imply that $(G,\Sigma,N,\Psi,\nabla\phi,\RE,\RB)$ have sufficiently high regularity that the bootstrap assumptions (see Assumption \ref{ass:bootstrap}) hold on some interval $[t_1,t_0]$ with $t_1\in(0,t_0)$ for $\sigma=\epsilon^\frac1{16}$, since all norms and energies are continuous in time. Let $\mathfrak{t}\in(0,t_0)$ be such that $(\mathfrak{t},t_0]\times M$ is the maximal domain on which the solution variables exist and satisfy the bootstrap assumptions. For contradiction, we now assume that $\mathfrak{t}>0$ were to hold.\\
...
\mathcal{C}(t)\leq K_1\epsilon^\frac94a(t)^{-c_1\epsilon^\frac18}\,
\end{equation}
is satisfied as well as, due to Propositions \ref{prop:en-bs-imp} and Proposition \ref{prop:en-bs-imp-top},
\begin{subequations}
\begin{align}
\E^{(\leq 19)}(\phi,t)\leq&\,K_1\epsilon^{\frac{11}4}a(t)^{-c_1\epsilon^\frac18}\label{eq:en-imp-main-SF}\\
...
\[\|\Psi(t,\cdot)-({\Psi}_{Bang}-C)\|_{C^l_\gamma(M)}\lesssim\begin{cases}
\epsilon a(t)^{4-c\epsilon^\frac18} & l\leq 14\\
\epsilon a(t)^{2-c\epsilon^\frac18} & l=15
\end{cases}\]
...
\eqref{eq:Bang-CMC} follows directly from the CMC condition \eqref{eq:CMC}, the asymptotic behaviour \eqref{eq:asymp-K} of $a^3k$ and the Friedman equation \eqref{eq:Friedman}. For \eqref{eq:Bang-Hamil}, the Hamiltonian constraint \eqref{eq:Hamilton} implies
\[8\pi\lvert a^3n^{-1}\del_t\phi\rvert^2+(a^3{k^l}_m)(a^3{k^m}_l)=9a^6\left(\frac{\dot{a}}a\right)^2+a^4\cdot \left(a^2g^{ij}\right)\left(\Ric[g]_{ij}-\nabla_i\phi\nabla_j\phi\right)\,.\]
The left hand side converges to the left hand side of \eqref{eq:Bang-Hamil} by \eqref{eq:asymp-lapse},\eqref{eq:asymp-Psi} and \eqref{eq:asymp-K}, while the right hand side converges to $12\pi C^2$ by \eqref{eq:Friedman} and by \eqref{eq:bs-imp-main-thm}. \eqref{eq:asymp-G} follows exactly as in \cite[Theorem 15.1]{Rodnianski2014}, and \eqref{eq:footprint-vol}-\eqref{eq:footprint-G} are a direct result of the initial data assumptions and applying the respective asymptotic estimates to $t=t_0$.

\noindent For the first estimate in \eqref{eq:asymp-B}, we apply the momentum constraint \eqref{eq:REEqConstrB} to get
\[\lvert\nabla^l B\rvert_G=a^{-3}\lvert\nabla^l \RB\rvert_G= a^{-1}\lvert\nabla^l\curl_g\Sigma\rvert_G\lesssim a^{-2}\lvert\nabla^{l+1}\Sigma\rvert_G\]
...
\end{align*}
The remaining estimates in \eqref{eq:asymp-E} and \eqref{eq:asymp-B} are contained in \eqref{eq:bs-imp-main-thm}. \\

The results \eqref{eq:causal-disconn} and \eqref{eq:geod-incomp} follow as in the proofs of (15.6) and (15.7) in \cite[Theorem 15.1]{Rodnianski2014} from the asymptotic behaviour of the solution variables in \eqref{eq:asymp-lapse}-\eqref{eq:asymp-K} and \eqref{eq:asymp-G}.\\
...
and, using \eqref{eq:Weyl-reconstruct} and standard algebraic manipulations, get
\[a^{12}W_{\alpha\beta\gamma\delta}W^{\alpha\beta\gamma\delta}=a^{12}\left(20\lvert E\rvert_g^2+8\lvert B\rvert_g^2\right)=20\lvert\RE\rvert_G^2+8\lvert\RB\rvert_G^2\,.\]
...
\end{align*}
Since $\Sigma^\sharp$ and $\Psi$ converge to footprint states $\hat{K}_{Bang}=K_{Bang}+\sqrt{\frac{4\pi C}3}\I$ and $\Psi_{Bang}$ in $C^{15}_\gamma(M)$ respectively, this shows that $W_{Bang}\in C^{15}_\gamma(M)$ exists, which is a fourth order polynomial in $\hat{K}_{Bang}$ and $\Psi_{Bang}$, such that
\[\left\|\lvert\RE\rvert_G^2-\frac1{20}W_{Bang}\right\|_{C^0(M)}\lesssim \epsilon a^{2-c\epsilon^\frac18}\]
is satisfied, where \eqref{eq:footprint-W} is a consequence of \eqref{eq:footprint-Psi} and \eqref{eq:footprint-K}. Due to \eqref{eq:asymp-B}, the $\lvert\RB\rvert_G^2$-term in the Weyl curvature scalar is negligible in comparison, and thus \eqref{eq:blowup-W} immediately follows.\\
Furthermore, denoting the Schouten tensor with regards to $\g$ by $\text{Sch}$, one has 
\begin{align*}
P_{\alpha\beta\gamma\delta}P^{\alpha\beta\gamma\delta}=&\,8\text{Sch}_{\alpha\beta}\text{Sch}^{\alpha\beta}-4{\text{Sch}^\alpha}_{\alpha}\\
=&\,2\Ric[\g]_{\alpha\beta}\Ric[\g]^{\alpha\beta}-\frac29 R[\g]^2-\frac16R[\g]
\end{align*}
...
\subsubsection{Levi-Civita tensor identites}\label{subsubsec:basic-contract}
...
\end{align}
\end{subequations}
The analogous formulas hold for $\epsilonLC[G]$ when raising indices with regards to $G$ instead of $g$.\\
\noindent For a tracefree and symmetric $\Sigma_t$-tangent $(0,2)$-tensor $\mathfrak{T}$ and a $\Sigma_t$-tangent $(0,2)$-tensor $\mathfrak{A}$, the following simplified identites hold:
\begin{subequations}
\begin{align}
...
\begin{subequations}
\begin{align}
\lvert \mathfrak{M}\odot_g\mathfrak{N}\rvert_g\leq\lvert \mathfrak{M}\rvert_g\lvert \mathfrak{N}\rvert_g,&\quad \lvert\mathfrak{M}\odot_G\mathfrak{N}\rvert_G\leq \lvert \mathfrak{M}\rvert_G\lvert \mathfrak{N}\rvert_G\label{eq:odot}\\
\lvert \mathfrak{S}\times \mathfrak{T}\rvert_g\lesssim\lvert \mathfrak{S}\rvert_g\lvert \mathfrak{T}\rvert_g,&\quad \lvert \mathfrak{S}\times \mathfrak{T}\rvert_G\lesssim a^{-2}\lvert \mathfrak{S}\rvert_G\lvert \mathfrak{T}\rvert_G \label{eq:times}\\
\lvert \mathfrak{S}\wedge \mathfrak{T}\rvert_g\leq \lvert \mathfrak{S}\rvert_g\lvert \mathfrak{T}\rvert_g,&\quad \lvert \mathfrak{S}\wedge \mathfrak{T}\rvert_G\leq a^{-3}\lvert\mathfrak{S}\rvert_G\lvert \mathfrak{T}\rvert_G\label{eq:wedge}\\
\lvert \xi\wedge \mathfrak{T}\rvert_g\leq\lvert \xi\rvert_g\lvert \mathfrak{T}\rvert_g,&\quad \lvert\xi\wedge \mathfrak{T}\rvert_G\leq a^{-1}\lvert\xi\rvert_G\lvert \mathfrak{T}\rvert_G\label{eq:wedge2}\\
\lvert\curl\mathfrak{M}\rvert_g\lesssim \lvert\nabla \mathfrak{M}\rvert_g,&\quad \lvert\curl \mathfrak{M}\rvert_G\lesssim a^{-1}\lvert\nabla \mathfrak{M}\rvert_G\label{eq:curl}
\end{align}
\end{subequations}
\end{lemma}
\begin{proof}
...
\end{align*}
The respective inequality with regards to $G$ now follows by rescaling.\\
...
\begin{corollary}[Schematic first order spatial commutators]
For $\zeta$ and $\mathfrak{T}$ as above, the following identities hold:
\begin{subequations}
\begin{align}
[\Lap,\nabla]\zeta=&\,\Ric[G]\ast\zeta\label{eq:[Lap,nabla]SF}\\
...
[\Lap,\curl_g]=&\,a^{-1}\epsilonLC[G]\ast\left(\Ric[G]\ast\nabla\mathfrak{T}+\nabla\Ric[G]\ast\mathfrak{T}\right)\label{eq:[Lap,curl]}
\end{align}
\end{subequations}
\end{corollary}
\begin{proof}
Since we are working three spatial dimensions, the following identity holds:
\begin{align*}
...
\end{align*}
Hence, for any $I\in\N_0$, any $\nabla^I\Riem[G]$-term reduces to a sum of products and contractions of $\nabla^I\Ric[G]$ with various metric tensors that are all surpressed in schematic notation. With this in mind, \eqref{eq:[Lap,nabla]SF}-\eqref{eq:[Lap,div]} are simply schematic versions of \eqref{eq:[Lap,nabla]SF-full}-\eqref{eq:[Lap,div]-full}. To derive \eqref{eq:[Lap,curl]} from \eqref{eq:[Lap,curl]-full}, additionally recall
...
\numberthis\label{eq:[Lap-l,nabla]SF-junk}\mathfrak{J}([\Lap^l,\nabla]\zeta)=&ß,\sum_{I_1+I_\zeta=2(l-1),\,I_\zeta\geq 2}\nabla^{I_1}\Ric[G]\ast\nabla^{I_\zeta+1}\zeta\\
&\,+\sum_{m=0}^{l-2}\sum_{I_1+\dots+I_{l-m}+I_{\zeta}=2m}\nabla^{I_1}\Ric[G]\ast\dots\ast\nabla^{I_{l-m}}\Ric[G]\ast\nabla^{I_\zeta+1}\zeta\\[1em]
\numberthis\label{eq:[Lap-l,nabla2]SF}[\Lap^l,\nabla^2]\zeta=&\,\nabla\Lap^{l-1}\Ric[G]\ast\nabla\zeta+\nabla^2\Lap^{l-2}\Ric[G]\ast\nabla^2\zeta+\mathfrak{J}([\Lap^l,\nabla^2]\zeta)\\
\numberthis\label{eq:[Lap-l,nabla2]SF-junk}\mathfrak{J}([\Lap^l,\nabla^2]\zeta)=&\,\sum_{I_1+I_\zeta=2(l-1)+1,\,I_1,I_\zeta\geq 2}\nabla^{I_1}\Ric[G]\ast\nabla^{I_\zeta+1}\zeta\\
&\,+\sum_{m=0}^{l-2}\sum_{I_1+\dots+I_{l-m}+I_\zeta=2m+1}\nabla^{I_1}\Ric[G]\ast\dots\ast\nabla^{I_{l-m}}\Ric[G]\ast\nabla^{I_\zeta+1}\zeta\\[1em]
\numberthis\label{eq:[Lap-l,nabla]T}[\Lap^l,\nabla]\mathfrak{T}=&\,\nabla\Lap^{l-1}\Ric[G]\ast \mathfrak{T}+\nabla^2\Lap^{l-2}\Ric[G]\ast\nabla \mathfrak{T}+\mathfrak{J}([\Lap^l,\nabla]\mathfrak{T})\,,\\
\numberthis\label{eq:[Lap-l,nabla]T-junk}\mathfrak{J}([\Lap^l,\nabla]\mathfrak{T})=&\,\sum_{I_1+I_\mathfrak{T}=2(l-1)+1,\,I_\mathfrak{T}\geq2}\nabla^{I_1}\Ric[G]\ast\nabla^{I_\mathfrak{T}}\mathfrak{T}\\
&\,+\sum_{m=0}^{l-2}\sum_{I_1+\dots+I_{l-m}+I_\mathfrak{T}=2m+1}\nabla^{I_1}\Ric[G]\ast\dots\ast\nabla^{I_{l-m}}\Ric[G]\ast\nabla^{I_\mathfrak{T}}\mathfrak{T}\\[1em]
\numberthis\label{eq:[Lap-l,nabla2]T}[\Lap^l,\nabla^2]\mathfrak{T}=&\,\nabla^2\Lap^{l-1}\Ric[G]\ast \mathfrak{T}+\nabla^3\Lap^{l-2}\Ric[G]\ast \nabla \mathfrak{T}+\mathfrak{J}([\Lap^l,\nabla^2]\mathfrak{T})\,,\\
\numberthis\label{eq:[Lap-l,nabla2]T-junk}\mathfrak{J}([\Lap^l,\nabla^2]\mathfrak{T})=&\,\sum_{I_1+I_\mathfrak{T}=2l,\,I_\mathfrak{T}\geq2}\nabla^{I_1}\Ric[G]\ast\nabla^{I_\mathfrak{T}}\mathfrak{T}\\
&\,+\sum_{m=0}^{l-2}\sum_{I_1+\dots+I_{l-m}+I_\mathfrak{T}=2m+2}\nabla^{I_1}\Ric[G]\ast\dots\ast\nabla^{I_{l-m}}\Ric[G]\ast\nabla^{I_\mathfrak{T}}T\\[1em]
\numberthis\label{eq:[Lap-l,div]T}[\Lap^l,\div_G]\mathfrak{T}=&\,\nabla\Lap^{l-1}\Ric[G]\ast \mathfrak{T}+\nabla^2\Lap^{l-2}\Ric[G]\ast \nabla \mathfrak{T}+\mathfrak{J}([\Lap^l,\div_G]\mathfrak{T})\,,\\
\numberthis\label{eq:[Lap-l,div]T-junk}\mathfrak{J}([\Lap^l,\div_G]\mathfrak{T})=&\,\sum_{I_1+I_\mathfrak{T}=2(l-1)+1,\,I_\mathfrak{T}\geq2}\nabla^{I_1}\Ric[G]\ast\nabla^{I_\mathfrak{T}}\mathfrak{T}\\
&\,+\sum_{m=0}^{l-2}\sum_{I_1+\dots+I_{l-m}+I_\mathfrak{T}=2m+1}\nabla^{I_1}\Ric[G]\ast\dots\ast\nabla^{I_{l-m}}\Ric[G]\ast\nabla^{I_\mathfrak{T}}\mathfrak{T}\\[1em]
\numberthis\label{eq:[Lap-l,curl]}[\Lap^l,\curl]\mathfrak{T}=&\,a^{-1}\epsilonLC[G]\ast\left(\nabla\Lap^{l-1}\Ric[G]\ast \mathfrak{T}+\nabla^2\Lap^{l-2}\Ric[G]\ast\nabla \mathfrak{T}\right)+\mathfrak{J}([\Lap^l,\curl]\mathfrak{T})\\
\numberthis\label{eq:[Lap-l,curl]-junk}\mathfrak{J}([\Lap^l,\curl]\mathfrak{T})=&\,a^{-1}\epsilonLC[G]\ast\left[\sum_{I_1+I_\mathfrak{T}=2(l-1)+1,\,I_\mathfrak{T}\geq2}\nabla^{I_1}\Ric[G]\ast\nabla^{I_\mathfrak{T}}\mathfrak{T}\right.\\
&\,\left.+\sum_{m=0}^{l-2}\sum_{I_1+\dots+I_{l-m}+I_\mathfrak{T}=2m+1}\nabla^{I_1}\Ric[G]\ast\dots\ast\nabla^{I_{l-m}}\Ric[G]\ast\nabla^{I_\mathfrak{T}}\mathfrak{T}\right]
\end{align*}
\end{subequations}
\end{lemma}
\begin{proof}
...
\begin{lemma}[Time derivative commutators]\label{lem:com-time-first} With respect to a solution to the Einstein scalar field system as in Proposition \eqref{prop:eq}, the following commutator formulas hold:
\begin{subequations}
\begin{align*}
...
\numberthis\label{eq:[del-t,nabla]T}[\del_t,\nabla_i]\mathfrak{T}_{kl}%=&\,-2\del_t\Gamma^m_{i(k}T_{ml)}\\
=&\,\left\{2(N+1)a^{-3}\left[\nabla_i\Sigma_{(k}^m+\nabla_{(k}\Sigma_i^{\sharp m}-\nabla^m\Sigma_{i(k}\right]\right.\\
&\ +2a^{-3}\left.\left[\nabla_i N\Sigma^{\sharp m}_{(k}+\nabla_{(k}N\Sigma^{\sharp m}_i-\nabla^{\sharp m}N\Sigma_{i(k}\right]\right\}\mathfrak{T}_{ml)}\\
&-\frac{\dot{a}}a\left[\nabla_iN\mathfrak{T}_{kl}+\nabla_{(k}N\mathfrak{T}_{il)}-\nabla^{\sharp m}NG_{i(k}\mathfrak{T}_{ml)}\right]\\
\numberthis\label{eq:[del-t,Lap]T}[\del_t,\Lap]\mathfrak{T}_{kl}%&\,(\del_tG^{-1})^{ij}\nabla_i\nabla_j\mathfrak{T}_{kl}-[(G^{-1})^{ij}\del_t\Gamma_{ij}^m]\nabla_m\mathfrak{T}_{kl}\\
...
=&\,a^{-3}(N+1)\Sigma\ast\nabla^2\mathfrak{T}+\frac{\dot{a}}aN\mathfrak{T}+a^{-3}\nabla((N+1)\Sigma)\ast\nabla \mathfrak{T}+\frac{\dot{a}}a\nabla N\ast\nabla \mathfrak{T}\\
&\,+a^{-3}\nabla^2((N+1)\Sigma)\ast \mathfrak{T}-\frac{\dot{a}}a\nabla^2N\ast \mathfrak{T}
\end{align*}
\end{subequations}
\end{lemma}
\begin{proof}
\eqref{eq:[del-t,nabla]zeta} is simply that coordinate derivatives commute, and \eqref{eq:[del-t,nabla-sharp]zeta} follows by applying \eqref{eq:REEqG-1} and the product rule.\\
For the tensorial commutators \eqref{eq:[del-t,nabla]T} and \eqref{eq:[del-t,Lap]T}, we write out the covariant derivatives in local coordinates, apply the product rule, and then the evolution equations \eqref{eq:REEqG-1} and \eqref{eq:REEqChr} for the inverse metric and Christoffel symbols. For \eqref{eq:[del-t,Lap]zeta} we do the same at first and get the following:
\begin{align*}
...
\end{align*}
where the junk terms take are as follows:
{\scriptsize
\begin{align*}
\numberthis\label{eq:[del-t,nabla-Lap-l]zeta-junk}\mathfrak{J}([\del_t,\nabla\Lap^l]\zeta)=&\,
a^{-3}\sum_{\substack{I_N+I_\Sigma+I_\zeta=2(l-1)+1\\(I_\Sigma, I_\zeta)\neq(0,2(l-1)+1),(1,2(l-1))}}\nabla^{I_N}(N+1)\ast\nabla^{I_\Sigma}\Sigma\ast\nabla^{I_\zeta+1}\zeta\\
&\,+a^{-3}\sum_{\substack{I_N+I_\Psi+I_\phi+I_\zeta=2(l-1)+1\\\,I_N,I_\Psi,I_\phi,I_\zeta\neq2(l-1)+1}}\nabla^{I_N}(N+1)\ast\nabla^{I_\Psi}(\Psi+C)\ast\nabla^{I_\phi+1}\phi\ast\nabla^{I_\zeta+1}\zeta\\
&\,+\frac{\dot{a}}a\sum_{\substack{I_N+I_\zeta=2l\\\,I_\zeta\neq 0}}\nabla^{I_N}N\ast\nabla^{I_\zeta+1}\zeta\\
&\,+a^{-3}\sum_{m=0}^{l-2}\sum_{\substack{I_N+I_\Sigma+I_\zeta\\+\sum_{i=1}^{l-m-1}I_i=2m+1}}\nabla^{I_N}(N+1)\ast\nabla^{I_\Sigma}\Sigma\ast\nabla^{I_1}\Ric[G]\ast\dots\ast\nabla^{I_{l-m-1}}\Ric[G]\ast\nabla^{I_\zeta+2}\zeta\\
&\,+a^{-3}\sum_{m=0}^{l-2}\sum_{\substack{I_N+I_\Sigma+I_\zeta\\+\sum_{i=1}^{l-m-1}I_i=2m+1\\I_1\neq 2l-3}}\nabla^{I_N}(N+1)\ast\nabla^{I_\Sigma}\Sigma\ast\nabla^{I_1+1}\Ric[G]\ast\dots\ast\nabla^{I_{l-m-1}}\Ric[G]\ast\nabla^{I_\zeta+1}\zeta\\
&\,+a^{-3}\sum_{m=0}^{l-2}\sum_{\substack{I_N+I_\Psi+I_\phi+I_\zeta\\+\sum_{i=1}^{l-m-1}I_i=2m+1}}\nabla^{I_N}(N+1)\ast\nabla^{I_\Psi}(\Psi+C)\ast\nabla^{I_1}\Ric[G]\ast\dots\ast\nabla^{I_{l-m-1}}\Ric[G]\\
&\,\phantom{+a^{-3}\sum_{m=0}^{l-2}\sum_{\substack{I_N+I_\Psi+I_\phi+I_\zeta\\+\sum_{i=1}^{l-m-1}I_i=2m}}}\ast\nabla^{{I_\phi}+1}\phi\ast\nabla^{I_\zeta+1}\zeta\\
&\,+\frac{\dot{a}}a\sum_{m=0}^{l-1}\sum_{\substack{I_N+\sum_{i=1}^{l-m-1}I_i+I_\zeta=2m\\\,I_\zeta\neq2(l-1)}}\nabla^{I_N}N\ast\nabla^{I_1}\Ric[G]\ast\dots\ast\nabla^{I_{l-m-1}}\Ric[G]\ast\nabla^{I_\zeta+1}\zeta\\
%%%%%%%%%%%%%%
\numberthis\label{eq:[del-t,Lap]zeta-junk}\mathfrak{J}([\del_t,\Lap^l]\zeta)=&\,a^{-3}\sum_{\substack{I_N+I_\Sigma+I_\zeta=2(l-1)\\I_\zeta\neq2(l-1)}}\nabla^{I_N}(N+1)\ast\nabla^{I_\Sigma}\Sigma\ast\nabla^{I_\zeta+2}\zeta\\
...
&\,+a^{-3}\sum_{m=0}^{l-2}\sum_{\substack{I_N+I_\Sigma+I_\zeta\\+\sum_{i=1}^{l-m-1}I_i=2m}}\nabla^{I_N}(N+1)\ast\nabla^{I_\Sigma}\Sigma\ast\nabla^{I_1}\Ric[G]\ast\dots\ast\nabla^{I_{l-m-1}}\Ric[G]\ast\nabla^{I_\zeta+2}\zeta\\
&\,+a^{-3}\sum_{m=0}^{l-2}\sum_{\substack{I_N+I_\Sigma+I_\zeta\\+\sum_{i=1}^{l-m-1}I_i=2m\\I_1\neq 2l-4}}\nabla^{I_N}(N+1)\ast\nabla^{I_\Sigma}\Sigma\ast\nabla^{I_1+1}\Ric[G]\ast\dots\ast\nabla^{I_{l-m-1}}\Ric[G]\ast\nabla^{I_\zeta+1}\zeta\\
&\,+a^{-3}\sum_{m=0}^{l-2}\sum_{\substack{I_N+I_\Psi+I_\phi+I_\zeta\\+\sum_{i=1}^{l-m-1}I_i=2m}}\nabla^{I_N}(N+1)\ast\nabla^{I_\Psi}(\Psi+C)\ast\nabla^{I_1}\Ric[G]\ast\dots\ast\nabla^{I_{l-m-1}}\Ric[G]\\
&\,\phantom{+a^{-3}\sum_{m=0}^{l-2}\sum_{\substack{I_N+I_\Psi+I_\phi+I_\zeta\\+\sum_{i=1}^{l-m-1}I_i=2m}}}\ast\nabla^{{I_\phi}+1}\phi\ast\nabla^{I_\zeta+1}\zeta\\
&\,+\frac{\dot{a}}a\sum_{m=0}^{l-1}\sum_{\substack{I_N+\sum_{i=1}^{l-m-1}I_i+I_\zeta=2m-1\\\,I_\zeta\neq2(l-1)}}\nabla^{I_N}N\ast\nabla^{I_1}\Ric[G]\ast\dots\ast\nabla^{I_{l-m-1}}\Ric[G]\ast\nabla^{I_\zeta+1}\zeta\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
&\,+a^{-3}\sum_{m=0}^{l-1}\sum_{\substack{I_N+I_\Sigma+I_\mathfrak{T}\\+\sum_{i=1}^{l-m-1}I_i=2m\\ I_1<2l-3}}\nabla^{I_N}(N+1)\ast\nabla^{I_\Sigma}\Sigma\ast\nabla^{I_1}\Ric[G]\ast\dots\ast\nabla^{I_{l-m-1}}\Ric[G]\ast\nabla^{I_\mathfrak{T}}\mathfrak{T}\\
&\,+\frac{\dot{a}}a\sum_{I_N+I_\mathfrak{T}=2l,\,I_\mathfrak{T}\geq 2}\nabla^{I_N}N\ast\nabla^{I_\mathfrak{T}}\mathfrak{T}\\
%&\,+a^{-3}\sum_{\substack{I_N+I_\Psi+I_\phi+I_\mathfrak{T}=2l-1\\I_i\neq0}}\nabla^{I_N}(N+1)\ast\nabla^{I_\Psi}(\Psi+C)\ast\nabla^{I_\phi+1}\phi\ast\nabla^{I_\mathfrak{T}}\mathfrak{T}\\
%&\,+a^{-3}\sum_{m=0}^{l-2}\sum_{\substack{I_N+I_\Psi+I_\phi+I_\mathfrak{T}\\+\sum_{i=1}^{l-m-1}I_i=2m-1}}\nabla^{I_N}(N+1)\ast\nabla^{I_\Psi}(\Psi+C)\ast\nabla^{I_\phi+1}\phi\ast\nabla^{I_1}\Ric\ast\dots\ast\nabla^{I_{l-m-1}}\Ric\ast\nabla^{I_\mathfrak{T}}\mathfrak{T}
...
\end{lemma}
\begin{proof}
Again, this follows by iteratively applying the commutators in Lemma \ref{lem:com-time-first}.
\end{proof}
...
\numberthis\label{eq:comeq-mom-curl-junk}\tilde{\mathfrak{M}}_{L,Junk}=&\,\underbrace{-a^{-1}\epsilonLC[G]\ast\nabla^{L-3}\Ric[G]\ast\nabla\Sigma}_{\text{if }L\neq 2}-\mathfrak{J}([\Lap^\frac{L}2,\curl]\Sigma)\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{subequations}
\begin{align*}
\mathfrak{P}_{L,Border}=&\,-3\Psi\frac{\dot{a}}a\Lap^{\frac{L}2}N+\frac{\dot{a}}a\nabla\Psi\ast\nabla^{L-1}N \numberthis\label{eq:comeq-Psi-even-border}\\
&\,+2a^{-3}(N+1)\langle\Sigma,\nabla^2\Lap^{\frac{L}2-1}\Psi\rangle_G+2a^{-3}(N+1)\nabla^{L-3}\Ric\ast\Sigma\ast\nabla\Psi\\
...
&\,+a^{-3}(N+1)\nabla\phi\ast\nabla\Psi\ast\nabla\Lap^{\frac{L}2-1}\Psi+\mathfrak{J}([\del_t,\nabla\Lap^\frac{L}2]\Psi)\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{align*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\mathfrak{Q}_{1,Border}=&\,a^{-3}\Psi\Lap N+a^{-3}(N+1)\Sigma\ast\nabla^2\phi \numberthis\label{eq:comeq-Q-1-border}+a^{-3}(N+1)(\Psi+C)\nabla\phi\ast\nabla\phi\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{subequations}
\begin{align*}
\mathfrak{S}_{L,Border}=&\,a^{-3}(N+1)\left(\Sigma\ast\nabla^2\Lap^{\frac{L}2-1}\Sigma+\nabla\Sigma\ast\nabla^3\Lap^{\frac{L}2-2}\Sigma\right) \numberthis\label{eq:comeq-Sigma-border}\\
&\,+a^{-3}\left(\Lap^{\frac{L}2}N\cdot(\Sigma\ast\Sigma)+\nabla\Lap^{\frac{L}2-1}N\ast\nabla\Sigma\ast\Sigma\right)\\
&\,+a^{-3}(N+1)\Sigma\ast\Sigma\ast\nabla^2\Lap^{\frac{L}2-2}\Ric[G]\\
&\,+\frac{\dot{a}}a\Lap^{\frac{L}2}N\ast\Sigma+\frac{\dot{a}}a\nabla\Lap^{\frac{L}2-1}N\ast\nabla\Sigma\\
&\,+\underbrace{a^{-3}[(N+1)\nabla\Sigma\ast\Sigma+\nabla N\ast\Sigma\ast\Sigma]\ast\nabla^{L-3}\Ric[G]}_{\text{not present for }L=2}\\
\mathfrak{S}_{L,Junk}=&\,-a[\Lap^\frac{L}2,\nabla^2]N+a\sum_{I_N+I_\Ric=L,I_N\neq 0}\nabla^{I_N} N\ast\nabla^{I_\Ric}\Ric[G] \numberthis\label{eq:comeq-Sigma-junk}\\
&\,+\frac{\dot{a}}a\sum_{I_N+I_\Sigma=L}\nabla^{I_N}N\ast\nabla^{I_\Sigma}\Sigma+a^{-3}\sum_{I_1+I_2=L,\,I_i>0}\nabla^{I_1}\Sigma\ast\nabla^{I_2}\Sigma\\
&\,+a^{-3}\sum_{I_N+I_{1}+I_{2}=L,\,I_N<L}\nabla^{I_N}N\ast\nabla^{I_1}\Sigma\ast\nabla^{I_2}\Sigma\\
%&\,+a\sum_{m=0}^{\frac{L}2}\sum_{I_N+I_{\phi,1}+I_{\phi,2}+\sum_{i=1}^{\nicefrac{L}2-m}I_i=2m}\nabla^{I_N}(N+1)\ast\nabla^{I_1}\Ric[G]\ast\dots\ast\nabla^{I_{\frac{L}2-m}}\Ric[G]\\
&\,+a\sum_{I_N+I_1+I_2=L}\nabla^{I_N}(N+1)\ast\nabla^{I_1+1}\phi\ast\nabla^{I_2+1}\phi\\
&\,+\left(4\pi C^2a^{-3}+\frac13a\right)\Lap^\frac{L}2N\cdot G+\mathfrak{J}([\del_t,\Lap^{\frac{L}2}]\Sigma)
\end{align*}
\end{subequations}
while the commuted Ricci tensor evolution equations have error terms
{\scriptsize
\begin{subequations}
\begin{align*}
...
&\,+a^{-3}\sum_{m=0}^{\frac{L}2-1}\sum_{{I_N+I_\Sigma+\sum_{i=1}^{\nicefrac{L}2-m+1}I_i=2m}}\nabla^{I_N}(N+1)\ast\nabla^{I_\Sigma}\Sigma\ast\nabla^{I_1}\Ric[G]\ast\dots\ast\nabla^{I_{\frac{L}2-m+1}}\Ric[G]\\
...
&\,+a\left\{(\Psi+C)[\Lap^\frac{L}2,\nabla]N+(N+1)[\Lap^\frac{L}2,\nabla]\Psi\right\}\ast\nabla\phi+(N+1)\mathfrak{J}([\Lap^\frac{L}2,\curl]\RB)+\mathfrak{J}([\del_t,\Lap^{\frac{L}2}]\RE)\\
...
&\,-(N+1)\mathfrak{J}([\Lap^\frac{L}2,\curl]\RE)+\mathfrak{J}([\del_t,\Lap^{\frac{L}2}]\RB)+\Lap^\frac{L}2\left[a^{-3}(N+1)\RB\ast\Sigma\right]\cdot G\\
...
\begin{remark}\label{rem:notation-parallel}
For junk terms that occur in an inner product with a tracefree symmetric tensor, any terms that are pure trace or antisymmetric will immediately cancel and thus don't need to be taken into consideration for the following estimates, even if they have to be written down in the junk terms. Hence, we will denote with a superscript \enquote{$\parallel$} (for example $\mathfrak{H}_{L,Junk}^\parallel$) on a schematic error term the expressions that arise when dropping all terms of the form $\zeta\cdot G$ for some scalar function $\zeta$ and those of the form $\xi^k\epsilonLC[G]_{(\cdot)k(\cdot)}$ for some $(1,0)$-tensor $\xi$ that occur in this term's definition (see, for example, \eqref{eq:comeq-Ham-junk}).%\footnote{We could also move directly to the tracefree components of these schematic terms, but this creates further notational baggage in identifying the tracefree parts of all individual terms, so we stick to this simplified notation that is sufficient for our purposes}
\end{remark}

\subsection{$L^2_G$ error term estimates}\label{subsec:L2-error-est}

In this subsection, we collect how the error terms can be controlled in terms of energies as well as homogenous Sobolev norms of $\phi$. We don't claim that these estimates are optimal -- in particular, we note that at low order (like $L=2$), many of the curvature errors that appear in the estimates below could be avoided entirely: These arise as a result of applying the general estimates in Lemma \ref{lem:Sobolev-norm-equivalence-improved} where the Ricci tensor doesn't naturally occur in the respective equations, and can be avoided at low orders by applying \eqref{eq:APmidRic} on all curvature terms that occur. \\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\numberthis\label{eq:L2-Border-P-even}\|\mathfrak{P}_{L,Border}\|_{L^2_G}\lesssim&\,\sqrt{\epsilon}a^{-3}\sqrt{\E^{(L)}(\phi,\cdot)}+\epsilon a^{-3}\sqrt{\E^{(L)}(N,\cdot)}+\epsilon a^{-3-c\sqrt\epsilon}\sqrt{\E^{(\leq L-2)}(N,\cdot)}\\
&\,+\sqrt{\epsilon}a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L-2)}(\phi,\cdot)}+\epsilon a^{-3-c\sqrt{\epsilon}}\|\nabla\Lap^{\frac{L}2-1}\phi\|_{L^2_G}\\
...
+\sqrt{\epsilon}a^{-3}\sqrt{a^{-4}\E^{(L)}(\phi,\cdot)}\\
&\,+\sqrt{\epsilon}a^{-3-c\sqrt{\epsilon}}\sqrt{a^{-4}\E^{(\leq L-2)}(\phi,\cdot)}\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\numberthis\label{eq:L2-Border-P-odd}\|\mathfrak{P}_{L+1,Border}\|_{L^2_G}\lesssim&\,\sqrt{\epsilon}a^{-3}\sqrt{\E^{(L+1)}(\phi,\cdot)}+\epsilon a^{-3}\sqrt{\E^{(L+1)}(N,\cdot)}+\epsilon a^{-3-c\sqrt\epsilon}\sqrt{\E^{(\leq L-1)}(N,\cdot)}\\
&\,+\sqrt{\epsilon}a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L-1)}(\phi,\cdot)}+\epsilon a^{-3-c\sqrt{\epsilon}}\|\nabla^2\Lap^{\frac{L}2-1}\phi\|_{L^2_G}\\
...
&\,+\sqrt{\epsilon} a^{-3}\sqrt{a^{-4}\E^{(L+1)}(\phi,\cdot)}+\sqrt{\epsilon}a^{-3-c\sqrt{\epsilon}}\sqrt{a^{-4}\E^{(\leq L-1)}(\phi,\cdot)}\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{align*}
Further, we have the alternative estimates
\begin{align*}
...
\end{lemma}
\begin{proof}
All of these estimates follow from applying $L^2_G$-$L^\infty_G$-type Hölder estimates to the individual nonlinear terms. The lower order terms are either controlled by the zero order estimates in subsection \ref{subsec:APlow} or the a priori estimates in Lemma \ref{lem:AP}. Furthermore, we apply Lemma \ref{lem:Sobolev-norm-equivalence-improved}, along with again Lemma \eqref{lem:AP}, to translate $L^2_G$-norms into energies up to additional curvature terms. For the sake of simplicity, we always estimate $\frac{\dot{a}}a$ by $a^{-3}$ up to constant (see \eqref{eq:Friedman}), and liberally apply \eqref{eq:ibp-trick} to deal with odd order energies and distribute $a^{-c\sqrt{\epsilon}}$ factors to lower orders while updating $c>0$ whereever this is convenient.
...
\numberthis\label{eq:L2-top-B}\|\mathfrak{B}_{L,top}\|_{L^2_G}\lesssim&\,\epsilon a^{-1}\sqrt{\E^{(L-1)}(\Ric,\cdot)}=\epsilon a^{-1-c\sqrt{\epsilon}}\sqrt{a^4\E^{(L-1)}(\Ric,\cdot)}
\end{align*}
\end{subequations}
\end{lemma}
\begin{proof}
...
\begin{lemma}[Junk terms] Recalling $\parallel$-notation explained in Remark \ref{rem:notation-parallel}, the following holds:
\begin{subequations}
\begin{align*}
...
+a^{-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L-1)}(\Sigma,\cdot)}\\
&\,+\sqrt{\epsilon}a^{-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L-2)}(\Ric,\cdot)}\\
%%%%%%%%%%%%%%%%%%%%%%
...
&\,+\left[\epsilon a^{-3-c\sqrt{\epsilon}}+\sqrt{\epsilon}a^{-1-c\sqrt{\epsilon}}\right]\sqrt{\E^{(\leq L-2)}(N,\cdot)}+\underbrace{\epsilon^2a^{1-c\sigma}\sqrt{\E^{(\leq L-2)}(\Ric,\cdot)}}_{\text{not present for} L=2}\\
&\,+\underbrace{\epsilon a^{-3-c\sqrt{\epsilon}}\|\nabla\phi\|_{H^{L-3}_G}++\epsilon^2 a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L-3)}(\Ric,\cdot)}}_{\text{not present for }L=2}\\
...
\numberthis\label{eq:L2-junk-P-odd}\|\mathfrak{P}_{L+1,Junk}\|_{L^2_G}\lesssim&\,\epsilon a^{1-c\sigma}\sqrt{\E^{(L+1)}(\phi,\cdot)}+\epsilon a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L-1)}(\phi,\cdot)}+\epsilon a^{-3-c\sqrt{\epsilon}}\|\nabla\phi\|_{H^{L-2}_G}\\
...
&\,+\epsilon^2a^{-1-c\sigma}\sqrt{a^4\E^{(L-1)}(\Ric,\cdot)}+\left(\epsilon^2a^{-1-c\sigma}+\epsilon^2 a^{-3-c\sqrt{\epsilon}}\right)\sqrt{\E^{(\leq L-2)}(\Ric,\cdot)}\\
%%%%%%%%%%%%%%%%%%%%%%%%
...
&\,+\sqrt{\epsilon}a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L-1)}(\Sigma,\cdot)}+\sqrt{\epsilon}a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L)}(N,\cdot)}\\
&\,+\underbrace{\epsilon a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L-2)}(\Ric,\cdot)}}_{\text{not present for }L=2}\\
%%%%%%%%%%%%%%%%%%%%%%%%%
\numberthis\label{eq:L2-junk-Q-1}\|\mathfrak{Q}_{1,Junk}\|\lesssim&\,\epsilon a^{-3-c\sqrt{\epsilon}}\E^{(1)}(N,\cdot)+\epsilon^\frac32a^{-3-c\sqrt{\epsilon}}\|\nabla\phi\|_{L^2_G}+\sqrt{\epsilon}a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(0)}(\Sigma,\cdot)}\\
%%%%%%%%%%%%%%%%%%%%%%%%
\numberthis\label{eq:L2-junk-Q-odd}\|\mathfrak{Q}_{L+1,Junk}\|_{L^2_G}\lesssim&\,\epsilon a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L)}(\phi,\cdot)}+\sqrt{\epsilon}a^{-3-c\sqrt{\epsilon}}\|\nabla\phi\|_{H^{L}_G}+\sqrt{\epsilon}a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(L)}(\Sigma,\cdot)}\\
&\,+\sqrt{\epsilon} a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L+1)}(N,\cdot)}+\epsilon a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L-1)}(\Ric,\cdot)}
\end{align*}
\begin{align*}
\numberthis\label{eq:L2-junk-S}\|\mathfrak{S}_{L,Junk}^\parallel\|_{L^2_G}\lesssim&\,\epsilon a^{1-c\sigma}\sqrt{\E^{(L)}(\Sigma,\cdot)}+\epsilon a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L-2)}(\Sigma,\cdot)}\\
&\,+\sqrt{\epsilon}a^{-1-c\sqrt{\epsilon}}\sqrt{\E^{(L)}(\phi,\cdot)}+\left(\epsilon a^{-3}+a^{1-c\sqrt{\epsilon}}\right)\sqrt{\E^{(\leq L)}(N,\cdot)}\\
&+\epsilon a^{5-c\sigma}\sqrt{\E^{(\leq L-1)}(\Ric,\cdot)}+\underbrace{\epsilon a^{-3}\E^{(L-2)}(\Ric,\cdot)+\epsilon a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L-4)}(\Ric,\cdot)}}_{\text{not present for }L=2}\\
...
&\,+\epsilon a^{1-c\sigma}\sqrt{\E^{(\leq L+2)}(\Sigma,\cdot)}+a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L)}(\Sigma,\cdot)}+a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L)}(N,\cdot)}\\
\numberthis\label{eq:L2-junk-R-odd}\|\mathfrak{R}_{L+1,Junk}\|_{L^2_G}\lesssim&\,\epsilon^2a^{1-c\sigma}\sqrt{\E^{(\leq L)}(\Ric,\cdot)}+\epsilon a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L-1)}(\Ric,\cdot)}+\epsilon a^{1-c\sigma}\sqrt{\E^{(\leq L+3)}(\Sigma,\cdot)}\\
&\,+a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L+1)}(\Sigma,\cdot)}+a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L+1)}(N,\cdot)}\\
\end{align*}
\vspace{-1em}
\begin{align*}
