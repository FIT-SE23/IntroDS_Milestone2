...
The existing literature on spectra of $\Lap_\gamma$ usually considers hyperbolic manifolds with sectional curvature $\kappa=-1$, and thus one needs to verify that $\mu_0$ is strictly greater than $1$ to verify the analogue of Definition \ref{def:spatial-mf-spectral} after \change{rescaling.}\\%the homothety $\gamma\leadsto \frac19\gamma$.

\noindent Numerical work, e.g., \cite{Cornish99, Ino01}, provides evidence for over 250 compact hyperbolic $3$-manifolds to satisfy this spectral bound, many of which are \change{closed}. In particular, both \change{\cite{Cornish99}}\footnote{\change{These results have }to be interpreted cautiously since the numerical method cannot detect eigenvalues below $1$.} and \change{\cite{Ino01} consider }the smallest closed orientable \change{hyperbolic }$3$-manifold, the Weeks space m003(-3,1), \change{and compute that it falls under Definition \ref{def:spatial-mf-spectral} with $\mu_0\approx 27,8$ in \cite[Table IV]{Cornish99} and $26\lessapprox\mu_0\lessapprox 27,8$ in \cite[p.\,639, Table 2]{Ino01}}. Moreover, as demonstrated in \change{\cite[p. 642,\,Figure 6]{Ino01}}, \change{many }manifolds with small enough diameter $d$ satisfy this condition. \change{In fact, this is ensured by, for example, the analytical bound
...
(cf. \cite[Theorem 1.1-1.2]{ChZ95} with $L=2$) which implies $\mu_0>10$ for Weeks space, which has diameter $d\approx 0.843$. Furthermore, \cite{Ino01} finds no closed hyperbolic manifold violating this bound. We also note that it is conjectured that, for any arithmetic hyperbolic 3-manifold, one at least has $\mu_0\geq 1$ (see \cite[Conjecture 2.3]{Ber03}). In fact, this is tied to the Ramanujan conjecture for automorphic forms. Finally, one can construct compact hyperbolic manifolds where $\mu_0$ becomes arbitrarily small, see \cite[Corollary 4.4]{Cal94}.}
\end{remark}
...
\left(\Ric[\fg]-\Ric[\gamma]\right)_{ij}=\frac12\LG(\fg-\gamma)_{ij}+J_{ij},\quad \|J_{ij}\|_{H^{l-1}}\lesssim \|\fg-\gamma\|_{H^l}\,,
\end{equation}
...
8\pi\left[\lvert\phi^\prime\rvert^2+\lvert\nabla\phi\rvert_{\fg}^2\right],\ \div_{\fg}\fk_a=%2\tau^2\fg_{ab}\jmath^b
8\pi\tau^{3}\phi^\prime\nabla_{b}\phi,
\end{equation}
...
\begin{assumption}[Bootstrap assumption]\label{ass:fut-bootstrap} We assume that, on the bootstrap interval $T\in[0,T_{Boot})$, \change{one has}
%\begin{subequations}
\begin{align*}
...
\end{lemma}
\begin{proof}
We denote the smallest positive eigenvalue of $\fLap$ acting on scalar functions on $\Sigma_T$ as $\mu_0(\fg_T)$. By the bootstrap assumption \eqref{eq:fut-bootstrap} and since $\mu_0$ depends continuously on the metric, we obtain the following for small enough $\epsilonnew>0$:
\begin{equation*}
...
\[\lvert\fdel\phim\rvert\lesssim\|\phi^\prime\|_{L^2}+\|\nabla\phi\|_{L^2}\|\fN\|_{L^2_{\fg}}\lesssim\epsilonnew e^{-\frac{T}2}\,.\]
\eqref{eq:fut-SF-technicality-2} now follows by combining this with \eqref{eq:fut-SF-technicality-1}.}
...
\begin{equation}
\del_TE_{SF}^{(0)}\leq \change{-2+E_{SF}^{(0)}+K\delta e^{-T}\sqrt{\E_{SF}^{(0)}}\sqrt{\left[\|\Sigma\|_{L^2}^2+\|\fg-\gamma\|_{L^2}^2\right]}+K\epsilonnew^3 e^{-\frac52T}}
\end{equation}
\end{lemma}
\begin{proof}
...
\leq&\int_M-4\lvert\phi^\prime\rvert^2\,\vol{\fg}+K\epsilonnew e^{-\frac{T}2}\change{\sqrt{\fE^{(0)}}}\sqrt{\|\fN\|_{H^1}}
\end{align*}
...
&\,+\change{K\epsilonnew e^{-\frac{T}2}\sqrt{\fE^{(0)}}\sqrt{\|\fN\|_{H^1}}+K\epsilonnew^3 e^{-\frac52T}}\\
\leq&\change{\,-2E_{SF}^{(0)}+K\epsilonnew e^{-T}\sqrt{E^{(0)}_{SF}}\left(\|\Sigma\|_{L^2}+\|\fg-\gamma\|_{L^2}+\sqrt{E^{(0)}_{SF}}\right)+\change{K\epsilonnew^3 e^{-\frac52T}}}
\end{align*}
\end{proof}
...
&\,+K\|\nabla\fLap^k\phi\|_{L^2}\left(\|\nabla\phi\|_{H^{2k+1}}\|\phi^\prime\|_{C^0}+\|\fN\|_{C^{2k}}\|\phi^\prime\|_{H^{2k}}\right)\\
...
\begin{align*}
\|[\fdel,\fLap^k]\phi^\prime\|_{L^2}\lesssim&\,\change{\|\fn\|_{C^{2k-1}}\left(\|\phi^\prime\|_{C^1}\|\fk\|_{\dot{H}^{2k-1}}+\|\fk\|_{C^{2k-2}}\|\phi^\prime\|_{H^{2k}}\right)}\\
...
\begin{prop}[Improved bounds for future stability]\label{prop:fut-bs-imp} Let the bootstrap assumption (see Assumption \ref{ass:fut-bootstrap}) be satisfied for $T\in[0,T_{Boot})$ and assume the initial data holds at $T=0$ (see Assumption \ref{ass:fut-init}). For $\epsilonnew>0$ sufficiently small and $\alpha$ as in \eqref{eq:fut-alpha} with $\epsilonnew^\prime>0$ sufficiently small, the following estimates hold: 
\begin{subequations}
\begin{align}
...
\end{subequations}
\end{prop}
\begin{proof}\change{In the following, the positive constant $K$ that may be updated from line to line.\\
...
\[\del_T\left(E^{(4)}_{SF}+E_{geom}\right)\leq -2\alpha (E_{SF}^{(4)}+E_{geom})+K\epsilonnew e^{-\frac{T}2}\left(E_{geom}+E_{SF}^{4}\right)+K\epsilonnew^3e^{-\frac52T}\,.\]
and consequently, since $\alpha\leq1$,
\[\del_T\left[e^{2\alpha T}\left(E^{(4)}_{SF}+E_{geom}\right)\right]\lesssim \epsilonnew e^{-\frac{T}2}\cdot e^{2\alpha T}\left(E^{(4)}_{SF}+E_{geom}\right)+\epsilonnew^3 e^{-\frac{T}2}\]
...
E^{(4)}_{SF}+E_{geom}\lesssim \epsilonnew^3e^{-2\alpha T}\,.
\end{equation}
}
...
\end{equation}
due to Lemma \ref{lem:fut-Ric-est}, and for $\epsilonnew^\prime>0$ small enough, inserting \eqref{eq:fut-en-imp} and \eqref{eq:fut-curv-imp} into \eqref{eq:fut-coerc} shows \eqref{eq:fut-sf-imp}. This in particular improves the $C$-norms we just used the bootstrap assumption for, and reinserting this improvement into \eqref{eq:fut-coerc} improves the bound to \eqref{eq:fut-en-imp}. Moreover, \eqref{eq:fut-ell-imp} follows directly from Lemma \ref{lem:fut-ell-est} and the already obtained improvements. \delete{Finally [..]}
...
Notice that
\[P:H^{20}_\gamma(M)\rightarrow H^{18}_\gamma(M),\,Y^i\mapsto \Lap_\gamma Y^i+(\gamma^{-1})^{il}{\Ric[\gamma]}_{lj}Y^j=\Lap_{\gamma} Y^i-\frac29 Y^i=0\]
...
Note that the scalings in Definition \ref{def:fut-rescaled} can be rewritten as follows:
\begin{gather*}
