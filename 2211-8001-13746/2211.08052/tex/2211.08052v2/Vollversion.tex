...
\newcommand{\change}[1]{\color{black}#1\color{black}}
\usepackage{ulem}
\newcommand{\delete}[1]{}
\usepackage{cancel}
...
Within this paper, the second fundamental form with regards to $\Sigma_t$ is defined \change{as the $(0,2)$-tensor $k$ given by
\[k(X,Y)=-\g(\nabbar_X\del_0,Y)\,,\]
...
\begin{align*}
A\cdot\tilde{A}&=\change{{A^a}_b{\tilde{A}_a}^b}=\langle A,\tilde{A}\rangle_g\\
...
\end{equation}
holds for some $a\in C^\infty((0,\infty))$, with the conventions $a(0)=0$ and $\dot{a}(T)>0$ for some arbitrary $T>0$. Further, choose a (smooth) scalar function $\phibar$ such that the following holds:
...
\end{remark}
\begin{proof}
For the first statement, we refer to \cite[Lemma 2.1]{Urban22} with $\gamma=2$. We also collect from there\footnote{\label{fn:t0-small}In \cite[Lemma 2.1]{Urban22}, one at first only has $\int_t^{t_0}a(s)^{-3}\lesssim \log(t_0)-\log(t)$ for $t_0$ small enough that we can estimate $a(t)$ by $t^\frac13$ up to constant. However, assuming this inequality were to hold up to $\overline{t}>0$ and one had $t_0>\overline{t}$, the contribution $\int_{\overline{t}}^{t_0}a(s)^{-3}\,ds$ only adds a constant that we can absorb into our notation. Similarly, $a(t)\simeq t^\frac13$ can be assumed to hold on $(0,t_0]$ for any fixed $t_0>0$, and we can ignore this technicality in proving the integral formulas in Lemma \ref{lem:scale-factor}.} that, for $t<t_0$, 
...
\eqref{eq:a-exp-est} follows by applying $a(t)\simeq t^\frac13$. Noting that $a^{-3}\simeq \nicefrac{\dot{a}}a$ holds, one further has
\begin{equation}
...
\end{lemma}
\begin{proof}
For \eqref{eq:EEqE}-\eqref{eq:EEqB}, we refer to \cite[(3.11a)-(3.11b)]{AM03}.\change{\footnote{\change{Note that there is a minor error in the statement in \cite{AM03}, where the authors forget to symmetrize the $J$-tensors when applying the symmetrization to \cite[(3.14)]{AM03}. This error seems to also occur in \cite[(7.2.2jk)]{ChrKl93}.}} } \eqref{eq:constr-E}-\eqref{eq:constr-B} follow as in \cite[(3.63a)-(3.63b)]{Wang19} from contracting the Gauss-Codazzi constraints.
...
\end{equation}
\end{subequations}
the \textbf{rescaled evolution equations for the Bel-Robinson variables}{\footnote{\delete{The dot, wedge and cross products as well as $\curl$ as an operator still include raising and lowering with regards to the unrescaled spatial metric, this will be dealt with in estimation to avoid too much notational baggage.}}}
\begin{subequations}
\begin{align*}
...
\begin{subequations}
\begin{align*}
\numberthis\E^{(l)}(\phi,t)=&\,(-1)^l\int_M\Psi\Lap^l\Psi-a^4\phi\Lap^{l+1}\phi\,\vol{G}\\
...
For some small enough $\epsilon\in(0,1)$ and the solution norms $\mathcal{H}\change{, \mathcal{H}_{top}}$ and $\mathcal{C}$ as in Definition \ref{def:sol-norm}, we assume the rescaled inital data to be close to that of the FLRW solution in Lemma \ref{lem:FLRW} in the following sense:
%\begin{subequations}
...
\noindent \change{Thus, }from here on out, we will assume \change{without loss of genearlity that }all energies \change{and squared norms are }continuously differentiable on the domain of existence, and similarly all variables \change{are }continuously differentiable for the lower order $C_G$-norm improvements in Section \ref{subsec:AP}.
%one has:
...
%
\underline{\eqref{eq:APmidB}:} This is obtained immediately from commuting \eqref{eq:REEqConstrB} with $\nabla^J$ and applying \eqref{eq:APmidSigma}. Notice that the Levi-Civita tensor can be absorbed into the implicit constants since $\lvert\epsilon[G]\rvert=\sqrt{6}$ holds (see \eqref{eq:LCS-contr3}).\\
...
\end{align*}
\change{and get the same estimate without the final term for $l\leq 5.$ }By assumption, we can estimate $\|\Lap\mathfrak{T}\|_{H^{2l}_G}^2$, $\|\mathfrak{T}\|_{H^{2l}_G}^2$ and $\|\Ric[G]+2G\|_{H^{2l}_G}^2$ as in \eqref{eq:Sobolev-norm-equiv-T2l}, and get the following \change{for $l>5$} :
\begin{align*}
\int_M\lvert\nabla^{2l+2}\mathfrak{T}\rvert_G^2\,\vol{G}\lesssim&\,\left[\|\Lap^{l}\Lap\mathfrak{T}\|^2_{L^2_G}+a^{-c\sqrt{\epsilon}}\sum_{m=0}^{l-1}\|\Lap^{m+1}\mathfrak{T}\|_{L^2_G}^2+a^{-c\sqrt{\epsilon}}\|\Lap\mathfrak{T}\|_{C^{\change{2l-12}}_G}^2\E^{(\leq 2l-2)}(\Ric,\cdot)\right]\\
...
&\,+a^{-c\sqrt{\epsilon}}\|\mathfrak{T}\|_{C^{\change{2l-11}}_G}^2\left[\E^{(2l)}(\Ric,\cdot)+\left(\E^{(\leq 2l-2)}(\Ric,\cdot)+\epsilon a^{-c\sqrt{\epsilon}}\E^{(\leq 2l-2)}(\Ric,\cdot)\right)\right]\\
...
While the estimates in the previous subsection are useful at high orders, they aren't enough to close the bootstrap assumptions for $N$. This can be achieved by deriving estimates in terms of $\Ltilde$ -- however, due to the explicit presence of Ricci terms in this version of the lapse equation, we use this to bound $N$ at \change{lower }orders. Since the arguments are largely identical to the ones above, we only sketch the proofs.
...
\begin{corollary}For $l\in\{0,\dots,\change{8}\}$, the following holds:
...
For some inequalities, we won't be able to derive any \change{benefitial }$\epsilon$-prefactors in the penultimate line. \change{For example, for $\E^{(L)}(\Sigma,\cdot)$, linear lapse terms in the evolution of $\Sigma$ incur a term of the form
\[\int_t^{t_0}a(s)^{-3}\cdot a(s)^4\|\Lap^\frac{L}2N\|_{\dot{H}^2_G}\cdot\sqrt{\E^{(L)}(\Sigma,s)}\,ds\]
on the right hand side, which after applying lapse energy estimates creates $\epsilon^{-\frac18}\E^{(L)}(\phi,\cdot)$ on the right}. However, combining the respective inequalities for the core energy mechanism at each derivative level with appropriate scaling, this will then combine to an inequality of the following form for a total energy, which we informally denote by $\mathcal{F}_{\text{total},L}$:
...
\end{align*}
In the mentioned example, scaling $\E^{(L)}(\Sigma,\cdot)$ by $\epsilon^\frac14$ turns the offending term int $\epsilon^\frac18a(s)^{-3}\E^{(L)}(\phi,s)$, which can be absorbed into the first line.\\}
...
\change{\begin{align*}
\mathcal{F}_{\text{total},L}(t)\lesssim&\,\biggr(\epsilon^4+\underbrace{\int_t^{t_0}\epsilon^\frac18 a(s)^{-3-c\epsilon^\frac18}\langle\text{already improved terms}\rangle\,ds}_{\text{not present for }L=0}\biggr)\\
&\,\cdot\exp\left(K\cdot \int_t^{t_0}\epsilon^\frac18a(s)^{-3}+a(s)^{-1-c\sqrt{\epsilon}}\,ds\right)\\
&\,+\epsilon^\frac14\mathcal{F}_{total,L}(t)+\underbrace{\sqrt{\epsilon}\cdot\langle\text{small lower order terms}\rangle(t)}_{\text{not present for }L=0}
\end{align*}}
for some constant $K>0$. By \eqref{eq:log-est} and \eqref{eq:a-integrals}, the exponential factor \delete{in the final line }can be bounded by $a^{-c\epsilon^\frac18}$, up to constant and updating $c>0$\change{. Furthermore, $\epsilon^\frac14\mathcal{F}_{total,L}(t)$ in the final line can be absorbed by the left hand side after updating the implicit constant in \enquote{$\lesssim$}. \\
...
\begin{proof}
\change{These estimates follow }completely analogously to Lemma \ref{lem:en-est-SF}, with the exception that high order lapse terms can now be estimated at order $L+2$ due to the the scalar field energy being scaled by $a^4$. In particular, we note that to deal with the analogous term to \eqref{eq:diff-eq-SF1}, one now inserts the following zero on the right and applies the commuted lapse equation \eqref{eq:comeq-lapse-odd}:
\begin{align*}
...
&\,\,\phantom{\int_M}\left.-6C\frac{\dot{a}}a\cdot a^4\langle\nabla\Lap^{\frac{L}2}N,\nabla\Lap^{\frac{L}2}\Psi\rangle-\frac{3}{8\pi}\dot{a}a^7\langle\mathfrak{N}_{L+1,Border}+\mathfrak{N}_{L+1,Junk},\nabla\Lap^{\frac{L}2}N\rangle_G\right\}\,\vol{G}
\end{align*}
...
\change{\begin{align*}
\numberthis\label{eq:en-est-BR}&\,\E^{(L)}(W,t)+\int_t^{t_0}\int_M\left[8\pi C^2a(s)^{-3}(N+1)\langle\Lap^\frac{L}2\Sigma,\Lap^\frac{L}2\RE\rangle_G+6\frac{\dot{a}(s)}{a(s)}\lvert\Lap^\frac{L}2\RE\rvert_G^2\right]\,\vol{G}\,ds\\
...
\lesssim&\,\left(\epsilon^\frac18a^{-3}+a^{-1-c\sqrt{\epsilon}}\right)\E^{(M)}(W,\cdot)+\epsilon^{-\frac18}a^{-3}\cdot a^4\E^{(M+1)}(\phi,\cdot)+a^{-1}\E^{(\leq L)}(\phi,\cdot)\\[0.5em]}
...
\end{equation}
\end{lemma}
\begin{proof}\change{We prove the statement for $L=2$, since the proof of \eqref{eq:en-est-Sigma-1} is entirely analogous.}\\
...
=&\,\int_M\frac32\cdot\lvert\div_G\Lap^\frac{\change{L}}2\Sigma\rvert_G^2+a^2\lvert\curl\Lap^\frac{\change{L}}2\Sigma\rvert_G^2\,\vol{G}
\end{align*}
...
&\,+\left(\|\mathfrak{R}_{L-2,Border}\|_{L^2_G}+\|\mathfrak{R}_{L-2,Junk}\|_{L^2_G}\right)\sqrt{\E^{(L-2)}(\Ric,\cdot)}\,.
\end{align*}
\eqref{eq:en-est-Ric} now follows inserting the borderline and junk term estimates \eqref{eq:L2-Border-R-even} and \eqref{eq:L2-junk-R-even} and applying the lapse energy estimates from Corollary \ref{cor:en-est-lapse}.\\
\eqref{eq:en-est-Ric0} follows almost identically by inserting \eqref{eq:REEqRic} instead of \eqref{eq:comeq-Ric-even} as well as \eqref{eq:REEqG} for the additional $\del_tG\ast(\Ric[G]+\nicefrac29G)$-terms. These can be estimated as
\begin{align*}
...
%\end{proof}
\begin{lemma}[\change{Odd }order curvature energy estimate]\label{lem:en-est-Ric-top} For $\change{L}\in2\N$, $4\leq \change{L}\leq \change{18}$ and $t\in(t_{Boot},t_0]$, the following holds:
\begin{align*}
...
\begin{lemma}[Sobolev norm estimates for Christoffel symbols]\label{lem:int-est-Chr} Let $U$ be a coordinate neighbourhood on $M$, viewed as a coordinate neighbourhood on $\Sigma_t$ for $t\in(t_{Boot},t_0]$. For any $l\in\N,\,l\leq \change{15}$, the following Sobolev estimate then holds:
...
&\,+\epsilon^\frac{46}8a^{-c\epsilon^\frac18}+\epsilon^\frac{6}8a^{-c\epsilon^\frac18}\sup_{s\in(\cdot,t_0)}\left(\|N\|_{H^{J-1}(\Sigma_s)}^2+\|\Sigma\|_{H^{J-1}(\Sigma_s)}^2\right)\,,
\end{align*}
...
\change{\begin{prop}[Improved energy bounds]\label{prop:en-bs-imp} Under the bootstrap assumptions (see Assumption \ref{ass:bootstrap}) and the initial data assumptions \eqref{ass:init}, the following improved estimates hold on $(t_{Boot},t_0]$:
\begin{subequations}
\begin{align}
...
\eqref{eq:en-est-SF} and \eqref{eq:en-est-Ric}, and finally the last three lines are precisely the scaled right hand side of \eqref{eq:en-est-Sigma-top}. Regarding the curvature energies in the various individual energy estimates, any summand with $\E^{(L-3)}(\Ric,\cdot)$ can be split using \eqref{eq:ibp-trick}, the resulting summands containing $\E^{(L-2)}(\Ric,\cdot)$ can always be absorbed in to the total energy term in the first line, and anything with $\E^{(\leq L-4)}(\Ric,\cdot)$ is tracked in $\langle\text{Err}\rangle_L$ for $L\geq 4$.\\
...
\begin{equation}\label{eq:nabla-phi-norm-imp}
\|\nabla\phi\|_{H^{18}_G}\lesssim \left(1+\epsilon a^{-c\sqrt{\epsilon}}\right)\|\Sigma\|_{H^{19}_G}+\epsilon\|\Psi\|_{H^{19}_G}\lesssim \epsilon^\frac{15}4a^{-c\epsilon^\frac18}\,.
\end{equation}}
We take particular care in showing that the improved bound holds for $a^2\|\nabla\phi\|_{\dot{H}^{19}_G}$: First, note that \eqref{eq:en-bs-imp-Ric} \change{implies }$\E^{(\leq 17)}(\Ric,\cdot)\lesssim\epsilon^\frac72 a^{-c\epsilon^\frac18}$. Applying this along with \eqref{eq:APmidphi} to \eqref{eq:Sobolev-norm-equiv-nablazeta2l+1}, as well as \eqref{eq:Sobolev-norm-equiv-zetalow} in the second line and \change{\eqref{eq:en-bs-imp-SF} as well as \eqref{eq:nabla-phi-norm-imp} }in the final step, we obtain:
\begin{align*}
...
\lesssim&\,a^{-c\sqrt{\epsilon}}\cdot \E^{(\leq 19)}(\phi,\cdot)+a^{4-c\sqrt{\epsilon}}\|\nabla\phi\|_{H^{17}_G}^2+\epsilon^\frac{9}2a^{-c\epsilon^\frac18}\\
...
\begin{subequations}
\begin{align}
\|\zeta\|_{H^l_\gamma(\change{M})}^2\lesssim&\,a^{-c\sqrt{\epsilon}}\|\zeta\|^2_{H^l_G(\change{M})}+a^{-c\epsilon^\frac18}\|\zeta\|_{C_G^{\lceil \frac{l-1}2\rceil}(\Sigma_t)}^2\left(\epsilon^4+\epsilon^{-\frac14}\sup_{s\in(t,t_0)}\left(\|N\|_{H^{l-1}_G(\Sigma_s)}^2+\|\Sigma\|_{H^{l-1}_G(\Sigma_s)}^2\right)\right)\label{eq:H-norm-exch-zeta}\,,\\
\|\mathfrak{T}\|^2_{H^l_\gamma(\change{M})}\lesssim&\,a^{-c\sqrt{\epsilon}}\|\mathfrak{T}\|^2_{H^l_G(\change{M})}+a^{-c\epsilon^\frac18}\|\mathfrak{T}\|_{C_G^{\lceil \frac{l-1}2\rceil}(\Sigma_t)}^2\left(\epsilon^4+\epsilon^{-\frac14}\sup_{s\in(t,t_0)}\left(\|N\|_{H^{l}_G(\Sigma_s)}^2+\|\Sigma\|_{H^{l}_G(\Sigma_s)}^2\right)\right) \label{eq:H-norm-exch-T}
\end{align}
\end{subequations}
%with
%\begin{equation}
...
\end{align*} 
Here, $\ast_\gamma$ encodes the analogous schematic product notation with regards to $\gamma$ (see subsection \ref{subsubsec:schematic-notation}). Regarding the Christoffel symbols, notice \eqref{eq:C-norm-exch-T} with roles of $\gamma$ and $G$ reversed holding up to $l-1$ implies that, for any $m\in\{0,\dots,l-1\}$ and some multivariate polynomial $\tilde{P}_m$, we have
...
\end{equation}
Moving back to \eqref{eq:nabla-nabhat} and just considering the first line for now, this implies the following:
\begin{align*}
\numberthis\label{eq:C-norm-exch-T-last-step}\left\lvert\|\mathfrak{T}\|_{\dot{C}^l_G(\change{M})}^2-\|\mathfrak{T}\|^2_{\dot{C}_\gamma^l(\change{M})}\right\rvert\lesssim&\,a^{-c\sqrt{\epsilon}}\left(\|\mathfrak{T}\|_{C^l_\gamma(\change{M})}^2+\sum_{m=0}^{l-1}\|\nabla^{m}\mathfrak{T}\|_{C^0_\gamma(\change{M})}^2\right)\\
&\,+\left(\|\mathfrak{T}\|_{C^{\lceil\frac{l-1}2\rceil}_\gamma(\change{M})}^2+\sum_{m=0}^{\lceil\frac{l-1}2\rceil}\|\nabla^{m}\mathfrak{T}\|_{C^0_\gamma(\change{M})}^2\right)\|\Gamma-\Gamhat\|_{C^{l-1}_\gamma(\change{M})}^2\,\\
&\,+\langle\text{at least cubic nonlinear terms}\rangle
\end{align*}
We can rewrite $\nabla^m\mathfrak{T}$-norms in $C_\gamma$ as ones in $C_G$ up to $a^{-c\sqrt{\epsilon}}$ as before. Then, we can apply the already obtained estimates up to order $l-1$ show that the first two lines of the right hands side can be estimated by the right hand side of \eqref{eq:C-norm-exch-T}. The highly nonlinear terms can be dealt with similarly, closing the induction over admissible $l$. \delete{ on local coordinate [...]} %neighbourhoods. Since $\Sigma_t$ is compact, this also implies \eqref{eq:C-norm-exch-T} for $U=\Sigma_t$ after updating constants. 
\eqref{eq:AP-exch-T} immediately follows by applying \eqref{eq:APmidG}-\eqref{eq:APmidG-1} and \eqref{eq:APmidChr}.
...
&\,+\left(\|\mathfrak{T}\|^2_{C^{\lceil\frac{l-1}2\rceil}_G(\Sigma_t)}+\|\nabhat^{\leq \lceil\frac{l-1}2\rceil}\mathfrak{T}\|^2_{C^{0}_G(\Sigma_t)}\right)\cdot \\
...
\change{\mathcal{C}+\mathcal{C}_\gamma\lesssim\epsilon^\frac72a^{-c\epsilon^\frac18}}
\end{equation}
\end{corollary}
\begin{proof}
...
holds for any multivariate polynomial $P$ that appears when applying \eqref{eq:C-norm-exch-zeta}-\eqref{eq:C-norm-exch-T}. Again using the strong $C_G$-norm estimates from Lemma \ref{lem:AP}, this then implies $\mathcal{C}\lesssim \change{\epsilon^\frac72} a^{-c\epsilon^\frac18}$.
...
\change{To make this a bit more precise, and also since this it is a little less involved than the arguments in \cite{Rodnianski2014}, we will briefly sketch how the arguments from \cite[Section 2.5]{FajKr20} extend to our setting. \\%
...
One easily checks that $(\g_{FLRW},t_0)$ is a regular point of $H_0$. By the implicit function theorem for Banach manifolds, this means there is a (unique) smooth function $F$ that maps an open neighbourhood of $\g_{FLRW}$ in $\mathcal{M}^{17}$ to an open neighbourbood of the constant function $x\mapsto t_0$ in $C^{16}(M)$ such that $H_0(\cdot,F(\cdot))=\tau(t_0)$ holds in that neighbourhood.\\
...
\end{equation}
where $\gamma$ is the negative Einstein spatial reference metric on $M$ (see Definition \ref{def:spatial-mf}).\\
...
\end{equation}
holds for suitable constants $\mathcal{K}_1,\mathcal{K}_2>0$ that are independent of $\curve$, and thus any such geodesic crashes into the Big Bang hypersurface in finite affine parameter time. \change{Hence, for sufficiently points $p,q\in\Sigma_t$ with $\text{dist}_\gamma(p,q)>2\mathcal{K}a(t)^{2-c\epsilon^\frac18}$, the causal pasts of $p$ and $q$ cannot intersect.}\\
...
\begin{align}
\label{eq:blowup-scalar} \|a^6R[\g]-8\pi(\Psi_{Bang}+C)^2\|_{C^0(M)}\lesssim&\,\epsilon a^{4-c\epsilon^\frac18}\,\text{and}\\
\label{eq:blowup-Ricci}\|a^{12}\Ric[\g]_{\alpha\beta}\Ric[\g]^{\alpha\beta}-(8\pi)^2(\Psi_{Bang}+C)^4\|\lesssim&\,\epsilon a^{4-c\epsilon^\frac18}\,,
\end{align}
...
\mathcal{C}(t)\leq K_1\change{\epsilon^\frac72}a(t)^{-c_1\epsilon^\frac18}\,
\end{equation}
...
Thus, the rescaled solution variables induce a unique solution to the Einstein scalar-field system on $(0,t_0]\times M$ such that \eqref{eq:bs-imp-total} \change{is }satisfied for any $t\in(0,t_0]$. The core estimate \eqref{eq:bs-imp-main-thm} follows since Corollary \ref{cor:H-imp} and Corollary \ref{cor:C-impr} now hold on $(0,t_0]$.\\
...
\noindent \eqref{eq:Bang-CMC} follows directly from the CMC condition \eqref{eq:CMC}, the asymptotic behaviour \eqref{eq:asymp-K} of $a^3k$ and the Friedman equation \eqref{eq:Friedman}. \eqref{eq:Bang-Hamil} follows from asymptotic limit of the Hamiltonian constraint \eqref{eq:Hamilton} with \eqref{eq:Friedman}, \eqref{eq:asymp-lapse}, \eqref{eq:asymp-Psi} and \eqref{eq:asymp-K} as well as \eqref{eq:bs-imp-main-thm} for lower order terms. \eqref{eq:asymp-G} follows exactly as in \cite[Theorem 15.1]{Rodnianski2014}, and \eqref{eq:footprint-vol}-\eqref{eq:footprint-G} are a direct result of the initial data assumptions and applying the respective asymptotic estimates to $t=t_0$.
...
and consequently $\lvert\mathcal{A}^\prime(t)\rvert\leq \lvert \mathcal{A}^\prime(t_0)\rvert a^{-1-c\epsilon}$ by the Gronwall lemma. \eqref{eq:geod-incomp} follows by integrating.\\}
...
%\end{align*}
Since $\Sigma^\sharp$ and $\Psi$ converge to footprint states $\hat{K}_{Bang}=K_{Bang}+\sqrt{\frac{4\pi C}3}\I$ and $\Psi_{Bang}$ in $C^{15}_\gamma(M)$ respectively, this shows that $W_{Bang}\in C^{15}_\gamma(M)$ exists, is a fourth order polynomial in $\hat{K}_{Bang}$ and $\Psi_{Bang}$, and satisfies
\[\left\|\lvert\RE\rvert_G^2-\change{\frac1{8}}W_{Bang}\right\|_{C^0(M)}\lesssim \epsilon a^{2-c\epsilon^\frac18}\]
as well as where \eqref{eq:footprint-W}. Due to \eqref{eq:asymp-B}, the $\lvert\RB\rvert_G^2$-term in the Weyl curvature scalar is negligible in comparison, and thus \eqref{eq:blowup-W} immediately follows.\\
Furthermore, %denoting the Schouten tensor with regards to $\g$ by $\text{Sch}$, 
one has 
\begin{align*}
...
\end{align*}
and the statement follows once more with \eqref{eq:asymp-Psi}, \eqref{eq:asymp-lapse} as well as \eqref{eq:bs-imp-main-thm} for lower orders.
...
\end{align*}
Hence, for any $I\in\N_0$, any $\nabla^I\Riem[G]$-term reduces to a sum of products and contractions of $\nabla^I\Ric[G]$ with various metric tensors that are all surpressed in schematic notation. With this in mind, the above statements are simply direct consequences of standard commutation cormulas and \eqref{eq:epsilonLC-resc}. 
\end{proof}
...
\begin{lemma}[Junk terms] Recalling $\parallel$-notation from Remark \ref{rem:notation-parallel}, the following holds:
\begin{subequations}
\begin{align*}
...
\change{\epsilon a^{-1-c\sqrt{\epsilon}}\sqrt{a^{-4}\E^{(L)}(\phi,\cdot)}+\epsilon a^{-3-c\sqrt{\epsilon}}\E^{(\leq L-2)}(\phi,\cdot)}\\
...
&\,+\underbrace{\epsilon a^{-3}\E^{(L-2)}(\Ric,\cdot)+\epsilon a^{-3-c\sqrt{\epsilon}}\sqrt{\E^{(\leq L-4)}(\Ric,\cdot)}}_{\text{not present for }L=2}\\
