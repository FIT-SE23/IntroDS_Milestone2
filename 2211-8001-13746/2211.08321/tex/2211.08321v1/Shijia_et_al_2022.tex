...
\begin{figure*}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{plans.pdf}
  \caption{Some successful as well as failed plans. The first column represents the initial scene, each following column represents then an individual action step. In the failure cases, the red dashed box means a wrong step.}  \label{plans}
\end{figure*}
...
\begin{table*}[!ht]
\centering
\caption{Success rates for 126 planning cases with different plan lengths.}
\label{table:4}
    \resizebox{0.85\width}{!}{
    \begin{tabular}{|c|c|c|c|c|c|c|c|c|c|}
    \hline
        steps of plans & 0 step & 1 step & 2 steps & 3 steps & 4 steps & 5 steps & 6 steps & 7 steps & total \\ \hline
        success cases & 15 & 16 & 21 & 24 & 19 & 9 & 5 & 4 & 113 \\ \hline
        failure cases & 1 & 1 & 3 & 5 & 3 & 0 & 0 & 0 & 13 \\ \hline
        total cases & 16 & 17 & 24 & 29 & 22 & 9 & 5 & 4 & 126 \\ \hline
        success rate & 93.75\% & 94.12\% & 87.50\% & 82.76\% & 86.36\% & 100.00\% & 100.00\% & 100.00\% & 89.68\% \\ \hline
    \end{tabular}
    }
\end{table*}
...
\begin{table*}[htbp]
\centering
\caption{Success rates for step by step analysis. A total of 343 planning steps were performed for the 126 different cases.}
\label{table:3}
    \resizebox{0.85\width}{!}{
    \begin{tabular}{|c|c|c|c|c|c|c|c|c|}
    \hline
        n-th step of plans & step1 & step2 & step3 & step4 & step5 & step6 & step7 & total \\ \hline
        valid steps & 106 & 90 & 68 & 40 & 18 & 9 & 4 & 335 \\ \hline
        invalid  steps & 4 & 3 & 1 & 0 & 0 & 0 & 0 & 8 \\ \hline
        total  steps & 110 & 93 & 69 & 40 & 18 & 9 & 4 & 343 \\ \hline
        success rate & 96.36\% & 96.77\% & 98.55\% & 100.00\% & 100.00\% & 100.00\% & 100.00\% & 97.67\% \\ \hline
    \end{tabular}
}
\end{table*}
...
We selected 126 representative samples of scenes for our test dataset. These scenes differ in the number and location of objects. Our target is to place as many objects from outside the box as possible into the appropriate compartments in the box. To save computational resources, a simple depth-first search is used to find the complete plans, which are then checked whether they are suitable. 

Of these 126 samples there exist plans with zero (box is already packed) up to seven packing steps. Table~\ref{table:4} shows how many of these different cases had been successful. The grand average success rate across all cases was 89.68\%. The majority of failure cases are due to small inaccuracies in object and affordance segmentation.

In Table \ref{table:3} we analyse all cases in a step-wise manner asking whether in a plan step $n$ has been successful or not. A total of 343 steps were performed across all 126 cases in the plan-search process. The average single-step success rate is 97.67\%. The success rate does not show any deterioration for late versus early steps. This demonstrates that our method has a high success rate for every step to step transition which did not drop along longer plans. 

In Figure \ref{plans}, we show some successful and some failed plans. Three successful plans were able to complete our box packing task in a few steps. For the failed plans we observed here the following characteristic cases: In failure case 1, the "apple" was placed on the edge of the "cup" which is not detected in the checking process. This is, because the conflict area calculated between the apple and the cup is smaller than the 30-pixel threshold value, which is caused by the inaccuracy of the result in affordance segmentation. In failure case 2, the "apple" was never moved because the "apple" had a very inaccurate segmentation result in the instance segmentation.
