...
\begin{document}
	...
	\begin{abstract}
		In this paper, we prove that a $\mathrm{GL}(2n)$-eigenvariety is \'etale over the (pure) weight space at non-critical Shalika points, and construct multi-variable $p$-adic $L$-functions varying over the resulting Shalika components. Our constructions hold in tame level 1 and Iwahori level at $p$, and give $p$-adic variation of $L$-values (of regular algebraic cuspidal automorphic representations of $\mathrm{GL}(2n)$ admitting Shalika models) over the whole pure weight space. In the case of $\GL(4)$, these results have been used by Loeffler and Zerbes to prove cases of the Bloch--Kato conjecture for $\mathrm{GSp}(4)$.
	...
	\begin{itemize}\setlength{\itemsep}{0pt}
		\item[(A)] we prove that the Iwahoric $\GL_{2n}$-eigenvariety is \'etale over the ($n+1$-dimensional) pure weight space at $\tilde\pi$, and that the unique connected component $\sC$ through $\tilde\pi$ contains a Zariski-dense set $\sC_{\mathrm{Sha}}$ of classical Shalika points; and
...
\]
 where $\Omega$ is a (2-dimensional) parahoric $p$-adic family of weights, $\sD_\Omega^Q$ is a local system attached to a space of parahoric distributions, and $\cD(\Zp^\times,R)$ is the space of $R$-valued locally analytic distributions on $\Zp^\times$; so $\cD(\zp^\times,\cO_\Omega)$ is a space of 3-variable distributions. These interpolated the $\mathrm{Ev}_{\chi,j}^{\lambda,\mathrm{DJR}}$ for varying $\lambda, \chi$ and $j$. They were used to construct $p$-adic $L$-functions $L_p(\tilde\pi^Q)$ attached to finite slope $Q$-parahoric  Shalika $p$-refinements $\tilde\pi^Q$, to construct 2-dimensional (parahoric) $p$-adic families through $\tilde\pi^Q$, and to vary $L_p(\tilde\pi^Q)$ over these families.
	...
	Let $\cH_p^G$ be the Hecke algebra at $p$ (see \S\ref{sec:hecke p}). An (Iwahoric) \emph{$p$-refinement} of $\pi_p$ is a 
	pair $\tilde\pi_p = (\pi_p,\alpha)$, where  $\alpha : \cH_p^G \longrightarrow \overline{\Q}$ is a 
...
\]
and show the system $\alpha^{\cG}$ then appears in $\Pi_p^{\Iwahori_{\cG}}$. When $\pi_p$ (hence $\Pi_p$) is regular, there are $2^n n!$ such spin $p$-refinements. 
...
Our approach exploits properties of \emph{spherical varieties}: in particular, we use crucially the existence of an `open orbit element' $u \in \GL_{2n}(\Zp)$ such that $\overline{B}_{2n}(\Zp) u^{-1}H(\Zp)$ is Zariski-open in $\GL_{2n}(\Zp)$ (for $\overline{B}_{2n} \subset \GL_{2n}$ the opposite Borel of lower triangular matrices). The approach should apply to much more general spherical pairs $(G,H)$; this has now been studied, for example, in \cite{rockwood-spherical}.
...
that is everywhere spherical.  Suppose that $\pi_p$ is regular and that 
	...
	\begin{theorem-intro}\label{thm:intro family}
		Let $\tilde\pi$ satisfy the hypotheses of Theorem \ref{thm:intro A}, and let $\sC \subset \sE^{\GL_{2n}}$ be the (unique) corresponding $p$-adic family through $\tilde\pi$. There exists a rigid analytic function
			\[
	...
	\end{theorem-intro}
	 ...
	 One could restate this interpolation property in terms of critical $L$-values as follows: for all $y \in \sC^{\mathrm{Sha}}$ of weight $\lambda_y = w(y)$, all integers $j$ with $-\lambda_{y,n+1} \geq j \geq -\lambda_{y,n}$, and all finite order characters $\chi$ of $\Q^\times\backslash \A^\times$ of $p$-power conductor (including trivial conductor), we have
		...
		\]
		Here $\chi_{\cyc}$ is the cyclotomic character, the sign is determined by $\chi(-1)(-1)^j = \pm 1$,  $\gamma_{(pm)}$ is a volume constant, $e_p$ is the Coates--Perrin-Riou factor at $p$, $\zeta_j(W_{y,\infty}^\pm)$ is a (non-zero) local zeta integral at infinity, $w_n$ is the longest Weyl element in $\GL_n(\Zp)$, $L^{(p)}$ is the $L$-function with the local factor at $p$ removed, and $\Omega_{\pi_y}^\pm$ are periods. All of this is explained in Theorem \ref{thm:non-ordinary}. 
...
A second minor assumption is the weight condition that $\lambda_n > \lambda_{n+1}$. This rules out, for example, weight 2 modular forms in the case of $\GL_2$. This condition is used in exactly one place: to guarantee existence of a non-central, and hence non-vanishing, Deligne-critical $L$-values. It could be replaced by instead assuming the existence of $j \in \mathrm{Crit}(\lambda)$ and finite order Hecke characters $\chi^\pm$, with $\chi^\pm_\infty(-1)(-1)^j = \pm 1$, such that $L\left(\pi\otimes\chi^\pm,j+\tfrac{1}{2}\right) \neq 0$ (see \cite[Def.\ 7.3]{BDW20}). One would expect this to always be true, and indeed it is known for weight 2 modular forms thanks to Rohrlich \cite{Roh89}.
...
This leads to tensions between working at level $K$, where we see $L$-values, and level $K_1(\tilde\pi)$, suitable for proving strong results about families. In tame level 1, we have  $K=K_1(\tilde\pi)$ and there is no problem. For simplicity -- and since this issue is somewhat perpendicular to the main novelties of the present paper -- we have presented all our proofs for tame level 1.
	...
	We briefly indicate how to drop this assumption in proving Theorem \ref{thm:intro A}, following the strategy employed in 
	 \cite[\S7]{BDW20}, in the parahoric setting. There it was first shown that there exists a $p$-adic family of the correct dimension at level $K$, containing a Zariski-dense set of classical points corresponding to RASCARs. Then, by studying conductors and the Local Langlands Correspondence, a delicate level-switching operation was carried out that transferred this family from level $K$ to level $K_1(\tilde\pi)$, where strong results can be deduced about its geometry. Since nothing about this argument used the parahoric setting, it applies equally well to our Iwahoric case, and the same methods prove the following generalisation of Theorem \ref{thm:intro A}:
	
	
		\begin{theorem-intro-2}\label{thm:intro A prime}
			Let $\pi$ be a RASCAR of $\GL_{2n}(\A_{\Q})$ that is spherical and regular at $p$. Suppose that $\lambda_n > \lambda_{n+1}$, where $\pi$ has weight $(\lambda_1,\dots,\lambda_{2n})$. Let $\tilde\pi = (\pi, \alpha)$ be a non-critical slope $p$-refinement of $\pi$. Then the level $K_1(\tilde\pi)$ eigenvariety $\sE^{G}(K_1(\tilde\pi))$ for $\GL_{2n}$ is \'etale over $\sW_0^G$ at $\tilde\pi$. There exists a neighbourhood of $\tilde\pi$ in $\sE^{G}(K_1(\tilde\pi))$ containing a very Zariski-dense set of classical points corresponding to RASCARs.
		\end{theorem-intro-2}
...
%	
	...
	This paper owes a great debt to the observations of David Loeffler, and we thank him profusely. We thank Mahdi Asgari for patiently answering our questions about his work on $\mathrm{GSpin}_{2n+1}$. We also thank the anonymous referees for their valuable comments and corrections.
...
We will use the following notation for almost all of this paper. In Sections 5, 6, 7 and 9,  however, we will use purely local analogues of this notation, as fixed in the introduction to Part II.
	...
	\end{equation}
	We repeatedly use that if $\pi_\ell$ is the local component at $\ell$ of a RACAR $\pi$, and $\pi_\ell^{\GL_{2n}(\Z_\ell)} \neq 0$ (so $\pi_\ell$ is spherical), 
 then $\pi_\ell$ is a generic unramified principal series representation. By this, we mean there exists an unramified character $\UPS : T(\Ql) \to \C^\times$ such that
	\begin{equation}\label{eq:unramified induced}
		\pi_\ell = \Ind_B^G \UPS,
	\end{equation}
the normalised parabolic induction of $\theta$ to $G$ (that is, the true induction of $\delta_B^{1/2}\UPS$).
...
\end{itemize}
	...
	Suppose $\pi_p$ is spherical. In particular $\pi_p^{\GL_{2n}(\Zp)} \neq 0$, hence $\pi_p^{\Iw} \neq 0$. 
...
\end{definition}
	...
	\end{equation}
	We let $\m_{\tilde\pi} \defeq \ker(\psi_{\tilde\pi})$ be the corresponding maximal ideal of $\cH \otimes E$. If $M$ is a module upon which $\cH\otimes E$ acts, we write $M_{\tilde\pi}$ for  the localisation of $M$ at $\m_{\tilde\pi}$. If $L$ is a field containing $E$ and $M$ is a finite-dimensional $L$-vector space, $M_{\tilde\pi}$ is the generalised eigenspace for  $\cH\otimes L$ corresponding to $\psi_{\tilde\pi}$. 
	
	 Let $\pi_p = \Ind_B^G \UPS$ be a generic unramified principal series representation (as in \eqref{eq:unramified induced}). We recall the standard classification of $p$-refinements for $\pi_p$. Recall $\cW_G = \mathrm{S}_{2n}$ and $\delta_B$ from \S\ref{sec:notation}.
...
\]
 induced by the above. This is \emph{not} canonical, depending on the choice of character $\UPS$ from which we induce. For $\tau \in \cW_G$, replacing $\UPS$ by $\UPS^\tau$ conjugates the image of $\Delta_\UPS$ by $\tau$. 
			...
			\item[(iii)] When $\pi_p$ is the local component of a RACAR, the eigenvalues $\alpha_{p,r}$ are algebraic but not necessarily $p$-integral. To account for this, we make the following definition.
		\end{itemize}
	\end{remark}
...
\end{proof}
 ...
 We have $L_p^{P,\beta} \subset L_p^{Q,\beta}$.
	...
	Define $J_P \subset \GL_{2n}(\Zp)$ to be the parahoric subgroup for $P$. Define $\Delta_P \subset \GL_{2n}(\Qp)$ to be the semigroup generated by $J_P$ and the matrices
	\[
...
We give two applications of these maps. Let $\pi$ be any RACAR of weight $\lambda$ with attached maximal ideal $\m_\pi \subset \cH'$ as in \S\ref{sec:hecke algebra}. 
	...
	\end{proposition}
	\begin{proof}
		This is proved exactly as in \cite[Prop.\ 5.15]{BDW20}. Whilst $\xi$ is replaced by $u^{-1}$, and some non-zero volume factors change depending on $P$, the argument of proof is identical.
	\end{proof}
	...
	\begin{lemma}\label{lem:average A}
		...
		\begin{align*} 
			\int_{\gamma \in A} \chi(\operatorname{det}\gamma)&\psi\Big[ \operatorname{tr}(-p^{-n \beta} z^\beta x \gamma z^{-\beta} w_n ) \Big] d^{\times}A\\
			 &= \prod_{i=1}^n \chi(-1)\int_{\mathbb{Z}_p^{\times}}  \chi( c_{i, i} )\psi\left(p^{-(2\beta(n-i) + \beta)} x_{n+1 -i, i} c_{i,i} \right) d^{\times} c_{i, i} \\
			&= \left\{ \begin{array}{cc} \frac{\chi(-1)^n\tau(\chi)^n}{p^{n\beta}(1-p^{-1})^n} \prod_{i=1}^n \chi(x'_{n+1-i, i})^{-1} & \text{ if } x_{n+1-i, i} \in p^{2\beta(n-i)} \mathbb{Z}_p^{\times} \ \forall i \\ 0 & \text{ otherwise } \end{array} \right.
		\end{align*}
...
\end{proof}
 ...
 In this section, we begin to make this assertion rigorous. We define `spin' $p$-refinements as those `that come from $\mathrm{GSpin}_{2n+1}$', made precise in Proposition \ref{prop:spin refinement}. In later sections we will show that for any spin $p$-refinement $\tilde\pi$, there exists $W\in \cS_{\psi}^\eta(\tilde\pi)$ such that $W\smallmatrd{w_nz^{2\beta}}{}{}{1} \neq 0$. Our key application of spin $p$-refinements will ultimately be summarised in Corollary \ref{cor:L-value}.
	...
	\begin{notation}
		Let $\cG \defeq \mathrm{GSpin}_{2n+1}$, where we take the split form as in \cite{AS06}. As $\pi$ admits a Shalika model, by Theorem \ref{thm:transfer} it is the functorial transfer of a representation $\Pi$ on $\cG(F)$.
	\end{notation}
	...
	Recall here $\eta$ is the Shalika character (see p\pageref{shalika character}). We will show $\tilde\pi$ is spin if and only if $\alpha$ factors through an eigensystem occuring in $\Pi^{\Iwahori_{\cG}}$, for $I_{\cG}$ the Iwahori subgroup of $\cG$ (see \S\ref{sec:spin via gspin}).
	...
	Given choices of a decomposition $X_1 \sqcup X_2$ and a bijection $\nu$, we can find an identification $\pi = \Ind_B^G \UPS$ with $\UPS_i\UPS_{\eta(i)} = \eta$. 
...
Note that the Ash--Ginzburg and Asgari--Shahidi choices are interchanged by conjugation by $\tau \defeq \smallmatrd{1}{}{}{w_n}$. In particular, given the choice of $\UPS$ fixed above, we have $\UPS^\tau_{i}\UPS^{\tau}_{2n+1-i} = \eta$. This helps when it is more convenient to use Asgari--Shahidi's choice (see e.g.\ Remark \ref{rem:AG vs AS}).
	...
	\begin{proposition}\label{prop:spin root system}
		...
		\]
		In particular, $\cW_{\cG}$ is the semidirect product $\{\pm1\} \rtimes S_n$.
	\end{proposition}
	\begin{proof}
	...
	Recall in \S\ref{sec:shalika conventions} we fixed an unramified character $\theta$ of $T(\Qp)$ such that $\pi = \Ind_B^G \UPS$ and $\UPS_i\UPS_{n+i} = \eta$ (cf.\ \cite[(43)]{DJR18}, where $\UPS$ is denoted $|\cdot|^{(2n-1)/2}\lambda$). Recall $\tau = \smallmatrd{1_n}{}{}{w_n} \in \cW_G$, and that $\UPS^\tau$ satisfies the Asgari--Shahidi condition $\UPS_i^\tau\UPS_{2n-i}^\tau = \eta$. From \cite[p.177(i)]{AS06} and \cite[Prop.\ 5.1]{AS14}, we see:
 ...
 The $\tau$ is necessary as we are using the convention of Ash--Ginzburg (see \S\ref{sec:shalika conventions}). From this, we see that $\UPS^\tau$ is a more natural convention for GSpin computations. If we chose $\UPS$ as in Asgari--Shahidi, we could remove $\tau$ from (ii) and henceforth in this section.
...
The following is our main motivation for the definition of spin $p$-refinement.
	...
	\begin{definition}\label{def:shalika refinement 2}
...
\end{itemize}
	...
	\begin{proof}
		...
		\[
		U_{p,r}\fsigma(\rho) = \sum_{m \in M_{r,2n-r}(\Zp)/M_{r,2n-r}(p)}\fsigma\left(\rho\smallmatrd{1_r}{m}{0}{1_{2n-r}}t_{p,r}\right).
		\]
...
\begin{enumerate}[(1)]\s
    ...
    \[
    k \in B_n(\mathbb{Z}_p) \cdot w_n \cdot \Iwn.
    \]
...
with $\alpha \in B_n(\mathbb{Z}_p)$ and $\beta \in \Iwn$. We have
\[
k w_n \mu = \alpha w_n \beta w_n \mu \in B_n(\mathbb{Z}_p) \cdot \overline{\Iwn} \cdot \mu .
\]
But by the Iwahori decomposition, we see that
\[
B_n(\mathbb{Z}_p) \cdot \overline{\Iwn} \cdot \mu \subset B_n(\mathbb{Q}_p) \cdot \overline{\Iwn} \subset B_n(\mathbb{Q}_p) \cdot \overline{J}_r.
\]
We also have
\[
...
Conversely, suppose \eqref{eq:k and X}, and write $k = A w_n d$, with $A \in B_n(\Zp)$ and $d \in \Iwn$. Via the Iwahori decomposition, we may assume $d \in N_n(\Zp)$ is upper unipotent. 
    ...
    For $A$ and $d$ as above, set $B = 0$ and $D = A z^{2\beta} \in B_n(\mathbb{Q}_p)$. Also set $c = 0$ and $a = z^{-2\beta} w_n d w_n z^{2\beta} \in \Iwn$ (since $\beta \geq 1$). If we set $b = D^{-1}Xk \in M_n(\Qp)$, then \eqref{eq:support equality} holds. Clearly $\smallmatrd{A}{B}{0}{D} \in B_n(\Qp)$, so we are done if we can show $\smallmatrd{a}{b}{c}{d} \in \Iw$. This will hold if $b \in M_n(\Zp)$. Observe
    \begin{align*}
...
\begin{proposition}\label{prop:spin refinements are shalika}
	\begin{enumerate}[(i)]
		...
		\end{align*}
	where $\Upsilon'' = \vol\big[B_n(\mathbb{Z}_p) \cdot w_n \cdot \Iwn \big]$ is a constant independent of $\beta$.
	\end{enumerate}
...
\begin{proof}
	...
	\begin{align}\notag
		...
		&= (\delta_B^{1/2}\UPS)\Big[\smallmatrd{1}{}{}{z^{2\beta}}\Big]\cdot \operatorname{vol}\Big(B_n(\mathbb{Z}_p) \cdot w_n \cdot \Iwn \Big) \operatorname{vol}\Big(z^{2\beta}M_n(\zp)\Big),\label{eq:F_1 value 1}
	\end{align}
	...
	\end{align}
Here in the second equality we use that $\delta_B^{w_{2n}} = \delta_B^{-1}$, in the third we use the definition of spin refinements (Definition \ref{def:shalika refinement}), and in the fourth that $\eta(p)\cdot \eta(p)^2 \cdots \eta(p)^{n-1} = \eta(\det z)$ and $\alpha_{p,1}\cdots\alpha_{p,n-1}\cdot\alpha_{p,n+1}\cdots\alpha_{p,2n-1} = \alpha_p/\alpha_{p,n}.$ Finally, we obtain (ii) after combining \eqref{eq:F_1 value 1}, \eqref {eq:F_1 value 2} and \eqref{eq:F_1 value 3}, as
\[
...
(When $n=1$, we just have $\alpha_0 = (1,1)$ and $\alpha_1 = (1,0)$). 
	...
	Now we $p$-adically interpolate, with $\beta = 1$. Assume the choices $v_{(0)}, v_{(1)},...,v_{(n-1)},v_{(n),1},v_{(n),2}$ in Notation \ref{not:v_i} were all normalised so $v_{(i)}(u) = 1$, whence $v_{(i)}(N^1(\Zp)) \subset 1+p\Zp \subset \Zp^\times$, as in Proposition \ref{prop:support in N1} (and its proof).
...
\end{proof}
	...
	\begin{definition}[Modified Euler factors]  \label{def:modified euler factors} Let $\chi$ be a finite order character of conductor $p^\beta$ with $\beta \geq 0$, and $j \in \mathrm{Crit}(\lambda)$. At $p$, let 
				\[
			e_p(\tilde\pi,\chi,j) \defeq \bigg(\frac{p^{nj + \tfrac{n^2-n}{2}}}{\alpha_{p,n}}\bigg)^\beta \tau(\chi)^n \ \text { for }\chi \neq 1, \qquad \text{and} \qquad e_p(\tilde\pi,1,j) = \prod_{i=n+1}^{2n}
			\frac{1-p^{-1}\alpha_{i,j}^{-1}}{1 - \alpha_{i,j}},
			\]
			where $\alpha_{i,j} = \UPS_i(p)/p^{j+1/2}$, recalling $\pi_p = \Ind_B^G \UPS$. At $\infty$, let $\varepsilon_{\pi} \in \{\pm 1\}$ be $\det(w_n)\epsilon(\pi_\infty)$, where $\epsilon(\pi_\infty)$ is the sign attached to $ \pi_\infty$ in \cite[(5.13)]{JST}, and let
			\[
				e_\infty(\pi, \chi, j) \defeq \varepsilon_{\pi}^j \cdot i^{-jn}  \cdot L(\pi_\infty \otimes \chi_\infty, j+1/2).
			\]
	\end{definition}
...
This leaves interpolation at the unramified character $\chi = 1$. We will complete the proof in this case in Proposition \ref{prop:unramified interpolation}.
	\end{proof}
	
	
	\begin{remark}
		The factor $e_p(\tilde\pi,\chi,j)$ is consistent with the Coates--Perrin-Riou conjecture from \cite{coates89}; this is mostly explained in \cite[\S3]{AG94}.
	\end{remark}
	...
	\begin{proof}
		...
		(ii) This follows from rigid delocalisation of (i) (as in \cite[Cor.\ 4.7]{BW18} and \cite[\S2.7]{BW-Iwasawa}; cf.\ \cite[Prop.\ 8.16]{BDW20}).
	\end{proof}
	...
	\begin{proof}
		...
		\]
		where $(*)$ is non-zero. As $\mathrm{sp}_{\lambda_\pi}\left(\mathrm{Ev}_{B,\beta}^{\Omega,\mathbf{1}}(\Phi_{\sC}^\pm)\right) \neq 0$, we deduce $\mathrm{Ev}_{B,\beta}^{\Omega,\mathbf{1}}(\Phi_{\sC}^\pm) \neq 0$, that is, it is not the zero distribution. Since $\cD(\Zp^\times,\cO_\Omega)$ is a torsion-free $\cO_\Omega$-module, it follows that $\mathrm{Ann}_{\cO_\Omega}(\Phi_{\sC}^\pm) = 0$ (cf.\ \cite[Prop.\ 7.11]{BDW20}). Thus $I^\pm_{\sC} = 0$, and $\hc{t}(S_K,\sD_\Omega)^{\leq h,\pm}\otimes_{\bT_{\Omega,h}^\pm}T^\pm \cong \cO_\Omega$.
	...
	\begin{proof}
		(i) Lemma \ref{lem:spec isomorphism} and Proposition \ref{prop:mult one} show $\m_{\tilde\pi}$ appears in $\hc{t}(S_K,\sD_\Omega)^{\leq h,\pm}$, giving points $x_{\tilde\pi}^\pm \in \cE_{\Omega,h}^\pm$. Moreover Proposition \ref{prop:etale} shows $\sE_{\Omega,h}^\pm \to \Omega$ is \'etale at $x_{\tilde\pi}^\pm$.  Let $\sC^\pm \subset \sE_{\Omega,h}^\pm$ be the unique connected component through $x_{\tilde\pi}^\pm$. Using strong non-criticality of $\tilde\pi$, we deduce that $\sC^\pm$ contain Zariski-dense sets $\sC_{\mathrm{nc}}^\pm$ of cuspidal non-critical slope classical points by \cite[Prop.\ 5.15]{BW20}. Now, as in \cite[Prop.\ 8.18]{BDW20}, we can exhibit a bijection between $\sC_{\mathrm{nc}}^+$ and $\sC_{\mathrm{nc}}^-$ and (via \cite[Thm.\ 3.2.1]{JoNew}) a canonical isomorphism $\sC^+ \isorightarrow \sC^-$, whence $\sC\cong \sC^+ \cong \sC^-$ is independent of sign. Part (i) follows immediately.
	...
	\begin{definition} \label{def:Lp families} Let $\cL_{p}^{\sC, \pm} \defeq {\color{ blue}[\det(-w_n)]}\mu^{\Omega,\mathbf{1}}(\Phi_{\sC}^\pm)$, now considering the Dirac measure as being in $\cD(\Zp^\times,\cO_\Omega)$. Also let $\Phi_{\sC} = \Phi_{\sC}^+ + \Phi_{\sC}^- \in \hc{t}(S_{K(\tilde\pi)},\sD_\Omega)$, a Hecke eigenclass, and define the \emph{$p$-adic $L$-function over $\sC$} to be 
		\[
...
We finally show that the $p$-adic $L$-functions we've constructed at Iwahori level agree with previous constructions, and deduce their interpolation property at unramified characters.
	...
	If $\tilde\pi$ is a non-critical slope regular $p$-refinement (to Iwahori level), let $\tilde\pi^Q \defeq (\pi, \alpha_{p,n})$. This is a non-$Q$-critical slope $Q$-regular $Q$-refinement as in \cite[\S2.7,\S3.5]{BDW20}, and Theorem 6.23 \emph{op.\ cit}.\ attaches a $p$-adic $L$-function $\cL_p(\tilde\pi^Q) \in \cD(\Zp^\times,L)$ to $\tilde\pi^Q$.
...
\end{equation}
 where $\lambda_\pi$ is the weight of $\pi$. 	In this case, both distributions have sufficiently small growth that they are uniquely determined by their interpolation properties at ramified characters. Their respective interpolation properties agree exactly except at the volume terms at $p$ (denoted $\gamma$ in \cite{DJR18,BDW20} and $\gamma_{(pm)}$ in the present paper). In particular at every character, the interpolation formulas differ by a rational number $\Upsilon$ independent of $\pi$. The result follows.
\end{proof}
...
The $p$-adic $L$-function $\cL_p(\tilde\pi^Q)$ satisfies the interpolation
		\begin{align*}
			\cL_p(\tilde\pi, z^j)
			= \Upsilon^{-1} \cdot \gamma_{(pm)} \cdot e_p(\tilde\pi,1,j) \cdot e_\infty(\pi,\chi,j)\cdot \frac{L^{(p)}\big(\pi, j+\tfrac{1}{2}\big)}{\Omega_\pi^{\pm}},
		\end{align*}
...
\begin{align*}
	...
	&= \Upsilon_Q \cdot \frac{p^{n^2} }{\alpha_{p,n}} \cdot e_\infty(\pi,\chi,j) \cdot \frac{L^{(p)}(\pi,j+\tfrac{1}{2})}{\Omega_\pi^\pm} \cdot \zeta_p\Big(j+\tfrac{1}{2}, (u^{-1}t_{p,n}^\beta)\cdot W_p, 1\Big).
\end{align*}
...
$\Delta_{\UPS}$ \dotfill function, Remark \ref{rem:conjugate}\\
...
Shalika refinement \dotfill Defn.\ \ref{def:shalika refinement 2}\\
spin refinement \dotfill Defn.\ \ref{def:shalika refinement}\\
