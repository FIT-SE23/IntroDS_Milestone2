...
%\title{On $p$-adic interpolation of branching laws for $\GL_{2n}$: $p$-adic $L$-functions in Shalika families}
...
\begin{document}
	...
	\renewcommand{\thefootnote}{\fnsymbol{footnote}} 
		%AMS : Comment here
	\footnotetext{\today. \emph{2020 MSC:} Primary 11F33,  11F67; Secondary 11R23, 11G22
	}     
	\renewcommand{\thefootnote}{\arabic{footnote}} 

	
	\begin{abstract}
		In this paper, we prove that a $\mathrm{GL}(2n)$-eigenvariety is \'etale over the (pure) weight space at non-critical Shalika points, and construct multi-variabled $p$-adic $L$-functions varying over the resulting Shalika components. Our constructions hold in tame level 1 and Iwahori level at $p$, and give $p$-adic variation of $L$-values (of regular algebraic cuspidal automorphic representations, or RACARs, of $\mathrm{GL}(2n)$ admitting Shalika models) over the whole pure weight space. In the case of $\GL(4)$, these results have been used by Loeffler and Zerbes to prove cases of the Bloch--Kato conjecture for $\mathrm{GSp}(4)$.
		
		Our main innovations are: a) the introduction and systematic study of `Shalika refinements' of local representations of $\mathrm{GL}(2n)$, evaluating their attached local twisted zeta integrals; and b) the $p$-adic interpolation of representation-theoretic branching laws for $\mathrm{GL}(n) \times \mathrm{GL}(n)$ inside $\mathrm{GL}(2n)$. Using (b), we give a construction of many-variabled $p$-adic functionals on the overconvergent cohomology groups for $\mathrm{GL}(2n)$, interpolating the zeta integrals of (a). We exploit the resulting non-vanishing of these functionals to prove our main arithmetic applications.
	\end{abstract}
	...
	Crucial to proofs of Bloch--Kato/Iwasawa Main Conjectures is a good understanding of $p$-adic $L$-functions, eigenvarieties, and $p$-adic $L$-functions over eigenvarieties. In this paper, we prove new results about these objects for $\GL_{2n}/\Q$. In particular, let $\pi$ be a regular algebraic symplectic cuspidal automorphic representation (RASCAR) of $\GL_{2n}(\A)$ that is everywhere spherical; here \emph{symplectic} implies $\pi$ admits a Shalika model (i.e.\ is a functorial transfer from $\mathrm{GSpin}_{2n+1}(\A)$). This ensures  there is an integer $\sw$ such that $\pi \cong \pi^\vee \otimes |\cdot|^{\sw}$, i.e.\ $\pi$ is essentially self-dual. Let $\tilde\pi$ be a non-critical slope Iwahoric $p$-refinement. By \cite[Thm.\ A]{BDW20}, there is a $p$-adic $L$-function $L_p(\tilde\pi)$ attached to $\tilde\pi$, that is, a locally analytic distribution on $\Zp^\times$ of controlled growth that interpolates its Deligne-critical $L$-values. In this paper:
	\begin{itemize}\setlength{\itemsep}{0pt}
		\item[(A)] we prove that that (full, Iwahoric) $\GL_{2n}$-eigenvariety is \'etale over the pure weight space at $\tilde\pi$, and that the unique ($n+1$ dimensional) connected component $\sC$ through $\tilde\pi$ contains a Zariski-dense set $\sC_{\mathrm{Sha}}$ of classical Shalika points; and
		\item[(B)] we construct an $(n+2)$-variable $p$-adic $L$-function $\mathcal{L}_p^{\sC}$ interpolating $L_p(\tilde\pi_y)$ for $y \in \sC_{\mathrm{Sha}}$.
	\end{itemize} 
...
\]
 where $\Omega$ is a (2-dimensional) parahoric $p$-adic family of weights, $\cD_\Omega^Q$ is a space of parahoric distributions, and $\cD(\Zp^\times,R)$ is the space of $R$-valued locally analytic distributions on $\Zp^\times$; so $\cD(\zp^\times,\cO_\Omega)$ is a space of 3-variable distributions. These interpolated the $\mathrm{Ev}_{\chi,j}^{\lambda,\mathrm{DJR}}$ for varying $\lambda, \chi$ and $j$. They were used to construct $p$-adic $L$-functions $L_p(\tilde\pi^Q)$ attached to finite slope $Q$-parahoric  Shalika $p$-refinements $\tilde\pi^Q$, to construct 2-dimensional (parahoric) $p$-adic families through $\tilde\pi^Q$, and to vary $L_p(\tilde\pi^Q)$ over these families.
	...
	Let $\cH_p^G$ be the Hecke algebra at $p$ (see \S\ref{sec:hecke p}). An (Iwahoric) \emph{$p$-refinement} of $\pi_p$ is a system of Hecke eigenvalues 
\[
	\alpha : \cH_p^G \longrightarrow \overline{\Q}
\]
 appearing in $\pi_p^{\Iw}$; we write $\tilde\pi_p = (\pi_p,\alpha)$. If $\pi_p$ has regular semisimple Satake parameter, there are $(2n)!$ such $p$-refinements, indexed by elements of the Weyl group $\cW_G = \mathrm{S}_{2n}$. 
	
	Attached to any $p$-refinement $\tilde\pi_p$ is a certain family of twisted local zeta integrals at $p$. We call $\tilde\pi_p$ a \emph{Shalika $p$-refinement} if one of these local zeta integrals is non-vanishing. 
...
In \cite{DJR18,BDW20}, it is implicitly predicted that a $p$-refinement should be Shalika if and only if it lies in a certain class of `spin' $p$-refinements, i.e.\ those that interact well with the Shalika model. In the parahoric case, an ad-hoc definition of a spin refinement -- there called a `$Q$-regular $Q$-refinement' -- is given in \cite[\S3.3]{DJR18}, inspired by \cite{AG94}; and the relevant zeta integrals are shown to be non-vanishing.
	...
	In \S\ref{sec:spin refinements}, we give a much more conceptual definition of spin $p$-refinements, generalising and justifying \cite[\S3.3]{DJR18}. Since $\pi_p$ admits a Shalika model, it is the functorial transfer of a representation $\Pi_p$ of $\cG(\Q_p)$, where $\cG = \mathrm{GSpin}_{2n+1}$. Via a careful study of the root systems of $G$ and $\cG$, we construct a map $\jmath^\vee : \cH_p^{G} \to \cH_p^{\cG}$ of Hecke algebras at $p$. We then say $\alpha$ is a \emph{spin $p$-refinement} if there exists an eigensystem $\alpha^{\cG}$ for $\cG$ such that $\alpha$ factors as
\[
	\alpha : \cH_p^G \xrightarrow{\ \jmath^\vee\ } \cH_p^{\cG} \xrightarrow{\ \alpha^{\cG}\ } \overline{\Q},
\]
and show the system $\alpha^{\cG}$ then appears in $\Pi_p^{\Iwahori_{\cG}}$. There are $2^nn!$ such spin $p$-refinements. 
	
	We prove that the family of local zeta integrals attached to a spin $p$-refinement is non-vanishing, and hence that spin $p$-refinements are Shalika $p$-refinements. We hope to prove the converse (that Shalika refinements are spin refinements) in a sequel. 
...
Attached to $H$ are a family of \emph{automorphic cycles} $\{X_\beta\}_{\beta \geq 1}$, which are modified locally symmetric spaces for $H$ which crucially have dimension $t$. For $j \in \mathrm{Crit}(\lambda)$, the evaluation maps $\mathrm{Ev}_{\chi,j}^{\lambda,\mathrm{DJR}}$ of \cite{DJR18} were constructed as a composition
\[
\hc{t}\big(S_K,\sV_\lambda\big) \to \hc{t}\Big(X_\beta,\sV_\lambda|_H\Big) \to \hc{t}\Big(X_\beta, \sV^H_{(-j,\sw+j)}\Big) \to \bigoplus_{(\Z/p^\beta)^{\times}}\overline{\Q}_p \to \overline{\Q}_p,
\]
where:
\begin{itemize}\setlength{\itemsep}{0pt}
...
\end{itemize}	
	...
	The full pure weight space $\sW_0^G$ for $\GL_{2n}$ has dimension $n+1$, whilst the families of \cite{BDW20} are only 2-dimensional. The trade-off made \emph{op.\ cit}.\ was that variation in lower dimension allowed weaker assumptions on $\tilde\pi$, giving an `optimal' notion of non-criticality for $\tilde\pi$. However, if one assumes stronger conditions on $\tilde\pi$, then one expects to be able to vary the $p$-adic $L$-function over the full $(n+1)$-dimensional pure weight space; and it is this, higher-dimensional, variation that is required for the application to the Bloch--Kato conjecture of \cite{LZ20}.
...
In Proposition \ref{prop:distribution commute}, we give a full interpolation of ($\dagger$) over $\sW_0^G$ (in the language of $p$-adic distributions). This result occupies the entirety of \S\ref{sec:branching laws}. Our approach exploits properties of \emph{spherical varieties}, and should apply in much more general settings.
	...
	We give two main applications of these results. Using the Shalika refinements of \S\ref{sec:shalika refinements}, we give simple automorphic criteria for the non-vanishing of the evaluation maps \eqref{eq:intro evaluation}. This non-vanishing puts tight restrictions on the structure of $\hc{t}(S_K,\sD_\Omega)$, and thus -- via \cite{Han17} -- on the structure of the $\GL_{2n}$-eigenvariety $\sE^{\GL_{2n}}$. Exploiting ideas developed in \cite{BDW20}, we prove:
	
	\begin{theorem-intro}\label{thm:intro A}
		Suppose $\lambda$ is regular, and let $\tilde\pi$ be a regular non-critical slope $p$-refinement of $\pi$. Then the $\GL_{2n}$-eigenvariety $\sE^{\GL_{2n}}$ is \'etale over $\sW_0^G$ at $\tilde\pi$. There exists a neighbourhood of $\tilde\pi$ in $\sE^{\GL_{2n}}$ containing a very Zariski-dense set of classical points corresponding to RASCARs.
	\end{theorem-intro}
	
	In other words, there exists an $(n+1)$-dimensional neighbourhood $\Omega \subset \sW_0^G$ of $\lambda$ such that:
	\begin{itemize}\setlength{\itemsep}{0pt}
		\item[(a)] $\tilde\pi$ varies in a unique $p$-adic family $\sC \subset \sE^{\GL_{2n}}$ over $\Omega$,
		\item[(b)] $\sC$ contains a very Zariski-dense set $\sC^{\mathrm{Sha}}$ of classical points corresponding to RASCARs,
		\item[(c)] and the weight map $\sC \isorightarrow \Omega$ is an isomorphism.
	\end{itemize}
	To prove this result, we observe that regularity of $\lambda$ implies existence of a non-vanishing Deligne-critical $L$-value. Since $\mathrm{Ev}_\beta^\Omega$ interpolates these $L$-values, it is therefore non-vanishing. We use this twice: once to produce existence of an $(n+1)$-dimensional family, and again to prove existence of a Zariski-dense set of classical points attached to RASCARs.
	
	Our second main result, under the same hypotheses, is the construction of an $(n+2)$-variable $p$-adic $L$-function over $\sC$. We show that $\mathrm{Ev}_{\beta+1}^\Omega = \mathrm{Ev}_{\beta}^\Omega \circ U_p^\circ$, where $U_p^\circ$ is the full (normalised) Iwahori Hecke operator at $p$. We thus use \eqref{eq:intro evaluation} to attach a well-defined distribution $\mu^\Omega(\Phi) \defeq (\alpha_p^\circ)^{-\beta}\mathrm{Ev}^{\Omega}_\beta(\Phi)$ to any finite-slope eigenclass $\Phi \in \hc{t}(S_K,\sD_\Omega)$ with $U_p^\circ \Phi = \alpha_p^\circ \Phi$. Note this is independent of $\beta$. We show existence of a distinguished eigenclass $\Phi_{\sC} \in \hc{t}(S_K,\sD_\Omega)$ attached to the family $\sC$, and then define $\cL_p^{\sC} \defeq \mu^\Omega(\Phi_{\sC}) \in \cD(\Zp^\times,\cO_\Omega)$. Under the Amice transform, and via (b), we view $\cL_p^{\sC}$ as a rigid analytic function $\cL_p^{\sC}$ on $\sC\times \sX(\Zp^\times)$, 	where $\sX(\Zp^\times)$ is the $\overline{\Q}_p$-rigid space of characters on $\Zp^\times$. We show:
	
	
	\begin{theorem-intro}\label{thm:intro family}
		For all $y \in \sC^{\mathrm{Sha}}$,  there exist $c_y^\pm \in \overline{\Q}_p^\times$ such that for all $\chi \in \sX(\Zp^\times)$ with $\chi(-1) = \pm 1$, we have
		\begin{equation}\label{eq:int in C}
			\cL_p^{\sC}(y,\chi) = c_y^{\pm} \cdot L_p(\tilde\pi_y, \chi),
		\end{equation}
	where $L_p(\tilde\pi_y,-)$ is the (1-variable) $p$-adic $L$-function from \cite{BDW20}.
	\end{theorem-intro}
	
	In other words, $\cL_p^{\sC}$ interpolates the $p$-adic $L$-functions from \cite{BDW20} as $y$ varies in $\sC$.
	
	
	
	\subsection{Application: Bloch--Kato for $\mathrm{GSp}(4)$}
	
	In \cite{LZ20}, Loeffler and Zerbes prove new cases of the Bloch--Kato conjecture for Galois representations attached to Siegel modular forms of genus 2 (i.e.\ for automorphic representations of $\mathrm{GSp}_4$). More precisely, if $\cF_{\mathrm{new}}$ is a Siegel modular form of level 1 and sufficiently high weight, and $\cF$ is an ordinary $p$-stabilisation, they prove the Bloch--Kato conjecture holds for the spin Galois representation attached to $\cF$ in analytic rank 0. This has also led to new understanding of the Bloch--Kato conjecture for symmetric cube modular forms \cite{LZ-cube} and of Iwasawa theory for quadratic Hilbert modular forms \cite{LZ-HilbertQuadratic}.
...
This non-vanishing is delicate, since the $p$-adic $L$-value seen by the explicit reciprocity law is \emph{outside} the region of interpolation (so it does not directly relate to a Deligne-critical classical $L$-value). In \cite[\S17]{LZ20}, Loeffler--Zerbes deform this into the region of interpolation -- and thus prove the Bloch--Kato conjecture -- conditional on the existence of a family of $p$-adic $L$-functions on $\GL_4$, stated as Theorem 17.6.2 \emph{op.\ cit}. This theorem, whose proof was deferred to `forthcoming work' of the present authors, is a special case of Theorem \ref{thm:intro family}.
	
	
	
	\subsection{Remarks on assumptions}
	
	We restrict to base field $\Q$ and $\pi$ of tame level 1, a setting where all of our key new ideas are already present. These assumptions drastically simplify the notation and reduce technicality, allowing for a shorter, more conceptual article, whilst still including the results required by \cite{LZ20}. We indicate which of our various assumptions could be relaxed.
	
	Firstly, all of these results can be modified in a conceptually straightforward (but notationally awkward) way to work for $\GL_{2n}$ over an arbitrary totally real field $F$. This was the setting treated in \cite{BDW20}; the reader could consult that paper for the extra details occurring in this case.
	
	Our most serious assumption is that $\pi$ has tame level 1. This is certainly not necessary to $p$-adically interpolate branching laws and evaluation maps, and our results in this direction make no assumption on the tame level. We can also entirely remove this assumption in proving Theorem \ref{thm:intro A}, following \cite[\S7]{BDW20}. However, to prove Theorem \ref{thm:intro family} requires much more control: we need not only that systems of Hecke eigenvalues vary $p$-adically, but also that we can vary certain local test vectors in a family. This is unconditionally possible in tame level 1. This could be relaxed to the assumption that $\pi$ admits parahoric-fixed vectors at every finite place using \cite{DJ-parahoric}. For a discussion on possible generalisations beyond this, see \cite[\S8]{BDW20}.
	...
	This paper owes a great debt to the observations of David Loeffler, and we thank him profusely. We thank Mahdi Asgari for patiently answering our questions about his work on $\mathrm{GSpin}_{2n+1}$. 
	
	DBS was supported by FONDECYT 11201025 and PAI 7718007. MD was supported by Agence	Nationale de la Recherche grant ANR-18-CE40-0029. AG was supported in part by ERC-2018-COG-818856-HiCoShiVa. AJ is grateful for support from CEMPI, Lille, the Center for Mathematics at Notre Dame, and the NSF. CW was supported by EPSRC Postdoctoral Fellowship EP/T001615/1. Finally we thank the CIRM in Luminy and the CMND program on Eigenvarieties in Notre Dame, where parts of this work were carried out.
	...
	%\addcontentsline{toc}{part}{Part I: Automorphic results}
	...
	Let $n\geq 1$ and let $G \defeq \mathrm{GL}_{2n}$. We write $B = B_{2n}$ for the Borel subgroup of upper triangular matrices, $\overline{B} = \overline{B}_{2n}$ for the opposite Borel of lower triangular matrices and $T = T_{2n}$ for the maximal split torus of diagonal matrices. We have decompositions $B= TN$ and $\overline{B} = \overline{N}T$ where $N = N_{2n}$ and $\overline{N} = \overline{N}_{2n}$ are the unipotent radicals of $B$ and $\overline{B}$. We also let $G_n = \GL_n$, with $B_n, T_n, N_n$ etc.\ the analogous subgroups. Let $H \defeq \GL_n \times \GL_n$, with an embedding $\iota : H \hookrightarrow G$, $\iota(h_1,h_2) = \smallmatrd{h_1}{0}{0}{h_2}$. 
	
	Let $\cW_G = \mathrm{S}_{2n}$ (resp.\ $W_{n} = \mathrm{S}_n$) be the Weyl group of $G$ (resp.\ $G_n$), identified with the permutation subgroup of $G(\Z)$ (resp.\ $G_n(\Z)$). We write $w_{2n}$ and $w_n$ for the longest Weyl elements (i.e. the antidiagonal matrices with 1s on the antidiagonal). 
	
	Let $K_\infty=C_\infty Z_\infty$, where  $Z_\infty$ is the center and $C_\infty$ is the maximal compact subgroup of $G(\R)$. 
	For any reductive real Lie group $A$ we let $A^\circ$ denote the connected component of the identity.
	...
	\end{equation}
	We repeatedly use that if $\pi_\ell$ is a smooth irreducible representation of $\GL_{2n}(\Ql)$ that is spherical, i.e.\ $\pi_\ell^{\GL_{2n}(\Ql)} \neq 0$, then there is an unramified character $\UPS : T(\Ql) \to \C^\times$ such that
	\begin{equation}\label{eq:unramified induced}
		\pi_\ell = \Ind_B^G \UPS,
	\end{equation}
the normalised parabolic induction of $\theta$ to $G$.
	...
	\end{equation}
	This forces $\eta^n$ to be the central character of $\pi$, and $\eta = \eta_0|\cdot|^{\sw}$, where $\eta_0$ has finite order and $\sw = \mathrm{pure}(\lambda)$. Let $\eta\otimes\psi$ be the character of $\cS(\A)$ given by $(\eta\otimes\psi)(s) = \eta(\det(h))\psi(\mathrm{tr}(X))$. If \eqref{eq:shalika integral} holds, then $\cS_\psi^\eta$ defines an intertwining $\pi \hookrightarrow \mathrm{Ind}_{\cS(\A)}^{G(\A)}(\eta \otimes \psi)$.
	...
	By \cite[Prop.~1.3]{AG94}, if $\pi_\ell$ is spherical then it admits a $(\eta_\ell,\psi_\ell)$-Shalika model if and only if $\pi_\ell^\vee =\pi_\ell\otimes \eta_\ell^{-1}$, in which case case $\eta_\ell$ is unramified. 
	
	We recall another characterisation of Shalika models:
	
	\begin{theorem}[Jacquet--Shalika, Asgari--Shahidi]
	...
	\end{theorem}
	\begin{proof}
		By \cite{JS90}, having a global Shalika model is equivalent to a (partial) exterior square $L$-function having a pole at $s=1$. But in \cite{AS14} this is shown to be equivalent to being such a functorial transfer. (For further details see \cite[Prop.\ 3.1.4]{GR2}).
	\end{proof} 
	...
	\subsubsection{The Hecke algebra at $p$}
...
\end{definition}
	...
	\end{equation}
	We let $\m_{\tilde\pi} \defeq \ker(\psi_{\tilde\pi})$ be the corresponding maximal ideal of $\cH \otimes E$. If $M$ is a module upon which $\cH\otimes E$ acts, we write $M_{\tilde\pi}$ for  the localisation of $M$ at $\m_{\tilde\pi}$. If $L$ is a field containing $E$ and $M$ is a finite-dimensional $L$-vector space, $M_{\tilde\pi}$ is the generalised eigenspace upon which $\cH\otimes L$ acts by $\psi_{\tilde\pi}$. 
	
	We recall the standard classification of $p$-refinements for spherical $\pi_p$. Let $\UPS$ be an unramified character such that $\pi_p = \Ind_B^G \UPS$ (as in \eqref{eq:unramified induced}). Recall $\cW_G = \mathrm{S}_{2n}$ and $\delta_B$ from \S\ref{sec:notation}.
	
	\begin{proposition}\emph{\cite[Lem.\ 4.8.4]{Che04}.} \label{prop:p-refinement}
		\begin{enumerate}[(i)]
			\item 
		 The semisimplification of $\pi_p^{\Iw}$ as a $\cH_p$-module is isomorphic to $\bigoplus_{\sigma\in \cW_G}(\delta_B^{1/2}\UPS^\sigma) \circ \mathrm{ev}_p$, where $\mathrm{ev}_p$ is the map sending $[\Iw \cdot \mu \cdot \Iw] \in \cH_p$ to $\mu(p)$, where $\mu \in X_*(T)$. 
	...
	\begin{align*}
		\text{$\pi_p$ admits a regular $p$-refinement} &\iff \text{its Satake parameter is regular semisimple}\\
...
\end{enumerate}
	...
	\begin{remark}\label{rem:conjugate}
		\begin{itemize}\setlength{\itemsep}{0pt}
			\item[(i)] This normalisation, with $w_{2n}$, matches \cite{DJR18} but might appear strange. Chenevier uses antidominant cocharacters, and switching to dominant characters is equivalent to conjugating by $w_{2n}$. This normalisation will be convenient in \S\ref{sec:shalika refinements}.
			
			\item[(ii)] When the Satake parameter is regular semisimple, there is a bijection 
\[
...
\]
 induced by the above. This is \emph{not} canonical, depending on the choice of character $\UPS$ from which we induce. For $\tau \in \cW_G$, replacing $\UPS$ by $\UPS^\tau$ conjugates the image of $\Delta_\UPS$ by $\tau$. 
			...
			\item[(iii)] When $\pi_p$ is the local component of a RACAR, the eigenvalues $\alpha_{p,r}$ are algebraic but not $p$-integral. To account for this, we make the following definition.
		\end{itemize}
	\end{remark}
...
\]
 The $\alpha_{p,r}^\circ$ are $p$-integral (see Remark \ref{rem:integral normalisation} or \cite[Rem.\ 3.23]{BW20}).
	...
	For $K \subset G(\A_f)$ open compact, the \emph{locally symmetric space of level $K$} is 
\[
...
\]
 It is a $(2n-1)(n+1)$-dimensional real orbifold. We will now recall how to realise RACARs in the compactly supported Betti cohomology of $S_K$.
	...
	\subsection{Local systems}\label{sec:local systems} We recall standard facts about local systems on $S_K$ (e.g.\ \cite[\S1]{Urb11}, \cite[\S2.3]{BDW20}). If $M$ is a left $G(\Q)$-module such that the centre of $G(\Q)\cap KK_\infty^\circ$ acts trivially, let $\cM$ be the local system on $S_K$ given by locally constant sections of $G(\Q)\backslash [G(\A) \times M]/KK_\infty^\circ$, with action $\gamma(g,m)kz = (\gamma g kz, \gamma \cdot m)$. We denote such local systems with calligraphic letters.
	...
	\subsubsection{Localisation at RACARs} 
	...
	We have $K_\infty/K_\infty^\circ = \{\pm 1\}$. This group has two characters $\epsilon^\pm : K_\infty/K_\infty^\circ \to \{\pm 1\}$, where $\epsilon^\pm$ sends $-1$ to $\pm 1$. If $M$ is a module on which $K_\infty/K_\infty^\circ$ acts and 2 acts invertibly -- for example, the cohomology of $S_K$ over a field -- then we have $M = M^+ + M^-$, where $M^\pm$ are the eigenspaces where $K_\infty/K_\infty^\circ$ acts via $\epsilon^\pm$. We obtain a (Hecke-equivariant) decomposition of the cohomology groups $\hc{\bullet}(S_K,-)$ into $\pm$-submodules (as the action of $K_\infty/K_\infty^\circ$ commutes with the $G(\A_f)$-action).
	...
	In \S\ref{sec:slope-decomp}, we will equip $V_\lambda^\vee(\overline{\Q}_p)$ with another natural action of $\GL_{2n}(\Zp)$ and $t_{p,r}$, denoted $*$. Concretely, we will have $t_{p,r} * \mu = \lambda(t_{p,r})(t_{p,r} \cdot \mu)$. In light of Definition \ref{def:integral}, if we let $U_{p,r}^*$ be the Hecke operator defined via \eqref{eq:Hecke composition} with the $*$-action instead of the $\cdot$-action, then $U_{p,r}^* = U_{p,r}^\circ$ is integrally normalised. This is all explained in detail in \cite[Rem.\ 3.13]{BDW20}.
	
	
	\subsection{Cohomology classes attached to RACARs} \label{sec:cohomology for RACARs}
	
	Let $t = n^2 + n -1$, which is the top degree of cohomology to which RACARs for $G(\A)$ contribute. In particular, let $\pi$ be a RACAR of $G(\A)$; then we recall that there exists a  a Hecke-equivariant isomorphism
	\begin{equation}\label{eq:pi to cohomology}
		\pi_f^K \isorightarrow \hc{t}(S_K,\sV_\lambda^\vee(\overline{\Q}_p))^\pm_{\pi},
	\end{equation}
	for a unique $\lambda \in X_0^*(T)$. The isomorphism \eqref{eq:pi to cohomology} is non-canonical, depending on our fixed choice of $i_p : \C \isorightarrow \overline{\Q}_p$ and a choice of basis $\Xi_{\infty}^\pm$ of the 1-dimensional $\C$-vector space $\h^t(\fg_\infty, K_\infty^\circ; \pi_\infty\otimes V_\lambda^\vee(\C))^\pm$, where $\fg_\infty \defeq \mathrm{Lie}(G_\infty)$. This is all standard, explained e.g. in \cite[Prop.\ 2.3]{BDW20}.
	...
	\begin{remark}
		Since the notation is heavy, we sketch the differences between our new definitions and those of \cite{BDW20}. Firstly, to better suit the more general theory, we replace the twisting operator $\xi = \smallmatrd{1_n}{w_n}{0}{w_n}$ of \cite[Def.\ 4.2]{BDW20} with $u^{-1}$, where $u = \smallmatrd{1_n}{w_n}{0}{1_n} \in G(\Zp)$. Unlike $\xi$, the element $u^{-1}$ lies in $\Iw$. We will show that the definitions/results of \cite{BDW20} are essentially unchanged with this switch.
	...
	\subsubsection{Automorphic cycles}
	...
	\begin{definition}\label{def:parabolic}
		Let $P\subset Q \subset \GL_{2n}$ be  a standard parabolic with Levi $\GL_{m_1} \times \cdots \times \GL_{m_r}$. Define a diagonal matrix 
		\[
			t_P = \mathrm{diag}(p^{r-1}\mathrm{I}_{m_1}, p^{r-2}\mathrm{I}_{m_2}, \cdots, p\mathrm{I}_{m_{r-1}}, \mathrm{I}_{m_r}).
		\]
		Note $t_P \in T_P^{++}$ as in \cite[\S2.5]{BW20}; and since $P \subset Q$, the first $n$ diagonal entries are all a positive power of $p$.
	\end{definition}
	...
	\begin{definition}\label{def:auto cycle}
		Fix $m \in \Z_{\geq 0}$ prime to $p$, and let $K = K_pK^p \subset G(\A_f)$ be open compact. For $\beta \in \Z_{\geq 0}$, define an open compact subgroup $L_\beta^P = L_p^{P,\beta} L^p \subset H(\A_f)$ by setting:
		\begin{itemize}\setlength{\itemsep}{0pt}
...
\]
by definition of the modulus character $\delta_B$. It follows that $\vol(L_p^{P,\beta}) = \delta_B(t_P^{\beta-1}) \vol(L_p^{P,1})$, from which the result follows.
\end{proof}
 ...
 We have $L_p^{P,\beta} \subset L_p^{Q,\beta}$.
	...
	\begin{equation}\label{eq:component group}
		\pi_0(X_\beta^P) \defeq \Cl(p^\beta m) \times \Cl( m)
	\end{equation}
	...
	Define $J_P \subset \GL_{2n}(\Zp)$ be the parahoric subgroup for $P$. Define $\Delta_P \subset \GL_{2n}(\Qp)$ to be the semigroup generated by $J_P$ and the matrices
	\[
	...
	\begin{lemma}(Variation in $M$; \cite[Lem.\ 4.6]{BDW20}) \label{lem:pushforward}
	...
	\begin{proposition}(Variation in $\delta$; \cite[Prop.\ 4.9]{BDW20}) \label{prop:ind of delta}
	...
	\begin{proposition}(Variation in $\beta$; \cite[Prop.\ 4.10]{BDW20}) \label{prop:evaluations changing beta} Let $N$ and $\kappa$ be as in Proposition~\ref{prop:ind of delta}. If $\beta > 0$, then as maps $\hc{t}(S_K,\sM) \to N$ we have
		\[
	...
	\begin{proof}
		...
		\] 
		where $K^0_P(p) = K\cap t_PKt_P^{-1}$, the map $\iota_\beta^{P,0}$ is induced by the map $[h] \mapsto [\iota(h)u^{-1}t_P^\beta],$ and where the horizontal maps are the natural projections. We need to show that this square is Cartesian in this generality. For this, since the vertical maps are embeddings, it is enough to show that the horizontal maps have the same degree; i.e., that $[K : K^0_P(p)] = [L_p^{P,\beta} : L_p^{P,\beta+1}]$ for any $\beta \geq 1$. Let $N_P$ denote the unipotent radical of $P$ and set $N_{\beta} = t_P^\beta N_P(\mathbb{Z}_p) t_P^{-\beta}$. By the Iwahori decomposition for $K_p$, we easily find that $[K : K^0_P(p)] = [N_P(\mathbb{Z}_p) : N_1] = [N_{\beta} : N_{\beta+1}]$ for any $\beta \geq 1$. On the other hand, the element $u^{-1}$ is a representative of the (unique) Zariski dense $H$-orbit in the flag variety $G/\overline{P}$, where $\overline{P}$ denotes the opposite of $P$. Therefore the proof of \cite[Lem.\ 4.4.1]{loeffler-parabolics} implies that $[L_p^{P,\beta} : L_p^{P,\beta + 1}] = [N_{\beta} : N_{\beta+1}]$. Hence we have 
	\[
	[K : K^0_P(p)] = [L_p^{P,\beta} : L_p^{P,\beta + 1}]% = \delta_P(t_P) = \delta_B(t_P) 
	\]
	as required.
	\end{proof}
	...
	\end{equation}	
	It is the $p$-adic character associated to the adelic norm \cite[\S2.2.2]{BW_CJM}. It is trivial on $F_\infty^\circ$.
	...
	\begin{definition}\label{def:ev chi}
		...
		\[
		\cE_{P,\beta,\brep}^{j,\eta_0} \defeq \sum_{[\delta] \in \mathrm{pr}_\beta^{-1}(\brep)} \eta_0\big(\mathrm{pr}_2([\delta])\big)\ \cE_{P,\beta,[\delta]}^{j,\sw} \ \ \ : \hc{t}(S_K,\sV_\lambda^\vee(L)) \to L.
		\]
		...
		\begin{align}\label{eq:classical evaluation}
			...
			\phi &\longmapsto \sum_{[\delta] \in (\Z/p^\beta m)^\times \times (\Z/m)^\times} \chi\big(\mathrm{pr}_\beta([\delta])\big)\cdot  \eta_0\big(\mathrm{pr}_2([\delta])\big)\cdot\left( \delta*\left[\kappaj \circ \mathrm{Ev}_{P,\beta,\delta}^{V_{\lambda}^\vee}(\phi)\right]\right).\notag
		\end{align}
	\end{definition}
	...
	\begin{remark}\label{rem:classical diagram}
		...
		\[
		\Xi_{\brep}^{\eta_0} : (m_{[\delta]})_{[\delta]} \longmapsto \sum_{[\delta] \in \mathrm{pr}_{\beta}^{-1}(\brep)} \eta_0(\mathrm{pr}_2([\delta])) \cdot m_{[\delta]}.
		\]
	\end{remark}
...
We give two applications of these maps. Let $\pi$ be any RACAR of weight $\lambda$ with attached maximal ideal $\m_\pi \subset \cH'$ as in \S\ref{sec:hecke algebra}. 
	...
	\begin{proposition}\label{prop:shalika non-vanishing}
		Suppose there exists $\phi \in \hc{t}(S_K,\sV_{\lambda}^\vee(\overline{\Q}_p))^\pm_{\m_\pi}$ and some  $\chi, j$ and $\eta_0$ such that
	...
	\end{proposition}
	\begin{proof}
		This is proved exactly as in \cite[Prop.\ 5.14]{BDW20}. Whilst $\xi$ is replaced by $u^{-1}$, and some non-zero volume factors change depending on $P$, the argument of proof is identical.
	\end{proof}
	...
	\begin{theorem}\label{thm:critical value}
		Let $\chi$ be a finite-order Hecke character of conductor $p^{\beta'}$, let $\beta = \mathrm{max}(1,\beta')$, and let $j \in \mathrm{Crit}(\lambda)$. If $(-1)^j\chi\eta(-1) \neq \pm 1$, then $\cE_{P,\chi}^{j,\eta_0}(\Theta^\pm(W)/\Omega_\pi^\pm) = 0$. If $(-1)^j\chi\eta(-1) = \pm 1$, then
		\begin{align*}
		\cE_{P,\chi}^{j,\eta_0}\left(\frac{\Theta^\pm(W)}{\Omega_\pi^\pm}\right) = \delta_B(t_P^{-\beta})&\Upsilon_{P}\cdot \lambda(t_{P}^\beta)\cdot
		\zetainfty \\
&\times \frac{L(\pi\otimes\chi,j+1/2)}{\Omega_\pi^\pm} \cdot \zeta_p\Big(j+\tfrac12, (u^{-1}t_{P}^\beta)\cdot W_p, \chi_p\Big).
		\end{align*}
	\end{theorem} 
	
	Here $\Upsilon_{P}$ is a non-zero rational volume constant independent of $\chi$ and $j$; we have 
\[
\Upsilon_P = \gamma \cdot A_P^{-1} \cdot p^{n^2}\cdot [\#\GL_n(\Z/p\Z)]^{-1},
\] 
where $\gamma$ is the constant from \cite[(77)]{DJR18} and $A_P$ is the constant from Lemma \ref{lem:volume}. (Note that when $P = Q$ has Levi $\GL_n \times \GL_n$, we have $\Upsilon_Q = \gamma$). Also, $\zetainfty$ is an archimedean zeta integral (in the notation of \cite[Thm.~4.7]{DJR18}, our $\zetainfty/\Omega_\pi^\pm$ is denoted $\zeta_\infty(j+\tfrac{1}{2}, W_{\pi_\infty,j}^{(\varepsilon^j\chi\eta)_\infty})$; this is non-zero by \cite[Thm.~5.5]{Sun19}).
	
	\begin{proof}
		A rephrasing of \cite[Prop.\ 4.6, Thm.\ 4.7]{DJR18} in this language is described in \cite[Thm.\ 5.15]{BDW20}. As in \cite[Prop.\ 4.6]{DJR18}, one first  writes $\cE_{\chi}^{j,\eta_0}(-)$ as an explicit integral over $X_\beta^P$, introducing the factor $\lambda(t_{P}^\beta)$. One lifts this to an integral over $Z_G(\A)H(\Q)\backslash H(\A)$, introducing the volume constant $\delta_B(t_P^{-\beta})\Upsilon_{P}$ (as we divide by $\mathrm{Vol}(L_\beta^P)$, using Lemma \ref{lem:volume}). By \cite[Prop.\ 2.3]{FJ93} the integral equals a global Friedberg--Jacquet integral, which breaks into a product of local integrals. Away from $p$, the computation of these local zeta integrals is literally identical to that \emph{op.\ cit}.; and at $p$, by definition the zeta integral is the one in the statement of the theorem.
	\end{proof}
	
	We will evaluate this local zeta integral for $P=B$ and for specific choices of $W_p$ over the next few sections.
	...
	%\addcontentsline{toc}{part}{Part II: Local theory: Shalika $p$-refinements}
	...
	For the remainder of the paper, unless otherwise specified we specialise to $P = B$ and consider Iwahori level. 
		...
		\] 
	Over the next few sections, we compute the third term in this product -- the local zeta integral at $p$ -- for `nice' choices of $W_p$. This is a significant computation, spanning several sections, so we briefly sketch the steps.
	\begin{itemize}\s
	\item	In \S\ref{sec:local zeta p}, we compute $\zeta_p(s,(u^{-1}t_p^\beta)\cdot W_p, \chi_p)$ as an explicit non-zero multiple of a specific value of $W_p$ (depending on $\beta$).
	...
	\end{itemize}
		...
		\begin{notation*}
		Since it will be entirely focused on local theory, in Part II we henceforth drop subscripts $p$. In particular, we let $\pi$ be a spherical representation of $\GL_{2n}(\Qp)$ admitting an $(\eta,\psi)$-Shalika model, for $\eta : \Qp^\times \to \C^\times$ a smooth character and $\psi : \Qp \to \C^\times$ the usual additive character (e.g.\ \cite[\S4.1]{DJR18}). We continue to write $\Iw$ for the Iwahori subgroup of $\GL_{2n}(\Zp)$. We write $\zeta(-)$ in place of $\zeta_p(-)$. We keep the notation of \S\ref{sec:hecke algebra}, with matrices $t_{p,r}$ and Hecke operators $U_{p,r}$ on $\pi^{\Iw}$. A $p$-refinement $\tilde\pi = (\pi,\alpha)$ is a choice of Hecke eigensystem $\alpha$ occurring in $\pi^{\Iw}$.
	\end{notation*}
	...
	\begin{proof}
		...
		\]
		If $x \not\in M_{n}(\Z_p) \cap \GL_n(\Q_p)$, then we can choose $y$ such that $\mathrm{tr}(xy) \not\in \Z_p$, so $\psi(\mathrm{tr}(xy)) \neq 1$.
	\end{proof}
	...
	\begin{proposition}\label{prop:local zeta p}
		Let $\chi$ have conductor $p^\beta > 1$, and let $W \in \cS_{\psi}^{\eta}\left(\pi_p^{\Iw}\right)$. Then
		%\[
		%\zeta(s, (u^{-1}t_{p,n}^\beta)\cdot W, \chi) = \chi(-1)^{n-1}p^{rn\left(s-1/2\right)} \tau(\chi)^n W\smallmatrd{w_n}{}{}{1}.
		%\]
		\[
	\zeta\Big(s,(u^{-1}t_p^{\beta}\Big) \cdot W, \chi) = \Upsilon' \cdot \eta(\det z^\beta) \cdot p^{-\beta \tfrac{n^2+n}{2}} \cdot p^{\beta n(s-1/2)} \cdot \tau(\chi)^n  \cdot \chi(\det w_n)  \cdot W\tbyt{w_n z^{2\beta}}{}{}{1},
		\]
...
\end{align}
where in the trace term, we have conjugated by $z^{\beta}$, and where we define
\begin{equation}
...
We now cut down the support of this integral. Firstly, by Lemma \ref{SupportLemma} we can immediately reduce the support to $\GL_n(\Qp) \cap M_n(\Zp)$. To go further, we exploit Iwahori invariance of $W$.	 
		...
		\begin{enumerate}[(1)]
			...
			\]
\end{enumerate}
We will consider matrices of the form $\alpha = \gamma + \delta \in M_n(\Zp)$ for $\gamma \in A$ and $\delta \in B_\beta$. Note that $\alpha$ is in the depth $p^\beta$ Iwahori subgroup $\operatorname{Iw}_n(p^\beta) \subset \GL_n(\Zp)$ (the matrices that are upper-triangular modulo $p^\beta$). We set 
			\[
...
Now we have 
	...
	\psi\Big[ \mathrm{tr}(-p^{-n \beta} z^\beta x \alpha z^{-\beta} w ) \Big] = \psi\Big[ \mathrm{tr}(-p^{-n \beta} z^\beta x \gamma z^{-\beta} w ) \Big] \cdot \psi\Big[\mathrm{tr}(-p^{-n \beta} z^\beta x \delta z^{-\beta} w ) \Big].
	\end{equation}
	...
	\delta &\longmapsto \psi\Big[\mathrm{tr}(-p^{-n \beta} z^\beta x \delta z^{-\beta} w )\Big].
\end{align*}
This is a group homomorphism by additivity of trace and $\psi$.
	...
	\begin{lemma}\label{lem:F_x}
		...
		x_{n+1 - i, j} \in \left\{ \begin{array}{cc} p^{2\beta (n-i) + \beta} \mathbb{Z}_p & \text{ if } i > j \\ p^{2\beta (n-i)} \mathbb{Z}_p & \text{ if } i < j \end{array} \right.
		\end{equation}
	\end{enumerate}
	\end{lemma}
	\begin{proof}
		...
		\[
		\mathrm{tr}\Big[-p^{-n \beta} z^\beta x \delta z^{-\beta} w \Big] = -\sum_{i=1}^n \left( p^{\beta (1-2i)} x_{i, k} \left( \sum_{k < n+1 - i} c_{k, n+1 -i} + \sum_{k > n+1-i} p^{\beta} c_{k, n+1 - i} \right) \right) . 
		\]
	...
	\begin{corollary}\label{cor:average B}
		...
		\[
		\zeta(s,(u^{-1} t_p^{\beta}) \cdot W, \chi) = \eta(\det z^\beta) p^{n^2 \beta (s-1/2)} \chi(\operatorname{det} \gamma) \int_{M_\beta'} \psi\Big[ \mathrm{tr}(-p^{-n \beta} z^\beta x \gamma z^{-\beta} w ) \Big] \cdot I(x) dx.
		\]
	\end{corollary}
	\begin{proof}
	...
	\begin{lemma}\label{lem:average A}
		We have 
		\[
		\chi(\operatorname{det}\gamma)\psi\Big[ \mathrm{tr}(-p^{-n \beta} z^\beta x \gamma z^{-\beta} w ) \Big] = \prod_{i=1}^n \chi( c_{i, i} )\psi\left( - p^{-(2\beta(n-i) + \beta)} x_{n+1 -i, i} c_{i,i} \right).
		\]
		Therefore
		\begin{align*} 
			\int_{\gamma \in A} \chi(\operatorname{det}\gamma)&\psi\Big[ \operatorname{tr}(-p^{-n \beta} z^\beta x \gamma z^{-\beta} w ) \Big] d^{\times}A\\
			 &= \prod_{i=1}^n \int_{\mathbb{Z}_p^{\times}}  \chi( c_{i, i} )\psi\left( - p^{-(2\beta(n-i) + \beta)} x_{n+1 -i, i} c_{i,i} \right) d^{\times} c_{i, i} \\
			&= \left\{ \begin{array}{cc} \frac{\tau(\chi)^n}{p^{n\beta}(1-p^{-1})^n} \prod_{i=1}^n \chi(x'_{n+1-i, i})^{-1} & \text{ if } x_{n+1-i, i} \in p^{2\beta(n-i)} \mathbb{Z}_p^{\times} \ \forall i \\ 0 & \text{ otherwise } \end{array} \right.
		\end{align*}
	...
	\begin{proof}
		...
		and
		\begin{align*} 
			\psi\Big[\operatorname{tr}(-p^{-n \beta} z^\beta x \gamma z^{-\beta} w ) \Big] &= \prod_{i=1}^n \psi\left( - p^{\beta(1-2i)} x_{i, n+1 - i} c_{n+1-i,n+1 -i} \right) \\
	...
	Let $M_{\beta} \subset M_\beta'$ be the subset where $x_{n+1-i, i} \in p^{2\beta(n-i)} \mathbb{Z}_p^{\times}$ for all $i$. Note that 
	\[
		M_{\beta} = w_n z^{2\beta} \operatorname{Iw}_n(p^{\beta}).
	\]
	
	\begin{corollary}\label{cor:average A}
		We have 
		\[
		\zeta(s,(u^{-1}t_p^\beta) \cdot W, \chi) = \eta(\det z^\beta) p^{n^2 \beta (s-1/2)} \frac{\tau(\chi)^n}{p^{n\beta}(1-p^{-1})^n} \int_{M_{\beta}} \prod_{i=1}^n \chi(x'_{n+1-i, i})^{-1} I(x) dx.
		\]
	\end{corollary}
	\begin{proof}
	...
	Now using that fact that $M_{\beta} = w z^{2\beta} \operatorname{Iw}_n(p^{\beta})$ we write $x = wz^{2\beta} x''$ and note that $\operatorname{det}x'' \equiv \prod_{i=1}^n x'_{n+1-i, i}$ modulo $p^\beta$. Making the change of variables, we see that
	\begin{align}
...
Finally note that 
	...
	\operatorname{Vol}(\Iwn(p^\beta)) = p^{-(\beta-1)\tfrac{n^2-n}{2}} \operatorname{Vol}(\operatorname{Iw}_n).
	\end{equation}
	Putting \eqref{eq:vol Iw} and \eqref{eq:W value} into Corollary \ref{cor:average A} completes the proof of Proposition \ref{prop:local zeta p}.
\end{proof}
	...
	\begin{quote}
		\emph{For which $p$-refinements $\tilde\pi$ does there exist $W \in \cS_{\psi}^\eta(\tilde\pi)$ with $\zeta(s,(u^{-1}t_p^\beta)\cdot W,\chi) \neq 0$?}
	\end{quote}
 ...
 In this section, we begin to make this assertion rigorous. We define `spin' $p$-refinements as those `that come from $\mathrm{GSpin}_{2n+1}$', made precise in Proposition \ref{prop:spin refinement}. In later sections we will show that spin $p$-refinements contain eigenvectors $W$ such that $W\smallmatrd{w_nz^{2\beta}}{}{}{1} \neq 0$. Our key application of spin $p$-refinements will ultimately be summarised in Corollary \ref{cor:L-value}.
	
	
	\begin{notation}
		Let $\cG \defeq \mathrm{GSpin}_{2n+1}$, where we take the split form as in \cite{AS06}. Since it admits a Shalika model, we know from \cite{AS06} that $\pi$ is the  functorial transfer of a representation $\Pi$ on $\cG(F)$.
	\end{notation}
	...
	We will show $\tilde\pi$ is spin if and only if $\alpha$ factors through an eigensystem occuring in $\Pi^{\Iwahori_{\cG}}$, for $I_{\cG}$ the Iwahori subgroup of $\cG$ (see \S\ref{sec:spin via gspin}).


	\subsection{Conventions for Shalika models}\label{sec:shalika conventions}
	In the next two sections, there will be \emph{two} equivalent but competing conventions for Shalika models -- those of Ash--Ginzburg from \cite{AG94}, and of Asgari--Shahidi from \cite{AS06}. For clarity, we highlight both here. In both cases the set $\{1,...,2n\}$ is partitioned into pairs, and:
	\begin{itemize}\setlength{\itemsep}{0pt}
		\item[--] Ash--Ginzburg identify $i$ and $n+i$, i.e.\ $\{1,n+1\}, \{2,n+2\}, \dots, \{n,2n\}.$ This is used in \cite[\S1]{AG94} to give explicit intertwining operators from $\pi$ into its Shalika model (see \S\ref{sec:AG}).
		\item[--] Asgari--Shahidi identify $i$ and $2n+1-i$, i.e.\  $\{1,2n\}, \{2,2n-1\}, \dots, \{n,n+1\}.$ This interacts better with $\mathrm{GSpin}_{2n+1}$, and hence is more natural when discussing spin $p$-refinements.
	\end{itemize}
	These normalisations are interchanged by the element $\tau = \smallmatrd{1_n}{}{}{w_n} \in \cW_G$, recalling $w_n \in \cW_n$ is the longest Weyl element (that interchanges $[n+1 \leftrightarrow 2n], [n+2\leftrightarrow 2n-1]$, etc.).

	As $\pi$ is spherical, it can be written as a (normalised) induction from the upper-triangular Borel, i.e.\ there exists an unramified character $\UPS : T(F) \to \C^\times$ such that	$\pi = \Ind_B^G \UPS$. 
	
	
	Since each normalisation has its own advantages and disadvantages, we will use each in their favoured setting, and carefully track the differences between them (as e.g.\ in Remark \ref{rem:AG vs AS}). In particular:
\begin{itemize}\setlength{\itemsep}{0pt}
\item[--] For the root systems and spin refinements in the present section, we adopt Asgari--Shahidi's normalisation.
\item[--] For convenience in \S\ref{sec:local zeta integrals}, for the rest of the paper we \emph{always} adopt the Ash--Ginzburg normalisation on $\UPS$, i.e.:
	\end{itemize}

	\begin{proposition}\cite[Prop.\ 1.3]{AG94}. \label{prop:AG 1} $\pi$ admits an $(\eta,\psi)$-Shalika model if and only if we may normalise $\UPS$ so that $\UPS_{i}\UPS_{n+i} = \eta$. 	
	\end{proposition}

	
%When $\pi$ admits a Shalika model, the choice of $\UPS$ with $\UPS_i\UPS_{n+i} = \eta$ is far from unique.%; this is also true of $\UPS^\sigma$, for any element $\sigma$ in a subgroup $\cW_G^0 \leq \cW_G$ that we define below in \eqref{eq:W_G^0}. 
	...
	\]
	Write $\langle-,-\rangle_G$ for the natural pairing on $X \times X^\vee$. The corresponding root system is $A_{2n-1}$, with roots $R = \{\pm(e_i - e_j) : 1 \leq i < j \leq 2n\}$. The Weyl group $\cW_G = \mathrm{S}_{2n}$ acts by permuting the $e_i$, with longest Weyl element $w_{2n}^G$ the permutation that sends $e_i \mapsto e_{2n+1-i}$ for all $i$.  
	...
	\begin{proposition}
		The root system for $\cG$ is $(\cX, \cR, \cX^\vee, \cR^\vee)$, where%algebraic characters and cocharacters of $\cT$ are given by
		\[
	...
	\subsection{The maps $\jmath$ and $\jmath^\vee$}
	...
	\begin{proposition}\label{prop:weyl transfer}
		...
		\begin{itemize}\setlength{\itemsep}{0pt}
			\item[(i)] $\jmath$ induces an isomorphism $\cW_{\cG} \cong \cW_G^0 \subset \cW_G$;
		%	\item[(ii)] on longest elements we have $\jmath(w_{2n}^{\cG}) = w_{2n}^G$;
			\item[(ii)] for all  $\sigma \in \cW_{\cG}$ and $\mu \in \cX$, we have $\jmath(\mu^\sigma) = \jmath(\mu)^{\jmath(\sigma)}$.
		\end{itemize}
	\end{proposition}
	\begin{proof}
		We know $\cW_{\cG}$ is generated by permutations and sign changes in $\{f_1,...,f_n\}$, and $\cW_G$ is permutations in $\{e_1,...,e_{2n}\}$. If $\sigma \in S_n \subset \cW_{\cG}$ is a permutation, then define
		\[
		\jmath(\sigma) : (e_1,...,e_n,e_{n+1},...,e_{2n}) \mapsto \left(e_{\sigma(1)}, ..., e_{\sigma(n)}, e_{2n-\sigma(n)+1}, ..., e_{2n-\sigma(1)+1}\right),
		\]
	...
	\begin{equation}\label{eq:cocharacter}
		\big[\nu_{r} : x \mapsto (e_1^* + \cdots + e_r^*)(x)\big] \in X^\vee,
	\end{equation}
	...
	\]
	for each $0 \leq s \leq n-1$. From the definitions, we obtain:
	...
	\text{\emph{From now on, we assume that the Satake parameter of $\pi$ is regular semisimple.}}
	\end{equation}
	 We now show in this case that if $(\pi,\alpha)$ is a spin refinement, then $\alpha^{\cG}$ occurs in $\Pi$.
	
	Recall in \S\ref{sec:shalika conventions} we fixed an unramified character $\theta$ of $T(\Qp)$ such that $\pi = \Ind_B^G \UPS$ (cf.\ \cite[(43)]{DJR18}, where $\UPS$ is $|\cdot|^{(2n-1)/2}\lambda$). Recall $\tau = \smallmatrd{1_n}{}{}{w_n} \in \cW_G$. From \cite[p.177(i)]{AS06} and \cite[Prop.\ 5.1]{AS14}, we see:
 ...
 The $\tau$ is necessary as we are using the normalisation of Ash--Ginzburg (see \S\ref{sec:shalika conventions}). From this, we see that $\UPS^\tau$ is a more natural normalisation for GSpin computations. If we normalised $\UPS$ as in Asgari--Shahidi, we could remove $\tau$ from (ii) and henceforth in this section.
	\end{remark}
	

	
By Remark \ref{rem:conjugate}(ii) and \eqref{eq:satake}, our fixed choice of $\UPS$ (hence $\UPS^\tau$)  fixes a bijection 
	\[
	...
	\end{lemma}
	\begin{proof}
		Conjugating Proposition \ref{prop:AG 1} by $\tau$, we see for each $i$, we have $\UPS_i^\tau \cdot \UPS_{2n+1-i}^\tau = \eta$ as characters of $F^\times$. Let $\sigma = \Delta_{\UPS^\tau}(\tilde\pi)$. By definition of $\cW_G^0$, we see if $\sigma \in \cW_G^0$ then 
		\begin{equation}\label{eq:sigma}
			\UPS_{\sigma(i)}^\tau \cdot \UPS_{\sigma(2n+1-i)}^\tau = \eta,
		\end{equation}
		whilst as the Satake parameter is regular semisimple, if $\sigma \notin \cW_G^0$, then \eqref{eq:sigma} fails for some $i$. Thus \eqref{eq:sigma} holds for all $i$ if and only if $\sigma \in \cW_G^0$.
...
The following is our main motivation for the definition of spin $p$-refinement.
	...
	\begin{proposition}\label{prop:spin refinement}
		Suppose the Satake parameter of $\pi$ is regular semisimple, and let $\tilde\pi = (\pi,\alpha)$ be a $p$-refinement. Then $\tilde\pi$ is a spin $p$-refinement if and only if there exists a $p$-refinement $(\Pi,\alpha^{\cG})$ of $\Pi$ such that $\alpha = \alpha^{\cG} \circ \jmath^\vee$ as characters $\cH_{p} \to \overline{\Q}$.
	\end{proposition}
	...
	We now define another class of $p$-refinements. Let $\tilde\pi = (\pi,\alpha)$ be a $p$-refinement. Recall we write $f \in \tilde\pi$ as shorthand for $f \in \pi_p^{\Iw} [\![U_{p,r} - \alpha(U_{p,r}) : 1 \leq r \leq 2n-1]\!]$ (and similarly for $W \in \cS_\psi^\eta(\tilde\pi)$). Recall that $z = \mathrm{diag}(p^{n-1},p^{n-2},...,p,1)$.
	...
	Note that via Proposition \ref{prop:local zeta p}, this implies that the local zeta integral $\zeta(s,  (u^{-1}t_{\pri,n}^\beta)\cdot W, \chi)$ is non-vanishing for any smooth character $\chi : \Qp^\times \to \C^\times$ of conductor $p^\beta$. Accordingly this generalises the condition \cite[Cond.\ 2.9(C2)]{BDW20} required to construct a $p$-adic $L$-function for $\tilde\pi$ via the methods of \cite{BDW20}. 
	
	Our main results of this section (Proposition \ref{prop:spin refinements are shalika} and Corollary \ref{cor:spin refinements are shalika}) show that all regular spin $p$-refinements are Shalika $p$-refinements, and precisely compute the value \eqref{eq:shalika refinement}.
	
	\subsection{Explicit $p$-refinements}\label{ss:intertwining} 
	
We now write down an explicit eigenvector in any given $p$-refinement. 

Recall that we fixed an identification $\pi \cong \Ind_B^G\UPS$, for $\UPS$ in the Ash--Ginzburg normalisation (i.e.\ $\UPS_i \UPS_{n+i} = \eta$ for all $i$ as in \S\ref{sec:shalika conventions}). Recall  also that our choice of $\UPS$ fixes a bijection $\Delta_\UPS : \{p\text{-refinements of }\pi\} $ $\isorightarrow \cW_G$ (see Remark \ref{rem:conjugate}(ii)). 

For $\sigma \in \cW_G$, let $\tilde\pi_\sigma = (\pi, \alpha_\sigma) \defeq \Delta^{-1}_\UPS(\sigma)$; then every $p$-refinement is of the form $\tilde\pi_\sigma$ for some $\sigma$. Let
\[
	f_{0}^\sigma \in \Ind_B^G \UPS^\sigma \cong \pi
	\]
	be the unique function that is: 
\begin{itemize}\s
\item Iwahori-invariant, 
\item supported on the big Bruhat cell $B(\Qp)\cdot w_{2n} \cdot \Iw$, and 
\item normalised so that $f_0^\sigma(w_{2n}) = p^{n(n-1)}$. 
\end{itemize}
	...
	\begin{proposition}	\label{prop:W_p eigenvector}
		We have $f_0^\sigma \in \tilde\pi_\sigma$. Further, $f_0^\sigma$ is a Hecke eigenvector.
	\end{proposition}
	
	\begin{proof}
		We must show that for each $r = 1,...,2n-1$, we have $U_{p,r}f_{0}^\sigma = \alpha_\sigma(U_{p,r}) f_{0}^\sigma$.
		...
		\begin{equation}\label{eq:single cosets}
			\Iw t_{p,r} \Iw \bigsqcup_{m \in M_{r,2n-r}(\Zp)/M_{r,2n-r}(p)} \smallmatrd{1_r}{m}{0}{1_{2n-r}} \cdot t_{p,r}\cdot \Iw.
		\end{equation}
		We have Bruhat decomposition 
		\begin{equation}\label{eq:bruhat}
			\GL_n(\Qp) = \bigsqcup_{\rho \in W_{n}} B_n(\Qp)\cdot \rho \cdot \Iwn,
		\end{equation}
		so it suffices to compute $U_{p,r}f_0^\sigma(\rho)$ for $\rho \in \cW_G$. By \eqref{eq:single cosets}, we have
		\[
		U_{p,r}f_0^\sigma(\rho) = \sum_{m \in M_{r,2n-r}(\Zp)/M_{r,2n-r}(p)}f_0^\sigma\left(\rho\smallmatrd{1_r}{m}{0}{1_{2n-r}}t_{p,r}\right).
		\]
		
		\begin{claim}
			$\rho\smallmatrd{1_r}{m}{0}{1_{2n-r}}t_{p,r} \in B(\Qp) \cdot w_{2n} \cdot \Iw$ if and only if $\rho = w_{2n}$ and $m \in M_{r,2n-r}(p)$.		
		\end{claim}
		...
		We return to Proposition \ref{prop:W_p eigenvector}. The claim implies $U_{p,r}f_0^\sigma(\rho) = 0$ unless $\rho = w_{2n}$, and
		\begin{align*}
			U_{p,r}f_0^\sigma(w_{2n}) &= f_0^\sigma\left(w_{2n}t_{p,r}\right) = f_0^\sigma\left((t_{p,r}'w_{2n}\right)\\
			&= \delta_B^{1/2}\theta^\sigma(t_{p,r}') f_0^\sigma(w_{2n}) = (\delta_B^{1/2}\theta^\sigma)^{w_{2n}}(t_{p,r})f_0^\sigma(w_{2n}) = \alpha_\sigma(U_{p,r}) f_0^\sigma(w_{2n}),
		\end{align*}
...
We've written down explicit eigenvectors in $\pi \cong \Ind_B^G \UPS^\sigma$. However, we really want to study the corresponding eigenvectors in the Shalika model of $\pi$. To describe these precisely, we recall results of Ash--Ginzburg, who in \cite{AG94} defined and studied an explicit intertwining of $\Ind_B^G \UPS$ into its Shalika model. This intertwining is the major reason we normalise $\UPS$ as in Ash--Ginzburg (see \S\ref{sec:shalika conventions}).
...
\end{equation}
The map $\cS_\psi^\eta$ converges absolutely only for certain characters $\theta$ (described before \cite[Lem.\ 1.4]{AG94}). For $\theta$ such that it converges, $\cS_\psi^\eta$ is $\GL_{2n}(\Qp)$-equivariant and (by \cite{BFG92}) we have  $\cS_\psi^\eta(f) \in \cS_\psi^\eta(\pi)$. 
For $\sigma \in \cW_G$, let
\begin{equation}\label{eq:P(sigma)}
	P(\sigma) \defeq \prod_{r=1}^n\prod_{s=n+1}^{2n} (\UPS_{\sigma(r)}(p) - \UPS_{\sigma(s)}(p)) \in \C.
\end{equation}
Across Lemmas 1.4--1.6 of \cite{AG94},  Ash--Ginzburg prove:

\begin{proposition}\label{prop:AG 2}
	The function $P(1) \cdot \cS_\psi^\eta(f)$ converges absolutely for all characters $\theta$.  Thus when $P(1) \neq 0$, the map $\cS_\psi^\eta$ defines a non-trivial intertwining $\cS_\psi^\eta : \Ind_B^G \UPS  \hookrightarrow \cS_{\psi}^\eta(\pi)$.
\end{proposition}


Recall $\tilde\pi_\sigma = \Delta_\theta^{-1}(\sigma)$. By Lemma \ref{lem:Shalika W_G^0}, $\tilde\pi_\tau$ is a spin $p$-refinement. For later use, we record:

\begin{lemma}\label{lem:regular implies intertwining}
	If the Satake parameter of $\pi_p$ is regular semisimple, then $\cS_\psi^\eta$ defines a non-trivial intertwining  $\cS_\psi^\eta : \Ind_B^G \UPS  \hookrightarrow \cS_{\psi}^\eta(\pi)$. 
\end{lemma}
\begin{proof}
	Since the Satake parameter is regular semisimple, all the $\UPS_i(p)$ are pairwise distinct, so $\UPS_{r}(p)-\UPS_{s}(p) \neq 0$ for all $r \neq s$. We see $P(1) \neq 0$, and conclude by Proposition \ref{prop:AG 2}.
\end{proof}
	...
	\subsection{Spin $p$-refinements under intertwining maps} For  $\UPS$  unramified, 
	we have  shown:
	\begin{itemize}\setlength{\itemsep}{0pt}
		
		\item[--]  for any $\sigma\in \cW_G$, an Iwahori-invariant function in $\pi_\sigma=\Ind_B^G\UPS^\sigma$ supported only on the big Bruhat cell is always a Hecke eigenvector in the generalised eigenspace for $\tilde\pi_\sigma$. 
		
		\item[--] for $\sigma \in \cW_G^0$, when $\tilde\pi_\sigma$ is regular we have an explicit intertwining $\Ind_B^G\UPS^\sigma \to \cS_\psi^\eta(\pi)$.
		
		\item[--] by Lemma \ref{lem:Shalika W_G^0},	$\tilde\pi_\sigma$ is a spin $p$-refinement if and only if $\sigma \in \cW_G^0 \tau$. 
	\end{itemize}
	For $n>1$, the last two cases are disjoint: there is a gap between normalisations where we can write down explicit eigenvectors in the principal series, and normalisations where we have explicit intertwinings into the Shalika model. We now bridge this gap: for $\sigma \in \cW_G^0 \tau$, we transfer the explict eigenvector from $\Ind_B^G\UPS^\sigma$ into $\Ind_B^G\theta$, where we can then compute its local zeta integral via the Ash--Ginzburg intertwining.
	
	Given $\sigma=\smallmatrd{1}{}{}{\rho}\in \cW_{2n}$ with $\rho\in \cW_n$,  there is an isomorphism 
	\[
	M_\rho: \Ind_B^G \UPS^\sigma \longrightarrow \Ind_B^G \UPS, 
	\]
	which is unique up-to-scalar  by Schur's Lemma. 
	We are most interested in  
	\[M_{w_n} : \Ind_B^G \UPS^\tau \to \Ind_B^G \UPS.\]
	
	\begin{definition}\label{def:F_w}
	For each $w, \nu \in \cW_G$:
\begin{itemize}\s
\item Let $f_w^\nu\in \Ind_B^G \UPS^\nu$ be the
	unique $\Iw$-invariant function supported on $B(\Qp) w \Iw$ such that
	$f_w(w)=p^{n(n-1)}$. 
	
\item For $\rho \in \cW_n$ (and $\sigma$ as above), let  
	\[
F_{\rho}^\nu \defeq f_{\smallmatrd{}{w_n}{\rho w_n}{}}^\nu =f_{\sigma w_{2n}}^\nu.
\] 
\item If $\nu = 1$, we drop the superscript and just write $F_w = F_w^1$. Note that in  this notation $F_{1_n}^\nu=f_{w_{2n}}^\nu$ is  $f_0^\nu$ in the above notation. 
\end{itemize}
For any $\delta \in \cW_n$, let $\ell(\delta)$ be its Bruhat length.	
\end{definition}

	\begin{lemma} \label{prop:intertwining}
		After possibly renormalising $M_{\rho}$, we have
		\begin{equation}\label{eq:intertwining}
			M_{\rho} (f_0^\sigma) = F_{\rho} +  	\sum_{ \ell(\delta)<\ell(\rho)}c_{\delta} F_{\delta}, \text{ with } c_\delta \in \C. 
		\end{equation}
	\end{lemma}
	
	
	\begin{proof} 
		For any simple reflection $s\in \cW_n$ and $\nu \in \cW_n$, we have an intertwining isomorphism
		\[
		M_{s}^\nu : \Ind_B^G \UPS^\nu \longrightarrow \Ind_B^G \UPS^{s \nu}.
		\]
		...
		\[
		M_{s}^\nu(F_\delta^\nu)= \begin{cases} F_{s \delta}^{s \nu} +c_{\delta, s}F_{\delta}^{s \nu}  &, \text{ if } \ell(s\delta)=\ell(\delta)+1, \\ 
			p^{-1} F_{s \delta}^{s \nu} +c_{\delta, s}F_{\delta}^{s\nu}   &, \text{ if } \ell(s\delta)=\ell(\delta)-1.\end{cases}
		\]
		
		Writing $\rho = s_1\cdots s_r$,  $M_{\rho}$ is the composition $M_{s_r}^{s_{r-1}\cdots s_1 \rho} \circ\cdots \circ M_{s_1}^\rho$. The lemma is then obtained by induction on $\ell(\rho)$, using the basic properties of the Bruhat length. 
	\end{proof}


Now finally we map into the Shalika model. For $\delta \in \cW_n$, let 
\[
W_\delta \defeq \cS_\psi^\eta(F_\delta) \in \cS_\psi^\eta(\pi),
\]
for $\cS_\psi^\eta$ the intertwining of \eqref{eq:AG}. This is well-defined, since $F_\delta \in \Ind_B^G\UPS$ and we normalised $\UPS$ as in Ash--Ginzburg.

\begin{corollary}\label{cor:spin eigenvector}
 Let $\tilde\pi$ be a regular spin $p$-refinement. Up to possibly renormalising $\UPS$ by $\tau^{-1} \cW_G^0\tau$ (so that it still satisfies the Ash--Ginzburg condition), $\cS_\psi^\eta(\tilde\pi)$ is spanned by an eigenvector of the form
\begin{equation}\label{eq:spin eigenvector}
	W_0 = W_{w_n} + \sum_{w_n \neq \delta \in \cW_n} c_\delta W_\delta \in \cS_\psi^\eta(\Ind_B^G \UPS).
\end{equation}
\end{corollary}
	\begin{proof}
Let $\sigma = \Delta_\UPS(\tilde\pi)$; then by Lemma \ref {lem:Shalika W_G^0} we have $\sigma = \sigma'\tau \in \cW_G^0\tau$. After renormalising $\UPS$ by $\tau^{-1}(\sigma')^{-1}\tau$, which preserves the Ash--Ginzburg normalisation of \S\ref{sec:shalika conventions}, we may without loss of generality assume $\sigma' = 1$, so $\sigma = \tau$.

By Proposition \ref{prop:W_p eigenvector}, we know $f_0^\tau \in \Ind_B^G\UPS^\tau$ is an eigenvector in $\tilde\pi_\tau$. By Lemma \ref{prop:intertwining}, its image in $\Ind_B^G$ under the intertwining $M_\tau$ has the form
\[
	F_{w_n} + \sum_{w_n \neq \delta \in \cW_n} c_\delta F_\delta \in \Ind_B^G \UPS.
\]
By definition the image of this under $\cS_\psi^\eta$ is \eqref{eq:spin eigenvector}, which hence gives a non-zero eigenvector in $\cS_\psi^\eta(\tilde\pi)$. By regularity this space is a line, so this eigenvector spans.
\end{proof}


\subsection{The local zeta integral at Iwahori level, II}\label{sec:local zeta integrals}

From Corollary \ref{cor:spin eigenvector} we've written down an explicit eigenvector $W_0 \in \cS_\psi^\eta(\pi)$ in any spin $p$-refinement. We now show $W_0\smallmatrd{w_nz^{2\beta}}{}{}{1} \neq 0$, recalling $z = \operatorname{diag}(p^{n-1}, p^{n-2}, \dots, 1),$ and deduce that all spin $p$-refinements are Shalika $p$-refinements. Moreover, we compute the value exactly, and hence -- via Proposition \ref{prop:local zeta p} -- complete the computation of the local zeta integral for $W_0$. 
...
where
\[
A,D \in B_n(\Qp), \ \ B \in M_n(\Qp), \ \ a,d \in I_n, \ \ c \in pM_n(\Zp), \ \ b \in M_n(\Zp).
\]
...
\begin{enumerate}[(1)]
    ...
    \item $-Aw_n (c a^{-1} b - d ) = k$ ((1) and top right), whence $A \in B_n(\mathbb{Z}_p)$ and 
    \[
    k \in B_n(\mathbb{Z}_p) \cdot w_n \cdot I_n.
    \]
...
\[
B_n(\mathbb{Q}_p) \cdot \delta w_n \cdot \overline{J}_r \cap \; B_n(\mathbb{Q}_p) \cdot \overline{J}_r = \varnothing.
\]
\end{claim}
...
\]
Note that the valuation of $\mu$ under any positive root is non-negative, so $\mu^{-1} \overline{N}_n(\mathbb{Z}_p) \mu \subset \overline{N}_n(\mathbb{Z}_p)$, where $\overline{N}_n$ is the lower triangular unipotent. 
...
with $\alpha \in B_n(\mathbb{Z}_p)$ and $\beta \in \operatorname{Iw}_n$. We have
\[
k w_n \mu = \alpha w_n \beta w_n \mu \in B_n(\mathbb{Z}_p) \cdot \overline{\operatorname{Iw}}_n \cdot \mu .
\]
But by the Iwahori decomposition, we see that
\[
B_n(\mathbb{Z}_p) \cdot \overline{\operatorname{Iw}}_n \cdot \mu \subset B_n(\mathbb{Q}_p) \cdot \overline{\operatorname{Iw}}_n \subset B_n(\mathbb{Q}_p) \cdot \overline{J}_r.
\]
We also have
\[
kw_n\mu = D \delta w_n a t_{p,r}^{-1} \in B_n(\mathbb{Q}_p) \cdot \delta w_n t_{p,r}^{-1} \cdot  \overline{J}_r \subset B_n(\mathbb{Q}_p) \cdot \delta w_n \cdot \overline{J}_r
\]
as the element $\delta w_n$ normalises the torus.

We must therefore have
\[
kw_n\mu \in \left( B_n(\mathbb{Q}_p) \cdot \delta w_n \cdot \overline{J}_r \right) \cap \left( B_n(\mathbb{Q}_p) \cdot \overline{J}_r \right) \neq \varnothing,
\] 
...
Conversely, suppose \eqref{eq:k and X}, and write $k = A w_n d$, with $A \in B_n(\Zp)$ and $d \in \Iwn$. Via the Iwahori decomposition, we may assume $d \in N_n(\Zp)$ is upper unipotent. 
    ...
    For $A$ and $d$ as above, set $B = 0$ and $D = A z^{2\beta} \in B_n(\mathbb{Q}_p)$. Also set $c = 0$ and $a = z^{-2\beta} w_n d w_n z^{2\beta} \in \operatorname{Iw}_n$ (since $\beta \geq 1$). If we set $b = D^{-1}Xk \in M_n(\Qp)$, then \eqref{eq:support equality} holds. Clearly $\smallmatrd{A}{B}{0}{D} \in B_n(\Qp)$, so we are done if we can show $\smallmatrd{a}{b}{c}{d} \in \Iw$. This will hold if $b \in M_n(\Zp)$. Observe
    \begin{align*}
...
We now put this all together. Recall that (without loss of generality) we have normalised $\pi \cong \Ind_B^G \UPS$, fixing an identification $\Delta_\UPS : \{\text{$p$-refinements}\} \isorightarrow \cW_G$ so that for our fixed spin $p$-refinement $\tilde\pi$, we have $\Delta_\UPS(\tilde\pi) = \tau = \smallmatrd{1}{}{}{w_n}$. By Proposition \ref{prop:p-refinement}, the Hecke eigenvalue of $U_{p,r}$ on $\tilde\pi$ is
\[
	\alpha_{p,r} = \Big[\delta_B^{1/2}\UPS^\tau\Big]^{w_{2n}}(t_{p,r}).
\]
Note also that since $\tilde\pi$ is a spin $p$-refinement, by Definition \ref{def:shalika refinement} we have $\alpha_{p,n+s} = \eta(p)^s \alpha_{p,n-s}$  for $0 \leq s \leq n-1$. We also have the $U_p$-eigenvalue $\alpha_p = \alpha_{p,1} \cdots \alpha_{p,2n-1}$.
...
\begin{proposition}\label{prop:spin refinements are shalika}
	\begin{enumerate}[(i)]
		...
		\end{align*}
	where $\Upsilon'' = \vol\big[B_n(\mathbb{Z}_p) \cdot w_n \cdot \operatorname{Iw}_n\big]\cdot p^{n(n-1)}$ is a constant independent of $\beta$.
	\end{enumerate}
...
\begin{proof}
	...
	\begin{align*}
		W_\delta&\smallmatrd{w_nz^{2\beta}}{}{}{1} = \cS_{\psi}^\eta(F_\delta)\smallmatrd{wz^{2\beta}}{}{}{1}\\
		&=  \int_{\GL_n(\Zp))}\int_{M_n(\Qp)} F_{w_n}\left[ \smallmatrd{}{1}{1}{}\smallmatrd{1}{X}{}{1}\smallmatrd{k}{}{}{k}\smallmatrd{wz^{2\beta}}{}{}{1}\right] \psi^{-1}(\mathrm{tr}(X))\eta^{-1}(\det(k)) dXdk.
	\end{align*}
	...
	\begin{itemize}
		...
		\begin{align*}
			...
			&=  (\delta_B^{1/2}\UPS)\Big[\smallmatrd{1}{}{}{z^{2\beta}}\Big]p^{n(n-1)},
		\end{align*}
	...
	\begin{align}\notag
		W_{w_n}&\smallmatrd{w_nz^{2\beta}}{}{}{1} = (\delta_B^{1/2}\UPS)\Big[\smallmatrd{1}{}{}{z^{2\beta}}\Big]p^{n(n-1)} \cdot \int_{B_n(\Zp)w_n\Iwn}\int_{kw_nz^{2\beta}M_n(\Zp)} dXdk\\
		&= (\delta_B^{1/2}\UPS)\Big[\smallmatrd{1}{}{}{z^{2\beta}}\Big]p^{n(n-1)}\cdot \operatorname{Vol}\Big(B_n(\mathbb{Z}_p) \cdot w_n \cdot \operatorname{Iw}_n \Big) \operatorname{Vol}\Big(z^{2\beta}M_n(\zp)\Big),\label{eq:F_1 value 1}
	\end{align}
	...
	\begin{align}
		(\delta_B^{1/2}\theta)\Big[\smallmatrd{1}{}{}{z^{2\beta}}\Big] &= 
		 \delta_B^{1/2}\Big[\smallmatrd{1}{}{}{z^{2\beta}}\Big] (\theta^\tau)^{w_{2n}}\Big[t_{p,1}^2 \cdots t_{p,n-1}^2\Big]\notag \\
		 &= 	 \delta_B^{1/2}\Big[\smallmatrd{1}{}{}{z^{2\beta}}\Big](\delta_B^{-1/2})^{w_{2n}}\Big[t_{p,1}^2\cdots t_{p,n-1}^2\Big] \alpha_{p,1}^{2\beta} \cdots \alpha_{p,n-1}^{2\beta}\notag\\
		 &= \eta(\det z^\beta)^{-1} \delta_B\Big[\smallmatrd{z^\beta}{}{}{z^{\beta}}\Big]\frac{\alpha_p^\beta}{\alpha_{p,n}^\beta}.\label{eq:F_1 value 3}
	\end{align}
Here in the second equality we use that $\delta_B^{w_{2n}} = \delta_B^{-1}$, and in the third that $\eta(p)\cdot \eta(p)^2 \cdots \eta(p)^{n-1} = \eta(\det z)$ and $\alpha_{p,1}\cdots\alpha_{p,n-1}\cdot\alpha_{p,n+1}\cdots\alpha_{p,2n-1} = \alpha_p/\alpha_{p,n}.$ Finally, we obtain (ii) after combining \eqref{eq:F_1 value 1}, \eqref {eq:F_1 value 2} and \eqref{eq:F_1 value 3}, as
\[
...
\end{corollary}
\begin{proof}
In Proposition \ref{prop:spin refinements are shalika}, we started with an arbitrary regular spin $p$-refinement $\tilde\pi$, and exhibited in (ii) an eigenvector $W_0 \in \cS_\psi^\eta(\tilde\pi)$ with $W_0\smallmatrd{wz^{2\beta}}{}{}{1}\neq 0$. By definition, $\tilde\pi$ is thus a Shalika refinement.
\end{proof}



%\subsection{A conjectural converse}

%\CWnote{Sharpen this up}

%The following observations will not be used later in the paper, but we include them for completeness. We have shown that every regular spin $p$-refinement is a Shalika $p$-refinement. We expect the converse to hold:

%\begin{conjecture}
%	The definition of spin and Shalika $p$-refinements are equivalent. 
%\end{conjecture}

%We comment a little on why this would follow from a folklore conjecture on the properties of eigenvarieties. We will use non-vanishing to show in the sequel that the $\GL_{2n}$-eigenvariety is \'etale over the pure weight space at suitable Shalika $p$-refined RASCARs $\tilde\pi$. This implies that $\tilde\pi$ varies in a classical family of RASCARs. It is expected (see e.g.\ \cite[Intro.]{Urb11}) that such families should only arise as a functorial transfer from an eigenvariety for a group with discrete series, in this case $\cG = \mathrm{GSpin}_{2n+1}$. But by definition, every point in such a transfer would correspond to a spin $p$-refinement; so Shalika $p$-refinements should be spin $p$-refinements. We hope to prove the conjecture, using this kind of argument, in a sequel paper.
	...
	\subsection{Proof of Proposition \ref{prop:local zeta p}}
		The proof will occupy the entirety of this subsection. First:
		
			\begin{lemma} \label{SupportLemma}
			Suppose $W \in \mathcal{S}^{\eta}_{\psi}(\pi_p)$ is fixed under the action of $\operatorname{Iw}_G$. Then the function
			\[
			\GL_n(\Q_p) \to \C,\qquad 
			x \mapsto W\smallmatrd{x}{}{}{1}
			\]
		...
		\end{proof}
			...
			Now let $(e_{ij})_{i,j=1,\dots, n}$ denote the standard basis of $M_{n}(\Z_p)$. We will consider a certain collection of elements in $\operatorname{Iw}_G$ and use the invariance of $W$ under $\operatorname{Iw}_G$ to prove  Proposition \ref{prop:local zeta p}. 
			...
			First, note $\smallmatrd{x}{}{}{1} u^{-1}t_{p,n}^{\beta} = \smallmatrd{1}{-x w_n }{}{1} \smallmatrd{p^{\beta} x}{}{}{1}$; thus by the Shalika transformation property
			\[ 
			\zeta\Big(s, (u^{-1} t_{p,n}^{\beta}) \cdot W, \chi\Big) = \int_{\GL_n(\Q_p)} \psi\Big(\mathrm{tr}( x w_n  ) \Big)^{-1} W\smallmatrd{p^{\beta} x}{}{}{1} \chi(\operatorname{det}x) |\operatorname{det}x|^{s-1/2} dx.
			\]
			Changing variables $x \mapsto p^{-\beta}xw_n$, recalling $\chi(p) = 1$, using $|p| = 1/p$ and $|w_n| = 1$, and using that $dx$ is a left and right Haar measure, we get
			\begin{equation}\label{eq:I(x)}
				\zeta\Big(s, (u^{-1} t_{p,n}^{\beta}) \cdot W, \chi\Big) = p^{n \beta (s-1/2)}\chi(\det w_n) \chi(p^{-\beta}) \int_{\GL_n(\Q_p)} I(x) dx, 
			\end{equation}
			where
			\begin{equation}\label{eq:I(x) def}
				I(x) = \psi(\mathrm{tr}( p^{-\beta} x) )^{-1} W\smallmatrd{xw_n}{}{}{1} \chi(\operatorname{det}x) |\operatorname{det}x|^{s-1/2}.
			\end{equation}
			Whilst this function depends on $s \in \C$, we fix $s$ throughout, so drop $s$ from the notation. Combining \eqref{eq:local zeta unchanged} with \eqref{eq:I(x)}, we get
			\begin{equation}\label{eq:int I equality}
				\int_{\GL_n(\Qp)} I(x) dx = \int_{\GL_n(\Qp)} I(x\cdot \alpha_{ij}(c)) dx.
			\end{equation}
			By Lemma \ref{SupportLemma}, the function $W\smallmatrd{x}{}{}{1}$ (hence $I(x)$) is supported on $M_n(\Z_p) \cap \GL_n(\Q_p)$, so 
			\begin{equation}\label{eq:int I equality integral}
				\int_{M_n(\Zp) \cap \GL_n(\Qp)} I(x) dx = \int_{M_n(\Zp) \cap \GL_n(\Qp)} I(x\cdot \alpha_{ij}(c)) dx.
			\end{equation}
			
			\textbf{Case 1:} Suppose that $c \in \Z_p$ and $1 \leq j < i \leq n$ (so $\alpha_{ij}(c)$ is \emph{lower} triangular). We have the identity
			\begin{equation} 
				\psi(\mathrm{tr}( p^{-\beta}  x \alpha_{ij}(c) ) ) = \psi(\mathrm{tr}(p^{-\beta}  x )) \psi(c p^{-\beta} x_{j, i}), \label{PsiTraceEqn}
			\end{equation}
			where $x_{j, i}$ is the $(j, i)$-th entry of the matrix. Note also that $W\smallmatrd{x\alpha_{ij}(c)w_n}{}{}{1} = W\smallmatrd{xw_n\alpha_{ji}(c)}{}{}{1} = W\smallmatrd{xw_n}{}{}{1},$ as $\alpha_{ji}(c)$ is upper-triangular (hence lies in $\Iwn$).  As $\chi|\cdot|^{s-1/2}(\det x)$ is right-invariant under $\alpha_{ij}(c)$, we deduce
			\[
			I(x \cdot \alpha_{ij}(c)) = \psi(-cp^{-\beta}x_{j,i}) \cdot I(x).
			\]
			For fixed $x \in M_n(\Z_p) \cap \GL_n(\Q_p)$, the character $\Z_p \to \C^{\times},$	$c \mapsto \psi(-cp^{-\beta} x_{j, i})$ factors through $\Z/p^{\beta}\Z$. Therefore, by character orthogonality relations one has
			\begin{equation}\label{eq:character orthogonality}
				\frac{1}{p^{\beta}}\sum_{c \in \Z/p^{\beta}\Z} I(x \cdot \alpha_{ij}(c)) = \left\{ \begin{array}{cl} I(x) & : x_{j, i} \in p^{\beta}\Z_p \\ 0 & :\text{otherwise. }  \end{array} \right.
			\end{equation} 
			Note $x_{j,i}$ is  above the diagonal. Let $\Sigma_{ij} \subset M_n(\Z_p) \cap \GL_n(\Q_p)$ be the subset of matrices $x$ with $x_{j, i} \in p^{\beta} \Z_p$. If $Z \subset \GL_n(\Q_p) \cap M_n(\Zp)$ is a subset preserved under translation by $\alpha_{ij}(c)$ with
			\begin{equation}\label{eq:int_Z}
				\int_{Z} I(x)dx = \int_{Z} I(x\cdot \alpha_{ij}(c))dx
			\end{equation}
			for any $c \in \Z_p$, then summing \eqref{eq:int_Z} over a set of representatives of $\Z/p^{\beta}\Z$, by \eqref{eq:character orthogonality} we see that 
			\begin{equation}\label{eq:intersection Z}
				\int_{Z} I(x) dx = \int_{Z \cap \Sigma_{ij}} I(x) dx.
			\end{equation}
			Set $Z_{1, 1} = M_n(\Zp) \cap \GL_n(\Zp)$ (for which \eqref{eq:int_Z} holds by \eqref{eq:int I equality integral}). We inductively define $Z_{i, j}$, for $1 \leq i < j \leq n$, as 
			\[
			Z_{i, j} = \left\{ \begin{array}{cl} Z_{i, j-1} \cap \Sigma_{i, j} & : i < j-1, \\ Z_{i-1, n} \cap \Sigma_{i, j} & : i = j-1.\end{array} \right.
			\]
			Then $Z_{n-1,n}$ is the subset of matrices in $Z_{1,1}$ where all entries above the diagonal lie in $p^\beta\Zp$. 
			
			Note that for any $c \in \Z_p$ and for $i \neq n-1$,  $Z_{i, j}$ is stable under right-translation by $\alpha_{i, j+1}(c)$ (resp. $\alpha_{i+1, i+2}(c)$) if $j < n$ (resp. $j=n$). Then
			\[
			\int_{Z_{i, j}} I(x)dx = \left\{ \begin{array}{cl} \int_{Z_{i, j}} I(x\alpha_{i, j+1}(c)) dx & : j < n, \\ \int_{Z_{i, j}} I(x\alpha_{i+1, i+2}(c)) dx & :j = n. \end{array} \right.
			\] 
			Thus \eqref{eq:int_Z} holds for $Z = Z_{i,j}$, and \eqref{eq:intersection Z} reduces the support of the integral to $Z_{i,j+1}$ (if $j < n$ or $Z_{i+1,i+2}$ (if $j = n$). Inducting on $i,j$, we conclude that
			\begin{equation}\label{eq:support Z}
				\int_{\GL_n(\Q_p)} I(x) dx = \int_{Z_{n-1, n}} I(x) dx.
			\end{equation}
			
			\textbf{Case 2:} 
			Let $c_1, \dots, c_n \in \Z_p^{\times}$ and $\underline{\alpha} = \alpha_{11}(c_1) \cdots \alpha_{nn}(c_n)$. Note $Z_{n-1, n}$ is stable under right-translation by $\underline{\alpha}$, and $\psi( \mathrm{tr}(p^{-\beta}x\cdot \underline{\alpha}))= \prod_{k=1}^n \psi(p^{-\beta}c_kx_{k,k}),$ so that
			\[
			I(x\underline{\alpha}) = I(x) \cdot \psi\Big(\mathrm{tr}(p^{-\beta}x)\Big) \prod_{k=1}^n \chi\big(c_k\big) \cdot\psi\Big(p^{-\beta}c_kx_{k,k}\Big)^{-1}.
			\]
			Let $d^\times c_1 = \cdots = d^\times c_n$ be the Haar measure on $\Zp^\times$ with total volume 1. Then
			\begin{align*} 
				\int_{ Z_{n-1, n}} I(x) dx &= \int_{Z_{n-1, n}} I(x \underline{\alpha}) dx \\
				& = \int_{(\Zp^\times)^n}\left[\int_{ Z_{n-1, n}} I(x \underline{\alpha}) dx\right]d^\times c_1 \cdots d^\times c_n\\
				& = \int_{Z_{n-1, n}} I(x) \psi(\mathrm{tr}(p^{-\beta} x)) \left[\prod_{k=1}^n \int_{\Z_p^{\times}} \chi( c_k) \psi\Big(-p^{-\beta} c_k x_{k, k} \Big) d^{\times}c_k \right] dx
			\end{align*}
			In the second equality, we use that the integrand is constant as the $c_i$ vary. In the third, we may exchange the order of the integrals  since $c_k \mapsto \chi(c_k)\psi(p^{-\beta} c_k x_{k, k} )^{-1}$ is constant on cosets of the form $a + p^\beta \Z_p$, so each integral over $\Zp^\times$ can be rewritten as a finite sum over $(\Z/p^\beta)^\times$.
			
			The $k$th inner integral is standard: as $\chi$ has conductor $p^{\beta}$, it is zero unless $x_{k, k} \in \Z_p^{\times}$, whence it equals 
			\[
			\frac{	\chi(-p^{-\beta} x_{k,k})^{-1}\tau(\chi)}{p^\beta(1-p^{-1})} = \chi(p^\beta) \frac{\chi(x_{k,k})^{-1}}{\tau(\chi^{-1})(1-p^{-1})}.
			\]
			The subset of $x \in Z_{n-1,n}$ where $x_{k,k} \in \Zp^\times$ for all $k$ is exactly $\Iwn^- = w_n\Iwn w_n$. Making the change of variables $y = w_nx w_n,$ via \eqref{eq:support Z} we have
			\begin{align*}
				\int_{\GL_n(\Qp)} I(x) dx &= \int_{\Iwn(p^\beta)} W\smallmatrd{w_ny}{}{}{1} \chi(\operatorname{det} y) |\operatorname{det} y|^{s-1/2} \left[\prod_{k=1}^n \frac{\chi(p^\beta) \chi(y_{k,k})^{-1}}{\tau(\chi^{-1})(1-p^{-1})} \right] dx\\
				&= W\smallmatrd{w_n}{}{}{1} \chi(p^\beta) \tau(\chi^{-1})^{-n} (1-p^{-1})^{-n} \cdot \operatorname{Vol}(\operatorname{Iw}_{\GL_n}(p^\beta)),
			\end{align*}
			as $|\det y| = 1$ and $\chi(\operatorname{det} y) = \prod_{k=1}^n \chi(y_{k, k})$ as $\chi$ has conductor $p^\beta$. Since $\mathrm{Vol}(\Iwn(p^\beta)) = \operatorname{Vol}(\operatorname{Iw}_{\GL_n})p^{-(\beta-1)(n^2-n)/2}$, we conclude the proof of Proposition \ref{prop:local zeta p} via  \eqref{eq:I(x)}, noting the $\chi(p^\beta)$'s cancel. \qed

		
		\begin{remark}
			This proof is heavily based on \cite[Prop.\ 3.4]{DJR18}, but there are some differences in the statements. Some of these are from the different choice of twisting element (with $u^{-1}$ replacing $\xi$ \emph{op.\ cit}.). We also renormalise to move the Gauss sum to the denominator.
			
			More importantly, the power of $p$ is also slightly different. This arises from a subtle error in the proof of \cite[Prop.\ 3.4]{DJR18}: whilst translations by $\alpha_{ij}(c)$ can reduce the domain to $\Iwn(p^\beta)$, as above, this set is not preserved by $\alpha_{ij}(c)$ for $i > j$, so the argument \emph{op.\ cit}.\ to reduce the domain further to $\mathrm{ker}(\GL_n(\Zp) \to \GL_n(\Z/p^\beta))$ is not valid. This means that \cite[Thm.\ 4.7]{DJR18} is slightly out: the power of $p$ appearing does not agree with the Coates--Perrin-Riou conjecture (as explained in \cite[\S3]{AG94}). The different power of $p$ in Proposition \ref{prop:local zeta p} rectifies this.
		\end{remark}
	...
	\begin{conditions}\label{cond:running assumptions}
		\begin{itemize}\setlength{\itemsep}{0pt}
			...
			\item[(C2)] $\pi_{p}$ is spherical and admits a regular Shalika $p$-refinement $\tilde\pi_{p} = (\pi_{p}, \alpha)$;
	...
	\end{equation}
	as follows: for each $\ell\neq p$, let $W_\ell = W_\ell^\circ$ be the spherical test vector, and at $p$, let $W_p$ be the vector $W_0 \in \cS_{\psi_p}^{\eta_p}(\pi_p,E)$ from Corollary \ref{cor:spin eigenvector}. Define 
	...
	\begin{itemize}\s
		...
		\[
			\cE_{B,\chi}^{j,\eta_0}\left(\phi_{\tilde\pi}^\pm\right) = \delta_B(t_P^{-\beta})\cdot\Upsilon_B\cdot\lambda(t_P^\beta)\cdot \zeta_j(W_\infty^\pm) \cdot \frac{L(\pi\otimes\chi,j+1/2)}{\Omega_\pi^\pm}\cdot \zeta_p\Big(j+\tfrac{1}{2},(u^{-1}t_B^\beta)\cdot W_p,\chi_p\Big).
		\]
		\item In Proposition \ref{prop:local zeta p}, we showed that if $\chi_p$ is ramified,
		\[
		\zeta_p\Big(j+\tfrac{1}{2},(u^{-1}t_p^{\beta}\Big) \cdot W_p, \chi_p) = \Upsilon' \cdot \eta(\det z^\beta) \cdot p^{-\beta \tfrac{n^2+n}{2}} \cdot p^{\beta nj} \cdot \tau(\chi)^n  \cdot \chi(\det w_n)  \cdot W_p\tbyt{w_n z^{2\beta}}{}{}{1}.
		\]
		
		\item In Proposition \ref{prop:spin refinements are shalika}, we showed
		\[
		...
		\]
	\end{itemize}
  Combining all of these, and letting $\gamma_{(pm)} \defeq \Upsilon_P \cdot \Upsilon' \cdot \Upsilon$, we deduce:
	
	\begin{corollary}\label{cor:L-value}
		Let $\chi$ be a Dirichlet character of conductor $p^{\beta},$ with $\beta \in \Z_{\geq 1}$, and let $j \in \mathrm{Crit}(\lambda)$. If $(-1)^j\chi\eta(-1) \neq \pm 1$, then $\cE_{\chi}^{j,\eta_0}(\phi_{\tilde{\pi}}^\pm) = 0$. If $(-1)^j\chi\eta(-1) = \pm 1$, we have
		\[
		\cE_{B,\chi}^{j,\eta_0}\left(\phi_{\tilde\pi}^\pm\right) = \gamma_{(pm)}\cdot\chi(\det w_n) \cdot \left(\frac{\alpha_p^\circ}{\alpha_{p,n}}\right)^\beta \cdot Q'(\pi,\chi,j)  \cdot
		\zetainfty \cdot \frac{L(\pi\otimes\chi,j+1/2)}{\Omega_\pi^\pm},
		\]
		where
		\[
...
\begin{notation}
		Let $F/\Qp$ be a finite extension, with ring of integers $\cO$ and maximal ideal $\pri = \varpi\cO$, and let $q \defeq \#\cO/\pri$. Let $\varpi^\delta\cO$ be the different of $F/\Qp$. 

		Let $\pi$ be a spherical representation of $\GL_{2n}(F)$ admitting an $(\eta,\psi)$-Shalika model, for $\eta : F^\times \to \C^\times$ a smooth character and $\psi : F \to \C^\times$ the usual additive character (e.g.\ \cite[\S4.1]{DJR18}). We will assume that \eqref{eq:AG} gives a non-trivial intertwining $\cS_\psi^\eta : \Ind_B^G \UPS \hookrightarrow \cS_\psi^\eta(\pi)$ for an unramified character $\UPS$ in the Ash--Ginzburg normalisation (cf.\ Lemma \ref{lem:regular implies intertwining}).
%A $p$-refinement $\tilde\pi = (\pi,\alpha)$ is a choice $\alpha$ of Hecke eigensystem occurring in $\pi^{J}$.
%We continue to write $\Iw$ for the Iwahori subgroup of $\GL_{2n}(\cO)$. By replacing $p$ with $\varpi$ in all the definitions of \S\ref{sec:hecke algebra}, we obtain matrices $t_{\pri,r}$ and Hecke operators $U_{\pri,r}$ on $\pi^{\Iw}$, replacing $t_{p,r}$ and $U_{p,r}$. A $p$-refinement $\tilde\pi = (\pi,\alpha)$ is a choice $\alpha$ of Hecke eigensystem occurring in $\pi^{\Iw}$.
		\end{notation}
	
Let $\tilde\pi = (\pi, \alpha)$ be a regular spin $p$-refinement of $\pi$ (to Iwahori level), normalised without loss of generality as in the proof of Corollary \ref{cor:spin eigenvector}. At parahoric level:
\begin{itemize}\s
	\item Rather than $\Iw$-invariant vectors we use $J$-invariant vectors.
	\item Attached to $\tilde\pi$ is a parahoric refinement $\tilde\pi^Q = (\pi,\alpha(U_{\pri,n}))$, which is a $Q$-regular $Q$-refinement in the sense of \cite[Def.\ 3.5]{BDW20}. (In the notation of \cite{DJR18}, it corresponds to the set $\tau = \{n+1,...,2n\}$).
	\item For a vector $W_{\pri} \in \pi_{\pri}^J$, the relevant twisted local zeta integral arising in Theorem \ref{thm:critical value} is $\zeta_{\pri}(j+\tfrac{1}{2}, (u^{-1}t_{\pri}^\beta)\cdot W_{\pri}, \chi_{\pri})$.
\end{itemize}



\begin{definition}
	Let $\cF_0\in \Ind_B^G \UPS$ be the
	unique $J$-invariant function supported on $B(F)\cdot w_{2n}\cdot J$ such that
	$\cF_0(w_{2n})=q^{n(n-1)}$.  By \cite[Lem.\ 3.6]{DJR18}, we have $\cF_0 \in \tilde\pi^Q$ is a generator.
	
	Let $W_0 = \cS_{\psi}^\eta(\cF_0) \in \cS_\psi^\eta(\pi)$ be the Shalika vector associated with $\cF_0$ by \eqref{eq:AG}.
\end{definition}
...
\]
Our normalisation means that if $\chi$ is a Dirichlet character, this recovers the usual Gauss sum \[	
...
\begin{proposition}\label{p:local-zeta}
	...
	=q^{\beta n\left(s-\tfrac{n}{2}\right)+\delta n(s-1)}\prod_{i=n+1}^{2n}
	\UPS_{i}(\varpi)^{-\delta}Q(\pi,\chi,s)
	\]
	where 
	\[
...
\begin{proof}
	...
	\]
	The proof of \cite[Lemma 3.6]{DJR18} implies that this vanishes unless $X\in M_n(\mathcal{O})$, in which case it equals $\displaystyle F_0\left[\begin{pmatrix}
		&1\\1&\end{pmatrix}\right]=q^{n(n-1)}$. The above integral becomes
	\begin{align*}
		&=q^{\beta n(s-1/2)+n(n-1)}\int_{B_n}\int_{K_n}\int_{K_n}\delta_B^{1/2}\theta\left[\begin{pmatrix} 1&\\&b\end{pmatrix}\right]  \chi\Big(\det k^{-1}b\ell\varpi^{-\beta}\Big)\\
	...
	\begin{align*}
		&=q^{\beta n(s-1/2)+n(n-1)}\int_{B_n\cap \varpi^{-\delta}M_n(\mathcal{O})}\int_{K_n}\int_{K_n}\delta_B^{1/2}\theta\left[\begin{pmatrix} 1&\\&b\end{pmatrix}\right]\\
	...
	\begin{align*}
		&=q^{\beta n(s-1/2)+n(n-1)}\int_{B_n\cap \varpi^{-\delta}M_n(\mathcal{O})}\int_{K_n}\delta_B^{1/2}\theta\left[\begin{pmatrix} 1&\\&b\end{pmatrix}\right]\\
	...
	integral becomes
	\begin{align*}
		&=q^{\beta n(s-1/2)+n(n-1)}\sum_{\rho\in W_n}\int_{B_n\cap \varpi^{-\delta}M_n(\mathcal{O})}\int_{N_n(\mathcal{O})}\int_{N_n^{-}(\varpi \mathcal{O})}\int_{B(\mathcal{O})}\delta_B^{1/2}\theta\left[\begin{pmatrix} 1&\\&b\end{pmatrix}\right]\\
	...
	\begin{align*}
		&=q^{\beta n(s-1/2)+n(n-1)}\sum_{\rho\in W_n}\chi(\det\rho w_{n}\varpi^{-\beta})\int_{B_n\cap \varpi^{-\delta}M_n(\mathcal{O})}\int_{N_n^{-}(\varpi \mathcal{O})}\delta_B^{1/2}\theta\left[\begin{pmatrix} 1&\\&b\end{pmatrix}\right]\\
	...
	\begin{align*}
		&=q^{\beta n(s-1/2)+n(n-1)}\chi(\det w_{n})\int\cdots\int\prod_{i=1}^n\theta_{n+i}|\cdot|^{s-1}(\varpi^{-\delta}t_i)\chi(\varpi^{-\beta-\delta}t_i) \psi\Big(-\varpi^{-\beta-\delta}t_i \Big)\\
	...
	which case the integral is 1, so the zeta integral is
	\begin{align*}
		&=q^{\beta n(s-1/2)+n(n-1)-\beta\binom{n}{2}}\chi(\det w_{n})\int\cdots\int\prod_{i=1}^n\theta_{n+i}|\cdot|^{s-1}(\varpi^{-\delta}t_i)\chi(\varpi^{-\beta-\delta}t_i) \psi\Big(-\varpi^{-\beta-\delta}t_i \Big)\prod
		dt_i.
	\end{align*}
	
	
	When $\chi$ is ramified with conductor $\beta$ then
	\begin{align*}
		&\
		...
		&=q^{\delta (s-1)}\cdot q^{-\beta }\frac{q}{q-1} \UPS_{n+i}(\varpi)^{-\delta}\tau(\chi),
	\end{align*}
	since the integral vanishes unless $k = 0$, whence it is the Gauss sum. If $\chi$ is unramified, then $\beta=1$ and, dropping indices for simplicity,
	\begin{align*}
		\int_{\mathcal{O}-0}\theta|\cdot|^{s-1}&(t\varpi^{-\delta})\chi(t
		...
		t\\
		&=\frac{1}{1-q}q^{\delta(s-1)}\theta(\varpi)^{-\delta}\chi(\varpi)^{-\delta-1} + \sum_{k=1}^\infty
		q^{-k-(k-\delta)(s-1)}\theta(\varpi)^{k-\delta}\chi(\varpi)^{k-\delta-1}\\
		&=q^{-1-(1-\delta)(s-1)}\theta(\varpi)^{1-\delta}\chi(\varpi)^{-\delta}\left(\sum_{k=0}^\infty
		\left(\frac{\theta
			\chi(\varpi)}{q^{s}}\right)^k+\frac{q^{s}\theta\chi(\varpi)^{-1}}{1-q}\right)\\
		&=q^{-1-(1-\delta)(s-1)}\theta(\varpi)^{1-\delta}\chi(\varpi)^{-\delta}\left(\frac{1}{1-\frac{\theta\chi(\varpi)}{q^{s}}}+\frac{q^{s}\theta\chi(\varpi)^{-1}}{1-q}\right)\\
		&=\frac{1}{1-q}q^{\delta(s-1)}\theta(\varpi)^{-\delta}\chi(\varpi)^{-\delta-1}\frac{1-\frac{\theta\chi(\varpi)}{q^{s-1}}}{1-\frac{\theta\chi(\varpi)}{q^{s}}},
	\end{align*}
...
\end{proof}
	...
	We recap the theory of overconvergent cohomology in $p$-adic families, as developed for example \cite{Urb11, Han17}. All of this material is explained in depth \emph{op.\ cit}., so we are terse with details.
	
	
	%%===================================================
	\subsection{Weight spaces}\label{sec:weight spaces}
	Recall $X^*(T),$ $X^*_0(T),$ $X^*(H)$ and $X^*_0(H)$ from \S\ref{sec:algebraic weights}. If $K \subset G(\A_f)$ is an open compact subgroup, let $\overline{Z}_K$ be the $p$-adic closure of $Z(G(\Q))\cap K$. 
	...
	Let $K \subset G(\A_f)$ be open compact such that $K_p\subset \Iwahori_G$. 
	...
	\begin{remark}\label{rem:integral normalisation}
		The operator $U_p^\circ$ preserves the integral structure $\hc{\bullet}(S_K,\sD_\lambda(\cO_L)) \subset \hc{\bullet}(S_K,\sD_\lambda(L))$. We also have a $*$-action of $\Delta_p$ on $V_\lambda^\vee(L)$, defined identically, giving an operator $U_p^\circ$ on $\hc{\bullet}(S_K,\sV_\lambda^\vee(L))$ that preserves its natural integral subspace. If $U_p$ denotes the automorphic Hecke operator from \S\ref{sec:hecke p}, one may check $U_p^\circ = \lambda(t_p) \cdot U_p$. This is all explained in \cite[Rem.\ 3.13]{BDW20}.
	\end{remark}
	...
	\begin{definition}\label{def:non-Q-critical}
		...
		We say $\tilde\pi$ is \emph{strongly non-critical} if this is true with $\hc{\bullet}$ replaced with $\h^\bullet$ (i.e., if $\tilde\pi$ is non-critical for $\h^\bullet$ \emph{and} for $\hc{\bullet}$ as in \cite[Rem.~4.6]{BW20}). 
	\end{definition}
	...
	A \emph{branching law} describes how an irreducible representation of $G$ decomposes upon restriction to $H$. Of particular interest to us is the branching law given by Lemma \ref{lem:branching law}, which provides a representation-theoretic interpretation of the Deligne-critical range. In this section, we provide a $p$-adic interpolation of the classical branching law Lemma \ref{lem:branching law}.
	...
	\end{lemma} 
	We now give a conceptual description of the generators of $V_\jw^H \subset V_{\lambda}|_{H(\Zp)}$. Define weights
	...
	\end{align}
(When $n=1$, we just have $\alpha_0 = (1,1)$ and $\alpha_1 = (1,0)$). 
	If $\lambda = (\lambda_1,...,\lambda_{2n}) \in X_0^*(T)$ is a dominant pure algebraic weight, then we easily see that
	...
	\begin{notation}\label{not:v_i}
		...
		\end{itemize}
		We view all of the elements $v_{(i)}$ as explicit algebraic functions $G(\Zp) \to \Qp$.
	\end{notation}
	
	\begin{proposition}\label{prop:product for v lambda j}
		Let $\lambda \in X_0^*(T)$ be a dominant pure algebraic weight, and let $j \in \mathrm{Crit}(\lambda)$. Then
		\[
	...
	\end{proposition}
	\begin{proof}
		To see this function is algebraic, note the $v_{(i)}$ and $v_{(n),i}$ are algebraic by construction, and:
		\begin{itemize}\setlength{\itemsep}{0pt}
	...
	\subsection{Support conditions for branching laws} Let $w_n$ denote the antidiagonal $n\times n$ matrix with $(w_n)_{ij} = \delta_{i, n+1-j}$, and recall
	...
	Let $\lambda \in X_0^*(T)$ be pure dominant algebraic, let $j \in \mathrm{Crit}(\lambda)$, and let $v$ be a generator of the line $V_\jw^H$ inside the $H(\Zp)$-representation $V_\lambda(\Qp)$, viewed as an explicit algebraic function $G(\Zp) \to \Qp$. The key examples we consider are $\lambda = \alpha_i$ from \eqref{eq:alpha_i}, with $v = v_{(i)}$ from Notation \ref{not:v_i}. The aim of this subsection is to prove:
	
	\begin{proposition}\label{prop:support in N1}
		Possibly rescaling $v \in V_\jw^H$, we have $v(N^\beta(\Zp)) \subset 1+p^\beta\Zp$ for all $\beta \geq 1$.
	\end{proposition}
	...
	Now let $v$ be as in Proposition \ref{prop:support in N1}. Since $v$ transforms by $\lambda(\overline{b}) \in \Zp^\times$ under left translation by $\overline{b}\in \overline{B}_{2n}(\Zp)$, and transforms like $\det(h_1)^{-j}\det(h_2)^{\sw+j} \in \Zp^\times$ under right translation by $h = (h_1,h_2) \in H(\zp)$, we see
	\[
	v\big[\overline{B}_{2n}(\Zp) \cdot u \cdot H(\Zp)\big] \subset \Zp^\times \cdot v(u).
	\]
	Suppose $v(u) = 0$; then $v$ vanishes on the cell $\overline{B}_{2n}(\Zp) \cdot u\cdot H(\Zp)$. This cell is open and dense in $G(\Zp)$ (e.g. \cite[\S5.1.3]{loeffler-parabolics}), forcing $v = 0$, which contradicts our assumptions. Thus $v(u) \neq 0$, and we are free to rescale it by an element of $\Qp^\times$ so that $v(u) = 1$. Further if $\overline{b}, h \equiv 1_{2n} \newmod{p^\beta}$, then $\lambda(\overline{b}), \det(h_1)^{-j}\det(h_2)^{\sw+j} \equiv 1 \newmod{p^\beta}$. Combining all of this with Lemma \ref{lem:big cell}, we deduce that for any $X \in M_n(\Zp)$, we have
	\begin{equation}\label{eq:cell to Zp^times}
		v\left[\smallmatrd{1_n}{w_n + p^\beta X}{0}{1_n}\right] \in  1 + p^\beta\Zp.
	\end{equation}
	
	\begin{proof} \emph{(Proposition \ref{prop:support in N1}).}
		A general element of $N_\beta(\Zp)$ looks like $n = \smallmatrd{A}{w_n + p^\beta Y}{0}{B}$, where $A,B \in N_n(\Zp)$ with $A \equiv B \equiv 1_n \newmod{p^\beta}$ and $Y \in M_n(\Zp)$. Letting
		\[
		...
		\]
		Then $v(n) = \det(A)^{-j}\det(B)^{\sw+j} \cdot v\left[\smallmatrd{1_n}{w_n+pY}{0}{1_n}\right] \in 1 + p^\beta\Zp$ by \eqref{eq:cell to Zp^times}.
	\end{proof}
	...
	Now we $p$-adically interpolate, with $\beta = 1$. Assume the choices $v_{(0)}, v_{(1)},...,v_{(n-1)},v_{(n),1},v_{(n),2}$ in \S\ref{sec:classical branching laws} were all normalised so $v_{(i)}(u) = 1$, whence $v_{(i)}(N^1(\Zp)) \subset 1+p\Zp \subset \Zp^\times$, as in Proposition \ref{prop:support in N1} (and its proof).
	...
	To $p$-adically vary branching laws, we take $R = \cO_\Omega$, for $\Omega \subset \sW_0^G$ an affinoid in the pure weight space, equipped with a character $\chi_\Omega : T(\Zp) \to \cO_\Omega^\times$ as in \S\ref{sec:weight spaces}. We allow $\Omega = \{\lambda\}$ to be a point.
	...
	Recall $\IwH^1$ from \eqref{eq:Iw_H^1}; this acts on $\cA(\Zp^\times,\cO_\Omega)$ and $\cA_\Omega$ by its embeddings into $H(\Zp)$ and $\Iw$. 
	...
	The overconvergent cohomology groups we consider have coefficients in $\cD_\Omega$, not $\cA_\Omega$, we now dualise the above. Finally, we collate everything we have proved in the main result of this section (Proposition \ref{prop:distribution commute}).\medskip
	...
	\begin{definition}\label{def:distribution support}
		Let $\beta \geq 1$. We say that $f \in \cA_\Omega$ (resp.\ $\mu \in \cD_\Omega$) has  \emph{support on $\Iw^\beta$} if $f(g) = 0$ for $g \notin \Iw^\beta$ (resp.\ $\mu(f) = \mu(f|_{\Iw^\beta})$ for all $f \in \cA_\Omega$).
		Let $\cA_\Omega^\beta \subset \cA_\Omega$ (resp.\ $\cD_\Omega^\beta \subset \cD_\Omega$) be the subspace of functions (resp.\ distributions) supported on $\Iw^\beta$. We similarly write $\cA_\lambda^\beta$, $\cD_\lambda^\beta$, etc.
	\end{definition}
	...
	\subsection{Combining evaluations and branching laws}
	...
	\end{lemma}
	\begin{proof}
		Note that $\mathrm{Ev}_{B,\beta,\delta}^{\cD_\Omega}(\Phi) \in [(u^{-1} t_p^\beta) * \cD_\Omega]_{\Gamma_{\beta,\delta}}$ by construction of evaluation maps. By definition, if $\mu \in \cD_\Omega, f \in \cA_\Omega$, and $n \in N(\Zp)$, then the action of $u^{-1} t_p^\beta$ is by
		\[
		\big[u^{-1} t_p^\beta * \mu\big](f(n)) = \mu\big[f(t_p^\beta n t_p^{-\beta} u)].
		\]
		It is easily seen that $t_p^\beta n t_p^{-\beta} \equiv 1_{2n} \newmod{p^\beta}$ (e.g.\ from \cite[\S2.5, Rem.\ 4.19]{BW20}). By definition of $N^\beta(\Zp)$ we deduce $t_p^\beta nt_p^{-\beta}u \in N^\beta(\Zp)$. Extending via \eqref{eq:Iw 1} and \eqref{eq:iwahori induction 2}, it follows $[(u^{-1}t_p^\beta)*\mu](f)$ depends only on $f|_{\Iw^\beta}$, so $(u^{-1}t_p^\beta * \cD_\Omega) \subset \cD_\Omega^\beta$, proving the lemma (and thus Proposition \ref{prop:evs main commutative diagram}).
	\end{proof}
	...
	Let $\eta_0$ be any character of $(\Z/m)^\times$. For $\beta \geq 1$ and $\brep \in (\Z/p^\beta\Z)^\times,$ and motivated by Definition \ref{def:ev chi}, define a map
	\begin{align*}
	...
	\begin{definition}
		Let $\Phi \in \hc{t}(S_K,\sD_\Omega)$ is a $U_p^\circ$-eigenclass with eigenvalue $\alpha_p^\circ$, fix $\beta \geq 1$, and define
		\[
	...
	\begin{definition}\label{def:non-critical slope Lp}
		Let $\cL_p^\pm(\tilde\pi) \defeq \mu^{\lambda,\mathbf{1}}(\Phi_{\tilde\pi}^\pm) \in \cD(\Zp^\times,L).$ Let $\Phi_{\tilde\pi} = \Phi_{\tilde\pi}^+ + \Phi_{\tilde\pi}^-$, and define the \emph{$p$-adic $L$-function} attached to $\tilde{\pi}$ to be $\cL_p(\tilde\pi) = \mu^{\lambda,\mathbf{1}}(\Phi_{\tilde\pi}) = \cL_p^+(\tilde\pi) + \cL_p^-(\tilde\pi)$. 
		Let $\sX \defeq (\mathrm{Spf}\ \Zp\lsem \Zp^\times \rsem )^{\mathrm{rig}}$. The Amice transform allows us to consider $\cL_p(\tilde\pi, -) : \sX \to \overline{\Q}_p$ as an element of $\cO(\sX)$.
	\end{definition}
	
	\begin{theorem} \label{thm:non-ordinary}
		The distribution $\cL_p({\tilde{\pi}})$ has growth order $h_p = v_p(\alpha_p^\circ)$. For every finite order character $\chi$ of $\Q^\times\backslash\A^\times$ of conductor $p^\beta$ with $\beta \in \Z_{\geq 0}$, and all $j \in \mathrm{Crit}(\lambda)$, we have
		\begin{align*}
			\cL_p(\tilde\pi, \chi(z)z^j) \defeq \int_{\Zp^\times} \chi(z) z^j \cdot d\cL_p(\tilde{\pi})
			= \gamma_{(pm)} \cdot e_p(\tilde\pi,\chi,j) \cdot \zetainfty \cdot \frac{L\big(\pi\otimes\chi, j+\tfrac{1}{2}\big)}{\Omega_\pi^{\pm}},
		\end{align*}
		where $\chi(-1)(-1)^j = \pm 1$. Here if $\chi \neq 1$ we define
		\[
		e_p(\tilde\pi,\chi,j) \defeq \bigg(\frac{p^{nj + \tfrac{n^2-n}{2}}}{\alpha_{p,n}}\bigg)^\beta \tau(\chi)^n,
		\]
			whilst if $\chi = 1$ we let $\alpha_{i,j} = \UPS_i(p)/p^{j+1/2}$, and define
			\[
			e_p(\tilde\pi,1,j) = \prod_{i=n+1}^{2n}
			\frac{1-p^{-1}\alpha_{i,j}^{-1}}{1 - \alpha_{i,j}}.
			\] 
		All further notation is as in Theorem~\ref{thm:critical value} and Corollary \ref{cor:L-value}. 
	\end{theorem}
	
	\begin{proof}
		For ramified characters $\chi$, this is analogous to \cite[Thm.\ 6.23]{BDW20}. A difference is as follows: via the methods \emph{op.\ cit}., we get a factor of $(\alpha_{p}^\circ)^{-\beta}$. Recall that in Corollary \ref{cor:L-value} we had a factor $(\alpha_{p}^\circ/\alpha_{p,n})^{\beta}$; so these combine to leave only $(\alpha_{p,n})^{-\beta}$.
		

This leaves $\chi = 1$. We will complete the proof in this case in Proposition \ref{prop:unramified interpolation}.
		%In the unramified case, we still get a term $\alpha_{p,n}^{-1} = p^{-n^2/2}\prod_{i=n+1}^{2n}\UPS_i(p)^{-1}$. We cancel this with the numerator in the product in $Q'(\pi,1,j)$ (from Corollary \ref{cor:L-value}) to get the desired expression.
	\end{proof}
	
	
	\begin{remark}
		The factor $e_p(\tilde\pi,\chi,j)$ is consistent with the Coates--Perrin-Riou conjecture from \cite{coates89}; this is mostly explained in \cite[\S3]{AG94}. We expect, but do not prove, that $\zetainfty$ recovers the factor at infinity in \cite{coates89}; similar questions are studied in \cite{JanPeriodII}. We note that for $\GL_4$, this could be proved in full by via comparison to the $p$-adic $L$-functions for $\mathrm{GSp}_4$ from \cite{LPSZ19} (where the factor at infinity is known in full), via the same strategy as \cite[\S9.5]{LW-GL3}. Finally, the global scalar $\gamma_{(pm)}$ can be removed by absorbing into $\Omega_\pi^\pm.$ 
	\end{remark}
	...
	This entire subsection is dedicated to the proof of Theorem \ref{thm:shalika family} below, in which we will use our evaluation maps to construct Shalika families. The proof closely follows the proofs of \cite[Thms.\ 7.6,8.14]{BDW20}. 
	
	
	
	\begin{definition}\label{def:local piece S}
		Define $\T\Uha$  to be the image of $\cH\otimes\cO_\Omega$ in $ \End_{\cO_\Omega}\big(\htc(S_K,\sD_{\Omega})^{\leqslant h}\big)$. Define a rigid space $\sE\Uha \defeq \mathrm{Sp}(\T\Uha)$, a rigid analytic space.
	...
	By definition, $\sE\Uha$ is a rigid space whose $L$-points $y$ biject with non-trivial homomorphisms $\T\Uha \rightarrow L$, i.e.\ with systems of eigenvalues of  $\psi_y : \cH \to L$ appearing  in $\htc(S_K,\sD_{\Omega})\ssh$. 
	
	
	\begin{definition}
		A point $y \in \sE_{\Omega,h}$ is \emph{classical} if there exists a cohomological automorphic representation $\pi_y$ of $G(\A)$ of weight $\lambda_y \defeq w(y)$ such that $\psi_y$ appears in $\pi_y^{K}$, whence $\tilde\pi_y = (\pi_y,\alpha_y)$ is a $p$-refined RACAR (where $\alpha_y = \psi_y|_{\cH_p})$.  A classical point $y$ is \emph{cuspidal} if $\pi_y$ is. 
	...
	\end{lemma}
	\begin{proof}
		The first isomorphism is proved identically to \cite[Prop.\ 7.8]{BDW20} (then applying $\pm$-projectors). The second follows from non-criticality of $\tilde\pi$.
	\end{proof}
	...
	\begin{corollary}\begin{enumerate}[(i)]\setlength{\itemsep}{0pt} \label{cor:cyclic}
			...
			\item  Possibly shrinking $\Omega$, there exist ideals $I^\pm_{\sC} \subset \cO_\Omega$ such that $\hc{t}(S_K,\sD_\Omega)^\pm \otimes_{\bT_{\Omega,h}^\pm} T^\pm \cong \cO_\Omega/I_{\sC}^\pm$.
		\end{enumerate}
	\end{corollary}
	\begin{proof}
		...
		(ii) This follows from rigid delocalisation of (i) (cf.\ \cite[Prop.\ 8.16]{BDW20}).
	\end{proof}
	...
	\begin{proposition}\label{prop:etale} Suppose $\lambda_{\pi,n} > \lambda_{\pi,n+1}$. Then, up to shrinking $\Omega$:
		\begin{enumerate}[(i)]\setlength{\itemsep}{0pt}
			...
			\item $\hc{t}(S_K,\sD_\Omega)^\pm \otimes_{\bT_{\Omega,h}^\pm} T^\pm$ is free of rank one over $T^\pm$.
		\end{enumerate}
	\end{proposition}
	\begin{proof}
		By the weight condition, as in \cite[Lem.\ 7.4]{BDW20}, there exist $\beta \geq 1$, $j \in \mathrm{Crit}(\lambda_\pi)$ and Hecke characters $\chi^\pm$ of conductor $p^\beta$ with $\chi^\pm(-1)(-1)^j = \pm 1$ such that $L(\pi\otimes\chi^\pm ,j+\tfrac{1}{2}) \neq 0$. 
		
		As the $\sC^\pm$ are connected components, there exist idempotents $e^\pm$ such that $T^\pm = e^\pm \bT_{\Omega,h}^\pm$, and $\hc{t}(S_K,\sD_\Omega)^\pm \otimes_{\bT_{\Omega,h}^\pm}T^\pm = e^\pm \hc{t}(S_K,\sD_\Omega)^\pm \subset \hc{t}(S_K,\sD_\Omega)^\pm$. Restricting $\mathrm{Ev}_{B,\beta}^{\Omega,\mathbf{1}}$ from \eqref{eq:final evaluation}, we get
		\[
		\mathrm{Ev}_{B,\beta}^{\Omega,\mathbf{1}}	: \hc{t}(S_K,\sD_\Omega)^\pm \otimes_{\bT_{\Omega,h}^\pm} T^\pm \to \cD(\Zp^\times,\cO_\Omega).
		\]
		Using Corollary \ref{cor:cyclic}, let $\Phi_{\sC}^\pm$ be a generator of $\hc{t}(S_K,\sD_\Omega)^\pm \otimes_{\bT_{\Omega,h}^\pm} T^\pm$ over $\cO_\Omega$. Note that $\mathrm{Ann}_{\cO_\Omega}(\Phi_{\sC}^\pm) = I^\pm_{\sC}$. Then via the interpolation of Theorem \ref{thm:non-ordinary}, we have
		\[
		\int_{\Zp^\times} \chi^\pm(z)z^j \cdot d\mathrm{Ev}_{B,\beta}^{\Omega,\mathbf{1}}(\Phi_{\sC}^\pm) = (*) \times L(\pi\otimes\chi^\pm, j+\tfrac{1}{2}) \neq 0,
		\]
		where $(*)$ is non-zero. Thus $\mathrm{Ev}_{B,\beta}^{\Omega,\mathbf{1}}(\Phi_{\sC}^\pm) \neq 0$. Since $\cD(\Zp^\times,\cO_\Omega)$ is a torsion-free $\cO_\Omega$-module, it follows that $\mathrm{Ann}_{\cO_\Omega}(\Phi_{\sC}^\pm) = 0$ (cf.\ \cite[Prop.\ 7.11]{BDW20}). Thus $I^\pm_{\sC} = 0$, and $\hc{t}(S_K,\sD_\Omega)^\pm\otimes_{\bT_{\Omega,h}^\pm}T^\pm \cong \cO_\Omega$.
		
		We deduce $T^\pm$ is the image of $\cH$ in $\mathrm{End}_{\cO}(\Omega)$. Since this image is non-zero we deduce (i). Finally since the actions of $\cO_\Omega$ and $T^\pm$ are compatible on $\hc{t}(S_K,\sD_\Omega)^\pm\otimes_{\bT_{\Omega,h}^\pm}T^\pm$, and both $T^\pm$ and $\hc{t}(S_K,\sD_\Omega)\otimes_{\bT_{\Omega,h}^\pm}T^\pm$ are free rank one $\cO_\Omega$-modules, we deduce (ii).
	\end{proof}
	
	We finally arrive at the main result of this section.
	
	\begin{theorem}\label{thm:shalika family}
		Let $\tilde\pi$ be a $p$-refined RACAR of weight $\lambda_\pi$ satisfying Conditions \ref{cond:running assumptions}. Suppose $\lambda_{\pi,n} > \lambda_{\pi,n+1}$. Then:
		\begin{enumerate}[(i)]\setlength{\itemsep}{0pt}
			\item There exists a point $x_{\tilde\pi} \in \cE_{\Omega,h}$ attached to $\tilde\pi$, and $w: \cE_{\Omega,h} \to \Omega$ is \'etale at $x_{\tilde\pi}$.
			\item The connected component $\sC = \mathrm{Sp}(T)$ in $\cE_{\Omega,h}$ through $x_{\tilde\pi}$ contains a Zariski-dense set $\sC_{\mathrm{nc}}$ of classical points corresponding to $p$-refined RACARs $\tilde\pi_y$.
	...
	\begin{proof}
		(i) First consider the $\pm$-analogues. Lemma \ref{lem:spec isomorphism} and Proposition \ref{prop:mult one} show $\m_{\tilde\pi}$ appears in $\hc{t}(S_K,\sD_\Omega)^\pm$, giving points $x_{\tilde\pi}^\pm \in \cE_{\Omega,h}^\pm$. Moreover Proposition \ref{prop:etale} shows $\sE_{\Omega,h}^\pm \to \Omega$ is \'etale at $x_{\tilde\pi}^\pm$. Using strong non-criticality of $\tilde\pi$, we deduce that $\sE_{\Omega,\h}^\pm$ contain Zariski-dense sets $\sC_{\mathrm{nc}}^\pm$ of cuspidal non-critical slope classical points by \cite[Prop.\ 5.15]{BW20}. Now, as in \cite[Prop.\ 8.20]{BDW20}, we can exhibit a bijection between $\sC_{\mathrm{nc}}^+$ and $\sC_{\mathrm{nc}}^-$ and (via \cite[Thm.\ 3.2.1]{JoNew}) a canonical isomorphism $\sC^+ \isorightarrow \sC^-$, whence $\sC\cong \sC^+ \cong \sC^-$ is independent of sign. Part (i) follows immediately.
		
		(ii) We let $\sC_{\mathrm{nc}} = \sC_{\mathrm{nc}}^+ = \sC_{\mathrm{nc}}^-$ be the set used in (i).
		
		(iii) Let $\Phi_{\sC}^\pm$ be $\cO_\Omega$-module generators of $\hc{t}(S_K,\sD_\Omega)^\pm\otimes_{\bT_{\Omega,h}^\pm}T^\pm \subset \hc{t}(S_K,\sD_\Omega)^\pm$, which are well-defined Hecke eigenclasses by Proposition \ref{prop:etale}. For each $y \in \sC_{\mathrm{nc}}$, let $\m_y \subset T^\pm$ be the attached maximal ideal. Reduction modulo $\m_{\lambda_y}$ induces a map
		\[
		\mathrm{sp}_{\lambda_y} : \hc{t}(S_K,\sD_\Omega)^\pm \otimes_{\bT_{\Omega,h}^\pm}T^\pm \twoheadrightarrow \hc{t}(S_K,\sD_{\lambda_y})^\pm_{\tilde\pi_y},
		\]
		...
		\end{align*}
		As in the proof of Proposition \ref{prop:etale}, we have $\mathrm{Ev}_{B,\chi,j}^\Omega(\Phi_{\sC}^\pm)(\lambda_\pi) = (*) \times L(\pi\otimes\chi^\pm, j+\tfrac{1}{2}) \neq 0$. Possibly shrinking $\Omega$, we may thus suppose $\mathrm{Ev}_{B,\chi,j}^\Omega(\Phi_{\sC}^\pm)$ is everywhere non-vanishing on $\Omega$. Let 
		\[
		...
		It remains to show (C2). Firstly, since failure to be spherical at $p$ is a closed condition in the eigenvariety, we can remove the points of $\sC_{\mathrm{nc}}$ corresponding to $\pi_y$ with $\pi_{y,p}$ is not spherical, and it remains Zariski-dense; so assume $\pi_{y,p}$ spherical. Let $\alpha_{y,p,r} = \alpha_y(U_{p,r})$ be the $U_{p,r}$-eigenvalue of $\phi_{\tilde\pi_y}^\pm$. As $\hc{t}(S_K,\sV_\lambda^\vee)^\pm_{\tilde\pi_y}$ is a line, by \eqref{eq:pi to cohomology} again we deduce that $\cS_\psi^{|\cdot|^{\sw_y}}(\pi_{y,p}^{\Iw})[U_{p,r} - \alpha_{y,p,r} : r = 1,...,2n-1]$ is a line, so $\tilde\pi_y$ is a regular $p$-refinement. Let $W_{y,p}$ be a generator; then we can relate $W_{y,p}\smallmatrd{w_n}{}{}{1}$ to a non-zero multiple of $\mathrm{Ev}_{B,\chi,j}^\Omega(\phi_{\tilde\pi_y}^\pm)$ exactly as in the proof of \cite[Prop.\ 8.22]{BDW20}. In particular, $W_{y,p}\smallmatrd{w_n}{}{}{1} \neq 0$, so $\tilde\pi_y$ satisfies (C2).
	\end{proof}
	...
	We finally give our main construction, of the variation of $p$-adic $L$-functions of RASCARs in pure weight families. Let $\tilde\pi$ of weight $\lambda_{\pi}$ satisfy (C1-4) of Conditions \ref{cond:running assumptions}, and suppose $\lambda_{\pi,n} > \lambda_{\pi,n+1}$. By Theorem \ref{thm:shalika family}(i), the eigenvariety for $G$ is \'etale over weight space at $\tilde\pi$, and by Theorem \ref{thm:shalika family}(iv) its connected component $\sC$ through $\tilde\pi$ contains a Zariski-dense set $\sC_{\mathrm{nc}}$ of classical points satisfying Conditions \ref{cond:running assumptions}. Let $\Phi_{\sC}^\pm \in \hc{t}(S_K,\sD_\Omega)^\pm$ be the classes of Theorem \ref{thm:shalika family}(iii). Possibly rescaling by $\cO_\Omega^\times$, we may always assume $\mathrm{sp}_{\lambda_\pi}(\Phi_{\sC}^\pm) = \Phi_{\tilde\pi}^\pm$.
	
	
	\begin{definition} \label{def:Lp families} Let $\cL_{p}^{\sC, \pm} \defeq \mu^{\Omega,\mathbf{1}}(\Phi_{\sC}^\pm)$. Also let $\Phi_{\sC} = \Phi_{\sC}^+ + \Phi_{\sC}^- \in \hc{t}(S_{K(\tilde\pi)},\sD_\Omega)$, a Hecke eigenclass, and define the \emph{$p$-adic $L$-function over $\sC$} to be 
		\[
		\cL_p^{\sC} \defeq \mu^{\Omega,\mathbf{1}}(\Phi_{\sC}) = \cL_p^{\sC,+} + \cL_p^{\sC,-} \in \cD(\Gal_p,\cO_\Omega).
		\]
		Via the Amice transform  (cf.\ Definition~\ref{def:non-critical slope Lp}), after identifying $\sC$ with $\Omega$ via $w$ we consider $\cL_p^{\sC}$ as a rigid function $\sC \times \sX \to \overline{\Q}_p$.
	\end{definition}
	...
	\begin{theorem}\label{thm:family p-adic L-functions}
		...
		\begin{equation}\label{eq:spec in families}
			\cL_p^{\sC}(y,\ \chi(z)z^j) = c_y^\pm \cdot A^\pm(\tilde\pi_y,\chi,j) \cdot L(\pi_y\times\chi, j+\tfrac{1}{2})/\Omega_{\pi_y}^\pm,
		\end{equation}
		where $\chi(-1)(-1)^j = \pm 1$ and $A^\pm(-)$ is as defined in Theorem~\ref{thm:non-ordinary}. Finally we have $c_{x_{\tilde\pi}}^\pm = 1$.
	\end{theorem}
...
\section{Comparison to existing constructions}
...
We finally show that the $p$-adic $L$-functions we've constructed at Iwahori level agree with previous constructions, and deduce their interpolation property at unramified characters.
	...
	If $\tilde\pi$ is a non-critical regular $p$-refinement (to Iwahori level), let $\tilde\pi^Q \defeq (\pi, \alpha_{p,n})$. This is a non-$Q$-critical $Q$-regular $Q$-refinement as in \cite[\S2.7,\S3.5]{BDW20}, and Theorem 6.23 \emph{op.\ cit}.\ attaches a $p$-adic $L$-function $\cL_p(\tilde\pi^Q) \in \cD(\Zp^\times,L)$ to $\tilde\pi^Q$.
	
	\begin{proposition}\label{cor:BDW}
		%When $v_p(\alpha_{p}^\circ) < \#\mathrm{Crit}(\lambda)$, 
...
\end{equation}
 where $\lambda_\pi$ is the weight of $\pi$. 	In this case, both distributions have sufficiently small growth that they are uniquely determined by their interpolation properties. Their respective interpolation properties agree exactly except for:
 \begin{itemize}\s
 	\item Volume terms at $p$ (the term denoted $\gamma$ in \cite{DJR18,BDW20} and $\gamma_{(pm)}$ in the present paper). These explicit factors differ by a rational number $\Upsilon$ independent of $\pi$.
 	\item The terms at infinity.
\end{itemize}
 One can check, via a long and tedious comparison of conventions, that these factors agree. Alternatively, we can deduce this directly by appealing to the same strategy as \cite[\S10.4]{LW-GL3}: if $v_p(\alpha_p^\circ) = 0$, then both $\cL_p(\tilde\pi)$ and $\cL_p(\tilde\pi^Q)$ are measures, and rescaling the various arbitrary choices (e.g. the classes $\Xi_\infty^\pm$ from \S\ref{sec:cohomology for RACARs}, hence the periods $\Omega_\pi^\pm$) we can dictate the terms at $\infty$ agree on finite order characters (i.e.\ when $j=0$). In particular, the ratio of the two measures is \emph{constant}, and we then deduce the terms at infinity agree for all higher $j$. But the terms at infinity are independent of the slope $v_p(\alpha_p^\circ)$.


\medskip

In general, when we drop assumption \eqref{eq:very small slope}, we instead argue in families (using that points satisfying \eqref{eq:very small slope} are Zariski-dense). 

Since $\tilde\pi$ is non-critical, Theorem \ref{thm:family p-adic L-functions} gives a $p$-adic $L$-function $\cL_p^{\sC} \in \cD(\Galp,\cO_\Omega)$ in an Iwahoric Shalika family through $\tilde\pi$. Let $\Omega^Q$ denote the intersection of $\Omega$ with the (2-dimensional) $Q$-parahoric weight space through $\lambda_\pi$ (see \cite[Def.\ 3.4]{BDW20}), and let $\sC^Q \defeq \sC \cap w^{-1}(\Omega^Q)$ be the resulting $Q$-parabolic Shalika family. This contains a Zariski-dense set of points $y$ where \eqref{eq:very small slope} is satisfied (by the same argument as Theorem \ref{thm:shalika family}(iv)). For these points, we have shown that $\cL_p(\tilde\pi_y) = \Upsilon \cdot \cL_p(\tilde\pi_y^Q)$. 

It follows that the restriction of $\cL_p^{\sC}$ to $\sC^Q$ satisfies the conditions of \cite[Prop.\ 8.28]{BDW20}; and by that Proposition, we see that we have
\[
	\cL_p^{\sC}(x_{\tilde\pi},-) = \Upsilon \cdot \cL_p(\tilde\pi^Q).
\] 
(Note that the function $\cB^\eps$ in the proof \emph{op.\ cit}.\ is constant and equal to $\Upsilon$ here, so this result is on the nose, not just up to scalar). By Theorem \ref{thm:family p-adic L-functions} we also have
\[
\cL_p^{\sC}(x_{\tilde\pi},-) = \cL_p(\tilde\pi).
\]
The result follows.
\end{proof}
...
Now let $\tilde\pi^Q$ be any non-$Q$-critical $Q$-refinement of $\pi$, and let $\cL_p(\tilde\pi^Q)$ be the $p$-adic $L$-function attached by \cite[Thm.\ 6.23]{BDW20}. The following strengthens the results of \cite{DJR18,BDW20}.

\begin{proposition} \label{prop:unramified interpolation}
The $p$-adic $L$-function $\cL_p(\tilde\pi^Q)$ satisfies the interpolation
		\begin{align*}
			\cL_p(\tilde\pi, z^j)
			= \Upsilon^{-1} \cdot \gamma_{(pm)} \cdot e_p(\tilde\pi,1,j) \cdot \zetainfty \cdot \frac{L\big(\pi, j+\tfrac{1}{2}\big)}{\Omega_\pi^{\pm}},
		\end{align*}
		for all $j \in \mathrm{Crit}(\lambda_\pi)$. Here $(-1)^j = \pm 1$ and all other notation is as in Theorem \ref{thm:non-ordinary}.
		
		In particular, $\cL_P(\tilde\pi)$ satisfies the interpolation of Theorem \ref{thm:non-ordinary} at $\chi = 1$.
\end{proposition}
...
\begin{align*}
	...
	&= \Upsilon_Q \cdot \frac{p^{n^2} }{\alpha_{p,n}} \cdot \zeta_j(W_\infty^\pm) \cdot \frac{L(\pi,j+\tfrac{1}{2})}{\Omega_\pi^\pm} \cdot \zeta_p\Big(j+\tfrac{1}{2}, (u^{-1}t_{p,n}^\beta)\cdot W_p, 1\Big).
\end{align*}
Here the first equality is shown in \cite[Thm.\ 6.23]{BDW20}. The second is Theorem \ref{thm:critical value}, noting that $\delta_B(t_Q^{-1}) = p^{n^2}$ and $\lambda(t_Q)/\alpha_{p,n}^\circ = 1/\alpha_{p,n}$. This local zeta integral was computed in Proposition \ref{p:local-zeta}. We find that this exactly agrees with the claimed formula (as by definition, $\Upsilon$ tracks the difference between the volume factor $\gamma_{(pm)}$ at Iwahori level and $\gamma$, from \cite{DJR18}, at parahoric level).
...
%	\begin{remark}
		%The factor $[L_\beta^Q:L_\beta^B] \in \Z$ can be absorbed into the period (which is in any case only well-defined up to scalar). 
%We note that under this small slope condition, the $p$-adic $L$-function depends only on the $Q$-refinement, not the full Iwahoric $p$-refinement, providing further evidence that the $Q$-parahoric constructions followed in \cite{DJR18,BDW20} was the `correct' approach for RASCARs.
		
%		We strongly expect that $\cL_p(\tilde\pi) = \cL_p(\tilde\pi^Q)$ always. It should be possible to prove this directly by a careful analysis of evaluation maps, relating our $\mathrm{Ev}_{Q,\beta,\delta}^M$ to the maps $\mathrm{Ev}_{\beta,\delta}^M$ of \cite{BDW20}. There would be two aspects to this comptuation: tracking how $\mathrm{Ev}_{Q,\beta,\delta}^M$ changes as $K_p$ changes from $\Iw$ to $J_Q$, and tracking the difference between $\xi$ \emph{op.\ cit}.\ and $u^{-1}$ here. We omit this, since in our main application -- namely, \cite[Thm.\ 17.6.2]{LZ20} -- we have $v_p(\alpha_p^\circ) = 0$, and Corollary \ref{cor:BDW} applies.
%	\end{remark}
