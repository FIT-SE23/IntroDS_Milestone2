...
\usepackage[%
  backend=biber,
  style=numeric, % alphabetic, numericS
  hyperref=auto,
  doi=false,
  isbn=false,
  url=false,
...
\titlerunning{Intrinsic Dynamics and Global Behavior of Robots}
\author{Alin Albu-Schäffer \and Arne Sachtler}
\institute{Institute of Robotics and Mechatronics\\German Aerospace Center\\82234 Weßling, Germany; and \\\vspace{.1cm}
...
\textcolor{blue}{\footnotesize Users may only view, print, copy, download and text- and data-mine the content, for the purposes of academic research. The content may not be (re-)published verbatim in whole or in part or used for commercial purposes. Users must ensure that the author's moral rights as well as any third parties' rights to the content or parts of the content are not compromised.}
...
An alternative perspective has been taken in robotics for a long time as well, leading to minimalistic and passivity-based control~\cite{Hogan1985,AlbuSchaeffer2007a}, bio-inspired design, embodied intelligence, passive walkers~\cite{McGeer1990}, and locomotion template anchoring~\cite{Holmes2006}.
...
Also research in neuroscience suggests that humans operate their arms on geodesics~\cite{Biess2007}.
...
\begin{eqnarray}
   S_M&= & \int_{t_1}^{t_2} 2T(\qq, \dqq) \d t  = \int_{s_1}^{s_2} 2(E-U(\qq)) \sqrt{\frac {m_{ij} q'^i q'^j} {2(E-U(\qq))}} \d s \nonumber\\
...
In contrast, for systems in potential fields, the geodesics generally vary with the total energy $E$, as the Jacobi metric is energy-dependent. %The necessary and sufficient conditions for the paths to be independent of energy also in presence of potential energy have been derived in~\cite{AlbuSchaeffer2021}.
...
We take the geodesic equation and make $\gamma(s, t)$ satisfy it more and more over time by the PDE \begin{equation}\label{eq:string_pde}
...
Note that we show here in (\ref{eq:update_rule}) an explicit Euler scheme  for discretizing (\ref{eq:string_pde}).
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{figures/coffee.png} 
...
\begin{figure}[b]
  \centering
  \def\svgwidth{0.2\textwidth}\input{pdftex/closed_curves_sphere.pdf_tex}\hspace{1cm}
  \def\svgwidth{0.35\textwidth}\input{pdftex/closed_curves_torus.pdf_tex}
...
\begin{figure}
  \centering
  \subfloat[Double Pendulum\label{fig:the_system}]{\def\svgwidth{.25\textwidth}\input{pdftex/dp_sym.pdf_tex}}\hfill
  \subfloat[Some Periodic Trajectories\label{fig:class_on_torus}]{\def\svgwidth{.7\textwidth}\tiny\input{pdftex/torus_with_all.pdf_tex}}
...
Iteratively updating the nodes of the string by the update rule (\ref{eq:update_rule}) will let the string converge to a geodesic - and thus to a periodic orbit of the double pendulum.
...
%For instance, for high enough energy, the egg-shaped $(0, 1)$-orbit in Fig.~\ref{tab:mn} also appears in a variant passing through the upright position.

\begin{figure}
  \centering
  \begin{tabular}{rcc}
    $(0, 1)$ & \mnmode{.7}{pdftex/mn_01.pdf_tex} & \mnmode{.25}{pdftex/torus_01.pdf_tex}\\
    $(1, 0)$ & \mnmode{.7}{pdftex/mn_10.pdf_tex} & \mnmode{.25}{pdftex/torus_10.pdf_tex}\\
    $(1, 2)$ & \mnmode{.7}{pdftex/mn_12.pdf_tex} & \mnmode{.25}{pdftex/torus_12.pdf_tex}\\
    $(2, 1)$ & \mnmode{.7}{pdftex/mn_21.pdf_tex} & \mnmode{.25}{pdftex/torus_21.pdf_tex}\\
  \end{tabular}
...
\begin{figure}
  \centering
  \subfloat{\def\svgwidth{.35\textwidth}\tiny\input{pdftex/00_eight.pdf_tex}}\hfill
  \subfloat{\def\svgwidth{.25\textwidth}\tiny\input{pdftex/00_eight_pic.pdf_tex}}\hfill
  \subfloat{\def\svgwidth{.33\textwidth}\tiny\input{pdftex/00_eight_q.pdf_tex}}\\

  \subfloat{\def\svgwidth{.35\textwidth}\tiny\input{pdftex/00_zero.pdf_tex}}\hfill
  \subfloat{\def\svgwidth{.25\textwidth}\tiny\input{pdftex/00_zero_pic.pdf_tex}}\hfill
  \subfloat{\def\svgwidth{.33\textwidth}\tiny\input{pdftex/00_zero_q.pdf_tex}}
...
\begin{figure}
  \centering
  \subfloat{\def\svgwidth{.33\textwidth}\tiny\input{pdftex/break_sym.pdf_tex}}\hfill
  \subfloat{\def\svgwidth{.3\textwidth}\tiny\input{pdftex/surface.pdf_tex}}\hfill
  \subfloat{\def\svgwidth{.33\textwidth}\tiny\input{pdftex/break_sym_q.pdf_tex}}
