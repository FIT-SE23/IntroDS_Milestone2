...
In our experiments, we evaluate one downstream task for each choice of target class following the previous works. For better illustration, we randomly sample three downstream tasks that contain the target class from ImageNet. The encoder compromised by our attack results in ASRs of 89.9\%, 91.6\%, and 90.1\% for them. In other words, our attack can simultaneously attack all three downstream tasks.
