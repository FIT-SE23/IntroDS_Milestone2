...
\begin{figure}[ht]
  ...
  \caption{}
  % \caption{The orientations of $ \Upsilon $ and $ \partial\Sigma_{\theta_A} $.}
...
\end{equation}
where
\begin{align*} % 2023-09-02
  \Upsilon_1 &:= \left\{
    -\log(1 + re^{i\alpha_A}) \mid r \in [0,r_A]
  \right\} \bigcup
  \big\{-\log(1 + re^{-i\alpha_A}) \mid r \in [0,r_A]\big\}, \\
    \Upsilon_2 &:= \left\{
      z = -\log(1+r_A e^{i\alpha_A}) + re^{-i\beta_A} \mid 
      r \geqslant 0, \, \operatorname{Im} z \geqslant -\pi
    \right\} \\
    &
    \qquad\qquad \bigcup \, \left\{
      z = -\log(1+r_Ae^{-i\alpha_A}) + re^{i\beta_A} \mid 
      r \geqslant 0, \, \operatorname{Im} z \leqslant \pi
    \right\}.
    \end{align*}
  The above complex logarithm function $ \log $ takes values in
  $ \{z \in \mathbb C \mid \snm{\operatorname{Im} z} < \pi \} $.
...
We also define
\[
  \{ \mathcal I(z) \mid z \in \Upsilon \} \subset
  \mathcal L\big(
...
\begin{lemma}
  ...
  The set $ \{ \mathcal I(z) \mid \, z \in \Upsilon \} $
  is $ \mathcal R $-bounded, and $ \mathcal R(\{\mathcal I(z) \mid z \in \Upsilon\}) $
...
\begin{lemma}
  ...
  and $ \mathcal R(\{\Pi_m \mid m \in \mathbb N\}) $
  is independent of $ \tau $.
\end{lemma}
