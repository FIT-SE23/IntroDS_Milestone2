...
\begin{equation}
\bA_1 = -k\begin{pmatrix}
\cos\beta\\
\sin\beta\\
0
\end{pmatrix}, \quad \bA_2 = -k\begin{pmatrix}
-\sin\beta\\
\phantom{-}\cos\beta\\
0
\end{pmatrix},
...
\bee
E_{(2)}(\bpsin)=\int\left( \frac{1}{2}\partial_i \bpsin\cdot\partial_i\bpsin + \bA_i\cdot (\bn_0 \times \partial_i \bpsin) +\bA_i\cdot(\bpsin\times\partial_i\bpsin)+ \frac{1}{2}\mu^2\lvert\bpsin\rvert^2  \right)d^2x,
\eee
...
Since $\bA_i\cdot (\bn_0 \times \partial_i \bpsin)$ is a divergence, it can be turned into a boundary term which does not affect the Euler-Lagrange equations. It can be removed entirely by a redefinition of the energy as in \cite{Melcher14}. For the purpose of determining these equations we therefore ignore this term, but will revisit it when calculating interaction energies. 



Further, since $(\bpsin\times\partial_i\bpsin)\parallel\bn_0$,
\bee
E_{(2)}(\bpsi)= \int\left( \frac{1}{2}(\partial \bpsi)^2 + \bA_i^{\parallel}\cdot (\bpsi \times \partial_i \bpsi) + \frac{1}{2}\mu^2\lvert\bpsi\rvert^2  \right)d^2x + \text{boundary term},
\eee
where we split $\bA_i$ into components parallel and perpendicular to $\bn_0$, so $\bA_i=\bA_i^{\parallel}+\bA_i^{\perp}$. We also replace $\bpsin$ with $\bpsi$, as it is a dummy variable at this point.

 Defining the abelian projection of the gauge field as  $a_i=\bA_i\cdot \bn_0$, and collecting the components of $\bpsi$  into complex scalar field, $\psi=\psi_1+i\psi_2$, this energy can be rewritten as 
...
\bee
\bn\times\left(-\partial_i\partial_i\bn + 2\partial_i\bn\times\bA_i + \frac{\partial V}{\partial\bn}\right)=0
\eee
...
The definition of the multipole moments of a soliton in the previous section relies on a division of the soliton field into a nonlinear core and linear tail, and the results depend on the choice of origin for polar coordinates. The numerical determination of the multipole moments demonstrates both of these features. We illustrate this by considering a  skyrmion and an  antiskyrmion in the model studied  in \cite{Kuchkin20feb},  with  a Bloch-type DMI  with parameters $\bA_i = -2\pi \be_i$ ($\beta=0$) and a tilted magnetic field  $\be_h= \bn_0 =\sin(\frac{\pi}{3})\be_1 + \cos(\frac{\pi}{3})\be_3$ of strength  $h_z=0.8\cdot(2\pi)^2 $. This means  $\phi_0=\frac{\pi}{2}$ and $m=2\pi\sqrt{0.05}\simeq 1.4$, so that the decay length of the soliton tail is $\simeq 0.7$. We choose the origin of our polar coordinate system and thus the location of our multipole sources to be at the point where $\bn(\bx)=-\be_3$. This lies on the fixed line  of the $\mathbb{Z}_2$ reflection symmetry \eqref{Z2_symmetry} of the skyrmion and antiskyrmion solution in this model  so we expect to find $\gamma_M$ fixed according to \eqref{gamma_phi0}. In the following we refer to this point as the  `soliton centre'. 
...
\begin{figure}[!ht]
  ...
  \end{subfigure}
    ...
    \caption{Magnitude of angular Fourier terms of soliton tails $c_M(r)$ relative to the Bessel function $K_M(mr)$, where $m=\sqrt{h_z-k^2\sin^2\theta_h}$, for the skyrmion and antiskyrmion in a tilted field. The deviation for small $r$ is expected, as this represents the nonlinear core of the soliton. The graph should reach a constant value as $r$ becomes larger, giving a measurement of the multipole strengths $q_M$. Errorbars come from the uncertainty in the position of the centre of the polar co-ordinate system.}
    \label{skaskcomp}
\end{figure}
...
\begin{subfigure}[t]{0.45\textwidth}
  \centering
    ...
    \caption{antiskyrmion}
 \end{subfigure}
 \caption{Argument of the angular Fourier terms of soliton tails $c_M(r)$, for the skyrmion and antiskyrmion in a tilted field. We exclude those $c_M(r)$ where $\lvert c_M(r)\rvert/K_M(mr) <0.01$ for large $r$, as the error in the argument becomes $O(2\pi)$. Again, these should approach a constant value at large $r$, while at small $r$ deviation is expected. If the soliton respects the symmetry \eqref{Z2_symmetry}, then \eqref{gamma_phi0} further tells us what this constant value is up to $\pm\pi$. This graph thus acts as both a test of the linearised Euler-Lagrange equations approximation and a check that numerically found skyrmions and antiskyrmions in a tilted field have the reflection symmetry described.} 
 \label{gammadevs}
 \end{figure}
...
\begin{figure}[!ht]
  \centering
    ...
    \caption{Comparison of numerically observed elliptical instability of skyrmions (red crosses) and antiskyrmions (blue crosses) in a tilted field  \cite{Kuchkin20feb}, Instability predicted by two methods compared to numerics. The blue shaded area inside the dashed blue line is the region where domain walls have negative energy per unit length. The grey shaded area inside the solid black line is the region where the vacuum is linearly unstable. Numerical results for the values of $(h_z,\theta_h)$ at which onset of elliptical instability is seen are shown in crosses for $Q=\pm 1$ solutions. These are related by the transformation \eqref{flipping_hz}.}
...
\bee
V^{AB}(\bR;\phi^A,\phi^B)=2\int_{\partial\sigma^A} \bpsiB\cdot( \partial_i\bpsiA  + \bA_i\times\bpsiA) dS^i+ O(e^{-\frac{3}{2}mR}).
...
We can rewrite this in terms of the complex field $\psin=(\psin)_1 + i(\psin)_2$ and $a_i= \bA_i\cdot\bn_0$ as defined in \eqref{LinearEnergy} to get
...
Again, we see that this is just like the interaction energy without DMI, but with a gauge twist applied to the fields, and with a modulation representing the parallel transport between the two soliton centres with respect to the gauge field. Integrating by parts in either the bulk $\sigma^A$ or $\sigma^B$  and using the fact that $\tpsiAlin$, $\tpsiBlin$ solve \eqref{AsymptoticEL2} with corresponding effective sources $\trhoA$, $\trhoB$, we can write  \eqref{interaction_psitilde}  also as bulk integrals:
\begin{equation}
...
The expression \eqref{interaction_psitilde} is valid without constraint on $\bA_i$ and $V(\bn)$, provided the tails fall off appropriately, so could be used more generally. However, as discussed above, the solutions to the linearised Euler-Lagrange equations for general $V(\bn)$ are more complicated. 
...
\begin{figure}[!ht]
  ...
  \centering
    ...
    \caption{antiskyrmion}
  \end{subfigure}
    \caption{Plotting the numerically extracted multipole sources $C_M$ seen in the general solution of the linearised Euler-Lagrange equations  $\tilde{\psi}_{\text{lin}}=\sum_{M=-\infty}^\infty C_M e^{iM\phi}K_M(mr)$, $m=\sqrt{h_z-k^2\sin^2\theta_h}$ for solitons in a tilted field.}
    \label{complexqs}
\end{figure}
...
\includegraphics[width=\linewidth]{interaction_potential_colorbar.pdf}
  ...
  \caption{Analytical (a,b,c) and numerical\cite{Kuchkin20feb} (d,e,f) results for the interaction potential in a tilted field. Dashed lines represent the limits of the numerical antiskyrmion-antiskyrmion interaction data, which is the most restrictive.}
...
We also have an explanation for the unusual assymmetric lobe structure of the skyrmion-antiskyrmion interaction potential, by analogy to the Roget's palisade illusion \cite{Roget1824}, or rolling shutter effect \cite{Gregorcic12}, where rotational motion of a some object at angular speed $\omega$ is combined with the lateral motion of a camera shutter (at speed $v$) to produce an image where the object is distorted. This distortion can be described in terms of a map on polar co-ordinates \cite{Walsh13}:
...
\begin{figure}[!ht]
  ...
  \centering
   ...
   \caption{$a\mapsto 0$}
  \end{subfigure}
    \caption{The skyrmion-antiskyrmion interaction potential for different values of $a=k\sin\theta_h$, leaving $m$, $q^{Sk}_M$, $q^{ASk}_M$,$\gamma^{Sk}_M$, $\gamma^{ASk}_M$ constant. This models how the interaction potential will qualitatively change as the field tilt is reduced.}
    \label{reducing_tilt}
\end{figure}
...
\begin{figure}[!ht]
  ...
  \caption{Magnitude of angular Fourier term $c_1(r)$ relative to the Bessel function $K_1(mr)$, for the solution found by arrested Newton flow (PDE) and for the solution of the hedgehog Euler-Lagrange equations found by shooting (ODE), for the chiral magnet energy functional \eqref{Energy} with axisymmetric DMI \eqref{axisymmetricDMI} of strength $k=2\pi$ and axisymmetric potential \eqref{standard_potential} $h_z=0.8(2\pi)^2$, $h_a=0$.}
\label{untiltedtest}
 \end{figure}
...
There are other choices that we could make to construct $\bn^{AB}$. Firstly, our choice of soliton centre is somewhat arbitrary. A more natural choice might be the point $\bx_0$ at which $\bn^{AB}(\bx_0)=-\bn_0$, as this continues to be unique as we rotate the groundstate from $\be_3$ to $-\be_3$ but because we compare to numerical results using $-\be_3$ we stick to that definition. The choice of soliton centre can also be on a case-by-case basis for different solitons.  As discussed in the main paper, we assume that in general we can modify the constraints to specify between all the soliton configurations under consideration.
