...
In order to differentiate clearly between NCS that are only coupled through their use of a shared communication medium and MAS in which the agents additionally cooperate on a common goal, we refer to the former as non-cooperative NCS throughout this paper.
...
In short, an ETC scheme is called consistent if it provides a better performance level than the TTC scheme with the same average triggering rate.
...
Therefore, we extend our previous theoretical evaluation in \cite{Meister2022}, which does not take network effects into account, by the consideration of shared network effects, namely packet loss and transmission delays.
...
In Section~\ref{sec:setup}, we introduce the setup and formulate the considered problem.
After that, we present our theoretical results in Sections~\ref{sec:ttc_etc} and \ref{sec:mac} where we first derive cost expressions for TTC and ETC and then, quantify packet loss probabilities and expected transmission delays for the considered MAC protocols in order to contrast the derived cost expressions.
...
Each agent acts as a single-integrator perturbed by noise
\begin{equation}\label{eq:agent}
	\mathrm{d}x_i = u_i \mathrm{d}t + \mathrm{d}v_i
\end{equation}
starting in consensus, i.e., initial states $x_i(0)=0$ for all $i\in \lbrace 1,\dots,N\rbrace$, and with $v_i(t)\in \mathbb{R}$ referring to a standard Brownian motion and $u_i(t)\in\mathbb{R}$ to the control input.

Furthermore, we assume that the agents can continuously monitor their own state but trigger discrete transmission events.
As laid out in the introduction, we intend to compare TTC and ETC schemes for triggering transmissions.
For that purpose, we consider the cost functional
...
The agents are controlled with an impulsive control input
\begin{equation}\label{eq:input}
	u_i(t) = \sum_{j\in\mathcal{N}_i} \sum_{k\in\mathcal{A}^j_i} \delta(t-t^j_{k}-d^j_{k})(x_j(t^j_{k})-x_i(t^j_{k})),
\end{equation}
where $\mathcal{N}_i = \lbrace1,\dots,N\rbrace\backslash\lbrace i\rbrace$ is the set of neighbors of agent $i$ and $\mathcal{A}^j_i$ is the set of indices $k$ for arrived packets at agent $i$ sent by agent $j$.
In addition, $\delta(\cdot)$ refers to the Dirac delta function.
...
Let us introduce three core assumptions in order to determine the mechanisms behind packet loss and transmission delays within this work:
...
Lastly, we consider two ways of notation regarding the series of transmission events in this work:
On the one hand, we have already introduced the series of triggering time instants $(t^j_k)_{k\in\mathbb{N}}$ for agent $j\in\lbrace1,\dots,N\rbrace$.
On the other hand, we will also refer to the event series of the complete MAS with the notation $(t_k)_{k\in\mathbb{N}}$.
...
First, we state two facts about the considered problem that will turn out to be useful for the following analysis of the TTC and ETC schemes.
Similar to \cite{Rabi2009,Meister2022}, we find:
\begin{fact}\label{fact:0T}
	If the sequence of inter-event times $(t_{k+1}-t_k)_{k\in\mathbb{N}}$ is independent and identically distributed, it suffices to evaluate the cost over one time interval between two successful transmissions $\lbrack t^\mathrm{s}_k,t^\mathrm{s}_{k+1}\rbrack$:
	\begin{equation*}
	...
	\end{equation*}
	where $(t^\mathrm{s}_k)_{k\in\mathbb{N}}=(t_k)_{k\in\mathcal{A}}$ are determined by the respective triggering scheme introduced in Sections~\ref{sec:TT} and \ref{sec:ET}.
\end{fact}
\begin{pf}
...
\begin{fact}[\cite{Blind2013}, Lemma~35]\label{fact:ETs-ET}
	Suppose the expected time between two transmission attempts is $\E{T}$ and the packet loss probability is $p$.
...
We have therefore proven that the cost can be separated into a component induced by the transmission delays and one that neglects transmission delays.
Let us refine this separation further with the following lemma focusing on the cost induced by packet loss.
...
TTC involves periodic transmission events with a constant inter-event time $\TTT = t_{k+1}-t_{k} = \mathrm{const.}$ for all $k\in\mathbb{N}$.
...
In the analyzed problem setup, the resulting cost does not depend on the choice of the transmitting agent.
...
\begin{thm}\label{thm:cost_TT}
	Suppose agents \eqref{eq:agent} are controlled by the impulsive input \eqref{eq:input} with constant inter-event times $\TTT$.
...
In ETC, a triggering condition is continuously evaluated to determine when an agent's state is to be transmitted.
In order to achieve a desirable performance, the triggering condition is designed such that it indicates communication necessity in the considered setup.
As in \cite{Meister2022}, we define
\begin{equation}\label{eq:ET_cond}
	\lvert x_i(t)-x_i(t_{\hat{k}}) \rvert \geq \Delta,
\end{equation}
where $\hat{k} = \max \left\lbrace k\in\mathbb{N} \mid t_{k} \leq t \right\rbrace$ and $\Delta>0$, as the triggering condition.
Contrasting the deviation of the current state $x_i(t)$ of agent $i$ from its state at the last event $x_i(t_{\hat{k}})$ to a threshold $\Delta$ is quite common in a distributed setup since each agent can evaluate this condition with its local information, see, e.g., \cite{Dimarogonas2009}.
We choose the same threshold $\Delta$ for all agents since the contribution of each agent's state to the cost is equal.
Note that our triggering condition is analogous to the one used by \cite{Astrom2002}, \cite{Rabi2009}, \cite{Blind2013} but is of a distributed nature.
...
In the ETC case, the inter-event time is a stochastic variable and can be defined as a stopping time $\TET = \inf\lbrace t>0 \mid \exists i\in\lbrace1,\dots,N\rbrace: \lvert x_i(t)\rvert = \Delta \rbrace$, leveraging the argument from Fact~\ref{fact:0T}.
...
This allows us to apply Lemma~\ref{lem:loss_cost}, but the probabilistic nature of the inter-event times combined with the coupling between the agents prohibits the derivation of a closed form expression for the cost $\dbar{J}_\mathrm{ET}$.
...
\begin{lem}[\cite{Meister2022}]\label{lem:cost_ET}
	Suppose agents \eqref{eq:agent} are controlled by the impulsive input \eqref{eq:input} with inter-event times $\TET = \inf\lbrace t>0 \mid \exists i\in\lbrace1,\dots,N\rbrace: \lvert x_i(t)\rvert = \Delta \rbrace$.
...
It is therefore not possible to simply transfer the result from \cite{Astrom2002}, that ETC schemes outperform TTC, to this very similar setup.
...
The packet loss probability $p$ and the expected transmission delay $\E{d_k}$ depend on the specific MAC protocol.
We will therefore analyze TTC and ETC under simple MAC protocols forming the basis of more complex ones.
As \cite{Blind2013}, we analyze Time Division Multiple Access (TDMA) for TTC and pure ALOHA for ETC.
...
\begin{thm}\label{thm:ttc_tdma}
	Applying TTC with the TDMA MAC protocol to agents \eqref{eq:agent} with input \eqref{eq:input} yields the normalized cost
	\begin{equation*}
...
The maximum network load for which the deterministic scheme makes sense is $\rho=1$ since all packets are lost beyond that load.


\subsection{Pure ALOHA}
In the pure ALOHA protocol, each agent has access to the medium at all times.
If a packet is sent while another one is still in transmission, both packets are lost.
...
\begin{lem}\label{lem:p_PA}
	Suppose agents \eqref{eq:agent} are controlled by the impulsive input \eqref{eq:input} with ETC trigger condition \eqref{eq:ET_cond}.
	Then, the packet loss probability under the pure ALOHA communication protocol can be computed as
	\begin{equation*}
...
After quantifying the expected transmission delay and packet loss probability for ETC with the pure ALOHA protocol, we are now able to arrive at the normalized cost for this combination.

\begin{thm}\label{thm:etc_pa}
	Applying ETC with trigger rule \eqref{eq:ET_cond} and the pure ALOHA MAC protocol to agents \eqref{eq:agent} with input \eqref{eq:input} yields the normalized cost
	\begin{multline*}
	...
	\end{multline*}
	where we normalized by $\tau N(N-1)$ and $\rho = \tau/\E\TET$ denotes the network load.
\end{thm}
\begin{pf}
...
Although we are not able to arrive at a fully explicit expression for the cost since $\dbar{J}_\mathrm{ET}(\Delta=1)$ and $\E{\TET\mid\Delta=1}$ are not known explicitly from our derivation, we can still deduce some important properties from Theorem~\ref{thm:etc_pa}.
For example, combining the findings from Lemma~\ref{lem:cost_ET} and \cref{thm:ttc_tdma,thm:etc_pa} allows us to deduce that there exists a number of agents $N_0$ beyond which TTC with TDMA certainly outperforms ETC and pure ALOHA regardless of the network load.
...
Thus, $p_\mathrm{PA}(\rho=1) = 1-\Prob(\TET>\E{\TET})^2 = 3/4$ while $\tilde{J}_{\mathrm{ET,PA}} > \tilde{J}_{\mathrm{TT,TDMA}}$ certainly for $p_\mathrm{PA} > 1/3$.
...
In addition, we can utilize the simulation results and method from \cite{Meister2022} to compute $\tilde{J}_{\mathrm{ET,PA}}$ for various network loads and network participant numbers and contrast it to $\tilde{J}_{\mathrm{TT,TDMA}}$.
...
To be able to evaluate the normalized ETC cost, we rely on the simulation results from \cite{Meister2022} to quantify $\E{\TET\mid\Delta=1}$ and $\dbar{J}_\mathrm{ET}(\Delta=1)$ for various $N$.
In order to facilitate a fair comparison of TTC and ETC, we choose $\TTT=\E{\TET}$ which results in the same average number of transmission attempts for both triggering schemes.
...
Although not guaranteed by our theoretical statements, we observe a clear performance advantage of the ETC scheme for low numbers of agents $N$ and low network loads $\rho$.
...
Furthermore, the TTC scheme outperforms the ETC scheme for larger numbers of agents $N$ regardless of the network load and for large network loads $\rho\in(0,1\rbrack$ regardless of the number of agents.
...
We arrived at a cost structure for our particular setup that separates the performance of the triggering scheme under perfect communication from the induced cost of the network effects.
Moreover, we derived packet loss probability and expected transmission delay for two MAC protocols and related the resulting cost expressions on a theoretical level.
Finally, we evaluated the cost expressions numerically and with the help of previous simulation results for a varying number of agents and varying network loads.

This work highlights that TTC might outperform ETC in this particular setup for a sufficiently large number of agents or high network loads.
Thus far, it has not been clear whether the former result, also stated in \cite{Meister2022}, still holds when network effects are taken into account.
We have shown that this is the case for the considered setup and MAC protocols.
In addition, the latter result originates purely from network effects and demonstrates that they are non-negligible for performance evaluations of ETC and TTC.
On the contrary, the numerical evaluation also confirms the intuition of ETC being able to provide the same performance level as TTC while saving network resources for small numbers of agents and sufficiently high performance levels in the considered distributed setting.
