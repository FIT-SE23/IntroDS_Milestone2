...
\begin{document}
	...
	\begin{proof}[Proof of Proposition \ref{Prop: sharpness of CLT}]
		...
		Using the equivalence of $\mu$ to Lebesgue density and the uniform ellipticity assumption on $\sigma$, we have that for $f,g\in\F$,
		\begin{equation}\label{Eq: d_G LB, midpoint}
			\rho_{\G}(f,g) \gtrsim \langle \nabla L^{-1}[f-g],\nabla L^{-1}[f-g]\rangle_{\mu}. 
		\end{equation}
		...
		In combination with (\ref{Eq: d_G LB, midpoint}) and the smoothing estimate Lemma \ref{Lemma: 2-smoothing estimate}, we deduce that
		$\rho_{\G}(f,g) \gtrsim \|f-g\|_{H^{-1}}^2.$
