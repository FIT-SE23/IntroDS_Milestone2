...
\begin{equation}\label{*****}dX_t=(\Delta_\sigma-\lambda)X_t dt+b_1(X_t)dt+\sigma(X_t)dW_t,\end{equation} where $\Delta_\sigma$ is the Laplacian, $\lambda>0$, $b_1$ is bounded and Hölder continuous, and $\sigma$ is bounded Hölder and nondegenerate, existence of a Lyapunov functional can be verified by hand, for example one can take $V(x)=\|x\|+1,x\in H$, see the next paragraph. Even existence and uniqueness of invariant measures for these simple examples appear to be new in the literature.

For readers familiar with the use of Lyapunov functions in finite dimensional SDEs, it could be a bit confusing why such a choice $V(x)=\|x\|+1$ is a Lyapunov function for \eqref{*****}, and how to verify the Lyapunov condition in general for SPDEs. A technical challenge one may encounter is that when trying to use Itô's formula, the cylindrical noise will pump into infinite energy and the estimate simply produces infinity. The idea is to use the mild formulation of the SPDE \eqref{*****} instead, that is, we are using an $L^1$ theory rather than $L^2$ theory, and in this case the constant $\lambda>0$ confines the solution within bounded regions in expectation, which is the Lyapunov property one seeks. A detailed proof that $V$ has the Lyapunov property is given in Appendix \ref{appendixC}.
\end{remark}
...
\end{theorem}
\begin{remark}\label{remark112}

A simple example where all these assumptions are satisfied is 
$$dX_t=A X_t dt -\lambda X_t dt+(-A)^{1/2}F(X_t)dt+\sigma(X_t)dW_t,$$ where $\lambda>0$, $F$ is bounded and satisfies $\mathbf{H}_6$, $\sigma$ is bounded, uniformly non-degenerate and satisfies $\mathbf{H}_3'$. The Lyapunov function can be chosen as $V(x):=\|x\|+1.$ Again, the proof that this choice $V$ is a Lyapunov function can be found in Appendix \ref{appendixC}.\end{remark}


\subsection{Discussion and a review of related literature}


\subsubsection{Stochastic heat equation with Hölder coefficients}\label{1.5.1review}
...
A naive attempt to prove this proposition is to use the Burkholder–Davis–Gundy (BDG) inequality directly to the stochastic integral defining $Z(t)$, but as the term $S(t-s)$ in the integral is $t$-dependent, we only obtain an estimate for $Z(t)$ with $t$ fixed. In later applications we will consider stopping times, so we need a uniform estimate on all of $[0,T]$ and this naive argument is not enough. The right way out is to use a factorization technique that is common in the estimate of SPDEs.


\begin{proof} 
We recall the following factorization theorem, see \cite{da2014stochastic}, Theorem 5.10 and 5.11:\footnote{With a slight abuse of notation the symbol $\alpha$ used in this proof has different meaning from the $\alpha$ used elsewhere.} if for some $\alpha\in(0,\frac{1}{2})$, we have
$$\int_0^T t^{-2\alpha}\|S(t)\|_{2}^2dt<+\infty,$$
then defining 
$$W_A(t)=\int_0^t S(t-s)\Phi(s)dW_s,\quad t\geq 0,$$
$$Y_\alpha(t)=\int_0^t (t-s)^{-\alpha}S(t-s)\Phi(s)dW_s,\quad t\geq 0,$$
then we have the factorization 
\begin{equation}\label{WAT}
    W_A(t)=\frac{\sin\alpha\pi}{\pi}\int_0^t (t-s)^{\alpha-1}S(t-s)Y_\alpha(s)ds,\quad t\in[0,T].
\end{equation}
Under the assumption $\mathbf{H}_1$, we can choose any $\alpha\in(0,\frac{1}{4})$. The trick is now to bound $Y_\alpha(t)$ via the BDG inequality. Then the bound on $W_A(t)$ follows from direct integration. 

We introduce the semigroup $S^\lambda$ generated by $A-\lambda$. Now we replace $S$ by $S^\lambda$ in the definitions of $W_A$ and $Y_\alpha$, and apply the infinite dimensional BDG inequality  (see for example Theorem 4.37 of \cite{da2014stochastic}) to the process $Y_\alpha(t)$:

for any $p\geq 2$ there exists $C_p>0$ such that for every $t\geq 0$,
\begin{equation}\label{BDG}\mathbb{E}\left|Y_\alpha(t)\right|^p\leq C_p\left[\int_0^t (\mathbb{E}[\|(t-s)^{-\alpha}S^\lambda(t-s)\Phi(s)\|_2]^p)^{2/p}ds\right]^{p/2},\end{equation}  where $\|\cdot\|_2$ denotes the Hilbert-Schmidt norm on $H$.

Therefore we must compute
\begin{equation}\label{summabilityfinal}
\begin{aligned}&\int_0^t \|S^\lambda(t-s)\|_2^2 (t-s)^{-2\alpha}ds\\&=\int_0^t\sum_{n\geq 1}e^{-2(\lambda_n+\lambda)(t-s)}(t-s)^{-2\alpha}ds.
\end{aligned}\end{equation}
If we do not have the $(t-s)^{-2\alpha}$ factor we would have obtained an upper bound of the form $$ \sum_{n\geq 1}\frac{1}{2(\lambda_n+\lambda)}.$$ Now the idea is to set $\alpha>0$ sufficiently small and try go get something similar. We take the sum out of the integral in \eqref{summabilityfinal} and use Hölder's inequality to deduce:
$$\int_0^t e^{-2(\lambda_n+\lambda)(t-s)}(t-s)^{-2\alpha}ds\leq( \int_0^t e^{-2\omega(\lambda_n+\lambda)(t-s)}ds)^{1/\omega}(\int_0^t (t-s)^{-2\alpha\sigma}ds)^{1/\sigma}$$
where $\omega,\sigma>1$ and $\frac{1}{\omega}+\frac{1}{\sigma}=1.$ As long as we choose $\alpha\sigma<\frac{1}{2}$, the last integral is always finite, and the right hand side of \eqref{summabilityfinal} is bounded, modulo constants depending on $\alpha$, by:
$$\sum_{n\geq 1}\frac{1}{(\lambda_n+\lambda)^{1/\omega}}.$$
More precisely, $\alpha>0$ confines the choice of $\sigma$ via $\alpha\sigma<1/2$, which further confines the choice of $\omega$ via $\frac{1}{\omega}+\frac{1}{\sigma}=1.$ The choice of $\alpha>0$ shall be fixed at the end.

Now we use Young's product inequality\footnote{which says for $a,b\geq 0$, and $p,q>1$ such that $\frac{1}{p}+\frac{1}{q}=1$, we have $$ab\leq \frac{a^p}{p}+\frac{b^q}{q}$$} for the sum $\lambda_n+\lambda$. For any $\eta\in(0,1)$ we may find some universal constant $C_\eta>0$ depending only on $\eta$, such that 
$$\lambda_n+\lambda\geq C_\eta {(\lambda_n})^{1-\eta}\lambda^{\eta},$$ so the right hand side of \eqref{summabilityfinal} be bounded by, modulo some constants depending on $\alpha$,
\begin{equation}\label{summation*} \lambda^{-\frac{\eta}{\omega}}\sum_{n\geq 1}\frac{1}{(\lambda_n)^{\frac{1-\eta}{\omega}}}.\end{equation}
Now by choosing $\alpha>0$ small enough, we can make $\omega$ as close to 1 as we wish, so that $\frac{1-\eta}{\omega}>\frac{1}{2}$ and $0<\frac{\eta}{\omega}<\frac{1}{2}.$ 
The summation in \eqref{summation*} is finite by the summability assumption in $\mathbf{H_1}$. Moreover, any $\eta'\in(0,\frac{1}{2})$ can be represented as the form $\frac{\eta}{\omega}$ for some $\omega>1$ close to 1 and $\eta\in(0,\frac{1}{2})$, so we now write $\eta'$ in place of $\frac{\eta}{\omega}$ as the exponent of $\lambda$.
We thus
deduce that for each $\eta'\in(0,\frac{1}{2})$ and $p\geq 1$ we may find some constant $C_{p,\eta'}$ such that the right hand side of \eqref{BDG} is bounded by $$\mathbb{E}[Y_\alpha(t)\|^p\leq C_{p,\eta'}\|\phi\|_\infty^p\lambda^{-\frac{p\eta'}{2}},$$ where $C_{p,\eta'}$ is some universal constant depending only on $p$ and $\eta'$.
Moreover, this estimate holds uniformly for all $t\in[0,T]$.

Now we are finally in the place to estimate $W_A(t)$. In the expression \eqref{WAT}, since $\alpha>0$, $\int_0^t (t-s)^{\alpha-1}ds<\infty$. Moreover, $S(t-s)$ is a contracting semigroup. Thus we can use Jensen's inequality in \eqref{WAT} and deduce that, for each $p\geq 1,$ $\eta\in(0,\frac{1}{2})$ and $T>0$ we may find some universal constant $C{p,\eta,T}$ depending only on $p,\eta,T$, such that
$$\mathbb{E}\sup_{0\leq t\leq T} \|Z(t)\|^p\leq C_{p,\eta,T}\|\phi\|_\infty^p\lambda^{-\frac{p\eta}{2}}.$$
...
\begin{equation}\label{05.1} \|S(t)A^{1/2}\|=\sup_{k\geq 1}e^{-\lambda_k t} \lambda_k^{1/2}\leq \frac{d}{t^{1/2}},\end{equation}
...
\section{Verification of Lyapunov functions}

\label{appendixC}
In this appendix we prove that $V(x)=\|x\|+1$ can serve as a Lyapunov function for the SPDEs we consider, closing the gap in Remark \ref{remark111} and \ref{remark112}.
\begin{theorem}
Consider the SPDE 
$$dX_t=A X_t dt -\lambda X_t dt+b(X_t)dt+(-A)^{1/2}F(X_t)dt+\sigma(X_t)dW_t,\quad X_0=x,$$
where $A$ satisfies $\mathbf{H_1'}$, $\sigma$ satisfies $\mathbf{H}_3'$, and also $b$,$F$,$\sigma$ satisfy $\mathbf{H}_2,\mathbf{H}_4,\mathbf{H}_5,\mathbf{H}_6$. 

Assume further that $b$, $F$ and $\sigma$ are uniformly bounded. Fix $\lambda>0$. 

Then $V(x):=\|x\|+1$ is a Lyapunov function, in the sense that we may find $T_0>0$, $M<\infty$ and $c\in(0,1)$ such that 
\begin{equation}
\label{lyapunov*}    
\mathbb{E}_x V(X_{T_0})\leq cV(x)+M,\quad \text{for all }x\in H.\end{equation}
\end{theorem}

\begin{proof}
Denote by $(S^\lambda(t))_{t\geq 0}$ the semigroup generated by $A-\lambda$. Since $A$ is a negative operator, necessarily we have $\|S^\lambda(t)\|\leq e^{-\lambda t}$. Writing the SPDE in its mild formulation as follows:
\begin{equation}\begin{aligned}
    X_t&=S^\lambda(t)x+\\&\int_0^t S^\lambda(t-s)b(X_s)ds+\int_0^t S^\lambda(t-s)(-A)^{1/2}F(X_s)ds+\int_0^t S^\lambda(t-s)\sigma(X_s)dW_s.\end{aligned}
\end{equation}
For the first term, we use $\|S^\lambda(t)x\|\leq e^{-\lambda t}\|x\|$. For the remaining three terms in the second line, we use the boundedness of $b$, $F$ and $\sigma$, boundedness of the semigroup $S^\lambda(t)$, as well as estimate \eqref{05.1} or \eqref{burgersusefulestimate}, to deduce that they are all bounded in expectation for any $t\in[0,T_0]$.

Therefore, we can choose any $T_0>0$ in \eqref{lyapunov*}, then choose $c=e^{-\lambda T_0}$, and choose some $M<\infty$ depending only on $T_0$ and the supremum norms of $b$, $F$ and $\sigma$ so that \eqref{lyapunov*} holds for all $x\in H$.
\end{proof}

\section*{Acknowledgements}
The author thanks Professor James Norris for helpful suggestions on this paper. The author is also grateful to an anonymous referee who pointed out a mistake in the first version of the paper, regarding the proof of Proposition \ref{stochasticinte}.
