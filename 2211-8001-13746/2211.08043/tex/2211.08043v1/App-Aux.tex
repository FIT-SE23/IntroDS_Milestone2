...
In this appendix, we provide a series of helper lemmas and auxiliary results that we use repeatedly our paper.

%----------------------------------------------------------------------
%%% NUMERICAL SEQUENCES
%----------------------------------------------------------------------
\para{Lemmas on numerical sequences}
The first two results we provide concern numerical sequences:

\begin{lemma}
%[\citeauthor{Pol87}, \citeyear{Pol87}, p.~46]
%[\citeauthor{Pol87}]
\label{lem:Polyak}
Consider two sequences of non-negative real numbers $\curr[\seq], \curr[\diff] \geq 0$, $\run=\running$, such that
\begin{equation}
\next[\seq]
	\leq \curr[\seq]
...
\end{equation}
As a first consequence, if $\init[\seq] \leq \eps$, \cref{lem:Polyak} readily implies that $\curr[\seq]$ converges to zero and that $\curr[\seq] \leq \eps$ for all $\run$.
...
In particular, it is then valid to consider the sequence $\curr[\seq]^{-\rexp}$, $\run = \running$, for which we get
\begin{align}
...
%%% BREGMAN
%----------------------------------------------------------------------
\para{Estimates for Bregman proximal steps}
...
\begin{proof}
Our proof follows \cite[Proposition B.3]{MLZF+19}, but with a slight modification to account for the extra term involving $\dbase\in\pcone(\base)$.
...
\begin{proof}
Our proof follows \citep[Proposotion B.4]{MLZF+19}, again with a slight modification to account for the extra terms involving $\dbase\in\pcone(\base)$.
...
\breg(\base,\new_{2})
	...
	&\leq \breg(\base, \point)
		...
		- \breg(\new_{2},\point)
\end{align}
To lower bound $\breg(\new_{2}, \point)$, we invoke again \cref{lem:onestep} with $\base \gets \new_{2}$ and $\new_{1} = \mprox_\point(\dpoint_{1})$;
this readily gives
\begin{equation}
\breg(\new_{2},\new_{1})
	\leq \breg(\new_{2},\point)
...
\end{align}
holds.
\Cref*{itm:inactive} corresponds to the situation where such coeffeicients indeed exist while \cref*{itm:active} holds when this is not possible.
...
Otherwise, for all $(\coef_\coord)_{\coord \in \actcoords\setminus\coords} \in (\R_{+})^{\actcoords\setminus \coords}$ \emph{not all zero} and $(\coefalt_\coord)_{\coord \in \coords} \in \R^{\coords}$, \eqref{eq:inclusion} does not hold.
...
Rescaling this solution $\pvec$ and setting $\polycst$ to $\max(\norm{\pvec}, \norm{\pvec}_\infty)$ then gives the second case.
\end{proof}