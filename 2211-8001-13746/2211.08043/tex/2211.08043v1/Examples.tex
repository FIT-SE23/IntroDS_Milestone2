...
In this section, we take a closer look at some commonly used Bregman regularizers and the induced prox-mappings with the goal of determining the rate of convergence of the associated Bregman proximal method.
%Our goal throughout will be to determine the rate of convergence of Bregman proximal methods as a function of the underlying regularizer.
%\smallskip
For concreteness, we will focus on one-dimensional problems where $\points$ is a closed interval of $\R$ (possibly infinite), and $\vecfield$ is the affine operator
\begin{equation}
\label{eq:simple}
\vecfield(\point)
	= \point - \sol,
...
%\footnote{Of course, $\sol$ satisfies \eqref{eq:strong} with $\strong=1$ and $\basin=\points$.}
Moreover, for ease of presentation and notation, we will only examine the \acl{MD} recursion \eqref{eq:MD} with constant step-size schedules of the form $\curr[\step] \equiv \step$ for some $\step>0$.
In this case, we obtain the general recursive scheme
\begin{equation}
\label{eq:MD-generic}
\next
	= \fixmap(\curr)
...
\end{equation}
and we will examine the rate of convergence of $\curr$ to $\sol$ by analyzing the behavior of $\fixmap$ near $\sol$.
\smallskip
...
\label{ex:Eucl}
We begin with the quadratic regularizer $\hreg(\point) = \point^{2}/2$ for $\point\in\points$.
Concretely, taking $\points = [0,\infty)$ and noting that $\hreg'(\point) = \point$, we have:
\begin{flalign}
...
\end{flalign}
%\begin{enumerate}
%[left=.5em,label={\itshape\alph*\hspace*{1pt}\upshape)}]
%\item
%The prox-domain of $\hreg$ is $\proxdom=\points$.
%\item
...
%\shortintertext{%
%\item
%The induced prox-mapping is the Euclidean projector}
%\label{eq:prox-Eucl}
%\proxof{\point}{\dvec}
%	&= \pospart{\point+\dvec}
%\end{align}
...
In this case, we have $\hreg'(\point) = 1 + \log\point$, which gives the following:
\begin{flalign}
\label{eq:mirror-ent}
\begin{alignedat}{3}
\quad
...
%[left=.5em,label={\itshape\alph*\hspace*{1pt}\upshape)}]
%\item
%The prox-domain of $\hreg$ is $\proxdom = \relint\points = (0,\infty)$.
%\item
%The associated Bregman divergence is
%\begin{align}
%\label{eq:Breg-ent}
...
%\PM{There was a problem with the refs here \textendash\ fixed.}
...
By means of this lemma (which we prove in \cref{app:aux}), we conclude that the iterates of \eqref{eq:MD-ent} converge to $0$ at a rate of
\begin{equation}
\tag*{\endenv}
\breg(\sol,\curr)
	= \curr
	...
	\sim 1/(\step\run).
\end{equation}
%or, in terms of the actual iterate values, $\abs{\curr - \sol} = \Theta(1/(\step\run))$.
%\hfill
%\endenv
\end{example}
...
For this choice of regularizer, we have $\hreg'(\point) = (1 - \qexp\point^{\qexp-1}) / [\qexp(1-\qexp)]$, and a series of straightforward calculations gives the following:%
\footnote{Strictly speaking, the expression we provide for $\proxof{\point}{\dvec}$ is only valid when $\dvec < \point^{\qexp-1}/(1-\qexp)$.
The reason for this is that the fractional power regularizer is not strongly convex over $[0,\infty)$, so the corresponding prox-mapping $\proxof{\point}{\dvec}$ is not well-defined for all values of $\dvec$.
...
\fixmap(\point)
	...
	%= \point \bracks{1 - \step\point^{2-\qexp} + o(\point^{2-\qexp})}
	= \point - \step\point^{3-\qexp} + o(\point^{3-\qexp})
	%\sim \point - \step\point^{3-\qexp}
	\quad
	\text{as $\point\to0$}.
%	\text{for small $\point$}.
\end{equation}
...
\end{equation}
%Alternatively, since $\breg(0,\point) = \point^{\qexp}/\qexp$, the rate of convergence of \eqref{eq:MD-frac} in terms of the Bregman divergence is
%\begin{equation}
%\breg(\sol,\curr)
%	\sim \frac{\const}{\run^{\qexp/(2-\qexp)}}
%\end{equation}
%for some constant $\const>0$ that depends on $\step$.
%\hfill
%\endenv
\end{example}
...
%\begin{remark*}
%Strictly speaking, the regularizers of \cref{ex:ent,ex:frac} are not $1$-strongly convex over $\points = [0,\infty)$, so they do not formally satisfy the requirements of \cref{def:Bregman}.
%There are several ways to overcome this limitation\PM{Weird, where did the rest of this phrase go?}
%\PM{Should probably merge this phrase with the footnote \textendash\ will do this when I'm back from vacation.}
%\FI{Up ?}
%\end{remark*}
...
On the other hand, if we consider the shifted operator $\vecfield(\point) = \point+1$, a somewhat tedious calculation (which we detail in \cref{app:aux}) gives the following Taylor expansion near $\sol = -1$:
\begin{equation}
\fixmap(\point)
% 	\sim \sol
% 		+ (\point - \sol)
% 		- 2\sqrt{2}\step (\point - \sol)^{5/2}.
			= \sol
		+ (\point - \sol)
...
The Euclidean and shifted Hellinger regularizers lead to a geometric rate of convergence (see left figure);
all other examples converge at a polynomial rate, and we provide the theoretically computed rate for comparison (dashed lines in the figure to the right).
}
\label{fig:examples}
\vspace{-2ex}
\end{figure}
...
Specifically, they show that the Bregman divergence induced by a given regularizer may exhibit a drastically different behavior at the boundary of $\points$:
when $\sol$ is a boundary point, $\breg(\sol,\point)$ could grow
...
%in the Euclidean case, we have $\breg(\sol,\point) = \Theta(\norm{\point - \sol}^{2})$,
%whereas in the entropic case, we have $\breg(\sol,\point) = \Theta(\norm{\point - \sol})$ when $\sol$ is a boundary point of $\points$.
As a result, when used as a measure of convergence, it is important to rescale the Bregman divergence accordingly in order to avoid inflating \textendash\ or \emph{deflating} \textendash\ an algorithm's rate of convergence. 
...
We will make this relation precise in the next section.