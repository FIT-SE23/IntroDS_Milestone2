...
Accordingly, the quantity of interest $\state_{3,\run} / \state_{2,\run}$ can be bounded as
\begin{align}
...
Hence, after telescoping, we conclude that 
\begin{equation}
\frac{\state_{3,\run}}{\state_{2,\run}}
	\leq 2 \step \run + o(\run),
\end{equation}
which in turn shows that $\state_{2,\run} \sim \state_{2,\run} / \state_{3,\run} = \Omega(1/\run)$, as claimed.
\endenv
\end{example}
