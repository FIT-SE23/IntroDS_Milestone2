...
For concreteness, we focus on one-dimensional problems where $\points$ is a closed interval of $\R$ and $\vecfield$ is the affine vector field
\begin{equation}
\label{eq:simple}
\vecfield(\point)
	= \point - \sol,
...
$\hreg'(\point) = 1 + \log\point$, which gives % the following:
\begin{flalign}
\label{eq:mirror-ent}
\begin{alignedat}{3}
\quad
...
Thanks to this lemma (which we prove in \cref{app:aux}), we readily conclude that $\curr$ converges to $0$ at a rate of
%\begin{equation}
$\breg(\sol,\curr)
	= \curr
	= \abs{\curr - \sol}
...
The reason for this is that this %fractional power 
regularizer is not strongly convex over $[0,\infty)$, so the %corresponding 
prox-mapping $\proxof{\point}{\dvec}$ is not well-defined for all values of $\dvec$.
...
On the other hand, if we consider the shifted operator $\vecfield(\point) = \point+1$, a somewhat tedious calculation (which we detail in \cref{app:ex}) gives the following Taylor expansion near $\sol = -1$:
\begin{equation}
\fixmap(\point)
% 	\sim \sol
% 		+ (\point - \sol)
...
%and we provide the theoretically computed rate for comparison (dashed lines in the figure to the right).
}
\label{fig:examples}
\vspace*{-2ex}
\end{figure}
...
Specifically, in terms of absolute values, %(or norms), 
we observe a
