...
\Crefrange{ex:Eucl-sharp}{ex:frac-sharp} show that the convergence rate of \eqref{eq:BPM} can change drastically depending on whether $\vecfield(\sol)$ is zero or not.
...
To simplify the presentation, we identify $\pspace$ with $\R^{\nCoords}$ endowed with the Euclidean scalar product $\braket{\cdot}{\cdot}$, and we do not distinguish between primal and dual vectors (meaning in particular that the distinction between normal and polar cones are likewise blurred).

Formally, we consider polyhedral domains written in normal form as
\begin{equation}
\label{eq:polyhedron}
\points
...
With all this in mind, we will distinguish the following solution configurations (see also \cref{fig:simplex}).
...
First, if $\lim_{\point\to0^{+}}\hker'(\point) = -\infty$, it is straightforward to see that $\dom\subd\hreg = \relint\points$ so, by \cref{lem:mirror} in \cref{app:aux}, the iterates $\curr$ of \eqref{eq:BPM} will remain in $\relint\points$ for all $\run$;
...
The proof of \cref{lem:separation} is based on Farkas' lemma so we relegate it to \cref{app:aux}.
...
Since the number of active constraints is finite, the progressive addition of these indices will allow us to reach $\coords_{\sharp} = \sharps$, thus proviing our initial claim.
...
Hence, \cref{lem:mirror} guarantees
\begin{equation}
...
\begin{enumerate}
[left=0em,label={\bfseries Case 1.\arabic*:}]
\item
...
\end{align}
Taking a scalar product between the last inclusion of \cref{lem:mirror} and $\pvec$, we get that,
\begin{align}
...
We may therefore iteratively add coordinates of $\actcoords$ for which $ \state_{\coord,\run} = 0$;
