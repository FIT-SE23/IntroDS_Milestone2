...
Foundation for Research, Technology and Development, Austria.}
...
\address[TECMM]{Instituto Tecnológico José Mario Molina Pasquel y Henríquez, Unidad Académica Zapopan, Jalisco, Mexico.}
...
For $c > 2$, define a sequence $\{b_k\}_{k=0}^N$ as
  \begin{align*}
    b_k = 
    \begin{cases}
      \max\left\{\underline{b}, \dfrac{c|x(t_k)|}{|\bar u_k| \Delta t_k} \right\} &\text{if }\bar u_k \neq 0,\\
...
\begin{lem}
    ...
    Consider the scalar system~\eqref{eq:sc-pert} with $b(x,t) \equiv 1$ and $f$ satisfying Assumption~\ref{as:practical}, under sampling and zero-order hold as per~\eqref{eq:zoh-law}, with sampling instants satisfying $t_{k+1} = t_k + \Delta$ for some fixed $\Delta > 0$ and all $k \in \{0,\ldots,N-1\}$. Suppose that $\lim_{|x|\to\infty} \psi(x)/|x| = \infty$. Then, there exists $M>0$ such that $|x(t_k)| \ge M$ implies $|x(t_{k+1})| \ge |x(t_k)|$.
\end{lem}
\begin{pf}
    Select $M>0$ such that $\psi(x)/|x| \ge 3/\Delta$ for all $|x| \ge M$ and let $|x_k| \ge M$. Distinguish the cases $|\bar u_k| \ge 2|x_k|/\Delta$ and $|\bar u_k| < 2|x_k|/\Delta$. In the first case, set $f\equiv 0$, causing $|x_{k+1}-x_k| = |\bar u_k| \Delta \ge 2|x_k|$. Otherwise, set $f=\psi(x_k)\sign(x_k)$, causing $x$ to increase in absolute value since $|f|= \psi(x_k)\geq 3|x_k|/\Delta>|\bar{u}_k|$. Moreover,  $|x_{k+1}-x_k| \ge (\psi(x_k) - |\bar u_k|) \Delta \ge |x_k|$. In any case, then $|x_{k+1}| \ge |x_k|$.
    \qed
\end{pf}
...
Therefore,
\begin{multline*}
  -\varepsilon \le |x(t_k)|- \varepsilon \le \\
...
\bibliography{Predefined_Sampling}
