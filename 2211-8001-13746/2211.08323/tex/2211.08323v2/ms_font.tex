...
SOC-predicted power law slope $\alpha_F=1.80$ for mean fluxes $F$, 
...
(SOC)} systems is applicable (Aschwanden 2011; 2014; Aschwanden et al.~2016;
...
Gaussians and power laws are both nonlinear functions of the independent
variable, but the fundamental distinction lies with the
generator of the statistics: Gaussian statistics reflect the
...
The flux $F_{\rm IRIS}$ of an IRIS event is defined by, 
\begin{equation}
	F_{\rm IRIS} = {4\ \pi\ f \ E_{\lambda}\ k \over A\ \Omega} , \quad  
	[{\rm erg}\ {\rm cm}^{-2} {\rm s}^{-1}] 
\end{equation}
where $f$ is the observed flux in [DN/s] (data number per second), 
...
grid, i.e., $F_{i,j} = F(x_i,y_j), i=0,...,n_x, j=0,...,n_y$, 
...
\end{equation}
Adding the areas $A$ and fluxes $F$ of all $k$ avalanche
components, we obtain then the following total flux 
$F_{\rm tot,pix}$, 
\begin{equation}
	F_{\rm tot,pix} = \Sigma_{i=1}^{n_x} \Sigma_{j=1}^{n_y} \ 
	F_{i,j}^{-\alpha_F} \ \Delta x^2 \ ,
\end{equation} 
which can be set equal to the value of $F_{\rm tot}$ of the
standard method (Eq.~4) and proves this way that the power
law slopes $\alpha_F$ of the two methods are identical.
...
$\alpha_{\rm F1}$ listed in Table 1 and Fig.~4.
...
(Aschwanden and  Vilangot Nhalil~2022). Curiously, events
...
In essence, we find 5 datasets (\# 6-9, 11)
...
In the following we discuss an incoherent random processe
...
{\sl The interpretation of granulation as the cause of the 
Gaussian "noise" in IRIS data is controversial. The intensity
measured by IRIS 1400 SJI in non-magnetic areas (i.e., no network
or plages) has no contribution from the photosphere -
it is either continuum created in the mid-chromosphere 
(Vernazza et al 1981; VALC model), or the emission from long
fibrils that outline the magnetic canopy (Rutten et al.~2020), 
or narrow band images of CaII 8542 core (observed
with IBIS or CRISP). No convective signal is left at these
heights and densities; at most, there might be some brightenings due
to acoustic shocks, resulting from vertical propagation of p-modes
(Martinez-Sykora et al 2015), but the scale of granulation 
is not there anymore. As a possible proof of this argument,
Fig.~4 shows that the Gaussian fit is performed on flux
values that are vastly different, up to an order of magnitude
(less than 10 erg/s for set \# 6, 7, 11; around 100 erg/s in
sets \# 3, 10); if granulation were the responsible process,
the low values well fitted by a Gaussian would be of similar
amplitude (private communication with anonymous reviewer.)}
...
over an arbitrary FOV (private communication with 
anonymous reviewer).
...
\item{ 
Tables 1 and 2 show the following characterizations 
of the 12 selected datasets: 
4 cases with sunspots,  
5 cases that have a max flux $<50$ DN/s, 
...
($a_F=1.80$) is only fulfilled in the absence of sunspots,
for small maximum fluxes, and for magnetic flux balance.
...
\ref{Verbeeck, C., Kraaikamp, E., Ryan, D.F., and Podladchikova, O. 2019,
 	...
 	ApJ 884, 50}
\ref{Vernazza,J.E., Avrett,E.H., and Loeser,R. 1981,
 	{\sl Structure of the solar chromosphere III. models of the 
	EUV brightness components of the quiet sun},
 	ApJS 45, 635.}
...
the separator flux $F_2$, the maximum flux $F_{max}$, and the
magnetic flux balance ratio $q_{pos}$.
...
a critical value of $F_{max} \lapprox 50$ DN/s. The values 
...
\hline
\hline
Number	& Phenomenon 	& Power law 	& Agrees with	  & Separator            & Maximum   & Max.flux   & Magnetic flux   \\
Dataset	& 1400 \ang     & slope fit     & prediction      & flux                 & flux      & criterion  & balance  \\
IRIS	&		& $\alpha_{\rm F}$ & $\alpha_{\rm F} \approx 1.8$ & $F_2$          & $F_{max}$ & $<50$ DN/s & $q_{pos}$ \\
\#	&		&  	        &                 & [DN/s]      & [DN/s]&            &           \\
\hline
\hline
1       & Sunspot       & (1.51$\pm$0.04) & NO            &  21  	& 121 & NO  & 0.47 \\
2       & Sunspot       & (1.23$\pm$0.02) & NO            &  32         & 190 & NO  & 0.81 \\
3       & Sunspot       & (2.13$\pm$0.06) & NO            & 128         & 243 & NO  & 0.68 \\
4       & Plage         & (0.94$\pm$0.02) & NO            &  20         & 108 & NO  & 0.64 \\
5       & Plage         & (1.02$\pm$0.01) & NO            &  36         & 199 & NO  & 0.76 \\
6       & Plage         &  1.59$\pm$0.02  & YES           &  17         &  50 & YES & 0.54 \\
7       & Plage         &  1.59$\pm$0.03  & YES           &   9         &  26 & YES & 0.37 \\
8       & Plage         &  1.92$\pm$0.03  & YES           &   8         &  28 & YES & 0.25 \\
9       & Plage         &  1.81$\pm$0.01  & YES           &  13         &  42 & YES & 0.36 \\
10      & Sunspot       & (1.25$\pm$0.02) & NO            & 120         & 501 & NO  & 0.90 \\
11      & Plage         &  1.61$\pm$0.02  & YES           &   9         &  31 & YES & 0.34 \\
12      & Plage         & (1.40$\pm$0.05) & NO            &  22         &  54 & NO  & 0.39 \\
\hline
        & Observations  & 1.70$\pm$0.15   &               &             &               &       & 0.54$\pm$0.21 \\       
        & Theory        & 1.80            &               &             &               &       & 0.50  \\
\hline
\end{tabular}
\end{center}
...
the power law slope $\alpha_F$ of the flux distribution,
the separator flux $F_2$,
the maximum flux $F_{max}$, and the fractal dimension $D_A$. Note
...
\hline
Number	& Phenomenon 	& Power law 	& Matching	  & Separator            & Magnetic  & Matching & Matching & Fractal \\
Dataset	&  		& slope fit     & prediction      & flux                 & field     & balance  & balance  & dimension \\
HMI	&		& $\alpha_{\rm F}$ & $\alpha_{\rm F}\approx 1.8$ & $F_2$       &   & $q_{pos} $ & $q_{pos}\approx 0.50$ & $D_A$\\
