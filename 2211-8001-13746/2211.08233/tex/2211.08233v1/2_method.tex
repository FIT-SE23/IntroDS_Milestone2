...
The DC Conv is then followed by a batch normalization, a ReLU activation function, and a spatial dropout.
...
Formally, for the $\vec{\mathcal{T}}_{j+1}$ in the forward direction with the input $\vec{\mat{F}}_{j}$ from previous TAB, the output $\vec{\mat{F}}_{j+1}$ is given by Equation~(1):
\begin{align}
    \vec{\mat{F}}_{j+1}= \mathcal{A}(\vec{\mat{F}}_{j})\odot \vec{\mat{F}}_{j},\\
    \cev{\mat{F}}_{j+1}= \mathcal{A}(\cev{\mat{F}}_{j})\odot \cev{\mat{F}}_{j},
\end{align}
where $\vec{\mat{F}}_{0}$ comes from the output of the first $1\times 1$ Conv layer and the backward direction can be defined similarly in Equation~(2).
...
\begin{align}
    \vct{g}_j&=\mathcal{G}(\vec{\mat{F}}_{j})+\mathcal{G}(\cev{\mat{F}}_{j}) \label{equ:GAP_feat},
\end{align}
