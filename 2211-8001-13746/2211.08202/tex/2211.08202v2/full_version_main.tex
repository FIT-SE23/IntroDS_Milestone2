...
\author{Simon Wietheger\setcounter{footnote}{6}\thanks{Hasso Plattner Institute, Potsdam, Germany} \and Benjamin Doerr\thanks{Laboratoire d'Informatique (LIX), CNRS, \'Ecole Polytechnique, Institut Polytechnique de Paris, Palaiseau, France}}
...
Later, the runtimes of more realistic algorithms have been studied mathematically \cite{BrockhoffFN08,NguyenSN15,LiZZZ16,HuangZCH19,HuangZ20%JV
}.
...
Regarding the normalization step, we do not consider the procedure as given in \cite{Deb_Jain_2014} but the improved and more detailed normalization given in \cite{Blank_Deb_Roy_2019} by one of the two original authors among others.
...
Further, for each objective $j$ an extreme point in that objective by using a achievement scalarization function. 
Consider the hyperplane spanned by these points.  
The intercepts of this hyper plane with the coordinate axes give the Nadir point estimate $\hat{z}^{nad}$.
In case that $H$ is not well-defined by the extreme points or if an intercept is either smaller than a given positive threshold $\epsilon_{nad}$ or larger than the highest observed value over all generations in that objective, $\hat{z}^{nad}$ is instead defined by the maximum value in each objective of individuals in the first non-dominated front.
Last, for each objective in which the Nadir point estimate is smaller than the ideal point estimate plus the threshold $\epsilon_{nad}$, the maximum value in that objective over all current non-dominated fronts is used for the Nadir point estimate in that objective instead.
The normalized objective functions $f^n$ are now defined as 
 \begin{equation}\label{eq:normalization}
     f_j^n(x) = \frac{f_j(x)-\hat{z}_j^*}{\hat{z}_j^{nad}-\hat{z}_j^*}
 \end{equation} 
 for $j\in \{1,\ldots,M\}$.
Algorithm~\ref{alg:normalize} formalizes the normalization procedure, for a more detailed description see \cite{Blank_Deb_Roy_2019}.
%
\begin{algorithm2e}[h]%
\Input{%
...
\If{$valid = False$}{ 
    \lFor{$j'=1$ \KwTo $M$}{$\hat{z}_j^{nad} = \max_{x\in F_1} f_j(x)$}
}
\For{$j=1$ \KwTo $M$}{
    \lIf{$\hat{z}_j^{nad} < \hat{z}_j^* + \epsilon_{nad}$}
       {$\hat{z}_j^{nad} = \max_{x\in F_1\cup\ldots\cup F_k} f_j(x)$}
}
Define $f^n_j(x) = (f_j(x)-\hat{z}_j^*) / (\hat{z}_j^{nad}-\hat{z}_j^*) \quad \forall x\in\{0,1\}^n, j\in\{1,\ldots,M\}$
...
%\simon{What about ties? Not clear in \cite{Deb_Jain_2014}}
...
For each reference point $r\in R$, let $\rho_r$ denote the number of individuals in $Z_t$ associated with $r$
...
For our analyses, we assume that the \NSGA employs a set of structured reference points in the normalized hyperplane as proposed by Deb and Jain \cite{Deb_Jain_2014}.
...
\begin{figure}
    \centering
    \includegraphics[width=.5\textwidth]{images/DebJain2014_Fig1_referencePoints.PNG}
    \caption{Structured set of reference points for 3 objectives with $p=4$ divisions ([Deb and Jain, 2014, Figure~1])}
...
\begin{lemma}\label{lem:normalization}
    ...
    Let for each objective value $i$, $z_i^{\min}$ and $z_i^{\max}$ be the minimum and maximum value in the current population.
...
\begin{proof}
   ...
   Further, $\colvec{z_1^{\max}\\z_2^{\max}\\z_3^{\max}}$ is the Nadir-point estimate $\hat{z}^{nad}$.
...
\begin{proof}
    ...
    To show the statement of the lemma, we (i) bound from above the angle between normalized objective values and their nearest reference point, (ii)~bound from below the angle between two different normalized objective values, and (iii)~use these estimates to show that for $p \ge 21n$ the latter is more than twice as large as the former.
    Thereby, individuals with different objective values are never associated with the same reference point.
...
Consider the \NSGA with population size $N \ge (\frac{n}{2}+1)^2$ and $n$ divisions along each objective optimizing \threeOMM. 
...
By Lemma~\ref{lem:notLooseSolution} the population will never loose a sampled objective value, so $d_i$ never decreases.
...
\begin{figure}
    \centering
    \includegraphics{images/coverage_3OMM_BS20.pdf}
    \caption{The number of covered points on the Pareto front per iteration of the NSGA-III and the NSGA-II with different population sizes when optimizing the 3-OMM benchmark. Three independent runs are depicted for each algorithm/population size. All algorithms only use mutation for the reproduction step. The lines for the NSGA-III (blue) stop after the iteration it has first covered the complete Pareto front.}
    \label{fig:coverages}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics{images/3OMM_N1.pdf}
...
In this first mathematical runtime analysis, we proved that the \NSGA (with suitable parameters) when optimizing the \threeOMM problem never loses a Pareto optimal solution value from its population. This is in drastic contrast to the \NSGAtwo, which provably also with large populations regularly suffers from such losses and consequently cannot find (or approximate below a certain constant factor) the Pareto front of \threeOMM in sub-exponential time~\cite{ZhengD22arxiv}.
From this positive property of the \NSGA, we derive that it finds the full Pareto front of \threeOMM in an expected number of $O(n \log n)$ iterations. Our experimental results confirm the drastically different behavior of the algorithms.

From the mathematical proofs we are optimistic that the key property of not losing desired solution values extends to broader classes of problems. In fact, our main argument was that the angle (with the origin) between any two different solutions of the normalized population is large enough to prevent that both are associated with the same reference point. We have not used any properties of the optimization process in this proof, but only the structure of the set of objective values of the problem. We are thus optimistic that similar properties can be shown for other optimization problems, say with integral objectives of a bounded range. 

This one first rigorous result on the \NSGA clearly is not enough to derive a reliable advice on which MOEAs to prefer, but we believe that it should motivate more algorithm users so far content with the \NSGAtwo to also try the \NSGA. If only part of the drastic differences proven here extend to broader classes of problems, this switch is definitely justified. 
