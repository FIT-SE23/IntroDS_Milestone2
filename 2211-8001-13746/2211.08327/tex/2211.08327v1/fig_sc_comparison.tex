...
\end{axis}
\end{tikzpicture}
\caption{The algorithm \gls{wmmse} is used as the baseline for benchmarking \gls{sc} methods for optimization, with $|\KK|=|\KU|=50$. The two commonly used methods are \gls{sc} (Free) and \gls{sc} (Conv), respectively the unconstrained and constrained version of~\eqref{eq:sc}. The method \gls{sc} center uses the center point of the convex hull for estimation of all units. The method \gls{sc} (Direch) generates a random Dirichlet distribution as the coefficients $\bm{\beta}$ such that a random point is selected inside the convex hull in each algorithm iteration step.}
