...
%Revenge of the Minions: Robust Promise CSPs}
\author{}
...
%\vg{Say we believe these are all the tractable cases?} \vg{Can we do threshold in addition to Majority?} 
...
\newpage
\setcounter{tocdepth}{2} % https://tex.stackexchange.com/questions/63422/help-removing-subsection-and-subsubsection-numbers-from-table-of-contents-toc
\tableofcontents
\pagenumbering{roman}
\newpage
...
%\footnote{For CSPs, this is equivalent to solvability by $O(1)$ rounds of the Sherali Adams LP hierarchy.}
\iffalse
These algorithms are based on the existence of locally consistent assignments. We start with an assignment to a set $S$ containing $l$ variables that satisfies all the constraints fully contained in $S$. Then for any set $S'$ that intersects with $S$ in at most $k$ variables, we can extend the existing assignment in $S \cap S'$ to find a new assignment to the variables in $S'$ that satisfies all the constraints in $S'$. If the instance is satisfiable, we can repeat the process indefinitely. However, for certain CSPs, including the above-mentioned Horn-SAT and 2-SAT, if we can repeat the process indefinitely (for small constants $k,l$), the instance is also guaranteed to be satisfiable overall. Such CSPs are said to have \textit{bounded width}. 
\fi
%\vg{Changed description of bounded width.}
%\vg{Would this be a legit definition for promise CSP as well?}
%\vg{Local propagation might be more suggestive. Propagation is of width 1 for Horn-SAT and three for 2-SAT. Introduce the term bounded width already here.}  
...
\iffalse
More generally, Ha\aa stad's result shows that every CSP over an arbitrary finite domain that can ``express linear equations''\footnote{Formal defn.}  does not admit a robust algorithm. Guruswami and Zhou~\cite{GZ} conjectured that in fact, this is the only obstruction to having robust algorithms i.e., every tractable CSP that does not express linear equations admits a robust algorithm. Or equivalently, they conjectured that every CSP that has bounded width has a robust algorithm. As a striking result,
\fi
...
\iffalse
properties of the 
aiming to make progress on two fronts: (i) understanding which PCSPs can be solved in polynomial time, motivated in part by questions such as approximate graph coloring and $(2+\epsilon)$-SAT; and (ii) understanding the power and limitations of various algorithmic approaches for PCSPs

leading to the emergence of a rich theory explaining al
These works have developed fragments of a rich a leading to a rich emerging theory of PCSPs. These works address both the algorithmic and hardness fronts, which are fitting together as pieces of a deep and rich theory. 


Broadly speaking, the study of PCSPs has been on two fronts: First, understanding which PCSPs can be solved in polynomial time, motivated by questions such as approximate graph coloring and $(2+\epsilon)$-SAT. Second, understanding the power of various algorithms for PCSPs. 
\fi
...
%\vg{Specify our goals, and perhaps also thesis, first up: We would like to understand which PCSPs have robust algorithms, and what those algorithms might be. Bounded width doesn't do it, SA might (we don't know), and we think SDP does. Note that SDP failing to solve immediately leads to UG hardness. Motivates the question of which PCSPs are solved by SDPs. The natural (correct?) approach to tackle this question is via the polymorphic theory (some glue to move into polymorphisms-based classification).}

%\vg{Fork into two threads - robust alg and hardness via polymorphisms, minion SDPs; subsection on our results}
...
%Can we characterize the existence of polynomial time robust algorithms in terms of specific algorithms such as Sherali Adams or SDPs? 
\end{question}
...
%This leads to the following questions: which PCSPs admit robust algorithms? Is it true that a PCSP admits a robust algorithm if and only if it can be solved by basic SDP? 

%\vg{Bit of abrupt jump; think of some glue}
...
%In particular, we show and show that in the absence of these polymorphisms, it is NP-hard  
%we study which PCSPs have robust algorithms via their polymorphisms, and show that for this special case of PCSPs, solvability by basic SDP is equivalent to having a robust algorithm. 
...
\iffalse
While $\AT$ and $\MAJ$ are noise stable functions, $\PAR$ is highly sensitive to noise, i.e., if we perturb each input with a small probability, the function output changes significantly. In fact, Parity having low noise sensitivity can also be viewed as one reason why $3$-LIN, despite having $\PAR$ as polymorphisms, does not admit robust algorithms. Thus, for Boolean symmetric folded PCSPs, a natural candidate characterization of robust algorithms is the existence of $\AT$ or $\MAJ$ polymorphisms. \vg{Does this make our result sound a bit unsurprising? Maybe rephrase a bit?}
\fi
%On the algorithmic front, we prove that this indeed is the case, and on the hardness side, assuming the Unique Games Conjecture~\cite{Khot02}, we show that the absence of $\AT$ of $\MAJ$ polymorphisms implies the lack of robust algorithms.
...
%that have $\MAJ$ polymorphisms and with the additional restriction that every weak predicate is of the form $k$-SAT. 
%
...
%\vg{Can skip idempotence here but should we include it in the formal theorem statement below?}

%, and we allow negations of variables and unary constraints that set variables to be True or False. \ssr{Explain the setting variables part in the context of robust algorithms.} 
%This means that the associated polymorphisms are folded and idempotent.\footnote{We say that a function $f:\{-1,+1\}^n \rightarrow \{-1,+1\}$ is folded if $f(-\textbf{x})=-f(\textbf{x})$. We say $f$ is idempotent if $f(b,b,\ldots,b)=b$ for every $b \in \{-1,+1\}$.} 

%\vg{Do we need idempotence or can we handle it via anti-Maj etc.}

%While we fall short of fully establishing this, we come close and reduce the problem to a single class of unresolved templates. In particular, we show that there exists a class of templates $\Gamma^*$ such that for each Boolean symmetric PCSP $\Gamma=(P,Q)$ (allowing negations and constants) without $\AT$ or $\MAJ$ polymorphisms, either $\Gamma$ does not have a robust algorithm assuming Unique Games Conjecture or there is a gadget reduction from a PCSP in $\Gamma^*$ to $\Gamma$. 
%\vg{Reworded last couple of sentences.} \vg{Using folded idempotent also for PCSP and not polymorphisms?}

  \iffalse
While we fall short of showing that every such (symmetric) PCSP without $\MAJ$ and $\AT$ as polymorphisms does not admit robust satisfaction algorithms, we isolate a fixed template $\Gamma^*$ without $\MAJ$ or $\AT$ polymorphisms where we do not have UG-hardness of obtaining robust algorithms. 
\fi
...
%Here, a PCSP $\Gamma'$ is said to be a relaxation of another PCSP $\Gamma$ if $\Pol(\Gamma) \subseteq \Pol(\Gamma')$. 
%If a PCSP $\Gamma'$ is a relaxation of $\Gamma$, there exists a gadget reduction from $\Gamma'$ to $\Gamma$~\cite{BrakensiekG21}. Such a gadget reduction also preserves the existence of robust algorithms i.e., if $\Gamma$ admits robust satisfaction algorithms, then $\Gamma'$ does as well. Thus, to prove that a Boolean symmetric folded idempotent PCSP $\Gamma$ admits a polynomial time robust algorithms if and only if $\AT \in \Pol(\Gamma)$ or $\MAJ \in \Pol(\Gamma)$, it suffices to show the class of PCSP templates $\Gamma^*$ do not admit robust algorithms, which we conjecture to be true.  

%\vg{replaced solved by decided for SDP}
% 
%For a Promise CSP $(P,Q)$, by being solved by the basic SDP, we mean that for every instance $I$ of a PCSP $\Gamma$ has an integral solution satisfying $Q$ if and only if the basic SDP relaxation using $P$ is feasible on $I$. In other words, we can use the basic SDP to solve the decision version of the PCSP $\Gamma$. 
...
%While being solved by the basic SDP is only relevant in the perfect completeness case, and the robust algorithms deal with the case when the instances have assignments satisfying $1-\epsilon$ fraction of the constraints, 
%We believe that this is a general phenomenon and should hold for arbitrary PCSPs. 
\begin{conjecture}
...
% Given the above conjecture, it's important to understand when the basic SDP solves a PCSP. Note that for CSPs this is precisely the bounded width case. The situation is much more subtle for PCSP.
...
%Relationships between a finite set of functions can be captured by \emph{identities}, but more advanced relationships can be captured by infinite objects known as \emph{minions}. 
%\vg{Maybe bring in homomorphism also in the text, saying it takes the role of identities.}
%
\begin{itemize}
\vspace{-1ex}
...
%\footnote{Here `exact' means that we verify that the basic SDP is solved to exact precision, whereas $\operatorname{poly}(n)$ bits of precision is the computational limit. A more thorough discussion of this technicality is in Section~\ref{sec:minion}.}  
\end{theorem}
...
%\ssr{Citing and Relating concurrent work.} \jb{Should this be the cutoff point to move to Section 3?} \vg{Sinnce it is brief perhaps can stay here?}
...
%\vg{We already talk about gaps and sphere colorings earlier. Perhaps what we want to highlight in the para below is that we place that in some formal framework to apply compactness etc. Please tie this with the earlier discussion nicely.}
%

 \medskip\noindent\textbf{Relation with sphere colorings.} 
 %As a direct consequence of the minion structure theorem, we can connect integrality gaps with certain sphere colorings. 
 ...
 %\ssr{Perhaps this fits better after Theorem 1.4?}
 %To make the integrality gaps in the paper more self-contained, we streamline the connection between sphere coloring and integrality gaps in Lemma~\ref{lem:sphere-coloring}. \vg{Can we give an informal statement here instead of pointing to a Lemma in the depths of the paper?} 
...
%\medskip \noindent \textbf{PCSPs and Polymorphisms.}
...
\end{definition}
%In general, we can have multiple relations $A_1,A_2,\dots,A_l$, and different constraints can use different relations. We denote such a CSP by $CSP(A_1,A_2,\ldots,A_l)$.  
...
%\vg{Can we only highlight the difference from the folded definition?}
...
\end{enumerate}
%each consisting of a tuple of elements $C_j = (x_{j,1}, x_{j,2}, \ldots, x_{j,{l_j}}) \subseteq \{V\cup \overline{V}\cup \{-1,+1\}\}^{l_j}$ and an associated predicate pair $(P^{(j)},Q^{(j)}) \in \Gamma$ of the same arity $l_j$. 

%Consider an assignment $\sigma : V \rightarrow \{-1,+1\}$, and let $\sigma' : V \cup \overline{V} \cup \{-1,+1\} \rightarrow \{-1,+1\}$ be defined as $\sigma'(u_i)=\sigma(u_i), \sigma'(\overline{u_i})=-\sigma(u_i)$ for every $i \in [n]$, and $\sigma'(b)=b\,\forall b \in \{-1,+1\}$. The assignment $\sigma$ is said to strongly (and resp. weakly) satisfy the constraint $C_j$ in $\Phi$ if $\sigma'(C_j) \in P^{(j)}$ (and resp. $\sigma'(C_j) \in Q^{(j)}$).
\end{definition}

\iffalse 
\begin{definition}[Boolean folded PCSP]
    A PCSP $\Gamma = {(A_1, B_1), (A_2, B_2), \ldots, (A_l, B_l)}$ over a pair of domains $D_1, D_2$ is said to be Boolean symmetric folded if the following hold:
    \begin{enumerate}
        \item (Boolean) The domains $D_1$ and $D_2$ are both equal to $\{-1,+1\}$. 
        \item (Symmetric) All the relations are symmetric i.e., for every $i \in [l]$, and $\textbf{x}, \textbf{y}$ such that $\textsf{hw}(\textbf{x})=\textsf{hw}(\textbf{y})$, we have $\textbf{x} \in A_i$ if and only if $\textbf{y} \in A_i$, and similarly, $\textbf{x}\in B_i$ if and only if $\textbf{y} \in B_i$. 
        \item (Folded) We allow negating the variables i.e., there exists $i \in [l]$ such that $A_i = B_i = \{ (-1,+1), (+1,-1)\}$. \jb{Allow idempotent} \jb{Rephrase as literals?}
    \end{enumerate}
\end{definition}
\fi 
...
\end{definition}
%Brakensiek and Guruswami~\cite{BrakensiekG21} (independent of Pippenger~\cite{pippenger2002galois}) showed that if $\Gamma'$ is a relaxation of $\Gamma$, then $\Gamma'$ is ppp-definable from $\Gamma$. 
...
We now observe that the same holds for the robust version as well. More formally, we have the following proposition.\footnote{We previously claimed falsely that the reduction holds more generally when we are given that $\Gamma'$ is a relaxation of $\Gamma$. We are grateful to Andrei Krokhin for pointing this mistake to us. Under the general assumption that $\Gamma'$ is a relaxation of $\Gamma$, we can reduce the robust version of $\Gamma'$ to the robust version of $\Gamma$ together with binary equality constraints. For the sake of exact satisfiability, the equality constraints do not matter~\cite{BartoKW17,BrakensiekG21} but we do not know a way to obtain a robust algorithm for the PCSP $\Gamma$ together with binary equality constraints just using the fact that $\Gamma$ has a robust algorithm. Even for CSPs, direct way to show this is not known~\cite{DalmauK13}, without using the full robust dichotomy result of~\cite{BartoK16}.}

% adding equality constraints in the definition 
\begin{proposition}
\label{prop:ppp-defn}
Suppose that the PCSP $\Gamma'$ over a pair of domains $D_1, D_2$ is ppp-definable from $\Gamma$ over the same domain pair. If $\Gamma$ has a polynomial time robust algorithm that finds an assignment weakly satisfying $1-f(\epsilon)$ fraction of the constraints on instances promised to have an assignment strongly satisfying $1-\epsilon$ fraction of the constraints, then $\Gamma'$ has a polynomial time robust algorithm as well, i.e., there is a polynomial time algorithm that finds an assignment weakly satisfying $1-O_{\Gamma,\Gamma'}(f(\epsilon))$ fraction of the constraints on instances promised to have an assignment strongly satisfying $1-\epsilon$ fraction of the constraints.
\end{proposition}
We defer the proof of~\Cref{prop:ppp-defn} to~\Cref{sec:gadget}. 

% \jb{Add some details about the homomorphism perspective}
% Throughout the literature (e.g., \cite{barto2019algebraic}), Promise CSPs have also been presented with a more ``stucture``-centric notation. This style of notation is useful for defining and analyzing the Basic SDP minion in Section~\ref{sec:minion}. 

\subsection{The basic SDP}

%\medskip \noindent \textbf{Basic SDP setup.}

%\jb{Make its own subsection. Start with a "textbook" fully general Basic SDP definition. Then give some variants that are useful for sections 3-6.}
...
%\ssr{By `exact' we mean an SDP solution that perfectly satisfies all the constraints. This is not feasible in principle as infinite precision is needed in the computed SDP matrix (c.f., \cite{freund2004introduction}). In practice, SDP solutions can be computed to $\poly(n)$ bits of precision, which we would consider to be a $1 - 1/2^{\poly(n)}$-robust solution. For PCSPs, it is unknown if there is a difference between exact, $1 - 1/2^{\poly(n)}$-robust and $1 - O(1)$ robust.}
%\medskip \noindent \textbf{Elementary properties of Gaussians.} 
...
%by the above rounding scheme, and thus, the assignment $\sigma$ satisfies the constraint $C_j$. %Our rounding function does not work if there are variables $u_i$ with $\langle \textbf{v}_i, \textbf{v}_0 \rangle = 0$. 
%For these variables whose SDP vectors are orthogonal to $\textbf{v}_0$, we , and $\sigma(u_i)$ to $+1$ if and only if . This uses the fact that  Thus, for at least one $i \in [2]$, we have $\langle \zeta, \textbf{v}_i \rangle >0 $.
...
%Zwick~\cite{Zwick98} gave the first robust algorithm for $2$-SAT where he does ``outward rotation'' where he rotates the vectors $\textbf{v}$ with $|\langle \textbf{v}, \textbf{v}_0 \rangle | \leq \epsilon^{\frac{1}{3}}$ by certain angle depending on $\langle \textbf{v}, \textbf{v}_0 \rangle$ before using the above algorithm.
...
%We first give an overview of their analysis. 
%If the underlying $2$-SAT instance is guaranteed to have a solution satisfying $1-\epsilon$ fraction of the constraints, on average, the SDP solution has an error of $\epsilon$ associated with each constraint. 
...
%$C_j$, we get a weaker guarantee that $\langle \textbf{v}_1, \textbf{v}_0 \rangle + \langle \textbf{v}_2, \textbf{v}_0 \rangle \geq -\epsilon$. 
...
%If $|\sum_{i \in [4]}\langle \textbf{v}_i, \textbf{v}_0 \rangle|$ is much larger than $\sqrt{\epsilon}$, then at least one of $\langle \textbf{v}_i, \textbf{v}_0 \rangle $ is much larger than $\sqrt{\epsilon}$, and then, with high probability, the literal is rounded to $+1$. 
...
%In this case, the random variable $\zeta_s = \sum_i \langle \zeta, \textbf{v}_i \rangle$ has $|\zeta_s|\leq \epsilon^{O(1)}$.  
...
\]
%As $|\langle \zeta, \sum_i \textbf{v}_i\rangle|$ and $|\langle \textbf{v}_i, \textbf{v}_0 \rangle |$ are small for every $i \in [4]$, there has to be some $i \in [4]$ for which $\langle \zeta, \textbf{v}_i \rangle$ is small.
...
%We can compactly do this operation as follows: 
 \[
        \sigma(u_i) = 
        \begin{cases}
...
%the probability of $\langle \textbf{v}_i-\textbf{v}_i, \textbf{v}_0 \rangle \textbf{v}_0, \zeta \rangle$ \vg{Some typo here} being close to $\delta$ is very small.
...
% In the SDP solution, the variable associated with a vector $\textbf{v}$ can be assigned $\textbf{v}$ itself, and this vector solution satisfies all the constraints with zero error. 
...
%\vg{Does connectivity lemma handle all the cases we need? What about the higher coloring trick?}
...
\begin{enumerate}
    ...
    %Let $m$ denote the number of constraints in $\Phi$.
...
%In particular, for every predicate pair $(P,\{-1,+1\}^k\setminus \{\textbf{x}\})$ in $\Phi^*$, we use a different predicate pair $(P',\{-1,+1\}^k\setminus \{(-1,-1,\ldots,-1)\})$ where $P'$ is obtained from $P$ by negating a set of coordinates according to $\textbf{x}$.
...
\iffalse 
\begin{claim}\label{claim:simp}
An algorithm $\cA$ is a robust algorithm for $\InvPol(\cF)$\footnote{We define $\InvPol(\cF)$ to be the set of \emph{finite} promise templates $\Gamma$ with $\cF \subseteq \Pol(\Gamma)$.} if and only if for all for all $k \ge 1$, $P \subseteq \{-1, 1\}^k$ and $x \in \{-1,1\}^k\setminus O_{\mathcal F}(P)$, we have that $\cA$ is a robust algorithm for $\PCSP(P, Q_{\setminus x}).$
\end{claim}

\begin{proof}
The ``only if'' direction is trivial as $(P, Q_{\setminus x}) \in \InvPol(\cF)$.

For the ``if'' direction, consider an instance and replace every clause of the form $(A, B)$ with at most $2^k$ clauses of the form $\{(A, Q_{\setminus x}): x \not\in B\}$ all on the same set of variables. By assumption, $\mathcal A$ is a $1 - \eps$ vs $1 - f(\eps)$ robust algorithm for these $(A, Q_{\setminus x})$ clauses, where $f$ is the maximum  of the tradeoff functions for all $(A, Q_{\setminus x})$. It is easy to see that the assignment $\cA$ produces is a $1 - \eps$ vs $1 - 2^k f(\eps)$ algorithm for $\PCSP(A,B)$, which is robust as $2^k$ depends only on the choice of template, which is independent of $\eps$. 
\end{proof}
...
%If at least $\eps^{1/3}$, then we are fine by the same argument. Also, note by a suitable Markov that $1- \eps^{1/4}$ fraction of the clauses have an error at most $\eps^{3/4}$. Thus, we may assume wlog that $\bw^{T}\mu \ge -\eps^{3/4}$. 
...
%Also observe that by definition of $\Sigma$ that for all $i \in [k]$, we have that $\be_i^{T} \Sigma \be_i = 1$. Thus, for all $i$, the PDF of $\zeta_i$ is $O(1)$ everywhere. 
 %Thus, if $\zeta \in \mathcal B$, that is $\bt - \zeta \ge 0$ and we condition on $\bw^{T}\zeta \ge -\sqrt{\kappa + c}\log \frac{1}{\epsilon}$.
...
%Horn-SAT is an example of a (P)CSP for which $\OR $ is a polymorphism.  

%As the $\OR$ operator is naturally over the $0/1$ basis, we shall assume that the predicates $P, Q \subseteq \{0,1\}^k$ for this section. 
...
\noindent\fbox{%
    ...
    \begin{enumerate}
        ...
        %\jb{Would eventually like to be oblivious to the clausal representation...}  \jb{We are also assuming constant-sized clauses, which Zwick does not do...}
...
\begin{proof}
%Since we assume $k$ is a constant, the size of $T$ is $O_{\Gamma}(1/\log(1/\eps))$. 
...
%As the PCSP $\Gamma'=\{(P,Q)\}$ contains AT of all odd arities as polymorphisms, $O_{AT}(P)\subseteq Q$. Henceforth, in our analysis, we assume that $Q=O_{AT}(P)$. 
...
\iffalse 
We use the fact that $P,Q$ are symmetric to reduce to a special case. Note that if $Q = \{-1,+1\}^k$, any assignment $\sigma$ satisfies the constraint $C_j$ with probability $1$, we are done. Henceforth, we assume that $Q \neq \{-1,+1\}^k$. 
Before stating a lemma regarding Boolean symmetric PCSPs with $\AT$ polymorphisms, we recall some notations. For a vector $\textbf{x} \in \{-1,+1\}^n$, we use $\textbf{hw}(\textbf{x})$ to denote the number of $+1$s in $\textbf{x}$, and for a set $S \subseteq \{0,1,\ldots,k\}$, $\Ham_k S$ is the subset of $\{-1,+1\}^k$ containing all the vectors $\textbf{x}$ with $\textbf{hw}(\textbf{x})\in S$.

Using the properties of symmetric PCSPs with AT polymorphisms proved by Brakensiek and Guruswami~\cite{BrakensiekG21}, we get the following lemma.
\begin{lemma}
\label{lem:at-reduction}
Suppose that $\Gamma'=\{(P,Q)\}$ is a symmetric Boolean PCSP where $P \subseteq Q \subsetneq \{-1,+1\}^k$ has $\AT \in \Pol(\Gamma_1)$ and $Q=O_{AT}(P)$. Then, either of the two following conditions holds:
\begin{enumerate}
    \item $P=Q$ and $P \subseteq \{ (-1,-1,\ldots,-1), (+1,+1,\ldots,+1)\}$. 
    \item There exists $l \in \{1,2,\ldots, k-1\}$ such that $P=\Ham_k\{l\}$, and $Q= \Ham_k\{1,2,\ldots,k-1\}$. 
\end{enumerate}
\end{lemma}
\begin{proof}
Using Claim $4.6$ in~\cite{BrakensiekG21}, if $\Ham_k \{l_1, l_2 \} \subseteq P$ where $l_1 \neq l_2, \{l_1, l_2 \} \neq \{0,k\}$, we get that $Q=\{-1,+1\}^k$, a contradiction. Thus, either $P=\Ham_k \{l\}$ for some $l \in \{0,1,\ldots, k\}$ or $P=\Ham_k \{0,k\}$. If $P \subseteq \Ham_k \{0,k\}$, we get that $P=Q$.
If not, then $P = \Ham_k \{l\}$ for $l \in \{1,2,\ldots, k-1\}$, and by the same Claim $4.6$ in~\cite{BrakensiekG21}, we get that $Q=O_{AT}(P)=\Ham_k \{1,2,\ldots,k-1\}$.
\end{proof}


For the rest of the section, we focus on the case when there exists $l \in \{1,2,\ldots, k-1\}$ such that $P=\Ham_{k}\{l\}$, and $Q=\Ham_k\{1,2,\ldots,k-1\}$.

\fi 

%More generally, we assume that there exists $\textbf{w} \in \mathbb{R}^k, b \in \mathbb{R}$ such that $w_i >0 \forall i \in [k]$, $\sum_i w_i > |b|$, 
%and $\textbf{w}\cdot \textbf{a} = b$ for all $\textbf{a} \in P$. 
%Here, for the case when $P=\Ham_k\{l\}$, we can take $\textbf{w} =(1,1,\ldots,1) $ and $b=2l-k$.
...
We have that each $p_i \geq 0$, $\sum_{i \in [K]}p_i = 1$ and 
    ...
    \]
%Let $s = l-(k-l) = 2l-k$. Note that for every $a_i \in P$, we have $\sum_{u \in a_i}u = 2l-k$.
...
%\vg{Define PPP-reduction. Should it be ppp?}

%Furthermore, ~\Cref{lem:equal-case} shows that our algorithm is a robust algorithm for $\Gamma_{const}$ as well. Together with~\Cref{thm:at-general}, we get that our algorithm is a robust algorithm for every PCSP that contains AT of all odd arities as polymorphisms. 
...
%$\AT \in \Pol(P_i,Q_i)$ for every $i \in [L]$ 
...
\MAJ_{L_1}, \AT_{L_2} \notin \Pol(\Gamma)$ for some odd integers $L_1, L_2$. Then, there exists a Boolean folded PCSP $\Gamma'=(P,Q)$ that is ppp-definable from $\Gamma$ that is equal to either of the following:
\begin{enumerate}
...
\end{enumerate}
\end{lemma}
%\vg{$k$ should be odd for $\Gamma_3,
%\Gamma_4$ as well?} \vg{$k$ is not multiple of $4$ for $\Gamma_1$ is hard.} 
We defer the proof of~\Cref{lem:reduction} to~\Cref{sec:missing-proofs}. 

Recall that if a PCSP $\Gamma'$ is ppp-definable from another PCSP $\Gamma$, if $\Gamma$ has a polynomial time robust algorithm, then $\Gamma'$ has a polynomial time robust algorithm as well (\Cref{prop:ppp-defn}). Thus, to show~\Cref{thm:main-hardness}, it suffices to show Unique Games based hardness of obtaining robust algorithms for the PCSPs $\Gamma_{1\text{---}5}$.
...
%$U^n_{\Gamma} \to B_{\Gamma}$ for all $n \ge 1$.
\end{lemma}
\begin{proof}
...
%Thus, there exists a dimension $n$ and a mapping $x \mapsto \bv_x$ of the variables to $n$-dimensional SDP vectors. By setting $x$ to $f(\bv_x)$ for each variable $x$, we get an assignment satisfying all the constraints in $I$. 
...
%We remark that as we restrict ourselves to Boolean folded PCSPs, we only study the colorings $f:\mathbb{S}^n \rightarrow \{-1,+1\}$ that are folded i.e., $f(-\textbf{x})=-f(\textbf{x})$.
...
%We use $Cong(S)$ to denote the set of all subsets of $\R^{n+1}$ that are congruent to $S$ under Euclidean motion. 
...
%As $S$ is affinely independent, they can be viewed as vertices of a simplex. Furthermore, t
...
%
%We set $J_1 = \{1,2,\ldots,2(k-1)\}$, $J_2 = \{2,3,2(k-1)+1, 2(k-1)+2, \ldots, 2(k-1)+2(k-2)\}$, \ldots, $J_i = \{ 2(i-1), 2(i-1)+1, 2(k-1)+2(i-2), 2(k-1)+2(i-2)+1, 2(k-1+k-2)+2(i-3), 2(k-1+k-2)+2(i-3)+1, \ldots, 2(k-1+k-2+\ldots+k-i+1)+1,2(k-1+k-2+\ldots+k-i+1)+2,\ldots,2(k+k-1+\ldots+k-i+1)+2(k-i-1)\}, 1 \leq i \leq k-1$, and we set $J_k = \{ 1, 2(k-1), 2(k-1)+1, 2(k-1)+2(k-2), \ldots, 2(k-1+k-2+\ldots + 2)+1,2(k-1+k-2+\ldots + 2)+2  \}$.  
\end{proof}

\iffalse 
\begin{lemma}
\label{lem:sphere-coloring-3}
Fix an even integer $k \geq 4$. There exists $n_0 := n_0(k)$ such that for every $n \geq n_0$ and (not necessarily folded) coloring  $f: \mathbb{S}^n \rightarrow \{-1,+1\}$, there exist a $k$-regular set of vectors $V = \{ \textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_k \} \subseteq \mathbb{S}^n$ such that either of the below holds.  
\begin{enumerate}
    \item All the $\textbf{v}_i$s are assigned the same color. 
    \item Exactly $\frac{k}{2}$ vectors in $V$ are colored $-1$.
\end{enumerate} 
\end{lemma}
\begin{proof}
We prove the lemma by showing the following claim: 
\begin{claim}
\label{claim:k-regular-half}
For every $r,k$ there exists $n_0 := n_0(k,r)$ such that for any sphere coloring $f': \mathbb{S}^n \rightarrow [r]$ with $n \geq n_0$, there exist $k$ unit vectors $V=\{ \textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_k\}$ that are all colored the same such that 
\[V'=\{\textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_{\frac{k}{2}}, -\textbf{v}_{\frac{k}{2}+1}, \ldots, -\textbf{v}_k\} \]
is $k$-regular.
\end{claim}
We note that~\Cref{claim:k-regular-half} immediately implies a proof of~\Cref{lem:sphere-coloring-3}: Set $f'(\textbf{v})=(f(\textbf{v}), f(-\textbf{v})) \in \{-1,+1\}^2$. By applying~\Cref{claim:k-regular-half}, we can infer that there exists a set of $k$ unit vectors $V = \{\textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_k \}$ with $f(\textbf{v}_i)=f(\textbf{v}_j)$ and $f(-\textbf{v}_i)=f(-\textbf{v}_j)$ for every $i \neq j$, and $V'=\{\textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_{\frac{k}{2}}, -\textbf{v}_{\frac{k}{2}+1}, \ldots, -\textbf{v}_k\} $ is $k$-regular. Note that $V'$ is a $k$-regular set of vectors in which the first $\frac{k}{2}$ vectors (and similarly, the last $\frac{k}{2}$ vectors) are all colored the same, hence proving the lemma. 

We now prove~\Cref{claim:k-regular-half}. Using~\Cref{coroll:spreads-ramsey}, it suffices to show that there exists a unit vector $\textbf{a}$ and integer $N:=N(k)$ such that there exist a set of $k$ vectors $\textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_k  \in \Spread_N(\textbf{a}, [N])$ with
\[V'=\{\textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_{\frac{k}{2}}, -\textbf{v}_{\frac{k}{2}+1}, \ldots, -\textbf{v}_k\} \]
being $k$-regular. As in the proof of~\Cref{lem:sphere-coloring-2}, we set $\textbf{a} = (\gamma, -\gamma, \gamma, -\gamma, \ldots, \gamma, -\gamma) \in \R^{2(k-1)}$ with $\gamma = \frac{1}{\sqrt{2(k-1)}}$. 
For ease of notation, let $\ell = \frac{k}{2}$. Our goal is to find a set of vectors $\textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_\ell \in \Spread_N(\textbf{a},[N])$, $\textbf{u}_1, \textbf{u}_2, \ldots, \textbf{u}_\ell \in \Spread_N(-\textbf{a},[N])$ such that $ \{ \textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_\ell, \textbf{u}_1, \textbf{u}_2, \ldots, \textbf{u}_\ell \} $ is $k$-regular. We construct these vectors inductively. First, we give a construction when $\ell =1$: 
\begin{align*}
    \textbf{v}_1 &= (\gamma, -\gamma) = \Spread(\textbf{a},\{1,2\}) \\ 
    \textbf{u}_1 &= (-\gamma, \gamma) =\Spread(-\textbf{a},\{1,2\})
\end{align*}
Now, suppose that we have sets $S_1, S_2, \ldots, S_\ell, T_1, T_2, \ldots, T_\ell$ each with cardinality $2(k-1)=4\ell-2$ such that $\textbf{v}_i = \Spread_N(\textbf{a},S_i)$, $\textbf{u}_i = \Spread_N(-\textbf{a}, T_i)$ for every $i \in [\ell]$ satisfy the property that $\{ \textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_\ell, \textbf{u}_1, \textbf{u}_2, \ldots, \textbf{u}_\ell \} $ is $k$-regular. 
We now construct $S'_1, S'_2, \ldots, S'_{\ell+1}, T'_1, T'_2, \ldots, T'_{\ell+1}$ with $\textbf{v}'_i = \Spread_{N'}(\textbf{a}',S'_i)$, $\textbf{u}'_i = \Spread_{N'}(-\textbf{a}', T_i)$ for every $i \in [\ell+1]$ satisfy the property that $\{ \textbf{v}'_1, \textbf{v}'_2, \ldots, \textbf{v}'_{\ell+1}, \textbf{u}'_1, \textbf{u}'_2, \ldots, \textbf{u}'_{\ell+1} \} $ is ($k+2$)-regular. Note that $\textbf{a}' = ( \gamma', -\gamma',\ldots,  \gamma', -\gamma')\in \R^{2(k+1)}$ with $\gamma'=\frac{1}{\sqrt{2(k+1)}}$, and we need that $|S'_i|=|T'_i|=2k+2 = 4+ |S_i|$ for every $i \in [\ell]$. We define these sets as follows: 
\begin{enumerate}
    \item We add four new coordinates for every $S'_i$: We set 
    \[
    S'_i = S_i \cup \{ N+4(i-1)+1, \ldots, N+4(i-1)+4\}, i \in [\ell]
    \]
    \item Similarly, we add four new coordinates for every $T'_i$ too.  Let $N_1=N+4l$. 
    \[
    T'_i = T_i \cup \{ N_1+4(i-1)+1, \ldots, N_1+4(i-1)+4\}, i \in [\ell] \}
    \]
    Since $\textbf{u}_i \cdot \textbf{v}_j = -\gamma^2$, we get that $\textbf{u}'_i \cdot \textbf{v}'_j = -(\gamma')^2$ for every $i,j \in [\ell]$.  
    \item We set $S'_{\ell+1}$ as follows:
    \begin{align*}
    S'_{\ell +1 } = \{ &N +2, N+3, N+6, N+7, \ldots, N+4(\ell-1)+2, N+4(\ell-1)+3,\\ 
    &N_1+1, N_1+4, N_1+5, N_1+8, \ldots, N_1+4\ell-3, N_1+4\ell, N_1+4\ell+1, N_1+4\ell+2 \}
    \end{align*}
    This ensures that 
    \[
    \textbf{v}'_{\ell+1} \cdot \textbf{v}'_i = \textbf{v}'_{\ell+1} \cdot \textbf{u}'_i
    -2(\gamma')^2 = \, \forall i \in [\ell]
    \]
    \item Finally, we set $T'_{\ell+1}$: 
    \begin{align*}
    T'_{\ell+1} = \{ &N +1, N+4, \ldots, N+4(\ell-1)+1, N+4\ell, \\ 
    &N_1+2, N_1+3, N_1+5, N_1+6, \ldots, N_1+4\ell-2, N_1+4\ell-1, N_1+4\ell+1,N_1+4\ell+2\}
    \end{align*}
    This ensures that 
    \[
    \textbf{u}'_{\ell+1} \cdot \textbf{v}'_i = \textbf{u}'_{\ell+1} \cdot \textbf{u}'_i = 
    -2(\gamma')^2  \, \forall i \in [\ell]
    \]

\end{enumerate}
We illustrate our construction for $\ell =1$: 
\begin{align*}
    \textbf{v}'_1 &= (&\gamma',&-\gamma',&\gamma',& -\gamma',&\gamma',&-\gamma',&0,&0,&0,&0,&0,&0) \\ 
    \textbf{u}'_1 &= (&-\gamma',&\gamma',&0,&0,&0,&0,&-\gamma',&\gamma',&-\gamma',&\gamma',&0,&0) \\
    \textbf{v}'_2 &= (&0,&0,&0,&\gamma',&-\gamma',&0,&\gamma',&0,&0,&-\gamma',&\gamma',&-\gamma') \\ 
    \textbf{u}'_2 &= (&0,&0,&-\gamma',&0,&0,&\gamma',&0,&-\gamma',&\gamma',&0, &-\gamma',&\gamma')
\end{align*}
Note that the above four vectors are a $4$-configuration after setting $\gamma'=\frac{1}{\sqrt{6}}$.
\end{proof}
\fi 

\iffalse 
We also need the following asymmetric spherical Ramsey theoretic statement. 
\begin{lemma}
\label{lem:sphere-coloring-2}
Fix an integer $k \geq 3$. There exists $n_0 := n_0(k)$ such that for every $n \geq n_0$ and coloring $f: \mathbb{S}^n \rightarrow \{-1,+1\}$, there exist a set of $k$ vectors $V = \{ \textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_k \} \subseteq \mathbb{S}^n$ such that $\textbf{v}_i \cdot \textbf{v}_j = \frac{-1}{k-1}$ for every $i \neq j$, and either of the two conditions hold:
\begin{enumerate}
    \item All the vectors in $V$ are assigned the color $-1$. 
    \item At least $k-1$ vectors in $V$ are assigned the color $+1$. 
\end{enumerate}
\end{lemma}
\begin{proof}
%We cannot use the same idea as above since the set of $k$ unit vectors with pairwise inner product equal to $\frac{-1}{k-1}$ cannot be all present on a sphere of radius strictly smaller than $1$.
As before, consider a set of unit vectors $S=\{\textbf{u}_1, \textbf{u}_2, \ldots,\textbf{u}_k \} $ in $\mathbb{S}^n$ such that $\textbf{u}_i \cdot \textbf{u}_j = \frac{-1}{k-1}$ for every $i \neq j$. Unlike before, we cannot apply~\Cref{thm:sphere-ramsey} directly as $\rho(S)=1$. Instead, we create a new set of unit vectors $S'$ as follows: Let $\textbf{u}_s = \sum_{i \in [k-1]}\textbf{u}_i$. Fix $0<\alpha <\frac{2}{k-1}$ and let $\textbf{c}=\alpha \textbf{u}_s$. Note that there exists $\delta <1$ such that $\norm{\textbf{c}-\textbf{u}_i}=\delta $ for every $i \in [k-1]$. Consider a sphere $\mathbb{T}$ centered at $\textbf{c}$ and radius $\sqrt{1+\alpha^2-2\frac{\alpha}{k}}$. Note that $\mathbb{T}$ and $\mathbb{S}^n$ intersect in $\textbf{u}_i, i \in [k-1]$. As $k \geq 3$, their intersection contains at least two points, and hence, their intersection is an $(n-1)$ dimensional sphere. 

Thus, we can find a unit vector $\textbf{u}' \in \mathbb{T}$ such that $S' =\{ \textbf{u}_1, \textbf{u}_2, \ldots, \textbf{u}_{k-1}, \textbf{u}'\}$ are affinely independent. Note that $\rho(S')<1$. 
Finally, consider a rotation of the sphere $\phi : \mathbb{S}^n \rightarrow \mathbb{S}^n$ such that $\phi(\textbf{u}')=\textbf{u}_k$.\ssr{Such a $\phi$ is guaranteed to exist?}
Now, let $f' : \mathbb{S}^n \rightarrow \{-1,+1\}^2$ be a four coloring of the sphere defined as $f'(\textbf{x})=(f(\textbf{x}), f(\phi(\textbf{x})))$. Using~\Cref{thm:sphere-ramsey} on $S'$ and $f$, we obtain a set of vectors $\textbf{v}_1, \ldots, \textbf{v}_{k-1}, \textbf{v}'$ such that all of them are assigned the same color with respect to $f'$, and $S'$ is congruent to $S$. Thus we get $\phi(\textbf{v}')=\textbf{v}_k$, where $\textbf{v}_k = -\sum_{i=1}^{k-1} \textbf{v}_i$. \ssr{False. $\phi$ and the congruence operator may not commute. } Now, let $(p,q)=(f(\textbf{v}_1), f(\phi(\textbf{v}_1))$. We consider two different cases:
\begin{enumerate}
    \item If $p \neq q$, then we get that either $\textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_{k-1}$ or applying $\phi$ on them are all assigned $+1$ according to $f$. 
    \item If $p=q =+1$, we get the same as above. 
    \item If $p=q=-1$, we get that $\textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_k$ are all assigned $+1$ according to $f$.
\end{enumerate}
...
%such that the gram matrix of $\textbf{v}_0, \textbf{v}_1, \ldots, \textbf{v}_k$ is a uniform convex combination from the vertices of $\KZ(P)$. 
...
%We now show that for large enough $n$, there is no folded $f:\mathbb{S}^n \rightarrow\{-1,+1\}$ that respects the PCSPs $\Gamma_1, \Gamma_2, \Gamma_4$. 
...
\end{lemma}
\begin{proof}
Fix a vector $\textbf{v}_0 \in \mathbb{S}^n$. 
We get the $P$-configuration of vectors $\textbf{v}_1, \textbf{v}_2, \ldots , \textbf{v}_k$ such that the gram matrix of $\textbf{v}_0, \textbf{v}_1, \ldots, \textbf{v}_k$ is a uniform convex combination from the vertices of $\KZ(P)$. 
The vectors satisfy this if we have 
\begin{enumerate}
    \item (First moments.) $\textbf{v}_i \cdot \textbf{v}_0 = 0$ for every $i \in [k]$. 
    \item (Second moments.) $\textbf{v}_i \cdot \textbf{v}_j = \frac{2 \binom{\frac{k}{2}}{2} - \frac{k^2}{4}}{\binom{k}{2}}  = \frac{-1}{k-1}$. 
\end{enumerate}
We restrict ourselves to vectors orthogonal to $\textbf{v}_0$, and apply~\Cref{lem:sphere-coloring-1} to obtain a set of $k-1$ monochromatic vectors $\textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_{k-1}$ such that $\textbf{v}_i \cdot \textbf{v}_j = \frac{-1}{k-1}$. We let $\textbf{v}_k = -\sum_{i\in[k-1]} \textbf{v}_i$. These vectors form a $P$-configuration, and we have that 
$ \{ f(\textbf{v}_1, f(\textbf{v}_2)), \ldots, f(\textbf{v}_k)\}$ contains either $1$ or $k-1$ $+1$s. \ssr{False. All of them could be colored the same.} By taking the negation of the vectors (using the fact that $f$ is folded) if needed, we get a $P$-configuration of vectors whose $f$ value contains exactly $b$ $+1$s, a contradiction. 
\end{proof}
...
%such that the gram matrix of $\textbf{v}_0, \textbf{v}_1, \ldots, \textbf{v}_k$ is obtained by first sampling $i \in \{l, \frac{k+1}{2}\}$ where we set 
\[
t = \begin{cases}
...
\]
where $s = l-(k-l)<0$. 
%Then, we sample a uniformly random vertex from $\KZ(\Ham_k \{i\})$.
...
%Configurations are $\textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_k$ that are all orthogonal to $\textbf{v}_0$, pairwise inner product is at least $\frac{-1}{k}$.
\end{proof}
...
% \begin{align*}
%     \norm{\sum_{i=1}^k \textbf{v}_i}^2 &= k + 2 \sum_{i, j \in [k], i \neq j} \textbf{v}_i \cdot \textbf{v}_j \\ 
%     &= k + 2 \sum_{i \in \{2,\ldots,k\}}\textbf{v}_1 \cdot \textbf{v}_i + 2 \sum_{i,j \in \{2,\ldots,k\}, i \neq j } \textbf{v}_i \cdot \textbf{v}_j \\ 
%     &= k + 2(k-1)\frac{-1}{k-1}+(k-1)(k-2)\frac{(2l-k)^2-(k-2)}{(k-1)(k-2)}\\ 
%     &= (2l-k)^2.
% \end{align*}
% Thus, we get 
% \begin{equation}
%     \label{eq:sum-norm}
%     \norm{\sum_{i=1}^k \textbf{v}_i} = k-2l
% \end{equation}
...
\iffalse 
We can compute their pairwise inner products: 
\begin{align*}
\textbf{u}_1 \cdot \textbf{u}_i &= \textbf{v}_1 \cdot \textbf{u}_i \\ 
&= \frac{\textbf{v}_1 \cdot \textbf{v}'_i}{\norm{\textbf{v}'_i}}\\ 
&= \frac{-1}{(k-1)\sqrt{1-\alpha^2}}\, \forall i \in \{2,\ldots,k\}. \\ 
\textbf{u}_i \cdot \textbf{u}_j &= \frac{\textbf{v}'_i \cdot \textbf{v}'_j}{1-\alpha^2} \\ 
&= \frac{(\textbf{v}_i -\alpha \textbf{v}_0)\cdot (\textbf{v}_j - \alpha \textbf{v}_0)}{1-\alpha^2}\\
&= \frac{\beta - \alpha^2}{1-\alpha^2}\, \forall i,j \in \{2,\ldots,k\}, i \neq j. 
\end{align*}
Let $\gamma = \frac{\beta-\alpha^2}{1-\alpha^2}$. To use~\Cref{lem:sphere-coloring-1}, we need the following lower bound on $\gamma$. 

\begin{align*}
    \gamma &= \frac{\beta-\alpha^2}{1-\alpha^2} \\ 
    &= \frac{\frac{(2l-k)^2-(k-2)}{(k-1)(k-2)}-\left(\frac{2l-k}{k-1}\right)^2}{1-\left(\frac{2l-k}{k-1}\right)^2}\\ 
    &=\frac{\left(\frac{k-1}{k-2}\right)\left((2l-k)^2-(k-2)\right)-(2l-k)^2}{(k-1)^2-(2l-k)^2}\\ 
    &=\frac{\frac{(2l-k)^2}{k-2}-(k-1)}{(k-1)^2-(2l-k)^2}.\\
    -(k-2)\gamma&= \frac{(k-1)(k-2)-(2l-k)^2}{(k-1)^2-(2l-k)^2}\\
    &< 1. 
\end{align*}
Thus, we have 
\begin{equation}
\label{eq:gamma-lower-bound}
    \gamma > \frac{-1}{k-2}
\end{equation}
\fi 

%Using~\Cref{eq:sum-norm}, we get the following bound on $\gamma$ that we will use later. 
...
\end{lemma}
\begin{proof}
Configurations are $\textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_k$ such that $\textbf{v}_i \cdot \textbf{v}_0 = \alpha = \frac{2l-k}{s}$, and pairwise inner product is such that $\textbf{v}'_i \cdot \textbf{v}'_j = \frac{-1}{k-1}$ where $\textbf{v}'_i = \textbf{v}_i - \alpha \textbf{v}_0 $ 
\end{proof}
\fi 
...
% The basic LP decides $\PCSP(\bA, \bB)$ if and only if $\mathcal M_{BLP} \mapsto \Pol(\bA, \bB)$.
% \end{theorem}
...
%The traditional basic SDP does not specify vectors $\bv_{\bx, \ba}$. Rather, the traditional basic SDP keeps track of a probability distribution $\lambda_{\bx}$ on assignments to $\bx$. And for any pairs of variable, assignment pairs $(x_i, a_i), (x_j, a_j)$, we have that
%\[
%    \bu_{x_i, a_i} \cdot \bu_{x_j, a_j} = \Pr_{\ba \sim \lambda_{\bx}}[\ba_i = a_i \wedge \ba_j = a_j].
%\]
%Clearly any solution to our basic SDP is a solution to that basic SDP, as the probability $\lambda_{\bx}(\ba)$ is precisely $\|\bv_{\bx, \ba}\|^2$, and the above linear condition checks out as a property of the vectors.
...
\iffalse \subsubsection{Proof system}

\jb{Need to update, we'll probably just go with the SDP solution existing, and ignore implementation details...}

We say that a solution to the basic SDP is a "nearly" PSD matrix $M^{\bX}$ with rational entries, corresponding to the above dot products, such that each inequality is true within $o(1)$ error. \jb{Lookup in an optimization textbook what the actual definition is? details do matter for us}

We call a \emph{exact} basic SDP solution a real\footnote{I'm assuming any such algorithm is the decision version...it only tells you if such an $M$ exists.} PSD matrix $M^{\bX}$ for which the above equations are exactly satisfied. If $\bX \to \bA$ such a solution must exist (in fact a solution exists with only $0$ and $1$ entries).

In general, we say that an algorithm $\cA$ decides $\PCSP(\bA, \bB)$ if for any $\bX$, $\cA$ accepting $\bX$ implies $\bX \to \bB$. \fi
...
%In this case, since the exact basic SDP is a stronger algorithm than the "normal" basic SDP with $o(1)$ noise, we then also have that the basic SDP solving $\PCSP(\bA, \bB)$ implies that $\cM_{\SDP} \to \Pol(\bA, \bB)$.
...
%Towards this, we need to study sphere Ramsey results when there are multiple configurations. \vg{Should we say this can be challenging, and give a specific interesting example that we can solve, and one that we are not able to? But also okay to leave it like this for now} 
...
%Even for the decision version, we do not yet have a dichotomy result for Boolean PCSPs. 
...
%\item Hardness overview: highlight generality of the proof technique (predicate agnostic recipe?) to obtain SDP gaps. Could be useful for non-symmetric case. Could it help to get SDP gaps for non-bounded-width CSPs?
...
%the expected value of $c$ is at most $\epsilon$, using Markov's inequality, with probability at least $1-\sqrt{\epsilon}$, $c$ is at most $\sqrt{\epsilon}$.
...
%\ssr{General case when multiple predicate pairs rule out Maj, AT separately?} 
%\vg{In [BG] we did the cases of excluding Maj and AT sequentially/separately, and in case we relaxed the PCSP to some particular kinds of pairs $(P,Q)$} 
%As $\Gamma$ is folded and idempotent, we can without loss of generality, assume that $(\Ham_2{1}, \Ham_2{1}), (\Ham_1{0}, \Ham_1{0}), (\Ham_1{1}, \Ham_1{1}) \in \Gamma$. 
...
Thus, we get a PCSP $(P,Q)$ that is ppp-definable from original PCSP where $P=\Ham_k \{l\}$, $Q=\Ham_k \{0,1,\ldots,k\} \setminus \{b\}$ where $b \in \{1,2,\ldots, k-1\} \setminus \{l\}$. 
Note that $\MAJ, \AT \notin \Pol(P,Q)$. 
We now modify this PCSP furthermore, updating $P,Q,l,k,b$ while preserving the following two properties:
\begin{enumerate}
...
We use the following two tools to repeatedly obtain a new PCSP that is ppp-definable from the previous one. 
\begin{enumerate}
    \item Given a PCSP $P = \Ham_k \{l\}, Q = \Ham_k \{0,1,\ldots,k\} \setminus \{b\}$ where $b \in \{1,2,\ldots,k-1\}\setminus\{l\}$, then the PCSP $P' = \Ham_{k-1} \{l\}, Q' = \Ham_{k-1} \{0,1,\ldots,k-1\} \setminus \{b\}$ is ppp-definable from $(P,Q)$ (Claim $4.2$ of~\cite{BrakensiekG21}). As long as $b<k-1$ and $b \neq 2l-k+1$, this update preserves the above two properties. 
    \item Given a PCSP $P = \Ham_k \{l\}, Q = \Ham_k \{0,1,\ldots,k\} \setminus \{b\}$ where $b \in \{1,2,\ldots,k-1\}\setminus\{l\}$, then the PCSP $P' = \Ham_{k-1} \{l-1\}, Q' = \Ham_{k-1} \{0,1,\ldots,k-1\} \setminus \{b-1\}$ is ppp-definable from $(P,Q)$ (Claim $4.4$ of~\cite{BrakensiekG21}). As long as $b >1$ and $b \neq 2l-1$, this update preserves the above two properties. 
\end{enumerate}
Now, we update the PCSP using the above two steps. As $k$ is decreasing at every step, this procedure terminates at some point. Then, either of the two conditions holds:
\begin{enumerate}
    \item $b=1, b=2l-k+1$. In this case, we get that $l = \frac{k}{2}$ and $b=1$. Thus, $(P',Q')$ is ppp-definable from $\Gamma$ where $P'=\Ham_{k}\{\frac{k}{2}\}, Q = \Ham_{k} \{0,1,\ldots,k\} \setminus \{1\}$ where $k$ is even and is at least $4$. 
    \item $b=k-1, b=2l-1$. In this case, we get that $l = \frac{k}{2}$ and $b=k-1$. Thus, $(P',Q')$ is ppp-definable from $\Gamma$ where $P'=\Ham_{k}\{\frac{k}{2}\}, Q = \Ham_{k} \{0,1,\ldots,k\} \setminus \{k-1\}$ where $k$ is even and is at least $4$. 
\end{enumerate}

Suppose that there is no $b \notin \{0,k\}$ such that $\Ham_k \{b\} \subseteq O_{\MAJ} (P) \setminus Q $. As $O_{\MAJ}(P) \nsubseteq Q$, by negating the variables if needed, we can assume that $\Ham_k \{0\} \in O_{\MAJ}(P) \setminus Q$. Furthermore, there exists $b \in \{1,2,\ldots, k-1\}$ such that $\Ham_k \nsubseteq  Q$ as $O_{\AT}(P) \nsubseteq Q$. Thus, we obtain $(P,Q)$ that is ppp-definable from the original PCSP such that $P=\Ham_k \{l\}, Q=\Ham_k \{1,\ldots,k\}\setminus \{b\}$ where $l,b \in \{1,2,\ldots,k-1\}, b >2l-1$. By using the first type of update used above (Claim $4.2$ of~\cite{BrakensiekG21}), we obtain a new PCSP $(P,Q)$ that is ppp-definable from the original PCSP such that $P=\Ham_k \{l\}, Q=\Ham_k \{0,1,\ldots,k\}\setminus \{0,k-1\}$, where $l \in \{1,2,\ldots,k-1\}, l \leq \frac{k-1}{2}$. 

\smallskip \noindent \textbf{Case 2}. There exist $l \neq l'$ such that $\Ham_k \{l,l'\}\subseteq P$. Recall that $P \nsubseteq \Ham_k \{0,k\}$. This implies that $O_{\AT}(P)=\Ham_k \{0,1,\ldots,k\}$. Hence, we can get a PCSP $(P,Q')$ that is ppp-definable from the original PCSP such that $Q' = \Ham_k \{0,1,\ldots, k\} \setminus \{b\}$ and $\Ham_k \{b\} \nsubseteq O_{\MAJ}(P)$. 

For ease of notation, let $P=\Ham_k S$, where $S \subseteq \{0,1,\ldots,k\}$. First, consider the case when $\min S=0, \max S=k$. As mentioned earlier, we know that there exists $l \in \{1,2,\ldots,k-1\}$ such that $\Ham_k \{l\} \subseteq P$. Thus, we can obtain a new PCSP $(P,Q)$ that is ppp-definable from the original PCSP where $P=\Ham_k \{0,l,k\}, Q = \Ham_k \{0,1,\ldots,k\}\setminus \{b\}$ and $b \notin \{0,l,k\}$. We consider three cases separately:
\begin{enumerate}
    \item Suppose that $l \leq \frac{k-1}{2}$. In this case, we have a PCSP $(P,Q)$ that is ppp-definable from the original PCSP where $P=\Ham_k \{l,k\}$ and $Q = \Ham_k \{0,1,\ldots,k\}\setminus \{b\}$ and $b \notin \{l,k\}$. Note that this PCSP does not contain $\AT$ or $\MAJ$ as polymorphisms. 
    \item Suppose that $l = \frac{k}{2}$. In this case, we have a PCSP $(P,Q)$ that is ppp-definable from the original PCSP where $P=\Ham_k \{l\}$ and $Q = \Ham_k \{0,1,\ldots,k\}\setminus \{b\}$ where $b \notin \{0,l,k\}$. This also doesn't have $\AT$ and $\MAJ$ as polymorphisms. We have already shown that we can relax this further to the earlier mentioned three PCSPs. 
    \item Suppose that $l \geq \frac{k+1}{2}$. In this case, we have a PCSP $(P,Q)$ that is ppp-definable from the original PCSP where $P=\Ham_k \{0,l\}$ and $Q = \Ham_k \{0,1,\ldots,k\}\setminus \{b\}$ where $b \notin \{0,l\}$. Note that this PCSP does not contain $\AT$ or $\MAJ$ as polymorphisms. 
\end{enumerate}
Thus, there is a PCSP $(P,Q)$ that is ppp-definable from the original PCSP where $P=\Ham_k \{l,l'\}, Q = \Ham_k \{0,1,\ldots,k\}\setminus \{b\}$ such that $l <l',\{l, l'\} \neq \{0,k\}$, $b \in O_{\MAJ}(P)$. 
We end up with the same PCSP when $\{\min S, \max S\} \neq \{0,k\}$. 

If $\{l,l'\} = \{1,k\}$ or $\{0,k-1\}$, we get that the PCSP $P=\Ham_k\{1,k\}, Q=\Ham_k \{0,1,\ldots,k\}\setminus\{b\}$ is ppp-definable from the original PCSP, and we are done. 
If not, we update the PCSP while maintaining the two below properties:
\begin{enumerate}
    \item $P=\Ham_k \{l,l'\}$ with $l <l'$ and $\{ l, l' \} \neq \{0,k\}$ and $Q = \Ham_k \{0,1,\ldots, k\} \setminus \{b\}$. We also assume that $\{l,l'\} \neq \{1,k\}$ and $\{l,l'\}\neq \{0,k-1\}$. 
    \item $\AT, \MAJ \nsubseteq \Pol(P,Q)$. 
\end{enumerate}
As with the earlier case, we obtain a series of new PCSPs that are ppp-definable from the previous PCSPs using the below tools. 
\begin{enumerate}
...
When we are unable to update the PCSP using the above procedures, one of the following must be true.
\begin{enumerate}
    \item $l'=k, b=0$. In this case, we have a PCSP $(P,Q)$ where $P=\Ham_k \{l,k\}$, $Q=\Ham_k \{1,2,\ldots,k\}$, where $l\neq 0, l \leq \frac{k-1}{2}$. 
    \item $b=k,l=0$. By negating the variables, we can observe that the above PCSP is ppp-definable from this PCSP. 
    ...
    \item We have $b=2l-k+1, b=0$. By negating the variables, we can observe that the above PCSP is ppp-definable from this PCSP. 
\end{enumerate}

Thus, we have obtained a new PCSP that is ppp-definable from the original PCSP and is equal to either of the following PCSPs. 
\begin{enumerate}
...
Finally, we note that if a Boolean folded idempotent PCSP $\Gamma'=\{(P_1, Q_1),\ldots,(P_l, Q_l)\}$ is ppp-definable from another Boolean folded idempotent PCSP $\Gamma$, then $\Gamma'$ remains ppp-definable from $\Gamma$ even when we disallow the constraints in $\Gamma'$ to use constants, i.e., the Boolean folded PCSP associated with $\Gamma'$ is ppp-definable from $\Gamma$ as well.
\end{proof}

\section{Gadget reductions of PCSPs and robust algorithms}
\label{sec:gadget}

\begin{proof}[Proof of Proposition~\ref{prop:ppp-defn}]
