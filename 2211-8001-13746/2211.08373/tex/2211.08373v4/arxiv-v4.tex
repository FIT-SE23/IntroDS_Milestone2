...
\title{SDPs and Robust Satisfiability of Promise CSP\thanks{\change{An abridged preliminary version of this paper appeared in the proceedings of the 2023 Symposium on the Theory of Computing (STOC 23). This version is significantly expanded with numerous revisions and corrections.}}}
%
\author{}

\author{
Joshua Brakensiek \thanks{University of California, Berkeley, {\tt josh.brakensiek@berkeley.edu}. Research conducted while affiliated with Stanford University and supported in part by an NSF Graduate Research Fellowship and a Microsoft Research PhD Fellowship.}
\and 
...
They are both examples of \emph{bounded width} CSPs: the existence of \change{a} locally consistent \change{set of} assignments (which satisfy all local constraints involving some bounded number of variables, and which are consistent on the intersections) implies the existence of a global satisfying assignment.
%
%
%
%
...
Polymorphisms are the central objects in the \textit{\change{u}niversal algebraic approach} to CSPs~\cite{JeavonsCG97, Jeavons98, BulatovJK05,BartoKW17,Bul17,Zhuk20}, which has then been extended to PCSPs~\cite{BrakensiekG21, BBKO21}. 

%

At a high level, the existence of non-trivial polymorphisms implies algorithms, and vice-versa. The key challenge is to precisely characterize which polymorphisms lead to algorithms. It is known that the polymorphism family of a PCSP fully captures its computational complexity, i.e., if there are \change{promise templates }%
$\Gamma, \Gamma'$ \change{(that is, a set of pairs of relations)} such that the polymorphism family of $\Gamma$, $\Pol(\Gamma)$ is contained in $\Pol(\Gamma')$, then $\change{\PCSP(\Gamma')}$ \change{(the decision problem for PCSP instances using relations from $\Gamma'$)} is formally easier than $\change{\PCSP(\Gamma)}$, i.e., there is a \textit{gadget reduction} from $\change{\PCSP(\Gamma')}$ to $\change{\PCSP(\Gamma)}$. It turns out that this gadget reduction preserves the existence of robust algorithms as well.  \change{This was used by Dalmau, Krokhin~\cite{DalmauK13} in the CSP setting, which we extend to PCSPs in \Cref{prop:ppp-defn})}. Thus,~\Cref{ques:robust} and~\Cref{ques:sdp} can be rephrased as \textit{Which polymorphisms lead to robust algorithms for PCSPs? Which polymorphisms lead to being decided by the basic SDP relaxation?}  
...
%
%
Second, towards understanding the power of basic SDP for promise CSPs, we introduce a minion $\mathcal{M}$ and show that \change{$\PCSP(\Gamma)$} can be decided by basic SDP if and only if there is a minion homomorphism from $\mathcal{M}$ to \change{$\Pol(\Gamma)$}.%


\subsection{Our results: Robust algorithms and hardness}


As is the case with CSPs, if a PCSP is NP-Hard, then it does not admit polynomial time robust algorithms, assuming $\text{P} \neq \text{NP}$. Thus, Question~\ref{ques:robust} is only relevant for PCSPs that can be solved in polynomial time. A large class of PCSPs for which polynomial time solvability has been fully characterized \change{is the problems based on Boolean symmetric promise templates}.
In~\cite{BrakensiekG21}, the authors showed that a Boolean symmetric PCSP with folding (i.e., we allow negating the variables) can be solved in polynomial time if and only if it contains at least one of Alternate-Threshold $(\AT)$, Majority ($\MAJ$) or $\PAR$ polymorphisms of all odd arities. \change{Later an analogous result was shown without the folding restriction in~\cite{FicakKOS19} where in addition one needs to consider all families of threshold polymorphisms.}
...
\smallskip \noindent \textbf{Robust algorithms.} Our main algorithmic result shows that in two of these cases when the PCSP has $\MAJ$ or $\AT$ polymorphisms of all odd arities, the PCSP admits a robust algorithm. \change{More strongly, we allow our instances to have negated variables and constnat literals, which we call $\fiPCSP$, short for \emph{folded, idempotent} PCSP.}

%

\begin{theorem}
\label{thm:main-algorithm}
\change{Let $\Gamma$ be a Boolean promise template. If $\Pol(\Gamma)$} contains $\AT$ or $\MAJ$ polymorphisms of all odd arities\change{. Then, the robust version of $\fiPCSP(\Gamma)$} admits a polynomial time algorithm. In particular, 
\begin{enumerate}
    \item If $\change{\Pol(\Gamma)}$ contains $\MAJ$ polymorphisms of all odd arities \change{then} there exists a polynomial time algorithm that given an instance of $\change{\fiPCSP}(\Gamma)$ \change{and $\eps > 0$} that is promised to have a solution satisfying $1-\epsilon$ fraction of the constraints, outputs a solution satisfying $1-\tilde O(\epsilon^{\frac{1}{3}})$ fraction of the constraints.\footnote{Here, $\tilde O$ hides multiplicative poly logarithmic factors.}
    \item If $\change{\Pol(\Gamma)}$ contains $\AT$ polymorphisms of all odd arities \change{then} there exists a polynomial time algorithm that outputs a solution satisfying $1-O\left(\frac{\log \log \frac{1}{\epsilon}}{\log \frac{1}{\epsilon}}\right)$ fraction of the constraints on an instance of $\change{\fiPCSP}(\Gamma)$ promised to have a solution satisfying $1-\epsilon$ fraction of the constraints. 
\end{enumerate}
\end{theorem}

Similar to the robust algorithms for CSPs~\cite{Zwick98,CharikarMM09,BartoK16}, our robust algorithms for \change{promise templates} with $\MAJ$ and $\AT$ polymorphisms are based on rounding the basic SDP relaxation. 
The main challenge here is to obtain robust algorithms for a large class of \change{promise template} without access to the predicates and just using the properties of their polymorphisms. We achieve this using a combination of polymorphic tools where we use the fact that the \change{promise template} contains $\AT$ or $\MAJ$ polymorphisms to deduce structural properties of the underlying predicate pairs, and SDP rounding tools where we then use these structural properties to get a robust algorithm. 

For $\MAJ$ polymorphisms, we first reduce the problem to the case when every weak predicate is of the form $k$-SAT. We then show that the robust algorithm of Charikar, Makarychev, and Makarychev~\cite{CharikarMM09} for the $2$-SAT \change{predicate} generalizes to these classes of \change{promise templates}.
%
%
While the analysis of~\cite{CharikarMM09} is tailored towards $2$-SAT, we give a completely different analysis that is not based on predicates and instead uses the existence of $\MAJ$ polymorphisms as a black box. Similarly, for the $\AT$ polymorphisms, we first use the properties of $\AT$ polymorphisms to reduce to the \emph{weighted hyperplane} \change{promise template} that generalizes the $(1$-in-$3$-SAT, NAE-$3$-SAT) \change{promise template}. We then give a robust algorithm for the weighted hyperplane \change{promise template} based on a random threshold rounding technique. A detailed overview of our algorithmic ideas appears in Sections~\ref{sec:maj-overview} and~\ref{sec:at-overview}.

\smallskip\noindent\textbf{Hardness results.}
Unlike our robust algorithms, which work for general Boolean \change{promise template} with the said polymorphisms, in our hardness results, we rely on the symmetry of the predicates of \change{the promise template}. 
Furthermore, we assume that the \change{promise template} contains a single predicate pair $\Gamma =(P,Q)$ that does not admit $\AT$ or $\MAJ$ polymorphisms of all odd arities. We show that for such Boolean symmetric folded PCSPs, the basic SDP relaxation has an integrality gap with perfect completeness, i.e., there is a finite instance on which the SDP relaxation satisfies all the strong constraints with zero error but the instance is not satisfiable even using the weak constraints. 
...
Let $\Gamma=\change{\{}(P,Q)\change{\}}$ \change{be a promise template consisting of} a pair of symmetric Boolean predicates such that  $\AT_{L_1},\MAJ_{L_2} \notin \Pol(\Gamma)$ for some odd integers $L_1, L_2$. Then, under the UGC, unless $\text{P}=\text{NP}$, there is no polynomial time robust algorithm for the PCSP associated with \change{$\fiPCSP(\Gamma)$.}%
\end{theorem}
...
The basic SDP decides \change{$\PCSP(\Gamma)$} if and only if there is a minion homomorphism from $\mathcal M_{\SDP}$ to $\Pol(\Gamma)$.
%
\end{theorem}
...
%
 ...
 Thus, the result of~\cite{BBKO21} translates to \change{Boolean promise templates} as stating that a Boolean folded $\change{\PCSP(\Gamma)}$ is decided by the basic SDP if and only if there is an assignment satisfying all the constraints in the infinite integrality gap instance. 
...
\bigskip\noindent\textbf{Acknowledgment.} We thank anonymous reviewers for numerous helpful comments. We thank Ryan O'Donnell and Anupam Gupta for helpful discussions on SDP solvability which led to Appendix~\ref{app:efficient-sdp}.

\section{Preliminaries}
\label{sec:prelims}


\noindent \textbf{Notations.} We use $[n]$ to denote the set $\{1,2,\ldots, n\}$. \change{We let $\B := \{-1,+1\}$} A predicate or a relation over a domain $D$ of arity $k \change{\ge 0}$ is a subset of $D^k$.
For a relation $P \subseteq \change{D}^k$ of arity $k$, we abuse the notation and use $P$ both as a subset of $\change{D}^k$, and also as a function $P:\change{D}^k \rightarrow \{0,1\}$. We use boldface letters to denote vectors and roman letters to denote their elements, e.g., $\textbf{x}=(x_1, x_2,\ldots,x_k)$. We have $\S^n := \{ \textbf{v} \in \R^{n+1}: \norm{\textbf{v}}_2=1\}$.
...
For a vector $\textbf{x} \in \change{\B}^k$, we use $\textsf{hw}(\textbf{x})$ to denote the number of $+1$s in $\textbf{x}$, i.e., $\textsf{hw}(\textbf{x})=\frac{k+\sum_{i=1}^nx_i}{2}$. For $S \subseteq \{0,1,\ldots,k\}$, we use $\Ham_k S$ to denote $\{\textbf{x} \in \{-1,+1\}^k : \textsf{hw}(\textbf{x})\in S\}$. We use $\NAE_k$ to denote the set $\Ham_k \{1,2,\ldots,k-1\}$, and $k$-SAT to denote the set $\Ham_k \{1,2,\ldots,k\}$.
For vectors $\textbf{x}, \textbf{y} \in \R^n$, \change{ we use  $\langle \textbf{x}, \textbf{y}\rangle$ to denote $\sum_i x_i y_i$}. 

\subsection{\change{(P)CSP instances, templates,} and polymorphisms}
%
%

\change{We now work toward defining the main computational problem we seek to study: symmetric idempotent Boolean folded promise constraint satisfaction problems (PCSP) as well as the main tool we shall use for analyzing the complexity of such problems: \emph{polymorphisms}. To build toward these definitions, we begin by defining constraint satisfaction problems (CSP). The main ``parameter'' defining a (P)CSP is its set of allowable constraint types, which we call a \emph{template}. We can then use the template to build \emph{instances} of the (P)CSP which have associated \emph{assignments} for which one can define multiple computational \emph{problems}.}

\begin{definition}[CSP \change{template/instance/assignment}] \change{We define a \emph{CSP template} to be a finite set of predicates $\Gamma =\{P_1, P_2, \ldots, P_l\}$ over a finite domain $D$, where for all $i \in [\ell]$, $P_i$ has arity $k_i \ge 0$.}
\change{An \emph{instance} $\Phi=(V,\mathcal{C})$ of the constraint satisfaction problem (CSP) associated with the template $\Gamma$ (i.e., ``$\Phi$ is an instance of $\CSP(\Gamma)$''), consists of a variable set $V$ and a constraint set $\mathcal C$. The variable set consists of} $n$ variables $V =\{u_1, u_2, \ldots, u_n\}.$ \change{The} $m$ constraints $\mathcal{C}=\{C_1, C_2, \ldots, C_m\}$ 
each \change{consist} of a tuple of variables $C_j = (u_{j,1}, u_{j,2}, \ldots, u_{j,{l_j}}) \in V^{l_j}$ and an associated predicate $P^{(j)} \in \Gamma$ of the same arity $l_j$. 
...
\change{We now define two computational problems associated with $\CSP(\Gamma)$ where an instance $\Phi$ of $\CSP(\Gamma)$ is given as input.

\begin{definition}[\change{CSP decision/search problems}] In the \emph{decision version} of $\CSP(\Gamma)$, the objective is to decide if there is an assignment to a given instance of $\CSP(\Gamma)$ that satisfies all the constraints. In the \emph{search version} of $\CSP(\Gamma)$, the objective is to output an assignment to a given instance of $\CSP(\Gamma)$ that satisfies all the constraints.
\end{definition}}
%

%

We next define \change{promise constraint satisfaction problems} (PCSP).  \change{We begin by defining what a promise template is.}\footnote{\change{Other works, such as \cite{BBKO21}, parameterize the promise CSP as a pair of a relational structures with a homomorphism between them.}}

\change{
\begin{definition}[promise template]
    Let $D_1, D_2$ be finite domains. A promise template consists of a finite collection $\Gamma = \{ (P_1,Q_1), (P_2,Q_2),\ldots, (P_l,Q_l)\}$ consists of pairs of predicates, where for all $i \in [\ell]$, $P_i$ has arity $k_i\ge 0$ over domain $D_1$ and each $Q_i$ has arity $k_i$ over domain $D_2$. Furthermore, we assert there exists a map $h:D_1\rightarrow D_2$ such that for all $i\in[l]$ and $\textbf{x} \in D_1^{k_i}$, $\textbf{x} \in P_i$ implies $h(\textbf{x}) \in Q_i$. In other words, $h$ is a homomorphism from $P_i$ to $Q_i$ for all $i \in [l]$.
\end{definition}
}
...
\begin{definition}[\change{PCSP instance/assignments}]
...
\change{A $P$-assignment is a map $\sigma_1 : V \rightarrow D_1$ and a $Q$-assignment is a map $\sigma_2 : V \rightarrow D_2$.} A \change{$P$}-assignment \change{$\sigma_1$} is said to strongly satisfy the constraint $C_j$ if $\sigma(C_j)\in P^{(j)}$, and \change{a $Q$-assignment $\sigma_2$} is said to weakly satisfy the constraint $C_j$ if $\sigma(C_j) \in Q^{(j)}$.
\end{definition}

\change{Crucially, due to the existence of the homomorphism $h$ connecting each $P_i$ with its corresponding $Q_i$, any $P$-assignment strongly satisfying a collection of constraint can be converted into a $Q$-assignment weakly satisfying that those constraints by composing $h$ with the $P$-assignment.} \change{If $\Gamma = \{(P,Q)\}$ consists a single pair of predicates, we often state $\Gamma = (P,Q)$ and write $\PCSP(P, Q)$ for $\PCSP(\Gamma)$.} The following are computational problems associated with \change{instances of} $\PCSP(\Gamma)$. 

\begin{definition}[\change{PCSP problems}]\label{def:pcsp-prob} In the decision version of $\PCSP(\Gamma)$, given an input instance $\Phi=(V,\mathcal{C})$ of $\PCSP(\Gamma)$, the objective is to distinguish between the two cases.
  \begin{enumerate}
...
In the search version of $\PCSP(\Gamma)$, given an input instance $\Phi=(V,\mathcal{C})$ of $\PCSP(\Gamma)$ with the promise that there is an assignment $\sigma_1: V\rightarrow D_1$ that strongly satisfies all the constraints, the objective is to find an assignment $\sigma_2: V \rightarrow D_2$ that weakly satisfies all the constraints.\footnote{\change{This explains the terminology \emph{promise} CSPs to refer to such problems. For the decision version, a promise problem consists of disjoint sets of Yes and No instances, which together need not exhaust all instances.}}
 
\change{An algorithm for $\PCSP(\Gamma)$ is said to be $f$-robust algorithm for a function $f:\R \rightarrow \R$ if the following holds. The inputs to the algorithm are an instance $\Phi=(V,\mathcal{C})$ of $\PCSP(\Gamma)$ and a parameter $\epsilon$} with the promise that there is an assignment $\sigma_1: V\rightarrow D_1$ that strongly satisfies $1-\epsilon$ fraction of the constraints. \change{The objective of the algorithm is to output an assignment} $\sigma_2: V \rightarrow D_2$ that weakly satisfies at least $1-f(\epsilon)$ fraction of the constraints. \change{We call such an algorithm a robust algorithm for $\PCSP(\Gamma)$ if it is $f$-robust for some monotone and non-negative function $f$ with $f(\epsilon)\rightarrow 0$ as $\epsilon \rightarrow 0$.} %
\end{definition}

In this paper, we restrict ourselves to PCSP \emph{templates} where both the domains are equal to $\change{\B :=} \{-1,+1\}$. \change{We call such templates \emph{Boolean} templates.} \change{For technical reasons, to more closely align with the conventions in the robust CSP literature, we consider a different notion of an PCSP instance which incorporates \emph{variable folding}, i.e., the negation of variables. We also assume that the PCSP instance allows for the setting of constants, which we call \emph{idempotence}.}

%

\begin{definition}[Boolean folded idempotent \change{instance/assignment}] \change{Given a Boolean template $\Gamma=\{(P_1,Q_1),$ $(P_2,Q_2),\ldots,$ $(P_l,Q_l)\}$ where $P_i \subseteq Q_i \subseteq \B^{k_i}$ for every $i \in [l]$, an instance $\Phi=(V,\mathcal{C})$ of $\fiPCSP(\Gamma)$ consists of a set of $n$ variables $V = \{u_1, \hdots, u_n\}$ and a set of $m$ constraints $\mathcal{C} = \{C_1, \hdots, C_m\}$. For $i \in [n]$, each variable $u_i$ has two corresponding \emph{literals}: $u_i$ and $\overline{u_i}$, the positive and negative literals, respectively. \change{Observe that we identify the positive literal with the variable itself.} We also have two constant literals corresponding to the elements of $\B$. For each $j \in [m]$, $C_j$ consists} of a tuple of literals $C_j = (x_{j,1}, x_{j,2}, \ldots, x_{j,{l_j}}) \in \change{(}V\cup \overline{V} \cup \B\change{)}^{l_j}$ and an associated predicate pair $(P^{(j)},Q^{(j)}) \in \Gamma$ of the same arity $l_j$. 

Given an assignment\footnote{\change{Since the families $P_i$ and $Q_i$ of predicates have a common domain $\B$, we need not distinguish between ``$P$-assignments'' and ``$Q$-assignments'' in the Boolean setting.}} $\sigma : V \rightarrow \B,$ \change{define its extended assignment $\sigma' : V \cup \overline{V} \cup \B \rightarrow \B$ to be} $\sigma'(u_i)=\sigma(u_i)$ and $\sigma'(\overline{u_i})=-\sigma(u_i)$ for every $i \in [n]$ \change{and $\sigma'(b) = b$ for $b \in \B$}. The assignment $\sigma$ is said to strongly (and \change{respectively} weakly) satisfy the constraint $C_j$ in $\Phi$ if $\sigma'(C_j) \in P^{(j)}$ (and \change{respectively} $\sigma'(C_j) \in Q^{(j)}$). 
\end{definition}

%
\change{We now restate the portion of Definition~\ref{def:pcsp-prob} for robust PCSP in the fiPCSP setting.
%
%
%
}
...
Associated with every PCSP \change{template, the notion of polymorphism captures the various} closure properties of the satisfying solutions \change{to any instance of the corresponding PCSP.} More formally, we can define the polymorphisms of a PCSP \change{template} as follows. 

\begin{definition}[Polymorphisms of PCSPs]
	\change{Given a promise template} $\Gamma = \{ ((P_1,Q_1), (P_2, Q_2), \ldots, (P_l,Q_l))\}$ such that for every $i \in [l]$, $P_i\subseteq D_1^{k_i}, Q_i\subseteq D_2^{k_i}$, a polymorphism of arity $\change{L}$ is a function $f:D_1^{\change{L}} \rightarrow D_2$ that satisfies the below property for every $i \in [l]$. %
	For all $\textbf{v}_1,\textbf{v}_2,\ldots,\textbf{v}_{k_i}\in D_1^\change{L}$ satisfying $( (\textbf{v}_1)_j, (\textbf{v}_2)_j, \ldots, (\textbf{v}_{k_i})_j ) \in P_i$ for each $j \in [\change{L}]$, we have 
	\[
	(f(\textbf{v}_1), f(\textbf{v}_2), \ldots , f(\textbf{v}_{k_i})) \in Q_i
	\]
    \change{If $\Gamma$ is Boolean (i.e., $D_1 = D_2 =\B$), we say that $f:\B^n\rightarrow \B$ is \emph{folded} if $f(-\textbf{v})=-f(\textbf{v})$ for all $\textbf{v}\in \B^n$.} \change{Likewise, we say that that $f$ is idempotent if $f(1,1,\ldots,1)=1$ and $\change{f(-1,-1,\ldots,-1)=-1}$.}
	We use $\emph{Pol}(\Gamma)$ to denote the family of all the polymorphisms of \change{$\Gamma$}. %
\end{definition}

\change{Given a predicate $P \subseteq D_1^k$ and a function $f : D_1^L \to D_2$, we let $f(P) \subseteq D_2^k$ denote the set
\[
...
\change{For our algorithmic results, we focus on studying Boolean promise templates $\Gamma$ whose corresponding polymorphisms contain particular families of functions.} \change{In particular, we} extensively study the \change{a}lternate-\change{t}hreshold $(\AT)$ and \change{m}ajority $(\MAJ)$ polymorphisms in this paper:
\begin{enumerate}
    \item For an odd integer $L\geq 1$ and $\textbf{x}\in\change{\B}^L$, we have $
    \AT_L(\textbf{x})=+1$, if $x_1-x_2+x_3-\ldots+x_L >0$, and $-1$, otherwise.
    \item For an odd integer $L\geq 1$ and $\textbf{x}\in\change{\B}^L$, we have $
    \MAJ_L(\textbf{x})=+1$, if $x_1+x_2+x_3+\ldots+x_L >0$, and $-1$, otherwise.
\end{enumerate}
\change{We let $\AT$ denote the set of $\AT_L$ for $L \ge 1$ odd and $\MAJ$ the set of $\MAJ_L$ for $L \ge 1$ odd.}
%

\smallskip \noindent \textbf{Relaxations of PCSPs.} We say that a \change{promise template} $\Gamma'$ is a relaxation of another \change{promise template }$\Gamma$ if $\Pol(\Gamma)\subseteq \Pol(\Gamma')$. If $\Gamma'$ is a relaxation of $\Gamma$, then there is a \textit{gadget reduction} from $\Gamma' \cup \{(=, =)\}$ to $\Gamma \cup \{(=, =)\}$, where $=$ denotes the equality predicate in the relevant domain. More formally, it is referred to as $\Gamma' \cup \{(=, =)\}$ is \textit{positive primitive promise (ppp)-definable} from $\Gamma \cup \{(=, =)\}$. 

\begin{definition}[ppp-definability of PCSPs~\cite{Chen09, BrakensiekG21}]
\label{def:ppp}
We say that a \change{pair of predicates $(P',Q')$ of the same arity} is ppp-definable from a \change{promise template} $\Gamma$ over the same domain pair if there exists a fixed constant $l$ and an instance $\Phi$ of $\PCSP(\Gamma)$ over $k+l$ variables $u_1, u_2, \ldots, u_k, v_1, v_2, \ldots, v_l$ such that 
\begin{enumerate}
...
\end{enumerate}
More generally, we say that \change{a promise template} $\Gamma'$ is ppp-definable from \change{another promise template} $\Gamma$ if every predicate pair in $\Gamma'$ is ppp-definable from $\Gamma$. 
\end{definition}
%
...
We now observe that the same holds for the robust version as well. More formally, we have the following proposition.\footnote{We previously claimed falsely that the reduction holds more generally when we are given that $\Gamma'$ is a relaxation of $\Gamma$. We are grateful to Andrei Krokhin for pointing this mistake to us. Under the general assumption that $\Gamma'$ is a relaxation of $\Gamma$, we can reduce the robust version of $\Gamma'$ to the robust version of $\Gamma$ together with binary equality constraints. For the sake of exact satisfiability, the equality constraints do not matter~\cite{BartoKW17,BrakensiekG21} but we do not know a way to obtain a robust algorithm for \change{$\PCSP(\Gamma)$} together with binary equality constraints just using the fact that $\Gamma$ has a robust algorithm. Even for CSPs, \change{a} direct way to show this is not known~\cite{DalmauK13}, without using the full robust dichotomy result of~\cite{BartoK16}.}

%
\begin{proposition}
\label{prop:ppp-defn}
Suppose that the \change{promise template} $\Gamma'$ over \change{$(D_1, D_2)$} is ppp-definable from $\Gamma$ over the same domain\change{.} \change{If there exists an $f$-robust algorithm for $\PCSP(\Gamma)$, then there exists an $O_{\Gamma,\Gamma'}(1)\cdot f$-robust algorithm for $\PCSP(\Gamma')$.}
\end{proposition}

\begin{proof}[\change{Proof}]
Given an instance $\Phi'=(V',\mathcal{C}')$ of $\change{\PCSP(\Gamma')}$ over a set of variables $V'$ and containing $m$ constraints $\mathcal{C}'=\{C'_1,C'_2,\ldots,C'_m\}$, we output an instance $\Phi$ of $\change{\PCSP(\Gamma)}$ containing $|V'|$ original variables and a set of dummy variables. 
For every constraint $C'_j$ using $(P', Q')$ of arity $k_j$ involving the \change{tuple of literals} $(u_{j,1}, u_{j,2}, \ldots, u_{j,k_j}) $ in $\Phi'$, we have a set of dummy variables $w_{j,1}, w_{j,2}, \ldots, w_{j,l_j}$ and a set of constraints $\change{\mathcal C}_j$ \change{whose variable scope is} $\change{S_j \cup \{}w_{j,1}, w_{j,2}, \ldots, w_{j,l_j}\}$ as in~\Cref{def:ppp}. Let $W=\cup_j \{ w_{j,1}, w_{j,2}, \ldots, w_{j,l_j}\}$ and let $V=V'\cup W$ be the set of variables of $\Phi$, and $\mathcal{C}=\cup_{j\in[m]}\change{\mathcal{C}}_j$ \change{is} the set of constraints in $\Phi$.
We claim that this reduction preserves robust algorithms. \begin{enumerate}
    \item (Completeness). Suppose that there exists an assignment $\sigma': V' \rightarrow D_1$  strongly satisfying $1-\epsilon$ fraction of the constraints in $\Phi'$. For every $j \in [m]$ such that $\sigma'$ strongly satisfies the constraint $C'_j$, there is an assignment $\sigma_j:\{ w_{j,1}, w_{j,2}, \ldots, w_{j,l_j}\}\rightarrow D_1$ such that $\sigma \cup \sigma_j : C_j\rightarrow D_1$ strongly satisfies all the constraints \change{of $\mathcal{C}_j$}. Consider an assignment $\sigma: V \rightarrow D_1$ where we set $\sigma(u_i)=\sigma'(u_i)$ for $u_i \in V'$, $\sigma(w_{j,i})=\sigma_j(w_{j,i})$ for all $w_{j,i}\in W$ such that $\sigma$ strongly satisfies the constraint $C'_j$. If $\sigma$ does not strongly satisfy the constraint $C'_j$, we set $\sigma(w_{j,i})$ arbitrarily. The assignment $\sigma$ strongly satisfies at least $1-O_{\Gamma,\Gamma'}(\epsilon)$ fraction of the constraints in $\Phi$. 
    \item (Soundness). Suppose that there is an assignment $\sigma : V \rightarrow D_2$
    weakly satisfying $1-\epsilon$ fraction of the constraints in $\Phi$. \change{By Markov's inequality, f}or at least $1-O_{\Gamma,\Gamma'}(\epsilon)$ values of $j \in [m]$, all the constraints in $\change{\mathcal C}_j$ are weakly satisfied by $\sigma$. This shows that the assignment $\sigma$ restricted to $V'$ weakly satisfies $1-O_{\Gamma,\Gamma'}(\epsilon)$ fraction of the constraints in $\Phi'$. \qedhere
\end{enumerate}
\end{proof}
%
...
\change{In the folded, idempotent setting, we also allow for negations of variables and the setting of constants, we represent these additional operations via what we coin as ``fippp''-definitions.}

\change{
\begin{definition}[fippp-definability of fiPCSPs]
\label{def:fippp}
We say that a \change{pair of predicates $(P',Q')$ of the same arity} is fippp-definable from a \change{promise template} $\Gamma$ over the same domain pair if there exists a fixed constant $l$ and an instance $\Phi$ of $\fiPCSP(\Gamma)$ over $k+l$ variables $u_1, u_2, \ldots, u_k, v_1, v_2, \ldots, v_l$ such that 
\begin{enumerate}
    \item If $(x_1, x_2, \ldots, x_k) \in P'$, then there exist $y_1, y_2, \ldots, y_l$ such that the assignment $(x_1, \ldots, x_k,y_1, \ldots, y_l)$ strongly satisfies all the constraints in $\Phi$. 
    \item If there is an assignment $(z_1, \ldots, z_{k+l})$ weakly satisfying all the constraints in $\Phi$, then $(z_1, z_2, \ldots, z_k)\in Q'$. 
\end{enumerate}
More generally, we say that \change{a promise template} $\Gamma'$ is fippp-definable from \change{another promise template} $\Gamma$ if every predicate pair in $\Gamma'$ is fippp-definable from $\Gamma$. 
\end{definition}
}
...
Suppose that the \change{promise template} $\Gamma'$ over \change{$(D_1, D_2)$} is fippp-definable from $\Gamma$ over the same domain\change{.} \change{If there exists an $f$-robust algorithm for $\fiPCSP(\Gamma)$, then there exists an $O_{\Gamma,\Gamma'}(1)\cdot f$-robust algorithm for $\fiPCSP(\Gamma')$.}
\end{proposition}

The proof of Proposition~\ref{prop:fippp-defn} is essentially identical to that of Proposition~\ref{prop:ppp-defn}. %

\begin{proof}[\change{Proof}] 
Given an instance $\Phi'=(V',\mathcal{C}')$ of $\change{\fiPCSP(\Gamma')}$ over a set of variables $V'$ and containing $m$ constraints $\mathcal{C}'=\{C'_1,C'_2,\ldots,C'_m\}$, we output an instance $\Phi$ of $\change{\fiPCSP(\Gamma)}$ containing $|V'|$ original variables and a set of dummy variables. 
For every constraint $C'_j$ using $(P', Q')$ of arity $k_j$ involving the \change{tuple of literals} $(u_{j,1}, u_{j,2}, \ldots, u_{j,k_j}) $ in $\Phi'$, we have a set of dummy variables $w_{j,1}, w_{j,2}, \ldots, w_{j,l_j}$ and a set of constraints $\change{\mathcal C}_j$ \change{whose variable scope is} $S_j \cup \{w_{j,1}, w_{j,2}, \ldots, w_{j,l_j}\}$ as in~\Cref{def:fippp}. Let $W=\cup_j \{ w_{j,1}, w_{j,2}, \ldots, w_{j,l_j}\}$ and let $V=V'\cup W$ be the set of variables of $\Phi$, and $\mathcal{C}=\cup_{j\in[m]}\change{\mathcal{C}}_j$ \change{is} the set of constraints in $\Phi$.
We claim that this reduction preserves robust algorithms. \begin{enumerate}
    \item (Completeness). Suppose that there exists an assignment $\sigma': V' \rightarrow D_1$  strongly satisfying $1-\epsilon$ fraction of the constraints in $\Phi'$. For every $j \in [m]$ such that $\sigma'$ strongly satisfies the constraint $C'_j$, there is an assignment $\sigma_j:\{ w_{j,1}, w_{j,2}, \ldots, w_{j,l_j}\}\rightarrow D_1$ such that $\sigma \cup \sigma_j : C_j\rightarrow D_1$ strongly satisfies all the constraints \change{of $\mathcal{C}_j$}. Consider an assignment $\sigma: V \rightarrow D_1$ where we set $\sigma(u_i)=\sigma'(u_i)$ for $u_i \in V'$, $\sigma(w_{j,i})=\sigma_j(w_{j,i})$ for all $w_{j,i}\in W$ such that $\sigma$ strongly satisfies the constraint $C'_j$. If $\sigma$ does not strongly satisfy the constraint $C'_j$, we set $\sigma(w_{j,i})$ arbitrarily. The assignment $\sigma$ strongly satisfies at least $1-O_{\Gamma,\Gamma'}(\epsilon)$ fraction of the constraints in $\Phi$. 
    \item (Soundness). Suppose that there is an assignment $\sigma : V \rightarrow D_2$
    weakly satisfying $1-\epsilon$ fraction of the constraints in $\Phi$. \change{By Markov's inequality, f}or at least $1-O_{\Gamma,\Gamma'}(\epsilon)$ values of $j \in [m]$, all the constraints in $\change{\mathcal C}_j$ are weakly satisfied by $\sigma$. This shows that the assignment $\sigma$ restricted to $V'$ weakly satisfies $1-O_{\Gamma,\Gamma'}(\epsilon)$ fraction of the constraints in $\Phi'$. \qedhere
...
%
We now describe the Basic SDP relaxation of an instance of a PCSP, similar to how it is presented in \cite{Raghavendra08}. Let \change{$\Gamma$ be a promise template over domain $(D,D')$, and let} $\Phi$ be an instance of \change{$\PCSP(\Gamma)$} over $n$ variables $V=\{u_1, u_2, \ldots, u_n\}$ and $m$ constraints $C_1, C_2, \ldots, C_m$. Suppose that the constraint $C_j$ \change{subjects the variables} $( u_{j,1}, u_{j,2}, \ldots, u_{j,l_j})$ \change{to the predicate pair} $(P^{(j)}, Q^{(j)})$.  %
In the basic SDP relaxation of $\change{\PCSP(\Gamma)}$ corresponding to $\Phi$, we have a vector $\textbf{v}_{i,a}$ corresponding to each variable $u_i$, $i \in [n]$, along with a label $a \in D$. We also have a unit vector $\textbf{v}_0$. 
For each constraint $C_j$, $j \in [m]$, there is a probability distribution (referred to as the \emph{local distribution} of the constraint $C_j$) \change{over the set of possible local assignments to that constraint.} 
\change{Since $C_j$ may contain repeated variables, we let $S_j := \{u_{j,1}, \hdots, u_{j,l_j}\}$ denote the set of variables appearing in $C_j$. Our probability distribution is then over the set of functions $\{f : S_j \to D\}$.} We represent this using a variable $\lambda_j(f)$ for every $j\in [m]$ and \change{local} assignment $f: \change{S_j} \rightarrow D$. \change{Note that we use $f(C_j)$ as shorthand for $( f(u_{j,1}), f(u_{j,2}), \ldots, f(u_{j,l_j}))$} Finally, we have an error parameter $\eps_j$ corresponding to the constraint $j, j \in [m]$, equal to the probability that $\lambda_j$ is supported outside $P^{(j)}$. We refer to $\epsilon_j$ as the error of the basic SDP relaxation on the constraint $j$, and $\sum_{j=1}^m \eps_j$ as the error of the basic SDP relaxation of the instance $\Phi$.
%
\begin{align*}
    \textbf{minimize: } & \sum_{j=1}^m \eps_j & \\
\textbf{subject to: } \eps_j &\geq 0  & \forall j \in [m]\\
\lambda_j(f) &\geq 0  & \forall j\in [m], ~ f:\change{S}_j \rightarrow \change{D} \\ 
\sum_{f:\change{S}_j \rightarrow D}\lambda_j(f)&=1  & \forall j \in [m] \\ 
 \sum_{f:\change{S}_j \rightarrow D, f(C_j)\in P^{(j)}} \lambda_j(f) &= 1-\eps_j \quad & \forall j \in [m]\\
\change{ \langle \bv_0, \bv_0\rangle} &\change{= 1}\\
\text{ (First moments.)  } \qquad \change{ \langle \textbf{v}_{i,a},   \textbf{v}_0 \rangle } &= \sum_{\substack{f:\change{S}_j \rightarrow D\\f(\change{u_i}) = a}}\lambda_j(f)&  \forall j\in [m],~\change{u_i} \in \change{S}_j,~ a \in D \\ 
\text{(Second moments.)  } \quad \change{ \langle \textbf{v}_{i,a} , \textbf{v}_{i',a'} \rangle } &= \sum_{\substack{f:\change{S}_j \rightarrow D\\f(\change{u_i}) = a, f(\change{u_{i'}}) = a'}}\lambda_j(f)  & \forall j\in [m],~\change{u_i,u_{i'}} \in \change{S}_j,~ a,a' \in D 
\end{align*}
%
%
\change{We implicitly assume that every variable appears in at least one clause, so the second moment condition also implies that $\langle v_{i,a}, v_{i,a'} \rangle = 0$ for $a \neq a'$ for all $i \in [n]$.}

%

\begin{proposition}\label{prop:basic-SDP-completeness}
Let $\Gamma$ be a promise template over domain $(D_1, D_2)$. Suppose that $\Phi=(V,\mathcal{C})$ is an instance of a $\PCSP(\Gamma)$ such that there is an assignment $\sigma : V \rightarrow D_1$ that strongly satisfies all the constraints. Then, the basic SDP \change{relaxation of $\Phi$} is feasible \change{with objective value $0$}.
\end{proposition}
\begin{proof}
    \change{We embed our vectors in the real line.} We set $\textbf{v}_0=1$, and for every $i \in [n]$ and $a \in D_1$, we set
    \[
        \textbf{v}_{i,a} := \begin{cases}
        1 & a = \sigma(u_i),\\
        0 & \text{otherwise.}
        \end{cases}
    \] \change{For any} $j \in [m]$ and $f:\change{S}_j \to D_1$, we set $\lambda_j(f)=1$ if $f(x)=\sigma(x)$ for every $x \in \change{S}_j$, and we set $\lambda_j(f)=0$ otherwise. These variables satisfy all the constraints in the basic SDP relaxation with $\eps_j=0$ for all $j \in [m]$.
\end{proof}

\noindent \textbf{Boolean folded \change{idempotent} variant.}
In Sections~\ref{sec:alg} and \ref{sec:ug-hardness} of the paper, we prefer to consider an alternative formulation of the Basic SDP for the \change{Boolean folded idempotent PCSP.} \change{We let $\Gamma$ be a promise template and c}onsider an instance $\Phi=(V,\mathcal{C})$ of \change{$\fiPCSP(\Gamma)$} where $V=\{u_1,u_2,\ldots,u_n\}$ and $\mathcal{C}=\{C_1,C_2,\ldots,C_m\}$ with the constraint $C_j$ using the predicate pair $(P^{(j)},Q^{(j)})$ for $j\in[m]$. \change{Let $\mathcal L = V \cup \overline{V} \cup \B$ be the set of literals. Each clause is a tuple of literals $C_j = (x_{j,1}, x_{j,2}, \ldots, x_{j,{l_j}}) \in \mathcal L^{l_j}$ bound by the predicate pair $(P^{(j)}, Q^{(j)})$.}

\change{For each variable $u_i \in V$, we have an associated vector $\textbf{v}_i$ along with a truth vector $\bv_0$} %
\change{We extend this association to all literals by assigning a map $\bv : \mathcal L \to \{\bv_1, \hdots, \bv_n\}$ where
\begin{align}
    \bv(x) = \begin{cases}
        \bv_i & x = u_i \in V\\
...
\change{For each clause $C_j \in \mathcal C$, we let $S_j$ denote the set of variables appearing in $C_j$. In particular, $S_j = \{u_i \in V : \exists k \in [l_j], x_{j,l_j} = u_i \vee x_{j,l_j} = \overline{u_i}\}$. Given a local assignment of variables $f : S_j \to \B$, we let $f(C_j) = (f(x_{j,1}), f(x_{j,2}), \ldots, f(x_{j,{l_j}}))$, where we define $f(\overline{u_i}) = -f(u_i)$ and $f(\pm 1) = \pm 1$.}

\change{The relaxation is as follows.}
%
\begin{align*}
    \textbf{minimize: } & \sum_{j=1}^m \eps_j & \\
\textbf{subject to: } \eps_j &\geq 0 & \forall j \in [m]\\
\lambda_j(f) &\geq 0 & \forall j\in [m], ~f:\change{S}_j \rightarrow \change{\B} \\ 
\sum_{f:\change{S}_j \rightarrow \change{\B}}\lambda_j(f)&=1 & \forall j \in [m] \\ 
 \sum_{f:\change{S}_j \rightarrow \change{\B}, f(C_j)\in P^{(j)}} \lambda_j(f) &= 1-\eps_j & \forall j \in [m]\\
\norm{\textbf{v}_i }_2^2 &= 1 & \forall i \in \{0,1,\ldots,n\} \\ 
\text{ (First moments.)  } \qquad \change{ \langle \textbf{v}(x) , \textbf{v}_0 \rangle }&= \sum_{f:\change{S}_j \rightarrow \change{\B}}\lambda_j(f) f(x) & \forall j\in [m],~x \in C_j \\
\text{(Second moments.)  } \quad \change{ \langle \textbf{v}(x) , \textbf{v}(x') \rangle } &= \sum_{f:\change{S}_j \rightarrow \change{\B}}\lambda_j(f) f(x)f(x') & \forall j\in [m],~x,x' \in C_j
\end{align*}
...
%
\begin{proposition}
    Suppose that $\Phi=(V,\mathcal{C})$ is an instance of a Boolean folded \change{idempotent} PCSP such that there is an assignment $\sigma : V \rightarrow \change{\B}$ that strongly satisfies all the constraints. Then, the basic SDP \change{relaxation of $\Phi$} is feasible \change{with objective value $0$}.
\end{proposition}
%
\begin{proof}
    \change{We embed our vectors in the real line.} We set $\textbf{v}_0=1$, and $\textbf{v}_i := \sigma(u_i) \in \R$ for every $i \in [n]$. \change{For any} $j \in [m]$ and $f:\change{S}_j \rightarrow \change{\B}$, we set $\lambda_j(f)=1$ if $f(x)=\sigma(x)$ for every $x \in C_j$, and we set $\lambda_j(f)=0$ otherwise. These variables satisfy all the constraints in the basic SDP relaxation with $\eps_j=0$ for all $j \in [m]$.
\end{proof}
...
\begin{definition}
\change{For a promise template $\Gamma$, w}e say that the basic SDP \textit{decides} \change{$\PCSP(\Gamma)$} if for every instance $\Phi$ such that the basic SDP is feasible on $\Phi$, there is an assignment to $\Phi$ that weakly satisfies all the constraints. 
\end{definition}

\change{\paragraph{Efficient solvability.} Polynomial-time SDP solving algorithms can only sovle the problem ``approximately.'' In the SDP literature, this notion of approximation can vary widely. For our purposes, we are satisfied if the objective is approximate (computed to within $1/\poly(n)$ precision), but the solution returned is a ``feasible'' solution to the SDP.
...
%
%
\change{\subsection{Gram and orthogonal matrices}\label{subsec:gram-ortho}
...
We \change{state} a couple of elementary properties of Gaussian distribution that we use later. 
...
Suppose that $X\sim \mathcal{N}(0,1)$ has the standard Gaussian distribution. Then, for every \change{ $c$ and} $\epsilon \geq 0$, 
\[
\change{\text{Pr}\left( X \in [c-\epsilon, c+\epsilon]\right)} \leq \epsilon.
\]
\end{proposition}
...
We have 
\[
    \change{\text{Pr}\left( X \in [c-\epsilon, c+\epsilon]\right) = \int_{c-\epsilon}^{c+\epsilon} \frac{1}{\sqrt{2\pi}}e^{-\frac{x^2}{2}}dx  \leq \int_{c-\epsilon}^{c+\epsilon} \frac{1}{\sqrt{2\pi}} dx} \le \epsilon. \qedhere
\]
\end{proof}
...
\section{Overview of \change{T}echniques}
\label{sec:overview}

\change{Moving forward, unless otherwise specified, all promise templates $\Gamma$ are Boolean, and we consider the folded, idempotent variant of promise constraint satisfaction, i.e., $\fiPCSP(\Gamma)$.}

%

\subsection{Robust algorithm for MAJ polymorphisms}
\label{sec:maj-overview}
We obtain our robust algorithms by first solving the basic SDP relaxation and then \textit{rounding} the vectors. We first illustrate the SDP rounding idea with a warm-up algorithm (originally appeared in~\cite{Zwick98}) to solve the decision version of $2$-SAT CSP predicate $P=\{(-1,+1),(+1,-1),(+1,+1)\}$. Consider an instance $\Phi$ of \change{2-SAT (i.e., $\fiPCSP(P,P)$)} over a set of $n$ variables and $m$ constraints. We first solve the basic SDP relaxation of $\Phi$. If the basic SDP relaxation has a strictly positive error, then the instance is clearly not satisfiable. \change{For the moment}, suppose that the relaxation has zero error and we found a set of vectors $\textbf{v}_0, \textbf{v}_1, \ldots,\textbf{v}_n$, and the local probability variables $\lambda_j(f)$ for all $j \in [m], f: \change{S}_j \rightarrow \change{\B}$ that satisfy all the constraints in the basic SDP relaxation with $\eps_j=0$ for every $j \in [m]$. Consider an arbitrary constraint $C_j$ using the literals $x_1, x_2$. We abuse the notation and let $\textbf{v}_1, \textbf{v}_2$ denote the vectors assigned by the SDP to $x_1$ and $x_2$ respectively \change{(i.e., $\bv(x_1)$ and $\bv(x_2)$)}.
...
These properties motivate the following simple rounding algorithm that outputs the assignment $\sigma: V \rightarrow \change{\B}$. We sample a random vector $\zeta \sim \mathcal{N}(0,\textbf{I})$, and set
\[
...
%
\change{By setting $\delta = \epsilon$ in \Cref{thm:sdp}, we get that the average error of the basic SDP relaxation is at most O($\epsilon$) on any instance that is promised to satisfy $1-\epsilon$ fraction of the constraints.}
Zwick~\cite{Zwick98} gave a robust algorithm \change{based on rounding the basic SDP relaxation} for $2$-SAT which was later improved by Charikar, Makarychev, and Makarychev~\cite{CharikarMM09}. They sample a random vector $\zeta \sim \mathcal{N}(0,\textbf{I})$, and set 
\[
\sigma(u_i) = \begin{cases}
...
\end{cases}
\]
One can view their algorithm as a smoothed version of the earlier discussed algorithm: \change{ Using~\Cref{prop:gaussian-multivariate}, the random variable $r_i = \langle \textbf{v}_i, \zeta \rangle$ follows $ \mathcal{N}(0,1)$ for all $i \in [n]$. Thus, if $|\langle \textbf{v}_i, \textbf{v}_0 \rangle| > \lambda\sqrt{\epsilon}$ %
...
%
The output assignment $\sigma$ violates $Q$ on this constraint only if $\change{\langle  \textbf{v}_i, \zeta \rangle} \leq -\frac{\langle \textbf{v}_i, \textbf{v}_0 \rangle}{\sqrt{\epsilon}}$ for every $i \in [4]$, or equivalently, $\change{\langle  \textbf{v}_i, \zeta \rangle} +\frac{\langle \textbf{v}_i, \textbf{v}_0 \rangle}{\sqrt{\epsilon}} \leq 0$ for every $i \in [4]$.
...
\[
\sum_{i \in [4]} \left(\change{\langle  \textbf{v}_i, \zeta \rangle} +\frac{\langle \textbf{v}_i, \textbf{v}_0 \rangle}{\sqrt{\epsilon}}\right) \geq -O(\epsilon^{1/4}).
\]
If $\sigma$ does not satisfy $Q$, then for some $i \in [4]$, we have that 
\[
\change{\langle  \textbf{v}_i, \zeta \rangle} +\frac{\langle \textbf{v}_i, \textbf{v}_0 \rangle}{\sqrt{\epsilon}} \in \left[ -O(\epsilon^{1/4}),0\right]
\]
%
Finally, we can upper bound the probability that this occurs to be at most $\tilde{O}\left(\epsilon^{1/4}\right)$ using anti-concentration of the Gaussian $\change{\langle  \textbf{v}_i, \zeta \rangle} \sim \mathcal{N}(0,1)$ \change{(\Cref{prop:gaussian-anticoncentration})}. Thus, we can infer that the assignment $\sigma$ satisfies at least $1-\tilde{O}(\epsilon^{1/4})$ fraction of the constraints in expectation. A careful analysis of the parameters gives a guarantee of $1-\tilde{O}(\epsilon^{1/3})$.

For an arbitrary \change{Boolean promise template} $\Gamma$ with $\MAJ\subseteq \Pol(\Gamma)$, we obtain a robust algorithm \change{for $\fiPCSP(\Gamma)$} by first reducing to the case when all the predicate pairs are of the form $(P,Q)$ with $Q=\change{\B}^k \setminus \{(-1,-1,\ldots,-1)\}$, generalizing the above two examples of \change{the} $2$-SAT \change{template} \change{(i.e., $P=Q=\{(-1,+1),(1,-1),(1,1)\}$)} and $(\Ham_{4}\{2,3,4\}$, $\Ham_4\{1,2,3,4\})$. Then, we find a weight vector $\textbf{w}$ which satisfies that $\change{ \langle \textbf{w}, \textbf{x}\rangle }\geq 0$ for all $\textbf{x}\in P$ ($\textbf{w}=(1,1,\ldots,1)$ suffices for the previous two examples). We prove the existence of such a vector $\textbf{w}$ by using a Linear Programming relaxation, and we crucially use the fact that $\change{\MAJ \subseteq \change{\Pol}(P,Q)}$ in the analysis of this LP relaxation.
...
As a concrete example, consider the \change{templates for $1$-in-$3$-SAT and NAE-SAT: $P = \{\bx \in \B^3 : x_1+x_2+x_3 = -1\}$ and $Q = \{\bx \in \B^3 : x_1+x_2+x_3 \in \{-1,1\}\}$, respectively. We have that $\AT \subseteq \Pol(P,Q)$~\cite{BrakensiekG21}.}
\change{Assume that an instance $(V,\mathcal C)$ of $\fiPCSP(P,Q)$ is perfectly satisfiable.} For \change{this} case, we can \change{efficiently find an exact solution} using the basic SDP relaxation via random hyperplane rounding as follows. Consider an arbitrary constraint $\change{C_j}$ and let $\textbf{v}_1, \textbf{v}_2, \textbf{v}_3$ denote the vectors assigned by the basic SDP to the literals in $\change{C_j}$. Using the fact that these vectors satisfy the first and second moment constraints of the basic SDP relaxation with zero error, we can infer that their sum $\textbf{v}_s = \textbf{v}_1 + \textbf{v}_2 + \textbf{v}_3$ is equal to $-\textbf{v}_0$ for every constraint $C$.  Let $\textbf{v}_i \perp \textbf{v}_0 = \textbf{v}_i - \langle \textbf{v}_i, \textbf{v}_0 \rangle \textbf{v}_0$ for $i \in [3]$. Note that $\sum_{i \in [3]} \textbf{v}_i \perp \textbf{v}_0 =0$. Using this observation, we can design a rounding scheme.
We first sample $\zeta \sim \mathcal{N}(0,\textbf{I})$, and set $\sigma(u_i)=+1$ if $\langle \textbf{v}_i \perp \textbf{v}_0 , \zeta \rangle > 0$, and $-1$ if $\langle \textbf{v}_i \perp \textbf{v}_0 , \zeta \rangle < 0$. \change{Each variable $u_i$ is assigned a value with probability $1$ unless $\bv_i \in \{\bv_0, -\bv_0\}$ since $\bv_i$ is a unit vector. In that undetermined case, we set $\sigma(\bu_i) = +1$ if $\bv_i = \bv_0$ and $\sigma(\bu_i) = -1$ otherwise.\footnote{\change{Observe this choice of assignment is consistent with the default rounding of the constant literals $\pm 1$.}}} As $\textbf{v}_1+\textbf{v}_2+\textbf{v}_3=-\textbf{v}_0$, the rounding scheme  ensures that at least one literal associated with these vectors is set to $+1$, and at least one literal is set to $-1$.

For the robust setting where we are only guaranteed that there is \change{an assignment strongly} satisfying $1-\epsilon$ fraction of the constraints. \change{By setting $\delta = \epsilon$ in \Cref{thm:sdp},} we get that the average SDP error is at most \change{O($\epsilon$)}. By Markov's inequality, we are guaranteed that for at least $1-\sqrt{\epsilon}$ fraction of the constraints, the SDP error is at most \change{O($\sqrt{\epsilon}$)}. For these constraints, we get that the sum vector $\textbf{v}_s$'s component orthogonal to $\textbf{v}_0$ has $\ell_2$ norm at most $O(\eps^{1/4})$, i.e., $\norm{\sum_{i \in 3}\textbf{v}_i \perp \textbf{v}_0}_2 \leq O(\epsilon^{1/4})$.
...
For an arbitrary Boolean \change{promise template} $\Gamma$ with $\AT \subseteq \Pol(\Gamma)$, we first show that \change{$\fiPCSP(\Gamma)$ reduces to $\fiPCSP(P,Q)$ where} 
\[
P=\{ \textbf{v} \in \change{\B}^k : \langle \textbf{v}, \textbf{w}\rangle = b \}, \quad Q = \change{\B}^k \setminus\{\textbf{x},-\textbf{x}\}
\]
for some $\textbf{w} \in \R^k, b \in \R$ with $w_i \neq 0\,~\forall i \in [k]$, where we set $\textbf{x}$ as $x_i = +1$ if $w_i>0$, and $-1$ otherwise.
This generalizes \change{our previous $1$-in-$3$-SAT versus NAE-$3$-SAT template} which corresponds to the case when $\textbf{w}=(1,1,1),b=-1$. We show that the algorithm above works for this general predicate pair as well, thereby obtaining \change{for every promise template $\Gamma$ with $\AT \subseteq \Pol(\Gamma)$} a robust algorithm for \change{$\fiPCSP(\Gamma)$.}
%
...
As mentioned earlier, our Unique Games hardness for \change{$\fiPCSP(\Gamma)$} (\Cref{thm:main-hardness}) is based on an integrality gap for the basic SDP relaxation, i.e., we need to show that there is a finite instance $\Phi$ of $\change{\fiPCSP(\Gamma)}$ that has SDP error of zero, yet there is no assignment that weakly satisfies all the constraints.  
...
\change{We \change{focus on Boolean promise templates which are symmetric.} We say that a predicate $P \subseteq \change{\B}^k$ is symmetric if for every $\textbf{x}, \textbf{y}\in \change{\B}^k$ such that $\textsf{hw}(\textbf{x})=\textsf{hw}(\textbf{y})$, we have $\textbf{x} \in P$ if and only if $\textbf{y} \in P$}. %

We first start by showing an integrality gap instance for $3$-LIN. Recall that the $3$-LIN CSP has the predicate $P$ with 
\begin{align}
P=\{ \textbf{x} \in \change{\B}^3 : x_1 + x_2 + x_3 = -1 \text{ or }x_1 + x_2 + x_3=+3\}.\label{eq:3-lin}
\end{align}
Consider the instance $\Phi$ \change{of $\fiPCSP(P,P)$} that uses three variables and uses two constraints $C_1, C_2$ with $C_1=\{(x_1,x_2,x_3)\},C_2=\{ (\overline{x_1}, \overline{x_2},\overline{x_3})\}$. The instance has no assignment that satisfies both the constraints, and we now show that the basic SDP solution has zero error on $\Phi$. We first describe the local probability variables: \change{for each $j \in [2]$ and each assignment $f : S_j \to \B$, where $S_j = \{u_1,u_2,u_3\}$,} we set 
\[
\lambda_j(f)=\change{\begin{cases}
1/4 & f(C_j) \in P\\
0 & \text{otherwise.}
\end{cases}}
\]
That is, we set each local distribution to be the uniform distribution over \change{satisfying assignments}. Substituting these in the first and second moment constraints, we get the following requirements that the vectors $\textbf{v}_1, \textbf{v}_2, \textbf{v}_3$ need to satisfy. 
\begin{align}
\label{eq:3lin-1}     \change{ \langle \textbf{v}_i , \textbf{v}_0 \rangle} &= 0 \quad \forall i \in [3] \\ 
    \label{eq:3lin-2} \change{ \langle \textbf{v}_i , \textbf{v}_{i'} \rangle } &= 0\quad \forall i \neq i' \in [3]
\end{align}
...
While the simple example gives an integrality gap for \change{3-LIN}, it is a challenging task to find such explicit integrality gap instances for general predicates. 
We develop a non-explicit approach where \change{for a given promise template $\Gamma$} we first construct an infinite integrality gap instance $\mathcal{I}^n(\Gamma)$ \change{for $\fiPCSP(\Gamma)$ and} then use it to show the existence of a finite integrality gap instance.
The variable set $V$ of $\mathcal{I}^n(\Gamma)$ to be the set of unit vectors in $\R^{n+1}$: $V=\{ u_{\textbf{v}} : \textbf{v} \in \S^{n} \}$. \change{We fix an arbitrary vector to be assigned $\textbf{v}_0$. We let $\mathcal L = V \cup \overline{V} \cup \B$ be our set of literals, and let $\hat{\bv} : \mathcal L \to 
\S^n$ be the map}
\[
    \change{\hat{\bv}(x) = \begin{cases}\bv & x = u_{\bv}\\
    -\bv & x = \overline{u_{\bv}}\\
...
\change{We now describe the clauses $\mathcal C$ of our integrality gap instance. Consider $(P,Q) \in \Gamma$ of arity $k$. For a potential clause $C = (x_1, \hdots, x_k) \in \mathcal L^k$, let $S$ be the set of variables making up $C$. We add $C$ to $\mathcal C$ if there exists a probability distribution $\lambda$ over assignments $f : S(C) \to \B$ satisfying the following constraints.}
\change{\begin{align*}
 \sum_{f:\change{S} \rightarrow \change{\B}, f(C)\in P} \lambda_j(f) &= 1\\
\text{ (First moments.)  } \qquad \change{ \langle \hat{\textbf{v}}(x) , \textbf{v}_0 \rangle }&= \sum_{f:\change{S} \rightarrow \change{\B}}\lambda(f) f(x) & ~x \in C\\
\text{(Second moments.)  } \quad \change{ \langle \hat{\textbf{v}}(x) , \hat{\textbf{v}}(x') \rangle } &= \sum_{f:\change{S} \rightarrow \change{\B}}\lambda(f) f(x)f(x') & x,x' \in C.
\end{align*}} 
%
We refer to \change{$\hat{\bv}(C) := (\hat{\bv}(x_1), \hdots, \hat{\bv}(x_k))$ satisfying the above as a \emph{$P$-configuration}} with respect to $\textbf{v}_0$. 
...
\change{We now return to examining $3$-LIN} in terms of $\mathcal{I}^n(P)$ \change{for $P$ as in (\ref{eq:3-lin})}. As mentioned earlier, a set of three vectors $\textbf{v}_1, \textbf{v}_2, \textbf{v}_3$ are a $P$-configuration with respect to $\textbf{v}_0$ if they satisfy~\Cref{eq:3lin-1} and~\Cref{eq:3lin-2}. 
Thus, to show that the instance $\mathcal{I}^n(P)$ does not have any assignment satisfying all the constraints, it suffices to show that for some positive integer $n$, there is no function $f:\mathbb{S}^n \rightarrow \change{\B}$ that satisfies the following condition:
...
\]
\change{Since we allow negations of variables in our instance}, we also require such a function $f$ to be folded, i.e., $f(-\textbf{v})=-f(\textbf{v})$ for every $\textbf{v}\in \mathbb{S}^{\change{n}}$. Such a coloring $f$ trivially does not exist: consider a set of three mutually orthogonal vectors that are all orthogonal to $\textbf{v}_0$, $V=(\textbf{v}_1, \textbf{v}_2, \textbf{v}_3)$ and their negations, $V'=(-\textbf{v}_1, -\textbf{v}_2, -\textbf{v}_3)$. Such a set of vectors is guaranteed to exist if $n \geq 4$.
Note that both these are valid $P$-configurations, but at least one of $f(\textbf{v}_1)+f(\textbf{v}_2)+ f(\textbf{v}_3)$, $f(-\textbf{v}_1)+ f(-\textbf{v}_2)+f(-\textbf{v}_3)$ does not belong to $\{-1,+3\}$, thus completing the proof that there is no assignment satisfying all the constraints of $\mathcal{I}^n(P)$. Hence, $\mathcal{I}^n(P)$ is an integrality gap instance for 3-LIN, i.e., $\change{\fiPCSP(P,P)}$, and this implies the existence of a finite integrality gap instance as well. While there is a direct integrality gap instance for the $3$-LIN, for an arbitrary \change{promise template} $\Gamma$, to show the existence of an integrality gap for the basic SDP relaxation, it is more convenient and practical to show the absence of any assignment $f:\S^{n}\rightarrow \change{\B}$ satisfying all the constraints of $\mathcal{I}^n(\Gamma)$ for some $n$ which then implies the existence of a finite integrality gap instance by a compactness argument.
%

While the $P$-configurations in the above proof for $3$-LIN are easy to study, in general, proving the absence of sphere coloring is challenging. For example, consider the \change{promise template} $\Gamma=(P,Q)$ where $P=\Ham_5 \{2,5\}$, $Q=\Ham_5 \{1,2,3,4,5\}$:
\[
P = \bigl\{ \textbf{x} \in \change{\B}^5 : |\{ i \in [5] : x_i = +1\} | \in \{2,5\} \bigr\}, \quad Q =\bigl\{ \textbf{x} \in  \change{\B}^5 : \sum_{i=1}^5 x_i \change{\neq}-5\bigr\}. 
\]
...
\subsection{CMM is a robust algorithm when \change{MAJ} is a polymorphism}

We restate~\Cref{thm:main-algorithm} for the case of $\MAJ$ polymorphisms. 
\begin{theorem}
\label{thm:maj-alg}
Let $\Gamma=\{(P_1,Q_1), (P_2, Q_2), \ldots, (P_l, Q_l)\}$ be a Boolean \change{promise template} with $\MAJ \subseteq \Pol(\Gamma)$. For every $\epsilon>0$, there is a randomized polynomial time algorithm that given an instance $\Phi$ of $\change{\fiPCSP}(\Gamma)$ that is promised to have an assignment satisfying $1-\epsilon$ fraction of the constraints, finds an assignment to $\Phi$ that satisfies $1-\tilde{O}_{\Gamma}(\epsilon^{\frac{1}{3}})$ fraction\footnote{We use $O_\Gamma$ to denote a hidden constant which depends on the specific template $\Gamma$.} of the constraints in expectation.
\end{theorem}

In the rest of this subsection, we prove~\Cref{thm:maj-alg}. Our strategy is to reduce the problem into a special case \change{in which} every predicate pair \change{of} $\Gamma$ is of the form $(P,\change{\B}^k \setminus \{(-1,-1,\ldots, -1)\})$, and then \change{solve the robust version of $\fiPCSP(\Gamma)$ using} the algorithm of Charikar, Makarychev, and Makarychev~\cite{CharikarMM09}.


For ease of notation, we use $O(\cdot)$ instead of $O_{\Gamma}(\cdot)$ when $\Gamma$ is clear from the context. 
We first \change{remove from $\Gamma$ all predicate pairs $(P,Q)$ for which $Q=\B^k$} for some integer $k$ since these constraints are trivially satisfied by any assignment. 
\change{To justify why we can remove these predicate pairs in the robust setting, let $\Phi$ be our instance of $\fiPCSP(\Gamma)$.} Suppose that there are $m$ constraints in $\Phi$, and $m'=\alpha m$ of them use predicates of the form $(P,Q)$ where $P\subseteq Q=\change{\B}^k$. We consider the instance $\Phi'$ containing $m-m'$ constraints obtained by deleting the constraints that use predicates of the form $(P,Q)$ where $P\subseteq Q=\change{\B}^k$. In the instance $\Phi'$, we are promised that there is a solution satisfying $m-m'-\epsilon m$ constraints, i.e., $1-\frac{\epsilon}{1-\alpha}$ fraction of the constraints. 
...
\]
constraints in $\Phi$. 
Thus, it suffices to study Boolean \change{promise templates} where no predicate pair is of the form $(P, \change{\B}^k)$. 

We further transform the instance into one in which every predicate pair is of the form $(P,\change{Q)}$ \change{with $Q = \change{\B}^k\setminus \{(-1,-1,\ldots,-1)\})$.}  

\begin{lemma}
\label{lem:maj-transform}
%
%
Consider a Boolean \change{promise template} $\Gamma=\{(P_1,Q_1), \ldots, (P_l, Q_l)\}$ where $P_i\subseteq  Q_i \subsetneq \change{\B}^{k_i}$ for every $i \in [l]$. \change{There exits a promise template $\Gamma'=\{(P'_1,Q'_1), \ldots, (P'_{l'}, Q'_{l'})\}$ and a polynomial time algorithm such that given an instance $\Phi$ of $\fiPCSP(\Gamma)$ over a set of variables $V$, the algorithm} outputs an instance $\Phi'$ of \change{$\fiPCSP(\Gamma')$} such that the following hold. 
\begin{enumerate}
    \item (Completeness.) \change{For every $\epsilon>0$,} if an assignment $\sigma : V \rightarrow \change{\B}$ strongly satisfies $1-\epsilon$ fraction of the constraints in $\Phi$, then $\sigma$ strongly satisfies at least \change{$1-O_{\Gamma}(\epsilon)$} fraction of the constraints in $\Phi'$ as well.
    \item (Soundness.) \change{For every $\epsilon>0$, }if an assignment $\sigma : V \rightarrow \change{\B}$ weakly satisfies $1-\epsilon$ fraction of the constraints in $\Phi'$, then $\sigma$ weakly satisfies at least \change{$1-O_{\Gamma}(\epsilon)$} fraction of constraints in $\Phi$. 
    \item The resulting \change{promise template} $\Gamma'$ satisfies the below two properties:
    \begin{enumerate}
        \item For every $i \in [l']$, $Q'_i$ is equal to $\change{\B}^{k'_i}\setminus \{(-1,-1,\ldots,-1)\}$ for some positive integer $k'_i$.
...
\begin{proof}
%
We obtain the above transformation in two steps. First, we construct a Boolean \change{promise template} $\Gamma^*$ from $\Gamma$ as follows: 
\[
\Gamma^* :=  \left\{ \left(P_i, \change{\B}^{k_i}\setminus \{\textbf{\change{b}}\}\right) : i \in [l], \textbf{\change{b}} \in \change{\B}^{k_i} \setminus Q_i \right\}
\]
...
Given an instance $\Phi$ of $\change{\fiPCSP(\Gamma)}$ over a set of variables $V$, we obtain an instance $\Phi^*$ of $\change{\fiPCSP(\Gamma^*)}$ over the same set of variables $V$ as follows. We order the constraints of $\Phi$ as $C_1, C_2, \ldots, C_m$. Consider a constraint $C_j \change{= (x_{j,1}, x_{j,2}, \ldots, x_{j,k_i})}$ in $\Phi$ using the predicate pair $(P_i,Q_i)$\change{.} 
In the instance $\Phi^*$, we add $2^{k_i}-|Q_i|$ constraints $C_{j,\textbf{\change{b}}}$ associated with every $\textbf{\change{b}} \in \change{\B}^{k_i} \setminus Q_i$. The constraint $C_{j,\textbf{\change{b}}}$ uses the same tuple of literals as $C_j$ but uses the predicate pair $(P_i, \change{\B}^k_i \setminus \{\textbf{\change{b}}\}).$ 
We analyze the completeness and soundness of this reduction. 
\begin{enumerate}
    \item (Completeness.) If $\sigma : V \rightarrow \change{\B}$ strongly satisfies a constraint $C_j$, then $\sigma$ strongly satisfies $C_{j,\textbf{\change{b}}}$ for every $\textbf{\change{b}} \in \change{\B}^{k_i}\setminus \change{Q_i}$. 
    %
    ...
    Here, we are using the fact that $m^* \geq m$, as we have $Q_i \neq \change{\B}^{k_i}$ for every $i \in [k]$.
    \item (Soundness.) Suppose that $\sigma : V \rightarrow \change{\B}$ weakly satisfies $1-\epsilon$ fraction of the constraints in $\Phi^*$. As $\sigma$ weakly violates at most $\epsilon m^* \leq 2^K \epsilon m$ constraints, for at least $1-2^K\epsilon$ fraction of the original constraints $C_j$, $\sigma$ weakly satisfies $C_{j,\textbf{\change{b}}}$ for every $\textbf{\change{b}} \in \change{\B}^{k_i}\setminus Q_i$. For these constraints $C_j$, $\sigma$ weakly satisfies $C_j$ as well, and thus, $\sigma$ weakly satisfies at least $1-2^K\epsilon$ fraction of the constraints in $\Phi$.
\end{enumerate}

Next, we transform $\Gamma^*$ to $\Gamma'$ to ensure that every weak predicate is of the form $\change{\B}^k\setminus\{(-1,\ldots,-1)\}$. 
%
We use the following entry-wise product notation: for a pair of vectors $\textbf{u},\textbf{v}\in\R^k$, we let
    ...
    \]
    For a predicate $P \subseteq \change{\B}^k$, we let 
    \[
    P \odot \textbf{v} := \{ \textbf{u} \odot \textbf{v} : \textbf{u} \in P \}.
    \]
We let $\Gamma'$ be the following. 
\[
\Gamma' := \left\{ \left(P \odot (-\textbf{\change{b}}), \change{\B}^k \setminus \{(-1,-1,\ldots,-1)\}\right) : \left(P,\change{\B}^k\setminus \{\textbf{\change{b}}\}\right) \in \Gamma^*\right\}
\]
Given the instance $\Phi^*$ of $\change{\fiPCSP(\Gamma^*)}$ that is obtained from $\Phi$,  we construct an instance $\Phi'$ of $\change{\fiPCSP(\Gamma')}$ on the same set of variables as follows. Consider a constraint $C_{j,\textbf{\change{b}}}$ using the predicate pair $(P_i, \change{\B}^{k_i}\setminus \{\textbf{\change{b}}\})$. We have a constraint $C'_{j,\textbf{\change{b}}}$ in $\Phi'$ using the predicate pair $(P_i \odot \textbf{\change{b}}, \change{\B}^{k_i}\setminus \{(-1,-1,\ldots,-1)\}$. However, in the constraint $C'_{j,\textbf{\change{b}}}$, we negate the literals corresponding to the indices $p\in[k_i]$ where $\change{b}_p=1$. More formally, \change{we define the} constraint $C'_{j,\textbf{\change{b}}}\change{= (y_{j,1},y_{j,2},\ldots,y_{j,k_i})}$ \change{as follows.}
\[
y_{j,p} := \begin{cases}
x_{j,p} , \text{ if } \change{b}_p = -1. \\
\overline{x_{j,p}} , \text{ if } \change{b}_p = 1.
\end{cases}
\]
\change{For the constant literals, $\overline{x_{j,p}}$ refers to a change of sign.} An assignment $\sigma : V \rightarrow \change{\B}$ strongly (and \change{respectively} weakly) satisfies $C'_{j,\textbf{\change{b}}}$ if and only if $\sigma$ strongly (and \change{respectively} weakly) satisfies $C_{j,\textbf{\change{b}}}$. Thus, we get that $\Phi'$ satisfies the completeness and soundness properties of the lemma. Finally, the operation of negating a subset of coordinates preserves all polymorphisms that are folded, and thus, if $\MAJ \subseteq \Pol(\Gamma)$, $\MAJ \subseteq \Pol(\Gamma')$ as well. 
\end{proof}

Given an instance $\Phi$ of $\change{\fiPCSP(\Gamma)}$, we transform it to an instance $\Phi'$ of $\change{\fiPCSP(\Gamma')}$ using~\Cref{lem:maj-transform}. 
If $\Phi$ is promised to have an assignment strongly satisfying at least $1-\epsilon$ fraction of the constraints, then $\Phi'$ has an assignment strongly satisfying $1-O(\epsilon)$ fraction of the constraints as well. If there is a polynomial time robust algorithm that outputs an assignment weakly satisfying $1-f(\epsilon)$ fraction of the constraints, then we can use this assignment to obtain a robust algorithm for $\change{\fiPCSP(\Gamma)}$ as well. Thus, a polynomial time robust algorithm for $\change{\fiPCSP(\Gamma')}$ gives a polynomial time robust algorithm for $\change{\fiPCSP(\Gamma)}$ as well. 

\change{Given a Boolean promise template} $\Gamma$ \change{in which} every predicate pair is of the form $(P, \change{\B}^{k}\setminus \{(-1,-1,\ldots,-1)\})$ with $\MAJ \subseteq \Pol(\Gamma)$, we show that the robust algorithm of Charikar, Makarychev, and Makarychev~\cite{CharikarMM09} for $2$-SAT generalizes and gives a robust algorithm for the Boolean folded PCSP $\Gamma$ as well.   
...
\noindent\fbox{%
    \parbox{\textwidth}{%
    \begin{enumerate}
        \item Given an instance $\Phi$ of $\change{\fiPCSP(\Gamma)}$ containing $n$ variables $u_1,u_2,\ldots,u_n$,
        solve the \change{Boolean folded idempotent} basic SDP and obtain a set of vectors $\textbf{v}_0, \textbf{v}_1, \ldots, \textbf{v}_n$ \change{and objective value $\eps > 0$}. Let $\boldsymbol \mu \in \R^n$ denote the first moments and $\Sigma \in \R^{n\times n}$ be the \change{Gram} matrix of these vectors. 
        \begin{align*}
            \mu_i &= \change{ \langle \textbf{v}_i , \textbf{v}_0 \rangle } \quad \forall i \in [n]\\ 
            \Sigma_{i,j} &= \change{ \langle \textbf{v}_i , \textbf{v}_j \rangle } \quad \forall i,j \in [n]
        \end{align*}
        \item Sample an $n$ dimensional Gaussian $\zeta \sim \mathcal N(\textbf{0}, \Sigma)$. Note that $\Sigma$ is positive semidefinite. \change{We can also equivalently sample $\zeta' \sim \mathcal{N}(\textbf{0}, \textbf{I})$ and use $\langle \textbf{v}_i, \zeta' \rangle$ in the place of $\zeta_i$ in step 4 below. We stick with $\zeta \sim \mathcal{N}(\textbf{0},\Sigma)$ for ease of notation in the analysis.}
...
Let $\Gamma=\{(P_1,Q_1),(P_2,Q_2),\ldots,(P_l,Q_l)\}$ be a Boolean \change{promise template} such that $\MAJ \subseteq \Pol(\Gamma)$ where $P_i \subseteq Q_i = \change{\B}^{k_i}\setminus \{(-1,-1,\ldots,-1)\} $ for every $i \in [l]$. Let $\Phi$ be an instance of $\change{\fiPCSP}(\Gamma)$ over $n$ variables and using $m$ constraints for which the basic SDP relaxation has a solution with error value at most $\eps m$. Then, the assignment $\sigma$ output by the above CMM algorithm weakly satisfies $1 - \tilde{O}_{\Gamma}(\eps^{1/3})$ fraction of the constraints in expectation.
\end{theorem}

\change{\Cref{thm:maj}, together with \Cref{lem:maj-transform} and \Cref{thm:sdp}, completes the proof of \Cref{thm:maj-alg}. Suppose that $\Gamma = \{(P_1, Q_1), (P_2, Q_2), \ldots, (P_l, Q_l)\}$ is a Boolean \change{promise template} with $\MAJ \subseteq \Pol(\Gamma)$ with $Q_i = \change{\B}^{k_i} \setminus \{(-1, -1, \ldots, -1)\}$ for every $i \in [l]$. Given $\epsilon > 0$ and an instance $\Phi$ of $\change{\fiPCSP}(\Gamma)$ that is promised to have an assignment satisfying $1 - \epsilon$ fraction of the constraints, we solve the basic SDP relaxation of the instance. Without loss of generality, we assume that $\epsilon > \frac{1}{m}$, where $m$ is the number of constraints in the instance. By setting $\delta = \epsilon$ in \Cref{thm:sdp}, we obtain that the basic SDP relaxation has an error of at most $2 \epsilon m$. Finally, \Cref{thm:maj} implies that the output assignment of the above algorithm weakly satisfies $1 - \tilde{O}_{\Gamma}(\epsilon^{1/3})$ of the constraints in expectation.}%

\change{Towards proving~\Cref{thm:maj},} we analyze the probability that the output assignment weakly satisfies each constraint separately. Fix a constraint $C_j$ using the predicate pair $(P,Q)$ of arity $k$ with $P\subseteq Q=\change{\B}^k \setminus \{(-1,-1,\ldots,-1)\}.$ Suppose that the basic SDP solution has error equal to $c$ on this constraint, i.e., $\eps_j=c$. 
...
\change{For our predicate $P \subseteq \B^k$, l}et $\mathcal P$ be the convex hull of $P$, where the tuples are viewed as vectors in $\R^k$. 
\[
...
\]
We prove the following property about $\mathcal P$ using the fact that \change{$\MAJ \subseteq \Pol(P, \B^k \setminus \{(-1, \hdots, -1)\}$}. We recall that $O_{\MAJ}(P)$ denotes the set $\bigcup_{\textbf{x}_1,\ldots,\textbf{x}_L\in P,L \in \mathbb{N},\text{ odd}}\MAJ_L(\textbf{x}_1,\ldots,\textbf{x}_L)$ for a predicate $P \subseteq \change{\B}^k$.


\begin{lemma}
\label{lem:separating-hyperplane}
Let $P \subseteq \change{\B}^k$ be a predicate such that $(-1,-1,\hdots, -1) \not\in O_{\MAJ}(P)$. Then, there is a hyperplane separating $\mathcal{P}$ from the origin: there exists $\textbf{w} \in \mathbb{R} ^k$, $\textbf{w} \geq 0$ and $\norm{\textbf{w}}_1 = 1$ such that for every $\textbf{a} \in \mathcal{P}$, $\langle \textbf{a}, \textbf{w} \rangle \geq 0$. 
\end{lemma}
...
Now we use this lemma to complete the proof of Lemma~\ref{lem:maj}. Recall that our goal is to lower bound the probability that the assignment $\sigma$ output by the above algorithm weakly satisfies the constraint $C_j$. Suppose that $\change{C_j=(x_{j,1},x_{j,2},\ldots,x_{j,k})}$, and uses the predicate pair $(P,Q)$ where $P\subseteq Q = \change{\B}^k\setminus\{(-1,-1,\ldots,-1)\}$. \change{Let $S_j$ denote the set of variables appearing in $C_j$.} For each $l \in [k]$, let $\hat{\bv}_l := \bv(x_{j,l})$, where the function $\bv$ is defined as in (\ref{eq:vl}). \change{Let \( \mu^{(j)} \) and \( \Sigma^{(j)} \) be defined as follows:
\begin{align*}
...
\vspace{-1ex}
    ...
    \begin{align*}
    \change{\langle \bw , \mu^{(j)} \rangle} &=  \change{\sum_{f : S_j \to \B} \lambda_j(f) \change{\langle \bw, f(C_j)\rangle}}\\
    & \geq -c \text{ (Using \Cref{lem:separating-hyperplane} and $-1 \leq \change{\langle \bw, \ba \rangle} \leq 1 \,\forall \change{\ba \in \B^k}$ )}
    \end{align*}
    \item (Second moment). We have
    \begin{align*}
    \bw ^T \change{\Sigma^{(j)}} \bw &= \change{\sum_{f : S_j \to \B} \lambda_j(f)} \change{\langle \bw,  f(C_j) \rangle ^2}\\
    &\leq  \change{\sum_{\substack{f : S_j \to \B\\f(C_j) \in P}} \lambda_j(f) (\langle \bw,  f(C_j) \rangle ^2} + c\\
    &\leq \change{\sum_{\substack{f : S_j \to \B\\f(C_j) \in P}} \lambda_j(f) \langle \bw,  f(C_j) \rangle } + c \leq \change{ \langle \bw, \mu \rangle} + 2c \ . \end{align*}
\end{enumerate}
%
We do casework on the value of $\change{\langle \bw, \mu^{(j)}\rangle}$. First, consider the case that $\change{\langle \bw, \mu^{(j)}\rangle} \geq \kappa = \gamma \sqrt{\log \frac{1}{\epsilon}}$. As $\norm{\bw}_1$=1, and $\bw \geq 0$, there exists $i \in [k]$ such that $\change{\mu^{(j)}_i} \geq \kappa$. As $\zeta_i \sim \mathcal{N}(0,1)$, using~\Cref{prop:gaussian-concentration}, with probability at least $1-\sqrt{\epsilon}$, we have $\zeta_i \geq -\change{\frac{\mu^{(j)}_i}{\gamma}}$. Thus, with probability at least $1-\sqrt{\epsilon}$, the rounded solution satisfies $Q$.  

%
Henceforth, we assume $\change{\bw^{T}\mu^{(j)}} < \kappa $.
For notational convenience let $\bt = -\change{\mu^{(j)} / \gamma }$.  We have 
\begin{equation}
\label{eq:wt-atmost}
\bw^{T} \bt \le \frac{c}{\gamma}
\end{equation}
and  $\change{\bw^{T}\Sigma^{(j)} \bw }\le \kappa +2c$. \change{From ~\Cref{prop:gaussian-multivariate}}, $\change{\langle \bw , \zeta \rangle} \sim \change{\mathcal{N}(0, \bw^T \Sigma^{(j)} \bw)}$. Thus, using~\Cref{prop:gaussian-concentration},  
...
%
 %
Note that the rounded solution does not satisfy $Q$ only if $\bt \geq \zeta$. We now upper bound the probability that this \change{occurs}. Together with~\Cref{eq:wt-atmost} and~\Cref{eq:wzeta-atmost},
...
\[
    \change{t_i}- \zeta_i \in \left[0, O\left(\sqrt{(\kappa + 2c)\log \frac{1}{\epsilon}}+\frac{c}{\gamma}\right)\right].
\]
\change{Since $t_i$ is a constant, this occurs with probability 
\[
...
As a stepping-stone for our algorithm for $\AT$ presented in the next section, we present a robust algorithm for the Dual-Horn SAT CSP. \change{The formal template Dual-Horn SAT allows for constraints of arbitrarily large arity, but we consider a finite fragment. In particular, our template} $\Gamma=\change{\{}P_1, P_2, \ldots, P_l\change{\}}$ \change{has} $P_i = \change{\B}^{k_i}\setminus\{(-1,-1,\ldots,-1)\}$ \change{for $i \in [l]$.} \change{Specifically to emulate Dual-Horn SAT, we restrict ourself to instances of $\fiPCSP(\Gamma)$ such that each clause has at most one negated literal and no constant literals appear.}
...
\noindent\fbox{%
    ...
    \begin{enumerate}
                \item Given an instance $\Phi$ of $\change{\fiPCSP(\Gamma)}$ containing $n$ variables $\{u_1,u_2,\ldots,u_n\}$,
        solve the basic SDP and obtain a set of vectors $\textbf{v}_0, \textbf{v}_1, \ldots, \textbf{v}_n$ \change{and objective value $\eps > 0$}. Let $\mathbf{\mu} \in \R^n$ denote the first moments.  
        \begin{align*}
            \mu_i &= \change{ \langle \textbf{v}_i , \textbf{v}_0\rangle } \quad \forall i \in [n]
        \end{align*}        
...
%
Let $\Phi$ be an instance of \change{$\fiPCSP(\Gamma)$ such that each clause has at most one negated literal and no constant literals} for which there is a basic SDP solution with error at most $\eps$. Then, the assignment $\sigma$ output by the above algorithm satisfies $1 - O_{\Gamma}(1/\log(1/\eps))$ fraction of the constraints of $\Phi$ in expectation. 
\end{theorem}

\begin{proof}
%
As with the analysis of \change{the} $\MAJ$ \change{polymorphism}, we fix a single constraint $C_j$ using the predicate $P$ and analyze the probability that $\sigma$ satisfies $C_j$.  Since the average SDP error over the constraints is equal to $\eps$, by Markov's inequality, for at least $1 - \sqrt{\eps}$ fraction of the constraints, the SDP error value is at most $\sqrt{\epsilon}$. Henceforth, we assume that $c \leq \sqrt{\epsilon}$. 

Suppose that the constraint $C_j$ uses the \change{literals} $x_1, \hdots, x_k$ \change{ with some potentially equal. As an abuse of notation, we let $\mu_i$ denote the inner product of $\bv_0$ with $\bv(x_i)$.} First, consider the case when none of the variables are negated. 
...
\change{Next, c}onsider the case \change{in which exactly one} literal is a negated variable in the constraint. Without loss of generality, assume that $x_1$ is negated. 
Then, we have that 
\[
...
The algorithm rounds $x_i$ to $-1$ only if $t > \frac{\mu_1+1-2\sqrt{\epsilon}}{k-1}$. On the other hand, the algorithm rounds $x_1$ to $+1$ only if $t \leq \mu_1+1$. Since there is at most one element in $T$ in $(\frac{\mu_1+1-2\sqrt{\epsilon}}{k-1},\mu_1+1)$, there exists at most one $t \in T$ which would round $x_i$ to $-1$ but $x_1$ to $+1$. Therefore, the probability the constraint is satisfied is at least $1 - 1/|T|$ = $1 - \change{O_{\Gamma}(1/\log(1/\eps))}$. By linearity of expectation over all the constraints with error at most $\sqrt{\epsilon}$, we get the required claim. 
\end{proof}

\change{~\Cref{thm:or} implies a polynomial time robust algorithm for Dual-Horn SAT: Suppose that the template $\Gamma=\change{\{}P_1, P_2, \ldots, P_l\change{\}}$ \change{has} $P_i = \change{\B}^{k_i}\setminus\{(-1,-1,\ldots,-1)\}$for $i \in [l]$. Consider an instance $\Phi$ of $\fiPCSP(\Gamma)$ such that each clause has at most one negated literal and no constant literals appear, with the property that there is an assignment that strongly satisfies $(1-\epsilon)$ fraction of the constraints. Without loss of generality, we can assume that $\epsilon \geq \frac{1}{m}$, where $m$ is the number of constraints in $\Phi$. By setting $\delta = \sqrt{\epsilon}-\epsilon$ in ~\Cref{thm:sdp}, we can obtain a solution to basic SDP relaxation of $\Phi$ with error at most $\sqrt{\epsilon}m$. ~\Cref{thm:or} then implies that the output of the above algorithm satisfies $1 - O_{\Gamma}(1/\log(1/\eps))$ fraction of the constraints of $\Phi$ in expectation.}

\begin{remark}
We remark that the above algorithm applies directly to Horn-SAT as well. Furthermore, for an arbitrary \change{Boolean promise template} $\Gamma=\{(P_1,Q_1),(P_2,Q_2),\ldots,(P_l,Q_l)\}$ with OR polymorphisms\footnote{$\OR(x_1,x_2,\ldots,x_L)$ is equal to $+1$ if there is $i \in [L]$ with $x_i=+1$, and $-1$ otherwise.}
$\OR \subseteq \Pol(\Gamma)$, it's been proved~\cite{BrakensiekG21} that $\OR \subseteq \Pol(\Gamma')$ where $\Gamma'$ is the CSP containing the predicates $\{Q_1,Q_2,\ldots,Q_l\}$. By Schaefer's theorem~\cite{Schaefer78}, any \change{template $\Gamma$} with $\OR \subseteq \Pol(\Gamma)$ is ppp-definable from $k$-Horn-SAT, and thus, our algorithm proves that there is a polynomial time robust algorithm \change{for $\PCSP(\Gamma)$ (i.e., negated literals are \emph{not} allowed)} for every \change{Boolean promise template} $\Gamma$ with $\OR \subseteq \Pol(\Gamma)$.
\end{remark}
...
We now show how to combine the ideas in the previous two subsections to obtain a polynomial time robust algorithm for every Boolean \change{promise template} $\Gamma=\{(P_1,Q_1),\ldots,(P_l,Q_l)\}, P_i \subseteq Q_i \subseteq \change{\B}^{k_i}$ with $\AT\subseteq \Pol(\Gamma)$. 
Similar to the $\MAJ$ polymorphisms case, we can without loss of generality assume that $Q_i \neq \change{\B}^{k_i}$ for every $i \in [l]$.
We first reduce an arbitrary \change{promise template} $\Gamma$ with $\AT \subseteq \Pol(\Gamma)$ to a specific \change{template} that we will work with. 
For a vector $\textbf{w} \in \R^k$, define $\sgn(\textbf{w})_i$ to be $-1$ if \change{$w_i < 0$} and $+1$ otherwise. Define $\Gamma_{AT}$ to be the following \change{infinite} family of \emph{weighted hyperplane predicates}:
%
\begin{align*}
    \Gamma_{\AT} &:= \{(P_{\textbf{w},b} := \{\change{\textbf{x}} \in \change{\B}^k: \change{ \langle \textbf{w} , \textbf{x} \rangle }= b\}, Q_{\textbf{w},b} := \change{\B}^k \setminus \{\sgn(\textbf{w}), -\sgn(\textbf{w})\}) :\\&\change{k \in \N,} b \in \Q, \change{\textbf{w} \in \Q^k\setminus \textbf{0}},\textbf{w}.\sgn(\textbf{w})>b, -\textbf{w}.\sgn(\textbf{w})<b\}
\end{align*}
We \change{prove} that these predicates indeed have AT of all odd arities as polymorphisms. 
\begin{claim}
$\AT \subseteq \Pol(\Gamma_{AT})$
\end{claim}
\begin{proof}
Fix $b \in \Q$ and \change{$\textbf{w} \in \Q^k\setminus \textbf{0}$}. Let $(P_{\change{\bw},b}, Q_{\change{\bw},b})$ be the corresponding predicate for these values. It suffices to show that $\AT \subseteq \Pol(P_{\change{\bw},b},Q_{\change{\bw},b})$. Fix an odd arity $L$ and pick $\textbf{x}_1, \hdots, \textbf{x}_L \in P_{\change{\bw},b}$.  Observe that
\[
    \AT(\textbf{x}_1, \hdots, \textbf{x}_L) = \sgn(\textbf{x}_1 - \textbf{x}_2 + \cdots + \textbf{x}_L).
\]
Further, $\change{ \langle \textbf{w} , (\textbf{x}_1 - \textbf{x}_2 + \cdots + \textbf{x}_L) \rangle }= b$. 
This implies that $\sgn(\textbf{x}_1 - \textbf{x}_2 + \cdots + \textbf{x}_L) \neq \sgn(\textbf{w})$ as otherwise,
\[
    b = \change{ \langle \textbf{w} , (\textbf{x}_1 - \textbf{x}_2 + \cdots + \textbf{x}_L) \rangle  \ge \langle \textbf{w} , \sgn(\textbf{w}) \rangle} > b.
\]
where we used the fact that the absolute value of each entry in $\textbf{x}_1-\textbf{x}_2+\ldots +\textbf{x}_L$ is at least $1$.
By a similar argument, $\sgn(\textbf{x}_1 - \textbf{x}_2 + \cdots + \textbf{x}_L) \neq - \sgn(\textbf{w})$. Thus, $\AT(\textbf{x}_1, \hdots, \textbf{x}_L) \in Q_{\change{\bw},b}$, as desired.
\end{proof}

\newcommand{\const}{\operatorname{const}}

Let $\Gamma_{\const}$ be the \change{Boolean promise template} $\{(\{-1\},\{-1\}),(\{+1\},\{+1\})\}$.
\change{Since $\AT$ is an idempotent family of polymorphisms, we have that $AT \subseteq \Pol(\Gamma_{\const})$.}
We show that an arbitrary Boolean \change{promise template} with $\AT$ polymorphisms can be reduced to \change{a finite fragment of} $\change{\Gamma_{\AT}\cup \Gamma_{const}}.$

%
\begin{lemma}\label{thm:at-general}
Let $\Gamma$ be any Boolean \change{promise template} with $\Gamma=\{(P_1,Q_1),\ldots,(P_l,Q_l)\}, P_i \subseteq Q_i \subsetneq \change{\B}^{k_i}$ with $\AT\subseteq  \Pol(\Gamma)$.
 Then, $\Gamma$ is ppp-definable from a \change{finite} Boolean \change{promise template} $\Gamma'$ with $\Gamma' \subseteq \Gamma_{\AT}\cup \Gamma_{const}$.
\end{lemma}
We prove~\Cref{thm:at-general} in~\Cref{subsec:at-general}. Recall that if a \change{promise template} $\Gamma$ is ppp-definable from another \change{promise template} $\Gamma'$ \change{and} if $\change{\fiPCSP(\Gamma')}$ has a polynomial time robust algorithm, then $\change{\fiPCSP(\Gamma)}$ has a polynomial time robust algorithm as well (up to losing constant factors in the error parameter).
In the rest of this section, \change{we prove that for any finite} Boolean \change{promise template} $\Gamma$ with $\Gamma \subseteq \Gamma_{\AT}$, \change{$\fiPCSP(\Gamma)$ has a robust algorithm}, thereby obtaining a robust algorithm for $\change{\fiPCSP(\Gamma)}$ \change{for} every Boolean \change{promise template} $\Gamma$ with $\AT \subseteq\Pol(\Gamma)$. 
We state our algorithm below. \smallskip 

\noindent\fbox{%
    \parbox{\textwidth}{%
    \begin{enumerate}
        \item Given an instance $\Phi$ of $\change{\fiPCSP(\Gamma)}$ containing $n$ variables $V=\{u_1,u_2,\ldots,u_n\}$,
        solve the basic SDP and obtain a set of vectors $\textbf{v}_0, \textbf{v}_1, \ldots, \textbf{v}_n$ \change{and objective value $\epsilon > 0$}.  \item \change{Sample an $n$ dimensional Gaussian $\zeta \sim \mathcal N(\textbf{0}, \textbf{I})$.}
...
We restrict our analysis to these constraints with SDP error $c \leq \sqrt{\epsilon}$ and show that the rounded solution violates the predicate $Q$ with probability at most $O\left(\frac{\log \log \frac{1}{\epsilon}}{\log \frac{1}{\epsilon}}\right)$, thereby proving~\Cref{thm:at}. \change{Finally, we note that the rounding algorithm preserves the properties of idempotence and folding: specifically, $\pm \textbf{v}_0$ are rounded to $\pm 1$, respectively. Moreover, for any vector $\textbf{v}$, the algorithm ensures that $\textbf{v}$ and $-\textbf{v}$ are rounded to distinct values.}

\change{We break the analysis into cases based on the predicate pair $(P,Q)$.} We first consider the case when $P=Q$ and $P \subseteq \Ham_k \{0,k\}$, which is a generalization of $\Gamma_{const}$.

\begin{lemma}
\label{lem:equal-case}
Let \change{$\Gamma = (P, Q)$ } be a \change{finite} Boolean \change{promise template} with $\AT \subseteq \Pol(\Gamma)$ and \change{$P=Q$, $P\subseteq \Ham_k \{0,k\}$ for an integer $k \geq 1$}.
Let $\Phi$ be an instance of $\change{\fiPCSP}(\Gamma)$, and in the basic SDP solution of $\Phi$, suppose that the constraint $\change{C_j}$ using the predicate pair $(P,Q)$ has error at most $\sqrt{\epsilon}$. Then, the assignment $\sigma$ output by the above algorithm does not weakly satisfy $\change{C_j}$ with probability at most $O\left(\frac{\log \log \frac{1}{\epsilon}}{\log \frac{1}{\epsilon}} \right)$.
\end{lemma}

We defer the proof of~\Cref{lem:equal-case} to~\Cref{sec:missing-proofs}. %

We now consider the case when $P=P_{\textbf{w},b}$ and $Q=Q_{\textbf{w},b}$ \change{for some $k \in \N, b \in \Q, \textbf{w} \in \Q^k \setminus \textbf{0},\textbf{w}.\sgn(\textbf{w})>b, -\textbf{w}.\sgn(\textbf{w})<b$}.
For ease of notation, we let \change{$x_1, x_2, \ldots, x_k$ denote the literals assigned to $\change{C_j}$, with $\bv_1, \hdots, \bv_k$ the corresponding values of $\bv(x_1), \hdots, \bv(x_k).$ We let $S$ denote the set of variables in the scope of $C_j$.} We have that
\[
    \change{\sum_{\substack{f : S \to \B\\f(C_j) \in P}} \lambda_j(f) \ge 1 - \sqrt{\eps}}.
\]

%
Let $\textbf{v}_s = \change{\sum_{i \in [k]}\textbf{w}_i\textbf{v}_i}$, and let the component of $\textbf{v}_s$ along $\textbf{v}_0$ be $\alpha \textbf{v}_0$, and the component normal to $\textbf{v}_0$ be equal to $\textbf{v}'_s$.
\[
...
\begin{enumerate}
    ...
    \begin{align*}
            \alpha &=  \change{\sum_{f : S \to \B}\lambda_j(f)\langle \textbf{w}, \change{f(C_j)}\rangle}\\
        &= \change{\sum_{\substack{f : S \to \B\\f(C_j) \in P}}} \change{\lambda_j(f)}\langle \textbf{w}, \change{f(C_j)}\rangle + \change{\sum_{\substack{f : S \to \B\\f(C_j) \not\in P}}} \change{\lambda_j(f)}\langle \textbf{w}, \change{f(C_j)}\rangle\\ 
        &= \change{\sum_{\substack{f : S \to \B\\f(C_j) \in P}}} \change{\lambda_j(f)}b + \change{\sum_{\substack{f : S \to \B\\f(C_j) \not\in P}}} \change{\lambda_j(f)}\langle \textbf{w}, \change{f(C_j)}\rangle\\
        &= b + \change{\beta} 
        \end{align*}
    where
    %
    \[
    \change{\beta} = \change{\sum_{\substack{f : S \to \B\\f(C_j) \not\in P}}} \change{\lambda_j(f)}(\change{b} - \langle \textbf{w}, \change{f(C_j)}\rangle)
    \]
    We have $ \change{|\beta|}=O(\sqrt{\epsilon})$. 
    \item (Second moments). We have 
    $$
        \norm{\textbf{v}_s}_2^2 = 
        \change{\sum_{i, j \in [k]}\textbf{w}_i \textbf{w}_j} \langle \textbf{v}_i, \textbf{v}_j \rangle = \change{\sum_{f : S \to \B}} \change{\lambda_j(f)} (\change{ \langle \change{f(C_j)} , \textbf{w}\rangle})^2 = b^2 + \change{\beta'}
        $$
    where $|\change{\beta'}|=O(\sqrt{\epsilon})$.
\end{enumerate}
%
Thus, we get 
$\norm{\textbf{v}'_s}_2^2 = \norm{\textbf{v}_s}_2^2 - \alpha^2 
    = \change{(b^2+\beta')-(b+\beta)^2}$
...
\change{From ~\Cref{prop:gaussian-multivariate}, $\langle \zeta, \textbf{v}'_j \rangle \sim \mathcal{N}(0, \norm{\textbf{v}'_j}_2^2)$ for every $j \in [k]$. 
...
Thus, with probability at least $1-O(\frac{1}{r})$, for every $i \in [k]$, $x_i$ is set to be \change{$-1$} if $\alpha_i \leq 0$, and \change{$+1$} otherwise. Combining this with the fact that $\change{\sum_i \textbf{w}_i} \alpha_i = b + O(\sqrt{\epsilon})$, and that \change{$\textbf{w}.\sgn(\textbf{w})>b, -\textbf{w}.\sgn(\textbf{w})<b$,} for small enough $\epsilon$,
we get the rounded solution has variables assigned $+1$ and $-1$. \change{Here, we assumed that $\epsilon$ is at most $c := c(\Gamma)$ for some function $c$. This is without loss of generality, since when $\epsilon$ is larger than $c$, a uniformly random assignment satisfies the robust algorithm requirement.}
%

\smallskip \noindent \textbf{Completing the proof.}
We finish the proof by showing that with probability at least $1-O(\frac{1}{r})$, at least one of the above two cases holds. None of the above two cases hold \change{only} if for some $i \in [k]$, we have 
\[
...
%
 ...
 For a predicate $P \subseteq \change{\B}^k$, we let $\Aff(P)\subseteq \R^k$ to denote the affine hull of $P$.
\[
\Aff(P):= \left\{ \sum_{\textbf{a} \in P} \lambda_{\textbf{a}} \textbf{a} : \sum_{\textbf{a} \in P}\lambda_\textbf{a}=1, \lambda_{\textbf{a}} \in \R\right\}.
\]
We recall that $O_{\AT}(P)$ denotes the set $\bigcup_{\textbf{x}_1,\ldots,\textbf{x}_L\in P,L \in \mathbb{N},\text{ odd},}\AT_L(\textbf{x}_1,\ldots,\textbf{x}_L)$ for a predicate $P \subseteq \change{\B}^k$.
\change{In order to} prove~\Cref{thm:at-general}, we need to use the following lemma implicit in~\cite{BrakensiekG21}.


\begin{lemma}\label{lem:AT}
Let $P\subseteq \change{\B}^k$ be a predicate such that there is non-trivial dependence in each coordinate, i.e., for every $i\in[k]$, there exist vectors $\textbf{x},\textbf{y}\in P$ with $x_i = -1$ and $y_i = +1$. Then, $O_{\AT}(P) = \{\sgn(\textbf{x} - \textbf{y}) : \textbf{x}, \textbf{y} \in \Aff(P), \forall i, x_i \neq y_i \}$.
\end{lemma}
...
\begin{claim}\label{claim:hyperplane}
    Let $H$ be a vector space in $\R^k$ such that there is no $\textbf{y} \in H$ with $y_i > 0$ for all $i$. Then, there exists \change{$\mathbf{w} \neq 0$ with $w_i \geq 0$} for all $i$ and $\change{ \langle \mathbf{w} , \mathbf{y} \rangle} = 0$ for all $\textbf{y} \in H$. \end{claim}


\newcommand{\by}{\textbf{y}}

\begin{proof}
Since $H$ and the positive orthant $\R_{+}^k:= \{ \textbf{y} \in \R^k : y_i >0\,\,\,\forall i \in [k]\}$  are \change{disjoint} convex bodies, \change{by the separating hyperplane theorem (e.g., \cite{boyd2023Convex}),} there exists \change{nonzero} $\change{\bw} \in \R^k$ and $b \in \R$ such that for all $\change{\bv}\in \R_{+}^k$, $\change{ \langle \bw , \bv \rangle \ge b}$ and for all $\textbf{y} \in H$, $\change{ \langle \by , \bw \rangle}\le b$. \change{Since $\bw$ is nonzero and $\R_{+}^k$ is open, we must more strongly have that $\change{ \langle \bw , \bv \rangle > b}$ and for all $\change{\bv}\in \R_{+}^k$.}

Taking the limit as $\change{\bv} \to 0$ \change{of $\change{ \langle \by , \bw \rangle}\le b$ for all $\textbf{y} \in H$}, we have that $b \le 0$. Further, since $(0,0,\ldots,0)\in H$, we must have that $b = 0$ and since $H$ is a vector space, $\change{ \langle \by , \bw \rangle } = 0$ for all $\by \in H$. Thus, $\change{\bw}$ is normal to $H$. Note that $\change{\bw}$ \change{is nonzero and} has all coordinates \change{non-negative} as $\change{ \langle \bv , \bw \rangle} >0$ for all $\change{\bv}$ in the positive orthant. 
\end{proof}
...
For every $\textbf{x} \in \change{\B}^k\setminus Q$, we find $\textbf{w}, b$ such that 
\[
P_{\textbf{w},b} := \{\textbf{v} \in \change{\B}^k: \change{ \langle \textbf{w} ,  \textbf{v} \rangle }= b\},
\quad Q_{\textbf{w},b} := \change{\B}^k \setminus \{\sgn(\textbf{w}), -\sgn(\textbf{w}\})
\]
satisfy $P \subseteq P_{\textbf{w},b}, \change{Q} \subseteq Q_{\textbf{w},b}$ and $\sgn(\textbf{w})=\textbf{x}$. By applying this for every $\textbf{x} \in \change{\B}\setminus Q$, we get a set of predicate pairs $(P_1, Q_1), (P_2, Q_2), \ldots, (P_L,Q_L)$ with $L\leq 2^k$ such that 
\begin{enumerate}
...
Henceforth, our goal is to show that for every $\textbf{x} \in \change{\B}^k\setminus Q$, we can find $\textbf{w}, b$ such that $P_{\textbf{w},b} := \{x \in \change{\B}^k: \change{ \langle \textbf{w} , x \rangle }= b\}$
satisfies $P \subseteq P_{\textbf{w},b}$, $\change{ \langle \textbf{w},  \sgn(\textbf{w})\rangle } >b, \change{ \langle \textbf{w}, \sgn(\textbf{w})\rangle} >-b$ and $\sgn(\textbf{w})=\textbf{x}$. Without loss of generality, we can assume that $\textbf{x}=(+1,+1,\ldots,+1)$. Fix an arbitrary vector $\overline{\textbf{x}} \in P$ such that $\overline{\textbf{x}}\notin \{(-1,-1,\ldots,-1), (+1,+1,\ldots,+1)\}$. Such a vector is guaranteed to exist as $P$ does not contain \change{${\textbf{x}}$} and has non-trivial dependence on each coordinate. 
...
Using~\Cref{claim:hyperplane}, we can obtain \change{nonzero $\textbf{w}\geq \textbf{0}$} such that $\change{ \langle \textbf{w}, \textbf{y}\rangle}=0$ for all $\textbf{y} \in H$. This shows that $\change{ \langle \textbf{w}, \textbf{y}\rangle} = b$ for every $\textbf{y} \in P$, where $\change{b := \langle \textbf{w}, \overline{\textbf{x}}\rangle}$ satisfies $\sum_i w_i >b$, $\sum_i w_i >-b$. 
\end{proof}
...
In this section, we prove~\Cref{thm:main-hardness}. First, we use the analysis of $\AT$ and $\MAJ$ polymorphisms for symmetric \change{promise tempaltes} with folding and idempotence in~\cite{BrakensiekG21} to show that we can relax $\Gamma$ into one of five candidate PCSP types. 

\begin{lemma}
\label{lem:reduction}
Let $\Gamma=(P,Q)$ be a Boolean \change{promise template} such that $
\MAJ_{L_1}, \AT_{L_2} \notin \Pol(\Gamma)$ for some odd integers $L_1, L_2$. Then, there exists a Boolean \change{template} $\Gamma'=(P,Q)$ that is \change{fi}ppp-definable from $\Gamma$ that is equal to \change{one} of the following:
\begin{enumerate}
...
Recall that if a \change{promise template} $\Gamma'$ is \change{fi}ppp-definable from another \change{promise template} $\Gamma$, if $\change{\fiPCSP(\Gamma)}$ has a polynomial time robust algorithm, then $\change{\fiPCSP(\Gamma')}$ has a polynomial time robust algorithm as well (\Cref{prop:fippp-defn}). Thus, to show~\Cref{thm:main-hardness}, it suffices to show Unique Games based hardness of obtaining robust algorithms for $\change{\fiPCSP(\Gamma_{i})}$ \change{for $i \in [5]$ from Lemma~\ref{lem:reduction}}.
We achieve \change{these hardness results} by showing integrality gaps for the basic SDP relaxation of \change{each $\change{\fiPCSP(\Gamma_{i})}$}. 
...
\begin{theorem}[Special case of ~\cite{Raghavendra08} for Boolean \change{fiPCSPs} when the SDP is feasible]
\label{thm:raghavendra}
Suppose that for a Boolean \change{promise template} $\Gamma$, there is a finite integrality gap for the basic SDP relaxation \change{$\fiPCSP(\Gamma)$}, i.e., there is a finite instance $I$ of $\change{\fiPCSP}(\Gamma)$ on which the basic SDP relaxation is feasible but there is no assignment that weakly satisfies $I$. Then, there exists a constant $s<1$ that is a function of $\Gamma, I$ such that the following decision problem is NP-hard for sufficiently small $\epsilon,\delta >0$, assuming UGC. Given an instance $\Phi$ of $\change{\fiPCSP(\Gamma)}$, distinguish between the two cases:
\begin{enumerate}
...
It is worth noting that Raghavendra does not work directly with the basic SDP relaxation of $\fiPCSP(\Gamma)$ but rather with the basic SDP relaxation of $\PCSP(\Gamma')$ for a suitable promise template $\Gamma'$. In particular, consider for each relation $(P, Q) \in \Gamma$ of arity $k$ the variable set $V = \{x_1, \hdots, x_k\}$. For every possible clause $C = (y_1, \hdots, y_k) \in (V \cup \bar{V} \cup \B)^k$ using variables $S \subseteq V$, we construct a corresponding relation pair $(P_C, Q_C)$ of arity $S$ such that
\begin{align*}
    P_C &= \{z \in \B^{S} : C(z) \in P\}\\
    Q_C &= \{z \in \B^{S} : C(z) \in Q\}.
\end{align*}
In particular, $\Gamma'$ consists of every $(P_C, Q_C)$ which can be defined from some $(P, Q) \in \Gamma$. The key observation is that every instance $\Psi$ of $\fiPCSP(\Gamma)$ can then be replaced by a suitable instance $\Psi'$ of $\PCSP(\Gamma')$ by replace each use of $(P,Q)$ with the corresponding $(P_C, Q_C)$. One can verify that each strong and weak assignment to $\Psi$ has the same value as the corresponding assignment to $\Psi'$ and that the SDP relaxations have the same value. Thus, Raghavendra's theorem does indeed apply.%
\end{remark}
}

Thus, to show~\Cref{thm:main-hardness}, our goal is to show the existence of finite integrality gaps for the basic SDP relaxations of the Boolean \change{promise templates} in~\Cref{lem:reduction}.
...
\begin{definition}
\label{def:p-conf}
Fix a predicate $P \subseteq \change{\B}^k$. We say that a tuple of vectors $V=(\textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_k), \textbf{v}_i \in \S^n\,\forall i \in [k]$ are a $P$-configuration with respect to another vector $\textbf{v}_0\in \S^n$ if the tuple of vectors can be assigned to a set of literals in a constraint by the basic SDP relaxation of an instance of $\change{\fiPCSP(P,Q)}$ with zero error, for some $Q \supseteq P$. 
...
\begin{enumerate}
    ...
    \item First moments: 
    \[
    \change{ \langle \textbf{v}_i , \textbf{v}_0 \rangle }= \sum_{ \textbf{a} \in P}\lambda(\textbf{a}) a_i\quad \forall i \in [k].
    \]
    \item Second moments: 
     \[\change{ \langle \textbf{v}_{i} , \textbf{v}_{i'} \rangle }= \sum_{\textbf{a} \in P}\lambda(\textbf{a}) a_i a_{i'} \quad \forall i,i' \in [k]. \]
\end{enumerate}
\end{definition}

We now define the notion of a function respecting a Boolean $\change{\fiPCSP}$. We refer to functions $f:\S^n \rightarrow \change{\B}$ as colorings of the sphere. 

%
\begin{definition}
Fix a vector $\textbf{v}_0 \change{\in \S^n.}$ \change{We} say that a coloring of the sphere $f:\mathbb{S}^n \rightarrow \change{\B}$ \change{is folded if} $f(-\bv) = -f(\bv)$ for every $\bv \in \S^n$\change{.} \change{We say that $f$ is idempotent if $f(\bv_0) = +1$ and $f(-\bv_0) = -1$.} \change{We say that a coloring $f$ \emph{respects} the promise template} $(P,Q)$ with respect to a vector $\textbf{v}_0\in\S^n$ if the following condition holds. For every $P$-configuration $V=(\textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_k)$ with respect to $\textbf{v}_0$, we have that the colors of the vectors satisfy $Q$, i.e.,
\[
 (f(\textbf{v}_1), f(\textbf{v}_2), \ldots, f(\textbf{v}_k)) \in Q
\]
More generally, we say that a coloring $f:\mathbb{S}^n \rightarrow \change{\B}$ respects a Boolean \change{promise template} $\Gamma$ with respect to a vector $\textbf{v}_0$ if it respects every predicate pair in $\change{(P,Q)\in \Gamma}$ with respect to $\textbf{v}_0$.
\end{definition}



Our key observation is that the absence of such a \change{folded} sphere coloring respecting $\Gamma$ for some finite $n$ gives an integrality gap for the basic SDP relaxation of $\change{\fiPCSP(\Gamma)}$. 
\begin{lemma}
\label{lem:sphere-coloring}
For every Boolean \change{promise template} $\Gamma=\{(P_1,Q_1),\ldots,(P_l,Q_l)\}$, the Basic SDP decides $\change{\fiPCSP(\Gamma)}$ if and only if for every integer $n \geq 1$, there exists a \change{folded, idempotent} coloring $f^{(n)}:\mathbb{S}^n \rightarrow \change{\B}$ that respects $\Gamma$ with respect to a vector $\textbf{v}_0^{(n)}$.
%
\end{lemma}
\begin{proof}
We slightly abuse the notation and say that a \change{``partial''} folded function $f: S\rightarrow \change{\B}, S \subseteq \S^n$ respects a Boolean \change{promise template} $(P,Q)$ \change{with respect to} $\textbf{v}_0$ if and only if for every $P$-configuration of vectors $V=(\textbf{v}_1,\ldots,\textbf{v}_k)$ \change{with respect to} $\textbf{v}_0$ with $\textbf{v}_i \in S\,\forall i \in [k]$, $f(V) \in Q$. More generally, for a Boolean \change{promise template} $\Gamma$, $f:S \rightarrow \change{\B}$ respects $\Gamma$ if and only if it respects every predicate pair in $\Gamma$.
Via a compactness\footnote{We assume the axiom of choice.} argument (e.g., like the De Brujin-Erdos theorem \cite{bruijn1951colour}, for more details see Remark 7.13 of \cite{BBKO21} or \cite{ciardo2022clap}), we can infer that there is a \change{folded} coloring $f:\mathbb{S}^n \rightarrow \change{\B}$ respecting $\Gamma$ \change{with respect to} $\textbf{v}_0$ if and only if for every finite subset $S \subset \mathbb{S}^n$, there exists a \change{folded} coloring $f_S:S\rightarrow \change{\B}$ that respects $\Gamma$ \change{with respect to} $\textbf{v}_0$. 

First, assume that the Basic SDP decides $\change{\fiPCSP}(\Gamma)$. Fix an arbitrary set of vectors $\textbf{v}_0^{(n)} \change{\in \mathbb{S}^n}, n \in \mathbb{Z}^{+}$. \change{Also pick a subset $T \subset \mathbb{S}^n$ such that for every $\bv \in \mathbb{S}^n$, exactly one of $\bv$ or $-\bv$ is in $T$.} For any finite subset $S \subset \mathbb{S}^n$, we construct an instance $\Phi_S$ of $\Gamma$ as follows. \change{By increasing the size of $S$, we may assume that $S$ is closed with respect to negation.} The variable set \change{of $\Phi_S$ is $\{x_{\textbf{v}} : \textbf{v}\in S \cap T\}$.} \change{We define the constraints as follows. For every $i \in [l]$ and every $P_i$-configuration $(\textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_{k_i}) \in S^{k_i}$ with respect to $\bv_0^{(n)}$, we} add a constraint over $(P_i,Q_i)$ using the \change{literals $\ell_{\bv_{1}}, \ldots, \ell_{\bv_{k_i}}$, where
\[
...
We have that $x_{\bv} \mapsto \bv$ is a basic SDP solution with zero error. Thus, there exists an assignment \change{$f_{S \cap T} : S \cap T \to \mathcal B$} to the variables that weakly satisfies all the constraints in $\Phi_S$, or equivalently, there exists \change{folded, idempotent} $f_S: S\rightarrow \change{\B}$ that respects the \change{promise template} $\Gamma$ \change{with respect to} $\textbf{v}_0^{(n)}$\change{, where
\[
    f_S(\bv) = \begin{cases}
    ...
    \end{cases}
\]}
\change{Hence,} there exists a \change{folded, idempotent} coloring $f:\mathbb{S}^n \rightarrow \change{\B}$ that respects $\Gamma$ \change{with respect to} $\textbf{v}_0^{(n)}$ for every positive integer $n$.

Second, suppose that for every integer $n \geq 1$, there exists a \change{folded, idempotent} coloring $f^{(n)}:\mathbb{S}^n \rightarrow \change{\B}$ that respects $\Gamma$ \change{with respect to} some vector $\textbf{v}_0^{(n)}$. We seek to show that the Basic SDP decides $\change{\fiPCSP}(\Gamma)$. Take an arbitrary instance $\Phi$ of $\change{\fiPCSP}(\Gamma)$ such that there is a solution to Basic SDP with zero error. We solve the SDP relaxation of $\Phi$ and obtain a set of vectors $\textbf{v}_0$ and $ \textbf{v}_1,\ldots, \textbf{v}_n \in \S^n$ corresponding to the variables in $\Phi$ that satisfies all the constraints in $\Phi$ with zero error. \change{By \Cref{prop:gram-ortho}, there exists an orthogonal matrix $Q \in \R^{(n+1)\times(n+1)}$ for which $Q\bv_0 = \bv_{0}^{(n)}$. Since orthogonal matrices preserve the inner products of vectors, we have that multiplication on the left by $Q$ is an automorphism of $\mathbb{S}^n$. Define $f'^{(n)} : \S^n \to \B$ by $f'^{(n)}(\bv) = f^{(n)}(Q\bv)$. Since multiplication by an orthogonal matrix%
preserves inner products, we have that $f'^{(n)}$ respects $\Gamma$ with respect to $\bv_0$ if and only if $f^{(n)}$ respects $\Gamma$ with respect to $\bv^{(n)}_0$. Therefore, the} assignment $f'^{(n)}(\textbf{v}_i)$ to the variable $u_i$ weakly satisfies all the constraints in $\Phi$. Thus, for every instance $\Phi$ of $\change{\fiPCSP}(\Gamma)$ with zero error on the basic SDP relaxation, there is an assignment that weakly satisfies all the constraints in $\Phi$, or equivalently, the basic SDP decides $\change{\fiPCSP(\Gamma)}$. \end{proof}
...
Let $\Gamma$ be a Boolean \change{promise template}. Then, there is a finite integrality gap for the basic SDP relaxation of $\change{\fiPCSP(\Gamma)}$ if and only if for some positive integer $n$, there exists no folded\change{, idempotent} coloring $f: \S^n \rightarrow \change{\B}$ that respects $\Gamma$.
\end{corollary}

~\Cref{thm:raghavendra} together with~\Cref{cor:basic-bool} shows that if for a Boolean \change{promise template} $\Gamma$ does not admit a sphere coloring $f: \mathbb{S}^n \rightarrow \change{\B}$ that respects $\Gamma$ for some positive integer $n$, then, $\change{\fiPCSP(\Gamma)}$ does not admit a polynomial time robust algorithm, assuming the Unique Games Conjecture. Thus, our goal is to show that the \change{promise templates} mentioned in~\Cref{lem:reduction} do not admit sphere coloring that respects them, and use~\Cref{cor:basic-bool} to prove~\Cref{thm:main-hardness}.

In the rest of this section, we first prove a couple of lemmas regarding sphere Ramsey theory. Then, we show that the earlier mentioned \change{promise templates} $\Gamma_{1\text{---}5}$ do not have folded sphere coloring respecting them using the sphere Ramsey results. 
%
\subsection{Sphere Ramsey theory}

We start with \change{some more notation}. For a tuple of vectors $S=(\textbf{v}_1, \textbf{v}_2,\ldots,\textbf{v}_k)$ with $\textbf{v}_i \in \S^{d}$, we use $\rho(S)$ to denote the \change{the smallest radius of a sphere} that contains $S$ as a subset. 
\[
\rho(S):=\min\{r: \exists\, \textbf{c}\in \R^d, \norm{\textbf{c}-\textbf{v}_i}_2=r\,\forall i \in [k]\}.
\]
Let $S_1 = (\textbf{u}_1, \textbf{u}_2,\ldots,\textbf{u}_k)$, $S_2 = (\textbf{v}_1, \textbf{v}_2,\ldots,\textbf{v}_k)$ with $\textbf{u}_i \in \S^{d_1}$, $\textbf{v}_i \in \S^{d_2}$ be two tuples with the same arity. We say that $S_1$ and $S_2$ are congruent if they have the same pairwise inner products, i.e., $\change{ \langle \textbf{u}_i , \textbf{u}_{j} \rangle =\langle \textbf{v}_i , \textbf{v}_{j} \rangle }$ for all $i,j\in [k]$.
%
...
Let $S=(\textbf{u}_1, \textbf{u}_2,\ldots,\textbf{u}_k)$ be a tuple of affinely independent vectors with $\rho(S)<1$. Then, for every positive integer $r \geq 2$, there exists an integer $n_0 := n_0(S,r)$ such that for every $n \geq n_0$, for every \change{coloring} $f:\mathbb{S}^n \rightarrow [r]$, there exists a tuple of vectors $S'=(\textbf{v}_1, \textbf{v}_2,\ldots,\textbf{v}_k), \textbf{v}_i\in \mathbb{S}^n\,\forall i \in [k]$ that is congruent to $S$, and is monochromatic, i.e., $f(\textbf{v}_i)=f(\textbf{v}_j)$ for every $i,j \in [k]$.
\end{theorem}
...
Fix an integer $k \geq 3$ and $r \geq 2$. There exists $n_0 := n_0(k\change{, r})$ such that for every $n \geq n_0$ and coloring $f: \mathbb{S}^n \rightarrow [r]$ and $\gamma \in \R$ with $ \frac{-1}{k-1} <\gamma <1$, there exists a monochromatic set of vectors $V = \{ \textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_k \} \subseteq \mathbb{S}^n$ such that $\change{ \langle \textbf{v}_i , \textbf{v}_j \rangle} = \gamma$ for every $i \neq j$. 
\end{lemma}
\begin{proof}
Consider an arbitrary set $S = \{ \textbf{u}_1, \textbf{u}_2, \ldots, \textbf{u}_k \}$ of $k$ unit vectors in $\mathbb{S}^n$ such that $\change{ \langle \textbf{u}_i , \textbf{u}_j \rangle }= \gamma$ for every $i \neq j$. Such a set $S$ is guaranteed to exist when $n$ is large enough. We show that the vectors are affinely independent: suppose for contradiction that there exists reals $c_1, c_2, \ldots, c_k$ not all zero, $\sum_i c_i = 0$ and $\sum_i c_i \textbf{u}_i = 0$.
We have 
\[
0=\change{ \left\langle \textbf{u}_1 , \left( \sum_i c_i \textbf{u}_i \right) \right\rangle }  = c_1 + \gamma ( c_2 + \ldots + c_k) = c_1 + \gamma ( - c_1) 
\]
...
\[
\norm{\textbf{u}_s}_2^2 = \sum_i \norm{\textbf{u}_i}_2^2 + 2 \sum_{i \neq j} \change{ \langle \textbf{u}_i , \textbf{u}_j \rangle }= \change{k +k(k-1) \gamma }
\]
Note that 
\begin{align*}
\norm{ \textbf{u}_i - \textbf{c} }_2^2 &=  \norm{\textbf{u}_i}_2^2 + \norm{\textbf{c}}_2^2 - 2 \change{ \langle \textbf{c} , \textbf{u}_i \rangle} \\ 
&= 1+\alpha^2 (\change{ k +k(k-1) \gamma })- 2 \alpha (1+(k-1)\gamma) \\ 
...
We use the following notion of $\Spread$ vectors from~\cite{matouvsek1995ramsey}. For an integer $n$, a vector $\textbf{a} \in \R^k$, and a set $J \subseteq [n]$ of cardinality $k$ with $J=\{ j_1, j_2, \ldots, j_k\}$ \change{where $j_1 < \cdots < j_k$}, %
we let 
\[
\SpreadSingle_n(\textbf{a}, J) = \change{ \sum_{i=1}^k a_i \textbf{e}_{j_i} }
\]
where \change{$\textbf{e}_1, \textbf{e}_2, \ldots, \textbf{e}_n$} is an orthonormal basis of $\R^n$. 
For a set $I \subseteq [n]$, we let 
\[
\Spread_n(\textbf{a}, I) = \{ \SpreadSingle_n (\textbf{a}, J) : J \subseteq I, |J|=k \}
\]
\change{Recall the hypergraph Ramsey theorem: for any $n, k, r$ there exists $N$ such that in any $r$-coloring of the hyperedges of the complete $k$-uniform hypergraph on $N$ vertices, there exists an induced subgraph on $n$-vertices which is monochromatic.} \change{As a direct consequence, we get the following application to spreads.} %
%
...
Let $U=\{\textbf{u}_1, \textbf{u}_2, \ldots, \textbf{u}_k \}$ be a set of $k$ unit vectors such that $\textbf{u}_i \in \Spread_N(\textbf{a},[N]) $ for all  $i \in [k]$ for an integer $N$, and a vector $\textbf{a} \in \R^N$ with $\norm{\textbf{a}}_2=1$. Then there exists $n_0 := n_0(U,\textbf{a},N)$ such that for every $n \geq n_0,r$, for every sphere coloring $f: \S^n \rightarrow [r]$, there exists a set of $k$ vectors $V=\{\textbf{v}_1, \ldots, \textbf{v}_k\}$ that are all colored the same, and $\change{ \langle \textbf{v}_i , \textbf{v}_j \rangle= \langle \textbf{u}_i , \textbf{u}_j\rangle }$ for every $i,j \in [k]$. 
\end{corollary}

We use~\Cref{coroll:spreads-ramsey} to prove a lemma regarding sphere colorings. For ease of notation, we call a set of $k$ unit vectors $V=\{ \textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_k \}$ to be \textit{$k$-regular} if $\change{ \langle \textbf{v}_i , \textbf{v}_j \rangle }= -\frac{1}{k-1}$ for every $i \neq j$. 

\begin{lemma}
\label{lem:sphere-coloring-2}
Fix an integer $k \geq 2$. There exists $n_0 := n_0(k)$ such that for every $n \geq n_0$ and folded coloring $f: \mathbb{S}^n \rightarrow \change{\B}$, there exist a $k$-regular set of vectors $V = \{ \textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_k \} \subseteq \mathbb{S}^n$ such that exactly $k-1$ vectors in $V$ are colored $-1$. 
\end{lemma}
...
\begin{enumerate}
    ...
    \]
    This ensures that $\change{ \langle \textbf{v}'_i , \textbf{v}'_j \rangle }= -(\gamma')^2$ for every $i,j \in [k-1], i \neq j$. 
    \item We obtain $J_{k+1}$ from $J_k$ by adding two new elements. 
    \[
    J_{k+1} = J_k \cup \{ N+1, N+2k\}
    \]
    This ensures that $\change{ \langle \textbf{v}'_i , \textbf{v}'_{k+1} \rangle }= -(\gamma')^2$ for every $i \in [k-1]$. 
    \item Finally, we set $J_k$. 
    \[
    J_k = \{ N+1, N+2, \ldots, N+2k\}
    \]
    This ensures that $\change{ \langle \textbf{v}'_i , \textbf{v}'_{k} \rangle }= -(\gamma')^2$ for every $i \in [k+1],i \neq k$.
\end{enumerate}
...
\begin{lemma}
\label{lem:gamma1}
Fix an even integer $k \geq 4$. There exists an integer $n_0$ such that for every $n \geq n_0$, there is no folded $f:\mathbb{S}^n \rightarrow \change{\B}$ that respects  $\Gamma_1 = (P,Q)$, $P=\Ham_{k}\{\frac{k}{2}\}$, $Q = \Ham_{k} \{0,1,\ldots,k\} \setminus \{b\}$ where $b \in \{1,k-1\}$.
\end{lemma}
\begin{proof}
Consider a large integer $n$ and suppose for the sake of contradiction that there is a folded function $f: \S^n \rightarrow \change{\B}$ that respects $\Gamma_1$ with respect to a vector $\textbf{v}_0 \in \mathbb{S}^n$. 
...
\begin{enumerate}
    \item (First moments.) $\change{ \langle \textbf{v}_i , \textbf{v}_0 \rangle }= 0$ for every $i \in [k]$. 
    \item (Second moments.) $\change{ \langle \textbf{v}_i , \textbf{v}_j \rangle }= \frac{2 \binom{\change{k/2}}{2} - \frac{k^2}{4}}{\binom{k}{2}}  = \frac{-1}{k-1}$ \change{for every $i \neq j \in [k]$}.
\end{enumerate}
...
\[
\textbf{v}_0^{\perp}:= \{ \textbf{u} \in \S^n : \change{ \langle \textbf{u}, \textbf{v}_0 \rangle }= 0\}
\]
Using~\Cref{lem:sphere-coloring-2}, we can obtain a set of $k$ vectors $\textbf{u}_1, \textbf{u}_2, \ldots, \textbf{u}_{k} \in \textbf{v}_0^{\perp}$ such that $\change{ \langle \textbf{u}_i , \textbf{u}_j \rangle }= \frac{-1}{k-1}$ and exactly $k-1$ of $\{ \textbf{u}_1, \textbf{u}_2, \ldots, \textbf{u}_k \}$ are colored $-1$. 
\[
...
\begin{lemma}
\label{lem:gamma2}
Fix an odd integer $k \geq 3$ and integer $l : 0 \leq l \leq \frac{k-1}{2}$. There exists an integer $n_0$ such that for every $n \geq n_0$, there is no folded $f:\mathbb{S}^n \rightarrow\change{\B}$ that respects $\Gamma_2 =  (P,Q)$, $P=\Ham_k \{l,\frac{k+1}{2}\}$, $Q=\Ham_k \{0,1,2,\ldots,k-1\}$.
\end{lemma}
\begin{proof}
Consider a large integer $n$ and suppose for contradiction that there is a folded function $f : \S^n \rightarrow \change{\B}$ that respects $\Gamma_2$ with respect to a vector $\textbf{v}_0 \in \mathbb{S}^n$.  
...
\begin{enumerate}
    \item (First moments).  \change{ By symmetry, $\langle \textbf{v}_i, \textbf{v}_0 \rangle = \langle \textbf{v}_j, \textbf{v}_0 \rangle $ for all $i, j \in [k]$.}
    
We have     
    $\change{ \langle \textbf{v}_i , \textbf{v}_0 \rangle = \frac{1}{k} \langle (\sum_{i \in [k]} \textbf{v}_i , \textbf{v}_0 \rangle = 
    \frac{1}{k} \left(\frac{1}{1-s}(l-(k-l))+\frac{-s}{1-s}1\right)=0}$ for every $i \in [k]$. 
    \item (Second moments). By symmetry of variables, we get that $\change{ \langle \textbf{v}_i , \textbf{v}_j \rangle}= \gamma$ for every $i \neq j$, for some $\gamma:=\gamma(k,l)$. 
    We have 
    \begin{align*}
        k+k(k-1)\gamma=\norm{\sum_i \textbf{v}_i}^2 = \change{\sum_{\textbf{a}\in P}\lambda(\textbf{a}) \left(\sum_{i \in [k]}a_i\right)^2}>0.
    \end{align*}
    Thus, we get that $\change{\frac{-1}{k-1}} < \gamma < 1$. 
\end{enumerate}
...
\begin{lemma}
\label{lem:gamma3}
Fix integers $k, l $ such that $0 < l \leq \frac{k-1}{2}$. Then, there exists an integer $n_0$ such that for every $n \geq n_0$, there is no folded $f:\mathbb{S}^n \rightarrow \change{\B}$ that respects $\Gamma_3 = (P,Q)$, $P=\Ham_k \{l,k\}$, $Q=\Ham_k \{1,2,\ldots,k\}$, where $l\neq 0, l \leq \frac{k-1}{2}$. 
\end{lemma}
\begin{proof}
Consider a large integer $n$ and suppose for contradiction that there is a folded function $f : \S^n \rightarrow \change{\B}$ that respects $\Gamma_3$ with respect to a vector $\textbf{v}_0 \in \mathbb{S}^n$.  
...
k \text{ with probability }\change{\frac{-s}{k-s}}
\end{cases}
\]
...
\begin{enumerate}
    \item (First moments). $\change{ \langle \textbf{v}_i , \textbf{v}_0 \rangle = \frac{1}{k}\left\{\left( \frac{k}{k-s} \right) \frac{s}{k} + \left( \frac{-s}{k-s} \right) 1 \right\} }=0$ for every $i \in [k]$. 
    \item (Second moments). As in~\Cref{lem:gamma2}, we have 
    $\change{ \langle \textbf{v}_i , \textbf{v}_j \rangle }= \gamma$ for every $i \neq j$, for some $\gamma:=\gamma(k,l)$ with
    \begin{align*}
        k+k(k-1)\gamma=\norm{\sum_i \textbf{v}_i}^2 = \change{\sum_{\textbf{a}\in P}\lambda(\textbf{a}) (\sum_{i \in [k]}a_i)^2}>0.
    \end{align*}
    Thus, we get that $\change{\frac{-1}{k-1}} < \gamma < 1$.
\end{enumerate}
    We restrict ourselves to vectors in $\mathbb{S}^n$ that are orthogonal to $\textbf{v}_0$, and applying~\Cref{lem:sphere-coloring-1}, we get that for any coloring $f:\mathbb{S}^n \rightarrow \change{\B}$, there is a monochromatic $P$-configuration that we described. By negating the vectors if needed, we get our required proof. 
\end{proof}
...
There exists integer $n_0$ such that for every $n \geq n_0$, there does not exist coloring $f:\mathbb{S}^n \rightarrow \{0,1\}$ that is folded and respects the \change{promise template} $\Gamma_4 = (P,Q),P=\Ham_k \{l\}, Q=\Ham_k \{0,1,\ldots,k\}\setminus \{0,k-1\}$.
\end{lemma}
\begin{proof}
As before, consider a large integer $n$ and suppose for contradiction that there is a folded function $f : \S^n \rightarrow \change{\B}$ that respects $\Gamma_4$ with respect to a vector $\textbf{v}_0 \in \mathbb{S}^n$.  
We partition the predicate $P$ into $P_1$ and $P_{-1}$ depending on the value of the first element, i.e., 
\[
P_i = \{ \textbf{x} \in P: x_1 = i \}, i \in \change{\B}
\]
We pick the probability distribution $\lambda$ as follows: sample $i$ from $\change{\B}$ uniformly at random, then, sample a uniformly random element from $P_i$. We get 
\begin{enumerate}
    \item (First moments). By our choice of $P_i$s, we get that 
    \[
    \change{ \langle \textbf{v}_1 , \textbf{v}_0 \rangle}= 0
    \]
    By using the symmetry of the rest of the variables and the fact that $\sum_{i=1}^k \change{ \langle \textbf{v}_i , \textbf{v}_0\rangle }=l-(k-l)=2l-k$
    , we get that 
    \[
    \change{ \langle \textbf{v}_i , \textbf{v}_0 \rangle }= \frac{2l-k}{k-1} \forall i \in \{2,3,\ldots,k\}.
    \]
    For ease of notation, let $\alpha = \frac{2l-k}{k-1}$. 
    \item (Second moments). 
    We have 
    \begin{align*}
        \change{ \left\langle \textbf{v}_1 , \left( \sum_{i=2}^k \textbf{v}_i\right) \right\rangle }&= \frac{1}{2} \left( 1 \cdot (2l-k-1) \right)+ \frac{1}{2}\left( (-1) \cdot (2l-k+1)\right)\\
        &=-1
    \end{align*}
    Thus, we get 
    \begin{align*}
    \change{ \left\langle \textbf{v}_1 , \textbf{v}_i \right\rangle } &= \frac{-1}{k-1}\,\forall i \in \{2,3,\ldots,k\}.
    \end{align*}
    Note that we have 
    \[
    ...
    \begin{align*}
    \change{ \langle \textbf{v}_i , \textbf{v}_j \rangle } &=\frac{(2l-k)^2-(k-2)}{(k-1)(k-2)} \,\forall i,j \in \{2,3,\ldots,k\}, i \neq j
    \end{align*}
...
Our goal is to show that there exists $n_0$ such that for every $n \geq n_0$, for every folded sphere coloring $f: \mathbb{S}^n \rightarrow \change{\B}$ and $\textbf{v}_0 \in \mathbb{S}^n$, there exists a set of $k$ vectors $V=\{ \textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_k \}$ that satisfy the above first and second moments, and exactly $b$ vectors in $V$ are colored $+1$, where $b \in \{0,k-1\}$. 
...
We apply~\Cref{lem:sphere-coloring-1} on the following coloring of the sphere. For a vector $\textbf{u} \in \S^n$ such that $\change{ \langle \textbf{u}, \textbf{v}_0\rangle }=0$, 
let $f':\S^{n-1} \rightarrow \change{\B}^2$ be defined as 
\[
...
\]
and $\textbf{v}^{(2)}_1=-\textbf{v}^{(1)}_1$. We now prove that the set of vectors $\textbf{v}^{(1)}_1, \ldots, \textbf{v}^{(1)}_k$ and the set of vectors $\textbf{v}^{(2)}_1, \ldots, \textbf{v}^{(2)}_k$ are a $P$-configuration with first and second moments as computed earlier, where we sampled $i$ from $\change{\B}$ uniformly at random and set the probability distribution $\lambda$ as the uniform distribution over $P_i$.
\begin{enumerate}
    \item (First moments). As $\change{\langle \textbf{u}_i , \textbf{v}_0 \rangle }= 0$ for all $i \in [k-1]$, we get that 
    \[
    \change{\langle \textbf{v}^{(1)}_1 , \textbf{v}_0 \rangle }= \change{\langle \textbf{v}^{(2)}_1 , \textbf{v}_0 \rangle }= 0
    \]
    and 
    \[
    \change{\langle \textbf{v}^{(1)}_i , \textbf{v}_0 \rangle }= \change{\langle \textbf{v}^{(2)}_i , \textbf{v}_0 \rangle }= \alpha \,\,\, \forall i \in \{2,\ldots,k\}
    \]
    \item (Second moments). We have $\forall i \in \{2,3,\ldots,k\}$,
    \begin{align*}
        \change{ \langle \textbf{v}^{(1)}_1 , \textbf{v}^{(1)}_i \rangle }&= - \frac{\change{ \langle (\sum_{j=1}^{k-1}\textbf{u}_j) , (\alpha \textbf{v}_0 + \sqrt{1-\alpha^2}\textbf{u}_{i-1})\rangle }}{\norm{\sum_{j=1}^{k-1} \textbf{u}_j}}  \\ 
        &= \frac{\sqrt{1-\alpha^2}}{k-1} -\frac{\change{ \langle (\sum_{j=1}^{k-1}\textbf{u}_j) , (\sum_{j=1}^{k-1}\textbf{u}_j) \rangle }}{\norm{\sum_{j=1}^{k-1} \textbf{u}_j}} \\ 
    ...
    \begin{align*}
        \change{ \langle \textbf{v}^{(1)}_i , \textbf{v}^{(1)}_j \rangle } &= \change{ \langle ( \alpha \textbf{v}_0 + \sqrt{1-\alpha^2}\textbf{v}_i), ( \alpha \textbf{v}_0 + \sqrt{1-\alpha^2}\textbf{v}_j) \rangle }\\ &= \alpha^2 + (1-\alpha^2)\gamma \\ 
        &= \beta  \ .
    \end{align*}
    Similarly, we have  
    \begin{align*}
        \change{ \langle \textbf{v}^{(2)}_1 , \textbf{v}^{(2)}_i \rangle } &=  \frac{\change{ \langle (\sum_{j=1}^{k-1}\textbf{u}_j) , (\alpha \textbf{v}_0 - \sqrt{1-\alpha^2}\textbf{u}_{i-1})\rangle }}{\norm{\sum_{j=1}^{k-1} \textbf{u}_j}}  = \frac{-1}{k-1}  \, \, \forall i \in \{2,3,\ldots,k\}.\\
        \change{ \langle \textbf{v}^{(2)}_i , \textbf{v}^{(2)}_j \rangle } &= \change{ \langle ( \alpha \textbf{v}_0 - \sqrt{1-\alpha^2}\textbf{v}_i), ( \alpha \textbf{v}_0 - \sqrt{1-\alpha^2}\textbf{v}_j)\rangle } \\ &= \alpha^2 + (1-\alpha^2)\gamma  = \alpha^2 + (1-\alpha^2)\gamma = \beta \, \, \forall i,j \in \{2,3,\ldots,k\}, i \neq j
    \end{align*}
\end{enumerate}
...
There exists an integer $n_0$ such that for every $n \geq n_0$, there does not exist coloring $f:\mathbb{S}^n \rightarrow \{0,1\}$ that is folded and respects the \change{promise template} $\Gamma_5 =  (P,Q)$, $P=\Ham_k \{1,k\}$, $Q=\Ham_k \{0,1,\ldots,k\}\setminus\{b\}$.
\end{lemma}
...
\begin{enumerate}
    \item (First moments). $\change{ \langle \textbf{v}_i , \textbf{v}_0 \rangle }= \left( \frac{k}{2k-2} \right) \frac{2-k}{k} + \left( \frac{k-2}{2k-2} \right) 1 =0$ for every $i \in [k]$. 
    \item (Second moments). For every $i \neq j \in [k]$, we get 
    \begin{align*}
        \change{ \langle \textbf{v}_i , \textbf{v}_j \rangle }&= \left( \frac{k}{2k-2} \right) \frac{\binom{k-1}{2}-(k-1)}{\binom{k}{2}} + \left( \frac{k-2}{2k-2} \right)1 = \frac{k-3}{k-1}
    \end{align*}
\end{enumerate}
...
\begin{align*}
    x^TM(\gamma, \epsilon, d)x &= \gamma ( \sum_{i=1}^{k+1}x_i)^2 + (\alpha - \gamma) ( \sum_{i=1}^{k}x_i)^2 + (1-\alpha) \sum_{i=1}^kx_i^2 + (1-\gamma)x_{k+1}^2 + \change{ 2\epsilon} (x_{k+1})(\sum_{i=1}^d x_i) \\ 
    &\geq (1-\alpha) \sum_{i=1}^{k+1} x_i^2 + \change{2\epsilon } (x_{k+1})(\sum_{i=1}^d x_i) \\
    &\geq \change{(1-\alpha) \sum_{i=1}^{k+1} x_i^2 -2\epsilon \left|(x_{k+1})(\sum_{i=1}^d x_i)\right|}\\
    &\geq \change{(1-\alpha) \sum_{i=1}^{k+1} x_i^2 -2\left(\frac{1-\alpha}{k}\right) \left|(x_{k+1})(\sum_{i=1}^d x_i)\right|}\\
    &\geq \left( \frac{1-\alpha}{k} \right) \left(k \sum_{i=1}^{k+1} x_i^2 - \change{2}\left|(x_{k+1})(\sum_{i=1}^d x_i)\right| \right) \\ 
    &\geq \change{\left( \frac{1-\alpha}{k} \right) \left(k \sum_{i=1}^{k+1} x_i^2 - \sum_{i=1}^d (x_i^2 + x_{k+1}^2) \right)}\\ 
    &\geq 0. \qedhere
\end{align*}
\end{proof}
...
Fix an integer $k\geq 2$ and $0 < \alpha < 1$. Suppose that $U$ and $V$ are two $\alpha$-configurations in $\mathbb{S}^n$ \change{ with cardinality $k$ each}. Then, there exists $n_0 := n_0(k,\alpha)$, and $L := L(k, \alpha)$ such that as long as $n \geq n_0$, there exist $\alpha$-configurations $V_1, V_2, \ldots, V_L$ \change{each with cardinality $k$ }such that 
\begin{enumerate}
...
\Cref{lem:gamma1,lem:gamma2,lem:gamma3,lem:gamma4,lem:gamma5} for templates $\change{\Gamma_{1-5}}$, together with~\Cref{lem:reduction},~\Cref{thm:raghavendra} and~\Cref{lem:sphere-coloring} finish the proof of our main hardness result~\Cref{thm:main-hardness}. 

\medskip \noindent \textbf{Explicit Construction.} We give an explicit construction of an integrality gap instance for \change{$\fiPCSP(\Gamma_5)$ where} $\Gamma_5=(P, Q), P=\Ham_k \{1,k\}, Q=\Ham_k \{0,1,\ldots,k\}\setminus\{b\}$ for arbitrary $b \in \{0,1,\ldots,k\}$. 
...
%
\change{For every subset $S \subseteq [2k-1]$ with $|S|=k$ and $S=\{i_1, i_2, \ldots, i_k\}$,  we add the following $L+k+1$ constraints to the instance:} 
\begin{align*}
...
We choose $L$ to be the constant factor from~\Cref{lem:connectivity} with $\alpha =\frac{k-3}{k-1}$. 
 The idea is that when all the \change{literals} in the constraint $\{x_{i_1}, x_{i_2}, \ldots, x_{i_k}\}$ are \change{set} to be True, all the \change{literals} in $\{\overline{x_{i_1}}, \overline{x_{i_2}}, \ldots, \overline{x_{i_k}}\}$ \change{are set to be False.} \change{Since} there \change{is} a series of constraints between them where we alter a single variable, there must exist a constraint where there are exactly $b$ variables that are set to True. 

Formally, we show that \change{this instance lacks a weakly satisfying assignment for} $Q=\Ham_k \{0,1,\ldots, k\} \setminus \{b\}$. 
Suppose for contradiction that there is an assignment that \change{weakly} satisfies all the constraints. Since there are $2k-1$ variables $x_1, x_2, \ldots, x_{2k-1}$, at least $k$ of them are set to be true. If not, then at least $k$ of the negated variables are set to be true. This implies that there is a sequence of constraints where the endpoints are assigned all True and all False, and at every point, we change a single variable. This implies that there is a constraint where there are exactly $b$ variables that are set to True, a contradiction. 

We now show that the instance has a basic SDP solution with zero error. Set $\textbf{v}_1, \textbf{v}_2, \ldots, \textbf{v}_{2k-1} \in \mathbb{S}^n$ to the variables $x_1, x_2, \ldots, x_{2k-1}$ such that $\change{ \langle \textbf{v}_i , \textbf{v}_0 \rangle }= 0$, and $\change{ \langle \textbf{v}_i , \textbf{v}_j \rangle}= \alpha$ for every $i \neq j$.  \change{
%
...
\change{We prove our minion characterization for ``ordinary'' promise constraint satisfaction problems over any domain. In particular, we do not consider instances with folding or the setting of constants.}
...
\change{
\paragraph{Identity minion.} \change{The \emph{identity minion} $\cM_{\id}$ is the identity functor from $\NeFinSet$ to itself (when viewed as a subcategory of $\Set$). In other words, for every non-empty finite set $A$ we have that $\cM_{\id}^A = A$, and for every map $\tau : A \to B$, we have that $\cM_{\id}^\tau = \tau$.}
}
...
\subsubsection{\change{Minion homomorphisms}}
...
\begin{definition}
A \emph{minion homomorphism} $\psi: \mathcal M \to \mathcal N$ between two minions \change{is a natural transformation between the respective functors. More concretely, for every non-empty finite set $A$, we have a map $\psi_A : \cM^A \to \cN^A$ such that for every map $\tau : A \to B$ and $M \in \cM^A$, we have that}
\[
    \change{\tau \oslash \psi_A(M) = \psi_B(\tau \oslash M).}
\]
...
\change{To assert the existence of a minion homomorphism from $\mathcal M$ and $\cN$, we write $\cM \to \cN$. A key result of \cite{BBKO21} is that minion homomorphisms govern the computational complexity of PCSPs.}

\change{\begin{theorem}[\cite{BBKO21}]
...
\change{Another result of \cite{BBKO21} is that for any promise template $\Gamma$, $\cQ_{\conv} \to \Pol(\Gamma)$ if and only if the basic LP solves $\PCSP(\Gamma)$. Our goal in the remainder of this section is to prove an analogous result for the basic SDP.} 
...
\change{Let $\Gamma$ be a promise template.} One of the most important tools for understanding minion \change{homomorphisms of} the form $\mathcal M \to \Pol(\change{\Gamma})$ \change{is} the \emph{free structure} \cite{BBKO21}. \change{Since in this paper we parameterize our promise constraint satisfaction problems using promise templates rather than a pair of relational structures, we restate the definition of a free structure in our language, which we call a \emph{free template}.}  \change{Given a relation $R \subseteq D^k$, we define the \emph{free relation} $\F_{\cM}(R)$ to be a predicate of arity $k$ over $\cM^D$ with the following property, we have that $(M_1, \hdots, M_k) \in (\cM^D)^k$ is an element of $R$ if and only if there exists $N \in \cM^{R}$ such that for all $i \in [k]$, we have that 
\[
    \pi_i \oslash N = \cM_i,
\]
where $\pi_i : R \to D$ is the projection of $R$ onto the $i$th coordinate.

Given a promise template $\Gamma = \{(P_1, Q_1), \hdots, (P_l, Q_l)\}$, we define the \emph{free relation} 
\[
...
%
\change{This is t}he fundamental property of the free structure\change{.}
...
\change{In the language of category theory, the free structure can be thought of as an adjoint functor. See the discussion preceding Remark 4.5 in \cite{BBKO21}.} 


\subsection{SDP \change{m}inion \change{d}efinition}

\change{
We now define the minion corresponding to the behavior of the basic SDP. We call this minion $\cM_{\SDP}.$ Recall that for $\Q_{\conv}$, for any non-empty finite set $A$, we have that $\Q_{\conv}^A = \Delta_A$, where $\Delta_A$ is the set of probability distributions over $A$. We define $\cM_{\SDP}^A$ in an analogous manner which captures orthogonal configurations of vectors index by $A$.}

\change{S}ince the SDP minion is a universal object, we need to be able to represent vectors of arbitrary large dimensions. We achieve this using infinite-dimensional vectors that are eventually zero. Similar techniques have been used in other minion constructions \cite{ciardo2022clap}.

Let $\R^{\omega}$ be infinite sequences of real numbers \change{indexed by $\N$} which are eventually $0$\change{.} \change{We note} that $\R^{\omega}$ is an inner product space \change{with the bilinear form}
\[
    \change{\langle \bx, \by \rangle := \sum_{i=1}^{\infty} x_i y_i,}
\]
\change{which is well-defined as only finitely many terms in the summation are nonzero.}

\change{We now formally define $\cM_{\SDP}$. For each non-empty finite set $A$, we define $\cM_{\SDP}^{A}$ to be the set of functions $\bw : A \to \R^{\omega}$ with the following properties.}

%

%
\begin{enumerate}
\item For all \change{distinct $a, a' \in A$, $\langle \bw(a), \bw(a')\rangle = 0$.}
\item $\sum_{\change{a \in A}} \|\change{\bw(a)}\|^2_2 = 1$.
\end{enumerate}
Observe that the \change{first condition implies that the} second condition is equivalent to $\|\sum_{\change{a \in A}} \change{\bw(a)}\|_2 = 1$.

The minors of $\cM_{\SDP}$ are \change{defined analogously to $\cQ_{\conv}$. Let $A$ and $B$ be non-empty finite sets and consider $\bw \in \cM_{\SDP}^A$ and $\tau : A \to B$. We define $\tau \oslash \bw : B \to \R^{\omega}$ to be for all $b \in B$,} %
\[
    \change{(\tau \oslash \bw)(b) := \sum_{a \in \tau^{-1}(b)} \bw(a),}
\]
\change{where the empty sum equals $\mathbf{0} \in \R^{\omega}$. We now finish the remaining details which prove that $\cM_{\SDP}$ is a minion.}
...
\end{lemma}}
\begin{proof}
First, for each \change{$\bw \in \cM_{\SDP}^{A}$} and \change{$\tau : A \to B$} we verify that $\change{\bw' := \tau \oslash \bw \in \cM_{\SDP}^{B}}$. First, fix \change{distinct $b, b' \in B$}. We have that
\begin{equation*}
    \change{ \langle \bw'(b) , \bw'(b') \rangle }= \change{ \left\langle \Bigl(\sum_{a \in \tau^{-1}(b)} \bw(a)\Bigr), \Bigl(\sum_{a' \in \tau^{-1}(b')} \bw(a')\Bigr)\right\rangle}
    = \sum_{\change{\substack{a \in \tau^{-1}(b)\\a' \in \tau^{-1}(b')}}} \change{ \langle \bw(a) , \bw(a')\rangle}
    = 0\change{,}
\end{equation*}
\change{since $b$ and $b'$ are distinct and $\tau$ is a function.} Further,
\begin{align*}
\change{ \left\langle \Bigl( \sum_{b \in B} \bw'(b)\Bigr), \Bigl(\sum_{b \in B} \bw'(b)\Bigr) \right\rangle} &= \change{ \left\langle \Bigl(\sum_{a\in A} \bw(a)\Bigr), \Bigl(\sum_{a\in A} \bw(a)\Bigr) \right\rangle}= 1.
\end{align*}
%
%
Thus, $\change{\bw' \in \cM_{\SDP}^{B}}.$ It is trivial to observe that \change{$\cM^{\id_A}$ is the identity map.}

The only remaining condition to check is that the minors commute. Consider \change{$\pi : A \to B$} and \change{$\tau : B \to C$. Consider $\bu \in \cM_{\SDP}^A.$}  We seek to verify that $\tau \oslash (\pi \oslash \bu) = (\tau \circ \pi) \oslash \bu$. For all \change{$c \in C$}, we have that
\change{
\begin{align*}
...
\text{(a4)} && \change{ \langle \textbf{v}_{i,a},   \textbf{v}_0 \rangle } &= \sum_{\substack{f \in \cF_j\\f(x_i) = a}}\lambda_j(f)&  \forall j\in [m],~x_i \in C_j,~ a \in D \\ 
\text{(a5)} && \change{ \langle \textbf{v}_{i,a} , \textbf{v}_{i',a'} \rangle } &= \sum_{\substack{f \in \cF_j\\f(x_i) = a, f(x_{i'}) = a'}}\lambda_j(f)  & \forall j\in [m],~x_i,x_{i'} \in C_j,~ a,a' \in D 
\end{align*}
}
%

\change{We call this formulation of the basic SDP the ``traditional'' basic SDP, in contrast to the ``alternative'' basic SDP we soon present.}
...
\begin{remark}
\change{Note that the special ``truth'' vector $\bv_0$ is omitted from this formulation. As we shall soon see, this does not change the power of the SDP relaxation, but it does make the analysis more straightforward.}
\end{remark}

\change{
\begin{remark}\label{rem:b3}
Note that (b3) can also be rewritten as $\bv_i = \tau_{i,j} \oslash \bw_j$, where $\tau_{i,j} : \cF_j \to D$ is the map $\tau_{i,j}(f) = f(x_i)$. Written in this level of generality, conditions (b1--b3) can then be readily modified to give a relaxation of \emph{any} minion $\cM$. However, such a relaxation is not algorithmically tractable in general and thus the study of such relaxations in the PCSP literature has been restricted to a restricted class of minions (e.g., ``linear'' minions in \cite{cz22-minion}). 
\end{remark}
}
...
We now prove that these two notions formulations of the basic SDP are equivalent. 
That is, a solution to either SDP can be transferred to the other. 

\change{\begin{lemma}\label{lem:SDP-equiv}
...
\change{Before we begin, we note that we often apply linear maps to the vectors into order to transform vectors from one SDP solution to another, extending the ideas in Section~\ref{subsec:gram-ortho}. In particular, we let $\R^{\omega \times \omega}$ denote the set of matrices such that each row and column has finite support. These finite support conditions are such that for any $M \in \R^{\omega \times \omega}$ and $v \in \R^{\omega}$ we have that $Mv \in \R^{\omega}$ and that $\R^{\omega \times \omega}$ is closed under the transpose operator.}

\change{The identity matrix $I_{\omega} \in \R^{\omega \times \omega}$ has $(I_{\omega})_{i,j} = 1$ if $i = j$ and $0$ otherwise. We say that $Q \in \R^{\omega \times \omega}$ is \emph{orthogonal} if $Q Q^{T} = Q^T Q = I_{\omega}.$ We now extend Proposition~\ref{prop:gram-ortho} to our setting.}

\change{\begin{proposition}\label{prop:ortho}
...
\medskip\textbf{Alternative to traditional.} First, we show that a solution to the alternative basic SDP is a solution to the traditional basic SDP. For each $\change{i \in [n]}$, define $\change{\bv_{i,D} = \sum_{a \in D} \bv_{i,a}}$. By (b1), we know that each $\change{\bv_{i,D}}$ is a unit vector. Further by (b3), we can deduce that $\change{\bv_{i,D} = \bv_{i',D}}$ whenever $\change{x_i}$ and $\change{x_{i'}}$ appear in a common constraint $\change{C_j}$. Thus, $\change{\bv_{i,D} = \bv_{i',D}}$ whenever $\change{x_i}$ and $\change{x_{i'}}$ are in the same connected component of the \change{hypergraph with vertices $\{x_1, \hdots, x_n\}$ and hyperedges $\{C_1, \hdots, C_m\}$}. \change{Let $I \subseteq [n]$ and $J \subseteq [m]$ be the indices of the variables and constraints appearing in some connected component. For any orthogonal matrix $Q \in \R^{\omega \times \omega}$,} each of the constraints (b1), (b2), and (b3) are preserved when we \change{multiply each vector $\bv_{i,a}$ for $i \in I$ and $\bw_{j,f}$ for $j \in J$ on the left by $Q$.} Thus, by \change{multiplying by suitable orthogonal matrices (i.e., apply Proposition~\ref{prop:ortho} for $n=1$)}, we can achieve a solution to (b1-3) such that \change{each $\bv_{i,D}$} is the same unit vector for all $\change{i \in [n]}$, which we shall call $\change{\bv_0}$. \change{We call this an \emph{aligned} solution to the alternative basic SDP.}

\change{We now transform this aligned solution into a solution to the traditional basic SDP. Each $\bv_{x,a}$ in the traditional basic SDP is actually the same as it appeared in the aligned solution to the alternative basic SDP. We set $\bv_0$ to the newly-defined $\bv_0$. Finally, we set $\lambda_j(f) = \|\bv_{j,f}\|_2^2$ for all $f \in \cF_j$.}

To see that the traditional basic SDP is satisfied, note that condition (a1) follows by inspection, and condition (a2) follows from (b2). \change{Conditions (a3) and (a4) follow} by substituting $\change{\bv_{i,D}}$ to $\change{\bv_0}$ and applying (b3). Condition \change{(a5)} follows \change{by} combining (b2) and (b3).

\medskip\textbf{Traditional to alternative.} We now show that any solution to the traditional basic SDP is a solution to the alternative basic SDP. Assume we have a traditional basic SDP solution. Recall that $\change{\bv_{i,a}} \in \R^\omega$ \change{for each $i \in [n]$ and $a \in D$}. We shall use the same vectors $\change{\bv_{i,a}}$ in \change{the solution to the alternative basic SDP}. Note that (b1) then follows from \change{(a5)} and (a2). 

Since there are finitely many vectors, there exists some $N \in \N$ such that each $\change{\bv_{i,a}}$ \change{(and $\bv_0$)} has its support within the first $N$ coordinates. \change{For each $j \in [m]$ and $f \in \cF_j$, p}rovisionally assign $\change{\hat{\bw}_{j,f}}$ to be $\sqrt{\lambda_{\change{j}}(\change{f})} \cdot e_i$, where \change{$e_i \in \R^{\omega}$ is the $i$th standard unit vector for some $i > N$} chosen uniquely for each constraint-assignment pair. As there are finitely many constraint-assignment pairs, all of these \change{new vectors} are in $\R^{\omega}$.

However, as written the $\change{\hat{\bw}_{j,f}}$'s are not compatible with the $\change{\bv_{i,a}}$'s. \change{To rectify this, for each $j \in [m], i \in S_j,$ and $a \in D$ we define}
\[\change{\hat{\bv}_{j,i,a}} := \sum_{\substack{\change{j \in \cF}\\\change{f(x_i)} = a}} \change{\hat{\bw}_{j,f}}\]
\change{Using (a5), we can readily verify that for any $j \in [m]$, $i,i' \in S_j$, and $a,a' \in D$, we have that $\langle \bv_{i,a}, \bv_{i,a'}\rangle = \langle \hat{\bv}_{j,i,a}, \hat{\bv}_{j,i',a'}\rangle$. Thus, by Proposition~\ref{prop:ortho}, for each $j \in [m]$, there exists an orthogonal matrix $Q_j \in \R^{\omega \times \omega}$ such that $Q_j \hat{\bv}_{j,i,a} = \bv_{i,a}$ for all $i \in S_j$ and $a \in D$.}

\change{For each $j \in [m]$ and $f \in \cF_j$, define $\bw_{j,f} := Q_j \hat{\bw}_{j,f}$. Observe that for each $j \in [m]$, $i \in S_j,$ and $a \in D$, we have that
\[
    \bv_{i,a} = Q_j \hat{\bv}_{j,i,a} = \sum_{\substack{\change{j \in \cF}\\\change{f(x_i)} = a}} Q_j \change{\hat{\bw}_{j,f}} = \sum_{\substack{\change{j \in \cF}\\\change{f(x_i)} = a}} \bw_{j,f}, 
\]}
as desired. Thus, conditions (b2) and (b3) hold. \change{Therefore,} the two SDP formulations are equivalent\change{, proving Lemma~\ref{lem:SDP-equiv}.}

\subsection{From minion homomorphism to SDP rounding algorithm}

%

Recall that we are trying to prove that \change{for any promise template $\Gamma$,} the basic SDP decides $\PCSP(\change{\Gamma}) $ if and only if $\cM_{\SDP} \to \Pol(\change{\Gamma})$.  We begin by showing the \change{easier} direction that the minion homomorphism implies that the basic SDP decides $\PCSP(\change{\Gamma})$. \change{In other words, for any instance of $\PCSP(\Gamma)$, the existence of a vector solution to alternative basic SDP of the instance implies that there exists a weakly satisfying assignment to the instance.}

\begin{theorem}\label{thm:hom_to_alg}
If $\cM_{\SDP} \to \Pol(\change{\Gamma})$, then the basic SDP decides $\PCSP(\change{\Gamma})$.
\end{theorem}
\change{
\begin{proof}
...
We now prove the converse. \change{To do this, we adapt} the proof techniques of \change{Theorem 4.11 of }\cite{ciardo2022clap}. 

\begin{theorem}
If the basic SDP decides $\PCSP(\change{\Gamma})$, then $\cM_{\SDP} \to \Pol(\change{\Gamma})$.
\end{theorem}

%

\begin{proof}
\change{Assume that $(D_1,D_2)$ is the domain of $\Gamma = \{(P_1,Q_1), \hdots, (P_l, Q_l)\}$ with a homomorphism $h : D_1 \to D_2$ such that $h(P_i) \subseteq Q_i$ for all $i \in [l]$. To simplify the proof, we make the following technical assumption similar to the reduction in Remark~\ref{rem:Ragh}. If a relation $(P,Q) \in \Gamma$ of arity $k$, then every possible relation which can be created by repeating variables is also in $\Gamma$. For example, if $P, Q$ have arity $3$, then $(P',Q') \in \Gamma$ where $P' = \{(x,y) \in D_1^2 : (x,y,x) \in P\}$ and $Q' = \{(x,y) \in D_2^2 : (x,y,x) \in Q\}$. Note that $\Pol(P,Q) \subseteq \Pol(P',Q')$, so adding $(P',Q')$ to $\Gamma$ does not make $\cM_{\SDP} \to \Pol(\change{\Gamma})$ harder to verify. Likewise, since repeating variables is allowed in PCSP instances, whether the basic SDP decides $\PCSP(\change{\Gamma})$ is unaffected by these extra relations.
...
\item As mentioned earlier, our algorithm for Majority polymorphism can be generalized for arbitrary threshold polymorphisms. Together with the hardness results in \cite{BGS21}, this gives a dichotomy w.r.t robust \change{PCSP} satisfiability for the class \emph{ordered} Boolean \change{promise templates} (whose polymorphisms are monotone functions).
...
Let $X$ be a symmetric matrix with $n+m+1$ rows/columns---the last $m$ columns correspond to the ``error'' in each of the $m$ clauses. Given a set $S$ of indices, we let $X|_{S}$ denote the square submatrix whose rows and columns come from $S$.
\begin{align*}
...
\subsection{\change{Missing proofs from Section 4}}
...
In order to show that our algorithm is a robust algorithm for this \change{fi}PCSP, it suffices to show that all the elements in the predicates are rounded to the same value with high probability. Consider $i,j \in [k]$. We show that the probability that the variables $x_i$ and $x_j$ get rounded to different values is at most $O\left( \frac{1}{r}\right)$. Using the union bound over all the $\binom{k}{2}$  pairs of indices, we get our required claim. 
...
\]
Thus, we get that $\textbf{a} \change{\in} \{\sgn(\textbf{x} - \textbf{y}) : \textbf{x}, \textbf{y} \in \Aff(P), \forall i, x_i \neq y_i \}$, completing the proof that 
\[
O_{AT}(P)\subseteq \{\sgn(\textbf{x} - \textbf{y}) : \textbf{x}, \textbf{y} \in \Aff(P), \forall i, x_i \neq y_i \}
\qedhere \]
\end{proof}

\subsection{\change{Missing proofs from Section 5}}
...
We recall that $O_{\AT}(P)$ (\change{respectively} $O_{\MAJ}(P)$) denotes the set $\bigcup_{\textbf{x}_1,\ldots,\textbf{x}_L\in P,L \in \mathbb{N},\text{ odd},}\AT_L(\textbf{x}_1,\ldots,\textbf{x}_L)$ (\change{respectively} $\MAJ_L$) for a predicate $P$.

%
%
%
Let $k$ denote the arity of $P,Q$, i.e., $P \subseteq Q \subseteq \change{\B}^k$.
...
Suppose that $b \notin \{0,k\}$. 
Let $Q' = \change{\B}^k \setminus \Ham_k \{b\}$. By definition, $\MAJ \nsubseteq \Pol(P,Q')$. 
Using the fact that $O_{\AT}(\Ham_k \{l\}) = \Ham_k \{1,2,\ldots,k-1\}$, we get that $\AT \nsubseteq \Pol(P,Q')$. 
Thus, we get a \change{promise template} $(P,Q)$ that is \change{fi}ppp-definable from original \change{promise template} where $P=\Ham_k \{l\}$, $Q=\Ham_k \{0,1,\ldots,k\} \setminus \{b\}$ where $b \in \{1,2,\ldots, k-1\} \setminus \{l\}$. 
Note that $\MAJ, \AT \notin \Pol(P,Q)$. 
We now modify this \change{promise template further}, updating $P,Q,l,k,b$ while preserving the following two properties:
\begin{enumerate}
...
We use the following two tools to repeatedly obtain a new \change{promise template} that is \change{fi}ppp-definable from the previous one. 
\begin{enumerate}
    \item Given a \change{promise template} $P = \Ham_k \{l\}, Q = \Ham_k \{0,1,\ldots,k\} \setminus \{b\}$ where $b \in \{1,2,\ldots,k-1\}\setminus\{l\}$, then the \change{promise template} $P' = \Ham_{k-1} \{l\}, Q' = \Ham_{k-1} \{0,1,\ldots,k-1\} \setminus \{b\}$ is \change{fi}ppp-definable from $(P,Q)$ (Claim $4.2$ of~\cite{BrakensiekG21}). As long as $b<k-1$ and $b \neq 2l-k+1$, this update preserves the above two properties. 
    \item Given a \change{promise template} $P = \Ham_k \{l\}, Q = \Ham_k \{0,1,\ldots,k\} \setminus \{b\}$ where $b \in \{1,2,\ldots,k-1\}\setminus\{l\}$, then the \change{promise template} $P' = \Ham_{k-1} \{l-1\}, Q' = \Ham_{k-1} \{0,1,\ldots,k-1\} \setminus \{b-1\}$ is \change{fi}ppp-definable from $(P,Q)$ (Claim $4.4$ of~\cite{BrakensiekG21}). As long as $b >1$ and $b \neq 2l-1$, this update preserves the above two properties. 
\end{enumerate}
Now, we update the \change{promise template} using the above two steps. As $k$ is decreasing at every step, this procedure terminates at some point. Then, either of the two conditions holds:
\begin{enumerate}
    \item $b=1, b=2l-k+1$. In this case, we get that $l = \frac{k}{2}$ and $b=1$. Thus, $(P',Q')$ is \change{fi}ppp-definable from $\Gamma$ where $P'=\Ham_{k}\{\frac{k}{2}\}, Q = \Ham_{k} \{0,1,\ldots,k\} \setminus \{1\}$ where $k$ is even and is at least $4$. 
    \item $b=k-1, b=2l-1$. In this case, we get that $l = \frac{k}{2}$ and $b=k-1$. Thus, $(P',Q')$ is \change{fi}ppp-definable from $\Gamma$ where $P'=\Ham_{k}\{\frac{k}{2}\}, Q = \Ham_{k} \{0,1,\ldots,k\} \setminus \{k-1\}$ where $k$ is even and is at least $4$. 
\end{enumerate}

Suppose that there is no $b \notin \{0,k\}$ such that $\Ham_k \{b\} \subseteq O_{\MAJ} (P) \setminus Q $. As $O_{\MAJ}(P) \nsubseteq Q$, by negating the variables if needed, we can assume that $\Ham_k \{0\} \in O_{\MAJ}(P) \setminus Q$. Furthermore, there exists $b \in \{1,2,\ldots, k-1\}$ such that $\Ham_k \nsubseteq  Q$ as $O_{\AT}(P) \nsubseteq Q$. Thus, we obtain $(P,Q)$ that is \change{fi}ppp-definable from the original \change{promise template} such that $P=\Ham_k \{l\}, Q=\Ham_k \{1,\ldots,k\}\setminus \{b\}$ where $l,b \in \{1,2,\ldots,k-1\}, b >2l-1$. By using the first type of update used above (Claim $4.2$ of~\cite{BrakensiekG21}), we obtain a new \change{promise template} $(P,Q)$ that is \change{fi}ppp-definable from the original \change{promise template} such that $P=\Ham_k \{l\}, Q=\Ham_k \{0,1,\ldots,k\}\setminus \{0,k-1\}$, where $l \in \{1,2,\ldots,k-1\}, l \leq \frac{k-1}{2}$. 

\smallskip \noindent \textbf{Case 2}. There exist $l \neq l'$ such that $\Ham_k \{l,l'\}\subseteq P$. Recall that $P \nsubseteq \Ham_k \{0,k\}$. This implies that $O_{\AT}(P)=\Ham_k \{0,1,\ldots,k\}$. Hence, we can get a \change{promise template} $(P,Q')$ that is \change{fi}ppp-definable from the original \change{promise template} such that $Q' = \Ham_k \{0,1,\ldots, k\} \setminus \{b\}$ and $\Ham_k \{b\} \nsubseteq O_{\MAJ}(P)$. 

For ease of notation, let $P=\Ham_k S$, where $S \subseteq \{0,1,\ldots,k\}$. First, consider the case when $\min S=0, \max S=k$. As mentioned earlier, we know that there exists $l \in \{1,2,\ldots,k-1\}$ such that $\Ham_k \{l\} \subseteq P$. Thus, we can obtain a new PCSP $(P,Q)$ that is \change{fi}ppp-definable from the original \change{promise template} where $P=\Ham_k \{0,l,k\}, Q = \Ham_k \{0,1,\ldots,k\}\setminus \{b\}$ and $b \notin \{0,l,k\}$. We consider three cases separately:
\begin{enumerate}
    \item Suppose that $l \leq \frac{k-1}{2}$. In this case, we have a \change{promise template} $(P,Q)$ that is \change{fi}ppp-definable from the original \change{promise template} where $P=\Ham_k \{l,k\}$ and $Q = \Ham_k \{0,1,\ldots,k\}\setminus \{b\}$ and $b \notin \{l,k\}$. Note that this \change{promise template} does not contain $\AT$ or $\MAJ$ as polymorphisms. 
    \item Suppose that $l = \frac{k}{2}$. In this case, we have a \change{promise template} $(P,Q)$ that is \change{fi}ppp-definable from the original \change{promise template} where $P=\Ham_k \{l\}$ and $Q = \Ham_k \{0,1,\ldots,k\}\setminus \{b\}$ where $b \notin \{0,l,k\}$. This also doesn't have $\AT$ and $\MAJ$ as polymorphisms. We have already shown that we can relax this further to the earlier mentioned three \change{promise templates}. 
    \item Suppose that $l \geq \frac{k+1}{2}$. In this case, we have a \change{promise template} $(P,Q)$ that is \change{fi}ppp-definable from the original PCSP where $P=\Ham_k \{0,l\}$ and $Q = \Ham_k \{0,1,\ldots,k\}\setminus \{b\}$ where $b \notin \{0,l\}$. Note that this \change{promise template} does not contain $\AT$ or $\MAJ$ as polymorphisms. 
\end{enumerate}
Thus, there is a \change{promise template} $(P,Q)$ that is \change{fi}ppp-definable from the original PCSP where $P=\Ham_k \{l,l'\}, Q = \Ham_k \{0,1,\ldots,k\}\setminus \{b\}$ such that $l <l',\{l, l'\} \neq \{0,k\}$, $b \in O_{\MAJ}(P)$. 
We end up with the same PCSP when $\{\min S, \max S\} \neq \{0,k\}$. 

If $\{l,l'\} = \{1,k\}$ or $\{0,k-1\}$, we get that the \change{promise template} $P=\Ham_k\{1,k\}, Q=\Ham_k \{0,1,\ldots,k\}\setminus\{b\}$ is \change{fi}ppp-definable from the original \change{promise template}, and we are done. 
If not, we update the \change{promise template} while maintaining the two below properties:
\begin{enumerate}
    \item $P=\Ham_k \{l,l'\}$ with $l <l'$ and $\{ l, l' \} \neq \{0,k\}$ and $Q = \Ham_k \{0,1,\ldots, k\} \setminus \{b\}$. We also assume that $\{l,l'\} \neq \{1,k\}$ and $\{l,l'\}\neq \{0,k-1\}$. 
    \item $\AT, \MAJ \nsubseteq \Pol(P,Q)$. 
\end{enumerate}
As with the earlier case, we obtain a series of new \change{promise templates} that are \change{fi}ppp-definable from the previous \change{promise templates} using the below tools. 
\begin{enumerate}
...
When we are unable to update the \change{promise template} using the above procedures, one of the following must be true.
\begin{enumerate}
    \item $l'=k, b=0$. In this case, we have a \change{promise template} $(P,Q)$ where $P=\Ham_k \{l,k\}$, $Q=\Ham_k \{1,2,\ldots,k\}$, where $l\neq 0, l \leq \frac{k-1}{2}$. 
    \item $b=k,l=0$. By negating the variables, we can observe that the above \change{promise template} is \change{fi}ppp-definable from this \change{promise template}. 
    \item $b=k, b=2l'-1$. We have $l'=\frac{k+1}{2}$. In this case, we have \change{promise template} $(P,Q)$ where $P=\Ham_k \{l,\frac{k+1}{2}\}$, $Q=\Ham_k \{0,1,2,\ldots,k-1\}$, where $l \leq \frac{k-1}{2}$. 
    \item We have $b=2l-k+1, b=0$. By negating the variables, we can observe that the above \change{promise template} is \change{fi}ppp-definable from this \change{promise template}. 
\end{enumerate}

Thus, we have obtained a new \change{promise template} that is \change{fi}ppp-definable from the original \change{promise template} and is equal to either of the following \change{promise templates}. 
\begin{enumerate}
...
Finally, we note that if a Boolean \change{promise template} $\Gamma'=\{(P_1, Q_1),\ldots,(P_l, Q_l)\}$ is \change{fi}ppp-definable from another Boolean \change{promise template} $\Gamma$, then $\Gamma'$ remains \change{fi}ppp-definable from $\Gamma$ even when we disallow the constraints \change{of} $\Gamma'$ \change{in the fippp-definition} to use constants\change{.}
\end{proof}
