...
The link between the auxiliary process and the population process is given by Many-to-One formulas. We refer to \cite{georgii2003supercritical,baake2007mutation,hardy2009spine,bansaye2011limit,cloez2017limit,marguet2016uniform, marguet2017law} for general 
results on these topics in the continuous-time case.
...
as the strong solution to a SDE. In Section \ref{sec_constant_diff} we provide necessary conditions for the cell population to get rid of parasites or for the quantity of parasites 
to explode in all the cells. Then, in Section \ref{sec:proportion}, we explore asymptotics of the proportion of cells with a given level of infection. In particular in Section \ref{sec:LDCD}, we investigate the case of a linear division rate.
...
\end{align*}
where $\brho \in (-1,0)$ and $c_\brho<0$ (see \cite[Section 1.2.6]{kyprianou2006introductory} for details on stable distributions and processes).
...
with associated symmetric distribution $\kappa(d\theta)$ satisfying $\int_0^1 |\ln \theta|\kappa(d\theta)<\infty$.
...
(production of proteins, division, etc.).

\subsection{Existence and uniqueness}

We use the classical Ulam-Harris-Neveu notation to identify each individual. Let us denote by
\begin{equation*} \label{ulam_not} \mathcal{U}:=\bigcup_{n\in\mathbb{N}}\left\{0,1\right\}^{n}
\end{equation*}
...
\end{equation}
where $V_t\subset\mathcal{U}$ denotes the set of individuals alive at time $t$ and $X_t^u$ the trait at time $t$ of the individual $u$. We denote by $N_{t} : = \mathrm{Card}(V_t)$ the number of cells in the population at time $t$.
...
We obtain a large class of branching Markov processes for the modelling of parasite infection in a cell population. Points $i)$ to $iii)$ of Assumption {\bf{EU}}
...
%We will now investigate the long time behaviour of the infection in the cell population.
...
\end{align}
where we recall that $\Theta$ is a random variable with distribution $\kappa$ and
...
$e^{-\int_0^tG_a(Y_s)ds}$ and thus, $G_a$ contains informations on the dynamics of the quantity of parasites in a typical cell. 
...
\begin{align*}
 \frac{g(x)}{x}-a\frac{\sigma^2(x)}{x^2} + x^{-\brho}C_a-p(x)I_a(x)-2r(x)\frac{\E[\Theta^{1-a}-1]}{a-1}\geq \gamma',
\end{align*}
...
\begin{align*}
 \frac{g(x)}{x}-a\frac{\sigma^2(x)}{x^2} + x^{-\brho}C_a-p(x)I_a(x)-2r(x)\frac{\E[\Theta^{1-a}-1]}{a-1}\leq \gamma'.
\end{align*}
...
\begin{itemize}
  \item[i)] Under Assumption \ref{ass_expl_tps_fini},
  $ \lim_{t \to \infty} \P_{\delta_x}\left( \exists u \in V_t, X_t^u \leq K \right) =0. $
  \item[ii)] Under Assumption \ref{ass_abs_tps_fini},
  $ \lim_{t \to \infty} \P_{\delta_x}\left( \exists u \in V_t, X_t^u \geq 1/K \right) =0. $
 \end{itemize}
\end{prop}

Thus in case {\it i)}, the quantity of parasites goes to infinity in all the cells, and
in case {\it ii)}, the quantity of parasites goes to zero in all the cells with a probability close to one.

Proposition \ref{prop_constant_diff} applies to a large class of dynamics for the growth of parasites within cells, but imposes a limited cell growth (bounded $r-q$). In the next two sections, we focus on more general cases for cell growth, requiring more assumptions on parasite dynamics.
...
\item There exist $\beta\in (0,+\infty)$ and $\mathfrak{c}\geq 0$ such that $\beta r(x) -q(x) = \mathfrak{c}$ for all $x\geq 0$. 
\end{itemize}
\end{customass}

Under Assumption \ref{ass:betar-q}, we provide conditions under which the mean number of cells with a small (resp. a large) quantity of parasites is equivalent to the mean number of cells alive in the population.

\begin{prop} \label{prop:betar-q} Let $\beta\in [0,+\infty)$ be such that Assumption \ref{ass:betar-q} holds and assume that there exists $\alpha_\beta\in\R$ such that $2\E\left[\Theta^{\alpha_\beta}\right] - 1 = \beta$. Then,
\begin{enumerate}[label=(\roman*)]
\item\label{point1-propbetar-q} if $\beta\in(0,1)$ (then $\alpha_\beta\in (0,1)$) and $\alpha_\beta<1-g/\sigma^2$, for all $K>0$,
\begin{align*}
\lim_{t\rightarrow+\infty}\frac{\E_{\delta_x}\left[\sum_{u\in V_t} \mathbf{1}_{\lbrace X_t^u>K\rbrace}\right]}{\E_{\delta_x}\left[N_t\right]} =  0.
\end{align*}
\item\label{point2-propbetar-q} if $\beta\in(0,1)$ (then $\alpha_\beta\in (0,1)$) and there exist $x_0(\alpha_\beta)\geq 0$ and $\eta>0$ such that 
\begin{equation} \label{star1} g\leq \sigma^2(1-2\alpha_\beta) + 2\E\left[\Theta^{\alpha_\beta}\ln(1/\Theta)\right]r(x)-\eta,\quad \forall x\geq x_0(\alpha_\beta),\end{equation} then for all $\eps,x>0$ there exists $K_{\eps,x}$ such that $K\geq K_{\eps,x}$ implies
\begin{align*}
...
\end{align*}
\item\label{point3-propbetar-q} if $\beta>1$ (then $\alpha_\beta<0$) and there exist $x_0(\alpha_\beta)\geq 0$ and $\eta>0$ such that
\begin{equation} \label{star2} 
g \geq \sigma^2(1+2|\alpha_\beta|) + 2\E\left[\Theta^{\alpha_\beta}\ln(1/\Theta)\right]r(x)+\eta,\quad \forall x\leq x_0(\alpha_\beta) ,
\end{equation}
...
The first point of Proposition \ref{prop:betar-q} corresponds to the case of a growing cell population ($r(x)-q(x)\geq 0$), with a moderate growth of the parasites compared to the noise of the environment $(g<\sigma^2)$. The condition $\alpha_\beta<1-{g}/{\sigma^2}$ links the growth of the population and the growth of the parasites: if ${g}/{\sigma^2}$ is small, there is no additional restriction on the growth of the cell population (except that $\beta\in[0,1)$), but if ${g}/{\sigma^2}$ is close to $1$, then $\beta$ has to be close to $1$ for the condition to be fulfilled, which corresponds to a growth of the cell population independent of the quantity of parasites ($(r-q)(x)$ close to $ \mathfrak{c}$).


\begin{rem}
The case $\beta=1$ ($\alpha_\beta=0$) corresponds to a constant $r-q$, and will be studied in details and in more generality in another work. In our case, if $r$ and $q$ are constant, for the second and third points, it amounts to look at the sign of
...
\item There exists $\mathfrak{c}\geq 0$ such that $g(x)/x -q(x) = \mathfrak{c}$ for all $x\geq 0$. 
\end{itemize}
\end{customass}

To state the next results, we introduce generalized conditions from \cite{companion} under which the quantity of parasites may not reach the state $0$ ({\bf{(SN0)}} for small noise at $0$) or reach the state $\infty$ ({\bf{(SN$\infty$)}} for strong noise at $\infty$). Recall the definition of $C_a$ and $I_a$ in \eqref{eq:Ia}. 
\begin{enumerate}[label=\bf{(SN0)}]
\item\label{A1} 
There exist $a>1$ such that $\E[\Theta^{1-a}]<\infty$ and $f:\R_+\to \R_+$ such that
\begin{equation*}
...
Under Assumption \ref{ass:gx-q}, $C_a=0$, but we will use later those general conditions.

\begin{prop} \label{prop:gx-q} Let $\mathfrak{c}\geq 0$ be such that Assumption \ref{ass:gx-q} holds. Then,
\begin{enumerate}[label=\roman*)]
...
\begin{equation} \label{add_ass} \limsup_{x \to \infty}\left\{\frac{\sigma^2(x)}{x^4}+ \frac{r(x)}{x^2}+\frac{p(x)}{x^3} \right\}<\infty,
 \end{equation} 
and there exists $x_0\geq 0$, $\eta_0>0$ such that
...
\end{equation}
and $x_1>0$, $\eta_1>0$ such that
\begin{equation}\label{eq:condinfty}
\frac{g(x)}{x} + \frac{\sigma^2(x)}{x^2} + \frac{p(x)}{x^2}\int_{\R_+}z^2\pi(dz) + 2r(x)\left(\E\left[\Theta^2\right]-1/2\right) \leq -\eta_1,\quad \forall x\geq x_1,
\end{equation}
then for all $\eps,x>0$ there exists $K_{\eps,x}$ such that $K\leq K_{\eps,x}$ implies
\begin{align*}
\limsup_{t \to \infty}\frac{\E_x\left[\sum_{u\in V_t} \mathbf{1}_{\lbrace X_t^u\leq K\rbrace}\right]}{\E_x\left[N_t\right]}\leq \eps.
\end{align*}
...
\begin{align*}
\limsup_{t \to \infty}\frac{\E_x\left[\sum_{u\in V_t} \mathbf{1}_{\lbrace X_t^u\geq K\rbrace}\right]}{\E_x\left[N_t\right]}\leq \eps.
\end{align*}
\end{enumerate}
\end{prop}
\begin{rem}
...
\begin{rem}
The results of Proposition \ref{prop:gx-q} are weaker than the ones of Proposition \ref{prop:betar-q}: they concern mean numbers of cells with given amounts of parasites, and not the proportions of such cells in the population. 
But the assumptions of Proposition \ref{prop:gx-q} are much more flexible than the assumptions of Proposition \ref{prop:betar-q}: they are in some way their local versions.  
\end{rem}
...
\item The measure $\pi$ satisfies $\int (z^2\wedge z^3)\pi(dz)<\infty$ and for all $x\geq 0$, $xp'(x)\geq p(x)$.
\end{itemize}
\end{customass}
...
A linear increase of the division rate in order to get rid of the parasites is an efficient strategy for the cell population, as stated in the next proposition. Recall that $\mathcal{G}$ has been defined in \eqref{def_gene}.

\begin{prop}\label{prop:containment}
Under Assumption \ref{ass_E}, for all $x\geq 0$,
\begin{enumerate}
\item[i)] For all $a >0$,
...
Moreover, we consider positive jumps and various diffusive functions for the dynamics of the parasites, and add the possibility for the cells to die.\\

From this result, we see that the proportion of very infected cells goes to $0$ as $t$ tends to infinity so that a linear division rate is sufficient 
...
\begin{customass}{\bf{LDCG+}} \label{ass_E2} We assume that Assumption \ref{ass_E} holds and that
...
To state the following results, we also need to introduce conditions under which the quantity of parasites may {\bf{(LN0)}} or may not {\bf{(SN0)}} reach the state $0$. They are almost necessary and sufficient conditions (see  \cite[Remark 3.2 and Theorem 3.3]{companion}).
Condition \ref{A1} has already been introduced, and we notice that under Assumption \ref{ass_E2} it reduces to $\sigma^2(u)/u^2 = o(\ln(u^{-1}))$ close to $0$. Condition \ref{A2} writes
...
Moreover, there exist two processes $(\mathfrak{Y}^{(1)},\mathfrak{Y}^{(2)})$ satisfying
$$ \mathfrak{Y}^{(1)}_t\leq Y^{(t)}_t\leq \mathfrak{Y}^{(2)}_t \quad \text{a.s.}\ \forall t \geq 0 $$
...
where
\begin{align*}
\mathcal{D}(\mathcal{A})=\left\lbrace f\in\mathcal{C}_b^2(\mathbb{R}_+)\text{ s.t. } m(\cdot,s,t)f\in\mathcal{C}_b^2(\mathbb{R}_+),\ \forall t\geq 0,\ \forall s\leq t \right\rbrace
\end{align*}
and
\begin{align*}
...
Those formulas come from \cite[Theorem 3.1]{marguet2016uniform}, with $B(x) = r(x)+q(x)$ and $m(x,dy) = 2r(x)(r(x)+q(x))^{-1}\int_0^1\delta_{\theta x}(dy)\kappa(d\theta)$.
...
\tY_s^{(t)}= (x,0)\right). 
\end{align*}
As we can see on the expression of the generator of the auxiliary process in \eqref{eq:gene_2var}, $D_s$ switches from $0$ to $1$ at rate $q(x)$ and 
...
f_2(y,s,\theta):=
2 (\alpha y+\beta)\frac{g-\beta+\alpha \theta y \left(e^{(g-\beta)s}-1\right)}{g-\beta+\alpha y\left(e^{(g-\beta)s}-1\right)},
\end{align}
and
...
\begin{prop}\label{prop_sol_SDE_auxi}
 Suppose that Assumption \ref{ass_E} holds and that $\int z^2\pi(dz)<\infty$, and that $xp'(x)\geq p(x)$ for every $x\geq 0$.
...
\subsection{Proof of Proposition \ref{prop_constant_diff}}
...
$$
\gamma_tf = e^{-\gamma t}\E\left[\sum_{u\in V_t} f(X_t^u)|Z_0 = \delta_x\right]
$$
be the renormalized first moment semigroup of $Z$. 
Then we have
\begin{align*}
\frac{\partial}{\partial t}\gamma_tf  = &\int_{\R_+}\left(\mathcal{G}f(\mathfrak{x})-\gamma f(\mathfrak{x})\right)\gamma_t(d\mathfrak{x})\\
& +\int_{\R_+}\left(r(\mathfrak{x})\int_0^1(f(\theta \mathfrak{x})+f((1-\theta)\mathfrak{x})-f(\mathfrak{x}))\kappa(d\theta)-
q(\mathfrak{x})f(\mathfrak{x})\right)\gamma_t(d\mathfrak{x})\\
= & \int_{\R_+}\left(\mathcal{G}f(\mathfrak{x})-\gamma f(\mathfrak{x})\right)\gamma_t(d\mathfrak{x})\\
& +\int_{\R_+}\left(2r(\mathfrak{x})\int_0^1(f(\theta \mathfrak{x})-f(\mathfrak{x}))\kappa(d\theta)+(r(\mathfrak{x})-q(\mathfrak{x}))f(\mathfrak{x})
\right)\gamma_t(d\mathfrak{x}),
\end{align*}
and using that $r(x)-q(x)\leq \gamma$ for all $x\geq 0$, we obtain
\begin{align*}
\frac{\partial}{\partial t}\gamma_tf\leq & \gamma_t(\mathcal{A}f),
\end{align*}
where $\mathcal{A}$ is defined in \eqref{eq:gene_auxi1}.
Finally, by unicity of the solution to the Kolmogorov's backward equation, 
\begin{align}\label{eq:ineq_MTO}
\gamma_tf =  \E\left[\sum_{u\in V_t} f(X_t^u)|Z_0 = \delta_x\right]e^{-\gamma t}\leq \E_{x}\left[f\left(Y_t\right)\right],
\end{align}
...
\begin{itemize}
 ...
 $$  \mathbf{1}_{\{ t>\tau^+(1/\eps)   \}}\left(Y_t \vee \zeta \right)^{1-a} = 0.$$
...
\end{align*}
by Markov's inequality. The other case is similar. Proposition~\ref{prop_constant_diff} follows from Lemma~\ref{lem_gene}.
 \end{proof}
...
First note that $\alpha_\beta$ is well-defined because $\psi:\alpha\mapsto  2\E\left[\Theta^\alpha\right] - 1$ is decreasing on $\R$, and $\psi((-\infty,1]) = [0,+\infty)$. Let $\mathcal{B}$ be the infinitesimal generator associated with the first moment semigroup of the branching process given for all $f\in\mathcal{C}^2_b(\mathbb{R}_+)$ by
...
\end{align*} Then, as  $2\E\left[\Theta^{\alpha_\beta}\right] - 1 = \beta$, we obtain
\begin{align*}
\mathcal{B}V_{\alpha_\beta}(x) = \left(\alpha_\beta g + \alpha_\beta(\alpha_\beta - 1)\sigma^2 + \mathfrak{c}\right)V_{\alpha_\beta}(x):=\lambda V_{\alpha_\beta}(x).
\end{align*}
Then, according to \cite[Lemma 3.2]{cloez2017limit}, we have for all $x\geq 0$, $f\in\mathcal{C}^2_b(\mathbb{R}_+)$, 
\begin{align}\label{eq:mtoV}
\E_{\delta_x}\left[\sum_{u\in V_t} f(X_t^u)\right]= x^{\alpha_\beta}e^{\lambda t}\E_x\left[ f(\mathcal{Y}_t)V_{\alpha_\beta}^{-1}(\mathcal{Y}_t)\right],
\end{align}
where $\mathcal{Y}$ is a Markov process with infinitesimal generator given by
\begin{align*}
\mathcal{A}_{V_{\alpha_\beta}}f(x) = \left(g + 2\sigma^2  \alpha_\beta\right)xf'(x) + \sigma^2 x^2f''(x) + 2r(x)\left[\int_0^1\theta^{\alpha_\beta}  \left(f(\theta x)-f(x)\right) \kappa(d\theta)\right],
\end{align*}
and $\mathcal{F}^\mathcal{Y}$ its natural filtration.
Note that in particular, as $2r(x)\E\left[\Theta^{\alpha_\beta}\right] = \beta  r(x) + r(x) = \mathfrak{c} + r(x) + q(x)$, we get
\begin{align*}
d\mathcal{Y}_t^{-\alpha_\beta} = \left( r(\mathcal{Y}_t)- q(\mathcal{Y}_t) -\lambda \right)\mathcal{Y}_t^{-\alpha_\beta}dt + dM^{(1)}_t,
\end{align*}
where $(M^{(1)}_t,t\geq 0)$ is a $\mathcal{F}^\mathcal{Y}_t$-martingale. Note also that taking $f\equiv 1$ in \eqref{eq:mtoV}, we obtain
\begin{align}\label{eq:Nt}
\E_{\delta_x}\left[N_t\right]= x^{\alpha_\beta}e^{\lambda t}\E_x\left[\mathcal{Y}_t^{-\alpha_\beta}\right].
\end{align}
Next, let $K\geq 0$ and $f(x)=\mathbf{1}_{\lbrace x>K\rbrace}$. Combining \eqref{eq:mtoV} and \eqref{eq:Nt}, we have
\begin{align*}
\frac{\E_{\delta_x}\left[\sum_{u\in V_t} \mathbf{1}_{\lbrace X_t^u>K\rbrace}\right]}{\E_{\delta_x}\left[N_t\right]}= \frac{\E_x\left[ \mathbf{1}_{\lbrace \mathcal{Y}_t>K\rbrace}\mathcal{Y}_t^{-\alpha_\beta}\right]}{\E_x\left[\mathcal{Y}_t^{-\alpha_\beta}\right]}.
\end{align*}

First, we prove the point \ref{point1-propbetar-q} of Proposition \ref{prop:betar-q}. Let $\beta\in (0,1)$. Then, $\alpha_\beta\in (0,1)$ and  
\begin{align}\label{eq:casebeta01}
\frac{\E_{\delta_x}\left[\sum_{u\in V_t} \mathbf{1}_{\lbrace X_t^u>K\rbrace}\right]}{\E_{\delta_x}\left[N_t\right]}\leq \frac{K^{-\alpha_\beta}\P_x(\mathcal{Y}_t>K)}{\E_x\left[\mathcal{Y}_t^{-\alpha_\beta}\right]}.
\end{align}
Moreover, in this case
\begin{align*}
\frac{d}{dt}\E_x\left[\mathcal{Y}_t^{-\alpha_\beta}\right] = \E_x\left[\left( (1-\beta)r(\mathcal{Y}_t) + \mathfrak{c} -\lambda \right)\mathcal{Y}_t^{-\alpha_\beta}\right]\geq\left( \mathfrak{c} -\lambda \right) \E_x\left[\mathcal{Y}_t^{-\alpha_\beta}\right],
\end{align*}
and by Grönwall's lemma we obtain
\begin{align*}
\E_x\left[\mathcal{Y}_t^{-\alpha_\beta}\right]\geq x^{-\alpha_\beta}e^{(\mathfrak{c} -\lambda)t}.
\end{align*}
Combining this inequality with \eqref{eq:casebeta01}, we have
\begin{align*}
\frac{\E_{\delta_x}\left[\sum_{u\in V_t} \mathbf{1}_{\lbrace X_t^u>K\rbrace}\right]}{\E_{\delta_x}\left[N_t\right]}\leq \left(\frac{x}{K}\right)^{\alpha_\beta}e^{\alpha_\beta(g + (\alpha_\beta -1)\sigma^2)t}
\end{align*}
where we used that $\lambda-\mathfrak{c} = \alpha_\beta(g + (\alpha_\beta -1)\sigma^2$. This ends the proof of point \ref{point1-propbetar-q}. 

For the point \ref{point3-propbetar-q}, let $\beta>1$. Then, $\alpha_\beta<0$. Using Itô's formula, we have for all $t\geq 0$,
\begin{align*}
\ln(\mathcal{Y}_t) = \ln(\mathcal{Y}_0) + (g + \sigma^2(2\alpha_\beta -1))t + 2\E\left[\Theta^{\alpha_\beta}\ln(\Theta)\right]\int_0^tr(\mathcal{Y}_s)ds + M^{(2)}_t, 
\end{align*}
where $(M^{(2)}_t,t\geq 0)$ is a $\mathcal{F}^\mathcal{Y}_t$-martingale. By assumption, there exist $x_0(\alpha_\beta),\eta>0$ such that \eqref{star2} holds.
To simplify, we write $x_0$ instead of $x_0(\alpha_\beta)$ later in the proof. Then, for $0<y_0<x_0$, 
...
Moreover, as \ref{A1} is satisfied thanks to \eqref{star2}, applying \cite[Theorem 3.3i)]{companion}, we obtain $\P(\mathcal{Y}_\infty = 0)=0$. Recalling that as $\alpha_{\beta}<0$, Fatou's lemma implies that for any $x>0$,
\begin{align} \label{min_Yinfty} \liminf_{t \to \infty} \E_x\left[\mathcal{Y}_t^{-\alpha_\beta}\right] \geq \liminf_{t \to \infty} \E_x[(\mathcal{Y}_t\wedge 1)^{-\alpha_\beta}] =\E_x[(\mathcal{Y}_\infty\wedge 1)^{-\alpha_\beta}] >0 .
\end{align}
Finally, as before, combining \eqref{eq:mtoV} and \eqref{eq:Nt}, we obtain for any $K\geq 0$, 
\begin{align*}
\frac{\E_{\delta_x}\left[\sum_{u\in V_t} \mathbf{1}_{\lbrace X_t^u\leq K\rbrace}\right]}{\E_{\delta_x}\left[N_t\right]}= \frac{\E_x\left[ \mathbf{1}_{\lbrace \mathcal{Y}_t\leq K\rbrace}\mathcal{Y}_t^{-\alpha_\beta}\right]}{\E_x\left[\mathcal{Y}_t^{-\alpha_\beta}\right]}&\leq  \frac{K^{-\alpha_\beta}}{\E_x\left[\mathcal{Y}_t^{-\alpha_\beta}\right]}.
\end{align*}
Using \eqref{min_Yinfty} ends the proof of \ref{point3-propbetar-q}.

The proof of point \ref{point2-propbetar-q} is very similar, and we only give the main ideas of the proof. Assume that there exist $x_0(\alpha_\beta),\eta>0$ such that \eqref{star1} holds.
...
Applying again \cite[Theorem 7.1.4]{BN}, $\mathcal{Y}_t$ converges in law to a variable $\mathcal{Y}_\infty$ on $\R_+$. As \ref{eq:SNinfinity-1} is satisfied thanks to \eqref{star1}, \cite[Theorem 4.1i)]{companion} implies $\P(\mathcal{Y}_\infty = \infty)=0$. Finally, as $\alpha_\beta \geq 0$,
$$ \E_x[\mathcal{Y}_\infty^{-\alpha_\beta}] \geq\E_x\left[(\mathcal{Y}_\infty\vee 1)^{-\alpha_\beta}\right] >0. $$
...
\begin{align*}
\mathcal{A}_{V_{1}}f(x) = & \left(g(x) + 2\frac{\sigma^2(x)}{x} +\frac{p(x)}{x}\int z^2\pi(dz)\right)f'(x) + \sigma^2(x)f''(x)\\
&+ p(x)\int \left(f(x+z)-f(x)-zf'(x)\right)\frac{(x+z)}{x}\pi(dz)   + 2r(x)\left[\int_0^1\theta \left(f(\theta x)-f(x)\right) \kappa(d\theta)\right].
\end{align*}
...
$$ \frac{d}{dt} \E \left[\mathcal{Y}_t^{-2} \right]\leq \left( -2\eta_1  \E \left[\mathcal{Y}_t^{-2} \right] +\frac{2}{x_0^2}\eta_1 + C_1 \right), $$
...
\begin{align*}
\frac{d}{dt}\E \left[\mathcal{Y}_t \right]&\leq  -\eta\E\left[\mathcal{Y}_t \right] + C_3,
\end{align*}
...
\begin{proof}
According to It\^o's formula, we have for all $t\geq 0$ and $x> 0$,
...
Point $i)$ is a direct consequence of the point $i)$ of Proposition \ref{prop:gx-q}. Let $a>0$. If we consider the process $\mathcal{Y}$ with infinitesimal generator \eqref{infgen_mathcalY}, we get from \cite[Lemma 3.2]{cloez2017limit} the following Many-to-One formulas, when considering the functions $f(x)=x\mathbf{1}_{x \in [a,a+da]}$ and $f(x) \equiv 1$,
\begin{align*}
a\E_{\delta_x}\left[\sum_{u\in V_t} \mathbf{1}_{X_t^u \in [a,a+da]} \right]= xe^{\mathfrak{c} t}\P_x\left(\mathcal{Y} \in [a,a+da] \right)
\quad
\text{and}
\quad
\E_{\delta_x}\left[\sum_{u\in V_t}1\right]= xe^{\mathfrak{c} t}\E_x\left[ \mathcal{Y}^{-1}_t\right].
\end{align*}
It concludes the proof of point $i)$.

We now prove point $ii)$. Proposition \ref{prop:gx-q}$i)$ and Markov's inequality yield for any $\eps>0$,
\begin{align*}
\lim_{K\rightarrow \infty}\lim_{t\rightarrow \infty} \mathbb{P}_{\delta_x}
...
Hence 
$$\E_{\delta_x}[P_t] \ll  \mathbb{E}_{\delta_x}\left[N_t\right], \quad t \to \infty. $$
...
Suppose that Assumptions \ref{ass_E} holds, that $\int_0^\infty (z \vee z^2)\pi(dz)<\infty$ and that $ xp'(x)\geq p(x)$ for every $x\geq 0$.
\begin{itemize}
\item[i)] 
...
However, the dependence in $\theta$ in the division rate can be removed by considering a new PPM $N'$ with a modified fragmentation kernel so that all the results derived above still hold. We refer the reader to Appendix \ref{app:generalization SDE} for more details.
\end{proof}
...
Let $T\geq 0$. Suppose that Assumption \ref{ass_E}, Equation \eqref{eq:limsup_sigma} and \ref{A2} hold, and that $\int_0^\infty (z \vee z^2)\pi(dz)$ and $xp'(x)\geq p(x)$ for every $x\geq 0$. 
...
According to \eqref{eq:limsup_sigma}, there exist $\eta,x_1>0$ such that for all $x>x_1$, $\widetilde{H}(x)\leq -\eta$ so that {\bf (LSG)} of \cite{companion} holds for $\tY$. Then, according to \cite[Theorem 6.2(i)]{companion}, $\tY$ converges in law to $\tY_\infty$ as $t$ goes to infinity.
...
To prove that the SDE \eqref{X_sans_sauts} admits a unique nonnegative strong solution with generator given by $\mathcal{G}$ defined in \eqref{def_gene}, we apply \cite[Proposition 1]{palau2018branching}. The proof is the same as the proof of \cite[Proposition 2.1]{companion}, except that we have to take into account the extra stable term. To prove that we still have a unique nonnegative strong solution with the addition of this term, it is enough to check that for any $n \in \N$, there exists a finite constant $A_n$ such that for any $0 \leq x,y \leq n$,
...
(1-a)I_a(x) = \int_{\mathbb{R}_+}\left((1+zx^{-1})^{(1-a)}-1-(1-a)zx^{-1}\right)\pi(dz).
\end{align}
...
Let $n \in \N$ be such that $n\geq 2$ and let $0<\varepsilon<b<1$ and $a>1$ such that $\E[\Theta^{1-a}]<\infty$ be such that  \ref{A1} holds for all $u\leq b$. Let $T_n = \tau^-(\varepsilon^n)\wedge \tau^+(b)$. As in \cite[Theorem 3.3i)]{companion}, combining Lemma \ref{prop:martingale-co} and \ref{A1},
...
\begin{equation} \label{bound_f3} p_t(x,s,z) \leq p(x) (1+ \mathfrak{c}_t z). \end{equation}
 For the existence of a unique pathwise solution to \eqref{eq:EDS-pt}, we refer to Appendix \ref{app:prop_sol_SDE_auxi}.

\begin{thm}\cite[Theorem 3.3i)]{companion}
Suppose that $\int (z\vee z^2)\pi(dz)<\infty$ and that \ref{ass_E} holds. Let $Y$ be a pathwise unique solution to \eqref{eq:EDS-pt}. Then, if there exist $a>1$ such that $\E\left[\Theta^{1-a}\right]<\infty$ and a non-negative function $f$ on $\mathbb{R}_+$ such that
...
\end{align*}
where $C_t$ is a finite constant. Therefore, the integral corresponding to the positive jumps is bounded in the neighborhood of $0$, and Condition \eqref{eq:SN0-zpifini} is enough to control the behaviour of the process around $0$. 
The proof of \cite[Theorem 3.3]{companion} is thus unchanged and the results hold also for processes whose rate of positive jumps satisfy \eqref{bound_f3}.
\end{proof}
...
Suppose that Assumption \ref{ass_E} and \eqref{eq:limsup_sigma} hold and that $\int (z^2\vee z^3)\pi(dz)<\infty$ and let $Y$ be a pathwise unique solution to \eqref{eq:EDS-pt}. If \ref{eq:SNinfinity-1} holds,
...
Suppose that Assumption \ref{ass_E}, \eqref{eq:limsup_sigma} and \ref{eq:SNinfinity-1} hold and that $\int (z^2\vee z^3)\pi(dz)<\infty$ and let $Y$ be a pathwise unique solution to \eqref{eq:EDS-pt}. Assume also that \ref{eq:LSG} holds (with $g,r$ instead of $\hat{g},\hat{r}$).
\begin{itemize}
...
Suppose that Assumption \ref{ass_E} and \eqref{eq:limsup_sigma} hold and that $\int (z^2\vee z^3)\pi(dz)<\infty$. Then, if \ref{eq:SNinfinity-1}  and \ref{eq:LSG} hold,
then $\E\left[T_i(x_0)\right]<\infty$ for all $i\geq 0$.
\end{lemma}

\begin{proof}(\cite[Lemma 7.2]{companion})
Let us choose $0<x_0<x_1$ and let $\tau = \tau^-(x_0)\wedge\tau^+(x_1)$, where $\tau^+,\tau^-$ are defined in \eqref{eq:coupling} and \eqref{tau1}.
...
We provide the proof of the proposition in the case $g \neq \beta$. The proof is very similar in the case $g = \beta$ and does not bring new insight. We thus skip it.
