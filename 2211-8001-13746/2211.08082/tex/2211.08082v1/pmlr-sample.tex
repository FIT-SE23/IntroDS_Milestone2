...
% Our framework significantly outperforms baseline models in multi-source learning tasks, including transfer and pooled learning, while also showing comparable results when trained on a single medical dataset.
% To empirically demonstrate the efficacy of our work, we conducted extensive experiments using various datasets, model structures, and tasks.
...
As there is no previous work, to our knowledge, that tackled exactly the same goal as ours, we modified well-known general-purpose EHR embedding frameworks(SAnD~\citep{song2018attend}, Rajkormar~\cite{rajkomar2018scalable}, DescEmb~\citep{hur2022unifying}.)
