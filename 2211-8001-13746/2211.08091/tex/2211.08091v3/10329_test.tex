...
\title{Reconstruction of Convex Sets from One or Two X-rays} %TODO Please add


\author{Yan Gerard\thanks{Address  for correspondence:  Universit\'e Clermont Auvergne,
LIMOS, France}\thanks{This work is supported by the French ANR PRC grant ADDS (ANR-19-CE48-0005)}
\\
...
\runninghead{Y. Gerard}{Reconstruction of Convex Sets}
...
\begin{keywords}
Discrete Tomography \and Polyomino \and Digital Convex sets \and Filling operations \and Switching Component
\end{keywords}
...
\vspace*{-3mm}
  \begin{center}
		\includegraphics[width=0.61\textwidth]{fig/fig2new.png}\vspace*{-5.5mm}
	\end{center}
...
\begin{figure}[!b]
  \begin{center}
		\includegraphics[width=0.77\textwidth]{fig/convexities.png}
...
\begin{figure}[ht!]
  \begin{center}
		\includegraphics[width=0.45\textwidth]{fig/graph.png}
...
\vspace*{1mm}
  \begin{center}
		\includegraphics[width=0.55\textwidth]{fig/feet0.png}
...
\vspace*{3mm}
  \begin{center}
		\includegraphics[width=0.94\textwidth]{fig/fat.png}
...
\section{The structure of the bad guy counter-example}
...
\vspace*{1mm}
  \begin{center}
		\includegraphics[width=0.95\textwidth]{fig/feet.png}
	\end{center}\vspace*{-5mm}
...
\begin{figure}[ht]
  \begin{center}
		\includegraphics[width=0.98\textwidth]{fig/filling.png}
	\end{center}\vspace*{-2mm}
...
\begin{figure}[ht]
  \begin{center}
		\includegraphics[width=0.42\textwidth]{fig/correspondences.png}
	\end{center}\vspace*{-3mm}
...
\vspace*{2mm}
  \begin{center}
		\includegraphics[width=0.98\textwidth]{fig/Fig9bis.png}
	\end{center}\vspace*{-3mm}
	\caption{\label{fig9} \textbf{Switching components}. In each switching component, we represent the points alternatively by squares $\square$ and diamonds $\diamond$ to express that their Boolean variables are negation from each other. Either the squares, or the diamonds of a switching component are in a solution.}\vspace*{-3mm}
\end{figure}
...
\vspace*{2mm}
  \begin{center}
		\includegraphics[width=0.96\textwidth]{fig/abcdef.png}
...
\vspace*{-3mm}
  \begin{center}
		\includegraphics[width=0.74\textwidth]{fig/configC.png}
	\end{center}\vspace*{-4mm}
...
%\begin{figure}[h!]
  \begin{center}
		\includegraphics[width=0.89\textwidth]{fig/cycles.png}
	\end{center}\vspace*{-4mm}
...
For each one of the six configurations, the set of undetermined points is restricted to a small number of regions. Each region corresponds horizontally and vertically to some other regions, so that we can build the graph of the vertical and horizontal correspondences of the regions. The correspondence graph in the configuration c) is for instance represented in Fig.~\ref{paths}. Such graphs admit a small number of fundamental cycles i.e minimal closed paths. There are exactly three minimal cycles in the configuration c). The closed path in the graph are obtained by concatenation of these minimal cycles. The
 fundamental minimal cycles in all configurations are represented in  Fig.~\ref{cycles}.
...
\vspace*{2mm}
  \begin{center}
		\includegraphics[width=0.74\textwidth]{fig/badguy.png}
	\end{center}
...
\begin{figure}[!h]
  \begin{center}
		\includegraphics[width=0.35\textwidth]{fig/region.png}
	\end{center}\vspace*{-4mm}
...
\end{figure}
  ...
  \begin{figure}[!h]
  \begin{center}
		\includegraphics[width=0.95\textwidth]{fig/quads.png}
	\end{center}\vspace*{-4mm}
...
\begin{figure}[h!]
  \begin{center}
		\includegraphics[width=0.95\textwidth]{fig/links.png}
	\end{center}\vspace*{-3mm}
...
\vspace*{2mm}
  \begin{center}
		\includegraphics[width=0.94\textwidth]{fig/path.png}
	\end{center}\vspace*{-4mm}
...
\vspace*{-1mm}
  \begin{center}
		\includegraphics[width=0.85\textwidth]{fig/fillingoperationsconvex.png}
	\end{center}\vspace*{-4mm}
...
Each time that a new point is added to the kernel, we update the kernel with \\ %%$Kernel  \leftarrow \conv Z2(Kernel)%%
 $\In \leftarrow \conv (\In)$ to maintain a convex solution. The number of nodes is bounded by $2(m+n)$. Computing the new convex hull takes at most $O(\log (m+n))$ (it is an insertion in the list of the nodes) and adding the lattice points in $\conv (\In)$ takes again $O(\log (m+n))$ per point namely a total time of $O(mn \log (m+n))$ (more efficient data structures can be found in  \cite{Brodal}). By repeating this operation at most $mn$ time, the total time taken by this filling operation is bounded by $O(m^2n^2 \log (m+n))$.
...
\vspace*{2mm}
  \begin{center}
		\includegraphics[width=0.45\textwidth]{fig/4borders.png}
	\end{center}\vspace*{-2mm}
...
\vspace*{2mm}
  \begin{center}
		\includegraphics[width=0.75\textwidth]{fig/critics.png}
	\end{center}\vspace*{-3mm}
...
\end{figure}
  ...
  \begin{figure}[!b]
  \begin{center}
		\includegraphics[width=0.84\textwidth]{fig/validsegments.png}
	\end{center}\vspace*{-2mm}
...
\begin{itemize}
  ...
  \end{enumerate}
   ...
   \begin{figure}[!ht]
    \begin{center}
		\includegraphics[width=0.7\textwidth]{fig/octogon.png}
	\end{center}\vspace*{-5mm}
...
\end{figure}
 ...
 \begin{itemize}
        \item[{4.}]  The next condition guarantees that the X-ray of the solution fits with the prescribed X-rays $h$ and $v$.
        ...
        $NorthRows(q)=rows(s_{\NW}) \cap rows(s_{\NE})$ and $SouthRows(q)=rows(s_{\SW}) \cap rows(s_{\SE})$ and in the columns in $WestColumn(q)=columns(s_{\NW}) \cap columns(s_{\SW})$, and $RightColumn(q)=columns(s_{\NE}) \cap$\linebreak  $columns(s_{\SE})$ (these rows and columns are colored in blue in Fig.~\ref{octogon}).
        Now we give a condition on the octagon $q$ to be a node: the four sets of rows
        $NorthRows(q)$, \linebreak $SouthRows(q)$ and columns $WestColumn(q)$, $RightColumn(q)$ must be non empty and for each of these rows and columns, the number of integer points between each pair of segments must be equal to the prescribed coordinate of the input X-ray.
    \end{itemize}
...
\vspace*{4mm}
  \begin{center}
		\includegraphics[width=0.75\textwidth]{fig/startend.png}
	\end{center}\vspace*{-3mm}
...
\vspace*{2mm}
  \begin{center}
		\includegraphics[width=0.71\textwidth]{fig/soltopath.png}
	\end{center}\vspace*{-2mm}
	\caption{\label{soltopath} \textbf{From a solution to a path in $\Gamma (h,v)$.}}\vspace*{-5mm}
\end{figure}
...
\bibitem{EM}
Carazo JM, Sorzano CO, Rietzel E, Schr{\"o}der R, Marabini R.
\newblock Discrete Tomography in Electron Microscopy, pp. 405--416.
\newblock Birkh{\"a}user Boston, Boston, MA.
\newblock ISBN 978-1-4612-1568-4, 1999.
\newblock \doi{10.1007/978-1-4612-1568-4_18}.
%%\newblock \urlprefix\url{https://doi.org/10.1007/978-1-4612-1568-4_18}.

\bibitem{Bat1}
Batenburg K, Bals S, Sijbers J, K\"ubel C, Midgley P, Hernandez J, Kaiser U,
  Encina E, Coronado E, van Tendeloo G.
\newblock 3D imaging of nanomaterials by discrete tomography.
\newblock \emph{Ultramicroscopy}, 2009.
\newblock \textbf{109}:730--40.
\newblock \doi{10.1016/j.ultramic.2009.01.009}.
\newblock 43.01.05; LK 01.

\bibitem{Bat2}
Van~Aert S, Batenburg KJ, Rossell MD, Erni R, Van~Tendeloo G.
\newblock Three-dimensional atomic imaging of crystalline nanoparticles.
\newblock \emph{Nature}, 2011.
\newblock \textbf{470}:374--377.
...
\bibitem{Herman1}
Herman GT, Kuba A.
\newblock Discrete Tomography - Foundations, Algorithms and Applications.
\newblock Birkhauser, 1999.
\newblock ISBN-0817636145.

\bibitem{Herman2}
Herman GT, Kuba A.
\newblock Advances in Discrete Tomography and Its Applications.
...
\newblock \emph{Pacific J. Math.}, 1957.
\newblock \textbf{7}:1073--1082.
\newblock \doi{doi:10.2140/pjm.1957.7.1073.}
%\newblock \urlprefix\url{https://msp.org/pjm/1957/7-2/pjm-v7-n2-p04-s.pdf}.

\bibitem{Ryser}
Ryser H.
\newblock Combinatorial properties of matrices of zeros and ones.
\newblock \emph{Can. J. Math.}, 1957.
\newblock \textbf{9}:371--377.

\bibitem{W1}
Woeginger GJ.
\newblock The reconstruction of polyominoes from their orthogonal projections.
\newblock \emph{Information Processing Letters}, 2001.
\newblock \textbf{77}(5):225 -- 229.
\newblock \doi{https://doi.org/10.1016/S0020-0190(00)00162-9}.
\newblock
  \urlprefix\url{http://www.sciencedirect.com/science/article/pii/S0020019000001629}.

\bibitem{Nivat}
Barcucci E, Lungo AD, Nivat M, Pinzani R.
\newblock Reconstructing Convex Polyominoes from Horizontal and Vertical
  Projections.
\newblock \emph{Theor. Comput. Sci.}, 1996.
\newblock \textbf{155}(2):321--347.
\newblock \doi{10.1016/0304-3975(94)00293-2}.
\newblock \urlprefix\url{https://doi.org/10.1016/0304-3975(94)00293-2}.

\bibitem{GG}
Gardner R, Gritzmann P.
\newblock Determination of finite sets by X-rays.
\newblock \emph{Transactions of the American Mathematical Society}, 1997.
\newblock \textbf{349}(6):2271–--2295.

\bibitem{Bru2}
Brunetti S, Daurat A.
\newblock Reconstruction of convex lattice sets from tomographic projections in
  quartic time.
\newblock \emph{Theoretical Computer Science}, 2008.
\newblock \textbf{406}(1):55 -- 62.
\newblock \doi{https://doi.org/10.1016/j.tcs.2008.06.003}.
\newblock Discrete Tomography and Digital Geometry: In memory of Attila Kuba.

\bibitem{Barcucci}
Barcucci E, Dulio P, Frosini A, Rinaldi S.
\newblock Ambiguity Results in the Characterization of hv-convex Polyominoes
  from Projections.
\newblock In: Discrete Geometry for Computer Imagery - 20th {IAPR}
  International Conference, {DGCI} 2017, Vienna, Austria, September 19-21,
  2017, Proceedings. 2017 pp. 147--158.
doi:10.1007/978-3-319-66272-5\_13.
...
\newblock In: Brlek S, Dolce F, Reutenauer C, Vandomme {\'{E}} (eds.),
  Combinatorics on Words - 11th International Conference, {WORDS} 2017,
  Montr{\'{e}}al, QC, Canada, September 11-15, 2017, Proceedings, volume 10432
  of \emph{Lecture Notes in Computer Science}. Springer, 2017 pp. 164--176.
\newblock \doi{10.1007/978-3-319-66396-8\_16}.
%%\newblock \urlprefix\url{https://doi.org/10.1007/978-3-319-66396-8\_16}.

\bibitem{Niccolo}
Marco ND, Frosini A.
\newblock Properties of {SAT} Formulas Characterizing Convex Sets with Given
  Projections.
\newblock In: Baudrier {\'{E}}, Naegel B, Kr{\"{a}}henb{\"{u}}hl A, Tajine M
  (eds.), Discrete Geometry and Mathematical Morphology - Second International
  Joint Conference, {DGMM} 2022, Strasbourg, France, October 24-27, 2022,
  Proceedings, volume 13493 of \emph{Lecture Notes in Computer Science}.
  Springer, 2022 pp. 153--166.
\newblock \doi{10.1007/978-3-031-19897-7\_13}.
%%\newblock \urlprefix\url{https://doi.org/10.1007/978-3-031-19897-7\_13}.

\bibitem{RSC}
Gerard Y.
\newblock Regular switching components.
\newblock \emph{Theororetical Computer Science}, 2019.
\newblock \textbf{777}:338--355.
\newblock \doi{10.1016/j.tcs.2019.01.010}.
%%\newblock \urlprefix\url{https://doi.org/10.1016/j.tcs.2019.01.010}.

\bibitem{Dulio2}
Dulio P, Frosini A.
\newblock Characterization of hv-Convex Sequences.
\newblock \emph{J. Math. Imaging Vis.}, 2022.
\newblock \textbf{64}(7):771--785.
\newblock \doi{10.1007/s10851-022-01093-z}.
%%\newblock \urlprefix\url{https://doi.org/10.1007/s10851-022-01093-z}.

\bibitem{Dulio3}
Dulio P, Frosini A.
\newblock On Some Geometric Aspects of the Class of hv-Convex Switching
  Components.
\newblock In: Lindblad J, Malmberg F, Sladoje N (eds.), Discrete Geometry and
  Mathematical Morphology - First International Joint Conference, {DGMM} 2021,
  Uppsala, Sweden, May 24-27, 2021, Proceedings, volume 12708 of \emph{Lecture
  Notes in Computer Science}. Springer, 2021 pp. 299--311.
\newblock \doi{10.1007/978-3-030-76657-3\_21}.
%%\newblock \urlprefix\url{https://doi.org/10.1007/978-3-030-76657-3\_21}.

\bibitem{Dulio1}
Dulio P, Frosini A, Rinaldi S, Tarsissi L, Vuillon L.
\newblock Further steps on the reconstruction of convex polyominoes from
  orthogonal projections.
\newblock \emph{J. Comb. Optim.}, 2022.
\newblock \textbf{44}(4):2423--2442.
\newblock \doi{10.1007/s10878-021-00751-z}.
%%\newblock \urlprefix\url{https://doi.org/10.1007/s10878-021-00751-z}.

\bibitem{Frosini10}
Frosini A, Vuillon L.
\newblock Tomographic reconstruction of 2-convex polyominoes using dual Horn
  clauses.
\newblock \emph{Theor. Comput. Sci.}, 2019.
\newblock \textbf{777}:329--337.
\newblock \doi{10.1016/j.tcs.2019.01.001}.
%%\newblock \urlprefix\url{https://doi.org/10.1016/j.tcs.2019.01.001}.

\bibitem{Sara}
Brunetti S, Daurat A.
\newblock Reconstruction of convex lattice sets from tomographic projections in
  quartic time.
\newblock \emph{Theor. Comput. Sci.}, 2008.
\newblock \textbf{406}(1-2):55--62.
\newblock \doi{10.1016/j.tcs.2008.06.003}.
%%\newblock \urlprefix\url{https://doi.org/10.1016/j.tcs.2008.06.003}.

\bibitem{Bru}
Brunetti S, Daurat A, Kuba A.
\newblock Fast Filling Operations Used in the Reconstruction of Convex Lattice
  Sets.
\newblock In: Kuba A, Ny{\'u}l LG, Pal{\'a}gyi K (eds.), Discrete Geometry for
  Computer Imagery. Springer Berlin Heidelberg, Berlin, Heidelberg, 2006 pp.
  98--109. doi:10.1007/11907350\_9.

\bibitem{Tarjan}
Aspvall B, Plass MF, Tarjan RE.
\newblock A Linear-Time Algorithm for Testing the Truth of Certain Quantified
  Boolean Formulas.
\newblock \emph{Inf. Process. Lett.}, 1979.
\newblock \textbf{8}:121--123.
doi:10.1016/0020-0190(79)90002-4.

\bibitem{Brodal}
Brodal G, Jacob R.
\newblock Dynamic planar convex hull.
\newblock In: The 43rd Annual IEEE Symposium on Foundations of Computer
  Science, 2002. Proceedings. 2002 pp. 617--626.
\newblock \doi{10.1109/SFCS.2002.1181985}.
\end{thebibliography}
                %\bibliographystyle{unsrt}\bibliographystyle{fundam}\bibliography{paper}
