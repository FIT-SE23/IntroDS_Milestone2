...
We find a strong connection between the STRP and shadowing, a.k.a. pseudo-orbit tracing property. In particular, we show that shadowing is generic for actions of free products of finite and cyclic groups, and that shadowing is not generic for groups without the STRP.
...
Let us start with some history and motivation. One of the earliest occurencies of `genericity results' in measurable and topological dynamics was Halmos' paper \cite{Halm}, following the previous result of Oxtoby and Ulam in \cite{OxUl}, where he showed that ergodicity and weak mixing are generic properties among p.m.p. bijections of the standard probability space, removing the fears that such useful properties might actually be rare among general p.m.p. bijections. He also showed that the conjugacy class of any aperiodic p.m.p. bijection is dense, a result which is also attributed to Rokhlin and often identified with the Rokhlin lemma. This also explains why the name `Rokhlin' is attached to results of this kind, we refer to the survey \cite{GlaWei08}. We refer to monographs \cite{AlpPra} and \cite{AkHurKen} for more historical background and many results of this sort and to \cite{Hoch08} for some more recent developments in the topological case.
...
The next results concern the shadowing property, also known as the pseudo-orbit tracing property. Shadowing is by now one of the fundamental notions in dynamical systems, closely related to hyperbolicity and topological stability. We refer to the monograph \cite{Palm} for an introduction and historical background. Although it was originally defined for single homeomorphisms (in fact, diffeomorphisms) it makes perfect sense for general discrete group actions and it was for the first time defined in this generality in \cite{OsTi} and investigated since then in numerous publications, see e.g. \cite{ChKeo}, \cite{Mey}, \cite{BarGaRaLi}. Genericity of shadowing has been also already extensively studied, see e.g. \cite{PilPla}, \cite{BrMeRa}, \cite{KoMaOp}, and references therein. In \cite{BeDa}, genericity of shadowing for homeomorphisms on the Cantor space has been derived as the property of the generic integer action. We continue in this line of research for more general groups and discover a strong relation with the strong topological Rokhlin property. We show that genericity of shadowing is a property of generic actions in the sense that if a group does not admit a generic action, then shadowing is not a generic property for actions of such a group. Conversely, if a group does admit a generic action, we do not know in general if such an action must have shadowing, but in all known cases it is the case and we have a sufficient condition when this happens. Here we state the following simpler version.

\begin{thmx}\label{thm:intro4}
Let $G$ be a countable group that does not have the strong topological Rokhlin property, e.g. $\Int^d$, for $d\geq 2$. Then shadowing is not generic for actions of $G$ on the Cantor space.\bigskip

On the other hand, if $G=\bigstar_{i\leq n} G_i$ is a free product of the groups $(G_i)_{i\leq n}$, each of them being either finite or cyclic, then the generic action of $G$ has shadowing, i.e. shadowing is generic.
\end{thmx}
...
Sometimes when we are given a subshift $X$ over group $G$, however the finite set $A$ is not clear from the context, we shall denote it by $\alpha(X)$, i.e. $X\subseteq \alpha(X)^G$. We may occassionally call the finite set $A$ an \emph{alphabet}.
\end{definition}
...
Let $G$ be a countable group. If there exists $n\geq 2$ such that the projectively isolated subshifts are not dense in $\SH_G(n)$, then $G$ does not have the strong topological Rokhlin property.
\end{proposition}
\begin{proof}
...
\end{definition}
It is straightforward to verify that shadowing does not depend on the choice of the compatible metric.

\begin{proposition}\label{prop:shadowing}
Let $G$ act on a zero-dimensional compact metrizable space. If the action is an inverse limit of subshifts of finite type, then it has the pseudo-orbit tracing property.
\end{proposition}
\begin{proof}
Fix the group $G$, zero-dimensional compact metrizable space $X$ and the action $\alpha: G\curvearrowright X$, which is an inverse limit of subshifts of finite type, as in the statement. We also fix a compatible metric $d$ on $X$.

We show that $\alpha$ has the pseudo-orbit tracing property. Fix $\varepsilon>0$. We can find a partition $\PP$ of $X$ into non-empty disjoint clopen sets whose diameters are less than $\varepsilon$ and such that $Z:=\QQ(\alpha,\PP)\subseteq A^G$, for some finite $A$, is a subshift of finite type. Let $\delta>0$ be the Lebesgue number of $\PP$ and let $S\subseteq G$ be a finite symmetric set defining the subshift, i.e. the allowed patterns $Z_S$ define $Z$. Suppose that $(x_g)_{g\in G}\subseteq X$ is a $(\delta,S)$-pseudo-orbit and for each $g\in G$ set $y_g:=Q_\PP^\alpha(x_g)\in Z$. Then for every $g\in G$ and $s\in S$ we have \[y_g(s^{-1})=y_{sg}(1_G).\] Indeed, since $(x_g)_{g\in G}$ is a $(\delta,S)$-pseudo-orbit, we have $d(s\cdot x_g,x_{sg})<\delta$ and thus, since $\delta$ is the Lebesgue number of $\PP$, $Q_\PP^\alpha(s\cdot x_g)$ and $Q_\PP^\alpha(x_{sg})$ lie in the same element of the partition of $\PP$, so by definition $y_g(s^{-1})=y_{sg}(1_G)$. We define $y\in A^G$ as follows. For $g\in G$ we set \[y(g):=y_{g^{-1}}(1_G).\] In order to check that $y\in Z$ we need to verify that for every $g\in G$, $g\cdot y\upharpoonright S\in Z_S$. So fix $g\in G$. Then for every $s\in S$ we have \[g\cdot y(s)=y(g^{-1}s)=y_{s^{-1}g}(1_G) =y_g(s).\] Since $y_g\in Z$, thus $y_g\upharpoonright S\in Z_S$, we get that $g\cdot y\upharpoonright S\in Z_S$ as well. This shows that $y\in Z$. Now let $x\in X$ be an arbitrary preimage of $y$ under $Q_\PP^\alpha$, i.e. any $x\in X$ satisfying $Q_\PP^\alpha(x)=y$. We claim that $x$ is as desired; that is, for every $g\in G$ \[d(g\cdot x,x_g)<\varepsilon.\] Fix $g\in G$. By definition, $Q_\PP^\alpha(g\cdot x)=g\cdot y$ and we have $g\cdot y(1_G)=y(g^{-1})=y_g(1_G)$. It follows that $g\cdot x$ and $x_g$ lie in the same element of the partition of $\PP$ and since the diameter of this element is less than $\varepsilon$, we are done.
\end{proof}

\begin{lemma}\label{lem:inverseofisolatedshfts}
Let $G$ be a countable group. Suppose that strongly projectively isolated subshifts are dense in $\SH_G(A)$, for every finite $A$ with at least two elements. Then the generic action of $G$, guaranteed by Theorem~\ref{thm:mainRokhlin}, is an inverse limit of subshifts of finite type.
\end{lemma}
\begin{proof}
Fix $G$ satisfying the assumption of the lemma. By Theorem~\ref{thm:mainRokhlin}, $G$ admits a generic action $\alpha$ that is, by the proof of Proposition~\ref{prop:MAINexistence}, an inverse limit of a sequence $(X_n)_{n\in\Nat}$ of projectively isolated subshifts satisfying \eqref{eq:universality} and \eqref{eq:cofinality}. In order to show that $\alpha$ is an inverse limit of subshifts of finite type, it is enough to show that for any clopen partition $\PP$ of $\CC$ there exists a refinement $\PP'\preceq\PP$ such that $\QQ(\alpha,\PP')$ is a subshift of finite type. Fix therefore a clopen partition $\PP$. Then there is a clopen partition $\PP''\preceq\PP$ such that $\QQ(\alpha,\PP'')=X_n$, for some $n\in\Nat$. Since $X_n$ is projectively isolated there exists a basic open neighborhood $\NN$ of some subshift $Y$ and a factor map $\phi$ defined on the whole $\NN$ such that for every $Z\in\NN$ \[\phi[Z]=X_n.\] Since by the assumption, strongly projectively isolated subshifts are dense, there exist a subshift $W\in\NN$, an isolated subshift $V$ and a factor onto map $\phi':V\rightarrow W$. So we have $\phi\circ \phi'[V]=X_n$. $V$ is, in particular, of finite type and projectively isolated. Therefore by \eqref{eq:cofinality}, there exist $m>n$ and $\psi_m^V: X_m\rightarrow V$ such that $\psi_m^n: X_m\rightarrow X_n=\phi\circ\phi'\circ \psi_m^V$. Let $\psi^m$ be the factor map from $\alpha$ onto $X_m$. Then $\psi_m^V\circ \psi^m$ is a factor map from $\alpha$ onto the subshift of finite type $V$ which is by Lemma~\ref{lem:factoringonshift} determined by some clopen partition $\PP'$. Since $\phi\circ\phi'\circ\psi_m^V\circ\psi^m$ is the factor map from $\alpha$ onto $X_n$ which is determined by $\PP''$, we get that $\PP'\preceq\PP''$, therefore $\PP'\preceq\PP$ and we are done.
\end{proof}

The preceding arguments conclude in the following result.
\begin{theorem}\label{thm:genericshadowing}
Let $G$ be a countable group such that strongly projectively isolated subshifts are dense in $\SH_G(A)$, for every finite $A$ with at least two elements. Then the pseudo-orbit tracing property (shadowing) is generic in $\Act_G(\CC)$, the space of Cantor $G$-actions.
\end{theorem}
\begin{proof}
This is an immediate consequence of Proposition~\ref{prop:shadowing} and Lemma~\ref{lem:inverseofisolatedshfts}.
\end{proof}
...
\end{corollary}
Now we reformulate the original definition of pseudo-orbit tracing property to an equivalent definition for zero-dimensional spaces that is more convenient for us as it employs clopen partitions instead of compatible metrics.
...
\begin{theorem}
Let $G$ be a countable group that does not have the strong topological Rokhlin property. Then the set \[\SSS:=\{\alpha\in\Act_G(\CC)\colon \alpha\text{ has shadowing}\}\] is not comeager, i.e. shadowing is not generic for actions of $G$.
\end{theorem}
\begin{proof}
Fix $G$ as in the statement. Suppose that $\SSS$ is comeager. Then it has a non-empty intersection with the set $\AAA$ from the statement of Proposition~\ref{prop:NoSTRPnonsoficfactors}, so there is $\alpha\in \SSS\cap\AAA$. It follows that there is a clopen partition $\PP$ of $\CC$ such that $\QQ(\alpha,\PP)$ is not sofic. That is however in contradiction with Proposition~\ref{prop:shadowingfactorsonsofic}.
\end{proof}
...
\end{question}
The following consists of two questions which the reader familiar with the results of this paper finds closely related.
\begin{question}
...
Does the generic action of every countable group with the STRP have shadowing?
\end{question}
...
\noindent{\bf Acknowledgements.} We would like to thank to Sebasti\' an Barbieri for explaining us the different notions of effective subshifts over general countable groups and to Alexander Kechris for several comments.
