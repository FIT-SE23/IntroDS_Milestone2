...
People often use language to achieve communicative goals~\cite{Wittgenstein1953-philosophical-investigations, %
Austin1975-things-with-words,Clark1996-using-language,frank2012predicting}, %
producing effects on other people and the world. To achieve their goals efficiently, people rely on various types of context to allow their conversational partners to enrich meaning beyond what the speaker has literally said, making language highly context-dependent; a simple sentence such as ``it's nice out today'' can invoke different meanings depending on the situation---it can be an implicit invitation, a statement contrasting the weather with a previous day, or even ironic if the weather is poor.
...
People often use language to achieve communicative goals~\cite{Wittgenstein1953-philosophical-investigations, %
Austin1975-things-with-words,Clark1996-using-language,frank2012predicting}, %
producing effects on other people and the world.
...
We focus on work
that incorporates multimodal context in pragmatic reasoning, motivated by the rich contexts of typical language use---our understanding of language is shaped by the environments that we use it in %
...
Finally, we
suggest further integrations of computational pragmatics with NLP: using pragmatic modeling to allow systems to communicate and interact more successfully and efficiently with people; focusing on developing tasks with rich contexts or contextualizing existing tasks; and tackling challenges of human evaluation and data sparsity. %
...
Theoretical work on the RSA framework has used it to model interpretation and generation of a wide range of pragmatic phenomena~\citep{goodman2016pragmatic,bergen2016pragmatic,kao2014formalizing,kao2014nonliteral}. In practice, most RSA implementations involve speaker agents which sample candidate generations from a conditional language model, and listener agents which execute these generations in the environment \citep{andreas-klein-2016-reasoning}. 
...
\subsection{What Can Pragmatics Give Us?}

We predict that developing pragmatic capabilities will advance the frontier of NLP, and vice versa.
Many of the shortcomings of current large-scale neural models are pragmatic limitations, such as processing implicatures \cite{jeretic-etal-2020-natural,Ruis2022-communicators}, predicting mental states \cite{sap2022neural}, and building common ground with people \cite{benotti-blackburn-2021-grounding}. 
...
We expect pragmatics to be a challenging and productive driver of progress on these problems and on developing more useful NLP models and agents.

Practical benefits of pragmatic inference include more efficient communication and learning. Reasoning about how context enriches meaning (\autoref{sec:types-context}) allows people to say less while still conveying their intentions, and to learn from what could have been said but was not. A long line of work in computational pragmatics explicitly examines this tradeoff between conciseness and communicative success (\autoref{sec:gricean-maxims}); we encourage future work in NLP to similarly evaluate models on communicative efficiency as well as task success (\autoref{sec:evaluation}) and consider approaches that aim to achieve both desiderata (\eg \autoref{sec:rsa}).
...
Pragmatic phenomena that have been understudied in NLP, such as convention formation (\autoref{sec:roles-pragmatics}), emerge when people can rely on shared dialogue, environmental, and task contexts to convey and decode meanings.
...
As discussed in Section~\ref{sec:evaluation}, some models may be evaluated in self-play, and future work might extend evaluation practices from multi-agent reinforcement learning, where methods like population play, imitation learning from human data, and fictitious co-play \citep{NEURIPS2021_797134c3} provide proxies for human interaction.
However, human evaluation remains the gold standard in most interactive settings, and we encourage future work to improve human evaluations and focus on tasks that are intrinsically-motivating (\eg useful or fun) for people (\eg \citealt{wang-etal-2017-naturalizing,meta2022human}).
