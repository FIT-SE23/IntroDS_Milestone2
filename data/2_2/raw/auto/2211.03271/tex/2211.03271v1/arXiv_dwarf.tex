...
Chodas 2004); a contact binary also emerges as the apparently rather common
...
directed --- as already noted --- essentially in the direction normal to the
orbital plane, was in extreme cases nearly comparable to the Sun's gravitational
acceleration.  The computation of the true orbit is time consuming, because the
parameter $A_3$ of the normal component of the nongravitational acceleration had
...
I chose to closely examine the orbital implications of the dependence of the
nominal perihelion latitude, $B_\pi$, on the nominal nodal longitude, $\Omega$,
...
nominal orbits from astrometric positions derived from measurements of all images,
taken with the C2 as well as C3 coronagraphs on board the SOHO spacecraft.  Because
the pixel size of the C3 coronagraph is about five times larger than the pixel
size of the C2 coronagraph, an obvious way to improve the quality of the
...
reproduced from Paper~1, shows that the data points line up along straight lines
%
\begin{equation}
...
for a brand new investigation of the process of cascading fragmentation
...
for segments of Populations~I, Ia, Pe, and Pre-I, with the ordinate scales
...
per second --- affects almost exclusively the period of the new orbit,
the difference compared to the parent's orbit reaching as much as several
hundred years; the aphelion distance does of course change accordingly.
On the other hand, when the breakup takes place near aphelion (say, near
$\sim$160~AU from the Sun), significantly affected are (i)~the angular elements
by the normal component of the separation velocity, shifting by more than
10$^\circ$ in the nodal longitude; and (ii)~the perihelion distance by the
...
period differs from the orbit of its parent by more than several days following
an event occurring at or very close to aphelion; and it is equally impossible
...
event taking place at or very close to perihelion.
...
mass of the fragments are at the time located at slightly different heliocentric
distances (Figure~4) but have the same orbital velocity:\ the fragment farther
from the Sun has to get into a greater orbit, with a longer period.  If the
heliocentric distance of the parent sungrazer at the time of fragmentation is
...
\end{equation}
%
where the orbital periods are in years, while $\Delta U_{{\rm p}\rightarrow{\rm r}}$
...
center of mass at the breakup is 5~km farther from the Sun than the parent's center
...
From the preceding it follows that fragments given birth in breakups of parent
fragments in close proximity of perihelion became members of the same population
...
of the ascending node (a total of 66$^\circ$) and in the other angular elements,
...
perihelion time of the comet --- if it indeed was a Kreutz sungrazer --- could
be estimated at better than $\pm$1~month.
%
...
A and B at the bottom.  Accordingly, the orientation relative to the Sun
alone assures that fragment A ends up in an orbit of a shorter period
and fragment~B in an orbit of a longer period than was the parent in
...
state explicitly that the earthquake was probably in December (or possibly in
late November) 373~BC rather than January or February 373~BC.  If Aristotle's
statement that the comet appeared at the time of the earthquake was meant to
suggest that the two events occurred in the same winter, under the same archon
...
influence of the indirect planetary perturbations.  Though ingenious, his method
...
progenitor of the Kreutz system.  The bullets indicate the sungrazers that
are assumed to move in the same orbit as their immediate parent.  The
sungrazers in the heavily framed boxes reached their first perihelion in
AD~363 as a swarm of daylight comets (Section~4.4).  Their orbits are
approximated as follows:\ Fragment~I by C/1843~D1; Fragment~II by C/1882~R1;
Fragment~Pe by C/1963~R1; Fragment~IIa by C/1970~K1; and Fragment~III by
C/2011~W3.  The orbits for Fragments~Ia, Pre-I, IIa$^\ast\!$ (see Section~5),
IIIa, and IV are described elsewhere in this paper. (Reproduced from
Paper~3.){\vspace{0.9cm}}}
\end{figure*}
% 
...
motion was closely checked, ended up in a slightly changed orbit with a period
...
and Ikeya-Seki --- complete the set of members of the Kreutz system with the
known periods, whose orbital evolution was simulated in Paper~2 starting with
...
object with any known candidate of the Kreutz system in the past.  For other
...
(C/1887~B1), and of course the eclipse comet of 1882 (C/1882~K1) --- all
...
ability to randomly disperse the fragments' orbits in time and space.  I present
...
$\Delta U_{{\rm p} \rightarrow {\rm f}}(r_{\rm frg})$, expressed by Equation~(4)
in Section~3.
...
\end{equation}
%
The size of the surface area is
%
\begin{equation}
...
%
\begin{equation}
\Delta {\cal N} = \zeta \Delta {\cal A} = {\textstyle \frac{1}{3}} \zeta \pi
...
both comet Pereyra and White-Ortiz-Bolelli, thus eliminating Population~IIa.
Aristotle's comet was rejected as the progenitor and there was no serious effort
...
series of populations, whose existence is now established by evidence from the
SOHO sungrazers, is shown to be the result of a stochastic process of cascading
fragmentation rather than ad hoc breakup events and the separation velocities
are kept below 5~m~s$^{-1}$.  The progenitor is now identified with Aristotle's
comet and the SOHO sungrazers are logically presented as the end products of the
...
also profits from the newly determined orbit of the 1843 sungrazer (Sekanina \&
Chodas 2008), which, unlike Kreutz's (1901) orbit, includes the planetary
perturbations and the relativistic effect.
...
but it is not.  One example is the case of the daylight comets in AD~363.
...
Kreutz sungrazers based solely on the circumstances of observation.  Not
to mention that this {\it virtually unique event\/} was highly significant
...
which was still missing when I wrote Paper~1!  It was really on a hunch that
a novel method was developed in Paper~2, which was, against all odds, successful
...
the computed date in late 1139 agreed, within the uncertainty of $\pm$2~years, with
...
from Aristotle's comet at perihelion in 372~BC, in which case its orbital
elements would have been essentially identical, except for the (barycentric)
period of 838~yr, compared to Aristotle comet's (osculating) period of 789~yr
(Table~3).
...
20~percent.  Both models predict that more bright, naked-eye sungrazers
...
of these objects were lower, closer to the solar radius than their nominal values.
...
a transverse component of the separation velocity of more than 5~m~s$^{-1}$
and its origin is not clear; it suggests that the fragmentation process was
still more complex than described by the contact-binary model.
...
information on the meaning of the nominal nodal longitudes and supplement the
results offered by the $q(\Omega)$ relation.  For Population~I the log-log
...
Population~I was dominated by sublimation effects, with only a minor contribution
...
suggesting limited sublimation.
...
a perihelion distance of 1.06~{\Rsun}.  Compared to{\vspace{-0.04cm}}
...
large $\Delta \Omega$s the formula (13), the curve of Population~Ia is
...
%
\begin{equation}
d{\cal N}_{\Delta \Omega}({\rm Ia}) \sim -C_0 \, {\cal N}_{\Delta \Omega}({\rm Ia})
  \, d(\Delta \Omega), % (14)
\end{equation}
%
or
...
is remarkable to see in Figure~12 the crowded area of the nominal nodal
longitudes near $\Omega_0$ on the one hand and the utter deficit of the
SOHO sungrazers with the nominal perihelion distances near the model's
...
from the parent's value.  The exception seems to be a class of fragments
...
In summary, it appears that the relations and distributions involving the
nominal perihelion distances of the SOHO dwarf sungrazers suggest that
...
orbits are surprising only in the populations deriving from Lobe~II,
...
mode.  On the other hand, the complicated relationship between the nominal
and true values of perihelion distance may lead to a conclusion that the
amount of information conveyed by the distribution of the nominal perihelion
distances may be extremely limited and should not be overrated.
%
...
reader-friendly, but it also has been necessitated by stepwise additions
of new information in between the three papers.  A striking example is
the derived birth place of the Great September Comet of 1882 and comet
Ikeya-Seki, an issue that was not tackled until Paper~2, which happened
...
unexpectedly{\vspace{-0.02cm}} below 1.2~{\Rsun} and way below the
...
a narrower neck, which eons ago merged into a single mass at a very low
relative velocity.

Whereas the data analysis dictated the narrative of Papers~1 to 3 to
...
substantial masses, this summary offers a description of the sequence of
...
distance --- an absolutely essential condition ensuring that a very
modest separation velocity should suffice to open up a sizable gap
...
to break apart, thus generating precursors of the nine populations,
whose existence was established in Paper~1.  The most massive surviving
...
IIIa, and IV.  The surviving mass of the neck separated sooner or
later from one of the two lobes (probably Lobe~I) to become the
...
the motions of the SOHO dwarf sungrazers that was just barely touched in
Papers~1 to 3.  Broadly, this problem concerns their gravitational orbits
derived by Marsden, which, useless when unprocessed, contain important
information that has allowed the detection of the nine populations discriminated
in terms of the longitude of the ascending node.  More specifically, the
unsettled problem involves the gravitational orbits' perihelion distances,
which were deemed extremely poorly determined (and published to merely two
decimals) by Marsden, but of potentially diagnostic value for continuing
classification efforts.  A limited study of this kind in Paper~1 bore
only on Population~IIa, leading to a possible further expansion of the
fragmentation process beyond the presented scenario of the contact-binary
model.  The extended investigation of the distribution of the nominal
perihelion distances and their relation to the nominal nodal longitudes
for a select subset of the SOHO dwarf sungrazers, presented in this paper
for the first time, does not provide substantially more information about
the subject beyond the acknowledgment of an essentially universal peak on
the perihelion-distance distribution at about 1.1~{\Rsun}, interpreted as
a potential preference for fragments released with a negative transverse
component of the separation velocity.  Considering Marsden's skepticism
about the poor quality of his determination of the SOHO sungrazers'
perihelion distances, one cannot rule out that these data provide only
...
Katsonopoulou, D. (ed.) 2017, Helike V:\ Ancient Helike and Aigia-{\linebreak}
 {\hspace*{-0.6cm}}leia.  Athens:\ Helike Society, 310\,pp
\\[-0.57cm]
%
% \item[\hspace{-0.3cm}]
%  Kelly, G. 2008, Ammianus Marcellinus:\ The Allusive Historian.  Cambridge, U.K.:\
%  Cambridge University Press. 378\,pp
% \\[-0.57cm]
%
% \item[\hspace{-0.3cm}]
...
%  {\hspace*{-0.6cm}}Cambridge, UK:\ Cambridge University Press, 580\,pp
% \\[-0.57cm]
%
% \item[\hspace{-0.3cm}]
% Kronk, G. W. 2004, Cometography:\ Volume 2 (1800--1899).  Cam-{\linebreak}
%  {\hspace*{-0.6cm}}bridge, UK:\ Cambridge University Press, 852\,pp
% \\[-0.57cm]
%
\item[\hspace{-0.3cm}]
...
Matthews, J. 2008, The Roman Empire of Ammianus.  Ann Arbor:{\linebreak}
 {\hspace*{-0.6cm}}Michigan Classical Press, 608\,pp
\\[-0.57cm]
%
\item[\hspace{-0.3cm}]
...
%
\item[\hspace{-0.3cm}]
Sekanina, Z., \& Chodas, P. W. 2012, ApJ, 757, 127 (33\,pp)
\\[-0.57cm]
%
\item[\hspace{-0.3cm}]
Sekanina, Z., \& Kracht, R. 2015, ApJ, 801, 135 (19\,pp)
\\[-0.57cm]
%
\item[\hspace{-0.3cm}]
