...
\begin{document}
	...
	\begin{abstract}
		...
		%The triangular coupled-top model admits both the global $Z_2$ symmetry and the translational symmetry.
	\end{abstract}
	...
	Quantum phase transitions are usually accompanied by a qualitative change in the nature of the ground state when the control parameter of a system passes through the quantum critical point. In contrast to its classical counterpart driven by thermal fluctuations, the quantum phase transition occurs at zero temperature due to the quantum fluctuations, where the non-commuting terms in the system Hamiltonian play a dominant role \cite{sachdev_2011}. The geometric frustration, which prohibits simultaneously satisfying every pairwise interaction due to local geometric constraints, also influences the quantum phase transition significantly \cite{Toulouse1977,Balents2010}. Frustration is closely related with exotic materials such as quantum spin liquid and spin ice \cite{Balents2010,RevModPhys.89.025003,doi:10.1126/science.1064761}, as well as complex networks \cite{RevModPhys.80.1275} and protein folding \cite{doi:10.1073/pnas.84.21.7524}, etc. The unconventional quantum critical phenomena and  qualitatively new states of matter induced by the frustration are still an active area of research \cite{Lee2002-cy,doi:10.1126/sciadv.1500001,PhysRevLett.127.063602,PhysRevLett.129.183602,PhysRevLett.128.163601,doi:10.1146/annurev.ms.24.080194.002321,doi:10.1139/p97-007}.
	
	As a paradigmatic system to study quantum phase transitions, the transverse-field Ising model has drawn persistent attention for over half a century \cite{DEGENNES1963132,BROUT1966507,PFEUTY197079,Stinchcombe_1973,sachdev_2011,Suzuki2012-ro,PhysRevLett.95.245701}. Besides, the transverse-field Ising model with antiferromagnetic interaction on a two-dimensional triangular lattice offers itself as an ideal platform to study the frustrated magnetic behaviors \cite{PhysRevB.63.224401,Kim2010-au}. A nature generalization, named coupled-top model, is to replace the widely used spin-$1/2$s in the transverse-field Ising model with large spins \cite{PhysRevA.71.042303,PhysRevE.102.020101,PhysRevE.104.024217,PhysRevE.104.034119,PhysRevE.105.014130}. %Previous studies on the coupled-top models mainly focused on the behaviors of two interacting large spins whose geometric structure is negligible. %, such as quantum-classical correspondence \cite{PhysRevA.63.052103}, quantum entanglement \cite{PhysRevA.71.042303}, chaos and quantum scars \cite{PhysRevE.60.1542,PhysRevE.104.024217,PhysRevE.105.014130}, excited-state phase transition \cite{PhysRevE.104.034119}, etc. 
	The coupled-top model shares some similarities with the Dicke model, a fundamental model describing the interaction between the light and matter \cite{PhysRevE.67.066203,PhysRevLett.90.044101}. (1) Both of them exist the quantum phase transition: the global $Z_2$ symmetry is spontaneously broken with increasing coupling strength, and the von Neumann entropy is diverged near the quantum critical point % accompanied by the divergence of the entanglement entropy 
	\cite{PhysRevA.71.042303,PhysRevLett.92.073602}. (2) The level statistics changes from Poissonian distribution to Wigner-Dyson distribution, which indicates a transition from quasi-integrability to quantum chaotic \cite{PhysRevE.102.020101,PhysRevE.67.066203}. (3) The density of states shows a singular behavior which corresponds to the excited-state quantum phase transitions \cite{PhysRevE.88.032133,PhysRevE.104.034119}. Recently, the light-matter interacting systems in a triangular structure have intrigued an enormous amount of attention due to the exotic behaviors, such as chiral superradiant phase \cite{PhysRevLett.127.063602,PhysRevLett.129.183602}, frustrated superradiant phase and novel critical exponent \cite{PhysRevLett.128.163601}. We expect that the triangular coupled-top model also exists rich quantum phases and unconventional quantum critical phenomena. 
	
	In this paper, we study the quantum phase transition in the triangular coupled-top model. Three large spins are located on a triangle. Each pairwise interaction can be ferromagnetic or antiferromagnetic. First, we introduce the mean-field approach which has been widely employed to study the large spin systems especially in the thermodynamic limit \cite{Lieb1973-sw,PhysRevA.100.063820}. The mean-field approach is equivalent to using a product state constructed by SU(2) coherent states  as the trial wavefunction. The ground state is achieved by minimizing the corresponding energy expectation value. With the mean-field approach, we confirm the existence of the quantum phase transition by the nonzero order parameter, which indicates the breaking of global $Z_2$ symmetry. Two critical coupling strengths separate the coupled-top model into three phases: disordered paramagnetic phase, ferromagnetic phase, and frustrated antiferromagnetic phase. Then, we introduce the Holstein-Primakoff transformation and symplectic transformation in order to capture the quantum fluctuation and quantum entanglement which are ignored by the mean-field approach. The Holstein-Primakoff transformation maps the collective spin operators into bosonic ones, which leads to an effective quadratic Hamiltonian in the thermodynamic limit. The covariance matrix of the effective quadratic Hamiltonian can be obtained by the symplectic transformation, with which we can analyze the behaviors of the quantum fluctuation and von Neumann entropy. We find that the geometric frustration comes into play in the antiferromagnetic phase, which leads to novel critical behaviors.
	...
	\end{eqnarray}
	which corresponds to a point on the Bloch sphere. Note that the external field creates an energetic bias for one $z$-axis spin direction over the other one, whereas the coupling strength leads to the alignment or anti-alignment of spin projections along the $x$ axis. Therefore, the large spins tend to be located on the $x$-$z$ plane which leads to $\phi_i = 0$ or $\pi$. Without loss of generality, we can introduce $\tilde{\theta}_i$,
	\begin{eqnarray}
	...
	\begin{eqnarray} \label{eq:E_GS}
		...
		%&=& \frac{1}{J} \sum_i^N \left(-\epsilon J_i^z + \frac{\chi}{J} J_i^x J_{i + 1}^x\right) .
	\end{eqnarray}
	...
	\end{eqnarray}
	%Here we introduce a dimensionless coupling strength $\lambda = \chi / \epsilon$. 
	By carefully analyzing the energy expectation value, we find two critical coupling strengths $\lambda_- = -\frac{1}{2}$ and $\lambda_+ = 1$, which separate the coupled-top model into three phases: disordered paramagnetic phase, ferromagnetic phase, and frustrated antiferromagnetic frustrated phase, as shown in Fig. \ref{fig:bloch}.%, depending on the choice of $\lambda$. $\lambda = \chi / \epsilon$ is the dimensionless coupling strength.
	...
	\end{eqnarray}
	and it is two-fold degenerate with the order parameter satisfying $J_i^x = \pm \sqrt{1 - \frac{1}{4\lambda^2}}$ respectively. We only show the positive branch of $J_i^x$ in Fig. \ref{fig:jx}. The critical exponent associated with the order parameter is $\beta = \frac{1}{2}$ due to $J_i^x \propto \left(\lambda_- - \lambda\right)^{1/2}$ near the quantum critical point ($\lambda \rightarrow \lambda_-$). The global $Z_2$ symmetry is broken in the ferromagnetic phase, whereas the translational symmetry is preserved.
	...
	To separate the mean-field contribution and the higher-order terms, we first perform a unitary transformation \cite{PhysRevB.71.224420,PhysRevLett.128.163601,PhysRevA.104.052423}, which leads to %an effective Hamiltonian,
	\begin{widetext}
		\begin{eqnarray} \label{eq:H_R}
			\hat{\bar{H}} &=& \hat{U}^{\dagger} \hat{H} \hat{U} = \sum_i^N - \left(\cos \tilde{\theta}_i \hat{J}_i^z + \sin \tilde{\theta}_i \hat{J}_i^x\right) +  \frac{\lambda}{J} \left(\cos \tilde{\theta}_i \hat{J}_i^x - \sin \tilde{\theta}_i \hat{J}_i^z\right) \left(\cos \tilde{\theta}_{i + 1} \hat{J}_{i + 1}^x - \sin \tilde{\theta}_{i + 1} \hat{J}_{i + 1}^z\right) , 
		\end{eqnarray}
	\end{widetext}
	with 
	\begin{eqnarray}
	...
	\end{eqnarray}
	%\end{subequations}
	%Both $J_i^z$ and $J_i^x$ are nonzero in the ordered phase in the original framework, whereas the large spins only have $z$ component after the unitary transformation in the rotated framework.
	The unitary transformation $\hat{U}$ % of the spin operators around the $y$ axis
	...
	\begin{subequations}\label{eq:HP}
		\begin{eqnarray} 
			\hat{J}_i^z &\approx& J - \hat{a}_i^{\dagger} \hat{a}_i,\\
	...
	\begin{eqnarray}
		...
		\hat{\bar{H}}_1 &=& - \frac{1}{\sqrt{2}}  \left( \sin \tilde{\theta}_i + \lambda \cos \tilde{\theta}_{i} \left(\sin \tilde{\theta}_{i + 1} + \sin \tilde{\theta}_{i - 1}\right) \right) \nonumber\\
	...
	\begin{eqnarray}
		...
		\bar{\chi}_{i,j} &=& \lambda \cos \tilde{\theta}_i \cos \tilde{\theta}_{j} = \chi_{j,i},
	\end{eqnarray}
	\end{subequations}
	%\begin{widetext}
	%	\begin{eqnarray}
	%		\hat{\bar{H}}_1 &=& - \frac{1}{\sqrt{2}} \left(\epsilon \sin \tilde{\theta}_i + \chi \cos \tilde{\theta}_{i} \left(\sin \tilde{\theta}_{i + 1} + \sin \tilde{\theta}_{i - 1}\right) \right) \left(\hat{a}_i^{\dagger} + \hat{a}_i\right), \label{eq:H1}\\
	%		\hat{\bar{H}}_2 &=& \sum_i^N \left(\epsilon \cos \tilde{\theta}_i - \chi \sin \tilde{\theta}_i \left(\sin \tilde{\theta}_{i - 1} + \sin \tilde{\theta}_{i + 1}\right)\right) \hat{a}_i^{\dagger} \hat{a}_i + \frac{\chi}{2} \cos \tilde{\theta}_i \cos \tilde{\theta}_{i + 1} \left(\hat{a}_i^{\dagger} + \hat{a}_i\right) \left(\hat{a}_{i + 1}^{\dagger} + \hat{a}_{i + 1}\right) , \label{eq:H2}
	%	\end{eqnarray}
	%\end{widetext}
	where we have ignored terms of order $J^{-l}$ with $l > 0$.
	The first term in Eq. (\ref{eq:H_eff}) corresponds to the energy expectation value (\ref{eq:E_GS}) given by the mean-field approach. Minimizing the energy expectation value gives rise to the ground-state energy, which also eliminates $\hat{\bar{H}}_1$, as indicated by Eqs. (\ref{eq:E_min}) and (\ref{eq:H1}). Finally, we only need to deal with the quadratic Hamiltonian $\hat{\bar{H}}_2$ which can be solved exactly by the symplectic transformation \cite{Serafini2017-sk}.%, as given in Appendix \ref{sec:covariance}.
	
	Before performing the symplectic transformation, we first introduce the vector of canonical operators $\hat{\mathbf{r}} = \left(\hat{x}_1, \hat{x}_2, \hat{x}_3, \hat{p}_1, \hat{p}_2, \hat{p}_3\right)^T$ with  $\hat{x}_i = \left(\hat{a}_i^{\dagger} + \hat{a}_i\right) / \sqrt{2}$ and $\hat{p}_i = \rmi \left(\hat{a}_i^{\dagger} - \hat{a}_i\right) / \sqrt{2}$. $\hat{\mathbf{r}}$ satisfies the canonical commutation relation $\left[\hat{\mathbf{r}}, \hat{\mathbf{r}}^T\right] = i \Omega$, where $\Omega$ is given by
	\begin{eqnarray}
	...
	%The ground state in the new frame is the vacuum state, whereas it is a typical Gaussian state $\ket{\psi} = \exp \left(\right)$
	...
	Instead of dealing with infinite dimension of the associated Hilbert space, one only needs to deal with the $6 \times 6$ covariance matrix $\sigma$ which is fulfilled to give the complete description of an arbitrary Gaussian state (up to local unitary operations) \cite{Serafini2017-sk,Adesso_2007}. The covariance matrix $\mathbf{\sigma}$ is defined as
	\begin{eqnarray}
	...
	%	\sigma = \left(\begin{array}{lll}
		%			\sigma^{(1)} & c_{1,2} & c_{1,3} \\
		%			c_{2,1} & \sigma^{(2)} & c_{2,3} \\
		%			c_{3,1} & c_{3,2} & \sigma^{(3)} 
		%		\end{array}\right)
	%\end{eqnarray}
	%with
	%\begin{widetext}
	%	\begin{eqnarray}
		%		\sigma^{(i)} &=& \left(\begin{array}{cc}
			%			2 \left(\expect{\hat{x}_i^2} - \expect{\hat{x}_i}^2\right) & \expect{\hat{x}_i \hat{p}_i + \hat{p}_i \hat{x}_i} - 2 \expect{\hat{x}_i} \expect{\hat{p}_i} \\
			%			\expect{\hat{x}_i \hat{p}_i + \hat{p}_i \hat{x}_i} - 2 \expect{\hat{x}_i} \expect{\hat{p}_i} & 2 \left(\expect{\hat{p}_i^2} - \expect{\hat{p}_i}^2\right)
			%		\end{array}\right) ,\\
		%		c^{(i,j)} &=& \left(\begin{array}{cc}
			%			2\left(\expect{\hat{x}_i \hat{x}_j} - \expect{\hat{x}_i} \expect{\hat{x}_j}\right) & 2\left(\expect{\hat{x}_i \hat{p}_j} - \expect{\hat{x}_i} \expect{\hat{p}_j}\right) \\
			%			2\left(\expect{\hat{p}_i \hat{x}_j} - \expect{\hat{p}_i} \expect{\hat{x}_j}\right) & 2\left(\expect{\hat{p}_i \hat{p}_j} - \expect{\hat{p}_i} \expect{\hat{p}_j}\right) 
			%		\end{array}\right) ,
		%	\end{eqnarray}
	%\end{widetext}
	...
	\end{eqnarray}
	\end{subequations}
	The quantum fluctuations in $x_i$ and $p_i$ quadrature are shown in Fig. \ref{fig:dx_dp_se} (a) and (b) respectively. Near the quantum critical point $\lambda_{\pm}$, the quantum fluctuation in $x_i$ quadrature tends to exponential diverge, whereas that in $p_i$ quadrature tends to a finite value. Furthermore, $\left(\Delta p_i\right)^2 < \frac{1}{2}$ indicates a strong squeezing effect especially near the quantum critical point \cite{scully_zubairy_1997}.
	...
	Detailed analytical exact results for the excitation energy, quantum fluctuation and von Neumann entropy, especially near the quantum critical point in different phases, are given in the follows.
	...
	The lowest excitation energy corresponds to $\Delta_{\text{min}} = \Delta_1$ for $\lambda_- < \lambda < 0$ and $\Delta_{\text{min}} = \Delta_2 = \Delta_3$ for $0 < \lambda < \lambda_+$. Therefore, the critical behavior associated with the excitation energy is $\Delta_{\text{min}} \propto \left(\lambda - \lambda_-\right)^{1/2}$ for $\lambda \rightarrow \lambda_-$, whereas it is $\Delta_{\text{min}} \propto \left(\lambda_+ - \lambda\right)^{1/2}$ for $\lambda \rightarrow \lambda_+$, as shown in Fig. \ref{fig:critical} (a) and (d). 
	...
	\end{eqnarray}
	with $C_i$ a constant independent on the coupling strength,
	which explains the divergence in $\mathcal{S}_i$ as shown in Fig. \ref{fig:critical} (c) and (f).
	...
	\begin{acknowledgments}
		Y.-Z. W. is supported by the National Natural Science Foundation of China under Grant No. 12105001 and Natural Science Foundation of Anhui Province under Grant No. 2108085QA24. Q.-H. C. is supported by the National Science Foundation of China under Grant No. 11834005.
	\end{acknowledgments}
	...
	\begin{thebibliography}{43}%
		...
		{\bibfield  {journal} {\bibinfo  {journal} {Annual Review of Materials
					Science}\ }\textbf {\bibinfo {volume} {24}},\ \bibinfo {pages} {453}
		...
		{\bibfield  {journal} {\bibinfo  {journal} {Journal of Physics C: Solid State
					Physics}\ }\textbf {\bibinfo {volume} {6}},\ \bibinfo {pages} {2459}
