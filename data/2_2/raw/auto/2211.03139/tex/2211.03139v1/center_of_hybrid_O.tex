...
The hybrid quantum group $U^\hb_\zeta$ was firstly introduced by Gaitsgory \cite{Gai18} with the perspective of generalizing the Kazhdan-Lusztig equivalence, and was used in \cite{BBASV} to study the center of the small quantum group. 
...
In this paper, we establish an isomorphism between the center of $\sO$ and the cohomology ring of $\zeta$-fixed locus on affine Grassmannian. 
...
\begin{thmx}[Proposition \ref{prop 4.12}, Theorems \ref{thm 5.10} and \ref{thm 5.17}] \label{thm A} 
...
In this case, $\omega=-\rho$ is the maximal parahoric type. 
...
Recall that for Lusztig's quantum group $U_\zeta$, we have the quantum Frobenius map  $U_\zeta\twoheadrightarrow U_1$, where $U_1$ is the enveloping algebra of $\g=\Lie(G)$. 
...
\end{equation} 
We further show that (\ref{equ 1.0}) is an isomorphism and coincides with the map $\overline{\bb}_{-\rho}$, whose proof relies on a deformed version of the constructions above. 
...
Recall in the classical case, computation of the center of the category $\sO^0(\g)$ (the principal block of the category $\sO$ for $\g$) follows from the three steps: 
\begin{enumerate}
\item[(a)] construct an algebra homomorphism $H^\bullet(\check{G}/\check{B})\rightarrow Z(\sO^0(\g))$; 
\item[(b)] consider the anti-dominant projective module $Q$, and show the composition 
\begin{equation}\label{equ 1.a} 
H^\bullet(\check{G}/\check{B})\rightarrow Z(\sO^0(\g))\rightarrow \End(Q)
\end{equation}
is an isomorphism; 
...
Let $G$ be a connected complex reductive group, with a Borel subgroup $B$ and a maximal torus $T$ contained in $B$. 
...
There is an order $\leq$ on $\Lambda$ given by $\lambda\leq \mu$ if $\lambda-\mu\in \rQ_{\leq}:=\Z_{\leq}\Sigma$. 
...
Let $T'=\check{T}\times \Gm$, $\check{T}$, $\Gm$ or the trivial group, the cohomology $H^\bullet_{T'}(\Fl^J)$ is freely generated as a $H^\bullet_{T'}(\pt)$-module by the fundamental classes $[\Fl^{J,x}]_{T'}$ of the finite codimensional Schubert varieties labelled by $x\in W^J_\ex$. 
...
and satisfy further conditions as in \cite[\textsection 2.3]{Situ1}. 
...
We define $\sO_{1,R}$ to be the full subcategory of $U^{\hb}_1\Mod^{l\Lambda}_R$ consisting of the modules that are finitely generated over $U^{\hb}_1\otimes R$ and are locally unilpotent under the action of $U\n$. 
...
The first one in Proposition \ref{prop 4.12} is geometric, obtained from the equivalence in Theorem \ref{thm ABG}. 
...
The following equivalence $\Phi$ is proved in {\cite[Thm 9.4.3]{ABG04}}, see also {\cite[Thm 1.4, Prop 6.5]{MR18}} for the deformation equivalence $\Psi$ and their compatibility in characteristic $p$. 
\begin{thm}[{\cite{ABG04}}, {\cite{MR18}}]\label{thm ABG} 
There are compatible equivalences of triangulate categories 
$$\begin{tikzcd} D^b\Coh^{B\times \C^*}(\b) \arrow[r,"\simeq"',"\Psi"] \arrow[d,"Li^*"'] & D^{b,\mx}_\sI(\Gr) \arrow[d,"\for"] \\ 
D^b\Coh^{B\times \C^*}(\n) \arrow[r,"\simeq"',"\Phi"] & D^{b,\mx}_{(\sI)}(\Gr) ,
\end{tikzcd}$$ 
such that $\Phi \circ \langle 1 \rangle=  \langle 1 \rangle [1] \circ \Phi$ and $\Psi \circ \langle 1 \rangle=  \langle 1 \rangle [1] \circ \Psi$. 
Moreover, $\Psi(\sO_\b\otimes \k_\lambda)=\nabla_\lambda$ and $\Psi(\sO_\n\otimes \k_\lambda)=\nabla_\lambda$ for any $\lambda\in \Lambda^{++}$. 
\end{thm} 
...
\end{equation} 
Consider a chain of algebra homomorphisms (see the notations in Appendix \ref{app A}) 
\begin{equation}\label{equ 4.12} 
Z^\bullet(\Coh^{B\times \C^*}(\b)) \hookrightarrow Z^\bullet(D^b\Coh^{B\times \C^*}(\b))
\xrightarrow[\sim]{\text{Thm \ref{thm ABG}}} 
Z_{\sI}^{\pur}(\Gr) \xs H_{\check{T}}^\bullet(\Gr)^{\wedge}, 
\end{equation} 
where the second isomorphism is by the equivalence $\Psi$ in Theorem \ref{thm ABG} which exchanges $\langle 1 \rangle$ and $\langle 1 \rangle[1]$, and the last one follows from Lemma \ref{lem A.3} and the fact that the cohomology of $\Gr$ is pure. 
...
is an injection. 
Indeed, by Theorem \ref{thm ABG} the equivalence $\Psi$ sends $\sO_\b\otimes \k_\lambda$ to the standard sheaf $\nabla_\lambda$ for any $\lambda\in \Lambda^{++}$. 
...
The composition 
$$H_{\check{T}}^\bullet(\Gr)^{\wedge}\xleftarrow[\sim]{(\ref{equ 4.13})} Z(\Coh^{B}(\b)) \xrightarrow{-\otimes_{S'} S} Z(\Coh^B(\n\times \t_{\hat{0}})) 
\xlongequal{\text{Cor \ref{cor 4.8}}} Z(\sO^{[-\rho]}_{S})$$ 
coincides with the map $\bb: H_{\check{T}}^\bullet(\Gr)^{\wedge} \rightarrow Z(\sO^{[-\rho]}_{S})$. 
\end{prop}
\begin{proof} 
By Theorem \ref{thm 3.11}, there is an isomorphism $\bb: H_{\check{T}}^\bullet(\Gr)^\wedge_S \xs Z(\sO^{[-\rho]}_{S})$, 
hence the (partial) restriction 
...
$$\begin{tikzcd}
H_{\check{T}}^\bullet(\Gr)^{\wedge} \arrow[r,hook,"\bb"] \arrow[dr,"\simeq","(\ref{equ 4.13})"'] & Z(\sO^{[-\rho]}_{S}) 
\arrow[r,hook,"\chi_{S}"] & \prod_{\lambda\in \Lambda^{++}}\End_{\sO_{S}}( M(-\rho+l\lambda)_S )= \prod_{\lambda\in \Lambda^{++}}S \\ 
 ...
 \end{tikzcd}$$
Note that the composition $H_{\check{T}}^\bullet(\Gr)^{\wedge}\rightarrow \prod_{\lambda\in \Lambda^{++}}S$ in two ways coincide, since they are both by restrictions on the $\check{T}$-fixed points. 
...
\begin{equation}\label{equ 4.14}
	Z(\Coh^{B}(\n)) \xs Z^\bullet(\Coh^{B\times \C^*}(\n)) \hookrightarrow Z^\bullet(D^b\Coh^{B\times \C^*}(\n)) \xrightarrow[\sim]{\text{Thm \ref{thm ABG}}} Z_{(\sI)}^{\pur}(\Gr) \xs H^\bullet(\Gr)^{\wedge} . 
\end{equation} 

\begin{prop}\label{prop 4.12} 
The composition 
$$H^\bullet(\Gr)^{\wedge} \xrightarrow{\overline{\bb}} Z(\sO^{[-\rho]}_{\k}) \xlongequal{\text{Cor \ref{cor 4.8}}} Z(\Coh^{B}(\n))  \xrightarrow{(\ref{equ 4.14})} H^\bullet(\Gr)^{\wedge}$$ 
is the identity map. 
...
\begin{equation}\label{equ 4.16} 
    \begin{tikzcd}
    Z(\Coh^{B}(\b)) \arrow[r,"\simeq"] \arrow[d,"i^*"] & Z^\bullet(\Coh^{B\times \C^*}(\b)) \arrow[d,"i^*"] \arrow[r,"\simeq"] 
    & Z_{\sI}^{\pur}(\Gr) \arrow[r,"\simeq"] & H_{\check{T}}^\bullet(\Gr)^{\wedge} \arrow[d] \\ 
    Z(\Coh^{B}(\n)) \arrow[r,"\simeq"] & Z^\bullet(\Coh^{B\times \C^*}(\n)) \arrow[r,"\simeq"] & 
    Z_{(\sI)}^{\pur}(\Gr) \arrow[r,"\simeq"] & H^\bullet(\Gr)^{\wedge} ,\end{tikzcd}
    \end{equation}
where the left two vertical maps are obtained by base changes (\ref{equ 4.10}), and the left square naturally commutes. 
We show that the diagram (\ref{equ 4.16}) commutes. 
Indeed, for any $Q\in \sP^{B,\leq \nu}_{S'}$ and a lifting of it in $\Coh^{B\times \C^*}(\b)$, still denoted by $Q$, there is commutative diagram 
$$\begin{tikzcd}
...
$$\begin{tikzcd}
    H_{\check{T}}^\bullet(\Gr)^{\wedge} \arrow[r]\arrow[d]  &\bigoplus_{d} \Hom_{D^{b,\mx}_{\sI}(\Gr)}(\Psi(Q),\Psi(Q)\langle d \rangle [d]) \arrow[d,"\for"] \\ 
    H^\bullet(\Gr)^{\wedge} \arrow[r] &\bigoplus_{d} \Hom_{D^{b,\mx}_{(\sI)}(\Gr)}(\Phi(Li^*Q),\Phi(Li^*Q)\langle d \rangle [d]) .
\end{tikzcd}$$
Any element in $Z(\Coh^{B}(\b))$, resp. in $Z(\Coh^{B}(\n))$, is determined by its restriction to the full subcategories $\sP^{B,\leq \nu}_{S'}$, resp. $\sP^{B,\leq \nu}_{\k}$, $\nu\in \Lambda$. 
It follows that the right rectangle of (\ref{equ 4.16}) commutes. 

So we have a commutative diagram, 
$$\begin{tikzcd}
H_{\check{T}}^\bullet(\Gr)^{\wedge} \arrow[rd,"\bb"] \arrow[dd,two heads] \arrow[r,"(\ref{equ 4.13})^{-1}","\simeq"']
& Z(\Coh^{B}(\b)) \arrow[d,"-\otimes_{S'}S"] \arrow[r,"(\ref{equ 4.13})","\simeq"'] 
& H_{\check{T}}^\bullet(\Gr)^{\wedge} \arrow[dd,two heads] \\ 
& Z(\sO^{[-\rho]}_{S})=Z(\Coh^{B}(\n\times \t_{\hat{0}})) \arrow[d,"-\otimes_{S'}\k"] & \\ 
H^\bullet(\Gr)^{\wedge} \arrow[r,"\overline{\bb}"] 
& Z(\sO^{[-\rho]}_{\k})=Z(\Coh^{B}(\n)) \arrow[r,"(\ref{equ 4.14})"] 
& H^\bullet(\Gr)^{\wedge}, 
 \end{tikzcd}$$
where the commutativity of the triangle is given by Proposition \ref{prop 4.11}. 
Since the the upper horizontal composition is the identity map, so is the lower one. 
\end{proof} 
...
\begin{rmk} 
The two descriptions (\ref{equ 4.26}) and (\ref{equ 3.25}) admit the following compatibility. 
Let $x\in \n\simeq N$ be a regular element, and denote by $\g^x$ the centralizer of $x$ in $\g$. 
Recall by \cite[Prop 1.7.2]{Gin95}, there is an algebra isomorphism $H^\bullet(\Gr^\circ)\simeq U\g^x$. 
The evaluation on $x$ gives a linear map $\ev_x:\k[N]\rtimes U\n\rightarrow U\n$. 
There is a commutative diagram 
$$\begin{tikzcd}
Z(\sO^{-\rho}_{\k})\arrow[r,"\simeq"]\arrow[d,"\simeq"'] & Z\big((\widehat{\k[N]\rtimes U\n})_0\big)\arrow[d,"\ev_x"] \\ 
H^\bullet(\Gr^\circ)^\wedge \arrow[r,"\simeq"] & (U\g^x)^\wedge,
\end{tikzcd}$$
where $(U\g^x)^\wedge$ is the degree completion of $U\g^x$. 
\end{rmk}
...
\item Let $\omega\in \Xi_\sc$. There is a natural isomorphism 
...
By the strong linkage principle, $(Q(w_0\bullet 0)_R^{\leq 0}:M(x\bullet 0)_R)\geq 1$ for each $x\in W$. 
It forces that $T^0_{-\rho}(M(-\rho)_R)=Q(w_0\bullet 0)_R^{\leq 0}$. 

Part (2) is proved by a standard argument, as in the case of the principal block of the category $\sO$ for $U\g$, see \cite[Thm 3.6]{Fie03}. 
\end{proof} 
...
Recall the natural isomorphism $\Theta:\id^{\oplus|W|}\xs T^{-\rho}_0T_{-\rho}^0$ in Lemma \ref{lem 5.0}(3). 
...
\Hom(Q_R,T_0^{-\rho}T^0_{-\rho}Q_R)\xrightarrow[\sim]{\Theta_{Q_R,*}^{-1}} \Hom(Q_R,Q_R^{\oplus |W|}) 
\end{equation} 
...
Note that 
$$\Theta^{-1}_{M_R}\circ \adj(\tau^{\leq -\rho}(\iota_w))=\Theta^{-1}_{M_R}\circ \tau^{\leq -\rho}(\adj(\iota_w))=\tau^{\leq -\rho}(\Theta^{-1}_{Q_R}\circ \adj(\iota_w))$$ 
...
Let $Q\in \sO^{0}_{\k}$ be a module admitting a Verma filtration, then 
\begin{enumerate} 
\item $\eta_Q: Q\rightarrow T_{-\rho}^{0}T^{-\rho}_{0}Q$ is injective; 
\item $T_0^{-\rho}: \End(Q) \rightarrow \End(T_0^{-\rho}Q)$ is an injection. 
\end{enumerate} 
\end{lem} 
\begin{proof} 
(1) By 5-lemma it is enough to show the case when $Q$ is a Verma module. 
Now $Q$ and $T_{-\rho}^{0}T^{-\rho}_{0}Q$ are free $\fU_\zeta^-$-modules, and $Q$ is of rank one. 
Since $\fU_\zeta^-$ is a torsion-free algebra, and $\eta_Q$ is nonzero, it must be an injection. 

(2) For any $f\in \End(Q)$, we have $\eta_Q\circ f=T_{-\rho}^{0}T^{-\rho}_{0}(f)\circ \eta_Q$. 
By (1), if $T_{-\rho}^{0}T^{-\rho}_{0}(f)=0$ then $f=0$. 
Namely, the composition 
$$\End(Q) \xrightarrow{T_0^{-\rho}} \End(T_0^{-\rho}Q) \xrightarrow{T^0_{-\rho}} \End(T_{-\rho}^{0}T_0^{-\rho}Q)$$ 
is an injection, so is the first map. 
\end{proof} 
...
Recall that for any element $\varphi\in \k[N]_{-\eta}$ with $\eta\geq 0$, there is a morphism $\iota^-_{\varphi}$ defined in (\ref{equ 4.21}). 
It gives a morphism $\iota^-_{\varphi}:Q(\bm{\nu-\eta})^{\leq \bm{\mu}}_\k \rightarrow Q(\bm{\nu})^{\leq \bm{\mu}}_\k$ by the equivalence in Theorem \ref{thm 4.7}. 
Consider the morphism 
$$T^0_{-\rho}\iota^-_{\varphi}: Q(w_0\bullet 0+l(\nu-\eta))^{\leq l\mu}_\k \rightarrow Q(w_0\bullet 0+l\nu)^{\leq l\mu}_\k.$$ 
Denote by $z_\nu$ the image of $z$ in 
$$\Lim{\mu\geq \nu}\End(Q(w_0\bullet 0+l\nu)_\k^{\leq l\mu})\xrightarrow{T_0^{-\rho}} \Lim{\mu\geq \nu}\End\big((Q(\bm{\nu})_\k^{\leq \bm{\mu}})^{\oplus |W|}\big)
\xrightarrow[\sim]{(\ref{equ 4.20})}
(\widehat{\k[N]\rtimes U\n})^{\op}_0\otimes \End_\k(\k^{\oplus |W|}).$$ 
Since $z$ is compatible with any $T^0_{-\rho}\iota^-_{\varphi}$, the family $\{z_\nu\}_\nu$ is compatible with $T_0^{-\rho}T^0_{-\rho}\iota^-_{\varphi}=(\iota^-_{\varphi})^{\oplus |W|}$. 
By the proof of Lemma \ref{lem 4.13}, $\{z_\nu\}_\nu$ is a constant family, and therefore is zero if $z_0=0$. 
Hence by Lemma \ref{lem 5.7}(2), $z$ acts by zero on each $Q(w_0\bullet 0+ l\nu)^{\leq l\mu}_\k$. 
...
Z(\sO^{\omega_2}_{\k})\arrow[r,"\tr_{T^{\omega_1}_{\omega_2}}"] & Z(\sO^{\omega_1}_{\k}).
\end{tikzcd}$$
\end{lem} 
\begin{proof} 
...
Z(\sO^{\omega_2}_{S})\arrow[r,"\tr_{T^{\omega_1}_{\omega_2}}"] & Z(\sO^{\omega_1}_{S}).
\end{tikzcd}$$
\end{proof}
...
\end{equation}
Recall the natural isomorphism $\Theta: \id^{\oplus |W_{l,\omega}|}\xs \sT\sT'$ in Lemma \ref{lem 5.0}(3). 
Consider the following diagram 
$$\begin{tikzcd}[column sep=large]
\id \arrow[r,"{(\sT\varepsilon \sT')\circ \varepsilon'}"] \arrow[d,"{z}"'] 
&\sT\sT'\sT\sT'\arrow[r,"{\sT\sT'\Theta^{-1}}","{\sim}"'] \arrow[d,"\sT\sT'z{\sT\sT'}"] 
...
& \sT\sT'\sT\sT'\arrow[r,"{\sT\sT'\Theta^{-1}}","{\sim}"'] \arrow[d,"\eta'\circ(\sT\eta \sT')"]
...
\end{corollary}
\begin{proof} 
Since $H^\bullet(\Fl^{\omega,\circ})^\wedge$ is pro-unilpotent, it is enough to show that the degree zero term of the element $\tr_{\omega}^{0}\circ \tr^{\omega}_{0}(1)$ does not vanish. 
...
It shows that the map $Z(\sC)\rightarrow Z(\sD^b\sC)$ is a direct inclusion. 
...
\end{lem} 
\begin{proof}
For any $z\in Z(\sC)$ and any $M\in \sD$, there is a family $(z_{d,M})_d$ that is zero except finitely many elements, such that $z_{vM}=\sum_d v(z_{d,M})$. 
We show that $(z_d)_d$ defines an element in $Z^\bullet(\sD)$. 
Indeed, for any morphism $f:M\rightarrow M'$ in $\sD$, we have $z_{vM'}\circ vf=vf\circ z_{vM}$, and by definition $\sum_d vz_{d,M'}\circ vf=\sum_dvf\circ vz_{d,M}$. 
It follows that $z_{d,M'}\circ f=f\circ z_{d,M}$ for each $d$. 
\end{proof} 

Suppose that $\sC$ and $\sD$ are abelian categories admitting enough projective objects, and $v$ and $\langle 1\rangle$ are exact functors. 
Let $\sP$ (resp. $\sQ$) be the full subcategory of projective objects in $\sC$ such that any objects in $\sC$ (resp. $\sD$). 
\begin{lem}\label{lem A.21} 
Suppose that any objects in $\sP$ admits a lifting in $\sD$, and $\sQ$ coincides with the additive full subcategory of $\sD$ generated by the liftings of objects in $\sP$. 
...
Let $X_0$ be an $\F_p$-variety admits a finite stratification $X_0=\bigsqcup_{s\in \mathscr{S}} X_{s,0}$ into locally closed affine spaces. 
...
It admits a retraction by restriction $\res: Z^\pur_{\mathscr{S}}(X) \rightarrow H^\bullet(X)^\pur$. 
...
\begin{rmk} 
	(1) The lemma above also works for ind-varieties, by replacing 
	$$ H^\bullet(X)^\pur:= \lim_{s\in \mathscr{S}} H^\bullet(\overline{X_s})^\pur .$$ 
	(2) If $X_0$ is equipped with an action of an algebraic group $\Gamma_0$ over $\F_p$, and the stratification $\mathscr{S}$ is given by the $\Gamma_0$-orbits. 
	Set $\Gamma=\Gamma_0\times_{\F_p} \overline{\F_p}$. 
	Then one can similarly define the ``pure center" $Z_{\Gamma}^\pur(X)$ of $D^{b,\mx}_{\Gamma}(X, \overline{\Q_l})$, and the lemma above also holds. 
\end{rmk} 
