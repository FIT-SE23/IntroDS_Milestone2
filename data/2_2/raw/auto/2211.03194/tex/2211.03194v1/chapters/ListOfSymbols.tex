...
\begin{longtable}{p{.39\textwidth}p{.57\textwidth}}
	...
	$\planningInterval \subseteq \IR_{\geq 0}$ 	& The \emph{planning horizon}, i.e. the time interval during which particles may enter the network \\
	...
	$\Lambda(r) \subseteq L_+^2(\planningInterval)^\Pc$ 
										& The set of feasible walk inflows with fixed network inflow rates $r$ \\
	$\Lambda(Q) \subseteq L_+^2(\planningInterval)^\Pc$ 
	...
	$\Psi:  L_+([t_0,t_f])^\Pc \to  \hat{M}([t_0,t_f])^\Pc$ 
										& \emph{\effWalkDelay}: the effective walk delay $\Psi_p(h,t)$ experienced under the flow induced by $h$ by particles entering walk $p$ at time $t$ (comprising e.g. travel time, early/late arrival penalties, energy costs, ...) \\
	$\truncated{\Psi}{M}: L_+([t_0,t_f])^\Pc \to  L^2([t_0,t_f])^\Pc$
	...
	$S \subseteq L_+^2(\planningInterval)^\Pc$ & \emph{\setS}: the set of all feasible walk inflows \\
	...
	$H_{p\to q}(h,J,\varepsilon,\Delta) \in L_+^2(\planningInterval)^\Pc$ 
										& The walk inflow obtained from $h$ by an \addmEpsDev{} $(q,J,\varepsilon,\Delta)$ \\
	$M_i(h) \subseteq L_+^2(\planningInterval)^\Pc$ 
...
\begin{itemize}
	...
	\item \globalEL[full] (\globalEL): The same as \globalSCDE{} but specifically for $S$ defined by edge-load constraints (i.e. by \eqref{eq:FeasibilitySetforEdgeLoad}) -- see \Cref{def:TypesOfCDE}.
