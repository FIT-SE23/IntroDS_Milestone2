...
There are continuous and nondecreasing cost  functions $\ell_{e}: \R^E \rightarrow \R_{\geq 0}, i\in I, e\in E$.
A \emph{path flow} for commodity~$i\in I$ is a nonnegative vector
$x_i \in \R^{|\Pc_i|}_{\geq 0}$ that  lives in the path flow polytope: 
\begin{align*}
X_i=\left\{x_i\in \R_{\geq 0}^{|\Pc_i|}\;\middle\vert \; \sum_{p\in\Pc_i} x^*_{i,p}=d_i \right\},
\end{align*}
...
\end{defn}
The interpretation here is that all agents are travelling along shortest paths
...
\item $\scalar{\ell( x^*)}{ x^*- y} \leq 0  \text{ for all }y\in X, \text{ with } \ell( x^*):=(\ell_{p}( x^*))_{p\in\Pc}$
\end{enumerate}
\end{lemma}
...
\begin{defn}\label{def:dafermos-sparrow}
...
If $\ell$ is  separable and non-decreasing,  it is the gradient of the Beckmann-McGuire-Whinston potential function  and one can further characterize Wardrop equilibria
...
\item $x^*\in X$ 
is a Wardrop equilibrium.
\item $x^*\in\arg\min_{x\in X} \Big\{  \sum_{e\in E}\int_{0}^{ x_e} \ell_{e}(z) dz  \Big\} $.
\item $\scalar{\ell( x^*)}{ x^*- y} \leq 0  \text{ for all }y\in X, \text{ with } \ell( x^*):=(\ell_{p}( x^*))_{p\in\Pc}.$
\end{enumerate}
\end{lemma}
...
Following Larsson and Patriksson~\cite{Larsson95} (which we abbreviate henceforth with LP) and Patriksson~\cite[pp. 73]{Patriksson1994tap},
a side-constrained equilibrium can be defined
via the notion of \emph{saturated} and \emph{unsaturated} paths.
Given a path flow $x\in Z$, a path $p$ is saturated, if there
is an edge $e\in p$ with $ x_e= c_e$ and, conversely, a path $p$ is unsaturated,
if $ x_e<c_e$ for all $e\in p$.

\begin{defn}\label{def:weakWE}
...
the definition of an LP-equilibrium in terms of feasible \emph{additive} $\varepsilon$-deviations
...
As observed by Marcotte et al.~\cite{Marcotte04}, this definition has the drawback of admitting
...
equilibria in case one of the two edges is longer but carries flow
and the other shorter one has free capacity.
...
\begin{defn} (Bernstein and Smith~\cite[Def. 2.]{BernsteinS94})\label{def:BS}
...
\end{defn}
Note that the original definition of Bernstein and Smith~\cite[Def. 2.]{BernsteinS94} does not involve capacities but by using latency functions $\tilde\ell$ that jump to $+\infty$ as soon as arc capacities are exceeded
an equilibrium will be capacity-feasible.
Following Correa et al.~\cite{CorreaCapEqInStaticFlows}, the definition of BS-equilibrium can be rephrased as ``no arbitrarily small bundle of drivers on a common path can strictly decrease its cost by switching to another path''. 

\subsection{Beckmann-McGuire-Whinston Equilibria and Variational Inequalities}
For the case of separable latency functions, a subset of BS-equilibria can be characterized as solutions
to an associated convex optimization problem, where the Beckmann-McGuire-Whinston potential function over the
convex space of  capacity-feasible flows is minimized:

\begin{align}
\label{beckmann-opt}\tag{BMW}
\min &  \sum_{e\in E}\int_{0}^{ x_e} \ell_{e}(z) dz  \\
\text{s.t.: }& x\in Z.\notag
\end{align}
We obtain the following well-known characterization (see, e.g. Patriksson~\cite{Patriksson1994tap}).
\begin{lemma}
\begin{enumerate}
...
Correa et al.~\cite{CorreaCapEqInStaticFlows} termed equilibria coming from optimal solutions
to~\eqref{beckmann-opt} as \emph{Beckmann-McGuire-Whinston} (BMW) equilibria while
Marcotte et al.~\cite{Marcotte04} termed them  \emph{Hearn-Larsson-Patriksson} equilibria.
...
to other possible side-constrained equilibria, see Correa et al.~\cite{CorreaCapEqInStaticFlows}.
\subsection{Discussion}
It is very instructive to restate a remark made by Marcotte et al.~\cite{Marcotte04}:
...
This observation was also made in Correa et al.~\cite{CorreaCapEqInStaticFlows}:
...
Bernstein and Smith~\cite{BernsteinS94}, Smith~\cite{Smith84}, Heydecker~\cite{Heydecker86} to the domain of \emph{dynamic} side-constrained traffic assignments.
