\begin{tikzpicture}
	\node[namedVertex] (v) at (0,0) {$v$};
	\node[namedVertex] (w) at (4,0) {$w$};
	\node[namedVertex] (t)  at (8,0) {$t$};
	
	\draw[edge] (v) to[bend left=30] node[below]{$e_1$} node[above]{$\tau_{e_1}=2, \nu_{e_1}=2$} (w);
	\draw[edge] (v) to[bend right=30] node[above]{$e_2$} node[below]{$\tau_{e_2}=2, \nu_{e_2}=2$} (w);
	\draw[edge] (w) -- node[below]{$e_3$} node[above](e3){$\tau_{e_3}=2, \nu_{e_3}=\infty$} (t);
	
	\node[right of=t, anchor=west,node distance=1cm] {
		\begin{tikzpicture}[scale=1,solid,black,
			declare function={
				c(\x)= 2;			
			}]
			
			
			\begin{axis}[xmin=0,xmax=4.5,ymax=2.5, ymin=0, samples=500,width=4.5cm,height=2.8cm,
				axis x line*=bottom, axis y line*=left, axis lines=middle, xtick={1,2,3,4}]
				\addplot[blue, ultra thick,domain=0:5] {c(x)} node[below,pos=.5]{$c_{e_3}$};
			\end{axis}
			
		\end{tikzpicture}
	};
	
	\node[left of=v,blue,node distance=1.5cm](){$r=2\cdot\CharF[{[0,2]}]$};
\end{tikzpicture}
