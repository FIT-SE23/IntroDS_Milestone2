...
as its definition -- see the discussions in \citeauthor*{CorreaCapEqInStaticFlows}~\cite[pp.968]{CorreaCapEqInStaticFlows}, Bernstein and Smith~\cite{BernsteinS94} and \citefulls{Marcotte04}.
...
of general existence results for VI's in appropriate function spaces by~Browder~\cite{BROWDER1968}.
 ...
 form as proposed by \citefulls{FrieszLTW89} and augment this model
 with side-constraints. In fact, we will consider a more general general \emph{walk}-delay-operator model, because some interesting side-constraints such as energy-constraints for electric vehicles (see \citefulls{GHP22}) require cyclic routes. Our contribution consists of four types of results.
...
\item We introduce -- in line with prior works for the static flow model (e.g.,  \citeauthor*{CorreaCapEqInStaticFlows}~\cite[pp.968]{CorreaCapEqInStaticFlows}, \citefull{BernsteinS94} and \citefulls{Marcotte04}) -- a \emph{behavioral equilibrium concept} via formally introducing a noncooperative game modeling the
 ...
 \item As the counter-example to \citefulls{zhong11} suggests, the model with edge-volume constraints
...
In a similar way \citefulls{Hamdouch2004} extended the static equilibrium concept from~\cite{Marcotte04} to a dynamic setting.
 ...
 In \Cref{sec:counter}, we give a counterexample to Zhong, Sumalee, Friesz and Lam~\cite{zhong11}, (Prop. 3.1., pp. 1040) which illustrates the need of rethinking an appropriate
