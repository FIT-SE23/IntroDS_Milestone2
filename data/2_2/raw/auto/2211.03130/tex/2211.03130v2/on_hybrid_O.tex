...
for some reflections $s_{\beta_i}$ in the $l$-affine Weyl group $W_{l,\af}$. 
...
$$\Theta_{(\omega_1,\omega_2)}:\ \id^{\oplus |W_{l,\omega_2}/W_{l,\omega_1}|} \xs T_{\omega_1}^{\omega_2}T^{\omega_1}_{\omega_2}$$ 
...
\end{prop}
\begin{proof} 
We prove the proposition by showing that the image of $g_\alpha$ generates $\End(Q( \bm{n}_\alpha )^{\leq \bm{m}_\alpha}_{S_\alpha})$ as an $S_\alpha$-algebra. 
...
By abuse of notation, we let $\Fl_{SL_2}$ and $\Gr_{SL_2}$ be the affine flag variety and the affine Grassmannian associated with $SL_2((t))$. 
...
In this appendix, we prove Proposition \ref{prop 5.0}. 
Our construction essentially appeared in the work of Anderson--Jantzen--Soergel \cite{AJS94} for small quantum groups and Fiebig \cite{Fie03} for symmetrizable Kac--Moody algebras. 
For simplicity, we fix an element $\omega\in \Xi_\sc$ and prove the proposition for $(\omega_1,\omega_2)=(0,\omega)$. 
...
For any $z\in  W_{l,\af}$, we define a $|W_{l,\omega}|\times |W_{l,\omega}|$-matrix with coefficients in $\hat{S}$ by 
...
Note that $\Theta_{\hat{\K},z}$ only depends on the class of $z$ in $W^\omega_{l,\af}$. 
Since the category $\sO^{\omega}_{\hat{\K}}$ is semisimple, and $\{M(y\bullet\omega)_{\hat{\K}}\}_{y\in W_{l,\af}^\omega}$ forms a complete set of simple objects, the collection $\{\Theta_{\hat{\K},y}\}_{y\in W_{l,\af}^\omega}$ determines a natural isomorphism 
...
between functors on $\sO^{\omega}_{\hat{S}}$, such that $\Theta_{\hat{\K}}=\Theta_{\hat{S}}\otimes_{\hat{S}} \hat{\K}$. 
\end{prop}
...
the isomorphism $\Theta_{\hat{\K}, M(z\bullet\omega)_{\hat{\K}}}$ restricts to an isomorphism on the subspaces 
$$\Theta_{\hat{S}_\p,z}: \bigoplus_{x\in W_{l,\omega}} M(z\bullet\omega)_{\hat{S}_\p} \rightarrow \sT\sT'M(z\bullet\omega)_{\hat{S}_\p}.$$ 
\item For any $\alpha\in \check{\Phi}_{l,\re}^+$ and any $z\in W_{l,\af}$ such that $s_\alpha z\bullet \omega> z\bullet \omega$, the isomorphism $\Theta_{\hat{\K}, Q(z\bullet\omega)_{\hat{S}_\alpha}\otimes_{\hat{S}_\alpha}\hat{\K}}$ restricts to an isomorphism on the subspaces 
$$\Theta_{\alpha,z}: \bigoplus_{x\in W_{l,\omega}}Q(z\bullet\omega)_{\hat{S}_\alpha} \rightarrow \sT\sT'Q(z\bullet\omega)_{\hat{S}_\alpha}.$$ 
\end{enumerate}
\end{prop}
...
Hence up to multiplication by an element in $GL_{|W_{l,\omega}|}(\hat{S}_\alpha)$ from the left, the matrix $\sH_1$ can be represented by 
...
It shows that 
$$\Theta_{\hat{S}_\p,1}:= (F_1 \otimes_{\hat{S}} \hat{S}_\alpha) \circ \sH_1$$ 
...
\end{tikzcd}$$ 
Therefore, to find the desired isomorphism $\Theta_{\alpha,1}$, it is enough to show that there is a morphism $\theta$ fitting into the commutative diagram 
$$\begin{tikzcd}0\arrow[r]& 
...
\end{tikzcd}$$ 
then we can set $\Theta_{\alpha,1}=\bigoplus\limits_{x\in W_{l,\omega}} (\epsilon^{\alpha}_{x})^{-1}\circ \theta$. 
We claim that 
...
By the equivalence (\ref{equ 2.10}), $\Theta_{\hat{S}}$ induces a natural isomorphism 
$$\Theta_{\hat{S}}\otimes_{\hat{S}}R:\id^{\oplus |W_{l,\omega}|}\xs \sT\sT'$$ 
of functors on $\sP^{A,\leq\nu}_R$ for each $\nu$, where $A=U^\hb_q$. 
Since $\sP^{A,\leq\nu}_R$ consists of projective modules in $\sO^{\leq\nu}_{R}$ and the functors $\id^{\oplus |W_{l,\omega}|}$, $\sT\sT'$ are exact, it extends to a natural isomorphism $\Theta_{(0,\omega)}$ on $\sO^{\leq \nu}_{R}$ for each $\nu$, and hence on $\sO_{R}$. 
\end{proof} 
