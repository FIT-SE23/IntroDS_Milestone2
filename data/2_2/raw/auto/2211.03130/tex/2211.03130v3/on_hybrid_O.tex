...
\end{abstract}
%including the Jantzen filtration for Verma modules, the linkage principle, the construction of projective modules and the BGG reciprocity. 
...
Recently, a new version of quantum group called the \textit{hybrid quantum group} has been introduced by Gaitsgory \cite{Gai18}, in view of extending the Kazhdan--Lusztig equivalence. 
...
\textit{De Concini--Kac's quantum group} $\fU_q$ generated by Chevalley generators and \textit{Lusztig's quantum group} $U_q$ generated by their divided powers. 
...
$$\g\mod^B$$ 
of finitely generated $\g$-modules whose restriction on $\b=\Lie(B)$ is integrable. 
Similarly we can interprete $\sO_q$ as the category 
$$\fU_q\mod^{U^{\geq}_q}$$ 
of finitely generated $\fU_q$-modules equipped with an integrable $U^{\geq}_q$-module structure that is compatible with the natural map $\fU^{\geq}_q\rightarrow U^{\geq}_q$, which is a reasonable quantum analogue of $\g\mod^B$. 


If the parameter $q$ is specialized to a transcendental element in $\C^\times$, the category $\sO_q$ is equivalent to the classical BGG category $\sO$. 
...
%greater than the Coxeter number for $G$, and coprime to the determinant of the Cartan matrix and to $3$ if $G$ contains a component of type $G_2$. 
...
%The simple objects $\{E(\lambda)\}_\lambda$ in $\sO_\zeta$ are parametrized by their highest weight in $X^*(T)$. 
...
\sO_{\zeta}= \bigoplus_{\omega\in X^*(T)/W_{l,\af}} \sO^{\omega}_{\zeta}, 
\end{equation}
...
For the classical category $\sO$ of $\g$, deformation techniques have been used extensively to study basic structures, e.g. Soergel \cite{Soe90} and Fiebig \cite{Fie03}. 
...
\Lim{n \leq m} \End\big(Q(\bm{n}_{\alpha})^{\leq \bm{m}_{\alpha}}_{S}\big).$$ 
%We compute this algebra explicitly and show that it is isomorphic to the center of the corresponding block. 
...
$$\sO_{\zeta,S}=\bigoplus\limits_{\omega\in X^*(T)/W_{l,\af}} \sO^{\omega}_{\zeta,S}.$$ 
...
$$\bb:\ H_{\check{T}}^\bullet(\Gr^\zeta)^\wedge_{S} \stackrel{\sim}\longrightarrow Z(\sO_{\zeta,S}),$$ 
...
$$\bb:\ H_{\check{T}}^\bullet(\Fl^{\omega,\circ})^\wedge_{S} \stackrel{\sim}\longrightarrow Z(\sO^{\omega}_{\zeta,S}),$$ 
...
Hence $F_i(\alpha).1_{\bm{n}_\alpha}\in M^i(\bm{n}_\alpha)_{S_\alpha}\backslash \alpha M(\bm{n}_\alpha)_{S_\alpha}$ and its image in $M^i(\bm{n}_\alpha)_{\k_\alpha}$ is nonzero, which shows that $\dim M^i(\bm{n}_\alpha)_{\k_\alpha,\bm{n-i}_\alpha}\geq 1$. 
Since $M(\bm{n}_\alpha)_{\k_\alpha}$ is a torsion free $\fU^-_\zeta\otimes \k_\alpha$-module, we have 
$$M(\bm{n-i}_\alpha)_{\k_\alpha}\cong (\fU^-_\zeta\otimes \k_\alpha) F_i(\alpha).1_{\bm{n}_\alpha} \subset M^i(\bm{n}_\alpha)_{\k_\alpha}$$ 
...
\end{equation}
Applying the Jantzen sum formula (\ref{equ 6.2}) for $M(\bm{n}_\alpha)_{\k_\alpha}$, we get 
...
A remarkable theorem given by De Concini--Kac--Procesi in {\cite[\textsection 5]{DeCKP92}} shows that $\kappa^*(f_{\alpha_i})= -K_i^l \underline{X}_{i}$, where $f_{\alpha_i}$ is viewed as a Killing vector field on $G$ (i.e. the vector fields induced by the conjugate action). 
...
\subsubsection{Endomorphism algebra of the ``big projective module"} 
...
When $\omega$ is an $\alpha$-singular weight, we have $P^b(\omega)_{\k_\alpha}=M(\omega)_{\k_\alpha}$ as $\fU^{b}_\zeta\otimes \k_\alpha$-modules, and in this case (c.f. (\ref{equ 3.20}))   
...
From (\ref{equ 6.9}) and Corollary \ref{cor 6.4}, we deduce that $E(\alpha)^{m-n}.Q(\bm{n_1}{}_\alpha)^{\leq \bm{m}_\alpha}_{\k_\alpha,\bm{n}_\alpha}=0$ if $n_1>n$. 
...
$$H^\bullet_{T'}(\Gr^\zeta)^\wedge_R$$ 
the space of formal series of $[\Fl^{\omega,x}]_{T'}$ with coefficients in $R$. 
...
\end{equation} 
such that $(\Fl^{\sX}_{_{SL_2}})^{\check{T}}=\sX$. 
We view $\check{\alpha}$ as the positive root of $SL_2$, and let $T_2\subset SL_2$ be the the standard torus. 
Let $\Fl_{_{SL_2}}$ and $\Gr_{_{SL_2}}$ be the affine flag variety and the affine Grassmannian associated with the loop group $SL_2((t^l))$, and identify their fix points $(\Fl_{_{SL_2}})^{T_2}=W_l(\alpha)$ and $(\Gr_{_{SL_2}})^{T_2}=W_l(\alpha)/\langle s_\alpha\rangle=l\Z\alpha$, respectively. 
...
$$\Fl^{-\rho+\sX^\omega_\alpha}_{_{SL_2}}\simeq 
\begin{cases}
\Fl_{_{SL_2}} & \text{if $\omega$ is $\alpha$-regular,} \\ 
\Gr_{_{SL_2}} & \text{if $\omega$ is $\alpha$-singular.} 
\end{cases}$$ 
Consider the composition 
$$H^\bullet_{\check{T}}(\Fl^{-\rho+\sX^\omega_\alpha}_{_{SL_2}})^\wedge_{S_\alpha} \xrightarrow{\bb} Z(\sO^{\sX^\omega_\alpha}_{S_\alpha})\xrightarrow{\chi_{S_\alpha}} \Fun(\sX^\omega_\alpha,S_\alpha).$$ 
...
Indeed, since $\phi_\alpha-i\alpha$ kills $M(\bm{i}_\alpha)_{S_\alpha}$, it maps $Q(\bm{i}_\alpha )^{\leq \bm{m}_\alpha}_{S_\alpha}$ to the submodule $Q(\bm{i+1}_\alpha )^{\leq \bm{m}_\alpha}_{S_\alpha}$. 
...
\begin{equation}\label{equ 6.18} 
 ...
 \end{equation}
Note that the coordinate for $\sT'\phi_\alpha$ in 
$$\End(Q(\bm{2j-1})^{\leq \bm{2m}}_{S_\alpha}) \otimes_{S_\alpha} \K 
	=\prod_{i=2j-1}^{2m} \End(M(\bm{i})_{\K})=\prod_{i=2j-1}^{2m}\K $$ 
reads as $((2j-1)\alpha,(2j-1)\alpha,\cdots ,m\alpha,m\alpha)$, which coincides with the one for $\phi'_\alpha$, hence $\sT'\phi_\alpha=\phi'_\alpha$ as endomorphisms for $Q(\bm{2j-1})^{\leq \bm{2m}}_{S_\alpha}$. 
...
By Lemma \ref{lem 6.2}, we have $\dim\Hom(M(\bm{0}_\alpha)_{\k_\alpha},M(\bm{j}_\alpha)_{\k_\alpha})=1$, hence the composition above is nonzero, so is (\ref{equ 6.17}). 
\end{proof} 
...
Note that the truncation $\tau^{\leq \bm{2j}}(\overline{\psi_\alpha})$ as an endomorphism of $Q(\bm{2j-1})^{\leq \bm{2j}}_{\k_\alpha}$ is also provided by $\overline{\psi_\alpha}$. 
...
Since by (\ref{equ 1.1}), $\Gr^\zeta$ is a disjoint union of partial affine flag varieties (with parameter $t$ replaced by $t^l$), we only prove Proposition \ref{prop B.1} for the connected component $\Fl^\circ$ without loss of generality. 
...
$$(\Fl^\circ)^{\check{T}_\alpha}=\bigsqcup_{\sX\in W(\alpha) \backslash W_{\af}} \Fl^{\sX}_{_{SL_2}},$$ 
such that $\sX= (\Fl^{\sX}_{_{SL_2}})^{\check{T}}$. 
...
We recall the GKM theory for the equivariant cohomology of $\Fl^{J,\circ}$, which describes the image of the embeddings 
\begin{equation}\label{equ B.1} 
H^\bullet_{\check{T}}(\Fl^{J,\circ})\rightarrow \Fun(W^J_\af,S') , \quad H^\bullet_{\check{T}\times \Gm}(\Fl^{J,\circ})\rightarrow \Fun(W^J_\af,\hat{S}') 
\end{equation} 
by restrictions to $\check{T}$-fixed points. 
...
Keep this procedure one can express $f$ into a formal sum of ${\xi}^{J,y}$'s, and such expression is unique by Properties (\ref{equ B.3})(2). 
...
Indeed, by Propositions \ref{prop B.2}, \ref{prop B.3} and Lemma \ref{lem B.5}, the LHS coincides with $\sGKM(W_\af,W_\af)_{S_\alpha}$, and the RHS coincides with $\prod_{\sX} \sGKM(W(\alpha),\sX)_{S_\alpha}$. 
The conclusion follows from the equalities 
$$\sGKM(W_\af,W_\af)_{S_\alpha}=\sGKM(W(\alpha),W_\af)_{S_\alpha}
=\prod_{\sX}\sGKM(W(\alpha),\sX)_{S_\alpha}.$$ 

(2) By Propositions \ref{prop B.2}, \ref{prop B.3} and Lemma \ref{lem B.5}, we have $H^\bullet_{\check{T}}(\Fl^{\circ})^\wedge_{S}=\sGKM(W_\af,W_\af)_{S}$. 
The conclusion follows from the proof of (1) and the equalities 
$$\sGKM(W_\af,W_\af)_{S}=\Fun(W_\af,S)\cap \bigcap_{\alpha\in \Phi^+} \sGKM(W(\alpha),W_\af)_{S_\alpha}. \qedhere $$ 
\end{proof}
...
In this appendix, we prove Proposition \ref{prop 5.0}, and give an enhanced statement in \textsection\ref{subsect B.4}. 
...
%We also write $\sT'_{z}=\sT'_{\lambda}$ and $\sT_{z}=\sT_{\lambda}$ if $\lambda=z\bullet 0$. 
...
The second equality follows from the vanishing $\Hom(M(\lambda), M(\lambda_1))=0$. 
