...
Despite the impressive text generation and representation power of PLMs, they can also come with the risk~\citep{Bender2021OnTD,Bender2020ClimbingTN,Brown2020LanguageMA} of generating disinformation~\citep{Pagnoni2021UnderstandingFI} or exacerbating biases~\citep{Prabhumoye2018StyleTT}. Instead of improving upon PLM architectures or generation techniques, our work focuses on using existing PLMs to create training data for NLU tasks. 
In practice, our method can be combined with any bias reduction and correction strategies~\citep{Gehman2020RealToxicityPromptsEN,Ma2020PowerTransformerUC} to reduce the adverse effects of PLMs.
...
Since the generated samples may contain label noise, we propose a simple training procedure that first trains classifiers on the few-shot training set and then on the generated set by applying regularization for noise-robustness.
...
Future directions may include: Using larger PLMs as the generator and the classifier, jointly training both models with each other's high-confident predictions, improving the robustness of models trained on synthetic data, and developing systematic metrics to evaluate the quality of generated training samples.
