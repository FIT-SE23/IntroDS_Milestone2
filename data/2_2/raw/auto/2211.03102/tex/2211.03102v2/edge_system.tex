...
%
R. Wang is with School of Computer Science and Engineering, Nanjing University of Aeronautics and Astronautics (NUAA), Nanjing 211106, China. Email: wangran@nuaa.edu.cn.
%
}
}
\maketitle
...
%
Video analytics is widely adopted in many computer vision-based applications, such as video surveillance \cite{xing2021noisyotnet}, augmented reality, autonomous driving, etc.
%
...
%
which are compute-intensive and may incur substantial delay for inference \cite{jiang2021joint}.
%
...
%
the DNN models can be deployed on edge nodes, which are proximal to the users \cite{yi2017lavea}.
%
The edge nodes can receive video data from users with negligible delay \cite{guan2020prefcache,yan2018toward}.
%
...
%
Reducto \cite{li2020reducto}, AppealNet \cite{li2021appealnet}, and CloudSeg \cite{wang2019bridging} adopted frame filtering, inference difficulty classification, and resolution downsizing to reduce communication cost for video transmission while maintaining accuracy.
%
FCHP \cite{zhang2022fchp}, \cite{kim2018real}, and \cite{zhang2016vlsi} adopted DNN pruning and dedicated hardware to improve the performance for video analytics.
...
%
Therefore, it is necessary to consider the collaboration of multiple edge nodes at different locations to improve the overall performance of the video analytics system.
...
%
FCHP \cite{zhang2022fchp} adopted DNN pruning to reduce inference delay and memory usage.
%
Kim \textit{et al.} \cite{kim2018real} and Zhang \textit{et al.} \cite{zhang2016vlsi} designed dedicated hardware to reduce processing delay.
%
...
%
In this section, we formulate the collaborative video analytics on the edge nodes as a Decentralized Partially Observable Markov Decision Process (Dec-POMDP).
%
...
%
the current lengths of the dispatching queues to the other edge nodes, 
%
and the bandwidths between the local edge node and the other edge nodes.
%
...
%
The critic network is a value function. The inputs of the critic network are the local states of all agents (i.e., the global state), and the output is the predicted value used to calculate Generalized Advantage Estimation (GAE).
%
...
%
The output of the critic network is the predicted value used to calculate GAE.
...
As the number of the edge nodes increases, the input dimension of the critic network will become larger, and some irrelevant information may cause the critic network to have a larger value prediction error, which in turn affects the actor network in learning the optimal policy.
%
...
%
With the attention mechanism, we can help the critic network increase the accuracy of value predictions and improve system performance.
%
...
After an episode, the estimated advantage is calculated with GAE in the trajectory $\tau$, and the reward-to-go is calculated in the trajectory. Finally, batches are sampled from all transitions in this trajectory to train the networks. 
%
...
%
Then, we optimize the policy objective and loss objective by Adam optimizer.
...
We denote the critic network of an agent as $V_{\phi}$ for computing GAE. The critic network is updated by minimizing the following loss,
%
\begin{equation}
\begin{aligned}
...
\label{eq:critic}
\end{equation}
where $ \hat{R}_{i} $ is the discounted reward-to-go, $s_i$ is the global state obtained from the buffer, $ V_{\phi} $ is the value function under parameters $ \phi $, $ V_{\phi_{old}} $ is the value function under parameters $ \phi_{old} $, $ \varepsilon $ is the hyperparameter that controls the clipping strength of the value loss to prevent the new value function from changing too much compared with the old value function.
...
1) \emph{IPPO}: the edge nodes learn their policies independently with Proximal Policy Optimization (PPO) \cite{schulman2017proximal}, which is a single-agent reinforcement learning method without communications among agents.  

2) \emph{Local-PPO:} each edge node only processes inference requests locally without dispatching them to other edge nodes, and selects models and resolutions with PPO. %\cite{schulman2017proximal}
...
%
However, Fig. \ref{fig: ablation delay} and Fig. \ref{fig: ablation drop} show that it has higher overall delay and video frame drop percentage, resulting in lower performance.
...
\bibliography{_edge_system_base}
\end{document}
