...
First, \bon has the highest number of neural network model parameters, which are 2.71$\times$, 13.33$\times$, and 2.94$\times$ more than \cc, \gpf, and \mechmp, respectively. \gs{Second, compared to \bon, which is the most accurate expert-designed basecaller, \mechmp provides 16.56$\times$ higher basecalling throughput without any loss in accuracy. This is because \mechmp has a mixed precision neural architecture that can exploit low-precision compute units present on our evaluated spatial vector computing systems, i.e., the AMD-Xilinx Versal AI Engine (AIE). Compared to \gpf, which is the fastest basecaller, \mechmp provides, on average, 2.97\% higher accuracy with 3.19$\times$ higher basecalling throughput. Therefore, \mechmp provides both accuracy and high basecalling throughput compared to the baseline basecallers.}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\linewidth]{images/CRF/read_identity_aie_CRF_2.pdf}
  \vspace{-15pt}
...
\mechmp improves average performance by 72.55$\times$,		3.19$\times$, and 16.56$\times$ over \cc,  \gpf, and \bon, respectively. Second, by leveraging low precision capabilities, \texttt{RUBICALL}-\texttt{MP} provides 7.45$\times$ higher performance  when compared to its floating-point implementation (\mechfp).
...
Third, \mechfp, by using floating-point precision, provides 9.72$\times$, 0.43$\times$, \sr{and} 2.22$\times$ performance compared to \cc, \gpf, and \bon, respectively.    We conclude \sr{that} using mixed-precision computation, \mechmp consistently performs better than the baseline basecallers.
% $\times$

\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\linewidth]{images/CRF/basecalling_speed_aie_AMD_CRF_2.pdf}
  \vspace{-13pt}
...
\begin{subfigure}[h]{0.5\textwidth}
  \centering
  \includegraphics[width=\linewidth,trim={0.2cm 0.2cm 0cm 0.2cm},clip]{images/CRF/samtools_mismatch_CRF_1.pdf}
%   \caption{High-end and middle-end (\textsf{H\&M}).
  \vspace{-0.6cm}
...
\begin{subfigure}[h]{0.5\textwidth}
  \centering
  \vspace{-0.04cm}
  \includegraphics[width=\linewidth,trim={0.2cm 0.2cm 0.2cm 0.2cm}, clip]{images/CRF/samtools_mapped_bases_CRF_1.pdf}
%   \caption{High-end and low-end (\textsf{H\&L}).
  \vspace{-0.6cm}
\end{subfigure}
...
\begin{subfigure}[h]{0.49\textwidth}
  \centering
  \vspace{-0.04cm}
  \includegraphics[width=\linewidth,trim={0.2cm 0.2cm 0.2cm 0.2cm}, clip]{images/CRF/samtools_read_map_CRF_1.pdf}
%   \caption{High-end and low-end (\textsf{H\&L}).
  \vspace{-0.6cm}
\end{subfigure}
\begin{subfigure}[h]{0.5\textwidth}
  \centering
  \vspace{-0.04cm}
  \includegraphics[width=\linewidth,trim={0.2cm 0.2cm 0.2cm 0.2cm}, clip]{images/CRF/samtools_read_unmap_CRF_1.pdf}
%   \caption{High-end and low-end (\textsf{H\&L}).
  \vspace{-0.6cm}
\end{subfigure}
