...
a convolutional neural network (CNN)-based version of  \texttt{Bonito} from ONT, has $\sim$10 million model parameters.    
...
This leads to high bandwidth and processing demands~\cite{singh2019low,singh2020nero}. Thus, current basecallers with floating-point arithmetic precision have inefficient hardware implementations. We observe in Section~\ref{suppsubsubsec:overprovision_quant} that the arithmetic precision requirements of current basecallers can be reduced $\sim$4$\times$ by adjusting the precision for each neural network layer based on the target hardware and desired accuracy. %?} with intelligent co-design of neural network architecture and precision for each neural network layer.   

Fifth, basecallers that provide higher throughput have lower basecalling accuracy. For example, we show in Section~\ref{subsection:results_overall} and Supplementary~\ref{suppsec:guppy_compare}  that \gpf provides up to 22.7$\times$ higher basecalling performance using  36.96$\times$ fewer model parameters at the expense of the   5.37\% lower basecalling accuracy compared to more accurate basecallers.
...
accuracy. \strim performs a skip removal process using knowledge distillation~\cite{bucilua2006model}, where  we train a smaller network (\textit{student}) without skip connections to mimic a pre-trained larger network (\textit{teacher}) with skip connections. 
...
basecaller that performs efficient basecalling, \hc{outperforming the state-of-the-art basecallers.}  We compare \mech to five different basecallers. We demonstrate \gont{six} key results.  First,   \mech provides, on average, 2.97\% higher basecalling accuracy with 3.96$\times$ higher basecalling throughput compared to the fastest basecaller. Compared to %the most accurate state-of-the-art 
an expert-designed basecaller \mech provides 141.15$\times$ higher basecalling throughput without any loss in basecalling accuracy  by leveraging mixed precision computation when implemented on a cutting-edge spatial vector computing system, i.e., the AMD-Xilinx Versal AIE-ML~\cite{aiemlvc2802}. Second, we show that \nas-designed models are 5.74$\times$ smaller in size with 2.41$\times$ fewer neural network model parameters than an expert-designed basecaller. Third, by further using our \strim approach, \mech achieves a 6.88$\times$ and 2.94$\times$ reduction in neural network model size and the number of parameters, respectively. \gont{
Fourth, we show in Supplementary~\ref{suppsec:guppy_compare} that compared to the most accurate state-of-the-art basecaller, \mech provides 301.92$\times$ speedup using 19.22$\times$ lower parameters at the expense of 2.55\% lower accuracy.}  Fifth, assemblies constructed
