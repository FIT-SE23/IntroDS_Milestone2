...
\sr{We compare} the \sr{overall} basecalling \sr{throughput} \sr{of} \mech with \sr{that of the} baseline basecaller\sr{s} in terms of model parameters, model size, and basecalling accuracy in Figure~\ref{fig:model_compare}(a), \ref{fig:model_compare}(b), and \ref{fig:model_compare}(c), respectively. We evaluate \mech using: (1) Mi50 GPU~\cite{mi50} (\mech-FP) \gssss{using floating-point precision computation}, and (2) Versal ACAP VC2802~\cite{aiemlvc2802}, a  cutting-edge spatial vector computing system (\mech-MP) \gssss{using mixed-precision computation}. \gont{Section~\ref{sec:evaluation} provides details on our evaluation methodology.}
%with trainable neural network parameters in a basecaller and the total model size, respectively. We draw four observations. 
 % \yboxbegin
 \begin{figure}[h]  
  \centering
  \includegraphics[width=1\linewidth]{images/NCS/2023.03.31.model_compare_speed_1_FLIP.pdf}%{images/model_compare_AMD_CRF_1.pdf}
%   \vspace{-15pt}
  \captionof{figure}{Comparison of \hc{average} basecalling  throughput for \mechmp with state-of-the-art basecallers in terms of:  (a) model parameters, (b) model size, and (c) \hc{average}  basecalling accuracy. \mechmp provides higher compute performance with lower model size when compared to \mechfp because of the mixed-precision computation.}
...
First,  \mechmp provides 141.15$\times$ higher basecalling throughput without any loss in accuracy compared to \bon, which is an expert-designed basecaller. Unlike \bon, This is because \mechmp has a mixed precision neural architecture that leads to high compute density. Second, \hy{compared to \dor, the fastest basecaller, \mbox{\mechmp} provides, on average, 2.97\% higher accuracy with 3.96$\times$ higher basecalling throughput. Therefore, \mbox{\mechmp} provides both accuracy and high basecalling throughput.
Third, by using mixed-precision quantization, \texttt{RUBICALL}-\texttt{MP} provides 63.61$\times$ higher performance  when compared to its floating-point implementation (\mechfp).
...
We conclude that \mechmp provides the ability to basecall quickly, and efficiently scale basecalling by providing reductions in both model size and  neural network model parameters.
...
\hy{\mbox{\mechmp} improves average performance by 618.36$\times$,		27.62$\times$,  141.15$\times$, 166.40$\times$ and 3.96$\times$ over \cc,  \gpf, \bon, \sac, and \mbox{\dor}, respectively.} Second, as \texttt{RUBICALL}-\texttt{MP} each layer is quantized to a different precision, it provides 63.61$\times$ higher performance  when compared to its floating-point only implementation (\mechfp).
...
Third, \mechfp, by using floating-point precision, provides 17.27$\times$, 8.96$\times$, 4.17$\times$, and 4.83$\times$, performance compared to \cc, \gpf, \bon, and \sac respectively.    We conclude \sr{that} using mixed-precision computation, \mechmp consistently performs better than the baseline basecallers.
% $\times$

\begin{figure}[h]
 % \yboxbegin
  \centering
  \includegraphics[width=0.8\linewidth]{images/NCS/2023.31.03_basecalling_speed_mi50AMD_2.pdf}%{images/CRF/basecalling_speed_aie_AMD_CRF_2.pdf}
  \vspace{-13pt}
  \captionof{figure}{Performance comparison of \mech (using floating-point precision (\mechfp) and mixed-precision (\mechmp))~\sr{and five state-of-the-art basecallers. The y-axis is on a logarithmic scale.}}
  \label{fig:perf}
  \vspace{12pt}
 % \yboxend
\end{figure}
...
\sr{We compare} the basecalling accuracy of \mech against baseline basecallers in Figure \ref{fig:identity}.  \mechmp and \mechfp use the same model architecture and produce the same basecalled reads, so we report results as \mech.
...
First, \mech provides 6.54\% and 0.57\% higher accuracy than CNN-based basecaller \cc and \bon, respectively. \hy{Compared to a state-of-the-art transformer-based basecaller, \mbox{\sac}, \mbox{\mech} achieves 2.14\% higher basecalling accuracy. As \mbox{\dor} and \mbox{\gpf} have the same neural network architecture, we observe \mbox{\mech} achieves 2.97\% higher accuracy over these RNN-based basecallers.} Second, \bon has 2.93$\times$ higher parameters (Figure~\ref{fig:model_compare}(a)) but is 0.57\% less accurate than \mech. %The skip connections in  \bon, similar to \cc, create a performance bottleneck by increasing the data lifetime and adding additional computation. 
...
\begin{figure}[h]
 % \yboxbegin
  \centering
  \includegraphics[width=0.8\linewidth]{images/NCS/2023.31.03_basecalling_accuracy_1.pdf} %{images/CRF/read_identity_aie_CRF_2.pdf}
  \vspace{-15pt}
  \captionof{figure}{\gs{Basecalling accuracy} comparison of \mech.}
  \label{fig:identity}
%   \vspace{-12pt}
\end{figure}
 % \yboxend
...
\hy{We provide the statistics related to the accuracy, completeness, and contiguity of assemblies we generate using the basecalled reads from \cc, \gpf, \bon, \sac, \dor, and \mbox{\mech} in \mbox{Table~\ref{tab:downstream}}.} 
...
\hy{We provide the comparison of \mbox{\mech} with \cc, \gpf, \bon, \sac, and \mbox{\dor} in terms of the total number of base mismatches, the total number of  mapped bases, the total number of mapped reads, and the total number of unmapped reads in Figure~\mbox{\ref{fig:samtool}}(a), \mbox{\ref{fig:samtool}}(b), \mbox{\ref{fig:samtool}}(c), and \mbox{\ref{fig:samtool}}(d), respectively.}
...
\begin{subfigure}[h]{0.5\textwidth}
  \centering
  \includegraphics[width=\linewidth,trim={0.2cm 0.2cm 0cm 0.2cm},clip]{images/NCS/2023.3.31.samtools_mismatch_CRF_1.pdf}
...
\begin{subfigure}[h]{0.5\textwidth}
  \centering
  \vspace{-0.04cm}
  \includegraphics[width=\linewidth,trim={0.2cm 0.2cm 0.2cm 0.2cm}, clip]{images/NCS/2023.3.31.samtools_mapped_bases_CRF_1.pdf}%{images/CRF/samtools_mapped_bases_CRF_1.pdf}
%   \caption{High-end and low-end (\textsf{H\&L}).
  \vspace{-0.6cm}
\end{subfigure}
...
\begin{subfigure}[h]{0.49\textwidth}
  \centering
  \vspace{-0.04cm}
  \includegraphics[width=\linewidth,trim={0.2cm 0.2cm 0.2cm 0.2cm}, clip]{images/NCS/2023.3.31.samtools_read_map_CRF_1.pdf} %{images/CRF/samtools_read_map_CRF_1.pdf}
%   \caption{High-end and low-end (\textsf{H\&L}).
  \vspace{-0.6cm}
\end{subfigure}
\begin{subfigure}[h]{0.5\textwidth}
  \centering
  \vspace{-0.04cm}
  \includegraphics[width=\linewidth,trim={0.2cm 0.2cm 0.2cm 0.2cm}, clip]{images/NCS/2023.3.31.samtools_read_unmap_CRF_1.pdf}%{images/CRF/samtools_read_unmap_CRF_1.pdf}
%   \caption{High-end and low-end (\textsf{H\&L}).
  \vspace{-0.6cm}
\end{subfigure}
\captionof{figure}{Comparison of \mech for (a) mismatches, (b) bases mapped, (c) reads mapped, and (d) reads unmapped.
\label{fig:samtool}}
% \vspace{-0.5cm}
\end{figure*}
% \yboxend

We make four key observations. \hy{First, \mbox{\mech} provides the lowest number of base mismatches, which are 26.49\%, 21.35\%, 1.82\%, 13.3\%, and 21.35\% lower compared to \cc, \gpf, \bon, \sac, and \dor, respectively.} 
...
\hy{Second, \mbox{\mech} provides, on average, the highest number of mapped bases, which are 0.56\%, 0.83\%, 0.77\%, 1.04\%, and 0.77\% higher compared to \cc, \gpf, \bon, \sac, and \dor, respectively.}
...
\mbox{\mech} achieves 78.04\%, 19.98\%, 14.23\%, 62.1\%, and 19.98\% lower unmapped reads compared to \cc, \gpf, \bon, \sac, and \mbox{\dor} respectively.
