...
First,  \mechmp provides 141.15$\times$ higher basecalling throughput without any loss in accuracy compared to \bon, which is an expert-designed basecaller. This is because \mechmp has a mixed precision neural architecture that can exploit low-precision compute units present on our evaluated spatial vector computing systems. \hy{Compared to \dor, the fastest basecaller, \mbox{\mechmp} provides, on average, 2.97\% higher accuracy with 3.96$\times$ higher basecalling throughput. Therefore, \mbox{\mechmp} provides both accuracy and high basecalling throughput compared to the baseline basecallers.
Second, \mbox{\sac} has the highest number of neural network model parameters, which are 2.74$\times$, 13.49$\times$, 1.01$\times$, 13.49$\times$, and 2.97$\times$ more than \cc, \gpf, \bon, \dor, and \mechmp, respectively.} 
...
We conclude that \mechmp provides the ability to basecall accurately, quickly, and efficiently scale basecalling by providing reductions in both model size and  neural network model parameters.
...
\hy{\mbox{\mechmp} improves average performance by 618.36$\times$,		27.62$\times$,  141.15$\times$, 166.40$\times$ and 3.96$\times$ over \cc,  \gpf, \bon, \sac, and \mbox{\dor}, respectively.} Second, by leveraging low precision capabilities, \texttt{RUBICALL}-\texttt{MP} provides 63.61$\times$ higher performance  when compared to its floating-point implementation (\mechfp).
