...
Sixth, basecallers that provide higher throughput have lower basecalling accuracy. For example, we show in Section~\ref{subsection:results_overall} and \fgb{Additional file 1:} Section S4  that \gpf provides up to 51.65$\times$ higher basecalling performance using  36.96$\times$ fewer model parameters at the expense of the   5.37\% lower basecalling accuracy compared to most accurate basecaller.
...
accuracy. \strim performs a skip removal process using knowledge distillation~\cite{bucilua2006model}, as shown in \fgb{Additional file 1:} Figure S2 in \fgb{Additional file 1:} Section S2, where  we train a smaller network (\textit{student}) without skip connections to mimic a pre-trained larger network (\textit{teacher}) with skip connections. \gsss{Figure~\ref{fig:framework} shows the key components of \framework. It consists
...
\begin{figure*}[h]
  \centering
  \includegraphics[width=0.7\linewidth,bb=0 13 568 158]{figure_framework.pdf}
  % \vspace{-24pt}
...
Fourth, we show in \fgb{Additional file 1:} Section S4 that compared to the most accurate state-of-the-art basecaller \hgb{(i.e., \gp)}, \mech provides \rmpSpeedupSUP speedup using 19.22$\times$ lower parameters at the expense of, on average, \rmpAccSUP lower accuracy.}}  Fifth, assemblies constructed
