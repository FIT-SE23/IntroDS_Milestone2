...
accuracy. \strim performs a skip removal process using knowledge distillation~\cite{bucilua2006model}, as shown in Supplementary Figure~\ref{fig:skiptrim} in Supplementary Section~\ref{supsec:skip}, where  we train a smaller network (\textit{student}) without skip connections to mimic a pre-trained larger network (\textit{teacher}) with skip connections. \gsss{Figure~\ref{fig:framework} shows the key components of \framework. It consists
...
Fourth, we show in Supplementary~\ref{suppsec:guppy_compare} that compared to the most accurate state-of-the-art basecaller \hgb{(i.e., \gp)}, \mech provides \rmpSpeedupSUP speedup using 19.22$\times$ lower parameters at the expense of, on average, \rmpAccSUP lower accuracy.}}  Fifth, assemblies constructed
