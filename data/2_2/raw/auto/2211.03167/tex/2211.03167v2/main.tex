...
\title{Radiation estimates of the Minkowski space: coupled Einstein-Yang-Mills perturbations}
\author{Puskar Mondal\fnref{fn1,fn2}}
\ead{puskar_mondal@fas.harvard.edu}
\author{Shing-Tung Yau\fnref{fn1,fn2}}
\ead{yau@math.harvard.edu}
\fntext[fn1]{Centre of Mathematical Sciences and Applications, Harvard University}
\fntext[fn2]{Department of Mathematics, Harvard University}
...
=\nabla_{e_{a}}e_{b}-\frac{1}{2}\langle D_{e_{a}}e_{b},e_{3}\rangle e_{4}-\frac{1}{2}\langle D_{e_{a}}e_{b},e_{4}\rangle e_{3}
\end{eqnarray}
simply because the basis $(e_{3},e_{4},e_{a})_{a=1}^{2}$ are not sections of the gauge bundle and therefore gauge covariant derivative acts as ordinary covariant derivative. Now recall the following definitions of the outgoing and incoming null second fundamental form of $S_{u,v}$
\begin{eqnarray}
...
D_{3}R_{44}-D_{4}R_{34}\sim\langle\alpha^{F},\hnabla_{3}\alpha^{F}\rangle+\underline{\omega}|\alpha^{F}|^{2}+\eta_{A}(\alpha^{F}\cdot\sigma^{F}+\alpha^{F}\cdot\rho^{F})+\langle\rho^{F},\hnabla_{4}\rho^{F}\rangle\\\nonumber+\langle\sigma^{F},\hnabla_{4}\sigma^{F}\rangle +\underline{\eta}(\rho^{F}\underline{\alpha}^{F}+\rho^{F}\cdot\sigma^{F}),\\
D_{b}R_{43}-D_{3}R_{4b}\sim\langle\rho^{F},\hnabla\rho^{F}\rangle+\langle\sigma^{F},\hnabla\sigma^{F}\rangle-\underline{\chi}\alpha^{F}(\rho^{F}+\sigma^{F})+\chi\underline{\alpha}^{F}(\rho^{F}+\sigma^{F}),\\
...
\label{eq:source2}
D_{b}R_{33}-D_{3}R_{3b}\sim \langle\underline{\alpha}^{F},\hnabla\underline{\alpha}^{F}\rangle-\underline{\chi} \underline{\alpha}^{F}(\rho^{F}+\sigma^{F})+\underline{\eta}|\underline{\alpha}^{F}|^{2}-2\underline{\omega} \underline{\alpha}^{F}(\rho^{F}+\sigma^{F})\\\nonumber-\hnabla_{3}(\underline{\alpha}^{F}\rho^{F}+\underline{\alpha}^{F}\sigma^{F}).
\end{eqnarray}
Note the most important point: the Yang-Mills curvatures appear in the source term for Einstein's equations in a gauge-invariant fashion.

\noindent Now we define the norm of the initial data (the Weyl curvature and the Yang-Mills curvature)
\noindent In this section,  Let us now write down the main theorem of the article. First, let us define the generalized Sobolev norm for the data on the initial slice that we shall consider \footnote{One could use one less derivative to close the argument. See \cite{puskar} where a characteristic initial value problem is solved in one order lower regularity} 
\begin{eqnarray}
...
\end{eqnarray}
The energy (the generalized Sobolev norm) that we shall control in the bulk $D_{u,\ubar}$ (i.e., the causal globally hyperbolic portion of the spacetime) is defined as follows 
\begin{eqnarray}
...
Let us now extend all the Sobolev and trace inequalities to the fibers of the vector bundle $\otimes^{k}T^{*}M\otimes _{M}\mathfrak{P}_{Ad,\mathfrak{g}}$. Let us write both the degenerate and the non-degenerate (no loss in decay along $\ubar$) versions \footnote{Notice that these inequalities are nothing but exhibitions of the scaling properties of the associated physical entities}. Assume $\sup_{D_{u,\ubar}}|\tr\chi-\frac{2}{r}|\leq \epsilon,~\sup_{D_{u,\ubar}}|\tr\underline{\chi}+\frac{2}{r}|\leq \epsilon$ for sufficiently small $\epsilon$.
\begin{lemma}
...
\begin{proposition}
Assume that the boot-strap assumption (remark 2) holds. The Ricci coefficient $\chibarhat$ verifies the following estimate 
\begin{eqnarray}
...
\begin{proposition}
Assume that the boot-strap assumption (remark 2) holds. The Ricci coefficients $\eta,\etabar$ verify the following estimate 
\begin{eqnarray}
...
\begin{proposition}
Assume that the boot-strap assumption (remark 2) holds. The Ricci coefficients verify the following estimate 
\begin{eqnarray}
...
\begin{proposition}
Assume that the boot-strap assumption (remark 2) holds. The Ricci coefficients verify the following second derivative estimates 
\begin{eqnarray}
...
\end{proof}
\begin{proposition}
The following third derivative estimates hold for the connection coefficients 
\begin{eqnarray}
...
\label{integration}
Let $f:M\to\mathbb
R$ is a gauge invariant object on the spacetime. The following integration by parts identities holds for $f$ \footnote{Note that instead of the canonical foliation and space-time harmonic gauge, if one were to choose a maximal slice, then it would be impossible to connect the leaves of the null foliation $H_{u}$ to that of the space-like maximal foliation. In such a case one would require an additional oscillation lemma.}
\begin{eqnarray}
