...
\begin{document}
	...
	\begin{figure*}[t]
		...
		\subfigure[The Calculation of Peak-First Regularization]{
			\centering
			\label{fig:pfloss}
			\includegraphics[width=0.58\linewidth]{pf-loss.pdf}}
		% \vspace{-5pt}
	...
	\begin{equation}
		\mathcal{L}_{PFR}=\sum_{t=2}^{T}\bm{p}^{t+1}\log\frac{\bm{p}^{t+1}}{\bm{p}^{t}}=\sum_{t=2}^{T}\sum_{k=1}^{V} p_{k}^{t+1}\log\frac{p_{k}^{t+1}}{p_{k}^{t}}
	\end{equation}
