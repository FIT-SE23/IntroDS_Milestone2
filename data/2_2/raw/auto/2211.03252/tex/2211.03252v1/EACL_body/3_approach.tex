...
This stage is responsible for detecting concepts mentioned in an explanation as well as recovering the logical structure on top of these concepts. Taking the explanation in Figure~\ref{fig:approach}(b) as an example, the goal is to get the tree structure as in Figure~\ref{fig:approach}(d). We divide this parsing into 2 steps:

\paragraph{Step 1: Selecting Concept Candidates} We deploy a concept detector to mark a list of concept candidates in the explanations in the form of token masks. First we encode the explanation sentence with a pre-trained language encoder, such as RoBERTa~\citep{liu2019roberta}. This outputs a sentence embedding vector and a sequence of token embedding vectors. Then we apply an attention-based Gated Recurrent Unit (GRU) network ~\citep{qiang2017hierarchical}. The sentence embedding vector is used as the initialization vector $h^0$ for GRU, and word embeddings are used as contextual inputs $\boldsymbol{x}^t_c$ to attend on. We run GRU for a maximum of $T$ steps, and get $T$ attention weight maps. We interpret these attention maps as word masks for concept candidates, and use the attention weights to sum over explanation word embeddings to obtain $T$ concept candidate embeddings $w_t$.

\paragraph{Step 2: Parsing Logical Structure} The goal of this step is to generate a logical structure over the concept candidates in the previous step. As shown in Figure~\ref{fig:approach}(d), the logical structure is a binary directed tree composed of logical operators \texttt{AND} or \texttt{OR}, and each leaf node corresponds to a concept candidate. In this work, we need to deal with the problem of undetermined number of concepts, and also allow for differentiable optimization. To this end, we define a fixed list of tree structures within maximum number of $T$ leaf nodes. Then we apply an multi-layer perceptron (MLP) with a final soft-max operator onto the explanation sentence embedding. The final layer width of this MLP is the same as number of templates we define. This provides a non-negative vector $\boldsymbol{p}$ with sum 1, which we interpret as a distribution over the logical structure templates. A list of the logical structure templates is shown in Appendix~\ref{appsec:templates}.


\subsection{Logical Reasoning}
After getting concept candidates and a distribution over logical structures, we conduct logical reasoning on the input to get the classification score. An illustration is provided in Figure~\ref{fig:approach}(e).
\paragraph{Step 1: Matching Concepts with Inputs}
 We assume that the input is represented as a sequence of feature vectors $X=(x_1,x_2,\cdots,x_K)$. First we define a matching score between concept embedding $w_t$ and input $X$ as the maximum cosine similarity:
\[
    \text{sim}(X,w_t) \colon= \max_k \cos(x_k,w_t).
\]

\paragraph{Step 2: Probabilisitc Logical Reasoning} For each logical tree template, we \textit{assume} that this is the correct logical structure, and walk along the tree bottom-up to get the intermediate reasoning scores node by node. First, for leaf nodes in the logical tree (which are associated with concepts), we use the concept-input matching scores in the previous step as their intermediate scores. Then, for a non-leaf node, if it is associated with an \texttt{AND} operator, we define its intermediate score as $\min(s_1,s_2)$ with $s_1$ and $s_2$ as intermediate scores of the children nodes. If the non-leaf node is associated with an \texttt{OR} operator instead, we use $\max(s_1,s_2)$ as the intermediate score. The intermediate score of the root node $s_{root}$ serves as  the reasoning score of this logical structure.
Note that we generated a \textit{distribution over logical structures} rather than a deterministic structure. Therefore, after acquiring the reasoning scores on each structure, we use the probability distribution weight $\boldsymbol{p}$ to sum over the scores for each structure $\boldsymbol{s}$. The resulting score is then equivalent to probabilistically logical reasoning over a distribution of logical structures.
\[
    s_{expl} = \boldsymbol{p}^\top \boldsymbol{s}
\]

We also consider that some explanations might be more or less certain than others. When using words like ``maybe'', the explanation is less certain than another explanation using word ``always''. We measure this effect by associating each explanation with a certainty value $c_{certainty}$, which is produced by another MLP on top of the explanation sentence embedding. So we scale the score $s_{expl}$ with $c_{certainty}$ in logit scale: 
\[
    s_{scaled} = \sigma(c_{certainty} \cdot \text{logit}(s_{expl}))
\]

\paragraph{Step 3: Reasoning over Multiple Explanations} There might be multiple explanations associated with a category. In this case, we take the maximum $s_{scaled}$ over the set of explanations as the classification score for this category.
