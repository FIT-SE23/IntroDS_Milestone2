        ...
        \else
            ...
            \else
                \ifcdr@efirst
                    ...
                    % \fi
                \else
                    \textbf{\currentyear}, \currentvolume\currentissue@G{, \no}, \prepageinfo\pageinfo
                \fi
            \fi
            \\\texttt{\printDOI}
        \fi
        \hfill\raisebox{-3mm}[0pt][0pt]{\includegraphics[height=16mm]{Logo_Academie_grisfonce.pdf}}%
...
\def\@settitle{%
    ...
    {\Huge\ifcdr@swaptitles\cdr@specialtitle@english\else
        \ifcdr@Attached Supplementary material: \fi\@title
    \fi\par}
    {\huge\itshape
        \ifx\@alttitle\@empty\else
            \vspace*{14pt}
            \ifcdr@swaptitles\cdr@specialtitle@french\else
                \ifcdr@Attached Document compl\'ementaire\,: \fi\@alttitle % TODO French spacing for lt title
            \fi
            \par
        \fi}
...
%
 \def\@setdate{%
   \ifx\@empty\@dateaccepted \MissingInfo{This article is a draft (not yet accepted!)}%
   \else
  \if@francais
...
\ifcdr@Thematic
    ...
    \def\@setdate{
        \ifx\@dateposted\@empty\else
            \ifcdr@efirst Online first, \else Available online \fi
            {\selectlanguage{english}\printdate{\@dateposted}}
        \par\fi
    }
\fi
%
...
\ExplSyntaxOn
\NewDocumentCommand {\currentissue@G} {m} {
    \exp_args:NV \str_if_in:nnF \currentissue {G} {#1\currentissue}
}
\ExplSyntaxOff
