...
\maketitle
\begin{abstract}
The number of Lame equations with finite (ordinary or projective) monodromy had been conjectured by S. R. Dahmen. It is known that Lame equations with unitary monodromy are corresponding to spherical tori with one conical singularity, and the geometry of such surfaces had been studied with triangulation recently. In this paper we will apply the results on spherical tori to prove Dahmen's conjectures.
\end{abstract}
...
It is known that all the finite monodromy groups of Lam\'e equations are cyclic. The following two conjectures are proposed by S. R. Dahmen \cite{S1}\cite{S2}.
...
Note that in Dahmen's paper, the conjecture was originally stated for pushforward Lam\'e equation on $\mathbb{CP}^1$, thus the finite monodromy groups they concerned becomes the dihedral groups $D_N$ instead of $C_N$.\\

Up to $N\leq 4$ these two conjectures were verified by Y.C. Chou via modular form calculations (\cite{MF2}, appendix). In this paper we give a complete proof of these conjectures from the perspective of spherical tori, developed by A. Eremenko et al.\ \cite{E}. It should be mentioned that C.S. Lin et al.\ have also announced a proof of Dahmen's conjectures \cite{PE}, which involves more technical analyses on Painlev\'e equations.\\
...
This paper is adapted from part of my 2021 master thesis \cite{TH}. I would like to appreciate Prof. Wang for his guidance on this vast topic. Thanks to Prof. Hui-Wen Lin for encouraging me to join this project. Thanks also to You-Cheng Chou, Hung-Hsun Yu and Ting-Wei Chao for discussing and reviewing my paper. I am also grateful to my family for their financial and mental support.
...
\proof Note that for any fixed $(2s,2t)$, there are $4$ choices of $(s,t)$, so we ought to determine for each $\theta_1,\theta_2,\theta_3$ which of the $8$ regions in Figure 4 $(s,t)$ should lies in. For the case $\theta_1=\theta_2=\theta_3=1$, $(s,t)$ must lies in the region we have described, due to the $\mathbb Z_3$-symmetricity on cyclically permuting $P_i$. For $n>1$, note that attaching two hemispheres on the side of $L_2$ or $L_3$ will translate the parameter $s \mathrm{(mod}\ 1)$ by $\frac{1}{2}$, so in total $s$ is translated by $\frac{\theta_1-1}{2}$. A similar argument holds for $t$. \qedhere\\
...
Now we are ready to prove Dahmen's conjectures.\\
...
\[\sum_{d|N}(3L_n(d)-2\epsilon(n,d))=\dfrac{n(n+1)}{2}(d^2-3d+2).\]
The formula (\ref{fin_proj_mono}) is then following from M\"obius inversion.\\
...
a_n\dfrac{3(m-1)(m-2)}{2}+(b_n-a_n)\dfrac{m(m-1)}{2} \qquad\textnormal{if }d=2m-1,\\
a_n\dfrac{3(m-1)(m-2)}{2}+(b_n-a_n)\dfrac{(m-1)(m-2)}{2} \qquad\textnormal{if }d=2m,\end{cases}
\\&=\begin{cases}
\dfrac{n(n+1)}{2}\dfrac{m(m-1)}{2}-3a_nm\qquad\textnormal{if }d=2m-1,\\
\dfrac{n(n+1)}{2}\dfrac{m^2}{2}-(2a_n+b_n)\dfrac{3m-2}{2}\qquad\textnormal{if }d=2m,
\end{cases}
\\&=\begin{cases}
\dfrac{n(n+1)}{16}(d^2-1)-\dfrac{3}{2} a_n (d+1) \qquad\textnormal{if }d=2m-1,\\
\dfrac{n(n+1)}{16}d^2-(2a_n+b_n)(\dfrac{3d}{4}-1) \qquad\textnormal{if }d=2m.\end{cases}
\end{align*}
The formula (\ref{fin_ordi_mono}) is then following from M\"obius inversion. The proof is complete. \qedhere\\
