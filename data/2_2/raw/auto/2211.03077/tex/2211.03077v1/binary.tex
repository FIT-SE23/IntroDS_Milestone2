...
Section \ref{sec:greedy-binary} examines the special case of binary values: an agent $i$'s value for an item $t$ is either $v_{it} = 0$, or some value $v_{it} = v_t$ that is the same for all agents.
...
\begin{proof}
    For any fixed $1 \le i \le N$ in the lemma statement, let $S_i$ be the set of items for which at least one of agents $1$ to $i$ have positive values, i.e.:
    %
    \[
...
We defer this relatively standard proof to Appendix~\ref{app:impartial}.
...
We will relax the denominators on the right-hand side to be $N$ in the sequel analysis.
We get that:
%
\begin{equation}
...
\begin{proof}
    ...
    We may further assume $a_1 = 1$ because the left-hand-side remains same if $a_1, a_2, \dots, a_N$ are multiplied by the same factor.
    ...
    For ease of exposition, in the remaining argument we artificially define $i_0 = 1, a_{i_0}=1$.
