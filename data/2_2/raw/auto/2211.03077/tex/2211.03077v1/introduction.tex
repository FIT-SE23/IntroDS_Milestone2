...
From now on, we will more generally talk about items and agents rather than the candies and children in the above toy example.
...
\begin{table}[t]
    ...
    \begin{tabular}{p{4cm}p{5cm}p{4cm}}
        ...
        $(\alpha, \beta)$-Predictions & $\alpha  \log \beta N ~ \text{\cite{BanerjeeGGJ:SODA:2022}} ~\to~ \log \alpha \beta N$ &  \\
        \bottomrule
    \end{tabular}
...
Subsequently, \citet{AnariOSS:ITCS:2017} applied the theory of stable polynomials to get an improved $e$-approximation algorithm.
Finally, \citet{ColeDGJMVY:EC:2017} built on convex program duality to design the state-of-the-art $2$-approximation algorithm.
...
\citet{AnariMOV:SODA:2018} introduced a constant-approximation algorithm for the separable, piecewise-linear concave utilities.
Last but not least, \citet*{GargKK:SODA:2020} proposed algorithms that are $O(N)$ and $O(N \log N)$-approximations for submodular utilities.
...
However, they assumed that any agent's values for different items can only differ by a bounded ratio;
...
Their assumption rules out the possibility that an agent may have zero value for some item, and therefore their results do not apply to our model.
