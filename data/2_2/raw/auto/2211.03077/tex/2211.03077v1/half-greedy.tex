...
Section~\ref{sec:half-greedy-bounded} introduces our algorithm under an additional assumption that we were given an upper bound of the balance ratio.
...
\begin{algorithm}[t]
    \caption{\textbf{Half-and-Half} (when we know that the instance is $\lambda$-balanced)}
...
\begin{proof}[Proof of Theorem~\ref{thm:half-greedy-lambda}]
    ...
    First, items $t^*$ to $T$'s contribution to any agent $i$'s utility in the optimal solution is at least $\frac{1}{2} u_i^*$.
    In other words, even if we had made completely wrong allocations and had gotten zero utilities for all agents so far, we would have lost at worst half of the Nash welfare.
...
\begin{algorithm}[t]
    \caption{\textbf{Half-and-Half} (when we have no prior knowledge of the balance ratio)}
