...
\label{sec:method}
\vspace{-0.1cm}
Inspired by previous work~\cite{kreuk2022audiogen}, the proposed system has three main components: (i) an audio encoder-decoder which encodes and decodes audio to and from a discrete representation; (ii) a pre-trained image encoder; and (iii) an audio language model which operates over the discrete audio tokens. 
