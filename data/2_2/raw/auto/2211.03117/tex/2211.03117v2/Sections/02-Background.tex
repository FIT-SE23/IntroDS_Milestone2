...
% \label{sub.aml}
AML studies threats that undermine ML applications like evasion, poisoning, and backdoor attacks.
...
Data manipulation is a concrete threat because building datasets often involves untrusted sources, and complex models training is outsourced to third parties. 
...
When the attack is dirty-label, the volunteer can easily spot inconsistencies between the provided audio and its transcription. However, in a clean-label attack, the trigger will remain unnoticed 
because the audio and its transcription will be correct. 
% Volunteers can easily spot and fix inconsistencies between audio and its transcription (dirty label), while this cannot happen if labels are correct (clean label).
In this work, we explore the effects of both approaches on a style-based backdoor attack in speech classification.
