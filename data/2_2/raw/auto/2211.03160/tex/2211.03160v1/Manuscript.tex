...
The iteration steps are fixed as 12000 and 6000, respectively, for the 2D and 3D simulations.
...
\begin{figure}[!h]
    ...
    \caption{Perform of (\textit{a}, \textit{c}) the 2D simulation and (\textit{b}, \textit{d}) the 3D simulation, in terms of (\textit{a}, \textit{b}) the MLUPS and (\textit{c}, \textit{d}) the parallel efficiency.}
    \label{fig:mono}
\end{figure}
...
\begin{figure}[!h]
    ...
    \caption{Perform comparisons between mono-dimensional and block partitioning of the computational domain for  (\textit{a}, \textit{c}) the 2D simulation and (\textit{b}, \textit{d}) the 3D simulation, in terms of (\textit{a}, \textit{b}) the MLUPS and (\textit{c}, \textit{d}) the parallel efficiency.}
    \label{fig:block}
\end{figure}
...
Previously, using the overlapping mode implemented in a hybrid CUDA and MPI approach, Wang and Takayaki \cite{xian2011multi} reported the MLUPS increased by 1.24X with 16 GPUs,
...
\begin{figure}[!h]
    ...
    \caption{Perform comparisons between non-overlapping and overlapping communication with computation for (\textit{a}, \textit{c}) the 2D simulation and (\textit{b}, \textit{d}) the 3D simulation, in terms of (\textit{a}, \textit{b}) the MLUPS and (\textit{c}, \textit{d}) the parallel efficiency.}
    \label{fig:overlap}
\end{figure}
...
\begin{figure}[!h]
    ...
    \caption{Perform comparisons between non-concurrent and concurrent computation for (\textit{a}, \textit{c}) the 2D simulation and (\textit{b}, \textit{d}) the 3D simulation, in terms of (\textit{a}, \textit{b}) the MLUPS and (\textit{c}, \textit{d}) the parallel efficiency.}
...
\section*{Appendix: Comparison of the parallel performance on CPUs versus GPUs}
