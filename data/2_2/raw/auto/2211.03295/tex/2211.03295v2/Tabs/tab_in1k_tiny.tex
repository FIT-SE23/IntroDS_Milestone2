\begin{table}[b]
    \vspace{-0.25em}
    \setlength{\tabcolsep}{0.3mm}
    \centering
\resizebox{\linewidth}{!}{
\begin{tabular}{llccccc}
    \toprule
    Architecture                                     & Date         & Type & Image   & Param. & FLOPs & Top-1     \\
                                                     &              &      & Size    & (M)    & (G)   & Acc (\%)  \\ \hline
    ResNet-18$^\dag$~\cite{he2016deep}               & CVPR'2016    & C    & $224^2$ & 11.7   & 1.80  & 71.5      \\
    ShuffleNetV2~$2\times$~\cite{eccv2018shufflenet} & ECCV'2018    & C    & $224^2$ & 5.5    & 0.60  & 75.4      \\
    EfficientNet-B0~\cite{icml2019efficientnet}      & ICML'2019    & C    & $224^2$ & 5.3    & 0.39  & 77.1      \\
    MobileNetV3~$1\times$~\cite{iccv2019mobilenetv3} & ICCV'2019    & C    & $224^2$ & 5.4    & 0.23  & 75.2      \\
    % RegNetY-400MF~\cite{cvpr2020regnet}              & CVPR'2020    & C    & $224^2$ & 5.3    & 0.40  & 74.1      \\
    RegNetY-800MF~\cite{cvpr2020regnet}              & CVPR'2020    & C    & $224^2$ & 6.3    & 0.80  & 76.3      \\
    DeiT-T$^\dag$~\cite{icml2021deit}                & ICML'2021    & T    & $224^2$ & 5.7    & 1.08  & 74.1      \\
    % DeiT-2G~\cite{icml2021deit}                      & ICML'2021    & T    & $224^2$ & 13.2   & 1.90  & 75.1      \\
    PVT-T~\cite{iccv2021PVT}                         & ICCV'2021    & T    & $224^2$ & 13.2   & 1.60  & 75.1      \\
    T2T-ViT-7~\cite{iccv2021t2t}                     & ICCV'2021    & T    & $224^2$ & 4.3    & 1.20  & 71.7      \\
    % T2T-ViT-12~\cite{iccv2021t2t}                    & ICCV'2021    & T    & $224^2$ & 6.9    & 1.80  & 76.5      \\
    ViT-C~\cite{nips2021vitc}                        & NIPS'2021    & T    & $224^2$ & 4.6    & 1.10  & 75.3      \\
    SReT-T$_{Distill}$~\cite{eccv2022SReT}           & ECCV'2022    & T    & $224^2$ & 4.8    & 1.10  & 77.6      \\
    PiT-Ti~\cite{iccv2021pit}                        & ICCV'2021    & H    & $224^2$ & 4.9    & 0.70  & 74.6      \\
    LeViT-S~\cite{iccv2021levit}                     & ICCV'2021    & H    & $224^2$ & 7.8    & 0.31  & 76.6      \\
    CoaT-Lite-T~\cite{iccv2021coat}                  & ICCV'2021    & H    & $224^2$ & 5.7    & 1.60  & 77.5      \\
    Swin-1G~\cite{liu2021swin}                       & ICCV'2021    & H    & $224^2$ & 7.3    & 1.00  & 77.3      \\
    % Swin-2G~\cite{liu2021swin}                       & ICCV'2021    & H    & $224^2$ & 12.8   & 2.00  & 79.3      \\
    % MobileViT-XS~\cite{iclr2022mobilevit}            & ICLR'2022    & H    & $256^2$ & 2.3    & 1.73  & 74.8      \\
    MobileViT-S~\cite{iclr2022mobilevit}             & ICLR'2022    & H    & $256^2$ & 5.6    & 4.02  & 78.4      \\
    % MobileFormer-151M~\cite{cvpr2022MobileFormer}    & CVPR'2022    & H    & $224^2$ & 7.6    & 0.29  & 75.2      \\
    MobileFormer-294M~\cite{cvpr2022MobileFormer}    & CVPR'2022    & H    & $224^2$ & 11.4   & 0.59  & 77.9      \\
    ConvNext-XT~\cite{cvpr2022convnext}              & CVPR'2022    & C    & $224^2$ & 7.4    & 0.60  & 77.5      \\
    VAN-B0~\cite{guo2022van}                         & arXiv'2022   & C    & $224^2$ & 4.1    & 0.88  & 75.4      \\
    ParC-Net-S~\cite{eccv2022edgeformer}             & ECCV'2022    & C    & $256^2$ & 5.0    & 3.48  & 78.6      \\
    \rowcolor{gray94}\bf{MogaNet-XT}                 & Ours         & C    & $256^2$ & 3.0    & 1.04  & 77.2      \\
    \rowcolor{gray94}\bf{MogaNet-T}                  & Ours         & C    & $224^2$ & 5.2    & 1.10  & 79.0      \\
    \rowcolor{gray94}\bf{MogaNet-T}$^\S$             & Ours         & C    & $256^2$ & 5.2    & 1.44  & \bf{80.0} \\
    \bottomrule
    \end{tabular}
    }
    \vspace{-0.5em}
    \caption{\textbf{Image classification} of lightweight ($\sim$5M parameters) models on IN-1K validation set. \small{$\dag$} and \small{$\S$} denote using RSB A2 \cite{wightman2021rsb} training scheme and the refined setting.}
    \label{tab:in1k_cls_tiny}
    \vspace{-0.5em}
\end{table}

% \begin{table}[h]
%     \vspace{-0.5em}
%     \setlength{\tabcolsep}{0.7mm}
%     \centering
% \resizebox{\linewidth}{!}{
% \begin{tabular}{llcccc}
%     \toprule
%     Architecture                                     & Date         & Image   & Param. & FLOPs & Top-1     \\
%                                                      &              & Size    & (M)    & (G)   & Acc (\%)  \\ \hline
%     ResNet-18$^\dag$~\cite{he2016deep}               & CVPR'2016    & $224^2$ & 11.7   & 1.80  & 71.5      \\
%     ShuffleNetV2~$2\times$~\cite{eccv2018shufflenet} & ECCV'2018    & $224^2$ & 5.5    & 0.60  & 75.4      \\
%     EfficientNet-B0~\cite{icml2019efficientnet}      & ICML'2019    & $224^2$ & 5.3    & 0.39  & 77.1      \\
%     MobileNetV3~$1\times$~\cite{iccv2019mobilenetv3} & ICCV'2019    & $224^2$ & 5.4    & 0.23  & 75.2      \\
%     RegNetY-800M~\cite{cvpr2020regnet}               & CVPR'2020    & $224^2$ & 6.3    & 0.80  & 76.3      \\ \hline
%     DeiT-T~\cite{icml2021deit}                     & ICML'201     & $224^2$ & 5.7    & 1.08  & 72.2      \\
%     PVT-T~\cite{iccv2021PVT}                       & ICCV'2021    & $224^2$ & 13.2   & 1.60  & 75.1      \\
%     T2T-ViT-7~\cite{iccv2021t2t}                   & ICCV'2021    & $224^2$ & 4.3    & 1.20  & 71.7      \\
%     T2T-ViT-12~\cite{iccv2021t2t}                  & ICCV'2021    & $224^2$ & 6.9    & 1.80  & 76.5      \\
%     ViT-C~\cite{nips2021vitc}                      & NIPS'2021    & $224^2$ & 4.6    & 1.10  & 75.3      \\ \hline
%     PiT-Ti~\cite{iccv2021pit}                      & ICCV'2021    & $224^2$ & 4.9    & 0.70  & 74.6      \\
%     LeViT-S~\cite{iccv2021levit}                   & ICCV'2021    & $224^2$ & 7.8    & 0.31  & 76.6      \\
%     CoaT-Lite-T~\cite{iccv2021coat}                & ICCV'2021    & $224^2$ & 5.7    & 1.60  & 77.5      \\
%     MobileViT-XS~\cite{iclr2022mobilevit}          & ICLR'2022    & $256^2$ & 2.3    & 1.73  & 74.8      \\
%     MobileViT-S~\cite{iclr2022mobilevit}           & ICLR'2022    & $256^2$ & 5.6    & 4.02  & 78.4      \\
%     Mobile-Former-151M~\cite{cvpr2022MobileFormer} & CVPR'2022    & $224^2$ & 7.6    & 0.29  & 75.2      \\
%     Mobile-Former-294M~\cite{cvpr2022MobileFormer} & CVPR'2022    & $224^2$ & 11.4   & 0.59  & 77.9      \\ \hline
%     ConvNext-XT~\cite{cvpr2022convnext}            & CVPR'2022    & $224^2$ & 7.4    & 0.60  & 77.5      \\
%     VAN-B0~\cite{guo2022van}                       & arXiv'2022   & $224^2$ & 4.1    & 0.88  & 75.4      \\
%     ParC-Net-S~\cite{eccv2022edgeformer}           & ECCV'2022    & $256^2$ & 5.0    & 3.48  & 78.6      \\
%     \rowcolor{gray94}\bf{MogaNet-XT}               & Ours         & $224^2$ & 3.0    & 0.80  & 76.3      \\
%     \rowcolor{gray94}\bf{MogaNet-T}                & Ours         & $224^2$ & 5.2    & 1.10  & 79.0      \\
%     \rowcolor{gray94}\bf{MogaNet-T}                & Ours         & $256^2$ & 5.2    & 1.44  & \bf{79.6} \\
%     \bottomrule
%     \end{tabular}
%     }
%     \vspace{-0.5em}
%     \caption{ImageNet-1K classification performance of lightweight (around 5M Parameters) models.}
%     \label{tab:in1k_cls_tiny}
%     \vspace{-0.75em}
% \end{table}
