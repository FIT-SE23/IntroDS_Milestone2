...
\begin{proposition}\label{prop:c-range}
    If the cumulant generating function of $f(X_T)$, $K_{f(X_T)}(s) := \log \E \left[ e^{sf(X_T)} \right]$, satisfies $K_{f(X_T)}(r) < \infty$ for some $r > 0$, then for all 
    \begin{equation}
       c \in \left( \frac{\E\left[ e^{-rf(X_T)} f(X_T)\right]}{\E\left[ e^{-rf(X_T)}\right]},\frac{\E\left[ e^{rf(X_T)} f(X_T)\right]}{\E\left[ e^{rf(X_T)}\right]} \right) ,
       \label{eq:c-interval}
    \end{equation}
...
\begin{proof}
Let $c$ be in the interval given in Equation \eqref{eq:c-interval} and assume there exists an $r>0$ such that the cumulant generating function of $f(X_T)$, $K_{f(X_T)}(s)$, satisfies $K_{f(X_T)}(r) < \infty$. The latter implies, by properties of the cumulant generating function, that $K_{f(X_T)}(s) < \infty$ for all $s \in (-r,r)$. As $K_{f(X_T)}(s)$ is convex and twice differentiable, its derivative, $\frac{d}{ds}K_{f(X_T)}(s)$, exists and is continuously increasing on $(-r,r)$. We observe that 
    \begin{equation*}
       \frac{d}{ds}K_{f(X_T)}(s) \Big |_{s = - r} = \frac{\E\left[ e^{-rf(X_T)} f(X_T)\right]}{\E\left[ e^{-rf(X_T)}\right]}  \quad \text{and} \quad \frac{d}{ds}K_{f(X_T)}(s) \Big |_{s = r} = \frac{\E\left[ e^{rf(X_T)} f(X_T)\right]}{\E\left[ e^{rf(X_T)}\right]} \, .
    \end{equation*}
    Thus $\frac{d}{ds}K_{f(X_T)}(s)$ must take on all intermediate values between $\E\left[ e^{-rf(X_T)} f(X_T)\right]/\E\left[ e^{-rf(X_T)}\right]$ and \\$\E\left[ e^{rf(X_T)} f(X_T)\right]/\E\left[ e^{rf(X_T)}\right]$. Hence 
    there must exist an $\eta^* \in (-r,r)$ such that 
    \begin{equation}
    \label{eq:c-deriv}
...
\begin{figure}[htb!]
\centering
\subfloat[Paths $X_t, \, t \in {[0,1]}$ under $\Q^*$]{\includegraphics[width=8.5cm]{figures/VaR_paths_updated.pdf}}
\subfloat[Stressed intensity process $\kappa^*(t,x)$]{\includegraphics[width=7.4375cm]{figures/VaR_kappa_updated.pdf}}
...
\begin{figure}[htb!]
\centering
\subfloat[Paths $X_t, \, t \in {[0,1]}$ under $\Q^*$]{\includegraphics[width=8.5cm]{figures/CVaR_paths_updated.pdf}}
\subfloat[Stressed intensity process $\kappa^*(t,x)$]{\includegraphics[width=7.4375cm]{figures/CVaR_kappa_updated.pdf}}
